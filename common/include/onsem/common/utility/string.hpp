...
public:
     ...
     */
    Replacer(bool pIsCaseSensitive, bool pHaveSeparatorBetweenWords);
...
std::string join(const std::vector<std::string>& pStrs, const std::string& pSeparator);
...
template<typename STRING_CONTAINER>
std::string join(const STRING_CONTAINER& pStrs, const std::string& pSeparator) {
    std::string res;
