...
inline static bool beginWithUppercase(const std::string& pStr, std::size_t pBegin = 0) {
...
// /!\ If pBegin is greater or equal to the size of pStr, it causes undefined behavior.
inline static bool beginWithLowerCase(const std::string& pStr, std::size_t pBegin = 0) {
    assert(pBegin < pStr.size());
    if (pStr[pBegin] >= 'a' && pStr[pBegin] <= 'z')
        return true;
    switch (pStr[pBegin]) {
        case "é"[0]: {
            if (pStr.compare(pBegin, _aGrave_size, "à") == 0 || pStr.compare(pBegin, _eAcute_size, "é") == 0
                || pStr.compare(pBegin, _eCirconflex_size, "ê") == 0 || pStr.compare(pBegin, _cCedilla_size, "ç") == 0
...
bool lowerCaseFirstLetter(std::string& pStrToModify, std::size_t pPos = 0);

bool lowerCaseText(std::string& pStrToModify, std::size_t pPos = 0);
...
inline bool isFirstLetterAVowel(const std::string& pText, std::size_t pPos = 0) {
    if (pText[pPos] >= 'a' && pText[pPos] <= 'z') {
        return pText[pPos] == 'a' || pText[pPos] == 'e' || pText[pPos] == 'i' || pText[pPos] == 'o'
            || pText[pPos] == 'u' || pText[pPos] == 'y';
    }
    ...
    if (pText[pPos] == "É"[0]) {
        return pText.compare(pPos, _capitalAGrave_size, "À") == 0 || pText.compare(pPos, _capitalEAcute_size, "É") == 0
            || pText.compare(pPos, _capitalECirconflex_size, "Ê") == 0
            || pText.compare(pPos, _capitalECirconflex_size, "Î") == 0
            || pText.compare(pPos, _capitalOTrema_size, "Ö") == 0 || pText.compare(pPos, _aGrave_size, "à") == 0
            || pText.compare(pPos, _eAcute_size, "é") == 0 || pText.compare(pPos, _eCirconflex_size, "ê") == 0
            || pText.compare(pPos, _iCirconflex_size, "î") == 0 || pText.compare(pPos, _oTrema_size, "ö") == 0;
    }
    return false;
}
