...
struct vector_map {
    ...
    bool upper_bound_index(std::size_t& pIndex, const KEY_TYPE& pKey) const;
    ...
    bool find_index(std::size_t& pIndex, const KEY_TYPE& pKey) const;
...
private:
    ...
    template<typename... Args>
    VALUE_TYPE& _get(const KEY_TYPE& pKey, Args&&... pArgs);
};
...
struct vector_map_iterator {
    ...
    vector_map_iterator(MAP_TYPE& pMap, std::size_t pIndex, PAIR_TYPE& pPair)
        : _mapPtr(&pMap)
        , _index(pIndex)
    ...
    bool operator==(const vector_map_iterator& pOther) const {
        return _mapPtr == pOther._mapPtr && _index == pOther._index && _valuePtr == pOther._valuePtr
            && (_valuePtr == nullptr || _reference->first == pOther._reference->first);
    }

    bool operator!=(const vector_map_iterator& pOther) const { return !operator==(pOther); }
...
template<typename KEY_TYPE, typename VALUE_TYPE>
bool vector_map<KEY_TYPE, VALUE_TYPE>::find_index(std::size_t& pIndex, const KEY_TYPE& pKey) const {
...
template<typename KEY_TYPE, typename VALUE_TYPE>
template<typename... Args>
VALUE_TYPE& vector_map<KEY_TYPE, VALUE_TYPE>::_get(const KEY_TYPE& pKey, Args&&... pArgs) {
    std::size_t index = 0;
    if (find_index(index, pKey))
        return _content[index].second;
    _content.insert(_content.begin() + index,
                    std::pair<const KEY_TYPE, VALUE_TYPE>(pKey, VALUE_TYPE(std::forward<Args>(pArgs)...)));
    return _content[index].second;
}

template<typename KEY_TYPE, typename VALUE_TYPE>
VALUE_TYPE& vector_map<KEY_TYPE, VALUE_TYPE>::operator[](const KEY_TYPE& pKey) {
    return _get(pKey);
}

template<typename KEY_TYPE, typename VALUE_TYPE>
template<typename... Args>
void vector_map<KEY_TYPE, VALUE_TYPE>::emplace(const KEY_TYPE& pKey, Args&&... pArgs) {
    _get(pKey, std::forward<Args>(pArgs)...);
}

template<typename KEY_TYPE, typename VALUE_TYPE>
void vector_map<KEY_TYPE, VALUE_TYPE>::emplace(const std::pair<const KEY_TYPE, VALUE_TYPE&>& pPair) {
    _get(pPair.first, pPair.second);
}

template<typename KEY_TYPE, typename VALUE_TYPE>
void vector_map<KEY_TYPE, VALUE_TYPE>::emplace(const std::pair<const KEY_TYPE, const VALUE_TYPE&>& pPair) {
    _get(pPair.first, pPair.second);
}
...
template<typename KEY_TYPE, typename VALUE_TYPE>
typename vector_map<KEY_TYPE, VALUE_TYPE>::iterator vector_map<KEY_TYPE, VALUE_TYPE>::upper_bound(
    const KEY_TYPE& pKey) {
    std::size_t index = 0;
...
template<typename KEY_TYPE, typename VALUE_TYPE>
typename vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator vector_map<KEY_TYPE, VALUE_TYPE>::find(
...
template<typename KEY_TYPE, typename VALUE_TYPE>
typename vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator vector_map<KEY_TYPE, VALUE_TYPE>::upper_bound(
    const KEY_TYPE& pKey) const {
    std::size_t index = 0;
    if (upper_bound_index(index, pKey))
        return vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator(*this, index, _content[index]);
...
template<typename KEY_TYPE, typename VALUE_TYPE>
bool vector_map<KEY_TYPE, VALUE_TYPE>::upper_bound_index(std::size_t& pIndex, const KEY_TYPE& pKey) const {
...
template<typename KEY_TYPE, typename VALUE_TYPE>
typename vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator vector_map<KEY_TYPE, VALUE_TYPE>::next(
    std::size_t pIndex) const {
    ++pIndex;
    if (_content.size() > pIndex)
...
template<typename KEY_TYPE, typename VALUE_TYPE>
typename vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator vector_map<KEY_TYPE, VALUE_TYPE>::prev(
    std::size_t pIndex) const {
    if (pIndex > 0) {
        --pIndex;
        return vector_map<KEY_TYPE, VALUE_TYPE>::const_iterator(*this, pIndex, _content[pIndex]);
