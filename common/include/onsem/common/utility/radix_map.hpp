...
template<typename KEY_TYPE,
         typename KEY_CONTAINER_TYPE,
         typename VALUE_TYPE,
         typename VALUE_TYPE_OF_REF,
         typename MAP_TYPE>
...
struct radix_map {
    ...
    typedef radix_map_iterator<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE, const VALUE_TYPE, const radix_map>
        const_iterator;

    template<typename... Args>
    void emplace(const KEY_CONTAINER_TYPE& pKey, Args&&... pArgs);
    ...
    const VALUE_TYPE* find_ptr(const KEY_CONTAINER_TYPE& pKeys, std::size_t pBegin, std::size_t pSize) const;
    ...
    std::size_t getMaxLength(const std::string& pKeys, const std::size_t pKeys_begin_in) const;
...
private:
    ...
    static VALUE_TYPE* _begin(KEY_CONTAINER_TYPE& pKeys,
                              const radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>& pMap);

    static VALUE_TYPE* _beforeEnd(KEY_CONTAINER_TYPE& pKeys,
                                  const radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>& pMap);
    ...
    template<typename VALUE_TYPE_OF_REF, typename MAP_TYPE>
    static radix_map_iterator<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE, VALUE_TYPE_OF_REF, MAP_TYPE>
        _next(MAP_TYPE& pMap, MAP_TYPE* pCurrNode, const KEY_CONTAINER_TYPE& pKeyPrefix, std::size_t pChildIndex);

    template<typename VALUE_TYPE_OF_REF, typename MAP_TYPE>
    static radix_map_iterator<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE, VALUE_TYPE_OF_REF, MAP_TYPE>
        _advance(MAP_TYPE& pMap, const KEY_CONTAINER_TYPE& pKeys, SearchEndingPoint pSearchEndingPoint);
...
struct radix_map_iterator {
    ...
    radix_map_iterator(MAP_TYPE& pMap, const KEY_CONTAINER_TYPE& pKeyContainer, VALUE_TYPE_OF_REF& pValue)
        : _mapPtr(&pMap)
        , _valuePtr(&pValue)
    ...
    bool operator==(const radix_map_iterator& pOther) const {
        return _mapPtr == pOther._mapPtr && _valuePtr == pOther._valuePtr
            && (_valuePtr == nullptr || _reference->first == pOther._reference->first);
    }

    bool operator!=(const radix_map_iterator& pOther) const { return !operator==(pOther); }
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::radix_map(
    std::initializer_list<std::pair<const KEY_CONTAINER_TYPE, VALUE_TYPE>> pInitList) {
    for (const auto& e : pInitList)
        operator[](e.first) = e.second;
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>& radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::operator=(
    radix_map&& pOther) {
    _keys = std::move(pOther._keys);
    _children = std::move(pOther._children);
    _value_ptr = std::move(pOther._value_ptr);
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
template<typename VALUE_TYPE_MAYBE_CONST, typename MAP_TYPE>
VALUE_TYPE_MAYBE_CONST* radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::_find(MAP_TYPE& pMap,
                                                                                   const KEY_CONTAINER_TYPE& pKeys,
                                                                                   std::size_t pKeys_begin,
                                                                                   std::size_t pKeys_size) {
    auto* currNode = &pMap;
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
template<typename VALUE_TYPE_OF_REF, typename MAP_TYPE>
radix_map_iterator<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE, VALUE_TYPE_OF_REF, MAP_TYPE>
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::_next(MAP_TYPE& pMap,
                                                               MAP_TYPE* pCurrNode,
...
template<typename VALUE_TYPE_OF_REF, typename MAP_TYPE>
radix_map_iterator<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE, VALUE_TYPE_OF_REF, MAP_TYPE>
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::_advance(MAP_TYPE& pMap,
                                                                  const KEY_CONTAINER_TYPE& pKeys,
    ...
    while (true) {
        ...
        if (keys_size <= pKeys_size
                    ...
                    switch (pSearchEndingPoint) {
                        case SearchEndingPoint::PREV: {
                            while (!parentNodes.empty()) {
                                ...
                                if (parentNode.second > 0) {
                                    ...
                                    if (valuePtr != nullptr)
                                        return radix_map_iterator<KEY_TYPE,
                                                                  KEY_CONTAINER_TYPE,
                                                                  VALUE_TYPE,
                                                                  VALUE_TYPE_OF_REF,
                                                                  MAP_TYPE>(pMap, prevKeys, *valuePtr);
                                    break;
                                }
                        ...
                        case SearchEndingPoint::EQUAL_OR_NEXT:
                            return radix_map_iterator<KEY_TYPE,
                                                      ...
                                                      VALUE_TYPE,
                                                      VALUE_TYPE_OF_REF,
                                                      MAP_TYPE>(pMap, pKeys, *currNode->_value_ptr);
                        case SearchEndingPoint::NEXT: break;
                    };
                }
            } else {
                // so keys_size < pKeys_size
            ...
            if (pKeys_size == 0
                || (pKeys_size < keys_size
                    && memcmp(currNode->_keys.data(), pKeys.data() + pKeys_begin, pKeys_size) == 0)) {
                return goOnNext();
            } else {
                std::size_t i = 0;
    ...
    while (true) {
...

        {
            ...
            if (lastPosNotInCommon < keys_size)
                oldNode._keys.insert(
                    oldNode._keys.end(), currNode->_keys.begin() + lastPosNotInCommon + 1, currNode->_keys.end());
        ...
        if (lastPosNotInCommon < keys_size) {
            ...
            if (pKeys_size > lastPosNotInCommon) {
                ...
                newNode._keys.insert(
                    newNode._keys.end(), pKeys.begin() + pKeys_begin + lastPosNotInCommon + 1, pKeys.end());
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
VALUE_TYPE& radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::operator[](const KEY_CONTAINER_TYPE& pKeys) {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
template<typename... Args>
void radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::emplace(const KEY_CONTAINER_TYPE& pKeys, Args&&... pArgs) {
    auto& valPtr = _get(pKeys);
    if (!valPtr)
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
std::pair<typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator, bool>
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::insert(const KEY_CONTAINER_TYPE& pKeys) {
    bool inserted = false;
    auto& valPtr = _get(pKeys);
    if (!valPtr) {
        valPtr = std::make_unique<VALUE_TYPE>();
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::begin() {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::beforeEnd() {
    KEY_CONTAINER_TYPE keys;
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::end() {
    return radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator(*this);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::cbegin() const {
    KEY_CONTAINER_TYPE keys;
    auto* valPtr = _begin(keys, *this);
    if (valPtr != nullptr)
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::cend() const {
    return radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator(*this);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::begin() const {
    KEY_CONTAINER_TYPE keys;
    auto* valPtr = _begin(keys, *this);
    if (valPtr != nullptr)
        return radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator(*this, keys, *valPtr);
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::beforeEnd() const {
    KEY_CONTAINER_TYPE keys;
    auto* valPtr = _beforeEnd(keys, *this);
    if (valPtr != nullptr)
        return radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator(*this, keys, *valPtr);
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::end() const {
    return radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator(*this);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::find(const KEY_CONTAINER_TYPE& pKeys) {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::find(const KEY_CONTAINER_TYPE& pKeys) const {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::lower_bound(const KEY_CONTAINER_TYPE& pKeys) {
    return _advance<VALUE_TYPE>(*this, pKeys, SearchEndingPoint::EQUAL_OR_NEXT);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::lower_bound(const KEY_CONTAINER_TYPE& pKeys) const {
    return _advance<const VALUE_TYPE, const radix_map>(*this, pKeys, SearchEndingPoint::EQUAL_OR_NEXT);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::upper_bound(const KEY_CONTAINER_TYPE& pKeys) {
    return _advance<VALUE_TYPE>(*this, pKeys, SearchEndingPoint::NEXT);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::upper_bound(const KEY_CONTAINER_TYPE& pKeys) const {
    return _advance<const VALUE_TYPE, const radix_map>(*this, pKeys, SearchEndingPoint::NEXT);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::prev(const KEY_CONTAINER_TYPE& pKeys) {
    return _advance<VALUE_TYPE>(*this, pKeys, SearchEndingPoint::PREV);
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::prev(const KEY_CONTAINER_TYPE& pKeys) const {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
const std::vector<std::pair<KEY_TYPE, radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>>>&
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::getChildrenContent() const {
    return _children.getContent();
}

template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
std::size_t radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::erase(const KEY_CONTAINER_TYPE& pKeys) {
                ...
                if (currNode->_value_ptr) {
                    ...
                    if (currNodeChildrenSize == 0) {
                        if (parentNode != nullptr) {
                            ...
                            if (!parentNode->_value_ptr && parentNode->_children.size() == 1)
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::erase(const iterator& pIt) {
...
template<typename KEY_TYPE, typename KEY_CONTAINER_TYPE, typename VALUE_TYPE>
typename radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::const_iterator
    radix_map<KEY_TYPE, KEY_CONTAINER_TYPE, VALUE_TYPE>::erase(const const_iterator& pIt) {
