...
private:
  ...
  void insert(iterator pIteratorPos,
              iterator pIteratorBegin,
              iterator pIteratorEnd);
...
{
  return vector_of_refs<T>::const_iterator(_elts.find(const_cast<T*>(&pElt))); // very rare case where I think it's ok to do a const_cast
}
