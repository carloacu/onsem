...
struct optional {
    ...
    optional(optional&& pOther) { _val = std::move(pOther._val); }
    ...
    bool operator<=(const optional<T>& pOther) const { return _val < pOther._val || _val == pOther._val; }
    bool operator>(const optional<T>& pOther) const {
        if (_val && pOther._val)
            return *_val > *pOther._val;
        return _val && !pOther._val;
    ...
    bool has_value() const { return _val.operator bool(); }

    const T& value() const { return *_val; }

    operator bool() const { return _val.operator bool(); }
    ...
    bool operator!=(const optional& pOther) const { return !operator==(pOther); }
...
template<typename T>
bool operator==(const optional<T>& pOpt, const T& pOther) {
    if (pOpt.has_value())
        return *pOpt == pOther;
    return false;
}
...
template<typename T>
bool operator==(const T& pOther, const optional<T>& pOpt) {
    if (pOpt.has_value())
        return *pOpt == pOther;
    return false;
}
