...
static inline bool allocMemZone(signed char** pMem, std::istream& pBinDatabaseFile, std::size_t pSize) {
    try {
...
static inline bool allocMemZoneU(unsigned char** pMem, std::istream& pBinDatabaseFile, std::size_t pSize) {
    try {
...
static inline signed char const* getStrWithOffset(std::string& pOutStr, signed char const* pInputMem) {
    pOutStr.clear();
