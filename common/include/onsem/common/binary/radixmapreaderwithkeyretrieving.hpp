...
const unsigned char* readWithKeyRetrieving
  ...
  for (unsigned char i = 0u; i < nbOfChildren; ++i)
  {
    ...
    if (charToFind == firstletter)
      return readWithKeyRetrieving(pBeginMemoryPtr + binaryloader::alignedDecToInt(*offsetOfTheCild),
                                   pBeginMemoryPtr, pKey, childKeyIndex + 1, pEndPos, pIsEndOfAWord);
    if (charToFind < firstletter)
      return nullptr;
  }
  return nullptr;
...
std::string readKey(const unsigned char* pPtr,
                    int pNodeOffset,
                    int pChildNodeOffset = 0)
{
