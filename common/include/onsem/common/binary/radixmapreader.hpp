...
struct ONSEM_COMMON_API StaticRadixMapIterator {
    StaticRadixMapIterator(std::string pKey, const unsigned char* pValue)
        : key(pKey)
        , value(pValue) {}
...
static inline const unsigned char* read(const unsigned char* pPtr,
...
static inline StaticRadixMapIterator _read_begin(const unsigned char* pPtr,
...
static inline StaticRadixMapIterator _read_end(const unsigned char* pPtr,
                                               const unsigned char* pBeginMemoryPtr,
                                               const std::string& pKeyPrefix,
                                               const std::function<bool(const unsigned char*)>& pIsEndOfAWord) {
    std::string keysAfterPrefix;
...
static inline const unsigned char* _read_value(const unsigned char* pPtr,
                                               const std::function<bool(const unsigned char*)>& pIsEndOfAWord) {
    std::size_t nbOfKeys = _readNbOfKeys(pPtr);
    pPtr += nbOfKeys;
    unsigned char nbOfChildren = *(pPtr++);
...
static inline const unsigned char* _getChildIdOfARadixNodePtr(const unsigned char* pPtr, unsigned char pChildId) {
    std::size_t nbOfKeys = _readNbOfKeys(pPtr);
    pPtr += nbOfKeys;
    unsigned char nbOfChildren = *(pPtr++);
...
static inline StaticRadixMapIterator _advance(const unsigned char* pPtr,
                                              const std::string& pKey,
                                              const std::function<bool(const unsigned char*)>& pIsEndOfAWord,
                                              std::size_t pBeginPos,
                                              std::size_t pEndPos,
    ...
    while (true) {
        ...
        std::size_t nbOfKeys = _readNbOfKeys(pPtr);
        std::size_t keySize = pEndPos - pBeginPos;
        if (nbOfKeys <= keySize && (nbOfKeys == 0 || memcmp(pPtr, pKey.data() + pBeginPos, nbOfKeys) == 0)) {
            pPtr += nbOfKeys;
            unsigned char nbOfChildren = *(pPtr++);
                        ...
                        while (!parentNodes.empty()) {
                            ...
                            if (parentNode.second > 0) {
                                ...
                                if (prevChildPtr != nullptr) {
                                    std::string prevKeys =
                                        getKeysPrefix() + _getFirstLetterOfARadixNodeChildren(prevChildPtr);
        ...
        } else if (keySize == 0 && pSearchEndingPoint == SearchEndingPoint::EQUAL_OR_NEXT) {
...
static inline StaticRadixMapIterator read_lower_bound(const unsigned char* pPtr,
                                                      const std::string& pKey,
                                                      const std::function<bool(const unsigned char*)>& pIsEndOfAWord) {
    return _advance(pPtr, pKey, pIsEndOfAWord, 0, pKey.size(), SearchEndingPoint::EQUAL_OR_NEXT);
}

static inline StaticRadixMapIterator read_upper_bound(const unsigned char* pPtr,
                                                      const std::string& pKey,
                                                      const std::function<bool(const unsigned char*)>& pIsEndOfAWord) {
    return _advance(pPtr, pKey, pIsEndOfAWord, 0, pKey.size(), SearchEndingPoint::NEXT);
}

static inline StaticRadixMapIterator read_previous(const unsigned char* pPtr,
                                                   const std::string& pKey,
                                                   const std::function<bool(const unsigned char*)>& pIsEndOfAWord) {
