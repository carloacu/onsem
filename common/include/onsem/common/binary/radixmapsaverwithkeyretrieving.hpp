...
template<typename T>
binarymasks::Ptr _writeARadixMapNodeWKR
(int pParentPtr,
 ...
 const std::function<void(binarymasks::Ptr&, const T*)>& pPrintEndOfNode)
{
  ...
  for (auto i = 0u; i < children.size(); ++i)
  {
    ...
    pEndMemory = _writeARadixMapNodeWKR(decNode, children[i].second, pBeginMemory, pEndMemory,
                                        pKeysToOffset, subKeyPrefix, pPrintEndOfNode);
  }
  return pEndMemory;
}
...
template<typename T>
binarymasks::Ptr writeWithKeyRetrieving
(const mystd::radix_map_str<T>& pRadixMap,
 binarymasks::Ptr pEndMemory,
 std::map<std::string, int>& pKeysToOffset,
 const std::function<void(binarymasks::Ptr&, const T*)>& pPrintEndOfNode)
