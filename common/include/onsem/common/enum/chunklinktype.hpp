...
enum class ChunkLinkType { SYNTACTIC_CHUNK_LINK_TABLE };
#undef SYNTACTIC_CHUNK_LINK

#define SYNTACTIC_CHUNK_LINK(a, b, c) b,
static const std::vector<std::string> _chunkLinkType_toStr = {SYNTACTIC_CHUNK_LINK_TABLE};
#undef SYNTACTIC_CHUNK_LINK

#define SYNTACTIC_CHUNK_LINK(a, b, c) c,
static const std::vector<std::string> _chunkLinkType_toColorStr = {SYNTACTIC_CHUNK_LINK_TABLE};
#undef SYNTACTIC_CHUNK_LINK

#define SYNTACTIC_CHUNK_LINK(a, b, c) {b, ChunkLinkType::a},
static const std::map<std::string, ChunkLinkType> _chunkLinkType_fromStr = {SYNTACTIC_CHUNK_LINK_TABLE};
#undef SYNTACTIC_CHUNK_LINK
#undef SYNTACTIC_CHUNK_LINK_TABLE

static inline std::string chunkLinkType_toStr(ChunkLinkType pChunkLinkType) {
    return _chunkLinkType_toStr[mystd::to_underlying(pChunkLinkType)];
}

static inline std::string chunkLinkType_toColorStr(ChunkLinkType pChunkLinkType) {
...
inline static ChunkLinkType chunkLinkType_fromCompPolarity(ComparisonOperator pComparisonOperator) {
    switch (pComparisonOperator) {
        case ComparisonOperator::DIFFERENT: return ChunkLinkType::COMPARATOR_DIFFERENT;
        case ComparisonOperator::EQUAL: return ChunkLinkType::COMPARATOR_EQUAL;
        case ComparisonOperator::MORE: return ChunkLinkType::COMPARATOR_MORE;
        case ComparisonOperator::LESS: return ChunkLinkType::COMPARATOR_LESS;
    }
    assert(false);
    return ChunkLinkType::SIMPLE;
}

inline static ComparisonOperator chunkLinkType_toCompPolarity(ChunkLinkType pChunkLinkType) {
...
inline static bool chunkLinkType_isAComparator(ChunkLinkType pChunkLinkType) {
    return pChunkLinkType == ChunkLinkType::COMPARATOR_DIFFERENT || pChunkLinkType == ChunkLinkType::COMPARATOR_EQUAL
        || pChunkLinkType == ChunkLinkType::COMPARATOR_MORE || pChunkLinkType == ChunkLinkType::COMPARATOR_LESS;
}
