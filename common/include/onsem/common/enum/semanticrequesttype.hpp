...
#define SEMANTIC_REQUEST_TABLE                                                 \
    ADD_SEMANTIC_REQUEST_TYPE(YESORNO, "yes or no", GrammaticalType::UNKNOWN)  \
    ADD_SEMANTIC_REQUEST_TYPE(ACTION, "action", GrammaticalType::UNKNOWN)      \
    ADD_SEMANTIC_REQUEST_TYPE(LOCATION, "location", GrammaticalType::LOCATION) \
    ADD_SEMANTIC_REQUEST_TYPE(OBJECT, "object", GrammaticalType::OBJECT)       \
    ADD_SEMANTIC_REQUEST_TYPE(VERB, "verb", GrammaticalType::UNKNOWN)          \
    ADD_SEMANTIC_REQUEST_TYPE(TIMES, "times", GrammaticalType::UNKNOWN)        \
    ADD_SEMANTIC_REQUEST_TYPE(PURPOSE, "purpose", GrammaticalType::PURPOSE)    \
    ADD_SEMANTIC_REQUEST_TYPE(CAUSE, "cause", GrammaticalType::CAUSE)          \
    ADD_SEMANTIC_REQUEST_TYPE(CHOICE, "choice", GrammaticalType::OBJECT)       \
    ADD_SEMANTIC_REQUEST_TYPE(MANNER, "manner", GrammaticalType::MANNER)       \
    ADD_SEMANTIC_REQUEST_TYPE(QUANTITY, "quantity", GrammaticalType::OBJECT)   \
    ADD_SEMANTIC_REQUEST_TYPE(SUBJECT, "subject", GrammaticalType::SUBJECT)    \
    ADD_SEMANTIC_REQUEST_TYPE(TIME, "time", GrammaticalType::TIME)             \
    ADD_SEMANTIC_REQUEST_TYPE(DISTANCE, "distance", GrammaticalType::LENGTH)   \
    ADD_SEMANTIC_REQUEST_TYPE(DURATION, "duration", GrammaticalType::DURATION) \
    ADD_SEMANTIC_REQUEST_TYPE(ABOUT, "about", GrammaticalType::UNKNOWN)        \
    ADD_SEMANTIC_REQUEST_TYPE(TOPIC, "topic", GrammaticalType::TOPIC)          \
    ADD_SEMANTIC_REQUEST_TYPE(WAY, "way", GrammaticalType::WAY)                \
    ADD_SEMANTIC_REQUEST_TYPE(NOTHING, "nothing", GrammaticalType::UNKNOWN)

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) a,
enum class SemanticRequestType : char { SEMANTIC_REQUEST_TABLE };
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) b,
static const std::vector<std::string> _semanticRequestType_toStr = {SEMANTIC_REQUEST_TABLE};
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) c,
static const std::vector<GrammaticalType> _semanticRequestType_toSemGram = {SEMANTIC_REQUEST_TABLE};
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) {c, SemanticRequestType::a},
static const std::map<GrammaticalType, SemanticRequestType> _semanticRequestType_fromSemGram = {SEMANTIC_REQUEST_TABLE};
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) {b, SemanticRequestType::a},
static const std::map<std::string, SemanticRequestType> _semanticRequestType_fromStr = {SEMANTIC_REQUEST_TABLE};
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) SemanticRequestType::a,
static const std::vector<SemanticRequestType> semanticRequestType_allValues = {SEMANTIC_REQUEST_TABLE};
#undef ADD_SEMANTIC_REQUEST_TYPE

#define ADD_SEMANTIC_REQUEST_TYPE(a, b, c) 1 +
static const std::size_t semanticRequestType_size = SEMANTIC_REQUEST_TABLE 0;
#undef ADD_SEMANTIC_REQUEST_TYPE
...
static inline std::string semanticRequestType_toStr(SemanticRequestType pRequestType) {
    return _semanticRequestType_toStr[semanticRequestType_toChar(pRequestType)];
}

static inline GrammaticalType semanticRequestType_toSemGram(SemanticRequestType pRequestType) {
    return _semanticRequestType_toSemGram[semanticRequestType_toChar(pRequestType)];
}

static inline SemanticRequestType semanticRequestType_fromSemGram(GrammaticalType pGrammType) {
    return _semanticRequestType_fromSemGram.find(pGrammType)->second;
}

static inline SemanticRequestType semanticRequestType_fromStr(const std::string& pRequestTypeStr) {
...
static inline void setRequest(std::vector<SemanticRequestType>& pRequests, SemanticRequestType pRequest) {
...
struct ONSEM_COMMON_API SemanticRequests {
    ...
    bool isAQuestionNotAskingAboutTheObject() const {
        bool res = false;
        for (const auto& type : types)
            if (type != SemanticRequestType::ACTION && type != SemanticRequestType::OBJECT)
                res = true;
        return res;
    }
    ...
    void addWithoutCollisionCheck(SemanticRequestType pRequest) { types.push_back(pRequest); }
    void set(SemanticRequestType pRequest) { setRequest(types, pRequest); }
    ...
    void swap(SemanticRequests& pOther) { types.swap(pOther.types); }
    ...
    bool operator==(const SemanticRequests& pOther) const { return types == pOther.types; }
    bool operator!=(const SemanticRequests& pOther) const { return types != pOther.types; }
