#ifndef ONSEM_SEMANTICTOTEXT_SEMANTICMEMORY_SEMANTICMEMORYBLOCK_HPP
#define ONSEM_SEMANTICTOTEXT_SEMANTICMEMORY_SEMANTICMEMORYBLOCK_HPP
...
struct ONSEMSEMANTICTOTEXT_API SemanticMemoryBlock {
    ...
    std::shared_ptr<ExpressionWithLinks> addRootSemExp(
    ...
    void clear();
    void clearSemExpBySemExpOnlyForTests(
        const linguistics::LinguisticDatabase& pLingDb,
        std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr);
    ...
    std::unique_ptr<GroundedExpressionContainer> getEquivalentGrdExpPtr(
        const std::string& pUserId,
        const linguistics::LinguisticDatabase& pLingDb) const;
    void getAllEquivalentUserIds(std::list<std::string>& pRes, const std::string& pUser) const;
    std::string getUserId(const std::vector<std::string>& pNames, const std::string& pUserIdWithoutContext) const;
    ...
    const SemanticMemoryBlock* getFallbacksBlockPtr() const {
        return _fallbacksBlockPtr ? &*_fallbacksBlockPtr : nullptr;
    }
    ...
    mystd::observable::ObservableUnsafe<void(const GroundedExpression&, std::list<const GroundedExpression*>)>
        grdExpReplacedGrdExps;
    ...
    static void extractConditionToActions(std::list<UniqueSemanticExpression>& pConditionToActionsSemExp,
...
private:
    ...
    void _pruneExceedingExpressions(
        const linguistics::LinguisticDatabase& pLingDb,
        std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr);
    ...
    bool _userIdToHardCodedUserId(std::string& pHardCodedUserId, const std::string& pUserId) const;
    void _nameToUserIds(std::set<std::string>& pUserIds, const std::string& pName) const;
    bool _doesUserIdExist(const std::string& pUserId) const;

    void _writeInBinary(binarymasks::Ptr& pPtr, const linguistics::LinguisticDatabase& pLingDb) const;
...
#endif    // ONSEM_SEMANTICTOTEXT_SEMANTICMEMORY_SEMANTICMEMORYBLOCK_HPP
