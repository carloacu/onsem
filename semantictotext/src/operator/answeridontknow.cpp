    ...
    if (statementGrdPtr != nullptr) {
        ...
        if (requestType != SemanticRequestType::NOTHING) {
            if (requestType == SemanticRequestType::ACTION) {
                if (pForActions)
                    return mystd::unique_propagate_const<UniqueSemanticExpression>(
                        SemExpCreator::sayThatTheRobotCannotDoIt(pGrdExp));
            } else if (pForQuestions)
                return mystd::unique_propagate_const<UniqueSemanticExpression>(
                    SemExpCreator::sayThatWeDontKnowTheAnswer(pGrdExp));
        }
    }
    return mystd::unique_propagate_const<UniqueSemanticExpression>();
}
        ...
        case SemanticExpressionType::COMPARISON: {
            ...
            if (compExp.request != SemanticRequestType::NOTHING) {
                if (compExp.request == SemanticRequestType::ACTION) {
                    if (pForActions)
                        return mystd::unique_propagate_const<UniqueSemanticExpression>(
                            SemExpCreator::sayThatTheRobotCannotDoIt(pSemExp));
                } else if (pForQuestions)
                    return mystd::unique_propagate_const<UniqueSemanticExpression>(
                        SemExpCreator::sayThatWeDontKnowTheAnswer(pSemExp));
            }
            break;
        }
