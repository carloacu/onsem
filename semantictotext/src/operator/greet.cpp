...
struct UserInfosWithoutMemoryLinks : SemExpUserInfosFiller::UserInfosContainer
{
  ...
  void addGenders(const std::string&,
                  const std::set<SemanticGenderType>&) override {}
  ...
  void addGrdExpToUserId(const GroundedExpression&,
...
std::string _extractNameFromGrdExp(const GroundedExpression& pGrdExp,
...
{
  if (!pInterlocutor)
  { // find name from semantic memory
    if (!pAuthorUserId.empty())
    {
      std::vector<std::unique_ptr<GroundedExpression>> grdExpAnswers;
      memoryOperation::get(
            grdExpAnswers, SemExpCreator::askWhatIsYourName(pAuthorUserId),
                        ...
                        const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  if (grdExpObjectPtr != nullptr)
  {
    ...
    if (genGrdPtr != nullptr)
    {
      ...
      const auto grdExpObjectSpecifierPtr = SemExpGetter::getGrdExpChild(*grdExpObjectPtr, GrammaticalType::SPECIFIER);
      if (grdExpObjectSpecifierPtr != nullptr)
        toPerson = toPerson && ConceptSet::haveAConceptThatBeginWith(grdExpObjectSpecifierPtr->grounding().concepts, "agent_human");
...
mystd::unique_propagate_const<UniqueSemanticExpression> _greet(
    ...
    std::unique_ptr<SemanticExpression> pInterlocutor)
{
  ...
  case SemanticExpressionType::GROUNDED:
  {
    ...
    else
    {
      ...
      { // CASE statement
        ...
        { // CASE request (== question OR order)

          if (statGrdPtr->requests.has(SemanticRequestType::ACTION) && isLookAtOtherOrder(grdExp, pLingDb))
          { // CASE human order an action AND action is "look at other"
            if(callbacks.lookAtOtherPerson)
              callbacks.lookAtOtherPerson(std::string());
          }
          else
          { // CASE might be a question
            mystd::unique_propagate_const<UniqueSemanticExpression> exp;

            // Nothing was specified to express the interlocutor
            if (!pInterlocutor)
            {  // find an expression from the semantic memory
              std::vector<std::unique_ptr<GroundedExpression>> grdExpAnswers;
              memoryOperation::get(grdExpAnswers, SemExpCreator::askWhatIsYourName(pAuthorUserId), pSemanticMemory, pLingDb);
              if (!grdExpAnswers.empty())
        ...
        { // CASE is a simple statement, might be human giving his/her name
  ...
  case SemanticExpressionType::LIST:
  {
    ...
    {
      auto res = _greet(*currElt, pSemanticMemory, pLingDb, callbacks, pAuthorUserId, std::move(pInterlocutor));
        ...
        ConceptSet::haveAConceptThatBeginWith(fdkGrdExpPtr->grounding().concepts, "engagement_"))
    {
      ...
      if (concernedGrdExpPtr != nullptr)
      {
        ...
        if (concernedAgentPtr != nullptr &&
            pSemanticMemory.memBloc.isItMe(concernedAgentPtr->userId))
        { // Human speaks to the robot with interjection. Might be "Hi N5", "Bye N5" ...
          return _answerHelloBye(
                *fdkExp.feedbackExp, pSemanticMemory, pLingDb, callbacks,
                pAuthorUserId, std::move(pInterlocutor));
        }
      }
    }
    return _greet(*fdkExp.concernedExp, pSemanticMemory, pLingDb,
                  callbacks, pAuthorUserId, std::move(pInterlocutor));
  }
  case SemanticExpressionType::METADATA:
  {
    const auto& metaExp = pSemExp.getMetadataExp();
    std::string authorUserId = metaExp.getAuthorId();
    return _greet(*metaExp.semExp, pSemanticMemory, pLingDb,
                  callbacks, authorUserId, std::move(pInterlocutor));
  }
  case SemanticExpressionType::ANNOTATED:
    return _greet(*pSemExp.getAnnExp().semExp, pSemanticMemory, pLingDb,
                  callbacks, pAuthorUserId, std::move(pInterlocutor));
  case SemanticExpressionType::INTERPRETATION:
    return _greet(*pSemExp.getIntExp().interpretedExp, pSemanticMemory, pLingDb,
                  callbacks, pAuthorUserId, std::move(pInterlocutor));
  case SemanticExpressionType::FIXEDSYNTHESIS:
    return _greet(pSemExp.getFSynthExp().getSemExp(), pSemanticMemory, pLingDb,
                  callbacks, pAuthorUserId, std::move(pInterlocutor));
