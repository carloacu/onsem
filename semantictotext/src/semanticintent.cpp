...
static const std::map<TypeOfIntent, std::string> _typeOfIntent_toStr = {SEMANTIC_TYPEOFINTENT_TABLE};
...
void extractIntents(std::list<SemanticIntent>& pRes, const SemanticExpression& pSemExp) {
    ...
    switch (engagement) {
        case SemanticEngagementValue::ENGAGE: pRes.emplace_back(SemanticGreetingIntent()); break;
        case SemanticEngagementValue::DISENGAGE_GOODBYE: pRes.emplace_back(SemanticFarewellIntent("goodbye")); break;
        case SemanticEngagementValue::DISENGAGE_NEEDTOGO: pRes.emplace_back(SemanticFarewellIntent("needToGo")); break;
        case SemanticEngagementValue::UNKNWON: break;
    }
}
