...
std::unique_ptr<SemanticExpression> _loadSemExp(const boost::property_tree::ptree& pTree,
...
static const std::string relativePersonWithoutNumberDefaultStr = relativePersonWithoutNumber_toStr(RelativePersonWithoutNumber::UNKNOWN);
...
void _loadGrd(SemanticGrounding& pGrd,
              const boost::property_tree::ptree& pTree)
{
...
void _loadSemanticDate(SemanticDate& pSemanticDate,
                       const boost::property_tree::ptree& pTree)
{
...
void _loadSemanticQuantity(SemanticQuantity& pSemanticQuantity,
                           const boost::property_tree::ptree& pTree)
{
  ...
  if (!pTree.empty())
    for (const auto& currChild : pTree)
      if (!_isASemExpLabel(currChild.first) && currChild.first != "grounding")
        res->children.emplace(grammaticalType_fromStr(currChild.first),
                                                     ...
                                                     LoaderSemExpLinks* pLinks)
{
  ...
  res->contextualAnnotation = contextualAnnotation_fromStr
  ...
  res->fromText = pTree.get("fromText", "");
  {
    childLoop(pTree, currRef, "references")
        res->references.emplace_back(currRef.second.data());
  }
                                                ...
                                                LoaderSemExpLinks* pLinks)
{
  ...
  {
    switch (semanticExpressionType_fromStr(pTree.get<std::string>(semExpTypeLabelStr,
                                                                  semanticExpressionType_toStr(SemanticExpressionType::GROUNDED))))
    {
    ...
    case SemanticExpressionType::LIST:
      return _loadListExp(pTree, pLinks);
    case SemanticExpressionType::CONDITION:
      return _loadConditionExp(pTree, pLinks);
    case SemanticExpressionType::COMPARISON:
      return _loadComparisonExp(pTree, pLinks);
    case SemanticExpressionType::INTERPRETATION:
      return _loadInterpretationExp(pTree, pLinks);
    case SemanticExpressionType::FEEDBACK:
      return _loadFeedbackExp(pTree, pLinks);
    case SemanticExpressionType::ANNOTATED:
      return _loadAnnotatedExp(pTree, pLinks);
    case SemanticExpressionType::METADATA:
      return _loadMetadataExp(pTree, pLinks);
    case SemanticExpressionType::SETOFFORMS:
      return _loadSetOfFormsExp(pTree, pLinks);
    case SemanticExpressionType::COMMAND:
      return _loadCommandExp(pTree, pLinks);
                       ...
                       const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  if (triggerAxiomIdOpt)
  {
    ...
    pContextAxiom.triggerAxiomId.listExpType = listExpressionType_fromStr(triggerAxiomId.get("listExpType", listExpTypeDefaultStr));
  }
 ...
 const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  {
    childLoop(pTree, currLI, "linkedInfos")
        res->linkedInfos.emplace(currLI.first, currLI.second.data());
  }
...
void _loadConceptSet(const boost::property_tree::ptree& pTree,
                     ConceptSet& pConceptSet)
{
  {
    childLoop(pTree, currConcept, "localConcepts")
        pConceptSet.addConcept(currConcept.second.get_value<std::string>());
  }
  childLoop(pTree, currOppConcepts, "oppositeConcepts")
    for (const auto& currOppCpts : currOppConcepts.second)
      pConceptSet.notifyOppositeConcepts(currOppConcepts.first,
                                         currOppCpts.second.get_value<std::string>());
}
...
void _loadWordAssociatedInfos(const boost::property_tree::ptree& pTree,
                              linguistics::WordAssociatedInfos& pInfos)
{
  _loadConcepts(pInfos.concepts, pTree);
  {
    childLoop(pTree, currContextualInfo, "contextualInfos")
        pInfos.contextualInfos.insert
...
void _loadLingDatabase(const boost::property_tree::ptree& pTree,
                       linguistics::LinguisticDatabase& pLingDb)
{
  _loadConceptSet(pTree.get_child("conceptSet"), pLingDb.conceptSet);
  childLoop(pTree, currLangToSpec, "langToSpec")
    _loadSpecificLingDatabase(currLangToSpec.second,
                              pLingDb.langToSpec[semanticLanguageEnum_fromStr(currLangToSpec.first)]);
