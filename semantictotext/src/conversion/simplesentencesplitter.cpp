...
void _splitGrdExp
(UniqueSemanticExpression& pRootSemExp,
  ...
  case SemanticExpressionType::GROUNDED:
  {
    ...
    if (grdExp->type == SemanticGroundingType::STATEMENT)
    {
      ...
      for (auto itChild = grdExp.children.begin();
           itChild != grdExp.children.end(); ++itChild)
      {
        ...
        if (subListExp != nullptr &&
            ...
            childType != GrammaticalType::SPECIFIER &&
            childType != GrammaticalType::SIMILARITY &&
            childType != GrammaticalType::WITH)
        {
          std::list<UniqueSemanticExpression> childrenToPutInSepSentences;
          childrenToPutInSepSentences.splice
              (childrenToPutInSepSentences.begin(),
               subListExp->elts);
          ListExpressionType listType = subListExp->listType;
          grdExp.children.erase(itChild);
          _splitGrdExp(pSemExp, grdExp, childType,
                       listType, childrenToPutInSepSentences);
          needToReloop = true;
          break;
        }
      }
