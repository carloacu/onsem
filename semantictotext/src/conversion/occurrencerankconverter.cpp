    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            if (grdExp->type == SemanticGroundingType::STATEMENT) {
                ...
                if (rootStatementGrd.concepts.count("verb_equal_be") > 0 && grdExp.children.size() == 1) {
                            ...
                            if (subjectGrdExp.children.size() == 2) {
                                ...
                                if (itSpecifierOfTheSubject != subjectGrdExp.children.end()
                                    ...
                                    if (itObjectOfTheSubject != subjectGrdExp.children.end()) {
                                        ...
                                        GroundedExpression* objectGrdExpPtr =
                                            objectUSemExp->getGrdExpPtr_SkipWrapperPtrs();
                                        if (objectGrdExpPtr != nullptr) {
                                            ...
                                            if (objectGrdExp->type == SemanticGroundingType::STATEMENT) {
                                                ...
                                                SemanticStatementGrounding& objectStatementGrd =
                                                    objectGrdExp->getStatementGrounding();
                                                if (rootStatementGrd.verbTense != SemanticVerbTense::PRESENT)
                                                    objectStatementGrd.verbTense = rootStatementGrd.verbTense;
                                                if (subjectGrdExp->concepts.find("occurrenceRank")
                                                    != subjectGrdExp->concepts.end()) {
                                                    objectStatementGrd.requests.set(SemanticRequestType::TIME);
                                                } else {
                                                    if (objectGrdExp.children.find(GrammaticalType::OBJECT)
                                                        == objectGrdExp.children.end()) {
                                                        objectGrdExp.children.emplace(
                                                            GrammaticalType::OBJECT,
                                                            std::make_unique<GroundedExpression>(
                                                                subjectGrdExp.cloneGrounding()));
                                                    }
                                                ...
                                                GroundedExpression* newRootGrdExpPtr =
                                                    newRoot->getGrdExpPtr_SkipWrapperPtrs();
                                                assert(newRootGrdExpPtr != nullptr);
                                                if (newRootGrdExpPtr != nullptr) {
                                                    SemExpModifier::addChild(*newRootGrdExpPtr,
                                                                             GrammaticalType::OCCURRENCE_RANK,
                                                                             std::move(itSubject->second));
                                                    auto res = std::make_unique<SetOfFormsExpression>();
                                                    res->prioToForms[-10].emplace_back(
                                                        std::make_unique<QuestExpressionFrom>(
                                                            std::move(cloneOfOldSemExp), true));
                                                    res->prioToForms[-11].emplace_back(
                                                        std::make_unique<QuestExpressionFrom>(std::move(newRoot),
                                                                                              false));
