...
void _addAgentInterpretations(UniqueSemanticExpression& pSemExp,
    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::ANNOTATED: {
            ...
            _addAgentInterpretations(annExp.semExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            for (auto& currAnn : annExp.annotations)
                _addAgentInterpretations(
                    currAnn.second, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
        case SemanticExpressionType::COMMAND: {
            CommandExpression& cmdExp = pSemExp->getCmdExp();
            _addAgentInterpretations(cmdExp.semExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
        case SemanticExpressionType::FEEDBACK: {
            FeedbackExpression& fdkExp = pSemExp->getFdkExp();
            _addAgentInterpretations(
                fdkExp.concernedExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
        case SemanticExpressionType::GROUNDED: {
            ...
            if (grdExp.grounding().type == SemanticGroundingType::NAME) {
                ...
                grdExp.moveGrounding(
                    std::make_unique<SemanticAgentGrounding>(userId, userIdWithoutContext, nameGrd.nameInfos));
                    ...
                    if (userId != SemanticAgentGrounding::getUserNotIdentified()) {
                        pSemExp = std::make_unique<InterpretationExpression>(
                            ...
                            UniqueSemanticExpression(
                                std::make_unique<GroundedExpression>(std::make_unique<SemanticAgentGrounding>(userId))),
                            std::move(pSemExp));
                        break;
                    }
                }
        ...
        case SemanticExpressionType::INTERPRETATION: {
            ...
            _addAgentInterpretations(
                intExp.interpretedExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            if (intExp.source == InterpretationSource::STATEMENTCOREFERENCE)
                _addAgentInterpretations(
                    intExp.originalExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
        case SemanticExpressionType::LIST: {
            ...
            for (auto& elt : listExp.elts) {
                bool isMeFromMyName = false;
                _addAgentInterpretations(elt, pSemanticMemory, pLingDb, pParentGrammaticalType, isMeFromMyName);
            }
            break;
        }
        case SemanticExpressionType::METADATA: {
            ...
            if (metaExp.source) {
                bool isMeFromMyName = false;
                _addAgentInterpretations(
                    *metaExp.source, pSemanticMemory, pLingDb, pParentGrammaticalType, isMeFromMyName);
            }
            _addAgentInterpretations(metaExp.semExp, pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
        case SemanticExpressionType::CONDITION: {
            ConditionExpression& condExp = pSemExp->getCondExp();
            {
                bool isMeFromMyName = false;
                _addAgentInterpretations(
                    condExp.conditionExp, pSemanticMemory, pLingDb, pParentGrammaticalType, isMeFromMyName);
            }
            {
                bool isMeFromMyName = false;
                _addAgentInterpretations(
                    condExp.thenExp, pSemanticMemory, pLingDb, pParentGrammaticalType, isMeFromMyName);
            }
            if (condExp.elseExp) {
                bool isMeFromMyName = false;
                _addAgentInterpretations(
                    *condExp.elseExp, pSemanticMemory, pLingDb, pParentGrammaticalType, isMeFromMyName);
            }
            break;
        }
        case SemanticExpressionType::FIXEDSYNTHESIS: {
            ...
            _addAgentInterpretations(
                fSynthExp.getUSemExp(), pSemanticMemory, pLingDb, pParentGrammaticalType, pIsMeFromMyName);
            break;
        }
...
void addAgentInterpretations(UniqueSemanticExpression& pSemExp,
                             ...
                             const linguistics::LinguisticDatabase& pLingDb) {
    bool isMeFromMyName = false;
    _addAgentInterpretations(pSemExp, pSemanticMemory, pLingDb, GrammaticalType::UNKNOWN, isMeFromMyName);
}
