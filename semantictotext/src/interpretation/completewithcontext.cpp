    ...
    switch (pSemExp->type) {
        ...
        case SemanticExpressionType::GROUNDED: {
            ...
            if (pAnnSpecPtr != nullptr) {
                ...
                if (statGrdExpPtr != nullptr && statGrdExpPtr->coreference
                    && (statGrdExpPtr->coreference->getDirection()
                            == CoreferenceDirectionEnum::ANNOTATION_SPECIFICATIONS
...
void _fillWithActionThatAskAboutAction(UniqueSemanticExpression& pSemExp,
    ...
    for (const auto& currChild : pContextGrdExp.children) {
        if (grdRootExp->children.find(currChild.first) == grdRootExp->children.end()) {
            grdRootExp->children.emplace(currChild.first, currChild.second->clone());
        }
    }
    pSemExp = std::make_unique<InterpretationExpression>(
        InterpretationSource::RECENTCONTEXT, UniqueSemanticExpression(std::move(grdRootExp)), std::move(pSemExp));
}

UniqueSemanticExpression _copyGrdExpForAChild(GrammaticalType pGramType, const GroundedExpression& pGrdExp) {
    if (pGramType == GrammaticalType::TIME && pGrdExp.children.empty()) {
        auto* genGrdPtr = pGrdExp->getGenericGroundingPtr();
        if (genGrdPtr != nullptr && genGrdPtr->quantity.type == SemanticQuantityType::NUMBER
            && genGrdPtr->quantity.nb.isPositive() && genGrdPtr->quantity.nb.isAnInteger()
...
void _addChild(UniqueSemanticExpression& pSemExp,
               const GroundedExpression& pContextGrdExp,
            ...
            if (genGrdToReplacePtr != nullptr) {
                ...
                if (SemExpModifier::putItAdjectival(*copyOfSemExpToAdd, pLingDb)) {
                    ...
                    SemExpModifier::addChild(
                        *grdExpToReplacePtr, GrammaticalType::SPECIFIER, std::move(copyOfSemExpToAdd));
                    childAdded = true;
                }
            }
        }
...
void _replaceYesOrNoQuestionAccordingToTheAgreement(UniqueSemanticExpression& pSemExp,
                                                    const SemanticExpression& pContextSemExp,
                                                    TruenessValue pAgreementVal) {
    static const std::set<SemanticExpressionType> expressionTypesToSkip = {SemanticExpressionType::METADATA,
                                                                           SemanticExpressionType::FIXEDSYNTHESIS};
    ...
    SemExpCreator::replaceSemExpOrAddInterpretation(
        InterpretationSource::YES_NO_REPLACEMENT, pSemExp, std::move(newSemExp));
}

bool _checkAgreementAndReplaceYesOrNoQuestion(UniqueSemanticExpression& pSemExp,
                                              const SemanticExpression& pContextSemExp,
                                              const GroundedExpression& pCurrGrdExp) {
    TruenessValue agreementVal = semExpAgreementDetector::getAgreementValue(pCurrGrdExp);
...
UniqueSemanticExpression _generateSentenceWhatHappenedJustAfterThat() {
    ...
    rootGrdExp->children.emplace(GrammaticalType::TIME, []() {
        ...
        relTimeGrdExp->children.emplace(
...
bool _tryToCompleteAnswerWithTheQuestionFromGrdExp(UniqueSemanticExpression& pSemExp,
    ...
    switch (pGrdExp->type) {
            ...
            // for sentences "it's X" the "X" should be considered has potential answer
                ...
                if (itSubject != pGrdExp.children.end()
                    ...
                    if (itObject != pGrdExp.children.end()) {
                        ...
                        SemExpCreator::replaceSemExpOrAddInterpretation(
                            InterpretationSource::RESTRUCTURING, pSemExp, std::move(newSemExp));
                        _tryToCompleteAnswerWithTheQuestion(
                            pSemExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
                    }
                }
                return true;
            }
        ...
        case SemanticGroundingType::NAME: {
            if (pContextRequest != SemanticRequestType::YESORNO && pContextRequest != SemanticRequestType::TIME) {
                _addChild(pSemExp, pContextGrdExp, pContextRequest, pGrdExp, pLingDb);
                return true;
            }
            break;
        ...
        case SemanticGroundingType::UNITY: break;
    }
    return false;
}
...
bool _tryToCompleteAnswerWithTheQuestion(UniqueSemanticExpression& pSemExp,
    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            return _tryToCompleteAnswerWithTheQuestionFromGrdExp(
                pSemExp, grdExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
        }
        case SemanticExpressionType::ANNOTATED: {
            auto& annExp = pSemExp->getAnnExp();
            return _tryToCompleteAnswerWithTheQuestion(
                annExp.semExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
        }
        case SemanticExpressionType::FEEDBACK: {
            ...
            if (fdkGrdExpPtr != nullptr) {
                if (ConceptSet::haveAConceptThatBeginWith(fdkGrdExpPtr->grounding().concepts, "engagement_"))
                    break;
                if (pContextRequest == SemanticRequestType::YESORNO) {
                    TruenessValue agreementVal = semExpAgreementDetector::semExpToAgreementValue(*fdkExp.feedbackExp);
                    if (agreementVal != TruenessValue::UNKNOWN) {
                        _replaceYesOrNoQuestionAccordingToTheAgreement(pSemExp, pContextSemExp, agreementVal);
                        break;
                    }
            ...
            return _tryToCompleteAnswerWithTheQuestion(
                fdkExp.concernedExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
        }
        case SemanticExpressionType::INTERPRETATION: {
            auto& intExp = pSemExp->getIntExp();
            return _tryToCompleteAnswerWithTheQuestion(
                intExp.interpretedExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
        }
        case SemanticExpressionType::LIST: {
            auto& listExp = pSemExp->getListExp();
            if (!listExp.elts.empty())
                return _tryToCompleteAnswerWithTheQuestion(listExp.elts.front(),
                                                           pContextSemExp,
                                                           pContextGrdExp,
                                                           pContextStatementGr,
                                                           pContextRequest,
        ...
        case SemanticExpressionType::METADATA: {
            ...
            return _tryToCompleteAnswerWithTheQuestion(
                metaExp.semExp, pContextSemExp, pContextGrdExp, pContextStatementGr, pContextRequest, pLingDb);
        }
        case SemanticExpressionType::FIXEDSYNTHESIS:
        case SemanticExpressionType::COMMAND:
        case SemanticExpressionType::COMPARISON:
        case SemanticExpressionType::CONDITION:
        case SemanticExpressionType::SETOFFORMS: return false;
    }
    return false;
}
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (contextObjGenGr != nullptr) {
            if (!contextObjGenGr->coreference
                ...
                && SemExpComparator::getQuantityImbrication(contextObjGenGr->quantity, pGenGrd.quantity)
            ...
            if (contextGrd.getStatementGrounding().requests.empty() && pGenGrd.concepts.empty()
            ...
            if (pGenGrd.entityType == SemanticEntityType::AGENTORTHING
                ...
                || pGenGrd.entityType == SemanticEntityType::SENTENCE) {
                if (contextGrd.type == SemanticGroundingType::NAME || contextGrd.type == SemanticGroundingType::AGENT
                ...
                if (contextStatGrdPtr != nullptr && !contextStatGrdPtr->requests.empty())
                    return nullptr;
                return &pContextSemExp;
            }
        }
...
void _tryToCompleteInputQuestion(UniqueSemanticExpression& pSemExp, const GroundedExpression& pContextGrdExp) {
    GroundedExpression* currGrdExp = pSemExp->getGrdExpPtr_SkipWrapperPtrs();
    if (currGrdExp != nullptr && currGrdExp->grounding().type == SemanticGroundingType::STATEMENT
        && currGrdExp->children.empty()) {
        const SemanticStatementGrounding& statGrd = (*currGrdExp)->getStatementGrounding();
        if (statGrd.word.lemma.empty() && !statGrd.requests.empty()) {
            const SemanticStatementGrounding* contextStatement = pContextGrdExp->getStatementGroundingPtr();
            if (contextStatement != nullptr && contextStatement->requests.empty()) {
                pSemExp = std::make_unique<InterpretationExpression>(
...
bool _isAndThenListExp(const ListExpression& pListExp) {
    return pListExp.listType == ListExpressionType::THEN && pListExp.elts.empty();
}

bool _mergeSemExp(UniqueSemanticExpression& pSemExp,
                  const SemanticExpression& pContextSemExp,
                  bool pSameAuthor,
                  const linguistics::LinguisticDatabase& pLingDb) {
    ...
    if (contextGrdExp->type == SemanticGroundingType::STATEMENT) {
        ...
        if (!pSameAuthor && !contextStatementGr.requests.empty()) {
            return _tryToCompleteAnswerWithTheQuestion(pSemExp,
                                                       pContextSemExp,
                                                       contextGrdExp,
                                                       contextStatementGr,
                                                       contextStatementGr.requests.first(),
                                                       pLingDb);
        } else {
                            ...
                            *newCorefSemExpPtr = pSemExp->clone();
                            if (genGrd.referenceType == SemanticReferenceType::UNDEFINED) {
                                ...
                                if (contextCorefGrdExpPtr != nullptr) {
                                    auto* contextCorefGenGrdPtr =
                                        contextCorefGrdExpPtr->grounding().getGenericGroundingPtr();
                                    if (contextCorefGenGrdPtr != nullptr
                                        && contextCorefGenGrdPtr->referenceType != SemanticReferenceType::UNDEFINED)
                                        SemExpModifier::setReferenceTypeOfSemExp(**newCorefSemExpPtr,
                                                                                 contextCorefGenGrdPtr->referenceType);
                                }
                            }
                            SemExpModifier::addChildrenOfAnotherSemExp(
                                **newCorefSemExpPtr, **contextCorefSemExpPtr, ListExpressionType::UNRELATED);
                            pSemExp = std::make_unique<InterpretationExpression>(
                                InterpretationSource::RECENTCONTEXT, std::move(newContextGrdExp), std::move(pSemExp));
                        }
                    }
                }
            } else {
...
bool _canBeInTheSameEqualitySentence(const SemanticExpression& pSemExp1, const SemanticExpression& pSemExp2) {
    ...
    if (grdType1 == SemanticGroundingType::RESOURCE || grdType2 == SemanticGroundingType::RESOURCE)
        return true;
...
bool _hasToReplaceAndThenByQuestionWhatHappenedJustAfterThan(const ListExpression& pListExp,
                                                             const SemanticExpression& pContextSemExp,
                                                             bool pSameAuthor) {
    const GroundedExpression* contextGrdExp = pContextSemExp.getGrdExpPtr_SkipWrapperPtrs();
    if (contextGrdExp != nullptr && contextGrdExp->grounding().type == SemanticGroundingType::STATEMENT) {
        const SemanticStatementGrounding& contextStatementGr = (*contextGrdExp)->getStatementGrounding();
        if (!pSameAuthor && contextStatementGr.requests.empty() && _isAndThenListExp(pListExp))
            return true;
    }
    return false;
}

void _replaceRecentFromContext_fromSemExp(UniqueSemanticExpression& pSemExp,
                                          GrammaticalType pGramParentLink,
    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            switch (grdExp->type) {
                case SemanticGroundingType::GENERIC: {
                    ...
                    if (genGrd.coreference && genGrd.coreference->getDirection() == CoreferenceDirectionEnum::BEFORE
                        && (genGrd.referenceType != SemanticReferenceType::DEFINITE
                            || genGrd.word.isEmpty()))    // TODO: hack to remove
                    {
                        const SemanticExpression* contextToAddPtr = _getSemExpIfCompatibleWithTheEntityType(
                            pContextSemExp, genGrd, pGramParentLink, pAuthorPtr, pReceiverPtr, pMemBlock, pLingDb);
                        ...
                        if (contextToAddPtr == nullptr) {
                            ...
                            if (contextGrdExpPtr != nullptr) {
                                ...
                                if (contextObjSemExp != nullptr)
                                    contextToAddPtr = _getSemExpIfCompatibleWithTheEntityType(*contextObjSemExp,
                                                                                              genGrd,
                                                                                              pGramParentLink,
                                                                                              pAuthorPtr,
                                                                                              pReceiverPtr,
                                ...
                                if (contextToAddPtr == nullptr) {
                                    ...
                                    if (contextSubjSemExp != nullptr)
                                        contextToAddPtr = _getSemExpIfCompatibleWithTheEntityType(*contextSubjSemExp,
                                                                                                  genGrd,
                                                                                                  pGramParentLink,
                                                                                                  pAuthorPtr,
                                                                                                  pReceiverPtr,
                        ...
                        if (contextToAddPtr != nullptr) {
                            ...
                            if (contextToAddPtr != nullptr) {
                                ...
                                auto contextGrdExpCloned =
                                    contextToAddPtr->clone(nullptr, false, &expressionTypesToSkip);
                                SemExpModifier::removeSpecificationsNotNecessaryForAnAnswerFromSemExp(
                                    *contextGrdExpCloned);
                                pSemExp =
                                    std::make_unique<InterpretationExpression>(InterpretationSource::RECENTCONTEXT,
                ...
                case SemanticGroundingType::STATEMENT: {
                    ...
                    if (statGrd.coreference) {
                        ...
                        if (contextGrdExpPtr != nullptr) {
                            const SemanticStatementGrounding* contextStatementGrdPtr =
                                contextGrdExpPtr->grounding().getStatementGroundingPtr();
                            if (contextStatementGrdPtr != nullptr) {
                                ...
                                for (const auto& currChildFroContext : contextGrdExpPtr->children) {
                                    SemExpModifier::addChild(
                                        *grdExpCopied, currChildFroContext.first, currChildFroContext.second->clone());
                                }
                ...
                case SemanticGroundingType::RELATIVETIME: {
                    ...
                    if (itSpecificationChild != grdExp.children.end()) {
                        const GroundedExpression* specificationGrdExpPtr =
                            itSpecificationChild->second->getGrdExpPtr_SkipWrapperPtrs();
                        if (specificationGrdExpPtr != nullptr) {
                            const SemanticGenericGrounding* specificationGenGrdPtr =
                                specificationGrdExpPtr->grounding().getGenericGroundingPtr();
                            if (specificationGenGrdPtr != nullptr && specificationGenGrdPtr->coreference) {
                                ...
                                if (timeContextInfo != nullptr) {
                                    auto interpretation = std::make_unique<InterpretationExpression>(
                                        InterpretationSource::RECENTCONTEXT,
                                        timeContextInfo->clone(),
                                        std::move(itSpecificationChild->second));
            ...
            for (auto& currChild : grdExp.children) {
                ...
                _replaceRecentFromContext_fromSemExp(currChild.second,
                                                     currChild.first,
                                                     pFatherGrdExpOfStatementPtr,
                                                     pContextSemExp,
                                                     pSameAuthor,
                                                     pAuthorPtr,
                                                     pReceiverPtr,
        ...
        case SemanticExpressionType::LIST: {
            ...
            if (_hasToReplaceAndThenByQuestionWhatHappenedJustAfterThan(listExp, pContextSemExp, pSameAuthor)) {
                SemExpCreator::replaceSemExpOrAddInterpretation(
                    InterpretationSource::ANDTHEN, pSemExp, _generateSentenceWhatHappenedJustAfterThat());
                _replaceRecentFromContext_fromSemExp(pSemExp,
                                                     pGramParentLink,
                                                     pFatherGrdExpOfStatementPtr,
                                                     pContextSemExp,
                                                     pSameAuthor,
                ...
                for (auto& currElt : listExp.elts)
                    _replaceRecentFromContext_fromSemExp(currElt,
                                                         pGramParentLink,
                                                         pFatherGrdExpOfStatementPtr,
                                                         pContextSemExp,
                                                         pSameAuthor,
        ...
        case SemanticExpressionType::CONDITION: {
            ...
            _replaceRecentFromContext_fromSemExp(condExp.conditionExp,
                                                 ...
                                                 pMemBlock,
                                                 pLingDb);
            _replaceRecentFromContext_fromSemExp(condExp.thenExp,
                                                 GrammaticalType::UNKNOWN,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
                                                 ...
                                                 pLingDb);
            if (condExp.elseExp) {
                _replaceRecentFromContext_fromSemExp(*condExp.elseExp,
                                                     GrammaticalType::UNKNOWN,
                                                     pFatherGrdExpOfStatementPtr,
                                                     pContextSemExp,
                                                     pSameAuthor,
        ...
        case SemanticExpressionType::COMPARISON: {
            ...
            if (compExp.whatIsComparedExp) {
                _replaceRecentFromContext_fromSemExp(*compExp.whatIsComparedExp,
                                                     GrammaticalType::UNKNOWN,
                                                     pFatherGrdExpOfStatementPtr,
                                                     pContextSemExp,
                                                     pSameAuthor,
            ...
            _replaceRecentFromContext_fromSemExp(compExp.leftOperandExp,
                                                 GrammaticalType::UNKNOWN,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
            ...
            if (compExp.rightOperandExp) {
                _replaceRecentFromContext_fromSemExp(*compExp.rightOperandExp,
                                                     GrammaticalType::UNKNOWN,
                                                     pFatherGrdExpOfStatementPtr,
                                                     pContextSemExp,
                                                     pSameAuthor,
        ...
        case SemanticExpressionType::INTERPRETATION: {
            InterpretationExpression& intExp = pSemExp->getIntExp();
            _replaceRecentFromContext_fromSemExp(intExp.interpretedExp,
                                                 pGramParentLink,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
        ...
        case SemanticExpressionType::FEEDBACK: {
            FeedbackExpression& fdkExp = pSemExp->getFdkExp();
            _replaceRecentFromContext_fromSemExp(fdkExp.concernedExp,
                                                 pGramParentLink,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
        ...
        case SemanticExpressionType::ANNOTATED: {
            AnnotatedExpression& annExp = pSemExp->getAnnExp();
            _replaceRecentFromContext_fromSemExp(annExp.semExp,
                                                 pGramParentLink,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
        ...
        case SemanticExpressionType::METADATA: {
            ...
            _replaceRecentFromContext_fromSemExp(metadataExp.semExp,
                                                 pGramParentLink,
                                                 pFatherGrdExpOfStatementPtr,
                                                 pContextSemExp,
                                                 pSameAuthor,
    ...
    if (listExp != nullptr && listExp->listType == ListExpressionType::UNRELATED) {
        ...
        while (itExp != listExp->elts.begin()) {
            ...
            if (!alreadyReplaceRecentFromContext) {
                _replaceRecentFromContext_fromSemExp(
                    pSemExp, pGramParentLink, nullptr, **itExp, pSameAuthor, pAuthorPtr, nullptr, pMemBlock, pLingDb);
        ...
        _replaceRecentFromContext_fromSemExp(
            pSemExp, pGramParentLink, nullptr, pContextSemExp, pSameAuthor, pAuthorPtr, nullptr, pMemBlock, pLingDb);
