                            ...
                            const linguistics::LinguisticDatabase& pLingDb)
{
  for (auto itChild1 = pChildren1.begin(); itChild1 != pChildren1.end(); ++itChild1)
  {
    ...
    if (!SemExpComparator::semExpsAreEqualFromMemBlock(*itChild1->second, *itChild2->second, pMemBlock, pLingDb, &compExceptions))
      pDiffChidren.push_back(itChild1);
  }
}
...
/**
 * Handle the case:
 *     A       +     A        =>     A          +      A     (but if this function succeed the second element will be removed)
 *    / \           / \             / \               / \
 *  B    C         B   D           B  AND            B   D
 *                                    /  \
 *                                   C    D
...
bool _tryToMerge2SemExpsWithOnlyOneChildThatDiffer(UniqueSemanticExpression& pSemExp1,
                                                   ...
                                                   const linguistics::LinguisticDatabase& pLingDb)
{
  if (pSemExp1->type == SemanticExpressionType::GROUNDED &&
      pSemExp2->type == SemanticExpressionType::GROUNDED)
  {
      ...
      if (diffChildren1.size() <= 1)
      {
        ...
        else if (diffChildren1.size() == 1 && diffChildren2.size() == 1)
        {
          ...
          if (itDiffChild1->first == itDiffChild2->first)
          {
            ...
            SemExpModifier::addChild(grdExp1, itDiffChild2->first,
                                     std::move(itDiffChild2->second), pListType);
            return true;
          }
        }
      }
                       ...
                       const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  if (statGrdPtr1 != nullptr &&
      statGrdPtr2 != nullptr &&
      statGrdPtr1->verbTense == statGrdPtr2->verbTense &&
      statGrdPtr1->verbGoal == statGrdPtr2->verbGoal &&
      statGrdPtr1->verbGoal == VerbGoalEnum::ABILITY)
  {
    ...
    if (itSubject2 == grdExp2.children.end())
      return false;
    return SemExpComparator::semExpsAreEqualFromMemBlock(*itSubject1->second, *itSubject2->second, pMemBlock, pLingDb, nullptr);
  }
  return false;
}


ListExpressionType _getListType(const ListExpression& pListExp,
                                bool pConvertUnrelatedListToAndList)
{
  ListExpressionType listType = pListExp.listType;
  if (pConvertUnrelatedListToAndList &&
...
bool _tryToMergeListElements(ListExpression& pListExp,
                             ...
                             bool pConvertUnrelatedListToAndList)
{
  for (auto itFirstListElt = pListExp.elts.begin();
       itFirstListElt != pListExp.elts.end(); ++itFirstListElt)
  {
    ...
    for (; itSecondListElt != pListExp.elts.end(); ++itSecondListElt)
    {
      ...
      if (_tryToMerge2SemExpsWithOnlyOneChildThatDiffer(*itFirstListElt, *itSecondListElt,
                                                        listType, pMemBlock, pLingDb))
      {
...
void process(UniqueSemanticExpression& pSemExp,
             ...
             bool pConvertUnrelatedListToAndList)
{
  processFromMemBlock(pSemExp, pSemanticMemory.memBloc, pLingDb,
                      pConvertUnrelatedListToAndList);
}
