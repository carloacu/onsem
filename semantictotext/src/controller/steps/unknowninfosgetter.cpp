...
namespace {
enum class ReplacingParamStrategy { RETURN_PARAM, ASK_WHAT_IS, RETURN_AN_INSTANCE };
    ...
    if (itSemExpChild == pGrdExp.children.end()) {
        ...
        if (itSpecSemExpChild != pGrdExp.children.end()) {
            ...
            if (specGrdExpChild != nullptr) {
                _recCheckIfMatchAndGetParams(pParams,
                                             pSubIntContext,
                                             pIncompleteRelations,
                                             pIsComplete,
                                             pSemMemSent,
                                             pMemChildType,
                                             pCurrMemChild,
        ...
        if (memGrdExpChild != nullptr) {
            if (memGrdExpChild->grounding().type == SemanticGroundingType::META) {
                ...
                const GroundedExpression* childCorespondingToTheParamGrdExpPtr =
                    childCorespondingToTheParam.getGrdExpPtr_SkipWrapperPtrs();
                ReplacingParamStrategy replacingParamStrategy = [&] {
                    ...
                    if (childCorespondingToTheParamGrdExpPtr != nullptr) {
                        const GroundedExpression& childCorespondingToTheParamGrdExp =
                            *childCorespondingToTheParamGrdExpPtr;
                        const SemanticStatementGrounding* childCorespondingToTheParamStatGrdPtr =
                            childCorespondingToTheParamGrdExp->getStatementGroundingPtr();
                        if (childCorespondingToTheParamStatGrdPtr != nullptr) {
                            if (childCorespondingToTheParamStatGrdPtr->requests.empty())
                                return ReplacingParamStrategy::RETURN_PARAM;
                        } else {
                            const SemanticGenericGrounding* childCorespondingToTheParamGenGrdPtr =
                                childCorespondingToTheParamGrdExp->getGenericGroundingPtr();
                            if (childCorespondingToTheParamGenGrdPtr != nullptr) {
                                const SemanticGenericGrounding& childCorespondingToTheParamGenGrd =
                                    *childCorespondingToTheParamGenGrdPtr;
                                if (childCorespondingToTheParamGenGrd.referenceType
                                    == SemanticReferenceType::INDEFINITE) {
                                    if (ConceptSet::haveAConceptThatBeginWith(
                                            childCorespondingToTheParamGenGrd.concepts, "story"))
                                        return ReplacingParamStrategy::RETURN_AN_INSTANCE;
                                    else
                                        return ReplacingParamStrategy::RETURN_PARAM;
                                } else if (childCorespondingToTheParamGenGrd.referenceType
                                           == SemanticReferenceType::UNDEFINED) {
                                    return ReplacingParamStrategy::RETURN_PARAM;
                                }
                            } else
                                return ReplacingParamStrategy::RETURN_PARAM;
                ...
                switch (replacingParamStrategy) {
                    case ReplacingParamStrategy::RETURN_PARAM: {
                        pParams[metaGr.paramId].emplace(
                            metaGr.attibuteName,
                        ...
                        if (subWorkStruct.askForNewRecursion()) {
                            ...
                            if (subWorkStruct.compositeSemAnswers) {
                                ...
                                if (paramSemExp) {
                                    if (!pSubIntContext)
                                        pSubIntContext =
                                            subWorkStruct.compositeSemAnswers->getInteractionContextContainer();
                                    pParams[metaGr.paramId].emplace(metaGr.attibuteName, std::move(paramSemExp));
                                    paramHasBeenInserted = true;
                                }
                            }

                            if (!paramHasBeenInserted && subWorkStruct.reactOperator == SemanticOperatorEnum::GET
                                && childCorespondingToTheParamGrdExpPtr != nullptr) {
                                subWorkStruct.reactOperator = SemanticOperatorEnum::ANSWER;
                                SemanticMemoryBlockViewer subMemView(pMemViewer);
                                controller::applyOperatorOnSemExp(
                                    subWorkStruct,
                                    subMemView,
                    ...
                    case ReplacingParamStrategy::RETURN_AN_INSTANCE: {
                        pParams[metaGr.paramId].emplace(
                            metaGr.attibuteName,
                            std::make_unique<UniqueSemanticExpression>(
                                SemExpCreator::sayThatWeDontKnowAnInstanceOf(childCorespondingToTheParam)));
                        break;
                    }
                }
            }
...
std::unique_ptr<UniqueSemanticExpression> convertToActionToDo(const GroundedExpression*& pActionThatCannotBeDone,
                                                              const GroundedExpression& pActionGrdExp,
                                                              const SemControllerWorkingStruct& pWorkStruct,
                                                              SemanticMemoryBlockViewer& pMemViewer) {
    const auto* statGrdExpPtr = pActionGrdExp->getStatementGroundingPtr();
    if (statGrdExpPtr != nullptr && (statGrdExpPtr->isAtInfinitive() || statGrdExpPtr->isMandatoryInPresentTense())) {
        SemControllerWorkingStruct subWorkStruct(pWorkStruct);
        if (subWorkStruct.askForNewRecursion()) {
                    ...
                    if (leafAnswPtr != nullptr && leafAnswPtr->reaction) {
                        ...
                        if (reactionResourcePtr != nullptr) {
                            ...
                            converter::extractParameters(semResource->resource.parametersLabelsToValue,
                                                         ...
                                                         std::move(genericMandatoryForm),
                                                         pWorkStruct.lingDb);
                            return std::make_unique<UniqueSemanticExpression>(
                                std::make_unique<GroundedExpression>(std::move(semResource)));
                        }
...
bool checkIfMatchAndGetParams(IndexToSubNameToParameterValue& pParams,
                              ...
                              const SemControllerWorkingStruct& pWorkStruct,
                              const SemanticMemoryBlockViewer& pMemViewer) {
    if (!SemExpComparator::haveSamePolarity(pGrdExp, pSemMemSent.grdExp, pWorkStruct.lingDb.conceptSet, true))
        return false;
...
std::unique_ptr<UniqueSemanticExpression> _extartActionToDoInBackground(
    ...
    if (itBackgroundChild != pGrdExp.children.end()) {
        ...
        if (backgroundGrdExpPtr != nullptr) {
            return convertToActionToDo(pActionThatCannotBeDone, *backgroundGrdExpPtr, pWorkStruct, pMemViewer);
        }
    }
    return {};
}
...
bool splitCompeleteIncompleteOfActions(SemControllerWorkingStruct& pWorkStruct,
    ...
    if (!checkIfMatchAndGetParams(
            *params, subIntContext, &pIncompleteRelations, pMemSentWithParams.links, pGrdExp, pWorkStruct, pMemViewer))
        return false;
            ...
            if (receiverGrdExp->grounding().type == SemanticGroundingType::AGENT) {
                ...
                if (agentGrd.userId != SemanticAgentGrounding::me
                    ...
                    if (agentGrd.userId == SemanticAgentGrounding::userNotIdentified
                        ...
                        pWorkStruct.addConditionForAUserAnswer(ContextualAnnotation::NOTIFYSOMETHINGWILLBEDONE,
                                                               SemExpCreator::sayWeWillDoIt_fromGrdExp(pGrdExp),
                                                               ConditionForAUser(agentGrd.userId, pGrdExp.clone()));
                        return true;
                    }
                }
            } else {
    ...
    for (const auto& currListElts : SemExpGetter::iterateOnList(infCommandToDo)) {
        const GroundedExpression* listEltGrdExpPtr = currListElts->getGrdExpPtr_SkipWrapperPtrs();
        if (listEltGrdExpPtr != nullptr) {
            const SemanticStatementGrounding* statGrdExpPtr = listEltGrdExpPtr->grounding().getStatementGroundingPtr();
            if (statGrdExpPtr != nullptr && !statGrdExpPtr->isAtInfinitive()
                && !statGrdExpPtr->isMandatoryInPresentTense()) {
                if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT && pWorkStruct.author != nullptr) {
                    pWorkStruct.addAnswerWithoutReferences(
                        ...
                        SemExpCreator::mergeInAList(
                            SemExpCreator::sayThatTheRobotIsNotAbleToDoIt(pGrdExp),
    ...
    if (!answerGenerated) {
        ...
        for (const auto& currListElts : SemExpGetter::iterateOnList(actionSemExp)) {
            GroundedExpression* listEltGrdExpPtr = (*currListElts)->getGrdExpPtr_SkipWrapperPtrs();
            if (listEltGrdExpPtr != nullptr) {
                if (!hasGrdExpOutputInformations(*listEltGrdExpPtr)) {
                    auto actionToDoPtr =
                        convertToActionToDo(actionThatCannotBeDone, *listEltGrdExpPtr, pWorkStruct, pMemViewer);
                ...
                auto actionToDoInBackground =
                    _extartActionToDoInBackground(actionThatCannotBeDone, *listEltGrdExpPtr, pWorkStruct, pMemViewer);
                if (actionThatCannotBeDone != nullptr)
                    break;
                if (actionToDoInBackground) {
                    listEltGrdExpPtr->children.erase(GrammaticalType::IN_BACKGROUND);
                    listEltGrdExpPtr->children.emplace(GrammaticalType::IN_BACKGROUND,
                                                       std::move(*actionToDoInBackground));
                }
            }
        }
        ...
        if (!answerGenerated && actionThatCannotBeDone == nullptr)
            actionToDoInBackground =
                _extartActionToDoInBackground(actionThatCannotBeDone, pGrdExp, pWorkStruct, pMemViewer);
        ...
        if (!answerGenerated) {
            if (behavLanguage != SemanticLanguageEnum::UNKNOWN || numberOfExcutionsSemExpPtr != nullptr
                || durationSemExpPtr != nullptr || actionToDoInBackground) {
                auto newAnnExp = std::make_unique<AnnotatedExpression>(std::move(actionSemExp));
                if (behavLanguage != SemanticLanguageEnum::UNKNOWN)
                    newAnnExp->annotations.emplace(GrammaticalType::LANGUAGE,
                                                   std::make_unique<GroundedExpression>(
                                                       std::make_unique<SemanticLanguageGrounding>(behavLanguage)));
                if (numberOfExcutionsSemExpPtr != nullptr)
                    newAnnExp->annotations.emplace(GrammaticalType::REPETITION, numberOfExcutionsSemExpPtr->clone());
                if (durationSemExpPtr != nullptr)
                    newAnnExp->annotations.emplace(GrammaticalType::DURATION, durationSemExpPtr->clone());
                if (actionToDoInBackground)
                    newAnnExp->annotations.emplace(GrammaticalType::IN_BACKGROUND, std::move(*actionToDoInBackground));
            ...
            pWorkStruct.addAnswer(ContextualAnnotation::BEHAVIOR, std::move(cmdExp), ReferencesFiller(contextAxiom));
...
bool getRequestToAskForPrecision(SemanticRequestType& pRequestType, const GrdKnowToUnlinked& pIncompleteRelations) {
    ...
    for (GrdKnowToUnlinked::const_iterator itIncRel = pIncompleteRelations.begin();
         itIncRel != pIncompleteRelations.end();
         ++itIncRel) {
...
bool isItfUnknown(const SemControllerWorkingStruct& pWorkStruct,
    ...
    if (subWorkStruct.askForNewRecursion()) {
        subWorkStruct.reactOperator = SemanticOperatorEnum::GET;
        SemanticMemoryBlockViewer subMemView(pMemViewer);
        controller::applyOperatorOnGrdExp(
            subWorkStruct, subMemView, pGrdExp, std::list<const GroundedExpression*>(), pGrdExp);
