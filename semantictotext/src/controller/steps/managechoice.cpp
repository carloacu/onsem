...
void _addNeitherInterjection(SemAnswer& pSemAnswer,
                             std::size_t pNbOfElts)
{
                  ...
                  const ListExpression& pChoiceListExp)
{
  ...
  case SemanticOperatorEnum::ANSWER:
  {
    ...
    if (!falseAnwers->isEmpty())
    {
      ...
      pWorkStruct.addAnswer(ContextualAnnotation::ANSWER, std::move(falseAnwers),
                            ReferencesFiller(relatedContextAxiom));
      return true;
    }
    break;
