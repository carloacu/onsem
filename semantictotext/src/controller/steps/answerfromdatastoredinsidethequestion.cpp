        ...
        switch (request) {
            case SemanticRequestType::TIME: {
                ...
                if (itSubject != pGrdExp.children.end()) {
                    const SemanticExpression* timeSemExpPtr =
                        SemExpGetter::getGrammTypeInfo(*itSubject->second, GrammaticalType::TIME);
                        ...
                        if (subjectGrdExpPtr != nullptr) {
                            ...
                            if (itObject != pGrdExp.children.end()) {
                                const GroundedExpression* objectGrdExpPtr =
                                    itObject->second->getGrdExpPtr_SkipWrapperPtrs();
                                if (objectGrdExpPtr != nullptr) {
                                    ...
                                    if (itSpecificChild != objectGrdExpPtr->children.end()) {
                                        const GroundedExpression* specificChildGrdExpPtr =
                                            itSpecificChild->second->getGrdExpPtr_SkipWrapperPtrs();
                                        if (specificChildGrdExpPtr != nullptr
                                            && SemExpComparator::isAnInstanceOf(
                                                *specificChildGrdExpPtr, *subjectGrdExpPtr, pLingDb)) {
                                            pAllAnswers[pRootRequest].answersGenerated.emplace_back(
                                                specificChildGrdExpPtr->clone());
                                        }
                                    }
                                }
                            }
