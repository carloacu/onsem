...
void _filterRevelantSimilaritiesForDifferentTimePrecision(GroundedExpression& pGrdExp,
    ...
    if (!pAnswers.empty() || pRequests.has(SemanticRequestType::TIME))
        return;
    ...
    if (haveYearInfo && haveDayInfo && SemExpModifier::removeDayInformation(*timeWithoutYear)) {
...
void _getRevelantSimilaritiesForDifferentTense(GroundedExpression& pGrdExp,
    ...
    _filterRevelantSimilaritiesForDifferentTimePrecision(pGrdExp, pAnswers.answers, pWorkStruct, pMemViewer, pRequest);
    if (pAnswers.answers.empty()) {
        if (isAPresentTense(pAnswers.inputVerbTense)) {
            ...
            _filterRevelantSimilaritiesForDifferentTimePrecision(
                pGrdExp, pAnswers.answers, pWorkStruct, pMemViewer, pRequest);
...
bool _reactToSimilarAnswers(SemControllerWorkingStruct& pWorkStruct,
    ...
    for (const auto& currAnsw : pAnswers) {
        if (!SemExpComparator::grdExpsReferToSameInstance(
                currAnsw->getGrdExp(), pSubjectGrd, pMemViewer.constView, pWorkStruct.lingDb)) {
            ...
            SemExpModifier::addChild(
                *pNewRootExp, pChildGrammaticalType, std::move(answerClonedGrdExp), ListExpressionType::AND);
            ++numberOfAnswers;
        }
    }
    ...
    if (pOtherChildGrammaticalType != GrammaticalType::UNKNOWN && numberOfAnswers > 1) {
    ...
    SemExpModifier::addChild(*pNewRootExp,
    ...
    if (subWorkStruct.askForNewRecursion()) {
        ...
        if (allAnswers.answers.empty()) {
            if (newRootExp->children.count(GrammaticalType::LOCATION) != 0
                ...
                _getRevelantSimilaritiesForDifferentTense(
                    *newRootExp, locationAnswers, subWorkStruct, pMemViewer, pRequest);
                if (!locationAnswers.answers.empty()) {
                    ...
                    _getRevelantSimilaritiesForDifferentTense(
                        *newRootExp, allAnswers, subWorkStruct, pMemViewer, pRequest);
                }
            }
        }
        ...
        if (!allAnswers.answers.empty()) {
            ...
            return _reactToSimilarAnswers(pWorkStruct,
                                          pMemViewer,
                                          std::move(newRootExp),
                                          pGrdExp,
                                          allAnswers.answers,
                                          childGramType,
                                          otherChildGrammaticalType,
                                          childGrdExp);
        }
    }
    return false;
