...
struct SemControllerWorkingStruct {
    SemControllerWorkingStruct(InformationType pInformationType,
                               ...
                               const std::list<mystd::unique_propagate_const<MemBlockAndExternalCallback>>*
    ...
    SemControllerWorkingStruct(const SemControllerWorkingStruct& pOther);
    ...
    void getAnswersForRequest(std::list<const AnswerExp*>& pAnswers, const SemanticRequests& pRequests) const;

    void addAnswerWithoutReferences(ContextualAnnotation pType, UniqueSemanticExpression pReaction);
    ...
    bool addAnswers(ListExpressionType pListExpType, SemControllerWorkingStruct& pOther);
    ...
    const GroundedExpression* getOriginalGrdExpPtr() const;
