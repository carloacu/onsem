...
void SemControllerWorkingStruct::getAnswersForRequest(std::list<const AnswerExp*>& pAnswers,
...
void SemControllerWorkingStruct::addAnswerWithoutReferences(ContextualAnnotation pType,
                                                            UniqueSemanticExpression pReaction) {
    contAnnotationOfPreviousAnswers = pType;
    compositeSemAnswers->semAnswers.emplace_back(std::make_unique<LeafSemAnswer>(pType, std::move(pReaction)));
}

void SemControllerWorkingStruct::addQuestion(UniqueSemanticExpression pReaction) {
    addAnswerWithoutReferences(ContextualAnnotation::QUESTION, std::move(pReaction));
}

void SemControllerWorkingStruct::addAnswer(ContextualAnnotation pType,
                                           UniqueSemanticExpression pReaction,
...
void SemControllerWorkingStruct::addConditionForAUserAnswer(ContextualAnnotation pType,
                                                            UniqueSemanticExpression pReaction,
...
bool SemControllerWorkingStruct::addAnswers(SemControllerWorkingStruct& pOther) {
...
bool SemControllerWorkingStruct::addAnswers(ListExpressionType pListExpType, SemControllerWorkingStruct& pOther) {
    ...
    if (compositeSemAnswers->listType == pListExpType && pOther.compositeSemAnswers->listType == pListExpType) {
    ...
    if (pOther.compositeSemAnswers->semAnswers.size() == 1
        && (compositeSemAnswers->semAnswers.size() <= 1 || compositeSemAnswers->listType == pListExpType)) {
        compositeSemAnswers->listType = pListExpType;
        compositeSemAnswers->semAnswers.splice(compositeSemAnswers->semAnswers.end(),
                                               pOther.compositeSemAnswers->semAnswers);
        return true;
    ...
    if (compositeSemAnswers->semAnswers.size() == 1) {
        compositeSemAnswers->listType = pListExpType;
        compositeSemAnswers->semAnswers.emplace_back(std::move(pOther.compositeSemAnswers));
        return true;
    }
    ...
    if (pOther.compositeSemAnswers->semAnswers.size() == 1) {
        ...
        compSemExp->semAnswers.splice(compSemExp->semAnswers.end(), pOther.compositeSemAnswers->semAnswers);
    ...
    auto compSemExp = std::make_unique<CompositeSemAnswer>(pListExpType);
    compSemExp->semAnswers.emplace_back(std::move(compositeSemAnswers));
    compSemExp->semAnswers.emplace_back(std::move(pOther.compositeSemAnswers));
    compositeSemAnswers = std::move(compSemExp);
    return true;
}
    ...
    if (reactionOptions.reactWithTextAndResource) {
        ...
        for (const auto& currGrdExpAnswer : grdExpAnswers) {
            if (SemExpGetter::isAResourceFromGrdExp(*currGrdExpAnswer)) {
                ...
                if (hasResourceAnswer || SemExpGetter::isGrdExpAQuestion(*currGrdExpAnswer))
                    return true;
...
const GroundedExpression& SemControllerWorkingStruct::getOriginalGrdExp(
    const GroundedExpression& pDefaultGrdExp) const {
...
SemControllerWorkingStruct::SemControllerWorkingStruct(
    ...
    comparisonExceptions.interpretations =
        true;    // We do not consider the interpretations from InterpretationExpression
}

SemControllerWorkingStruct::SemControllerWorkingStruct(const SemControllerWorkingStruct& pOther)
    : isAtRoot(false)
