                             ...
                             const GroundedExpression& pGrdExp)
{
  ...
  if (statGrdPtr == nullptr ||
      statGrdPtr->concepts.find("mentalState_know") == statGrdPtr->concepts.end())
    return false;

  const GroundedExpression* grdExpSubjectPtr =
  ...
  if (subjAgentGrdPtr == nullptr ||
      subjAgentGrdPtr->userId != SemanticAgentGrounding::me)
    return false;
  ...
  if (subWorkStruct.askForNewRecursion())
  {
        ...
        pWorkStruct.reactOperator == SemanticOperatorEnum::ANSWER)
    {
      pWorkStruct.addAnswer(ContextualAnnotation::ANSWER,
                            SemExpCreator::generateYesOrNoAnswerFromQuestion(pGrdExp, answerPolarity,
    ...
    if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT)
    {
      ...
      if (doTheRobotCanDoTheAction)
      {
        ...
        pWorkStruct.addQuestion
            (SemExpCreator::askDoYouWantMeToDoItNow(*pWorkStruct.author, cmdGrdExp));
      }
...
bool _tryToAnswerToBeVerbNameQuestion(const GroundedExpression& pGrdExp,
                                                               ...
                                                               const std::string&)>& pAnswerNameQuestion)
{
  const GroundedExpression* grdExpSubjectPtr =
      SemExpGetter::getGrdExpChild(pGrdExp, GrammaticalType::SUBJECT);
  if (grdExpSubjectPtr == nullptr ||
      !ConceptSet::haveAConcept(grdExpSubjectPtr->grounding().concepts, "name"))
    return false;
  ...
  if (ownerGrdExpPtr != nullptr)
  {
    ...
    if (ownerAgentGrdPtr != nullptr)
      return pAnswerNameQuestion(ownerAgentGrdPtr->userId,
                                 grdExpSubjectPtr->clone(), ConceptSet::conceptVerbEquality);
  }
  return false;
}


bool _tryToAnswerToHasNameVerbNameQuestion(const GroundedExpression& pGrdExp,
                                           const std::function<bool(const std::string&,
                                                                    UniqueSemanticExpression,
                                                                    const std::string&)>& pAnswerNameQuestion)
{
  ...
  if (subjectAgentGrdPtr != nullptr)
    return pAnswerNameQuestion(subjectAgentGrdPtr->userId,
                               grdExpSubjectPtr->clone(), "predicate_hasName");
  return false;
}


bool _tryToAnswerToNameQuestion(const GroundedExpression& pGrdExp,
                                const std::function<bool(const std::string&,
                                                         UniqueSemanticExpression,
                                                         const std::string&)>& pAnswerNameQuestion)
{
                      ...
                      const SemanticExpression& pObjectSemExp)
{
  ...
  if (pWorkStruct.reactOperator == SemanticOperatorEnum::ANSWER ||
      pWorkStruct.reactOperator == SemanticOperatorEnum::REACT)
  {
    UniqueSemanticExpression answerSemExp =
        SemExpCreator::generateYesOrNoAnswer(
          SemExpCreator::sentenceFromTriple(pSubjectSemExp.clone(),
                                            ConceptSet::conceptVerbEquality,
                                            std::move(nameSemExp)), !pStatementIsPositive);
    pWorkStruct.addAnswer(ContextualAnnotation::ANSWER, std::move(answerSemExp),
                          ReferencesFiller(relContextAxiom));
  }
  else
  {
                ...
                const GroundedExpression& pGrdExp)
{
  ...
  if (ConceptSet::haveAConcept(statGrdPtr->concepts, ConceptSet::conceptVerbEquality))
  {
    const SemanticExpression* subjectSemExpPtr = SemExpGetter::getChildFromGrdExp(pGrdExp, GrammaticalType::SUBJECT);
    ...
    if (objectSemExpPtr == nullptr)
      return false;
    return _checkNameBeVerb(pWorkStruct, pMemViewer, pGrdExp, statGrdPtr->polarity,
                            *subjectSemExpPtr, *objectSemExpPtr) ||
        _checkNameBeVerb(pWorkStruct, pMemViewer, pGrdExp, statGrdPtr->polarity,
                         *objectSemExpPtr, *subjectSemExpPtr);
  }
  if (ConceptSet::haveAConcept(statGrdPtr->concepts, "predicate_hasName"))
  {
    const SemanticExpression* subjectSemExpPtr = SemExpGetter::getChildFromGrdExp(pGrdExp, GrammaticalType::SUBJECT);
                                                          ...
                                                          const std::string&)>& pAnswerNameQuestion)
{
  ...
  if (objectGenGrdPtr == nullptr ||
      !ConceptSet::haveAnyOfConcepts(objectGenGrdPtr->concepts, {"agent", "name"}))
    return false;

  const SemanticStatementGrounding* statGrdPtr = pGrdExp->getStatementGroundingPtr();
  if (statGrdPtr == nullptr ||
      !ConceptSet::haveAConcept(statGrdPtr->concepts, ConceptSet::conceptVerbEquality))
    return false;
  ...
  const auto* subjectAgentGrdPtr = grdExpSubjectPtr->grounding().getAgentGroundingPtr();
  if (subjectAgentGrdPtr != nullptr)
    return pAnswerNameQuestion(subjectAgentGrdPtr->userId, grdExpSubjectPtr->clone(),
                               ConceptSet::conceptVerbEquality);
  return false;
}


bool _iterateOnBornTimes(SemControllerWorkingStruct& pWorkStruct,
                         SemanticMemoryBlockViewer& pMemViewer,
                         UniqueSemanticExpression pUserSemExp,
                         const std::function<bool(const SemanticTimeGrounding&,
                                                  SemControllerWorkingStruct&,
                                                  const AnswerExp&,
                                                  UniqueSemanticExpression&)>& pOnEachTimeGrd)
{
  SemControllerWorkingStruct subWorkStruct(pWorkStruct);
  if (subWorkStruct.askForNewRecursion())
  {
    ...
    subWorkStruct.reactOperator = SemanticOperatorEnum::GET;
    semanticMemoryLinker::satisfyAQuestion(subWorkStruct, pMemViewer, *newRootExp, {}, *newRootExp,
                                           timeRequest);
                                 ...
                                 const GroundedExpression& pGrdExp)
{
  ...
  const GroundedExpression* grdExpSubject =
      SemExpGetter::getGrdExpChild(pGrdExp, GrammaticalType::SUBJECT);
  if (grdExpSubject == nullptr ||
      !ConceptSet::haveAConcept(grdExpSubject->grounding().concepts, "age"))
    return false;
  auto itOwner = grdExpSubject->children.find(GrammaticalType::OWNER);
  if (itOwner == grdExpSubject->children.end())
    return false;

  return _iterateOnBornTimes(pWorkStruct, pMemViewer, itOwner->second->clone(),
                                     ...
                                     const GroundedExpression& pGrdExp)
{
                             ...
                             UniqueSemanticExpression&)
  {
    ...
    if (!timeGrd->date.empty())
    {
      ...
      {
        pWorkStruct.addAnswer(ContextualAnnotation::ANSWER,
                              SemExpCreator::saySemxExp1IsSemExp2(semExpSubject.clone(),
                                                                  std::move(timeGrdExp)),
                              ReferencesFiller(pAnswExp.relatedContextAxioms));
      }
      else
                                ...
                                const GroundedExpression& pGrdExp)
{
  ...
  if (objectStatPtr == nullptr ||
      objectStatPtr->verbTense != SemanticVerbTense::UNKNOWN)
    return false;
  ...
  pWorkStruct.addAnswerWithoutReferences
      (ContextualAnnotation::ANSWER,
       SemExpCreator::saySemxExp1IsSemExp2(objectSemExp.clone(),
                                           infCommandToDo->clone()));
  return true;
}
                                      ...
                                      const GroundedExpression& pGrdExp)
{
  ...
  if (grdExpSubjectPtr != nullptr)
  {
    ...
    if (agentSubject != nullptr &&
        agentSubject->userId == SemanticAgentGrounding::me)
    {
                      ...
                      for (const auto& currAnswerElts : leafPtr->answerElts)
                      {
                        ...
                        if (!answerElt.answersGenerated.empty())
                        {
                          pWorkStruct.addAnswer
                              (ContextualAnnotation::ANSWER,
                               SemExpCreator::formulateHowWeKnowSomething(*itObject->second,
                                            ...
                                            const GroundedExpression& pGrdExp)
{
  ...
  if (statGrdPtr == nullptr ||
      statGrdPtr->verbGoal != VerbGoalEnum::ABILITY ||
      statGrdPtr->verbTense != SemanticVerbTense::PRESENT ||
      statGrdPtr->concepts.count("verb_action_teach") == 0)
    return false;

  auto itSubject = pGrdExp.children.find(GrammaticalType::SUBJECT);
  if (itSubject == pGrdExp.children.end() ||
      pWorkStruct.author == nullptr ||
                                  ...
                                  const std::string& pVerbConcept)
{
  ...
  {
    pWorkStruct.addAnswer(ContextualAnnotation::ANSWER,
                          SemExpCreator::sentenceFromTriple(std::move(pSubjectSemExp),
                                                            pVerbConcept,
                                                            std::move(nameSemExp)),
                          ReferencesFiller(relContextAxiom));
  }
             ...
             SemanticRequestType pRequestType)
{
  if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT ||
      ...
      pWorkStruct.reactOperator == SemanticOperatorEnum::GET)
  {
    auto answerToNameQuestion = [&](const std::string& pUserId,
        UniqueSemanticExpression pSubjectSemExp,
        const std::string& pVerbConcept)
    {
      auto& memBlockPrivate = pMemViewer.getConstViewPrivate();
      return _tryToAnswerTheNameOfAUserId(pWorkStruct, memBlockPrivate, pUserId, std::move(pSubjectSemExp), pVerbConcept);
    };
