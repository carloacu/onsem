...
void _incrementNotDefiniteQuantitiesFromGrdExps(
    GRDEXP& pGrdExpRes,
...
void _incrementQuantities(SEMEXP& pSemExpRes,
    ...
    if (grdExpResPtr != nullptr) {
        ...
        if (grdExpPtr != nullptr)
            _incrementNotDefiniteQuantitiesFromGrdExps(
                *grdExpResPtr, *grdExpPtr, pMemBlock, pLingDb, pAlreadyCountedElts);
    }
}
...
void _incrementNotDefiniteQuantitiesFromGrdExps(
    ...
    if (genGrdResPtr != nullptr && genGrdResPtr->referenceType != SemanticReferenceType::DEFINITE
        ...
        if (nb) {
            ...
            if (itForCurrGenGrd != pAlreadyCountedElts.end()) {
                for (auto& currAlreadyCountedElt : itForCurrGenGrd->second) {
                    ...
                    auto imbrication = SemExpComparator::getGrdExpsImbrications(
                        pGrdExp, *currAlreadyCountedElt.first, pMemBlock, pLingDb, &compExceptions);
                    if (imbrication == ImbricationType::EQUALS || imbrication == ImbricationType::LESS_DETAILED
...
bool _doActions(SemControllerWorkingStruct& pWorkStruct,
    ...
    auto memSentenceIsTrue = [&](const GroundedExpWithLinks& pMemSent) {
        if (pWorkStruct.expHandleInMemory != nullptr || pWorkStruct.reactOperator == SemanticOperatorEnum::UNINFORM) {
            SemControllerWorkingStruct subWorkStruct(pWorkStruct);
            if (subWorkStruct.askForNewRecursion()) {
            ...
            ImbricationType imbr = SemExpComparator::getGrdExpsImbrications(
                *pGrdExpPtr, pMemSent.grdExp, pMemViewer.constView, pWorkStruct.lingDb, nullptr);
            if (imbr == ImbricationType::EQUALS || imbr == ImbricationType::ISCONTAINED
                || imbr == ImbricationType::MORE_DETAILED || imbr == ImbricationType::HYPONYM)
                return TruenessValue::VAL_TRUE;
    ...
    for (auto& currSent : pSentsWithAction.dynamicLinks) {
        ...
        if (semTrackerOpt) {
            ...
            if (requestsPtr != nullptr && !requestsPtr->empty() && !requestsPtr->has(SemanticRequestType::YESORNO)
                && pGrdExpPtr != nullptr
            ...
            switch (newTruenessValue) {
                ...
                case TruenessValue::VAL_FALSE: {
                    if (semTrackerOpt) {
                        SemanticTracker& semTracker = **semTrackerOpt;
                        const auto* requestsPtr = SemExpGetter::getRequestList(memSent.grdExp);
                        if (requestsPtr != nullptr && requestsPtr->has(SemanticRequestType::YESORNO))
...
bool _checkGlobalCondition(const SemControllerWorkingStruct& pWorkStruct,
    ...
    if (contextAxiom.memorySentences.elts.size() > 1 && contextAxiom.memorySentences.and_or) {
        bool res = true;
        for (const GroundedExpWithLinks& currElt : contextAxiom.memorySentences.elts) {
            if (&currElt == &pCurrMemSent || !currElt.isAConditionToSatisfy() || currElt.isANoun()
...
bool _areTimeInformationIncompatable(const SemControllerWorkingStruct& pWorkStruct,
    ...
    if (pFormRequest != SemanticRequestType::TIME && pFromGrdExpQuestion != nullptr
        ...
        if (questionStatGrdPtr != nullptr && questionStatGrdPtr->verbTense == SemanticVerbTense::PRESENT) {
...
void _filterWithOccurenceRank(std::map<semIdAbs, ANSWERTYPE>& pRes, const SemanticExpression& pOccurenceRankFilter) {
...
void _filterTimeImpossibilities(std::map<intSemId, MEM_SENTENCE_PTR>& pRes,
    ...
    for (auto itMemSent = pRes.begin(); itMemSent != pRes.end();) {
        auto& currMemSent = *itMemSent->second;
        if (!_areTimeInformationIncompatable(
                pWorkStruct, pMemViewer, currMemSent.getGrdExpRef(), pFromGrdExpQuestion, pFormRequest))
            ++itMemSent;
        else
            itMemSent = pRes.erase(itMemSent);
    }
...
void _getResultFromMemory(RelationsThatMatch<IS_MODIFIABLE>& pRes,
    ...
    semanticMemoryGetter::getResultFromMemory(pRes,
                                              pReqToList,
                                              pReqLinks,
                                              pRequestContext,
                                              pMemBlockPrivatePtr,
...
bool _addTriggerThatMatchTheMost(SemControllerWorkingStruct& pWorkStruct,
    ...
    if (pSemExpWrapperPtrsSize == 1) {
        ...
        return _addTriggerAnswer(
            pWorkStruct, pAnAnswerHasBeenAdded, *beginElt.first, beginElt.second, pMemViewer, pContAnnotation);
    }

    if (pSemExpWrapperPtrsSize > 1) {
        std::map<int, std::map<intSemId, std::pair<const ExpressionWithLinks* const, ParametersLabelsToValue>*>>
            similarityCoef;
        for (auto& currRel : pSemExpWrapperPtrs)
            similarityCoef[getSimilarityCoef(pInputSemExp, *currRel.first->semExp)].emplace(
                currRel.first->getIdOfFirstSentence(), &currRel);

        bool res = false;
        for (auto itExp = similarityCoef.rbegin(); itExp != similarityCoef.rend(); ++itExp) {
            for (auto& currExpHandleInMemory : itExp->second) {
                if (_addTriggerAnswer(pWorkStruct,
                                      pAnAnswerHasBeenAdded,
...
bool _evaluateMatchingAndExtractParameters(GroundedExpWithLinksWithParameters& pMatchCandidate,
    ...
    auto& outputToAnswerIfTriggerHasMatched =
        contextAxiom.getSemExpWrappedForMemory().outputToAnswerIfTriggerHasMatched;
    ...
    if (reactionResourcePtr != nullptr) {
        ...
        if (!reactionResource.parameterLabelsToQuestions.empty()) {
            ...
            converter::extractParameters(pMatchCandidate.parametersLabelsToValue,
                                         ...
                                         std::move(genericMandatoryFormUSemExp),
                                         pLingDb);
            ...
            if (pMatchCandidate.parametersLabelsToValue.empty()) {
                ...
                if (SemExpGetter::hasChild(pInputGrdExp, GrammaticalType::OBJECT)
                    && !SemExpGetter::hasChildOtherThanParameterToFill(pMatchCandidate.links.grdExp,
...
void _orderResultBySimilarity(
    ...
    std::map<SemExpComparator::ComparisonErrorsCoef,
             std::map<std::size_t, std::list<GroundedExpWithLinksWithParameters>>>&
        pNbOfErrorsToNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs,
    std::map<SemExpComparator::ComparisonErrorsCoef,
             std::map<std::size_t, std::list<GroundedExpWithLinksWithParameters>>>* pIncompleteLinksPtrs,
    ...
    for (const auto& currRel : pDynamicLinks) {
        ...
        auto imbrication = SemExpComparator::getGrdExpsImbrications(pInputGrdExp,
                                                                    memSent.grdExp,
                                                                    pMemViewer.constView,
                                                                    pWorkStruct.lingDb,
                                                                    &pWorkStruct.comparisonExceptions,
        ...
        if (imbrication == ImbricationType::EQUALS) {
            ...
            if (SemExpComparator::haveSamePolarity(pInputGrdExp, memSent.grdExp, pWorkStruct.lingDb.conceptSet, true)) {
                ...
                if (similarityValue == SemExpComparator::ComparisonErrorReporting::SmimilarityValue::YES)
                    pNbOfErrorsToNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs
                        [comparisonErrorReporting.getErrorCoef()][comparisonErrorReporting.numberOfEqualities]
                            .emplace_back(memSent);
                else if (pIncompleteLinksPtrs != nullptr
                         && similarityValue
                                == SemExpComparator::ComparisonErrorReporting::SmimilarityValue::YES_BUT_INCOMPLETE)
                    (*pIncompleteLinksPtrs)[comparisonErrorReporting.getErrorCoef()]
                                           [comparisonErrorReporting.numberOfEqualities]
                                               .emplace_back(memSent);
            }
        }
    }
...
void _matchAnyTrigger(std::map<const ExpressionWithLinks*, ParametersLabelsToValue>& pSemExpWrapperPtrs,
                      SemControllerWorkingStruct& pWorkStruct,
                      SemanticMemoryBlockViewer& pMemViewer,
    ...
    _getResultFromMemory(idsToSentences,
                         pReqToGrdExps,
                         pWorkStruct,
                         memBlockPrivateAccessor,
                         pMemViewer,
                         pReqLinks,
                         true,
                         semanticMemoryGetter::RequestContext::SENTENCE,
                         nullptr,
                         SemanticRequestType::NOTHING,
                         true,
                         true);
    ...
    std::map<SemExpComparator::ComparisonErrorsCoef,
             std::map<std::size_t, std::list<GroundedExpWithLinksWithParameters>>>
        nbOfErrorsToNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs;
    _orderResultBySimilarity(equalsGroundedExpWithLinksPtrs,
                             nbOfErrorsToNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs,
                             nullptr,
                             pWorkStruct,
                             pMemViewer,
                             idsToSentences.res.dynamicLinks,
                             pInputGrdExp);

    if (!equalsGroundedExpWithLinksPtrs.empty()) {
        for (auto& currElt : equalsGroundedExpWithLinksPtrs)
            pSemExpWrapperPtrs.emplace(&currElt.links.getContextAxiom().getSemExpWrappedForMemory(),
                                       std::move(currElt.parametersLabelsToValue));
...
void _matchTriggerSentences(std::map<const ExpressionWithLinks*, ParametersLabelsToValue>& pSemExpWrapperPtrs,
                            SemControllerWorkingStruct& pWorkStruct,
                            SemanticMemoryBlockViewer& pMemViewer,
                            const RequestLinks& pReqLinks,
    ...
    if (awLinks != nullptr) {
        ...
        _matchAnyTrigger(pSemExpWrapperPtrs, pWorkStruct, pMemViewer, linksForVerb, pReqLinks, pInputGrdExp);
    ...
    if (pMemViewer.constView.subBlockPtr != nullptr) {
        ...
        _matchTriggerSentences(
            pSemExpWrapperPtrs, pWorkStruct, subMemView, pReqLinks, pExpCategory, pAxiomId, pInputGrdExp);
    }
}

void _matchNominalGroupTrigger(std::map<const ExpressionWithLinks*, ParametersLabelsToValue>& pSemExpWrapperPtrs,
                               SemControllerWorkingStruct& pWorkStruct,
                               SemanticMemoryBlockViewer& pMemViewer,
                               const RequestLinks& pReqLinks,
                               const SemanticTriggerAxiomId& pAxiomId,
    ...
    if (recoLinks != nullptr) {
        ...
        _matchAnyTrigger(pSemExpWrapperPtrs, pWorkStruct, pMemViewer, reqToLinks, pReqLinks, pInputGrdExp);
        if (!pSemExpWrapperPtrs.empty())
            return;
    }
    if (pMemViewer.constView.subBlockPtr != nullptr) {
        SemanticMemoryBlockViewer subMemView(nullptr, *pMemViewer.constView.subBlockPtr, pMemViewer.currentUserId);
        _matchNominalGroupTrigger(pSemExpWrapperPtrs, pWorkStruct, subMemView, pReqLinks, pAxiomId, pInputGrdExp);
    }
}

void _matchGrdExpTrigger(std::map<const ExpressionWithLinks*, ParametersLabelsToValue>& pSemExpWrapperPtrs,
                         SemControllerWorkingStruct& pWorkStruct,
                         SemanticMemoryBlockViewer& pMemViewer,
                         const RequestLinks& pReqLinks,
                         SemanticExpressionCategory pExpCategory,
    ...
    if (pExpCategory == SemanticExpressionCategory::NOMINALGROUP)
        _matchNominalGroupTrigger(pSemExpWrapperPtrs, pWorkStruct, pMemViewer, pReqLinks, pAxiomId, pInputGrdExp);
    else
        _matchTriggerSentences(
            pSemExpWrapperPtrs, pWorkStruct, pMemViewer, pReqLinks, pExpCategory, pAxiomId, pInputGrdExp);
}

bool _addTriggerGrdExps(SemControllerWorkingStruct& pWorkStruct,
                        bool& pAnAnswerHasBeenAdded,
                        SemanticMemoryBlockViewer& pMemViewer,
    ...
    for (const auto* currGrdExpPtr : pGrdExpPtrs) {
        ...
        _matchGrdExpTrigger(
            subSemExpWrapperPtrs, pWorkStruct, pMemViewer, reqLinks, semExpCategory, axiomId, currGrdExp);
    ...
    return _addTriggerThatMatchTheMost(
        pWorkStruct, pAnAnswerHasBeenAdded, semExpWrapperPtrs, pMemViewer, ContextualAnnotation::ANSWER, pSemExp);
}
...
bool _handleActionRelations(SentenceLinks<false>& pIdsToSentences,
    ...
    _orderResultBySimilarity(equalsGroundedExpWithLinksPtrs,
                             ...
                             pWorkStruct,
                             pMemViewer,
                             pIdsToSentences.dynamicLinks,
                             pGrdExp);

    for (auto itRel = equalsGroundedExpWithLinksPtrs.rbegin(); itRel != equalsGroundedExpWithLinksPtrs.rend();
         ++itRel) {
        auto& currRel = *itRel;
        if (unknownInfosGetter::splitCompeleteIncompleteOfActions(
                pWorkStruct, pMemViewer, incompleteRelations, currRel, pGrdExp))
            return true;
    }

    auto considerErrorToSimilarity = [&](std::map<SemExpComparator::ComparisonErrorsCoef,
                                                  std::map<std::size_t, std::list<GroundedExpWithLinksWithParameters>>>&
                                             pErrorCoefToGrdExpWithLinks) {
        for (auto& currNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs : pErrorCoefToGrdExpWithLinks) {
            for (auto itLowPriorityGrdExpWithLinksPtrs =
                     currNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs.second.rbegin();
                 itLowPriorityGrdExpWithLinksPtrs != currNbOfEqualitiesToLowPriorityGrdExpWithLinksPtrs.second.rend();
                 ++itLowPriorityGrdExpWithLinksPtrs) {
                for (auto itRel = itLowPriorityGrdExpWithLinksPtrs->second.rbegin();
                     itRel != itLowPriorityGrdExpWithLinksPtrs->second.rend();
                     ++itRel) {
                    auto& currRel = *itRel;
                    if (unknownInfosGetter::splitCompeleteIncompleteOfActions(
                            pWorkStruct, pMemViewer, incompleteRelations, currRel, pGrdExp))
...
bool _getActionFromAMemblock(SemControllerWorkingStruct& pWorkStruct,
    ...
    _getResultFromMemory(idsToSentences,
                         ...
                         pReqLinks,
                         false,
                         semanticMemoryGetter::RequestContext::COMMAND,
                         nullptr,
                         SemanticRequestType::NOTHING,
                         false);

    if (!idsToSentences.empty() && _handleActionRelations(idsToSentences.res, pWorkStruct, pMemViewer, pGrdExp))
        return true;
...
const SemanticExpression* _getActionDefFromAMemblock(SemControllerWorkingStruct& pWorkStruct,
                                                     SemanticMemoryBlockViewer& pMemViewer,
                                                     const RequestLinks& pReqLinks,
                                                     const GroundedExpression& pGrdExp) {
    auto& memBlockPrivateViewer = pMemViewer.getConstViewPrivate();
...
enum class LinkPosition { FIRST, SECOND, THIRD, BEFORELAST, LAST };
                ...
                if (childGrdExpPtr != nullptr) {
                        ...
                        if (itSpecifier != childGrdExp.children.end()) {
                            ...
                            for (const auto& currSpecGrdExpPtr : specifierGrdExpPtrs) {
                                ...
                                if (specStatGrdPtr != nullptr) {
                                    GrammaticalType subGrammType =
                                        SemExpGetter::childGrammaticalTypeOfParentCoreference(*currSpecGrdExpPtr);
                                    if (subGrammType != GrammaticalType::UNKNOWN) {
                                        ...
                                        getLinksOfAGrdExp(subReqList,
                                                          pWorkStruct,
                                                          pMemViewer,
                                                          *currSpecGrdExpPtr,
                                                          pAddSubordinateLinks);
                    ...
                    switch (childGrdExp->type) {
                        ...
                        case SemanticGroundingType::GENERIC: {
                            ...
                            if (pAddSubordinateLinks && !pReqLinks.isEquVerb
                                && childGenGrd.referenceType != SemanticReferenceType::DEFINITE
                                && !ConceptSet::haveAConcept(childGenGrd.concepts, "agent")
                                &&    // Big optimization for who questions and it's not needed because the fact that
                                      // something is an agent cannot be found dynamically
                                ...
                                subReqList._addChildWithoutSortedContainer(SemanticRequestType::ACTION)
                                    .concepts.emplace("verb_equal_be", 4);
                                auto& subObjLinks =
                                    subReqList._addChildWithoutSortedContainer(SemanticRequestType::OBJECT);
                                subObjLinks.semExps.emplace_back(&*currChild.second);
                                subObjLinks.crossedLinks.semExpsWithSpecificFilter =
                                    subLinks.crossedLinks.semExpsWithSpecificFilter;
                        ...
                        case SemanticGroundingType::STATEMENT: {
                            ...
                            if (reqType == SemanticRequestType::TIME
                                ...
                                if (subWorkStruct.askForNewRecursion()) {
                                    ...
                                    request.set(reqType);
                                    satisfyAQuestion(
                                        subWorkStruct, pMemViewer, childGrdExp, otherGrdExps, childGrdExp, request);
                                    ...
                                    SubRequestLinks& linksToFill = pReqLinks._addChildWithoutSortedContainer(reqType);
...
void getInformationsLinkedToCondition(std::set<const GroundedExpWithLinks*>& pNewInformations,
    ...
    auto& memBlockPrivateViewer = pMemViewer.getConstViewPrivate();
    RelationsThatMatch<false> idsToSentences;
    RequestToMemoryLinks<false> reqToLinks(memBlockPrivateViewer.getLinks(
        SemanticTypeOfLinks::CONDITION_INFORMATION, pReqLinks.tense, pReqLinks.verbGoal));
    MemoryBlockPrivateAccessorPtr<false> memBlockPrivateAccessor(&memBlockPrivateViewer);
    _getResultFromMemory(idsToSentences,
                         reqToLinks,
                         pWorkStruct,
...
void getNowConditions(SemControllerWorkingStruct& pWorkStruct,
    ...
    RequestToMemoryLinks<true> reqToGrdExps(viewPrivatePtr->getLinks(
        SemanticTypeOfLinks::SENT_WITH_ACTION, SemanticVerbTense::UNKNOWN, VerbGoalEnum::NOTIFICATION));
    ...
    semanticMemoryGetter::getResultMatchingNowTimeFromMemory(
        idsToSentences, emptyLinks, reqToGrdExps, pNowTimeDuration, memBlockPrivate, false, pLingDb, false);
...
const SemanticExpression* getActionComposition(SemControllerWorkingStruct& pWorkStruct,
    ...
    MemoryBlockPrivateAccessorPtr<false> memBlockPrivateAccessor(&memBlockPrivateViewer);
    _getResultFromMemory(idToSentences,
                         reqToGrdExps,
                         pWorkStruct,
                         memBlockPrivateAccessor,
                         pMemViewer,
...
bool checkForConditionsLinkedToStatement(SemControllerWorkingStruct& pWorkStruct,
                                         ...
                                         const GroundedExpression& pGrdExp) {
    auto& memBlockPrivateViewer = pMemViewer.getConstViewPrivate();
    RequestToMemoryLinks<false> reqToGrdExps(
        memBlockPrivateViewer.getLinks(SemanticTypeOfLinks::SENT_WITH_ACTION, pReqLinks.tense, pReqLinks.verbGoal));
    RelationsThatMatch<false> idToSentences;
    MemoryBlockPrivateAccessorPtr<false> memBlockPrivateAccessor(&memBlockPrivateViewer);
    _getResultFromMemory(idToSentences,
                         reqToGrdExps,
...
void disableActionsLinkedToASentence(SemanticMemory& pSemanticMemory,
                                     const GroundedExpression& pGrdExp,
                                     const linguistics::LinguisticDatabase& pLingDb) {
    SemControllerWorkingStruct workStruct(InformationType::INFORMATION,
                                          nullptr,
    ...
    if (memViewPrivatePtr != nullptr) {
        RelationsThatMatch<true> idsToSentences;
        RequestToMemoryLinks<true> reqToLinks(
            memViewPrivatePtr->getLinks(SemanticTypeOfLinks::SENT_WITH_ACTION, reqLinks.tense, reqLinks.verbGoal));
...
bool addTriggerSentencesAnswer(SemControllerWorkingStruct& pWorkStruct,
                               ...
                               ContextualAnnotation pContAnnotation) {
    std::map<const ExpressionWithLinks*, ParametersLabelsToValue> semExpWrapperPtrs;
    _matchGrdExpTrigger(semExpWrapperPtrs, pWorkStruct, pMemViewer, pReqLinks, pExpCategory, pAxiomId, pInputGrdExp);
    return _addTriggerThatMatchTheMost(
        pWorkStruct, pAnAnswerHasBeenAdded, semExpWrapperPtrs, pMemViewer, pContAnnotation, pInputGrdExp);
}
...
bool satisfyAnAction(SemControllerWorkingStruct& pWorkStruct,
    ...
    if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT
        && addTriggerSentencesAnswer(pWorkStruct,
    ...
    return _getActionFromAMemblock(pWorkStruct, pMemViewer, reqLinks, pGrdExp) || anAnswerHasBeenAdded;
}
...
void _addGrdExpsFromASemExp(std::map<semIdAbs, std::list<AnswerExp>>& pSemExpsContainer,
    ...
    switch (pSemExpToAdd->type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            if (_isAValidAnswerForTheQuestionFilter(
                    grdExp, pQuestMetaGrdExp, pRequest, pWorkStruct.lingDb.conceptSet, pWorkStruct.lingDb)
                || (grdExp.children.count(GrammaticalType::SUB_CONCEPT) > 0
                    && pWorkStruct.lingDb.conceptSet.areConceptsCompatibles(grdExp->concepts,
                                                                            pQuestMetaGrdExp->grounding().concepts)
                           == true)) {
                auto grdExpContainer = makeGrdExpContainer(pSemExpToAdd);
                pSemExpsContainer[pId].emplace_back(pRelatedContextAxioms,
                                                    std::move(grdExpContainer),
                                                    pEqualityOfTheMemoryAnswer,
                ...
                if (itEquChild != grdExp.children.end()) {
                    _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                                     pId,
                                                     pSemExpToAdd.wrapInContainer(itEquChild->second),
                                                     pRelatedContextAxioms,
                                                     pEqualityOfTheMemoryAnswer,
                                                     pAnnotationsOfTheAnswer,
                                                     pQuestMetaGrdExp,
                                                     pWorkStruct,
        ...
        case SemanticExpressionType::LIST: {
            ...
            for (auto& elt : listExp.elts)
                if (semExpsToInore.count(&elt) == 0)
                    _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                                     pId,
                                                     pSemExpToAdd.wrapInContainer(elt),
                                                     pRelatedContextAxioms,
                                                     pEqualityOfTheMemoryAnswer,
                                                     pAnnotationsOfTheAnswer,
                                                     pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::INTERPRETATION: {
            ...
            _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                             pId,
                                             pSemExpToAdd.wrapInContainer(intExp.interpretedExp),
                                             pRelatedContextAxioms,
                                             pEqualityOfTheMemoryAnswer,
                                             pAnnotationsOfTheAnswer,
                                             pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::FEEDBACK: {
            ...
            _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                             pId,
                                             pSemExpToAdd.wrapInContainer(fdkExp.concernedExp),
                                             pRelatedContextAxioms,
                                             pEqualityOfTheMemoryAnswer,
                                             pAnnotationsOfTheAnswer,
                                             pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::ANNOTATED: {
            ...
            _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                             pId,
                                             pSemExpToAdd.wrapInContainer(annExp.semExp),
                                             pRelatedContextAxioms,
                                             pEqualityOfTheMemoryAnswer,
                                             pAnnotationsOfTheAnswer,
                                             pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::METADATA: {
            ...
            _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                             pId,
                                             pSemExpToAdd.wrapInContainer(metadataExp.semExp),
                                             pRelatedContextAxioms,
                                             pEqualityOfTheMemoryAnswer,
                                             pAnnotationsOfTheAnswer,
                                             pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::CONDITION: {
            ...
            simplifier::solveConditions<TUSEMEXP>(solvedConditons, pSemExpToAdd, pMemBlock, pWorkStruct.lingDb);
            for (const auto& currSolvedCond : solvedConditons) {
                ...
                if (currSolvedCond.truenessValue == TruenessValue::VAL_TRUE)
                    _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                                     pId,
                                                     pSemExpToAdd.wrapInContainer(condExp.thenExp),
                                                     pRelatedContextAxioms,
                                                     pEqualityOfTheMemoryAnswer,
                                                     pAnnotationsOfTheAnswer,
                                                     pQuestMetaGrdExp,
                                                     ...
                                                     pMemBlock,
                                                     pRequest);
                else if (currSolvedCond.truenessValue == TruenessValue::VAL_FALSE && condExp.elseExp)
                    _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                                     pId,
                                                     pSemExpToAdd.wrapInContainer(*condExp.elseExp),
                                                     pRelatedContextAxioms,
                                                     pEqualityOfTheMemoryAnswer,
                                                     pAnnotationsOfTheAnswer,
                                                     pQuestMetaGrdExp,
        ...
        case SemanticExpressionType::FIXEDSYNTHESIS: {
            ...
            _addGrdExpsFromASemExp<TUSEMEXP>(pSemExpsContainer,
                                             pId,
                                             pSemExpToAdd.wrapInContainer(fSynthExp.getUSemExp()),
                                             pRelatedContextAxioms,
                                             pEqualityOfTheMemoryAnswer,
                                             pAnnotationsOfTheAnswer,
                                             pQuestMetaGrdExp,
...
void _extractAnnotationsFromAnswer(const SemanticExpression*& pEqualityOfTheMemoryAnswer,
    ...
    for (const auto& currAnsGrdExpChild : pGrdExp.children) {
        ...
        if (childAnswerGramType == GrammaticalType::SPECIFIER) {
            pEqualityOfTheMemoryAnswer = [pFormRequest, &currAnsGrdExpChild]() -> const SemanticExpression* {
                if (pFormRequest != SemanticRequestType::OBJECT && pFormRequest != SemanticRequestType::MANNER)
                    return nullptr;
    ...
    pAnnotations.iterateOverAllValues([&](GrammaticalType pGrammaticalType, const SemanticExpression& pSemExp) {
...
bool _keepGroundingsWithSamePolarity(std::list<AnswerExp>& pAllAnswers,
    ...
    for (auto& samPolList : pSemExpsWithSamePolarity) {
        ...
        for (auto samPolIt = samPolList.second.begin(); samPolIt != samPolList.second.end();) {
    ...
    if (pAllAnswers.empty()) {
        for (auto& othPolList : pSemExpsWithOtherPolarity) {
            for (auto othPolIt = othPolList.second.begin(); othPolIt != othPolList.second.end(); ++othPolIt) {
    ...
    return !pAllAnswers.empty() || (!pSemExpsWithOtherPolarity.empty() && pRequest == SemanticRequestType::YESORNO);
}
...
void _genericFilterSemExpThatCanAnswer(std::map<QuestionAskedInformation, AllAnswerElts>& pAllAnswers,
    ...
    for (auto& relSemExp : pRelations) {
        ...
        auto grammaticalTypes = SemExpGetter::requestToGrammaticalTypes(pQuestionAskedInformation.request,
        ...
        _extractAnnotationsFromAnswer(equalityOfTheMemoryAnswer,
                                      annotationsOfTheAnswer,
                                      pQuestionAskedInformation.request,
                                      answGrdExp,
                                      ansElt.getAnnotations(),
                                      grammaticalTypes);
        ...
        if (grammaticalTypes.empty()) {
            ...
            semExpsContainer[relSemExp.first].emplace_back(
                ansElt.relatedContextAxioms, std::move(answGrdExpContainer), nullptr, annotationsOfTheAnswer);
        } else {
            for (const auto& currGramType : grammaticalTypes) {
                ...
                if (childSemExpPtr) {
                    ...
                    if (uSemExp != nullptr) {
                        _addGrdExpsFromASemExp(semExpsContainer,
                                               relSemExp.first,
                                               *uSemExp,
                                               ansElt.relatedContextAxioms,
                                               equalityOfTheMemoryAnswer,
                                               annotationsOfTheAnswer,
                                               questMetaGrdExp,
                                               pWorkStruct,
                                               pMemBlock,
    ...
    std::list<AnswerExp>& answersForDefaultRequ =
        pAllAnswers[QuestionAskedInformation(pDefaultRequType, pQuestionAskedInformation.typeOfUnityOpt)]
            .answersFromMemory;
    ...
    if (!_keepGroundingsWithSamePolarity(answersForDefaultRequ,
                                         ...
                                         semExpsWithSamePolarity,
                                         semExpsWithOtherPolarity,
                                         pDefaultRequType,
                                         pMemBlock,
                                         pWorkStruct.lingDb)) {
...
void _addCauseResult(std::map<QuestionAskedInformation, AllAnswerElts>& pAllAnswers, AnswerElement& pAnswerElement) {
    for (const GroundedExpWithLinks& currSent : pAnswerElement.getMemorySentences()) {
        if (currSent.isAConditionToSatisfy()) {
            ...
            pAllAnswers[SemanticRequestType::CAUSE].answersFromMemory.emplace_back(
                currSent.getContextAxiom(), std::move(grdExpRef), nullptr, annotationsOfTheAnswer);
        }
    }
}
...
void _replaceAnswersByNumberOfInstances(std::map<QuestionAskedInformation, AllAnswerElts>& pAllAnswers,
    ...
    if (answerGrdPtr) {
        ...
        pAllAnswers.clear();
        pAllAnswers.emplace(
            answerQuestionAskedInformation ? *answerQuestionAskedInformation : SemanticRequestType::QUANTITY,
...
bool matchAffirmationTrigger(SemControllerWorkingStruct& pWorkStruct,
                             ...
                             const GroundedExpression& pInputGrdExp) {
    bool anAnswerHasBeenAdded = false;
    bool res = addTriggerSentencesAnswer(pWorkStruct,
                                         anAnswerHasBeenAdded,
                                         pMemViewer,
                                         pReqLinks,
    ...
    if (!res && pWorkStruct.callbackToSentencesCanBeAnsweredPtr != nullptr) {
        for (const auto& currCallbackToSents : *pWorkStruct.callbackToSentencesCanBeAnsweredPtr) {
            ...
            SemControllerWorkingStruct subWorkStruct(pWorkStruct.informationType,
                                                     pWorkStruct.authorSemExp,
                                                     pWorkStruct.fromLanguage,
                                                     pWorkStruct.expHandleInMemory,
                                                     pWorkStruct.reactOperator,
                                                     nullptr,
                                                     nullptr,
                                                     nullptr,
                                                     nullptr,
            ...
            for (const auto& currRel : idsToSentences.res.dynamicLinks) {
                ...
                unknownInfosGetter::checkIfMatchAndGetParams(
                    *params, subIntContext, nullptr, *currRel.second, pInputGrdExp, pWorkStruct, subMemViewer);
                ...
                try {
                    ...
                    std::cerr << "Exception caught when calling an external trigger method. The error was: " << e.what()
                              << std::endl;
                }
            }
        }
...
void _getSentencesFromMemBlockRecursively(std::map<intMemBlockId, MemorySentencesInResult>& pMemSentencesInResult,
                                          SemanticMemoryBlockViewer& pMemViewer,
                                          const RequestLinks& pReqLinks,
                                          const linguistics::LinguisticDatabase& pLingDb,
    ...
    if (memViewPrivatePtr != nullptr) {
        ...
        semanticMemoryGetter::getResultFromMemory(idToSentences,
                                                  awLinks,
                                                  pReqLinks,
                                                  semanticMemoryGetter::RequestContext::SENTENCE,
                                                  memViewPrivateAccessor,
                                                  false,
                                                  pLingDb,
                                                  pCheckTimeRequest,
        ...
        _fillStaticLinksIfNecessary(
            pMemSentencesInResult, idToSentences.res.staticLinks, memBlockViewer.subBinaryBlockPtr);
    } else {
        RelationsThatMatch<false> idToSentences;
    ...
    if (pMemViewer.constView.subBlockPtr != nullptr) {
        SemanticMemoryBlockViewer subMemViewer(nullptr, *pMemViewer.constView.subBlockPtr, pMemViewer.currentUserId);
        _getSentencesFromMemBlockRecursively(
            pMemSentencesInResult, subMemViewer, pReqLinks, pLingDb, pCheckTimeRequest);
    }
}

void _getRelationsOfLinks(std::map<QuestionAskedInformation, AllAnswerElts>& pUserAnswers,
                          SemanticMemoryBlockViewer& pMemViewer,
                          const SemControllerWorkingStruct& pWorkStruct,
                          const RequestLinks& pReqLinks,
                          const GroundedExpression& pGrdExp,
    ...
    // get informations
    _getSentencesFromMemBlockRecursively(memSentencesInResult, pMemViewer, pReqLinks, pWorkStruct.lingDb, true);
    ...
    if (memSentencesInResult.empty()) {
        ...
        if (constFallbacksBlockPtr != nullptr) {
            SemanticMemoryBlockViewer fallbackSubMemViewer(
                fallbacksBlockPtr, *constFallbacksBlockPtr, pMemViewer.currentUserId);
            _getSentencesFromMemBlockRecursively(
                memSentencesInResult, fallbackSubMemViewer, pReqLinks, pWorkStruct.lingDb, false);
        }
    }
    ...
    switch (pRequestType) {
        case SemanticRequestType::YESORNO: {
            ...
            for (auto itAnsw = answElts.begin(); itAnsw != answElts.end();) {
                ...
                if (!SemExpGetter::isGrdExpPositive(currAnwGrdExp)) {
                    ...
                    ImbricationType imbr = SemExpComparator::getGrdExpsImbrications(
                        currAnwGrdExp, pGrdExp, pMemViewer.constView, pWorkStruct.lingDb, &compException);
            ...
            _extractAnnotationsFromAnswer(equalityOfTheMemoryAnswer,
                                          ...
                                          pRequestType,
                                          memGrdExp,
                                          moreRecentAnsElt.getAnnotations(),
                                          emptyGramTypes);

            bool samePolarity =
                SemExpComparator::haveSamePolarity(pGrdExp, memGrdExp, pWorkStruct.lingDb.conceptSet, true);
            if (samePolarity) {
                // genGrd from input -> grd exp of memory -> quantity of the grd exp from memory
                std::map<const SemanticGenericGrounding*, std::map<const GroundedExpression*, SemanticFloat>>
                    alreadyCountedElts;
                for (auto itAnsw = answElts.rbegin(); itAnsw != answElts.rend(); ++itAnsw)
                    _incrementNotDefiniteQuantitiesFromGrdExps(pGrdExp,
                ...
                if (!sameQuantity) {
                    if (pWorkStruct.reactOperator == SemanticOperatorEnum::ANSWER
                        ...
                        std::map<SemanticGenericGrounding*, std::map<const GroundedExpression*, SemanticFloat>>
                            alreadyCountedElts;
                        for (auto itAnsw = answElts.rbegin(); itAnsw != answElts.rend(); ++itAnsw)
                        ...
                        pUserAnswers[pRequestType].answersGenerated.emplace_back(
                            SemExpCreator::generateYesOrNoAnswerFromMemory(
                                std::move(inputWithNewQuantities), false, annotationsOfTheAnswer),
            ...
            if (pWorkStruct.reactOperator == SemanticOperatorEnum::ANSWER
                || pWorkStruct.reactOperator == SemanticOperatorEnum::REACT) {
                pUserAnswers[pRequestType].answersGenerated.emplace_back(
                    SemExpCreator::generateYesOrNoAnswerFromQuestion(pGrdExp, samePolarity, annotationsOfTheAnswer),
                    &relatedContextAxioms);
                return;
            }
            pUserAnswers[pRequestType].answersGenerated.emplace_back(SemExpCreator::sayYesOrNo(samePolarity),
                                                                     &relatedContextAxioms);
            return;
        }
        ...
        case SemanticRequestType::QUANTITY: {
            auto questionAskedInformation =
                QuestionAskedInformation(pRequestType, SemExpGetter::getTypeOfUnityFromGrdExp(pGrdExp));
            _genericFilterSemExpThatCanAnswer(pUserAnswers,
                                              answElts,
                                              pGrdExp,
                                              questionAskedInformation,
                                              pOriginalQuestRequestType,
                                              pWorkStruct,
                                              pMemViewer.constView);
    ...
    _genericFilterSemExpThatCanAnswer(
        pUserAnswers, answElts, pGrdExp, pRequestType, pOriginalQuestRequestType, pWorkStruct, pMemViewer.constView);
}
...
bool _answerToYesOrNoQuestionIfTheSemExpCorrespondToUniquePatternThatAlreadyHaveAValue(
    SemControllerWorkingStruct& pWorkStruct,
    SemanticMemoryBlockViewer& pMemViewer,
    const GroundedExpression& pGrdExp) {
        ...
        if (subWorkStruct.askForNewRecursion()) {
            ...
            auto subQuestionGrdExpPtr = pGrdExp.clone();
            {
                ...
                if (childThatShouldBeUnique && !(*childThatShouldBeUnique)->isEmpty())
                    SemExpModifier::setChild(
                        subQuestionGrdExp, grammTypeOfChildThatShouldBeUnique, std::move(*childThatShouldBeUnique));
                else
            ...
            if (!subWorkStruct.compositeSemAnswers->semAnswers.empty()) {
                        ...
                        if (SemExpComparator::semExpsAreEqualFromMemBlock(*itSubordinateThatShouldBeUnique->second,
                            ...
                            if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT) {
                                ...
                                pWorkStruct.addAnswer(ContextualAnnotation::ANSWER,
                                                      SemExpCreator::generateYesOrNoAnswerFromQuestion(
                                                          pGrdExp, true, annotationsOfTheAnswer),
                                ...
                                allAnswElts.answersGenerated.emplace_back(SemExpCreator::sayYesOrNo(true),
                                                                          &relatedContextAxioms);
                ...
                if (!grdExpAnswers.empty()) {
                    if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT) {
                        ...
                        SemExpModifier::setChild(
                            *answerGrdExp, grammTypeOfChildThatShouldBeUnique, grdExpAnswers.front()->clone());
                        pWorkStruct.addAnswer(ContextualAnnotation::ANSWER,
                        ...
                        auto newAnsw = std::make_unique<LeafSemAnswer>(ContextualAnnotation::ANSWER);
                        AllAnswerElts& allAnswElts = newAnsw->answerElts[SemanticRequestType::YESORNO];
                        allAnswElts.answersGenerated.emplace_back(SemExpCreator::sayYesOrNo(false),
                                                                  &relatedContextAxioms);
                        pWorkStruct.compositeSemAnswers->semAnswers.emplace_back(std::move(newAnsw));
                    }
                    return true;
...
bool satisfyAQuestion(SemControllerWorkingStruct& pWorkStruct,
    ...
    for (const auto& currRequest : pRequests.types) {
        ...
        if (grammTypeOfQuestion != GrammaticalType::UNKNOWN) {
            ...
            if (itChildOfQuestion != pGrdExp.children.end()) {
                const ListExpression* listExpOfQuestionPtr = itChildOfQuestion->second->getListExpPtr_SkipWrapperPtrs();
                if (listExpOfQuestionPtr != nullptr && listExpOfQuestionPtr->listType == ListExpressionType::OR)
                    return manageChoice(pWorkStruct, pMemViewer, pGrdExp, grammTypeOfQuestion, *listExpOfQuestionPtr)
                        || anAnswerHasBeenAdded;
            }
        }
    }
    ...
    for (const auto& currRequest : pRequests.types) {
        _getRelationsOfLinks(allAnswers, pMemViewer, pWorkStruct, reqLinks, pGrdExp, currRequest, currRequest);
        ...
        for (const auto& currOtherGrdExp : pOtherGrdExps) {
            ...
            if (requests != nullptr && !requests->empty()) {
                ...
                _getRelationsOfLinks(
                    allAnswers, pMemViewer, pWorkStruct, subReqLinks, *currOtherGrdExp, subRequestType, currRequest);
            }
        }
    }

    if (pRequests.has(SemanticRequestType::YESORNO) && allAnswers.empty()
        && _answerToYesOrNoQuestionIfTheSemExpCorrespondToUniquePatternThatAlreadyHaveAValue(
            pWorkStruct, pMemViewer, pGrdExp))
        return true;
    ...
    if (pWorkStruct.reactionOptions.canAnswerWithExternalEngines
        ...
        for (const auto& currCallbackToSents : *pWorkStruct.callbackToSentencesCanBeAnsweredPtr) {
            ...
            for (const auto& currRel : idsToSentences.res.dynamicLinks) {
                ...
                std::unique_ptr<InteractionContextContainer> subIntContext;
                unknownInfosGetter::checkIfMatchAndGetParams(
                    *params, subIntContext, nullptr, *currRel.second, pGrdExp, pWorkStruct, subMemViewer);
                ...
                try {
                    ...
                    if (answer) {
                        ...
                        allAnswers[pRequests.firstOrNothing()].answersGenerated.emplace_back(std::move(*answer),
                                                                                             &relatedContextAxiom);
                    }
                } catch (const std::exception& e) {
                    std::cerr << "Exception caught when calling an external answer method. The error was: " << e.what()
                              << std::endl;
                }
            }
        }
    ...
    if (allAnswers.empty()) {
        ...
        if (allAnswers.empty())
            for (const auto& currOtherGrdExp : pOtherGrdExps)
                if (allAnswers.empty())
                    answerFromDataStoredInsideTheQuestion::getAnswers(
                        allAnswers, *currOtherGrdExp, rootRequest, pWorkStruct.lingDb);
    }
            ...
            switch (pWorkStruct.reactOperator) {
                ...
                case SemanticOperatorEnum::REACT: {
                    pWorkStruct.addAnswerWithoutReferences(
                        ContextualAnnotation::ANSWER, SemExpCreator::generateNumberOfTimesAnswer(pOriginalGrdExp, 0));
                    return true;
                }
                default: return anAnswerHasBeenAdded;
            }
        } else {
            return anAnswerHasBeenAdded;
        }
        ...
        switch (pWorkStruct.reactOperator) {
            ...
            case SemanticOperatorEnum::REACT: {
                ...
                if (answerSemExpOpt) {
                    ...
                    if (sentimentSpec
                        && sentimentSpec->sentimentStrengh
                               > 1)    // 1 and not 0 because we don't want to react on too weak sentiments
                        SemExpModifier::addNewSemExp(*answerSemExpOpt, SemExpCreator::sayThanks());
                    pWorkStruct.addAnswer(
                        ContextualAnnotation::ANSWER, std::move(*answerSemExpOpt), ReferencesFiller(references));
                    return true;
                }
                break;
            }
