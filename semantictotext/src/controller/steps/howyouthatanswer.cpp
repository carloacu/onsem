...
void _addProvider(std::set<InformationProvider>& pInfosProv, const RelatedContextAxiom& pRelatedContextAxioms) {
    auto considerAxiom = [&](const SentenceWithLinks* pAxiomPtr) {
        if (pAxiomPtr == nullptr)
            return false;
        const auto& currAxiom = *pAxiomPtr;

        const MetadataExpression* metadataExpPtr =
            currAxiom.getSemExpWrappedForMemory().semExp->getMetadataPtr_SkipWrapperPtrs();
...
void process(std::unique_ptr<LeafSemAnswer>& pLeafAnswer) {
    ...
    for (const InformationProvider& pInfProvider : infosProv) {
        ...
        if (pInfProvider.from == SemanticSourceEnum::ASR) {
            ...
            howAnswerSemExp = SemExpCreator::getSemExpOfEventValue(pInfProvider.eventName, pInfProvider.eventValue);
        ...
        if (howAnswerSemExp) {
            ...
            pLeafAnswer->answerElts[SemanticRequestType::MANNER].answersGenerated.emplace_back(
                std::move(howAnswerSemExp));
        }
    }
}
