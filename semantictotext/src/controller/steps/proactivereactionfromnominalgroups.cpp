...
GroundingEntity _extractGroundingEntity(const GroundedExpression& pGrdExp)
{
  ...
  {
    if (!genGrdPtr->coreference &&
        genGrdPtr->word.partOfSpeech != PartOfSpeech::ADVERB &&
  ...
  {
    if (grd.getNameGroundingPtr() != nullptr ||
        grd.getAgentGroundingPtr() != nullptr)
      return GroundingEntity::AGENT;
  }
                               ...
                               UniqueSemanticExpression pQuestionSemExp)
{
  ...
  if (!reactOnNominalGroup &&
      pWorkStruct.reactionOptions.canDoAProactivity &&
      !pWorkStruct.haveAnAnswer())
  {
...
bool reactOnSentimentsFromNominalGroup(SemControllerWorkingStruct& pWorkStruct,
      ...
      if (sentimentSpec->sentiment == "sentiment_positive_joy")
      {
        pWorkStruct.addAnswerWithoutReferences
            (ContextualAnnotation::FEEDBACK,
             SemExpCreator::niceYouLikeIt(SemExpGetter::returnAPositiveSemExpBasedOnAnInput(pGrdExp), *pWorkStruct.author));
        return true;
      }
           ...
           const GroundedExpression& pGrdExp)
{
  if (pWorkStruct.reactionOptions.canDoAProactivity)
  {
    if (pWorkStruct.authorSemExp != nullptr)
    {
      ...
      {
        const SemanticExpression* semExpPtr =
            pMemViewer.constView.getBeforeLastSemExpOfNotAnAuthor(*pWorkStruct.authorSemExp, pWorkStruct.lingDb);
