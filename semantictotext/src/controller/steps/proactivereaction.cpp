...
bool _isTheReverseTrue(std::unique_ptr<GroundedExpression>& pReverseGrdExp,
    ...
    if (pGrdExp.children.count(GrammaticalType::SUBJECT) == 0 || pGrdExp.children.count(GrammaticalType::OBJECT) == 0)
        return false;

    pReverseGrdExp = pGrdExp.clone();
    pReverseGrdExp->children.find(GrammaticalType::SUBJECT)
        ->second.swap(pReverseGrdExp->children.find(GrammaticalType::OBJECT)->second);
    ...
    if (subWorkStruct.askForNewRecursion()) {
        ...
        controller::applyOperatorOnGrdExp(
            subWorkStruct, subMemView, *pReverseGrdExp, std::list<const GroundedExpression*>(), *pReverseGrdExp);
...
bool _sayDayOfBirthdate(SemControllerWorkingStruct& pWorkStruct,
    ...
    if (grdExpObject != nullptr) {
        ...
        if (agentObject != nullptr && agentObject->userId == pCurrentUserId) {
...
bool _isChildOfSemExpIsUnknown(const GroundedExpression& pGrdExp, GrammaticalType pChildType) {
    ...
    if (itChild != pGrdExp.children.end()) {
        ...
        if (grdExpChild != nullptr) {
            ...
            return genGrdChild != nullptr && genGrdChild->entityType == SemanticEntityType::THING
                && !genGrdChild->word.lemma.empty() && genGrdChild->word.partOfSpeech == PartOfSpeech::UNKNOWN;
        }
    }
    return false;
}
...
bool _sayItsNotFalseFromFrenchSentenceWhenObjectIsUnknown(SemControllerWorkingStruct& pWorkStruct,
    ...
    if (pWorkStruct.fromLanguage == SemanticLanguageEnum::FRENCH
        ...
        pWorkStruct.addAnswerWithoutReferences(
            ContextualAnnotation::FEEDBACK,
            std::make_unique<GroundedExpression>(std::make_unique<SemanticTextGrounding>("C'est pas faux.")));
        return true;
    }
    return false;
}

bool _sayOkIfTheUserIsTalkingAboutHim(SemControllerWorkingStruct& pWorkStruct, const GroundedExpression& pGrdExp) {
    ...
    if (pWorkStruct.author != nullptr && SemExpGetter::agentIsTheSubject(pGrdExp, pWorkStruct.author->userId)) {
        ...
        if (statGrdPtr == nullptr || ConceptSet::haveAConcept(statGrdPtr->concepts, "mentalState_know"))
            return false;

        pWorkStruct.addAnswerWithoutReferences(
            ContextualAnnotation::FEEDBACK,
            std::make_unique<FeedbackExpression>(SemExpCreator::sayOk(), pGrdExp.clone()));
        return true;
    }
    return false;
}
...
bool _reactOnSentiments(SemControllerWorkingStruct& pWorkStruct,
    ...
    bool reactOnSentiment = pWorkStruct.reactOperator == SemanticOperatorEnum::REACT
                         || (pWorkStruct.reactOperator == SemanticOperatorEnum::FEEDBACK
    ...
    if (sentimentSpec && sentimentSpec->sentimentStrengh > 1
                ...
                if (reactOnSentiment) {
                    if (ConceptSet::doesConceptBeginWith(sentimentSpec->sentiment, "sentiment_positive_")) {
                        ...
                        if (_isTheReverseTrue(reverseGrdExp, relatedContextAxioms, pWorkStruct, pMemViewer, pGrdExp)) {
                            SemExpModifier::addChild(*reverseGrdExp,
                                                     GrammaticalType::SPECIFIER,
                                                     SemExpCreator::sayAlso(),
                                                     ListExpressionType::UNRELATED,
                                                     false,
                                                     false);
                            pWorkStruct.addAnswer(ContextualAnnotation::FEEDBACK,
                                                  std::make_unique<FeedbackExpression>(SemExpCreator::sayThanks(),
                                                                                       std::move(reverseGrdExp)),
                                                  ReferencesFiller(relatedContextAxioms));
                        } else {
                            pWorkStruct.addAnswerWithoutReferences(
                                ContextualAnnotation::FEEDBACK, SemExpCreator::sayThanksThatsCool(*pWorkStruct.author));
                        }
                        return true;
                    }
            ...
            if (pGrdExp.children.find(GrammaticalType::CAUSE) == pGrdExp.children.end()) {
                ...
                if (SemExpGetter::doSemExpHoldUserId(*sentimentSpec->receiver, pWorkStruct.author->userId)) {
                    if (ConceptSet::doesConceptBeginWith(sentimentSpec->sentiment, "sentiment_positive_")) {
                        if (reactOnSentiment) {
                            pWorkStruct.addAnswerWithoutReferences(ContextualAnnotation::FEEDBACK,
                                                                   SemExpCreator::sayIAmHappyToHearThat());
                            res = true;
                        }
                        ...
                        if (reactOnSentiment) {
                            if (pWorkStruct.fromLanguage == SemanticLanguageEnum::ENGLISH)
                                pWorkStruct.addAnswerWithoutReferences(ContextualAnnotation::FEEDBACK,
                                                                       SemExpCreator::iAmSorryToHearThat());
                            else
                                pWorkStruct.addAnswerWithoutReferences(ContextualAnnotation::FEEDBACK,
                                                                       SemExpCreator::itIsABadNews());
                            res = true;
                        }
                    }
                }
                ...
                if (unknownInfosGetter::isItfUnknown(pWorkStruct, pMemViewer, *askWyGrdExp)) {
                    pWorkStruct.addAnswerWithoutReferences(ContextualAnnotation::QUESTION, std::move(askWyGrdExp));
                    return true;
                }
                return res;
            }
...
void _confirmOrCorrect(SemControllerWorkingStruct& pWorkStruct,
    ...
    auto res = SemExpCreator::sayThatTheAssertionIsTrueOrFalse(pGrdExp, pTruenessValue == TruenessValue::VAL_TRUE);
    pWorkStruct.addAnswer(ContextualAnnotation::FEEDBACK, std::move(res), ReferencesFiller(pAnswersContextAxioms));
}
        ...
        if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT
            ...
            if (pTruenessValue == TruenessValue::VAL_FALSE) {
                auto reactionOfOppositeInformation = [&](const SentenceWithLinks& pCurrContextAxiom) {
                    ImbricationType res =
                        SemExpComparator::getSemExpsImbrications(*pCurrContextAxiom.getSemExpWrappedForMemory().semExp,
                                                                 pGrdExp,
                    ...
                    UniqueSemanticExpression sentenceThatWeShougth(
                        pCurrContextAxiom.getSemExpWrappedForMemory().semExp->clone());
                    SemExpModifier::modifyVerbTenseOfSemExp(*sentenceThatWeShougth, SemanticVerbTense::PAST);
                    auto fdkExp = SemExpCreator::sayIThoughtThat(std::move(sentenceThatWeShougth));
                    pWorkStruct.addAnswer(
                        ContextualAnnotation::FEEDBACK, std::move(fdkExp), ReferencesFiller(pCurrContextAxiom));
                    return true;
                };
...
void processWithUpdatedMemory(bool& pRes,
    ...
    if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT && !pWorkStruct.haveAnAnswer())
        pRes = pRes || _sayDayOfBirthdate(pWorkStruct, pGrdExp, pMemViewer.currentUserId)
            || (pWorkStruct.reactionOptions.canSayOkToAnAffirmation
                && _sayOkIfTheUserIsTalkingAboutHim(pWorkStruct, pGrdExp))
