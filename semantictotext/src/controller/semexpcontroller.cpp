...
void _applyOperatorResolveSecondPersonOfSingular
...
void applyOperatorResolveAgentAccordingToTheContext
                                   ...
                                   const ComparisonExpression& pCompExp)
{
      ...
      pCompExp.rightOperandExp)
  {
    ...
    if (pCompExp.op == ComparisonOperator::EQUAL ||
        pCompExp.op == ComparisonOperator::DIFFERENT)
    {
      bool isEqual = SemExpComparator::semExpsAreEqualFromMemBlock(*pCompExp.leftOperandExp, **pCompExp.rightOperandExp,
                                                                   pMemViewer.constView, pWorkStruct.lingDb, nullptr);
                               ...
                               const ListExpression& pListExp)
{
  ...
  for (const auto& currElt : pListExp.elts)
  {
    ...
    if (pListExp.listType == ListExpressionType::OR &&
        agreementVal == TruenessValue::VAL_TRUE)
    {
      pWorkStruct.addAnswers(subWorkStruct);
      return;
    }

    if (pListExp.listType == ListExpressionType::AND &&
        agreementVal == TruenessValue::VAL_FALSE)
    {
      pWorkStruct.addAnswers(subWorkStruct);
      return;
    }
                          ...
                          const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  for (auto itDetAns = pSemAnwers.begin(); itDetAns != pSemAnwers.end(); )
  {
    ...
    if (leafPtr != nullptr)
    {
      ...
      for (auto itAnsForARequ = leafAnsw.answerElts.begin();
           itAnsForARequ != leafAnsw.answerElts.end(); )
      {
             ...
             itKnowAns != answerElt.answersGenerated.end(); )
        {
          ...
          if (grdExp != nullptr &&
              pAnd_Or == ifGrdStillExist(*grdExp))
            ++itKnowAns;
          else
  ...
  for (const auto& currElt : pListExp.elts)
  {
    ...
    else if (pListExp.listType == ListExpressionType::AND)
    {
      ...
      _filterDetailledAnsw(ansSubWorkStruct.compositeSemAnswers->semAnswers,
                           newGrdExpAnswers, true, pMemViewer.constView, pWorkStruct.lingDb);
    ...
    else
    {
      ...
      _filterDetailledAnsw(subWorkStruct.compositeSemAnswers->semAnswers,
                           existingGrdExpAnswers, false, pMemViewer.constView, pWorkStruct.lingDb);
        ...
        ConceptSet::haveAConceptThatBeginWith(fdkGrdExpPtr->grounding().concepts, "engagement_"))
    {
      ...
      if (concernedGrdExpPtr != nullptr)
      {
        ...
        if (concernedAgentPtr != nullptr &&
            pMemViewer.constView.isItMe(concernedAgentPtr->userId))
        {
          ...
          if (pWorkStruct.author != nullptr)
          {
            ...
            if (subWorkStruct.askForNewRecursion())
            {
              ...
              if (subWorkStruct.compositeSemAnswers)
              {
                ...
                CompositeSemAnswer::getGrdExps(grdExpAnswers, subWorkStruct.compositeSemAnswers->semAnswers);
                                     ...
                                     SemanticLanguageEnum pLanguage)
{
  ...
  SemControllerWorkingStruct subWorkStruct
      (InformationType::ASSERTION, &*robotAgent, pLanguage,
       ...
       pWorkStruct.externalFallbackPtr, pWorkStruct.callbackToSentencesCanBeAnsweredPtr,
       pWorkStruct.axiomToConditionCurrentStatePtr, pWorkStruct.lingDb);
...
{
  SemControllerWorkingStruct subWorkStruct
      (InformationType::ASSERTION, nullptr, pFromLanguage,
       pWorkStruct.expHandleInMemory, SemanticOperatorEnum::INFORM, pWorkStruct.proativeSpecificationsPtr,
       pWorkStruct.externalFallbackPtr, pWorkStruct.callbackToSentencesCanBeAnsweredPtr,
       pWorkStruct.axiomToConditionCurrentStatePtr, pWorkStruct.lingDb);
                           ...
                           const SemanticExpression& pSemExp)
{
    ...
    if (originalGrdExpForm != nullptr)
    {
      ...
      for (const auto& currForms : setOfFormsExp.prioToForms)
      {
        ...
        for (auto& currDetailledAnwer : lastTextAnalyze.compositeSemAnswers->semAnswers)
        {
          ...
          if (leafPtr != nullptr &&
              leafPtr->type != ContextualAnnotation::ANSWERNOTFOUND &&
  ...
  case SemanticExpressionType::METADATA:
  {
    ...
    if (storeSource)
    {
      ...
      _punctuallyAssertAboutTheSource(pWorkStruct, pMemViewer, *sourceInPresent,
                                      metadataExp.fromLanguage);
    }

    pWorkStruct.authorSemExp = metadataExp.getAuthorSemExpPtr();
    pWorkStruct.author = pWorkStruct.authorSemExp != nullptr ? SemExpGetter::extractAgentGrdPtr(*pWorkStruct.authorSemExp) : nullptr;
    ...
    {
      _permanentAssertAboutTheSource(pWorkStruct, pMemViewer, metadataExp.fromLanguage,
                                     **metadataExp.source);
    }
    break;
  }
                           ...
                           const linguistics::LinguisticDatabase& pLingDb)
{
  SemControllerWorkingStruct workStruct
      (pInformationType, nullptr, SemanticLanguageEnum::UNKNOWN,
       nullptr, pReactionOperator, &pSemanticMemory.proativeSpecifications,
       pSemanticMemory.getExternalFallback(), &pSemanticMemory.callbackToSentencesCanBeAnswered,
       nullptr, pLingDb);
  SemanticMemoryBlockViewer memViewer(&pSemanticMemory.memBloc, pSemanticMemory.memBloc, pSemanticMemory.getCurrUserId());
...
{
  SemControllerWorkingStruct workStruct
      (pInformationType, nullptr, SemanticLanguageEnum::UNKNOWN, nullptr, pReactionOperator,
       pProativeSpecificationsPtr, pExternalFallbackPtr, pCallbackToSentencesCanBeAnsweredPtr,
       pAxiomToConditionCurrentState, pLingDb);
  ...
  case SemanticOperatorEnum::REACT:
  {
        ...
        pWorkStruct.reactionOptions.canAnswerIDontKnow)
    {
      bool reAskTheQuestion = pWorkStruct.author != nullptr &&
          pWorkStruct.externalFallbackPtr == nullptr &&
      ...
      if (pWorkStruct.externalFallbackPtr != nullptr)
        pWorkStruct.externalFallbackPtr->addFallback(knowledgeWeCannotDoIt,
                                                     pMemViewer.currentUserId,
                                                     pOriginalGrdExp);
      pWorkStruct.addAnswerWithoutReferences
          (ContextualAnnotation::ANSWERNOTFOUND, std::move(knowledgeWeCannotDoIt));
      if (reAskTheQuestion)
...
bool _keepOnlyWhatIsLessDetailledFromAxiomList(
    std::list<AXIOMTYPE*>& pAxiomList,
    SemanticMemoryBlockViewer& pMemViewer,
    const linguistics::LinguisticDatabase& pLingDb,
    const GroundedExpression& pGrdExp)
{
...
bool _keepOnlyWhatISLessDetailled(
    RelatedContextAxiom& pAnswersContextAxioms,
    SemanticMemoryBlockViewer& pMemViewer,
    const linguistics::LinguisticDatabase& pLingDb,
    const GroundedExpression& pGrdExp)
{
  bool res = _keepOnlyWhatIsLessDetailledFromAxiomList(pAnswersContextAxioms.elts, pMemViewer,
                                                       pLingDb, pGrdExp);
  return _keepOnlyWhatIsLessDetailledFromAxiomList(pAnswersContextAxioms.constElts, pMemViewer,
                                                   pLingDb, pGrdExp) || res;
}


TruenessValue _checkIfItsTrue(
    RelatedContextAxiom& pAnswersContextAxioms,
    SemanticMemoryBlockViewer& pMemViewer,
    ...
    const GroundedExpression& pGrdExp)
{
  ...
  if (subWorkStruct.askForNewRecursion())
  {
    ...
    controller::applyOperatorOnGrdExp(subWorkStruct, pMemViewer, pGrdExp,
                                      emptyListOfGrdExps, pGrdExp);
                              ...
                              SemanticMemoryBlockViewer& pMemViewer)
{
  ...
  semanticMemoryLinker::checkForConditionsLinkedToStatement(pWorkStruct, pMemViewer, reqLinks,
                                                            pMemSentenceToUpdate.grdExp);
                      ...
                      const SemanticStatementGrounding& pStatementGrd)
{
  ...
  case SemanticOperatorEnum::HOWYOUKNOW:
  {
    ...
    if (!pStatementGrd.isAtInfinitive())
      applyOperatorOnSemExp(pWorkStruct, pMemViewer,
                            *SemExpCreator::askIfTrue(pGrdExp, pWorkStruct.lingDb));
    break;
  }
  case SemanticOperatorEnum::REACT:
  case SemanticOperatorEnum::INFORM:
  ...
  case SemanticOperatorEnum::TEACHINFORMATION:
  {
    ...
    {
      truenessValue = _checkIfItsTrue(answersContextAxioms, pMemViewer,
                                      pWorkStruct, pGrdExp);
      canSaveNewGrdExp = !_keepOnlyWhatISLessDetailled(answersContextAxioms, pMemViewer,
    ...
    bool isNewInformationRevelant =
        (pWorkStruct.informationType == InformationType::ASSERTION ||
         pWorkStruct.informationType == InformationType::FALLBACK ||
         !infoIsAlreadyAnAssertion) && canSaveNewGrdExp;

    if (pWorkStruct.reactOperator == SemanticOperatorEnum::REACT ||
        pWorkStruct.reactOperator == SemanticOperatorEnum::INFORM ||
        (pWorkStruct.reactOperator == SemanticOperatorEnum::TEACHBEHAVIOR && privateImplem::categorizeGrdExp(pGrdExp, pWorkStruct.getAuthorUserId()) == SemanticExpressionCategory::ACTIONDEFINITION) ||
        (pWorkStruct.reactOperator == SemanticOperatorEnum::TEACHINFORMATION && privateImplem::categorizeGrdExp(pGrdExp, pWorkStruct.getAuthorUserId()) == SemanticExpressionCategory::AFFIRMATION))
    {
      if (pWorkStruct.expHandleInMemory != nullptr &&
    ...
    if (pWorkStruct.reactionOptions.canDoAProactivity)
    {
          ...
          pWorkStruct.reactOperator == SemanticOperatorEnum::TEACHBEHAVIOR)
      {
                 ...
                 pWorkStruct.contAnnotationOfPreviousAnswers != ContextualAnnotation::QUESTION)
        {
          ...
          bool res = false;
          bool cannotHaveOtherFeedbacks = proactiveReaction::process(res, pWorkStruct, newContextAxiom, pMemViewer, pGrdExp,
                                                                     truenessValue, infoIsAlreadyAnAssertion,
                                                                     answersContextAxioms);
          setEnabledPreviousInformation(false);
          if (!cannotHaveOtherFeedbacks)
            proactiveReaction::processWithUpdatedMemory(res, pWorkStruct, newContextAxiom, pMemViewer, pGrdExp);
        }
      }
  ...
  for (auto& currDetAnsw : pCompositeSemAnswer.semAnswers)
  {
    ...
    if (currReaction)
    {
      if (!pReaction)
        ...
        SemExpModifier::addListElt(*pReaction,
                                   pCompositeSemAnswer.listType,
                                   *currReaction);
    }
  }
}
...
ContextualAnnotation compAnswerToContextualAnnotation
(CompositeSemAnswer& pCompositeSemAnswer)
{
...
TruenessValue operator_check_semExp
(const SemanticExpression& pSemExp,
 const SemanticMemoryBlock& pConstMemBlock,
 const std::string& pCurrentUserId,
 const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  applyOperatorOnSemExpConstMem(compositeSemAnswers, pSemExp,
                                SemanticOperatorEnum::CHECK, InformationType::INFORMATION,
                                pConstMemBlock, pCurrentUserId,
    ...
    if (leafAnswer.type == ContextualAnnotation::NOTIFYSOMETHINGWILLBEDONE)
    {
      if (leafAnswer.condition)
      {
        ...
        if (condRes.thenCategory == SemanticExpressionCategory::COMMAND)
        {
          ...
          {
            const GroundedExpression* conditionGrdExpPtr = condRes.condition.conditionExp.getGrdExpPtr_SkipWrapperPtrs();
            if (conditionGrdExpPtr != nullptr)
              semanticMemoryLinker::disableActionsLinkedToASentence(pSemanticMemory, *conditionGrdExpPtr, pLingDb);
            continue;
          }
      ...
      if (leafAnswer.conditionForAUser.thenSemExp &&
          !leafAnswer.conditionForAUser.user.empty())
      {
        pSemanticMemory.addNewUserFocusedToSemExp
            (std::move(leafAnswer.conditionForAUser.thenSemExp),
...
void convertToDetalledAnswer
(std::list<std::unique_ptr<SemAnswer>>& pDetailledAnswers,
 SemControllerWorkingStruct& pWorkStruct)
{
...
{
  SemControllerWorkingStruct workStruct
      (InformationType::ASSERTION, nullptr, SemanticLanguageEnum::UNKNOWN,
       nullptr, SemanticOperatorEnum::INFORM, &pSemanticMemory.proativeSpecifications,
       pSemanticMemory.getExternalFallback(), &pSemanticMemory.callbackToSentencesCanBeAnswered,
       pAxiomToConditionCurrentStatePtr, pLingDb);
  SemanticMemoryBlockViewer memViewer(&pSemanticMemory.memBloc, pSemanticMemory.memBloc, pSemanticMemory.getCurrUserId());
