    ...
    if (objectGrdExpPtr != nullptr) {
        ...
        auto getObjPositionOfGenericGrd = [&](const SemanticGenericGrounding& pGenGrd) {
            if ((pRequests.has(SemanticRequestType::OBJECT) || pRequests.has(SemanticRequestType::QUANTITY))
                && !pGenGrd.coreference && pGenGrd.referenceType != SemanticReferenceType::DEFINITE)
                ...
                if (objObjGrdExpPtr != nullptr) {
                    ...
                    if (objObjGenExpPtr != nullptr) {
                        ...
                        if (objObjGenExpObjPos == ObjectPosition::BEFOREVERB) {
                            ...
                            _syntGrounding.writeVerbalSemWord(pSentWorkStruct.outs.verb2.out,
...
LinguisticVerbTense LinguisticSynthesizerEnglish::_semanticVerbTenseToLinguisticVerbTense(
    ...
    switch (pSemVerbTense) {
        ...
        case SemanticVerbTense::PRESENT: return LinguisticVerbTense::PRESENT_INDICATIVE;
        case SemanticVerbTense::PUNCTUALPRESENT: {
            if (pWordInfosPtr != nullptr && pWordInfosPtr->hasContextualInfo(WordContextualInfos::CANNOTBECONTINUOUS))
    ...
    for (const auto& currRequest : pRequests.types) {
        ...
        switch (currRequest) {
            ...
            case SemanticRequestType::QUANTITY: {
                if (pObjectPtr != nullptr && SemExpGetter::isUncountableSemExp(**pObjectPtr, pConf.lingDb) == true)
            ...
            case SemanticRequestType::OBJECT: {
                ...
                if (pObjectPtr != nullptr) {
                    ...
                    if (objGrdPtr != nullptr) {
                        ...
                        if (objGenGrdPtr != nullptr && objGenGrdPtr->word.isEmpty() && objGenGrdPtr->concepts.empty()
                            && !objGenGrdPtr->coreference) {
                            _strToOut(pOut, PartOfSpeech::PRONOUN, "which");
                            pObjectPtr = nullptr;
                ...
                if (!introWordWritten)
                    _strToOut(pOut,
                              ...
                              pHoldingContextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERNOUN
            ...
            case SemanticRequestType::SUBJECT: {
                if (pSubjectPtr != nullptr && SemExpGetter::isWhoSemExp(**pSubjectPtr)) {
                    _strToOut(pOut, PartOfSpeech::PRONOUN, "who");
                    break;
                }
                _strToOut(pOut,
            ...
            case SemanticRequestType::WAY: _strToOut(pOut, PartOfSpeech::PRONOUN, "how"); break;
            ...
            case SemanticRequestType::CAUSE: _strToOut(pOut, PartOfSpeech::PRONOUN, "why"); break;
            case SemanticRequestType::PURPOSE: _strToOut(pOut, PartOfSpeech::PRONOUN, "for what purpose"); break;
            case SemanticRequestType::TIMES: _strToOut(pOut, PartOfSpeech::PRONOUN, "how many times"); break;
            case SemanticRequestType::ABOUT: _strToOut(pOut, PartOfSpeech::PRONOUN, "what about"); break;
            case SemanticRequestType::TOPIC: _strToOut(pOut, PartOfSpeech::PRONOUN, "what"); break;
...
void LinguisticSynthesizerEnglish::_getPossessiveDeterminer(std::list<WordToSynthesize>& pOut,
                                                            RelativePerson pRelPerson,
                                                            SemanticGenderType pGender,
                                                            SemanticNumberType) const {
    _strToOut(pOut, PartOfSpeech::DETERMINER, [&]() {
        switch (pRelPerson) {
            case RelativePerson::FIRST_SING: return "my";
            case RelativePerson::SECOND_SING:
            case RelativePerson::SECOND_PLUR: return "your";
            case RelativePerson::THIRD_SING:
            case RelativePerson::UNKNOWN: return pGender == SemanticGenderType::FEMININE ? "her" : "his";
            case RelativePerson::FIRST_PLUR: return "our";
            case RelativePerson::THIRD_PLUR: return "their";
        }
        assert(false);
        return "";
    }());
...
void LinguisticSynthesizerEnglish::_getBeginOfSpecification(std::list<WordToSynthesize>& pOut,
    ...
    pOut.emplace_back(SemanticWord(_language, specificationWordStr, PartOfSpeech::PREPOSITION),
                      InflectionToSynthesize(specificationWordStr, true, true, ifNextWordIsNotAPrepostion));
}

void LinguisticSynthesizerEnglish::_getBeginOfWithChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "with");
}

void LinguisticSynthesizerEnglish::_getBeginOfWithoutChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "without");
}

void LinguisticSynthesizerEnglish::_getBeginOfForChild(std::list<WordToSynthesize>& pOut,
                                                       const SemanticExpression& pSemExp) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, SemExpGetter::semExphasAStatementGrd(pSemExp) ? "in order to" : "for");
}

void LinguisticSynthesizerEnglish::_getBeginOfReceiverOfNounChild(std::list<WordToSynthesize>& pOut,
...
void LinguisticSynthesizerEnglish::_getBeginOfMitigationChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "but");
}

void LinguisticSynthesizerEnglish::_getBeginOfYesOrNoSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "if");
}

void LinguisticSynthesizerEnglish::_getWheneverCondition(std::list<WordToSynthesize>& pOut) const {
...
void LinguisticSynthesizerEnglish::_getBeginOfBetweenSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "between");
}

void LinguisticSynthesizerEnglish::_getBeginOfCauseSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "because");
}

void LinguisticSynthesizerEnglish::_getThenWord(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::ADVERB, "then");
}

void LinguisticSynthesizerEnglish::_getElseWord(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::ADVERB, "else");
}

void LinguisticSynthesizerEnglish::_getPossessiveWord(std::list<WordToSynthesize>& pOut,
    ...
    _strToOut(pOut, PartOfSpeech::DETERMINER, [&]() {
        switch (pOwnerRelativePerson) {
            case RelativePerson::FIRST_SING: return "mine";
            case RelativePerson::SECOND_SING: return "yours";
            case RelativePerson::THIRD_SING: {
            ...
            case RelativePerson::FIRST_PLUR: return "ours";
            case RelativePerson::SECOND_PLUR: return "yours";
            case RelativePerson::THIRD_PLUR: return "theirs";
            case RelativePerson::UNKNOWN: return "its";
        }
        return "its";
    }());
}
...
bool LinguisticSynthesizerEnglish::_doWeHaveToWriteBeginOfSpecifier(SynthesizerCurrentContext& pGrdContext,
...
void LinguisticSynthesizerEnglish::_getComparisonWord(std::list<WordToSynthesize>& pOut,
                                                      ComparisonOperator pCompPolarity,
                                                      SemanticGenderType,
                                                      SemanticNumberType) const {
    static const PartOfSpeech adj = PartOfSpeech::ADJECTIVE;
    switch (pCompPolarity) {
        case ComparisonOperator::DIFFERENT: _strToOut(pOut, adj, "different"); break;
        case ComparisonOperator::EQUAL: _strToOut(pOut, adj, "equal"); break;
        case ComparisonOperator::LESS: _strToOut(pOut, adj, "inferior"); break;
        case ComparisonOperator::MORE: _strToOut(pOut, adj, "superior"); break;
    }
}
