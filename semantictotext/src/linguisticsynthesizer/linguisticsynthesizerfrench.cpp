...
namespace {
const std::vector<std::string> _rootVerbsForSubjonctiveInSubordinate{"verb_like",
                                                                     "verb_want",
                                                                     "verb_doubt",
                                                                     "verb_require"};
...
bool _doWeNeedToPutObjectGenGrdBeforeVerb(const SemanticGenericGrounding& pGenGrd,
                                          ...
                                          bool pVerbPolarity) {
    return pGenGrd.coreference && pGenGrd.word.lemma.empty() && pGenGrd.quantity.type != SemanticQuantityType::MAXNUMBER
...
LinguisticVerbTense LinguisticSynthesizerFrench::_semanticVerbTenseToLinguisticVerbTense(
    SemanticVerbTense pSemVerbTense,
    ...
    switch (pSemVerbTense) {
        case SemanticVerbTense::FUTURE: return LinguisticVerbTense::FUTURE_INDICATIVE;
        case SemanticVerbTense::PAST: return LinguisticVerbTense::IMPERFECT_INDICATIVE;
        ...
        case SemanticVerbTense::PUNCTUALPRESENT: {
            if (pContextType == SYNTHESIZERCURRENTCONTEXTTYPE_OBJECTBEFOREVERB
                ...
                if (pRequests.empty() && pRootStatementPtr != nullptr
                    && ConceptSet::haveAnyOfConcepts(pRootStatementPtr->concepts,
                                                     _rootVerbsForSubjonctiveInSubordinate))
        ...
        case SemanticVerbTense::PUNCTUALPAST: return LinguisticVerbTense::SIMPLE_PAST_INDICATIVE;
        case SemanticVerbTense::UNKNOWN: return LinguisticVerbTense::INFINITIVE;
    }
    return LinguisticVerbTense::INFINITIVE;
}

void LinguisticSynthesizerFrench::_getPossessiveDeterminer(std::list<WordToSynthesize>& pOut,
    ...
    switch (pRelPerson) {
        ...
        case RelativePerson::FIRST_PLUR:
            _strToOut(pOut, det, pNumber == SemanticNumberType::PLURAL ? "nos" : "notre");
            return;
        case RelativePerson::SECOND_PLUR:
            _strToOut(pOut, det, pNumber == SemanticNumberType::PLURAL ? "vos" : "votre");
            return;
        case RelativePerson::THIRD_PLUR:
            _strToOut(pOut, det, pNumber == SemanticNumberType::PLURAL ? "leurs" : "leur");
            return;
    }
    assert(false);
}
...
void LinguisticSynthesizerFrench::_getBeginOfSpecification(std::list<WordToSynthesize>& pOut,
    ...
    if (pWordContext.number != SemanticNumberType::PLURAL
        ...
        pOut.emplace_back([&] {
            WordToSynthesize wordToToSynth(SemanticWord(_language, "de", PartOfSpeech::PREPOSITION),
                                           InflectionToSynthesize("d'", true, false, [](const WordToSynthesize& pNext) {
                                               return ifNeedAnApostropheBefore(pNext)
                                                   && ifNextWordIsNotAPrepostion(pNext);
                                           }));
...
void LinguisticSynthesizerFrench::_getNegationsBeforeVerb(std::list<WordToSynthesize>& pOut) const {
    _strWithApostropheToOut(pOut, PartOfSpeech::ADVERB, "n'", "ne");
}

void LinguisticSynthesizerFrench::_getNegationsAfterVerb(std::list<WordToSynthesize>& pOut) const {
...
LinguisticSynthesizerPrivate::ObjectPosition LinguisticSynthesizerFrench::_getObjectPosition(
    ...
    if (objectGrdExpPtr != nullptr) {
        ...
        auto getObjPositionOfGenericGrd = [&](const SemanticGenericGrounding& pGenGrd) {
            ...
            if (pRequests.has(SemanticRequestType::OBJECT) || pRequests.has(SemanticRequestType::QUANTITY)) {
        ...
        switch (objectGrd.type) {
            case SemanticGroundingType::STATEMENT: {
                ...
                if (objectStatGrd.isAtInfinitive() && objectGrdExp.children.size() == 1) {
                    ...
                    if (objObjGrdExpPtr != nullptr) {
                        ...
                        if (objObjGenExpPtr != nullptr) {
                            ...
                            if (objObjGenExpObjPos == ObjectPosition::BEFORESUBJECT) {
                                ...
                                _syntGrounding.writeSemWord(pSentWorkStruct.outs.verb2.out,
                                                            pSentWorkStruct.outs.verb2,
                                                            pSentWorkStruct.outs.verb2.out,
                                                            objectStatGrd.word,
                                                            pConf.lingDb,
                                                            context);
            ...
            case SemanticGroundingType::UNITY: return LinguisticSynthesizerPrivate::ObjectPosition::BEFORESUBJECT;
            default: return LinguisticSynthesizerPrivate::ObjectPosition::AFTERVERB;
        }
    }
    return LinguisticSynthesizerPrivate::ObjectPosition::AFTERVERB;
}
...
LinguisticSynthesizerPrivate::ReceiverPosition LinguisticSynthesizerFrench::_getReceiverPosition(
    ...
    if (objectGrdExpPtr != nullptr) {
        ...
        switch (objectGrd.type) {
            ...
            case SemanticGroundingType::NAME: return LinguisticSynthesizerPrivate::ReceiverPosition::AFTERVERB;
            default:
...
void LinguisticSynthesizerFrench::_writeQuel(std::list<WordToSynthesize>& pOut,
    ...
    if (SemExpGetter::getNumber(pSemExp) == SemanticNumberType::PLURAL)
        _strToOut(pOut, PartOfSpeech::PRONOUN, gender == SemanticGenderType::FEMININE ? "quelles" : "quels");
    else
        _strToOut(pOut, PartOfSpeech::PRONOUN, gender == SemanticGenderType::FEMININE ? "quelle" : "quel");
}
...
void LinguisticSynthesizerFrench::_getQuestionWord(std::list<WordToSynthesize>& pOut,
        ...
        switch (currRequest) {
            ...
            case SemanticRequestType::QUANTITY: {
                if (pContext.verbContextOpt) {
                    ...
                    pConf.lingDb.langToSpec[_language].getSemFrameDict().getIntroWord(
                        introWord,
                        linguistics::ChunkLinkType::DIRECTOBJECT,
            ...
            case SemanticRequestType::OBJECT: {
                ...
                } else if (objectPtr != nullptr && objectPtr->getSemExp().getListExpPtr_SkipWrapperPtrs() != nullptr) {
                    ...
                    if (pStatGrd.noVerb()) {
                        ...
                        if (pObjectPtr != nullptr) {
                            ...
                            if (objGrdPtr != nullptr) {
                                ...
                                if (objGenGrdPtr != nullptr && objGenGrdPtr->word.isEmpty()
                                    && objGenGrdPtr->concepts.empty() && !objGenGrdPtr->coreference) {
            ...
            case SemanticRequestType::SUBJECT: {
                ...
                if (pHoldingContextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERNOUN
                    || (pSubjectPtr != nullptr && SemExpGetter::isWhoSemExp(**pSubjectPtr))) {
                    _strToOut(pOut, PartOfSpeech::PRONOUN, "qui");
                    break;
                }
                ...
                bool needToWriteSubject =
                    pIsEquVerb && pSubjectPtr == nullptr
                    && (pObjectPtr == nullptr
                        || SemExpGetter::getMainPartOfSpeech(**pObjectPtr) != PartOfSpeech::ADJECTIVE);
                if (pHoldingContextType == SYNTHESIZERCURRENTCONTEXTTYPE_OBJECTAFTERVERB) {
                    _strToOut(pOut, PartOfSpeech::PRONOUN, "ce");
            ...
            case SemanticRequestType::MANNER: _strToOut(pOut, PartOfSpeech::ADVERB, "comment"); break;
            case SemanticRequestType::CAUSE:
            case SemanticRequestType::PURPOSE: _strToOut(pOut, PartOfSpeech::ADVERB, "pourquoi"); break;
            case SemanticRequestType::TIMES: _strToOut(pOut, PartOfSpeech::ADVERB, "combien de fois"); break;
            ...
            case SemanticRequestType::TOPIC: _strToOut(pOut, PartOfSpeech::PRONOUN, "que"); break;
            case SemanticRequestType::WAY: _strToOut(pOut, PartOfSpeech::PRONOUN, "en quoi"); break;
            ...
            case SemanticRequestType::VERB: break;
        }
    }
}

bool LinguisticSynthesizerFrench::_tryToWriteTopicBeforeVerb(std::list<WordToSynthesize>& pOut,
...
void LinguisticSynthesizerFrench::_getBeginOfWithChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "avec");
}

void LinguisticSynthesizerFrench::_getBeginOfWithoutChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "sans");
}

void LinguisticSynthesizerFrench::_getBeginOfForChild(std::list<WordToSynthesize>& pOut,
                                                      const SemanticExpression&) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "pour");
}

void LinguisticSynthesizerFrench::_getBeginOfReceiverOfNounChild(std::list<WordToSynthesize>& pOut,
                                                                 const SemanticExpression&) const {
...
void LinguisticSynthesizerFrench::_getBeginOfMitigationChild(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "mais");
}

void LinguisticSynthesizerFrench::_getBeginOfYesOrNoSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "si");
}

bool LinguisticSynthesizerFrench::_beginOfSubordonateIfNeeded(bool& pIsASubordinateWithoutPreposition,
    ...
    if (pSubordinateGrammaticalType == GrammaticalType::OBJECT) {
        ...
        if (statGrd != nullptr && statGrd->requests.empty()
            ...
            const bool objectIsAParentCoreference =
                itObject != pSubordinateGrdExp.children.end()
            ...
            if (!objectIsAParentCoreference
                ...
                _strWithApostropheToOut(pOutSemExp.out, PartOfSpeech::SUBORDINATING_CONJONCTION, "qu'", "que");
                return true;
            }
...
void LinguisticSynthesizerFrench::_getWheneverCondition(std::list<WordToSynthesize>& pOut) const {
    _strWithApostropheToOut(pOut, PartOfSpeech::CONJUNCTIVE, "à chaque fois qu'", "à chaque fois que");
}
...
void LinguisticSynthesizerFrench::_getBeginOfBetweenSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, "entre");
}

void LinguisticSynthesizerFrench::_getBeginOfCauseSubordonate(std::list<WordToSynthesize>& pOut) const {
    _strWithApostropheToOut(pOut, PartOfSpeech::CONJUNCTIVE, "parce qu'", "parce que");
}

void LinguisticSynthesizerFrench::_getThenWord(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::ADVERB, "alors");
}

void LinguisticSynthesizerFrench::_getElseWord(std::list<WordToSynthesize>& pOut) const {
    _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, "sinon");
}

void LinguisticSynthesizerFrench::_getPossessiveWord(std::list<WordToSynthesize>& pOut,
    ...
    _strToOut(pOut, PartOfSpeech::DETERMINER, [&]() {
        switch (pOwnerRelativePerson) {
            case RelativePerson::FIRST_SING: return "le mien";
            case RelativePerson::SECOND_SING: return "le tien";
            case RelativePerson::THIRD_SING: return "leur";
            case RelativePerson::FIRST_PLUR: return "le notre";
            case RelativePerson::SECOND_PLUR: return "le votre";
            case RelativePerson::THIRD_PLUR: return "leur";
            case RelativePerson::UNKNOWN: return "leur";
        }
        return "leur";
    }());
}
...
LinguisticSynthesizerPrivate::SpecifierPosition LinguisticSynthesizerFrench::_getSpecifierPosition(
    ...
    if (pInflWord.word.partOfSpeech == PartOfSpeech::NOUN || pInflWord.word.partOfSpeech == PartOfSpeech::PROPER_NOUN)
        return SpecifierPosition::AFTER;
    if (!pADetermantHasBeenWrote && pInflWord.word.partOfSpeech == PartOfSpeech::ADVERB
...
bool LinguisticSynthesizerFrench::_doWeHaveToWriteBeginOfSpecifier(SynthesizerCurrentContext& pGrdContext,
                                                                   const linguistics::InflectedWord& pInflWord,
    ...
    return grdOfTheWord.type == SemanticGroundingType::AGENT || grdOfTheWord.type == SemanticGroundingType::META;
}

void LinguisticSynthesizerFrench::_getComparisonWord(std::list<WordToSynthesize>& pOut,
    ...
    if (pGender == SemanticGenderType::FEMININE) {
        switch (pCompPolarity) {
            case ComparisonOperator::DIFFERENT:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "différentes" : "différente");
                break;
            case ComparisonOperator::EQUAL:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "égales" : "égale");
                break;
            case ComparisonOperator::LESS:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "inférieures" : "inférieure");
                break;
            case ComparisonOperator::MORE:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "supérieures" : "supérieure");
                break;
        }
    } else {
        switch (pCompPolarity) {
            case ComparisonOperator::DIFFERENT:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "différents" : "différent");
                break;
            case ComparisonOperator::EQUAL:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "égaux" : "égal");
                break;
            case ComparisonOperator::LESS:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "inférieurs" : "inférieur");
                break;
            case ComparisonOperator::MORE:
                _strToOut(pOut, adj, pNumber == SemanticNumberType::PLURAL ? "supérieurs" : "supérieur");
                break;
        }
    }
}
