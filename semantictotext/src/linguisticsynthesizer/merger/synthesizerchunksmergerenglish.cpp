...
void SynthesizerChunksMergerEnglish::formulateSentence(std::list<WordToSynthesize>& pOut,
                                                       OutSentence& pOutSentence) const {
    if (!pOutSentence.requests.empty()) {
        if (pOutSentence.requests.has(SemanticRequestType::ACTION) && !pOutSentence.verb.out.empty()
            && pOutSentence.verb.out.front().word.lemma == "let" && !pOutSentence.objectAfterVerb.out.empty()
...
void SynthesizerChunksMergerEnglish::_formulation_objectQuestionWithSubjetAfterTheVerb(
    ...
    if (pOutSentence.aux.out.empty()) {
        ...
        if (pOutSentence.requests.has(SemanticRequestType::SUBJECT) && !pOutSentence.aux.out.empty()
...
void SynthesizerChunksMergerEnglish::_formulation_default(std::list<WordToSynthesize>& pOut,
                                                          OutSentence& pOutSentence) const {
    if (pOutSentence.aux.out.empty()) {
        pOut.splice(pOut.end(), pOutSentence.subject.out);
...
void SynthesizerChunksMergerEnglish::_writEndOfSentence(std::list<WordToSynthesize>& pOut,
                                                        OutSentence& pOutSentence) const {
    pOut.splice(pOut.end(), pOutSentence.inCaseOf.out);

    const bool needToPopLocationPreposition = pOutSentence.verb2.getLastPartOfSpeech() == PartOfSpeech::PREPOSITION
                                           && pOutSentence.manner.empty() && pOutSentence.receiverAfterVerb.empty()
                                           && pOutSentence.objectAfterVerb.empty() && !pOutSentence.location.out.empty()
