...
bool _shouldWriteSubjectAfterTheVerb(const std::list<WordToSynthesize>& pOut, OutSentence& pOutSentence) {
    bool requestisYesOrNoOrCause = pOutSentence.requests.has(SemanticRequestType::YESORNO)
                                || pOutSentence.requests.has(SemanticRequestType::CAUSE);
    if (pOutSentence.requests.empty() || pOutSentence.requests.has(SemanticRequestType::SUBJECT)
    ...
    if (pOutSentence.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_GENERIC) {
        if (pOutSentence.equVerb
            || (!requestisYesOrNoOrCause && !pOutSentence.aux.out.empty()
                && pOutSentence.aux.out.front().word.lemma == "être")) {
            if (pOutSentence.subject.partOfSpeech == PartOfSpeech::PRONOUN_SUBJECT
                || pOutSentence.subject.partOfSpeech == PartOfSpeech::PRONOUN) {
                if (pOut.empty() || mystd::differentThanBoth(pOut.back().word.lemma, "qui", "que")
                    ...
                    return true;
                       ...
                       && !pOutSentence.subject.out.empty() && pOutSentence.subject.out.front().word.lemma != "ça")
                return true;
        } else {
            if (!pOut.empty() && pOut.back().word.lemma == "qui"
                && (pOutSentence.subject.out.empty() || pOutSentence.subject.out.front().word.lemma != "je"))
...
void _tryToAddEstCeQue(std::list<WordToSynthesize>& pOut, OutSentence& pOutSentence) {
    if (pOutSentence.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_GENERIC) {
        for (const auto& currRequest : pOutSentence.requests.types) {
            if (currRequest == SemanticRequestType::ABOUT) {
                synthTool::strWithApostropheToOut(
                    pOut, PartOfSpeech::DETERMINER, "d'", "de", SemanticLanguageEnum::FRENCH);
                break;
            }

            if (currRequest == SemanticRequestType::TIME || currRequest == SemanticRequestType::YESORNO
                ...
                if (pOutSentence.subject.out.empty()) {
                    if (pOutSentence.verbTense != SemanticVerbTense::UNKNOWN)
                        synthTool::strToOut(pOut, PartOfSpeech::ADVERB, "est-ce qui", SemanticLanguageEnum::FRENCH);
                    break;
                }
                synthTool::strWithApostropheToOut(
                    pOut, PartOfSpeech::ADVERB, "est-ce qu'", "est-ce que", SemanticLanguageEnum::FRENCH);
                break;
            }
        }
    }
...
void _tryToAddHyphen(std::list<WordToSynthesize>& pOut, OutSentence& pOutSentence) {
    if (!pOutSentence.subject.out.empty() && partOfSpeech_isPronominal(pOutSentence.subject.partOfSpeech)
        && !pOut.empty()) {
        WordToSynthesize& wordToSynth = pOut.back();
        std::string hyphenStr = "-";
        if (pOutSentence.subject.relativePerson == RelativePerson::THIRD_SING && !wordToSynth.inflections.empty()) {
...
void SynthesizerChunksMergerFrench::formulateSentence(std::list<WordToSynthesize>& pOut,
...
void SynthesizerChunksMergerFrench::_formulation_objectQuestionWithSubjetAfterTheVerb(std::list<WordToSynthesize>& pOut,
                                                                                      OutSentence& pOutSentence) const {
...
void SynthesizerChunksMergerFrench::_formulation_default(std::list<WordToSynthesize>& pOut,
                                                         OutSentence& pOutSentence) const {
    pOut.splice(pOut.end(), pOutSentence.objectBeforeSubject.out);
...
void SynthesizerChunksMergerFrench::_writEndOfSentence(std::list<WordToSynthesize>& pOut,
                                                       OutSentence& pOutSentence) const {
