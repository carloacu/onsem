...
UniqueSemanticExpression _semExpReplacement(UniqueSemanticExpression pSemExp,
    ...
    if (listExpPtr != nullptr && !listExpPtr->elts.empty()) {
        ...
        if (listExpBasePtr != nullptr && !listExpBasePtr->elts.empty()) {
            auto& lastListExpElt = listExpPtr->elts.back();
            const auto& lastListExpBase = listExpBasePtr->elts.back();
            return _semExpReplacement(
                std::move(lastListExpElt), *lastListExpBase, pGrdExpToReplace, pMemBlock, pLingDb);
        }
    }
    return pSemExp;
}
    ...
    switch (pSemExp->type) {
        ...
        case SemanticExpressionType::COMPARISON: {
            ...
            if (compExp.whatIsComparedExp)
                _replaceSemExpThatNeedToBeReplaced(
                    *compExp.whatIsComparedExp, pUsSemExp, pGrdExpToReplace, pMemBlock, pLingDb);
            ...
            if (compExp.rightOperandExp)
                _replaceSemExpThatNeedToBeReplaced(
                    *compExp.rightOperandExp, pUsSemExp, pGrdExpToReplace, pMemBlock, pLingDb);
            break;
        }
        ...
        case SemanticExpressionType::METADATA: {
            ...
            if (metadataExp.source)
                _replaceSemExpThatNeedToBeReplaced(
                    *metadataExp.source, pUsSemExp, pGrdExpToReplace, pMemBlock, pLingDb);
...
void synthesize(std::list<std::unique_ptr<SynthesizerResult>>& pNaturalLanguageResult,
    ...
    pLingDb.treeConverter.refactorSemExp(
        pSemExp, TREEPATTERN_MIND, TREEPATTERN_OUTTEXT, pTextProcContext.langType, pDebugOutput);
    _replaceSemExpThatNeedToBeReplaced(
        pSemExp,
        *pTextProcContext.usSemExp,
    ...
    synthPriv.getTranslationToNaturalLanguage(
        pNaturalLanguageResult, *pSemExp, pOneLinePerSentence, pMemBlock, pCurrentUserId, pTextProcContext, pLingDb);
}
