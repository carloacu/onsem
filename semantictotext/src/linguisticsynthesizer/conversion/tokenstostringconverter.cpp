...
void TokensToStringConverter::writeOutputText(std::list<std::unique_ptr<SynthesizerResult>>& pNaturalLanguageResult,
                                              std::list<WordToSynthesize>& pOutBlocs,
                                              bool pOneLinePerSentence) const {
    static const WordToSynthesize endOfTextWord(
        SemanticWord(SemanticLanguageEnum::UNKNOWN, "", PartOfSpeech::PUNCTUATION),
        InflectionToSynthesize("", false, false, alwaysTrue));
    {
        ...
        while (itBlocs != pOutBlocs.begin()) {
            ...
            for (auto itPossibility = itBlocs->inflections.begin(); itPossibility != itBlocs->inflections.end();
                 ++itPossibility) {
    ...
    for (auto itBlocs = pOutBlocs.begin(); itBlocs != pOutBlocs.end(); ++itBlocs) {
        ...
        if (inflToSynt.fromResourcePtr != nullptr) {
            ...
            pNaturalLanguageResult.emplace_back(
                std::make_unique<SynthesizerTask>(inflToSynt.fromResourcePtr->resource));
        } else {
...
void TokensToStringConverter::_addAWord(std::string& pNaturalLanguageText,
