...
void LinguisticSynthesizerPrivate::getTranslationToNaturalLanguage(
...

    {
        ...
        _writeSemExp(semExpSyntTypes, outBlocs, requests, &lastSubject, pSemExp, privateConf, rootContext);
...
void LinguisticSynthesizerPrivate::_addPunctuation(std::list<WordToSynthesize>& pOut,
                                                   const std::list<SemExpTypeEnumFormSynt>& pSemExpSyntTypes,
                                                   const SemanticRequests& pRequests,
                                                   bool pOneLinePerSentence) const {
    if (pOut.empty() || (!pOut.back().inflections.empty() && !pOut.back().inflections.front().canHavePunctionAfter))
        return;
...
void LinguisticSynthesizerPrivate::_writeSemExp(std::list<SemExpTypeEnumFormSynt>& pSemExpSyntTypes,
    ...
    switch (pSemExp.type) {
        ...
        case SemanticExpressionType::COMPARISON: {
            ...
            pSemExpSyntTypes.emplace_back(
                _writeComparisonExp(pOutSemExp.out, pRequests, pLastSubject, compExp, pConf, pContext));
            return;
        }
        ...
        case SemanticExpressionType::INTERPRETATION: {
            ...
            _writeSemExp(
                pSemExpSyntTypes, pOutSemExp, pRequests, pLastSubject, *intExp.interpretedExp, pConf, pContext);
            return;
        }
        ...
        case SemanticExpressionType::FEEDBACK: {
            ...
            _writeSemExp(intjTypeOfSemExps,
                         interjectionOut,
                         pRequests,
                         pLastSubject,
                         feedbackExpChild,
            ...
            _writeSemExp(pSemExpSyntTypes, mainSemExpOut, pRequests, pLastSubject, concernedExpChild, pConf, pContext);
            if (pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_GENERIC && !pSemExpSyntTypes.empty()
        ...
        case SemanticExpressionType::ANNOTATED: {
            ...
            if (annExp.synthesizeAnnotations) {
                ...
                for (const auto& currAnnotation : annExp.annotations)
                    _writeNominalChild(outNominalGroup,
                                       pRequests,
                                       pLastSubject,
                                       pConf,
                                       currAnnotation.first,
                                       *currAnnotation.second,
                                       grdContext,
                                       nullptr,
                                       nullptr);
                _getChunksMerger().formulateNominalGroup(pOutSemExp.out, outNominalGroup);
            } else if (_handleSpecificLanguageSynthesis(pSemExpSyntTypes,
                                                        pOutSemExp,
                                                        pRequests,
                                                        pLastSubject,
                                                        annExp.annotations,
        ...
        case SemanticExpressionType::FIXEDSYNTHESIS: {
            ...
            if (itLangToSynth != fSynthExp.langToSynthesis.end())
                ...
                _writeSemExp(
                    pSemExpSyntTypes, pOutSemExp, pRequests, pLastSubject, fSynthExp.getSemExp(), pConf, pContext);
            return;
        }
    }
...
SemExpTypeEnumFormSynt LinguisticSynthesizerPrivate::_writeComparisonExp(
    ...
    const UniqueSemanticExpression* objectPtr = pCompExp.rightOperandExp ? &*pCompExp.rightOperandExp : nullptr;
    ...
    _startAQuestion(pOut,
                    outSentence.subject,
                    pRequests,
                    true,
                    SemanticVerbTense::UNKNOWN,
                    SemanticStatementGrounding(),
                    &pCompExp.leftOperandExp,
                    objectPtr,
                    false,
                    ObjectPosition::AFTERVERB,
                    childToPutBeforeSubject,
                    pConf,
                    pContext.contextType,
                    pContext,
                    true);
    ...
    subjectContext.wordContext.relativePerson =
        _semExpToRelativePerson(*pCompExp.leftOperandExp, pConf, pContext.isPartOfANameAssignement);

    if (!pRequests.has(SemanticRequestType::SUBJECT) || !SemExpGetter::isWhoSemExp(*pCompExp.leftOperandExp)) {
        _writeSubjectContext(outSentence, pLastSubject, pCompExp.leftOperandExp, subjectContext, pConf, pRequests);
    }

    {
        SynthesizerCurrentContext verbContext;
        verbContext.verbTense = _semanticVerbTenseToLinguisticVerbTense(
            pCompExp.tense, pContext.contextType, pContext.rootStatement, pRequests);
        ...
        grdSynth.writeVerbalSemWord(
            outSentence.aux.out, outSentence.verb, outSentence.negation2.out, beWord, pConf.lingDb, verbContext);
    }
    ...
    if (pCompExp.whatIsComparedExp) {
        ...
        _writeSemExp(semExpSyntTypes,
                     outSentence.objectAfterVerb,
                     subRequests,
                     pLastSubject,
                     **pCompExp.whatIsComparedExp,
                     pConf,
                     objectContext);
    } else {
        if (_language == SemanticLanguageEnum::FRENCH) {
            subjectContext.wordContext.gender =
                synthGetter::getGenderFromSemExp(*pCompExp.leftOperandExp, pConf, pContext, grdSynth);
...
void LinguisticSynthesizerPrivate::_writeListExp(std::list<SemExpTypeEnumFormSynt>& pSemExpSyntTypes,
                                                 OutSemExp& pOutSemExp,
                                                 SynthesizerConfiguration& pConf,
    ...
    if (nbElts > 0) {
        ...
        for (const auto& currElt : pListExp.elts) {
            ...
            if (currId == nbElts && SemExpGetter::isACoreference(*currElt, CoreferenceDirectionEnum::AFTER)) {
                ...
                continue;
            } else if (currId == 1 && nbElts == 2
                       && SemExpGetter::isACoreference(*currElt, CoreferenceDirectionEnum::BEFORE)) {
                pSemExpSyntTypes.emplace_back(SEMEXPTYPEENUMFORSYNT_NOTHING);
                continue;
            }
            SemanticRequests requests;
    ...
    } else if (pListExp.listType != ListExpressionType::UNRELATED) {
        getSyntGrounding().writeListSeparators(pOutSemExp.out, pListExp.listType, true);
    }
}

void LinguisticSynthesizerPrivate::_startAQuestion(std::list<WordToSynthesize>& pOut,
...
SemExpTypeEnumFormSynt LinguisticSynthesizerPrivate::writeGrdExp(OutSemExp& pOutSemExp,
                                                                 SemanticRequests& pRequests,
                                                                 SemanticExpression const** pLastSubject,
    ...
    if (statementGrdPtr != nullptr) {
        ...
        _writeSentenceGrdExp(pOutSemExp.out, pRequests, pLastSubject, pGrdExp, statementGrd, pConf, pContext);
        return SEMEXPTYPEENUMFORSYNT_SENTENCE;
    }
    ...
    SynthesizerCurrentContext grdContext(pContext);
    grdSynth.modifyContextForAGrounding(
        grdContext.wordContext, outInfoGram, pConf, pGrdExp.grounding(), pContext.contextType, pContext.verbTense);
    _writeNominalGrdExp(pOutSemExp, pRequests, pLastSubject, pGrdExp, outInfoGram, pConf, grdContext);
...
bool LinguisticSynthesizerPrivate::_handleSpecificLanguageSynthesis(
    ...
    if (subLanguage != SemanticLanguageEnum::UNKNOWN) {
        ...
        subSynth._writeSemExp(pSemExpSyntTypes, pOutSemExp, pRequests, pLastSubject, pSemExpToExecute, pConf, pContext);
        return true;
    }
    return false;
}

void LinguisticSynthesizerPrivate::_writeSentenceGrdExp(std::list<WordToSynthesize>& pOut,
                                                        SemanticRequests& pRequests,
                                                        SemanticExpression const** pLastSubject,
                                                        const GroundedExpression& pGrdExp,
    ...
    if (_language == SemanticLanguageEnum::ENGLISH
        ...
        && (pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_CONDITION
            ...
            || (ConceptSet::haveAConceptThatBeginWithAnyOf(
                    pStatementGrd.concepts, {/*"appearance_",*/ "mentalState_", "perception_", "emotion_"})
        ...
        verbContext.verbTense = _semanticVerbTenseToLinguisticVerbTense(sentWorkStruct.outs.verbTense,
                                                                        pContext.contextType,
                                                                        pContext.rootStatement,
                                                                        pStatementGrd.requests,
                                                                        &inflectedVerb.infos);
    }
    ...
    if (pStatementGrd.word.lemma.empty() && pStatementGrd.concepts.count("verb_generality") > 0) {
        ...
        bool isPassiveFromSubject = sentWorkStruct.outs.verbTense != SemanticVerbTense::UNKNOWN
                                 && !pRequests.has(SemanticRequestType::SUBJECT) && sentWorkStruct.subjectPtr == nullptr
        ...
        if (pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERNOUN
            || pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERADJECTIVE) {
            if (sentWorkStruct.subjectPtr == nullptr && pRequests.has(SemanticRequestType::OBJECT))
    ...
    if (sentWorkStruct.outs.isPassive) {
        ...
        if (inflectedVerb.word.isReflexive())
            grdSynth.getIGramOfAWord(
                inflectedVerb, inflectedVerb.word.getRootFormFromReflexive(), pStatementGrd.concepts, pConf);
    }

    sentWorkStruct.objectIsAnNoElement =
        sentWorkStruct.objectPtr != nullptr
        && SemExpGetter::getNumberOfElements(**sentWorkStruct.objectPtr) == SemanticFloat(0);

    ObjectPosition objectPosition =
        _getObjectPosition(sentWorkStruct, pStatementGrd, pRequests, pConf, verbContext.verbTense);
    ...
    if (originRequests.has(SemanticRequestType::OBJECT)) {
        ...
        if (itChild != pGrdExp.children.end() && !SemExpGetter::isDefinite(*itChild->second))
    ...
    bool needToWriteTheVerb =
        !(_language == SemanticLanguageEnum::FRENCH && pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_GENERIC
          && originRequests.has(SemanticRequestType::OBJECT) && inflectedVerb.word.lemma == "être"
          && sentWorkStruct.objectPtr == nullptr && sentWorkStruct.subjectPtr != nullptr
          && !SemExpGetter::hasSpecifications(**sentWorkStruct.subjectPtr));

    if (!originRequests.empty()) {
        _startAQuestion(pOut,
                        sentWorkStruct.outs.subject,
                        originRequests,
                        sentWorkStruct.outs.equVerb,
                        sentWorkStruct.outs.verbTense,
                        pStatementGrd,
                        sentWorkStruct.subjectPtr,
                        sentWorkStruct.objectPtr,
                        sentWorkStruct.outs.isPassive,
                        objectPosition,
                        childToPutBeforeSubject,
                        pConf,
                        pContext.contextType,
                        verbContext,
                        needToWriteTheVerb);
    } else if (pStatementGrd.coreference && pGrdExp.children.empty()) {
        SemanticRequests requests;
        auto thirdSingRelPerson = RelativePerson::THIRD_SING;
        grdSynth.writeRelativePerson(pOut,
                                     thirdSingRelPerson,
                                     SemanticReferenceType::DEFINITE,
                                     true,
                                     SemanticEntityType::UNKNOWN,
                                     SemanticQuantity(),
                                     pContext,
                                     requests);
        return;
    ...
    if (!sentWorkStruct.outs.subject.empty() && sentWorkStruct.subjectPtr != nullptr
        && sentWorkStruct.objectPtr == nullptr) {
    ...
    if (sentWorkStruct.subjectPtr != nullptr) {
        subjectRelativePersonWithoutTransformation =
            _semExpToRelativePerson(**sentWorkStruct.subjectPtr, pConf, isANameAssignement);
        ...
        if (_language == SemanticLanguageEnum::ENGLISH && sentWorkStruct.objectPtr != nullptr) {
            subjectRelativePerson = _semExpToRelativePerson(**sentWorkStruct.objectPtr, pConf, isANameAssignement);
        } else {
        ...
        subjectRelativePerson = _semExpToRelativePerson(*pContext.parentSubject, pConf, isANameAssignement);
    }

    const bool subjectIsAParentCoreference =
        sentWorkStruct.subjectPtr != nullptr
        && SemExpGetter::isACoreference(**sentWorkStruct.subjectPtr, CoreferenceDirectionEnum::PARENT);
    const bool objectIsAParentCoreference =
        sentWorkStruct.objectPtr != nullptr
        && SemExpGetter::isACoreference(**sentWorkStruct.objectPtr, CoreferenceDirectionEnum::PARENT);
    if (subjectIsAParentCoreference
        || (sentWorkStruct.subjectPtr != nullptr && pRequests.has(SemanticRequestType::SUBJECT)
            && SemExpGetter::isWhoSemExp(**sentWorkStruct.subjectPtr)))
        sentWorkStruct.subjectPtr = nullptr;
    else if (objectIsAParentCoreference
             || (sentWorkStruct.objectPtr != nullptr && pRequests.has(SemanticRequestType::OBJECT)
    ...
    if (!pRequests.has(SemanticRequestType::ACTION)) {
        if (sentWorkStruct.subjectPtr != nullptr) {
            ...
            _writeSubjectContext(sentWorkStruct.outs, pLastSubject, subjectRef, subjectContext, pConf, pRequests);
    ...
    if (needToWriteTheVerb) {
        ...
        verbContext.isPassive =
            sentWorkStruct.outs.isPassive && !inflectedVerb.infos.hasContextualInfo(WordContextualInfos::PASSIVE);
        ...
        if (_language == SemanticLanguageEnum::FRENCH
            && (verbContext.verbTense == LinguisticVerbTense::SIMPLE_PAST_INDICATIVE || verbContext.isPassive)) {
            if (sentWorkStruct.subjectPtr != nullptr)
                verbContext.wordContext.gender = synthGetter::getGenderFromSemExp(
                    sentWorkStruct.subjectPtr->getSemExp(), pConf, verbContext, grdSynth);
            else
        ...
        grdSynth.statGroundingTranslation(sentWorkStruct.outs,
                                          pConf,
                                          pStatementGrd,
                                          inflectedVerb,
                                          sentWorkStruct.grdExp,
                                          verbContext,
                                          sentWorkStruct.subjectPtr);
    ...
    if (sentWorkStruct.objectPtr != nullptr) {
        ...
        if (!_writeReflexionIfNeeded(sentWorkStruct,
                                     sentWorkStruct.outs.objectAfterVerb,
                                     subjectRelativePersonWithoutTransformation,
                                     pStatementGrd,
                                     inflectedVerb,
                                     pGrdExp,
                                     ...
                                     pConf,
                                     verbContext,
                                     verbContext.rootSubject,
                                     verbContext.verbContextOpt)) {
    ...
    for (const auto& currChild : sentWorkStruct.grdExp.children) {
        if (currChild.first == GrammaticalType::SUBJECT || currChild.first == GrammaticalType::OBJECT)
            continue;
        ...
        if (!(_language == SemanticLanguageEnum::FRENCH && currChild.first == GrammaticalType::MANNER)
            ...
            std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
            _writeSemExp(semExpSyntTypes, outSE, pRequests, pLastSubject, **childToPutBeforeSubject, pConf, recContext);
            continue;
        }
        ...
        switch (currChild.first) {
            case GrammaticalType::STARTING_POINT: {
                ...
                _writeObjectAfterVerb(sentWorkStruct,
                                      outSE,
                                      currChild.first,
                                      pStatementGrd,
                                      inflectedVerb,
            ...
            case GrammaticalType::LENGTH: {
                OutSemExp& outSE = sentWorkStruct.outs.length;
                _writeObjectAfterVerb(sentWorkStruct,
                                      outSE,
                                      currChild.first,
                                      pStatementGrd,
            ...
            case GrammaticalType::LOCATION: {
                ...
                _writeLocation(
                    outSE, currSemExpChild, pConf, pRequests, verbContext, &sentWorkStruct, &verbContext, pLastSubject);
                break;
            }
            ...
            case GrammaticalType::SPECIFIER: {
                _writeEquality(
                    sentWorkStruct, currSemExpChild, pConf, pRequests, verbContext, pContext.contextType, pLastSubject);
                break;
            }
            case GrammaticalType::TIME: {
                ...
                if (timeGrdExpPtr != nullptr && timeGrdExpPtr->grounding().type != SemanticGroundingType::STATEMENT
                    && originRequests.has(SemanticRequestType::TIME) && childToPutBeforeSubject != nullptr) {
                ...
                std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
                _writeSemExp(
                    semExpSyntTypes, *outSE, subRequestType, pLastSubject, currSemExpChild, pConf, timeContext);
                break;
            }
            ...
            case GrammaticalType::UNKNOWN: break;
        }
    }
...
void LinguisticSynthesizerPrivate::_writeNominalGrdExp(OutSemExp& pOutSemExp,
                                                       SemanticRequests& pRequests,
                                                       SemanticExpression const** pLastSubject,
                                                       const GroundedExpression& pGrdExp,
    ...
    if (genGrdPtr != nullptr && genGrdPtr->coreference.has_value()) {
        ...
        if (ownerIt != pGrdExp.children.end()) {
            ...
            if (ownerGrdExp != nullptr) {
                RelativePerson ownerRelPerson =
                    _grdExpToRelativePerson(*ownerGrdExp, pConf, grdContext.isPartOfANameAssignement);
                _getPossessiveWord(outNominalGroup.noun.out,
                                   grdContext.wordContext,
                                   ownerRelPerson,
                                   ownerGrdExp->grounding(),
                                   pConf,
                                   grdContext);
                _getChunksMerger().formulateNominalGroup(pOutSemExp.out, outNominalGroup);
                return;
    ...
    grdSynth.writeGrounding(outNominalGroup.noun, pOutInfoGram, grdContext, pConf, grounding, pGrdExp, pRequests);
    ...
    grdSynth.writeGroundingIntroduction(
        outNominalGroup.determiner, outNominalGroup.noun, pOutInfoGram, grdContext, pConf, grounding, pGrdExp);
...
void LinguisticSynthesizerPrivate::_writeNominalChild(OutNominalGroup& outNominalGroup,
    ...
    switch (pChildGrammaticalType) {
        ...
        case GrammaticalType::LOCATION: {
            _writeLocation(
                outNominalGroup.location, currSemExpChild, pConf, pRequests, pContext, nullptr, nullptr, pLastSubject);
            break;
        }
        case GrammaticalType::OBJECT: {
            _writeNounSubordinates(outNominalGroup.subordinate,
                                   currSemExpChild,
                                   pChildGrammaticalType,
                                   pConf,
                                   pRequests,
                                   pGrdExpPtr,
                                   pLastSubject);
        ...
        case GrammaticalType::OWNER: {
            ...
            if (ownerGrdExpPtr != nullptr
                && ConceptSet::haveAConcept(ownerGrdExpPtr->grounding().concepts, "tolink_1p")) {
                _getPossessiveDeterminer(outNominalGroup.ownerBeforeMainWord.out,
                                         RelativePerson::FIRST_PLUR,
            ...
            if (ownerGrdExpPtr != nullptr) {
                ...
                RelativePerson ownerRelPerson =
                    _grdExpToRelativePerson(ownerGrdExp, pConf, pContext.isPartOfANameAssignement);
                ...
                switch (ownerRelPerson) {
                    ...
                    case RelativePerson::THIRD_PLUR: {
                        ...
                        if (ownerGenGrd != nullptr && ownerGenGrd->coreference && ownerGenGrd->word.lemma.empty()) {
                            _getPossessiveDeterminer(outNominalGroup.ownerBeforeMainWord.out,
                                                     ownerRelPerson,
                                                     pContext.wordContext.gender,
                                                     pContext.wordContext.number);
                            pContext.ownerWrittenBefore = true;
                                   ...
                                   && SemExpComparator::semExpsAreEqual(
                                       *pContext.rootSubject, ownerGrdExp, pConf.memBlock, pConf.lingDb)) {
                            ...
                            _getPossessiveDeterminer(
                                outNominalGroup.ownerBeforeMainWord.out, ownerRelPerson, gender, number);
                            pContext.ownerWrittenBefore = true;
                        }
                        break;
                    }
                ...
                if (_language == SemanticLanguageEnum::ENGLISH && !pContext.ownerWrittenBefore
                    ...
                    auto subRequests = pRequests;
                    linguistics::InflectedWord outInfoGram;
                    grdSynth.modifyContextForAGrounding(ownerContext.wordContext,
                                                        outInfoGram,
                                                        pConf,
                                                        ownerGrdExp.grounding(),
                                                        ownerContext.contextType,
                                                        ownerContext.verbTense);
                    _writeNominalGrdExp(outNominalGroup.ownerBeforeMainWord,
                                        subRequests,
                                        pLastSubject,
                                        ownerGrdExp,
                                        outInfoGram,
                                        pConf,
                                        ownerContext);
                    const std::string possessiveStr =
                        synthGetter::doesOutFinishedWithAS(outNominalGroup.ownerBeforeMainWord.out) ? "'" : "'s";
                    outNominalGroup.ownerBeforeMainWord.out.emplace_back(
                        SemanticWord(_language, possessiveStr, PartOfSpeech::ADVERB),
                        InflectionToSynthesize(possessiveStr, false, true, alwaysTrue));
                    pContext.ownerWrittenBefore = true;
                }

                if (outNominalGroup.ownerBeforeMainWord.empty() && ownerRelPerson != RelativePerson::FIRST_SING
                    ...
                    if (pGrdExpPtr != nullptr) {
                        ...
                        _getBeginOfSpecification(
                            outNominalGroup.ownerAfterMainWord.out, ownerGrdExp, grounding, ownerContext.wordContext);
                    }
                    _writeNominalGrdExp(outNominalGroup.ownerAfterMainWord,
                                        subRequests,
                                        pLastSubject,
                                        ownerGrdExp,
                                        outInfoGram,
        ...
        case GrammaticalType::SUB_CONCEPT: {
            if (pInflWordPtr != nullptr) {
                ...
                auto& subConcepts = _subConceptBeforeOrAfter(pInflWordPtr->infos)
                                      ? outNominalGroup.subConceptsBeforeNoun
                                      : outNominalGroup.subConceptsAfterNoun;
                std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
                _writeSemExp(semExpSyntTypes,
                             subConcepts,
                             subRequests,
        ...
        case GrammaticalType::TIME: {
            ...
            std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
            _writeSemExp(
                semExpSyntTypes, outNominalGroup.time, subRequests, pLastSubject, currSemExpChild, pConf, timeContext);
            break;
        }
        case GrammaticalType::OTHER_THAN: {
            ...
            if (!lingMeaning.isEmpty()) {
                ...
                grdSynth.writeSemWord(otherChildOutSemExp.out,
                                      otherChildOutSemExp,
                                      otherChildOutSemExp.out,
                                      inflWord.word,
                                      pConf.lingDb,
                                      pContext);
            }
...
SemExpTypeEnumFormSynt LinguisticSynthesizerPrivate::_writeCondExp(OutSemExp& pOutSemExp,
                                                                   SemanticRequests& pRequests,
                                                                   SemanticExpression const** pLastSubject,
    ...
    auto writeIntroOfConditionWord = [this, &pCondExp](std::list<WordToSynthesize>& pOut) {
    ...
    if (pCondExp.elseExp || pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_OBJECTAFTERVERB) {
        writeIntroOfConditionWord(pOutSemExp.out);
        {
            ...
            std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
            _writeSemExp(
                semExpSyntTypes, pOutSemExp, subCondRequests, pLastSubject, *pCondExp.conditionExp, pConf, condContext);
        }
        ...
        if (pCondExp.elseExp) {
            _getElseWord(pOutSemExp.out);
            {
                ...
                std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
                _writeSemExp(
                    semExpSyntTypes, pOutSemExp, pRequests, pLastSubject, **pCondExp.elseExp, pConf, elseContext);
            }
        }
    } else {
        ...
        writeIntroOfConditionWord(pOutSemExp.out);
        {
            ...
            std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
            _writeSemExp(
                semExpSyntTypes, pOutSemExp, subThenRequests, pLastSubject, *pCondExp.conditionExp, pConf, condContext);
        }
    }
    return SEMEXPTYPEENUMFORSYNT_SENTENCE;
}

void LinguisticSynthesizerPrivate::_writeObjects(OutSemExp& pOutSemExp,
    ...
    if (objectListExpPtr != nullptr) {
        ...
        if (nbElts > 0) {
            ...
            for (const auto& currElt : objectListExpPtr->elts) {
                ...
                if (objectListExpPtr->listType != ListExpressionType::UNRELATED && currId > 1)
                    getSyntGrounding().writeListSeparators(
                        pOutSemExp.out, objectListExpPtr->listType, currId == nbElts);
                if (currId == nbElts && SemExpGetter::isACoreference(*currElt, CoreferenceDirectionEnum::AFTER))
                    continue;
                else if (currId == 1 && nbElts == 2
                         && SemExpGetter::isACoreference(*currElt, CoreferenceDirectionEnum::BEFORE))
                    continue;
                _writeObjects(pOutSemExp,
                              pCurrSentence,
                              pSentWorkStruct,
                              pVerbContext,
                              pStatementGrd,
        ...
        } else if (objectListExpPtr->listType != ListExpressionType::UNRELATED) {
            getSyntGrounding().writeListSeparators(pOutSemExp.out, objectListExpPtr->listType, true);
        }
        return;
    }
    ...
    if (pObjectPosition == ObjectPosition::BEFOREVERB) {
        if (_language == SemanticLanguageEnum::FRENCH && pSentWorkStruct.objectIsAnNoElement
            ...
            objBefVerbContext.wordContext.relativePerson =
                _semExpToRelativePerson(pObjectSemExp, pConf, pIsANameAssignement);
            objBefVerbContext.requests = pVerbContext.requests;

            std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
            _writeSemExp(
                semExpSyntTypes, pOutSemExp, subRequests, pLastSubject, pObjectSemExp, pConf, objBefVerbContext);
        }
        ...
        objBefVerbContext.requests = pVerbContext.requests;
        std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
        _writeSemExp(semExpSyntTypes, pOutSemExp, subRequests, pLastSubject, pObjectSemExp, pConf, objBefVerbContext);
    } else {
        ...
        objAfterVerbContext.rootSubject =
            pSentWorkStruct.subjectPtr != nullptr ? &**pSentWorkStruct.subjectPtr : pVerbContext.rootSubject;
        ...
        if (objectGrdExpPtr != nullptr)
            _beginOfSubordonateIfNeeded(objAfterVerbContext.isASubordinateWithoutPreposition,
                                        ...
                                        *objectGrdExpPtr,
                                        pVerbContext.verbContextOpt,
                                        pConf);
        std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
        _writeSemExp(semExpSyntTypes, pOutSemExp, subRequests, pLastSubject, pObjectSemExp, pConf, objAfterVerbContext);
    }
}

void LinguisticSynthesizerPrivate::_writeSubjectContext(OutSentence& pOutSentence,
                                                        SemanticExpression const** pLastSubject,
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (genGrdPtr != nullptr) {
            ...
            if (genGrd.word.isEmpty() && !ConceptSet::haveAnotherConceptThan(genGrd.concepts, "agent_*")
    ...
    if (*pLastSubject != nullptr) {
        ...
        auto imbrication = SemExpComparator::getSemExpsImbrications(
            **pLastSubject, *pSemExp, pConf.memBlock, pConf.lingDb, &compExceptions);
        useLastSubject = imbrication == ImbricationType::EQUALS || imbrication == ImbricationType::MORE_DETAILED;
    }
    if (useLastSubject) {
        const auto& grdSynth = getSyntGrounding();
        pSubjectContext.wordContext.gender =
            synthGetter::getGenderFromSemExp(*pSemExp, pConf, pSubjectContext, grdSynth);
        ...
        pOutSentence.subject.partOfSpeech = grdSynth.writeRelativePerson(pOutSentence.subject.out,
                                                                         pSubjectContext.wordContext.relativePerson,
                                                                         SemanticReferenceType::DEFINITE,
                                                                         false,
                                                                         entityType,
                                                                         SemanticQuantity(),
        ...
        auto subRequests = pRequests;
        _writeSemExp(
            semExpSyntTypes, pOutSentence.subject, subRequests, pLastSubject, *pSemExp, pConf, pSubjectContext);
        *pLastSubject = &*pSemExp;

        if (_language == SemanticLanguageEnum::FRENCH && grdExpPtr != nullptr) {
            const GroundedExpression& grdExp = *grdExpPtr;
            const SemanticGrounding& grounding = grdExp.grounding();
...
RelativePerson LinguisticSynthesizerPrivate::_semExpToRelativePerson(const SemanticExpression& pSemExp,
                                                                     const SynthesizerConfiguration& pConf,
...
void LinguisticSynthesizerPrivate::_writeReceiver(OutSemExp& pOutSemExpBeforeVerb,
    ...
    auto subRequests = pRequests;
    OutSemExp outSemExp;
    _writeSemExp(semExpSyntTypes, outSemExp, subRequests, pLastSubject, pSemExp, pConf, recContext);
...
void _extractCommonBegin(std::list<WordToSynthesize>& pCommonBegin, std::list<OutSemExp>& pSubOutSemExps) {
    ...
    for (auto it = pSubOutSemExps.begin(); it != pSubOutSemExps.end(); ++it) {
        ...
        if (firstOutsPtr == nullptr) {
            ...
            for (auto itCommon = firstOutsPtr->begin(), itCurr = currOut.begin();
                 itCommon != firstOutsPtr->end() && &*itCommon != endOfCommonBeginPtr;
                 ++itCommon, ++itCurr) {
                if (itCurr == currOut.end() || itCurr->word.partOfSpeech == PartOfSpeech::DETERMINER
                    || *itCommon != *itCurr) {
    ...
    if (firstOutsPtr != nullptr) {
        ...
        for (auto itCommon = firstOutsPtr->begin();
             itCommon != firstOutsPtr->end() && &*itCommon != endOfCommonBeginPtr;) {
...
void LinguisticSynthesizerPrivate::_writeLocation(OutSemExp& pOutSemExp,
                                                  const SemanticExpression& pSemExp,
                                                  SynthesizerConfiguration& pConf,
                                                  const SemanticRequests& pRequests,
                                                  const SynthesizerCurrentContext& pContext,
    ...
    if (locationListExpPtr != nullptr) {
        ...
        for (auto& currElt : locationListExpPtr->elts) {
            ...
            _writeLocation(
                subOutSemExp, *currElt, pConf, pRequests, pContext, pSentWorkStructPtr, pVerbContextPtr, pLastSubject);
        }
    ...
    if (pVerbContextPtr != nullptr) {
        ...
        std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
        auto subRequests = pRequests;
        _writeSemExp(semExpSyntTypes, pOutSemExp, subRequests, pLastSubject, pSemExp, pConf, recContext);
        return;
    }
...
bool LinguisticSynthesizerPrivate::_writeReflexionIfNeeded(
    ...
    if (pSentWorkStruct.subjectPtr != nullptr) {
        if (SemExpComparator::semExpsAreEqualFromMemBlock(
                **pSentWorkStruct.subjectPtr, pSemExp, pConf.memBlock, pConf.lingDb, &compExceptions)) {
            if (_language == SemanticLanguageEnum::ENGLISH)
                grdSynth.writePreposition(
                    pOutSemExp.out, nullptr, pInflectedVerb, pContext, pConf, pStatementGrd, pHoldingGrdExp);
            grdSynth.writeReflexiveObject(pSentWorkStruct.outs,
                                          pOutSemExp.out,
                                          pSubjectRelativePerson,
                                          pSentWorkStruct.outs.subject.gender,
                                          pVerbContextOpt);
            return true;
        }
    } else if (pRootSubject != nullptr) {
        if (SemExpComparator::semExpsAreEqualFromMemBlock(
                *pRootSubject, pSemExp, pConf.memBlock, pConf.lingDb, &compExceptions)) {
            ...
            if (_language == SemanticLanguageEnum::ENGLISH)
                grdSynth.writePreposition(
                    pOutSemExp.out, nullptr, pInflectedVerb, pContext, pConf, pStatementGrd, pHoldingGrdExp);
            grdSynth.writeReflexiveObject(
                pSentWorkStruct.outs, pOutSemExp.out, pSubjectRelativePerson, gender, pVerbContextOpt);
            return true;
        }
    }
    return false;
...
void LinguisticSynthesizerPrivate::_writeObjectAfterVerb(SyntSentWorkStruct& pSentWorkStruct,
                                                         OutSemExp& pOutSemExp,
    ...
    if (_writeReflexionIfNeeded(pSentWorkStruct,
                                pOutSemExp,
                                pSentWorkStruct.outs.subject.relativePerson,
                                pStatementGrd,
                                pInflectedVerb,
                                pHoldingGrdExp,
                                pSemExp,
                                pConf,
                                objectContext,
                                pVerbContext.rootSubject,
                                objectContext.verbContextOpt))
        return;

    SemanticRequests subRequests = pRequests;
    std::list<SemExpTypeEnumFormSynt> semExpSyntTypes;
    _writeSemExp(semExpSyntTypes, pOutSemExp, subRequests, pLastSubject, pSemExp, pConf, objectContext);
}

void LinguisticSynthesizerPrivate::_writeEquality(SyntSentWorkStruct& pSentWorkStruct,
                                                  const SemanticExpression& pSemExp,
                                                  SynthesizerConfiguration& pConf,
                                                  const SemanticRequests& pRequests,
                                                  const SynthesizerCurrentContext& pContext,
    ...
    if (specGrdExpPtr != nullptr) {
        ...
        grdSynth.modifyContextForAGrounding(grdContext.wordContext,
                                            outInfoGram,
                                            pConf,
                                            specGrdExp.grounding(),
                                            grdContext.contextType,
                                            grdContext.verbTense);

        OutSemExp& outSemExp = [&]() -> OutSemExp& {
            bool beforeOrAfterTheVerb =
                outInfoGram.infos.hasContextualInfo(WordContextualInfos::CANBEBEFOREVERB)
                && (_language != SemanticLanguageEnum::FRENCH
                    || pParentContextType
                           == SynthesizerCurrentContextType::SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERNOUN);
        ...
        auto subRequests = pRequests;
        _writeNominalGrdExp(outSemExp, subRequests, pLastSubject, specGrdExp, outInfoGram, pConf, grdContext);
    }
}

void LinguisticSynthesizerPrivate::_writeNounSubordinates(OutSemExp& pOutSemExp,
                                                          const SemanticExpression& pSemExp,
    ...
    if (locationListExpPtr != nullptr) {
        ...
        for (const auto& currElt : locationListExpPtr->elts) {
            subOutSemExps.emplace_back();
            auto& subOutSemExp = subOutSemExps.back();
            _writeNounSubordinates(
                subOutSemExp, *currElt, pSemExpFatherGramType, pConf, pRequests, pGrdExpPtr, pLastSubject);
        }
    ...
    if (_language == SemanticLanguageEnum::FRENCH) {
        ...
        if (grdExpEltPtr != nullptr) {
            ...
            if (statGrdEltPtr != nullptr && statGrdEltPtr->verbTense == SemanticVerbTense::UNKNOWN) {
...
void LinguisticSynthesizerPrivate::_writeSomeModifiersOfAWord(OutSemExp& pOutBeforeDeterminer,
    ...
    if (modifierGrdExpPtr != nullptr) {
        ...
        grdSynth.modifyContextForAGrounding(
            wordContext, outInfoGram, pConf, modifierGrd, pContext.contextType, pContext.verbTense);
        ...
        auto subRequests = pRequests;
        writeGrdExp(outSemExp, subRequests, pLastSubject, modifierGrdExp, pConf, grdContext);
    ...
    if (modifierListExp != nullptr) {
        ...
        if (modifierListExp->listType == ListExpressionType::UNRELATED) {
            ...
            _fillOutBlocsList(pOutBeforeDeterminer.out, subOutEltsBeforeDeterminer, modifierListExp->listType);
            _fillOutBlocsList(pOutBeforeMainWord.out, subOutEltsBeforeMainWord, modifierListExp->listType);
            _fillOutBlocsList(pOutAfterMainWord.out, subOutEltsAfterMainWord, modifierListExp->listType);
            _fillOutBlocsList(pOutSubordinate.out, subOutEltsSubordinate, modifierListExp->listType);
        }
        return;
    }
}

RelativePerson LinguisticSynthesizerPrivate::_grdExpToRelativePerson(const GroundedExpression& pGrdExp,
                                                                     const SynthesizerConfiguration& pConf,
                                                                     bool pIsANameAssignement) const {
    ...
    switch (pGrdExp.grounding().type) {
        ...
        case SemanticGroundingType::GENERIC: {
            ...
            return (genGrd.entityType != SemanticEntityType::NUMBER && genGrd.quantity.isPlural())
                     ? RelativePerson::THIRD_PLUR
                     : RelativePerson::THIRD_SING;
        }
        default: {
            return RelativePerson::THIRD_SING;
        }
...
void LinguisticSynthesizerPrivate::_fillOutBlocsList(std::list<WordToSynthesize>& pOut,
                                                     std::list<OutSemExp>& pSubOutElts,
                                                     ListExpressionType pListType) const {
    std::size_t nbElts = pSubOutElts.size();
    std::size_t currId = 0;
    for (auto itOutElt = pSubOutElts.begin(); itOutElt != pSubOutElts.end(); ++itOutElt) {
        ++currId;
