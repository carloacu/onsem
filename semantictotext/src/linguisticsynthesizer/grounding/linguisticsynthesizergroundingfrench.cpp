...
LinguisticsynthesizergroundingFrench::LinguisticsynthesizergroundingFrench(
    const LinguisticSynthesizerPrivate& pLingSynth)
    : Linguisticsynthesizergrounding(SemanticLanguageEnum::FRENCH, pLingSynth)
    , _chunksMerger() {}

bool LinguisticsynthesizergroundingFrench::_dateTranslation(std::list<WordToSynthesize>& pOut,
...
std::string LinguisticsynthesizergroundingFrench::_usRelativePersonToStr(SynthesizerCurrentContextType,
                                                                         LinguisticVerbTense) const {
    return "nous";
}

bool LinguisticsynthesizergroundingFrench::_groundingAttributesToWord(
...
PartOfSpeech LinguisticsynthesizergroundingFrench::writeRelativePerson(std::list<WordToSynthesize>& pOut,
    ...
    if (pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_OBJECTBEFOREVERB
        ...
        return _writeRelativePersonObject(pOut,
                                          pRelativePerson,
    ...
    switch (pRelativePerson) {
        ...
        case RelativePerson::THIRD_PLUR: {
            if (pQuantity.type == SemanticQuantityType::MAXNUMBER) {
                ...
                _strToOut(
                    pOut, PartOfSpeech::PRONOUN, pAgentType == SemanticEntityType::HUMAN ? "tout le monde" : "tout");
        ...
        default: {
            if (pQuantity.type == SemanticQuantityType::EVERYTHING) {
                if (pHasToBeCompletedFromContext)
                    ...
                    _strToOut(pOut,
                              PartOfSpeech::PRONOUN,
                              pAgentType == SemanticEntityType::HUMAN ? "tout le monde" : "tout");
                return PartOfSpeech::PRONOUN;
                ...
                if (pAgentType == SemanticEntityType::HUMAN) {
                    if (pQuantity.type == SemanticQuantityType::NUMBER && pQuantity.nb == 0)
                ...
                _strToOut(pOut, PartOfSpeech::PRONOUN, pAgentType == SemanticEntityType::HUMAN ? "lui" : "cela");
                return PartOfSpeech::PRONOUN;
            } else {
                if (pAgentType == SemanticEntityType::HUMAN || pAgentType == SemanticEntityType::AGENTORTHING) {
                    switch (pContext.wordContext.gender) {
                        case SemanticGenderType::MASCULINE: _strToOut(pOut, PartOfSpeech::PRONOUN_SUBJECT, "il"); break;
                        case SemanticGenderType::FEMININE:
                            _strToOut(pOut, PartOfSpeech::PRONOUN_SUBJECT, "elle");
                            break;
                        default: _strWithApostropheToOut(pOut, PartOfSpeech::PRONOUN_SUBJECT, "c'", "ce"); break;
                    }
                    return PartOfSpeech::PRONOUN_SUBJECT;
                }
...
void LinguisticsynthesizergroundingFrench::rankConceptToInlfWord(linguistics::InflectedWord& pInflWord,
...
PartOfSpeech LinguisticsynthesizergroundingFrench::_writeRelativePersonObject(
    ...
    switch (pRelativePerson) {
        ...
        case RelativePerson::THIRD_SING:
        default: {
            if (pReferenceType == SemanticReferenceType::INDEFINITE && pHasToBeCompletedFromContext) {
...
void LinguisticsynthesizergroundingFrench::writeReflexiveObject(
    ...
    if (pVerbContextOpt) {
        if (linguistics::canBeAFrenchReflexiveVerb(pVerbContextOpt->verbInflWord.infos)) {
            ...
            _writeRelativePersonObject(pOutSentence.objectBeforeVerb.out,
...
bool LinguisticsynthesizergroundingFrench::_writeVerbGoal(OutSentence& pOutSentence,
    ...
    switch (pStatementGrd.verbGoal) {
        ...
        case VerbGoalEnum::MANDATORY: {
            auto canGenGrd = std::make_unique<GroundedExpression>([&pStatementGrd] {
                const std::string newStatementConcept =
                    pStatementGrd.verbGoal == VerbGoalEnum::ABILITY ? "verb_can" : "verb_haveto";
            ...
            statGroundingTranslation(subOutSentence, pConf, statGrd, verbInfoGram, *canGenGrd, subContext, nullptr);
        ...
        case VerbGoalEnum::NOTIFICATION: break;
    };
    return false;
}

void LinguisticsynthesizergroundingFrench::_writeDurationAgo(std::list<WordToSynthesize>& pOut) const {
...
bool LinguisticsynthesizergroundingFrench::_dayHourTranslation(std::list<WordToSynthesize>& pOut,
    ...
    if (durationPrint.hour) {
        ...
        if (!meaning.isEmpty()) {
            ...
            SemanticNumberType number =
                *durationPrint.hour > 1 ? SemanticNumberType::PLURAL : SemanticNumberType::SINGULAR;
...
void LinguisticsynthesizergroundingFrench::writeReLocationType(std::list<WordToSynthesize>& pOut,
    ...
    switch (pLocationType) {
        case SemanticRelativeLocationType::L_ABOVE:
            ...
            return;
        case SemanticRelativeLocationType::L_BEHIND: _strToOut(pOut, perp, "derrère"); return;
        case SemanticRelativeLocationType::L_BELOW:
            _strWithApostropheToOut(pOut, perp, "en dessous d'", "en dessous de");
            return;
        case SemanticRelativeLocationType::L_CLOSE: _strWithApostropheToOut(pOut, perp, "près d'", "près de"); return;
        case SemanticRelativeLocationType::L_EAST: _strToOut(pOut, perp, "est"); return;
        case SemanticRelativeLocationType::L_INFRONTOF: _strToOut(pOut, perp, "devant"); return;
        case SemanticRelativeLocationType::L_INSIDE: _strToOut(pOut, perp, "dans"); return;
        case SemanticRelativeLocationType::L_FAR: _strWithApostropheToOut(pOut, perp, "loin d'", "loin de"); return;
        case SemanticRelativeLocationType::L_HIGH: _strToOut(pOut, perp, "haut"); return;
        case SemanticRelativeLocationType::L_LEFT: _strToOut(pOut, perp, "gauche"); return;
        case SemanticRelativeLocationType::L_LOW: _strToOut(pOut, perp, "bas"); return;
        case SemanticRelativeLocationType::L_NORTH: _strToOut(pOut, perp, "nord"); return;
        case SemanticRelativeLocationType::L_ON: _strToOut(pOut, perp, "sur"); return;
        case SemanticRelativeLocationType::L_OUTSIDE: _strToOut(pOut, perp, "extérieur"); return;
        case SemanticRelativeLocationType::L_RIGHT: _strToOut(pOut, perp, "droit"); return;
        case SemanticRelativeLocationType::L_SOUTH: _strToOut(pOut, perp, "sud"); return;
        case SemanticRelativeLocationType::L_UNDER: _strToOut(pOut, perp, "sous"); return;
        case SemanticRelativeLocationType::L_WEST: _strToOut(pOut, perp, "ouest"); return;
    }
    assert(false);
}

void LinguisticsynthesizergroundingFrench::_writeReTimeType(std::list<WordToSynthesize>& pOut,
                                                            SemanticRelativeTimeType pTimeType,
                                                            LinguisticVerbTense pVerbTense) const {
    static const PartOfSpeech perp = PartOfSpeech::PREPOSITION;
    switch (pTimeType) {
        case SemanticRelativeTimeType::AFTER: _strToOut(pOut, perp, "après"); return;
        case SemanticRelativeTimeType::BEFORE: _strToOut(pOut, perp, "avant"); return;
        case SemanticRelativeTimeType::DELAYEDSTART: _strToOut(pOut, perp, "dans"); return;
        case SemanticRelativeTimeType::DURING: _strToOut(pOut, perp, "pendant"); return;
        case SemanticRelativeTimeType::JUSTAFTER: _strToOut(pOut, perp, "juste après"); return;
        case SemanticRelativeTimeType::JUSTBEFORE: _strToOut(pOut, perp, "juste avant"); return;
...
void LinguisticsynthesizergroundingFrench::_writeReDurationType(std::list<WordToSynthesize>& pOut,
...
void LinguisticsynthesizergroundingFrench::_getSeWord(std::list<WordToSynthesize>& pOut,
                                                      RelativePerson pRelativePerson) const {
    static const PartOfSpeech pronComp = PartOfSpeech::PRONOUN_COMPLEMENT;
    switch (pRelativePerson) {
        case RelativePerson::FIRST_SING: _strWithApostropheToOut(pOut, pronComp, "m'", "me"); break;
        case RelativePerson::SECOND_SING: _strWithApostropheToOut(pOut, pronComp, "t'", "te"); break;
        case RelativePerson::FIRST_PLUR: _strToOut(pOut, pronComp, "nous"); break;
        case RelativePerson::SECOND_PLUR: _strToOut(pOut, pronComp, "vous"); break;
        case RelativePerson::THIRD_SING:
        case RelativePerson::THIRD_PLUR:
        default: _strWithApostropheToOut(pOut, pronComp, "s'", "se"); break;
    }
}
...
void LinguisticsynthesizergroundingFrench::_getAllWord(std::list<WordToSynthesize>& pOut,
                                                       const SynthesizerCurrentContext& pContext) const {
    _strToOut(pOut,
...
void LinguisticsynthesizergroundingFrench::_getEveryWord(std::list<WordToSynthesize>& pOut,
                                                         const SynthesizerCurrentContext& pContext) const {
    _getAllWord(pOut, pContext);
}

void LinguisticsynthesizergroundingFrench::_getDeterminerThatReferToRecentContext(std::list<WordToSynthesize>& pOut,
...
void LinguisticsynthesizergroundingFrench::_getDeterminer(std::list<WordToSynthesize>& pOut,
    ...
    if (pGrounding.coreference) {
        ...
        if (pContext.wordContext.number == SemanticNumberType::PLURAL
            ...
            _strToOut(pOut, det, pContext.wordContext.gender == SemanticGenderType::FEMININE ? "une" : "un");
        ...
        if ((pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_OBJECTAFTERVERB
             || ((pContext.requests.has(SemanticRequestType::QUANTITY)
                  || pContext.requests.has(SemanticRequestType::DISTANCE)
            ...
            && pGrounding.quantity.type != SemanticQuantityType::NUMBER && !SemExpGetter::hasAChildModifier(pGrdExp)
        ...
        if (pGrounding.entityType != SemanticEntityType::MODIFIER
            ...
            && pContext.contextType != SYNTHESIZERCURRENTCONTEXTTYPE_GENERIC && pContext.verbContextOpt
            && pContext.verbContextOpt->verbInflWord.infos.hasContextualInfo(
                WordContextualInfos::FR_VERBFOLLOWEDBYWORDDE)) {
            _strWithApostropheToOut(pOut, det, "d'", "de");
        }
        if (pContext.wordContext.number == SemanticNumberType::PLURAL) {
...
void LinguisticsynthesizergroundingFrench::_writeReflexivePronoun(std::list<WordToSynthesize>& pOut,
                                                                  RelativePerson pRelativePerson,
                                                                  SemanticGenderType pGender) const {
    switch (pRelativePerson) {
        case RelativePerson::FIRST_SING: _strToOut(pOut, PartOfSpeech::PRONOUN, "moi-même"); break;
        case RelativePerson::SECOND_SING: _strToOut(pOut, PartOfSpeech::PRONOUN, "toi-même"); break;
        case RelativePerson::THIRD_SING:
            _strToOut(pOut, PartOfSpeech::PRONOUN, pGender == SemanticGenderType::FEMININE ? "elle-même" : "lui-même");
            break;
        case RelativePerson::FIRST_PLUR: _strToOut(pOut, PartOfSpeech::PRONOUN, "nous-mêmes"); break;
        case RelativePerson::SECOND_PLUR: _strToOut(pOut, PartOfSpeech::PRONOUN, "vous-mêmes"); break;
        case RelativePerson::THIRD_PLUR:
            _strToOut(
                pOut, PartOfSpeech::PRONOUN, pGender == SemanticGenderType::FEMININE ? "elles-mêmes" : "eux-mêmes");
            break;
...
bool LinguisticsynthesizergroundingFrench::_doWeHaveToPutSubMeaningBeforeOrAfterTheWord(
    ...
    switch (pDirection) {
        case LinkedMeaningDirection::FORWARD: return false;
        case LinkedMeaningDirection::BACKWARD: return true;
