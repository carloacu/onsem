...
LinguisticsynthesizergroundingEnglish::LinguisticsynthesizergroundingEnglish(
    const LinguisticSynthesizerPrivate& pLingSynth)
    : Linguisticsynthesizergrounding(SemanticLanguageEnum::ENGLISH, pLingSynth)
    , _chunksMerger() {}

bool LinguisticsynthesizergroundingEnglish::_dateTranslation(std::list<WordToSynthesize>& pOut,
    ...
    if (pDate.month) {
        if (pDate.day) {
            ...
            if (!meaning.isEmpty()) {
                ...
                _strToOutCptsMove(pOut,
                                  PartOfSpeech::NOUN,
                                  pDicoSynth.getLemma(meaning, false),
                                  std::move(monthConcept),
                                  WordToSynthesizeTag::DATE);
            }
            {
                std::stringstream ss;
                ss << *pDate.day;
...
PartOfSpeech LinguisticsynthesizergroundingEnglish::writeRelativePerson(std::list<WordToSynthesize>& pOut,
    ...
    return _writeRelativePerson(pOut,
                                pRelativePerson,
...
std::string LinguisticsynthesizergroundingEnglish::_usRelativePersonToStr(SynthesizerCurrentContextType pContextType,
                                                                          LinguisticVerbTense pVerbTense) const {
    return pContextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBJECT && pVerbTense != LinguisticVerbTense::INFINITIVE
             ? "we"
             : "us";
}

void LinguisticsynthesizergroundingEnglish::rankConceptToInlfWord(linguistics::InflectedWord& pInflWord,
...
bool LinguisticsynthesizergroundingEnglish::_groundingAttributesToWord(
...
PartOfSpeech LinguisticsynthesizergroundingEnglish::_writeRelativePerson(std::list<WordToSynthesize>& pOut,
                                                                         RelativePerson pRelativePerson,
        ...
        switch (pRelativePerson) {
            ...
            default: {
                ...
                if (pQuantity.type == SemanticQuantityType::EVERYTHING) {
                    return pAgentType == SemanticEntityType::HUMAN ? "everybody" : "everything";
                }
                if (pQuantity.type == SemanticQuantityType::ANYTHING) {
                    return pAgentType == SemanticEntityType::HUMAN ? "anybody" : "anything";
                }
                if (pQuantity.type == SemanticQuantityType::MAXNUMBER) {
                    return "all";
                }
                if (pReferenceType == SemanticReferenceType::INDEFINITE) {
                    return pAgentType == SemanticEntityType::HUMAN ? "somebody" : "something";
                } else if (pContextType != SYNTHESIZERCURRENTCONTEXTTYPE_SUBJECT) {
                    if (pAgentType == SemanticEntityType::HUMAN) {
                        switch (pGender) {
                            case SemanticGenderType::FEMININE: return "her";
                            case SemanticGenderType::MASCULINE: return "him";
                            case SemanticGenderType::NEUTRAL:
                            case SemanticGenderType::UNKNOWN: return "it";
                        }
                    }
                    return "that";
                } else {
...
bool LinguisticsynthesizergroundingEnglish::_writeVerbGoal(OutSentence& pOutSentence,
    ...
    switch (pStatementGrd.verbGoal) {
        case VerbGoalEnum::ABILITY: {
            if (pVerbContext.verbTense == LinguisticVerbTense::SIMPLE_PAST_INDICATIVE)
                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "could" : "could not");
            else
                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "can" : "can't");
            return true;
        }
        case VerbGoalEnum::ADVICE: {
            _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "should" : "shouldn't");
            return true;
        }
        case VerbGoalEnum::CONDITIONAL: {
            _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "would" : "won't");
            return true;
        }
        case VerbGoalEnum::MANDATORY: {
            if (pStatementGrd.verbGoal == VerbGoalEnum::MANDATORY && SemExpGetter::hasGenericConcept(pSubjectPtr)
                ...
                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "have to" : "have not to");
            else
                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "must" : "must not");
            return true;
        }
        case VerbGoalEnum::POSSIBILITY: {
            _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "might" : "might not");
            return true;
        }
        case VerbGoalEnum::NOTIFICATION: break;
    }

    if (pSubjectPtr != nullptr && pOutSentence.verbGoal.out.empty() && !pOutSentence.requests.empty()
                    ...
                    if (isAPastVerbTense(pVerbContext.verbTense)) {
                        if (pVerbContext.verbTense == LinguisticVerbTense::PRETERIT_CONTINUOUS) {
                            if (pVerbContext.wordContext.relativePerson == RelativePerson::FIRST_SING
                                ...
                                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "was" : "was not");
                            else
                                _strToOut(pOutSentence.aux.out, auxPOF, pVerbContext.isPositive ? "were" : "were not");
                        } else {
                            pOutSentence.aux.out.emplace_back(
                                SemanticWord(_language, "do", auxPOF),
                                InflectionToSynthesize(
                                    pVerbContext.isPositive ? "did" : "didn't", true, true, alwaysTrue));
                        }
                    } else {
...
void LinguisticsynthesizergroundingEnglish::_writeDurationAgo(std::list<WordToSynthesize>& pOut) const {
...
bool LinguisticsynthesizergroundingEnglish::_dayHourTranslation(std::list<WordToSynthesize>& pOut,
...
void LinguisticsynthesizergroundingEnglish::writeReLocationType(std::list<WordToSynthesize>& pOut,
                                                                SemanticRelativeLocationType pLocationType) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, [&]() {
        switch (pLocationType) {
            case SemanticRelativeLocationType::L_ABOVE: return "above";
            case SemanticRelativeLocationType::L_BEHIND: return "behind";
            case SemanticRelativeLocationType::L_BELOW: return "below";
            case SemanticRelativeLocationType::L_CLOSE: return "close to";
            case SemanticRelativeLocationType::L_EAST: return "east";
            case SemanticRelativeLocationType::L_INFRONTOF: return "in front of";
            case SemanticRelativeLocationType::L_INSIDE: return "in";
            case SemanticRelativeLocationType::L_FAR: return "far away from";
            case SemanticRelativeLocationType::L_HIGH: return "high";
            case SemanticRelativeLocationType::L_LEFT: return "left";
            case SemanticRelativeLocationType::L_LOW: return "low";
            case SemanticRelativeLocationType::L_NORTH: return "north";
            case SemanticRelativeLocationType::L_ON: return "on";
            case SemanticRelativeLocationType::L_OUTSIDE: return "outside";
            case SemanticRelativeLocationType::L_RIGHT: return "right";
            case SemanticRelativeLocationType::L_SOUTH: return "south";
            case SemanticRelativeLocationType::L_UNDER: return "under";
            case SemanticRelativeLocationType::L_WEST: return "west";
        }
        assert(false);
        return "";
    }());
...
void LinguisticsynthesizergroundingEnglish::_writeReTimeType(std::list<WordToSynthesize>& pOut,
                                                             SemanticRelativeTimeType pTimeType,
                                                             LinguisticVerbTense pVerbTense) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, [&]() {
        switch (pTimeType) {
            case SemanticRelativeTimeType::AFTER: return "after";
            case SemanticRelativeTimeType::BEFORE: return "before";
            case SemanticRelativeTimeType::DELAYEDSTART: return "in";
            case SemanticRelativeTimeType::DURING: return "during";
            case SemanticRelativeTimeType::JUSTAFTER: return "just after";
            case SemanticRelativeTimeType::JUSTBEFORE: return "just before";
...
void LinguisticsynthesizergroundingEnglish::_writeReDurationType(std::list<WordToSynthesize>& pOut,
                                                                 SemanticRelativeDurationType pDurationType,
                                                                 const GroundedExpression&) const {
    _strToOut(pOut, PartOfSpeech::PREPOSITION, [&]() {
        switch (pDurationType) {
            case SemanticRelativeDurationType::UNTIL: return "until";
        }
        assert(false);
        return "";
    }());
...
void LinguisticsynthesizergroundingEnglish::_getAllWord(std::list<WordToSynthesize>& pOut,
                                                        const SynthesizerCurrentContext&) const {
    _strToOut(pOut, PartOfSpeech::DETERMINER, "all");
}

void LinguisticsynthesizergroundingEnglish::_getEveryWord(std::list<WordToSynthesize>& pOut,
                                                          const SynthesizerCurrentContext&) const {
    _strToOut(pOut, PartOfSpeech::DETERMINER, "every");
}

void LinguisticsynthesizergroundingEnglish::_getDeterminerThatReferToRecentContext(std::list<WordToSynthesize>& pOut,
                                                                                   SemanticNumberType,
...
void LinguisticsynthesizergroundingEnglish::_getDeterminer(std::list<WordToSynthesize>& pOut,
    ...
    if (pGrounding.coreference) {
        ...
        if (pContext.wordContext.number == SemanticNumberType::PLURAL
            ...
            if (!pContext.isPositive && !pGrounding.quantity.isPlural())
...
void LinguisticsynthesizergroundingEnglish::_writeReflexivePronoun(std::list<WordToSynthesize>& pOut,
                                                                   RelativePerson pRelativePerson,
                                                                   SemanticGenderType pGender) const {
    switch (pRelativePerson) {
        case RelativePerson::FIRST_SING: _strToOut(pOut, PartOfSpeech::PRONOUN, "myself"); break;
        case RelativePerson::SECOND_SING: _strToOut(pOut, PartOfSpeech::PRONOUN, "yourself"); break;
        case RelativePerson::THIRD_SING: {
            switch (pGender) {
                case SemanticGenderType::FEMININE: _strToOut(pOut, PartOfSpeech::PRONOUN, "herself"); return;
                case SemanticGenderType::MASCULINE: _strToOut(pOut, PartOfSpeech::PRONOUN, "himself"); return;
                case SemanticGenderType::NEUTRAL:
                case SemanticGenderType::UNKNOWN: _strToOut(pOut, PartOfSpeech::PRONOUN, "itself"); break;
            }
            break;
        }
        case RelativePerson::FIRST_PLUR: _strToOut(pOut, PartOfSpeech::PRONOUN, "ourselves"); break;
        case RelativePerson::SECOND_PLUR: _strToOut(pOut, PartOfSpeech::PRONOUN, "yourselves"); break;
        case RelativePerson::THIRD_PLUR: _strToOut(pOut, PartOfSpeech::PRONOUN, "themselves"); break;
        case RelativePerson::UNKNOWN: _strToOut(pOut, PartOfSpeech::PRONOUN, "itself"); break;
    }
}
