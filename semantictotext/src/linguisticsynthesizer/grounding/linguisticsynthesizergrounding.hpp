...
public:
    ...
    void writeListSeparators(std::list<WordToSynthesize>& pOut,
                             ListExpressionType pListType,
                             bool pBeforeLastElt) const;

    void modifyContextForAGrounding(SynthesizerWordContext& pWordContext,
    ...
    void writeGroundingIntroduction(OutSemExp& pBeforeOut,
    ...
    void writeGrounding(OutSemExp& pOutSemExp,
                        const linguistics::InflectedWord& pOutInfoGram,
                        SynthesizerCurrentContext& pContext,
                        SynthesizerConfiguration& pConf,
                        const SemanticGrounding& pGrounding,
    ...
    void langGroundingTranslation(std::list<WordToSynthesize>& pOut, const SemanticLanguageGrounding& pGrounding) const;

    void textGroundingTranslation(std::list<WordToSynthesize>& pOut,
                                  const SemanticTextGrounding& pGrounding,
                                  const SynthesizerCurrentContext& pContext) const;

    void nameInfosTranslation(std::list<WordToSynthesize>& pOut, const NameInfos& pNameInfos) const;

    void resourceGroundingTranslation(std::list<WordToSynthesize>& pOut,
                                      const SemanticResourceGrounding& pGrounding) const;

    void unityGroundingTranslation(std::list<WordToSynthesize>& pOut,
    ...
    void beforeGenGroundingTranslation(OutSemExp& pBeforeOut,
                                       const linguistics::InflectedWord& pOutInfoGram,
                                       const SynthesizerCurrentContext& pContext,
                                       const SynthesizerConfiguration& pConf,
    ...
    void genGroundingTranslation(OutSemExp& pOutSemExp,
                                 const linguistics::InflectedWord& pOutInfoGram,
                                 SynthesizerCurrentContext& pContext,
                                 const linguistics::LinguisticDatabase& pLingDb,
                                 const SemanticGenericGrounding& pGrounding) const;

    void timeGroundingTranslation(std::list<WordToSynthesize>& pOut,
                                  const linguistics::LinguisticDatabase& pLingDb,
                                  const SemanticTimeGrounding& pGrounding,
                                  const GroundedExpression& pHoldingGrdExp) const;

    virtual PartOfSpeech writeRelativePerson(std::list<WordToSynthesize>& pOut,
    ...
    void writeVerbalSemWord(std::list<WordToSynthesize>& pBeforeOut,
    ...
    void writeSemWord(std::list<WordToSynthesize>& pBeforeOut,
                      OutSemExp& pOutSemExp,
                      std::list<WordToSynthesize>& pAfterOut,
                      const SemanticWord& pWord,
                      const linguistics::LinguisticDatabase& pLingDb,
    ...
    RelativePerson agentTypeToRelativePerson(const SemanticAgentGrounding& pGrounding,
                                             const SynthesizerConfiguration& pConf,
                                             bool pCanReplaceByEquivalent) const;

    void getIGramOfAStatementMeaning(linguistics::InflectedWord& pIGram,
                                     const SemanticStatementGrounding& pStatGrounding,
                                     const SynthesizerConfiguration& pConf) const;

    void getIGramOfAWord(linguistics::InflectedWord& pIGram,
                         const SemanticWord& pWord,
...
protected:
    ...
    Linguisticsynthesizergrounding(SemanticLanguageEnum pLanguage, const LinguisticSynthesizerPrivate& pLingSynth)
        : _language(pLanguage)
        , _lingSynth(pLingSynth) {}

    void _strToOut(std::list<WordToSynthesize>& pOut,
    ...
    void _strToOutCptsMove(std::list<WordToSynthesize>& pOut,
                           PartOfSpeech pPartOfSpeech,
                           const std::string& pStr,
                           const std::map<std::string, char>&& pConcepts,
    ...
    bool _strToOutIfNotEmpty(std::list<WordToSynthesize>& pOut,
                             PartOfSpeech pPartOfSpeech,
                             const std::string& pStr,
    ...
    virtual bool _dateTranslation(std::list<WordToSynthesize>& pOut,
    ...
    virtual void _writeReTimeType(std::list<WordToSynthesize>& pOut,
                                  SemanticRelativeTimeType pTimeType,
                                  LinguisticVerbTense pVerbTense) const = 0;

    virtual void _writeReDurationType(std::list<WordToSynthesize>& pOut,
                                      SemanticRelativeDurationType pDurationType,
                                      const GroundedExpression& pHoldingGrdExp) const = 0;

    virtual bool _doWeHaveToPutSubMeaningBeforeOrAfterTheWord(const SynthesizerCurrentContext&,
                                                              LinkedMeaningDirection pDirection) const {
        return pDirection != LinkedMeaningDirection::FORWARD;
    }

    virtual void _getPronounComplement(std::list<WordToSynthesize>& pOut,
                                       const StaticLinguisticMeaning& pMeaning,
                                       const linguistics::SpecificLinguisticDatabase& pSpecLingDb,
                                       const SynthesizerCurrentContext& pContext) const;

    virtual void _getAllWord(std::list<WordToSynthesize>& pOut, const SynthesizerCurrentContext& pContext) const = 0;
    virtual void _getEveryWord(std::list<WordToSynthesize>& pOut, const SynthesizerCurrentContext& pContext) const = 0;

    virtual void _getDeterminerThatReferToRecentContext(std::list<WordToSynthesize>& pOut,
    ...
    virtual void _writeReflexivePronoun(std::list<WordToSynthesize>& pOut,
                                        RelativePerson pRelativePerson,
                                        SemanticGenderType pGender) const = 0;
...
private:
    ...
    bool _needToWriteDeterminer(const linguistics::InflectedWord& pOutInfoGram,
                                const SynthesizerCurrentContext& pContext,
                                const SemanticGenericGrounding& pGrounding,
                                const GroundedExpression& pHoldingGrdExp,
                                const linguistics::LinguisticDatabase& pLingDb,
    ...
    void _statGroundingTranslation(OutSentence& pOutSentence,
                                   const linguistics::LinguisticDatabase& pLingDb,
                                   const linguistics::InflectedWord& pOutInfoGram,
                                   const GroundedExpression& pHoldingGrdExp,
                                   const SynthesizerCurrentContext& pContext) const;

    void _writeSubMeanings(std::list<WordToSynthesize>& pOut,
                           const linguistics::LinguisticDatabase& pLingDb,
                           const linguistics::LingWordsGroup& pMeaningsGroup,
                           const SynthesizerCurrentContext& pContext,
                           bool pBeforeVerb) const;

    void _writeGenGrounding(OutSemExp& pOutSemExp,
                            const linguistics::InflectedWord& pInfoGram,
                            const linguistics::LinguisticDatabase& pLingDb,
                            const SemanticGenericGrounding& pGrounding,
                            SynthesizerCurrentContext& pContext) const;

    void _modifyContextForAGenGrounding(SynthesizerWordContext& pWordContext,
                                        linguistics::InflectedWord& pOutInfoGram,
                                        const SynthesizerConfiguration& pConf,
                                        const SemanticGenericGrounding& pGrounding) const;
