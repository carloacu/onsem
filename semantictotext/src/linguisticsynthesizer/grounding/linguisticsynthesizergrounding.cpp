...
void _printValue(std::stringstream& pSs,
    ...
    if (!meaning.isEmpty()) {
        ...
        bool forceInPlural =
            pParentGrammaticalType == GrammaticalType::INTERVAL && pLanguage == SemanticLanguageEnum::FRENCH;
        std::string word;
        SemanticNumberType number = forceInPlural
                                      ? SemanticNumberType::PLURAL
                                      : (pValue > 1 ? SemanticNumberType::PLURAL : SemanticNumberType::SINGULAR);
...
void _printAngleValue(std::stringstream& pSs,
    ...
    _printValue(pSs,
                globalGender,
                pValue,
                semanticAngleUnity_toConcept(pAngle),
                pStatSynthDico,
                pLanguage,
                parentGrammaticalType);
}
...
void _printLengthValue(std::stringstream& pSs,
    ...
    _printValue(pSs,
                globalGender,
                pValue,
                semanticLengthUnity_toConcept(pLength),
                pStatSynthDico,
                pLanguage,
                parentGrammaticalType);
}
...
void _printTimeValue(std::stringstream& pSs,
    ...
    _printValue(pSs,
                pGlobalGender,
                pValue,
                semanticTimeUnity_toConcept(pTime),
                pStatSynthDico,
                pLanguage,
...
void Linguisticsynthesizergrounding::writeListSeparators(std::list<WordToSynthesize>& pOut,
    ...
    if (pBeforeLastElt)    // if we are before the last element
    {
        switch (pListType) {
            case ListExpressionType::AND:
                _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, _language == SemanticLanguageEnum::FRENCH ? "et" : "and");
                break;
            case ListExpressionType::OR:
                _strToOut(pOut, PartOfSpeech::CONJUNCTIVE, _language == SemanticLanguageEnum::FRENCH ? "ou" : "or");
                break;
...
void Linguisticsynthesizergrounding::modifyContextForAGrounding(SynthesizerWordContext& pWordContext,
    ...
    switch (pGrounding.type) {
        case SemanticGroundingType::GENERIC:
            _modifyContextForAGenGrounding(pWordContext, pOutInfoGram, pConf, pGrounding.getGenericGrounding());
            break;
        ...
        case SemanticGroundingType::NAME: {
            ...
            pWordContext.gender =
                synthGetter::getGender(pWordContext.gender, meaningPossGenders, nameGrd.nameInfos.possibleGenders);
            pOutInfoGram.word.language = _language;
    ...
    if (pOutInfoGram.word.isEmpty() && ConceptSet::haveAConceptThatBeginWith(pGrounding.concepts, "rank_"))
        rankConceptToInlfWord(pOutInfoGram, pGrounding.concepts);
}

void Linguisticsynthesizergrounding::writeGroundingIntroduction(OutSemExp& pBeforeOut,
...
void Linguisticsynthesizergrounding::writeGrounding(OutSemExp& pOutSemExp,
                                                    const linguistics::InflectedWord& pOutInfoGram,
                                                    SynthesizerCurrentContext& pContext,
                                                    SynthesizerConfiguration& pConf,
                                                    const SemanticGrounding& pGrounding,
    ...
    switch (pGrounding.type) {
        ...
        case SemanticGroundingType::STATEMENT: {
            const SemanticStatementGrounding& statementGrd = pGrounding.getStatementGrounding();
            OutSentence outSentence;
            linguistics::InflectedWord verbInfoGram;
            getIGramOfAStatementMeaning(verbInfoGram, statementGrd, pConf);
            statGroundingTranslation(outSentence, pConf, statementGrd, verbInfoGram, pHoldingGrdExp, pContext, nullptr);
            getChunksMerger().formulateSentence(pOutSemExp.out, outSentence);
            pOutSemExp.partOfSpeech = PartOfSpeech::VERB;
            break;
        }
        case SemanticGroundingType::AGENT: {
            agentGroundingTranslation(pOutSemExp, pConf, pGrounding.getAgentGrounding(), pContext, pRequests);
            break;
        }
        case SemanticGroundingType::ANGLE: {
            const auto& synthDico = pConf.lingDb.langToSpec[_language].synthDico;
            angleTranslation(pOutSemExp.out, synthDico, pGrounding.getAngleGrounding().angle, true);
            break;
        }
        case SemanticGroundingType::TIME: {
        ...
        case SemanticGroundingType::TEXT: {
            textGroundingTranslation(pOutSemExp.out, pGrounding.getTextGrounding(), pContext);
            break;
        }
        case SemanticGroundingType::DURATION: {
            const auto& synthDico = pConf.lingDb.langToSpec[_language].synthDico;
            durationTranslation(pOutSemExp.out,
                                synthDico,
                                pGrounding.getDurationGrounding().duration,
                                true,
                                pContext.grammaticalTypeFromParent);
            break;
        }
        ...
        case SemanticGroundingType::LENGTH: {
            const auto& synthDico = pConf.lingDb.langToSpec[_language].synthDico;
            lengthTranslation(pOutSemExp.out, synthDico, pGrounding.getLengthGrounding().length, true);
            break;
        }
        ...
        case SemanticGroundingType::UNITY: {
            const auto& synthDico = pConf.lingDb.langToSpec[_language].synthDico;
            unityGroundingTranslation(pOutSemExp.out,
                                      pGrounding.getUnityGrounding(),
                                      synthDico,
                                      pContext.grammaticalTypeFromParent,
                                      pRequests);
            break;
        ...
        case SemanticGroundingType::CONCEPTUAL: {
            if (ConceptSet::haveAConcept(pGrounding.concepts, "reflexive")) {
                if (pContext.currSentence != nullptr)
                    writeReflexiveObject(*pContext.currSentence,
                                         pOutSemExp.out,
                                         pContext.currSentence->subject.relativePerson,
                                         pContext.currSentence->subject.gender,
                                         pContext.verbContextOpt);
                else
                    _writeReflexivePronoun(
                        pOutSemExp.out, pContext.wordContext.relativePerson, pContext.wordContext.gender);
            } else {
                ...
                writeSemWord(pOutSemExp.out, pOutSemExp, pOutSemExp.out, pOutInfoGram.word, pConf.lingDb, pContext);

                if (pOutInfoGram.infos.linkedMeanings)
                    _writeSubMeanings(
                        pOutSemExp.out, pConf.lingDb, *pOutInfoGram.infos.linkedMeanings, pContext, false);
            }
            break;
        }
        case SemanticGroundingType::RELATIVELOCATION:
...
void Linguisticsynthesizergrounding::langGroundingTranslation(std::list<WordToSynthesize>& pOut,
                                                              const SemanticLanguageGrounding& pGrounding) const {
    _strToOutIfNotEmpty(pOut, PartOfSpeech::ADJECTIVE, sayLanguageInLanguage(pGrounding.language, _language));
}

void Linguisticsynthesizergrounding::textGroundingTranslation(std::list<WordToSynthesize>& pOut,
...
void Linguisticsynthesizergrounding::nameInfosTranslation(std::list<WordToSynthesize>& pOut,
                                                          const NameInfos& pNameInfos) const {
    for (const auto& currName : pNameInfos.names)
        _strToOut(pOut, PartOfSpeech::PROPER_NOUN, currName);
}

void Linguisticsynthesizergrounding::resourceGroundingTranslation(std::list<WordToSynthesize>& pOut,
...
void Linguisticsynthesizergrounding::unityGroundingTranslation(std::list<WordToSynthesize>& pOut,
    ...
    if (!meaning.isEmpty()) {
        ...
        if (pGrammTypeFromParent == GrammaticalType::UNITY || pGrammTypeFromParent == GrammaticalType::DURATION
            || pRequests.has(SemanticRequestType::QUANTITY)) {
            number = pGrounding.typeOfUnity == TypeOfUnity::PERCENTAGE ? SemanticNumberType::SINGULAR
                                                                       : SemanticNumberType::PLURAL;
        }
...
void Linguisticsynthesizergrounding::statGroundingTranslation(OutSentence& pOutSentence,
                                                              const SynthesizerConfiguration& pConf,
    ...
    if (_writeVerbGoal(pOutSentence, verbContext, pStatementGrd, pVerbInfoGram, pConf, pSubjectPtr)) {
        if (!verbContext.isPassive) {
            if (verbContext.verbTense == LinguisticVerbTense::PRETERIT_CONTINUOUS)
                ...
                verbContext.verbTense = _language == SemanticLanguageEnum::ENGLISH
                                          ? LinguisticVerbTense::PRESENT_IMPERATIVE
                                          : LinguisticVerbTense::INFINITIVE;
        }
        verbContext.isPositive = true;
        verbContext.requests.clear();
    }
    _statGroundingTranslation(pOutSentence, pConf.lingDb, pVerbInfoGram, pHoldingGrdExp, verbContext);
}

void Linguisticsynthesizergrounding::_statGroundingTranslation(OutSentence& pOutSentence,
                                                               const linguistics::LinguisticDatabase& pLingDb,
                                                               const linguistics::InflectedWord& pOutInfoGram,
                                                               const GroundedExpression& pHoldingGrdExp,
                                                               const SynthesizerCurrentContext& pContext) const {
    // write sub meanings before main word
    if (pOutInfoGram.infos.linkedMeanings && pHoldingGrdExp.children.count(GrammaticalType::RECEIVER) == 0)
        _writeSubMeanings(
            pOutSentence.receiverBeforeVerb.out, pLingDb, *pOutInfoGram.infos.linkedMeanings, pContext, true);

    // write the verb
    writeVerbalSemWord(
        pOutSentence.aux.out, pOutSentence.verb, pOutSentence.negation2.out, pOutInfoGram.word, pLingDb, pContext);
...
void Linguisticsynthesizergrounding::agentGroundingTranslation(OutSemExp& pOutSemExp,
                                                               SynthesizerConfiguration& pConf,
                                                               const SemanticAgentGrounding& pGrounding,
                                                               const SynthesizerCurrentContext& pContext,
                                                               const SemanticRequests& pRequests) const {
    RelativePerson relPerson = agentTypeToRelativePerson(pGrounding, pConf, !pContext.isPartOfANameAssignement);
    if (relPerson == RelativePerson::THIRD_SING && pGrounding.isSpecificUser()) {
        if (pContext.isPartOfANameAssignement && pConf.canDoAnotherRecurssiveCall()) {
        ...
        if (pGrounding.nameInfos) {
            ...
            if (!name.empty()) {
                _strToOutIfNotEmpty(pOutSemExp.out, PartOfSpeech::PROPER_NOUN, name);
            } else {
    ...
    pOutSemExp.partOfSpeech = writeRelativePerson(pOutSemExp.out,
                                                  relPerson,
                                                  SemanticReferenceType::INDEFINITE,
                                                  false,
                                                  SemanticEntityType::HUMAN,
                                                  SemanticQuantity(),
                                                  pContext,
                                                  pRequests);
}

void Linguisticsynthesizergrounding::_modifyContextForAGenGrounding(SynthesizerWordContext& pWordContext,
                                                                    linguistics::InflectedWord& pOutInfoGram,
                                                                    const SynthesizerConfiguration& pConf,
    ...
    if (!_groundingAttributesToWord(pOutInfoGram, pGrounding)
        && !synthGetter::getInflWordFromWordAndConcepts(
            pOutInfoGram, pGrounding.word, pGrounding.concepts, pConf.lingDb, _language)
        && !pGrounding.word.lemma.empty() && beginWithUppercase(pGrounding.word.lemma)) {
        pOutInfoGram.word.partOfSpeech = PartOfSpeech::PROPER_NOUN;
    }
    ...
    if (pOutInfoGram.word.partOfSpeech == PartOfSpeech::NOUN
        ...
        if (pGrounding.quantity.type == SemanticQuantityType::EVERYTHING) {
            if (_language == SemanticLanguageEnum::FRENCH)
                pWordContext.referenceType = SemanticReferenceType::DEFINITE;
                   ...
                   || (pGrounding.quantity.type == SemanticQuantityType::NUMBER && pGrounding.quantity.nb > 1)
        ...
        if (pGrounding.quantity.type == SemanticQuantityType::NUMBER && pGrounding.quantity.nb == 1) {
            pWordContext.number = SemanticNumberType::SINGULAR;
        } else if (_language == SemanticLanguageEnum::FRENCH && pWordContext.number != SemanticNumberType::UNKNOWN
        ...
        if (pOutInfoGram.word.partOfSpeech == PartOfSpeech::NOUN
            ...
            SemExpGetter::wordToAMeaning(nounLingMeaning, pOutInfoGram.word, _language, pConf.lingDb);
            ...
            if (nounLingMeaning.getLanguageIfNotEmpty(meaningLanguage))
                pConf.lingDb.langToSpec[meaningLanguage].synthDico.getNounGenders(
                    meaningPossGenders, nounLingMeaning, pWordContext.number);
        }
        pWordContext.gender =
            synthGetter::getGender(pWordContext.gender, meaningPossGenders, pGrounding.possibleGenders);
    }
}

bool Linguisticsynthesizergrounding::_needToWriteDeterminer(const linguistics::InflectedWord& pOutInfoGram,
                                                            const SynthesizerCurrentContext& pContext,
    ...
    if (pGrounding.entityType == SemanticEntityType::MODIFIER
        && (pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERNOUN
            || pContext.contextType == SYNTHESIZERCURRENTCONTEXTTYPE_SUBORDINATEAFTERADJECTIVE))
        return false;
    if (pOutInfoGram.word.partOfSpeech == PartOfSpeech::ADVERB && pGrounding.coreference)
        return false;
    if (pGrounding.entityType == SemanticEntityType::NUMBER && pGrounding.quantity.type == SemanticQuantityType::NUMBER)
    ...
    if (pContext.grammaticalTypeFromParent == GrammaticalType::REPETITION
        || ((pContext.grammaticalTypeFromParent == GrammaticalType::OBJECT
             ...
             || pContext.grammaticalTypeFromParent == GrammaticalType::UNKNOWN || pContext.isParentARelativeGrounding)
            && pGrounding.quantity.type == SemanticQuantityType::NUMBER && pGrounding.quantity.nb > 1
    ...
    if (SemExpGetter::getRank(pHoldingGrdExp) != 0)
        return true;
    if (pGrounding.entityType != SemanticEntityType::NUMBER && !pGrounding.quantity.isEqualToZero()
        ...
        && pOutInfoGram.word.partOfSpeech != PartOfSpeech::VERB && SemExpGetter::hasASpecificWord(pGrounding)) {
        if (!pGrounding.coreference && pGrounding.referenceType == SemanticReferenceType::DEFINITE) {
            switch (_language) {
                case SemanticLanguageEnum::ENGLISH: {
                    if (!pContext.requests.has(SemanticRequestType::ACTION)
                        && ConceptSet::haveAConceptThatBeginWithAnyOf(
                            pGrounding.concepts,
                            {"concrete_food_", "gas_", "liquid_", "sport_", "meal_", "number_", "time_day_", "url_"}))
                        return false;
...
void Linguisticsynthesizergrounding::writePreposition(std::list<WordToSynthesize>& pOut,
                                                      ...
                                                      const SemanticGrounding& pGrounding,
                                                      const GroundedExpression& pHoldingGrdExp) const {
    if (_language == SemanticLanguageEnum::FRENCH && pSubordinateOutPtr != nullptr
        && pSubordinateOutPtr->partOfSpeech == PartOfSpeech::PRONOUN && !pSubordinateOutPtr->out.empty()) {
    ...
    const std::list<WordToSynthesize>* subordinateWords =
        pSubordinateOutPtr != nullptr ? &pSubordinateOutPtr->out : nullptr;
        ...
        if (chkLinkTypeOpt) {
            ...
            if (pContext.contextType != SYNTHESIZERCURRENTCONTEXTTYPE_INDIRECTOBJECTBEFOREVERB
                ...
                if ((*chkLinkTypeOpt != linguistics::ChunkLinkType::DIRECTOBJECT
                     || pGrounding.type != SemanticGroundingType::STATEMENT
                     || pGrounding.getStatementGrounding().requests.empty()))
                    frameDict.getIntroWord(introWord,
                                           *chkLinkTypeOpt,
                ...
                frameDict.getIntroWordWithoutVerb(introWord,
                                                  *chkLinkTypeOpt,
                                                  &pHoldingGrdExp,
                                                  subordinateWords,
                                                  pContext.wordContext.number,
...
void Linguisticsynthesizergrounding::writePrepositionWithoutContext(std::list<WordToSynthesize>& pOut,
                                                                    const SynthesizerCurrentContext& pContext,
    ...
    if (introWord.language == SemanticLanguageEnum::FRENCH) {
        ...
        if (introWord.lemma == "au") {
            ...
            if (introWord.lemma.compare(introWord.lemma.size() - 3, 3, "que") == 0) {
                _strWithApostropheToOut(pOut,
                                        introWord.partOfSpeech,
                                        introWord.lemma.substr(0, introWord.lemma.size() - 1) + "'",
                                        introWord.lemma);
                return;
            }
        }
...
void Linguisticsynthesizergrounding::beforeGenGroundingTranslation(OutSemExp& pBeforeOut,
                                                                   const linguistics::InflectedWord& pOutInfoGram,
                                                                   const SynthesizerCurrentContext& pContext,
                                                                   const SynthesizerConfiguration& pConf,
                                                                   const SemanticGenericGrounding& pGrounding,
    ...
    if (SemExpGetter::hasASpecificWord(pGrounding) && pOutInfoGram.word.partOfSpeech != PartOfSpeech::DETERMINER
    ...
    if (_needToWriteDeterminer(
            pOutInfoGram, pContext, pGrounding, pHoldingGrdExp, pConf.lingDb, pContext.ownerWrittenBefore))
        ...
        if (pGrounding.quantity.nb > 1 || pGrounding.entityType == SemanticEntityType::NUMBER) {
            ...
            if (pGrounding.quantity.nb == 1) {
                ...
                if (itSpecifier != pHoldingGrdExp.children.end()) {
                    if (_language == SemanticLanguageEnum::FRENCH) {
                        if (synthGetter::getGenderFromSemExp(*itSpecifier->second, pConf, pContext, *this)
                            == SemanticGenderType::FEMININE)
...
void Linguisticsynthesizergrounding::genGroundingTranslation(OutSemExp& pOutSemExp,
                                                             const linguistics::InflectedWord& pOutInfoGram,
                                                             SynthesizerCurrentContext& pContext,
                                                             const linguistics::LinguisticDatabase& pLingDb,
                                                             const SemanticGenericGrounding& pGrounding) const {
...
void Linguisticsynthesizergrounding::getIGramOfAStatementMeaning(linguistics::InflectedWord& pIGram,
                                                                 const SemanticStatementGrounding& pStatGrounding,
                                                                 const SynthesizerConfiguration& pConf) const {
    if (_language == SemanticLanguageEnum::ENGLISH && pStatGrounding.verbGoal == VerbGoalEnum::CONDITIONAL
...
void Linguisticsynthesizergrounding::getIGramOfAWord(linguistics::InflectedWord& pIGram,
                                                     const SemanticWord& pWord,
                                                     const std::map<std::string, char>& pConcepts,
                                                     const SynthesizerConfiguration& pConf) const {
    StaticLinguisticMeaning lingMeaning(_language, SemExpGetter::wordToMeaningId(pWord, _language, pConf.lingDb));
    ...
    if (lingMeaning.isEmpty()) {
        ...
        for (const auto& currCpt : pConcepts) {
            ...
            if (newLingMeaning.isEmpty() && currCpt.first == "verb_generality") {
                if (_language == SemanticLanguageEnum::FRENCH)
...
bool Linguisticsynthesizergrounding::angleTranslation(std::list<WordToSynthesize>& pOut,
                                                      const linguistics::SynthesizerDictionary& pStatSynthDico,
...
bool Linguisticsynthesizergrounding::lengthTranslation(std::list<WordToSynthesize>& pOut,
                                                       const linguistics::SynthesizerDictionary& pStatSynthDico,
...
bool Linguisticsynthesizergrounding::durationTranslation(std::list<WordToSynthesize>& pOut,
                                                         const linguistics::SynthesizerDictionary& pStatSynthDico,
    ...
    if (!finishedToPrint && durationPrint.minute) {
        ...
        _printTimeValue(ss,
                        globalGender,
                        *durationPrint.minute,
                        SemanticTimeUnity::MINUTE,
                        pStatSynthDico,
                        _language,
                        pParentGrammaticalType);
        if (!pPrintPrecisely)
    ...
    if (!finishedToPrint && durationPrint.second) {
        ...
        _printTimeValue(ss,
                        globalGender,
                        *durationPrint.second,
                        SemanticTimeUnity::SECOND,
                        pStatSynthDico,
                        _language,
                        pParentGrammaticalType);
        if (!pPrintPrecisely)
    ...
    if (!finishedToPrint && durationPrint.millisecond) {
        ...
        _printTimeValue(ss,
                        globalGender,
                        *durationPrint.millisecond,
                        SemanticTimeUnity::MILLISECOND,
                        pStatSynthDico,
                        _language,
                        pParentGrammaticalType);
    }
    ...
    if (!timePrinted.empty()) {
        if (pParentGrammaticalType == GrammaticalType::INTERVAL) {
            if (_language == SemanticLanguageEnum::FRENCH)
                _strToOut(
                    pOut, PartOfSpeech::DETERMINER, globalGender == SemanticGenderType::FEMININE ? "toutes" : "tous");
            else
...
void Linguisticsynthesizergrounding::timeGroundingTranslation(std::list<WordToSynthesize>& pOut,
                                                              const linguistics::LinguisticDatabase& pLingDb,
    ...
    if (pGrounding.tryToConvertToATimeConcept(dateConceptToSay, refTimeGrd)) {
        if (dateConceptToSay == "time_relativeDay_today" && partOfDayMeaningToSay.isEmpty()) {
        ...
        if (!meaning.isEmpty()) {
            if (partOfDayMeaningToSay.isEmpty()) {
                ...
                if (dateConceptToSay == "time_relativeDay_today") {
                    ...
                    std::set<SemanticGenderType> meaningPossGenders;
                    synthDico.getNounGenders(meaningPossGenders, partOfDayMeaningToSay, numberSingular);
                    SemanticGenderType detGender =
                        meaningPossGenders.empty() ? SemanticGenderType::NEUTRAL : *meaningPossGenders.begin();
                ...
                _strToOut(pOut, PartOfSpeech::NOUN, synthDico.getLemma(partOfDayMeaningToSay, false));
                return;
            }
        }
    }
...
void Linguisticsynthesizergrounding::_writeSubMeanings(std::list<WordToSynthesize>& pOut,
                                                       const linguistics::LinguisticDatabase& pLingDb,
...
void Linguisticsynthesizergrounding::_writeGenGrounding(OutSemExp& pOutSemExp,
    ...
    if (!pInfoGram.word.lemma.empty())
        ...
        pOutSemExp.partOfSpeech = writeRelativePerson(pOutSemExp.out,
                                                      pContext.wordContext.relativePerson,
                                                      pGrounding.referenceType,
                                                      static_cast<bool>(pGrounding.coreference),
                                                      pGrounding.entityType,
                                                      pGrounding.quantity,
                                                      pContext,
                                                      pContext.requests);
    }
...
bool Linguisticsynthesizergrounding::_writeVerbalLinguisticMeaning(std::list<WordToSynthesize>& pBeforeOut,
    ...
    if (pContext.verbTense == LinguisticVerbTense::PRESENT_IMPERATIVE) {
        const auto& statSynthDico = specLingDb.synthDico.statDb;
        statSynthDico.getImperativeVerbForm(
            verbForm, pLingMeaning, pContext.wordContext.relativePerson, pContext.isPositive);
    } else if (pTryToWriteInSubordianteForm && _language == SemanticLanguageEnum::FRENCH
               && pContext.verbTense != LinguisticVerbTense::INFINITIVE) {
        ...
        statSynthDico.getVerbForm(verbForm,
                                  negationStr,
                                  pLingMeaning,
                                  pContext.wordContext.relativePerson,
                                  pContext.wordContext.gender,
                                  ...
                                  pContext.verbGoal,
                                  pContext.isPositive,
                                  pContext.hasASubject,
                                  isAGatheringMeaning,
                                  pContext.isPassive,
                                  pContext.isASubordinateWithoutPreposition,
                                  pContext.requests.has(SemanticRequestType::YESORNO));
    } else {
        std::string negationStr;
        bool isAGatheringMeaning = specLingDb.lingDico.statDb.isAGatheringMeaning(pLingMeaning);
        statSynthDico.getVerbForm(verbForm,
...
void Linguisticsynthesizergrounding::writeVerbalSemWord(std::list<WordToSynthesize>& pBeforeOut,
                                                        OutSemExp& pOutSemExp,
                                                        std::list<WordToSynthesize>& pAfterOut,
                                                        const SemanticWord& pWord,
                                                        const linguistics::LinguisticDatabase& pLingDb,
    ...
    if (pWord.partOfSpeech == PartOfSpeech::VERB || pWord.partOfSpeech == PartOfSpeech::AUX) {
        StaticLinguisticMeaning lingMeaning;
        SemExpGetter::wordToAStaticMeaning(lingMeaning, pWord, _language, pLingDb);
        if (!lingMeaning.isEmpty()
            && _writeVerbalLinguisticMeaning(pBeforeOut, pOutSemExp, pAfterOut, lingMeaning, pLingDb, pContext, false))
            return;
    }
...
void Linguisticsynthesizergrounding::writeSemWord(std::list<WordToSynthesize>& pBeforeOut,
                                                  OutSemExp& pOutSemExp,
                                                  std::list<WordToSynthesize>& pAfterOut,
                                                  const SemanticWord& pWord,
                                                  const linguistics::LinguisticDatabase& pLingDb,
    ...
    switch (pWord.partOfSpeech) {
        case PartOfSpeech::NOUN: {
            std::string nounStr;
            std::set<WordContextualInfos> contextualInfos;
            pLingDb.getInflectedNoun(
                nounStr, contextualInfos, _language, pWord, pContext.wordContext.gender, pContext.wordContext.number);
        ...
        case PartOfSpeech::ADJECTIVE: {
            ...
            if (!lingMeaning.isEmpty()) {
                ...
                const auto& statSynthDico = specLingDb.synthDico.statDb;
                std::string adjStr;
                statSynthDico.getAdjForm(adjStr,
                                         lingMeaning,
                                         pContext.wordContext.gender,
                                         pContext.wordContext.number,
                                         pContext.compPolarity);
        ...
        case PartOfSpeech::AUX: {
            ...
            SemExpGetter::wordToAStaticMeaning(lingMeaning, pWord, _language, pLingDb);
            if (!lingMeaning.isEmpty()
                && _writeVerbalLinguisticMeaning(
                    pBeforeOut, pOutSemExp, pAfterOut, lingMeaning, pLingDb, pContext, true))
                return;
            break;
        }
        default: break;
...
std::string Linguisticsynthesizergrounding::_getRootLemma(const std::string& pLemma) {
...
void Linguisticsynthesizergrounding::_getPronounComplement(std::list<WordToSynthesize>& pOut,
    ...
    _strToOut(pOut, PartOfSpeech::PRONOUN_COMPLEMENT, pSpecLingDb.synthDico.statDb.getLemma(pMeaning, false));
}

bool Linguisticsynthesizergrounding::_getIntroductingWordStoredInTheGrdExp(
    mystd::optional<SemanticWord>& pIntroWord,
    ...
    if (itIntroductingWord != pGrdExp.children.end()) {
        ...
        if (introWordGrdExpPtr != nullptr) {
            ...
            if (genGrdPtr != nullptr && genGrdPtr->word.language != _language
            ...
            modifyContextForAGrounding(wordContext,
                                       outInfoGram,
                                       pConf,
                                       grd,
