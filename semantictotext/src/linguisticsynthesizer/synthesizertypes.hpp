...
struct SynthesizerVerbContext {
    SynthesizerVerbContext(const linguistics::InflectedWord& pVerbInflWord, const SemanticStatementGrounding& pStatGrd)
...
private:
    ...
    static std::string _mergeUserIdWithContext(const std::string& pUserId, const std::string& pCurrentUserId);
};
...
struct SyntSentWorkStruct {
    ...
    bool beginOfAVerbalForm() const {
        return statementGrd.word.isEmpty() && statementGrd.concepts.empty()
    ...
    GroundingAnglePrettyPrintStruct(const SemanticAngle& pAngle)
        ...
        for (const auto& currAngleInfo : pAngle.angleInfos) {
            switch (currAngleInfo.first) {
                case SemanticAngleUnity::DEGREE: degree.emplace(currAngleInfo.second); break;
                case SemanticAngleUnity::RADIAN: radian.emplace(currAngleInfo.second); break;
                default: break;
            }
        }
    }
    ...
    GroundingLengthPrettyPrintStruct(const SemanticLength& pLength)
        ...
        for (const auto& currLengthInfo : pLength.lengthInfos) {
            switch (currLengthInfo.first) {
                case SemanticLengthUnity::KILOMETER: kilometer.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::HECTOMETER: hectometer.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::DECAMETER: decameter.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::METER: meter.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::DECIMETER: decimeter.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::CENTIMETER: centimeter.emplace(currLengthInfo.second); break;
                case SemanticLengthUnity::MILLIMETER: millimeter.emplace(currLengthInfo.second); break;
                default: break;
            }
        }
    }
    ...
    GroundingDurationPrettyPrintStruct(const SemanticDuration& pDuration)
        ...
        for (const auto& currTimeInfo : pDuration.timeInfos) {
            switch (currTimeInfo.first) {
                case SemanticTimeUnity::DAY: day.emplace(currTimeInfo.second); break;
                case SemanticTimeUnity::HOUR: hour.emplace(currTimeInfo.second); break;
                case SemanticTimeUnity::MINUTE: minute.emplace(currTimeInfo.second); break;
                case SemanticTimeUnity::SECOND: second.emplace(currTimeInfo.second); break;
                case SemanticTimeUnity::MILLISECOND: millisecond.emplace(currTimeInfo.second); break;
                default: break;
            }
        }
    }
