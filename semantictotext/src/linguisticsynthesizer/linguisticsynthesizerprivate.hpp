...
public:
    ...
    void getTranslationToNaturalLanguage(std::list<std::unique_ptr<SynthesizerResult>>& pNaturalLanguageResult,
    ...
    SemExpTypeEnumFormSynt writeGrdExp(OutSemExp& pOutSemExp,
...
protected:
    ...
    enum class SpecifierPosition { BEFORE_DETERMINER, BEFORE, AFTER };
    ...
    virtual LinguisticVerbTense _semanticVerbTenseToLinguisticVerbTense(
    ...
    void _strToOut(std::list<WordToSynthesize>& pOut, PartOfSpeech pPartOfSpeech, const std::string& pStr) const {
    ...
    void _strWithApostropheToOut(std::list<WordToSynthesize>& pOut,
                                 PartOfSpeech pPartOfSpeech,
    ...
    enum class ObjectPosition { BEFORESUBJECT, BEFOREVERB, JUSTAFTERVERB, AFTERVERB };

    enum class ReceiverPosition { BEFOREVERB, AFTERVERB };
    ...
    virtual void _getQuestionWord(std::list<WordToSynthesize>& pOut,
    ...
    virtual bool _tryToWriteTopicBeforeVerb(std::list<WordToSynthesize>&, const SemanticExpression&) const {
        return false;
    }
    ...
    virtual void _getBeginOfForChild(std::list<WordToSynthesize>& pOut, const SemanticExpression& pSemExp) const = 0;

    virtual void _getBeginOfReceiverOfNounChild(std::list<WordToSynthesize>& pOut,
    ...
    virtual void _getBeginOfMitigationChild(std::list<WordToSynthesize>& pOut) const = 0;

    virtual void _getBeginOfYesOrNoSubordonate(std::list<WordToSynthesize>& pOut) const = 0;
    ...
    virtual void _getWheneverCondition(std::list<WordToSynthesize>& pOut) const = 0;
    ...
    virtual void _writeAnythingHumanQuantity(OutSentence& pOutSentence) const = 0;
    ...
    virtual void _getThenWord(std::list<WordToSynthesize>& pOut) const = 0;

    virtual void _getElseWord(std::list<WordToSynthesize>& pOut) const = 0;

    virtual void _getPossessiveWord(std::list<WordToSynthesize>& pOut,
    ...
    virtual void _getPossessiveDeterminer(std::list<WordToSynthesize>& pOut,
                                          RelativePerson pRelPerson,
                                          SemanticGenderType pGender,
                                          SemanticNumberType pNumber) const = 0;

    virtual SpecifierPosition _getSpecifierPosition(const linguistics::InflectedWord& pInflWord,
                                                    const SemanticGrounding& pGrounding,
                                                    bool pADetermantHasBeenWrote,
                                                    const linguistics::InflectedWord* pMotherInflWordPtr) const = 0;

    virtual bool _doWeHaveToWriteBeginOfSpecifier(SynthesizerCurrentContext& pGrdContext,
...
private:
    ...
    void _writeSemExp(std::list<SemExpTypeEnumFormSynt>& pSemExpSyntTypes,
                      OutSemExp& pOutSemExp,
                      SemanticRequests& pRequests,
                      SemanticExpression const** pLastSubject,
                      const SemanticExpression& pSemExp,
    ...
    void _startAQuestion(std::list<WordToSynthesize>& pOut,
                         OutSemExp& pSubjectOut,
                         const SemanticRequests& pRequests,
                         bool pEquVerb,
                         SemanticVerbTense pVerbTense,
    ...
    void _writeSentenceGrdExp(std::list<WordToSynthesize>& pOut,
                              SemanticRequests& pRequests,
                              SemanticExpression const** pLastSubject,
                              const GroundedExpression& pGrdExp,
                              const SemanticStatementGrounding& pStatementGrd,
    ...
    void _writeNominalGrdExp(OutSemExp& pOutSemExp,
                             SemanticRequests& pRequests,
                             SemanticExpression const** pLastSubject,
                             const GroundedExpression& pGrdExp,
    ...
    SemExpTypeEnumFormSynt _writeCondExp(OutSemExp& pOutSemExp,
                                         SemanticRequests& pRequests,
                                         SemanticExpression const** pLastSubject,
    ...
    SemExpTypeEnumFormSynt _writeComparisonExp(std::list<WordToSynthesize>& pOut,
                                               SemanticRequests& pRequests,
                                               SemanticExpression const** pLastSubject,
    ...
    void _writeEquality(SyntSentWorkStruct& pSentWorkStruct,
                        const SemanticExpression& pSemExp,
                        SynthesizerConfiguration& pConf,
                        const SemanticRequests& pRequests,
                        const SynthesizerCurrentContext& pContext,
    ...
    RelativePerson _semExpToRelativePerson(const SemanticExpression& pSemExp,
                                           const SynthesizerConfiguration& pConf,
                                           bool pIsANameAssignement) const;

    void _writeSubjectContext(OutSentence& pOutSentence,
                              SemanticExpression const** pLastSubject,
    ...
    void _writeNounSubordinates(OutSemExp& pOutSemExp,
                                const SemanticExpression& pSemExp,
    ...
    void _writeSomeModifiersOfAWord(OutSemExp& pOutBeforeDeterminer,
    ...
    void _fillOutBlocsList(std::list<WordToSynthesize>& pOut,
