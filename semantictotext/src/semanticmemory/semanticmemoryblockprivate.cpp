...
bool SemanticMemoryBlockPrivate::empty() const {
    return expressionsMemories.empty() && expressionsMemoriesPtrToIt.empty() && answersLinks.empty()
        && conditionToInformationLinks.empty() && sentWithActionLinks.empty() && conditionToActions.empty()
        && sentWithInfActionLinks.empty() && infActions.empty() && userCenteredLinks.empty() && _triggerLinks.empty();
}
...
SemanticMemoryLinksForAnyVerbGoal& SemanticMemoryBlockPrivate::ensureSentenceTriggersLinks(
    SemanticExpressionCategory pSemExpCategory,
    const SemanticTriggerAxiomId& pAxiomId) {
    switch (pSemExpCategory) {
        case SemanticExpressionCategory::AFFIRMATION: return _triggerLinks[pAxiomId].affirmationLinks;
        case SemanticExpressionCategory::COMMAND: return _triggerLinks[pAxiomId].actionLinks;
        case SemanticExpressionCategory::QUESTION: return _triggerLinks[pAxiomId].questionLinks;
        default: {
            assert(false);
...
SemanticMemoryLinksForAnyVerbGoal* SemanticMemoryBlockPrivate::getSentenceTriggersLinks(
    ...
    if (it != _triggerLinks.end()) {
        switch (pSemExpCategory) {
            case SemanticExpressionCategory::AFFIRMATION: return &it->second.affirmationLinks; ;
            case SemanticExpressionCategory::COMMAND: return &it->second.actionLinks;
            case SemanticExpressionCategory::QUESTION: return &it->second.questionLinks;
            default: {
                assert(false);
                return nullptr;
            }
...
SemanticLinksToGrdExps& SemanticMemoryBlockPrivate::ensureNominalGroupsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) {
    return _triggerLinks[pAxiomId].nominalGroupsLinks;
}

SemanticLinksToGrdExps* SemanticMemoryBlockPrivate::getNominalGroupsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) {
    auto it = _triggerLinks.find(pAxiomId);
    if (it != _triggerLinks.end())
...
const SemanticLinksToGrdExps* SemanticMemoryBlockPrivate::getNominalGroupsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) const {
    auto it = _triggerLinks.find(pAxiomId);
    if (it != _triggerLinks.end())
        return &it->second.nominalGroupsLinks;
...
SemanticLinksToGrdExps& SemanticMemoryBlockPrivate::ensureRecommendationsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) {
    return _triggerLinks[pAxiomId].recommendationsLinks;
}

SemanticLinksToGrdExps* SemanticMemoryBlockPrivate::getRecommendationsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) {
    auto it = _triggerLinks.find(pAxiomId);
    if (it != _triggerLinks.end())
...
const SemanticLinksToGrdExps* SemanticMemoryBlockPrivate::getRecommendationsTriggersLinks(
    const SemanticTriggerAxiomId& pAxiomId) const {
    auto it = _triggerLinks.find(pAxiomId);
    if (it != _triggerLinks.end())
        return &it->second.recommendationsLinks;
...
void SemanticMemoryBlockPrivate::addConditionToAnAction(const GroundedExpWithLinks& pMemSent,
    ...
    for (auto& currReq : pMemSentLinks.reqToGrdExps) {
        currReq.second.timeToSemExps.for_each(
            [&](const std::string& pKey, const mystd::vector_of_refs<SemanticMemoryGrdExp>& pValue) {
...
void SemanticMemoryBlockPrivate::removeLinksIfEmpty(const SemanticTriggerAxiomId& pAxiomId) {
    auto it = _triggerLinks.find(pAxiomId);
    if (it != _triggerLinks.end() && it->second.empty())
        _triggerLinks.erase(it);
}

RequestToMemoryLinks<true> SemanticMemoryBlockPrivate::getLinks(SemanticTypeOfLinks pTypeOfLinks,
...
RequestToMemoryLinks<false> SemanticMemoryBlockPrivate::getLinks(SemanticTypeOfLinks pTypeOfLinks,
                                                                 SemanticVerbTense pTense,
