...
void SemanticMemory::addNewUserFocusedToSemExp(UniqueSemanticExpression pSemExp, const std::string& pUserId) {
    _newUserFocusedToSemExps[pUserId].emplace_back(std::move(pSemExp));
}

void SemanticMemory::extractSemExpsForASpecificUser(std::list<UniqueSemanticExpression>& pSemExps,
...
void SemanticMemory::registerExternalFallback(std::unique_ptr<ExternalFallback> pExtFallback) {
    _externalFallback = std::move(pExtFallback);
}

void SemanticMemory::setCurrUserId(const std::string& pNewUserId) {
...
void SemanticMemory::registerExternalInfosProvider(std::unique_ptr<MemBlockAndExternalCallback> pExtCallbackPtr,
    ...
    for (auto& currSemExp : pExtCallback.getTriggers())
        pExtCallback.memBlockForTriggers.addRootSemExpWithHisContent(
            InformationType::ASSERTION, std::move(currSemExp), pLingDb);
    pExtCallback.getTriggers().clear();

    callbackToSentencesCanBeAnswered.emplace_back(
        mystd::unique_propagate_const<MemBlockAndExternalCallback>(std::move(pExtCallbackPtr)));
}

void SemanticMemory::unregisterExternalInfosProvider(const std::string& pIdStr) {
    for (auto itSentCanBeAnsw = callbackToSentencesCanBeAnswered.begin();
         itSentCanBeAnsw != callbackToSentencesCanBeAnswered.end();
         ++itSentCanBeAnsw) {
