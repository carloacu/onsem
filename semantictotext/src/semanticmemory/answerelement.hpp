...
struct AnswerElement {
    ...
    virtual std::unique_ptr<SemanticExpressionContainer> getSemExpForGrammaticalType(
...
struct AnswerElementDynamic : public AnswerElement {
    ...
    AnswerElementDynamic(const GroundedExpWithLinks* pMemSentPtr)
        : AnswerElement()
    ...
    std::unique_ptr<GroundedExpressionContainer> getGrdExpContainer() override {
        return std::make_unique<GroundedExpressionRef>(_memSentPtr->grdExp);
    }
    const SemanticAnnotations& getAnnotations() const override { return *_annotations; }
    std::unique_ptr<SemanticExpressionContainer> getSemExpForGrammaticalType(
        GrammaticalType pGrammType,
        const GroundedExpression* pFromGrdExpQuestion,
        const linguistics::LinguisticDatabase* pLingDb,
...
struct AnswerElementStatic : public AnswerElement {
    AnswerElementStatic(const unsigned char* pMemSentPtr, const linguistics::LinguisticDatabase& pLingDb);
