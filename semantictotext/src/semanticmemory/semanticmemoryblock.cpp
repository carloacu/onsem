...
namespace onsem {
namespace {
static inline void _writeStringPtr(binarymasks::Ptr& pPtr, const std::string* pStr) {
...
SemanticMemoryBlock::SemanticMemoryBlock(std::size_t pMaxNbOfKnowledgesInAMemoryBloc)
...
void SemanticMemoryBlock::_pruneExceedingExpressions(
    const linguistics::LinguisticDatabase& pLingDb,
    std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr) {
...
std::shared_ptr<ExpressionWithLinks> SemanticMemoryBlock::_addRootSemExp(
    UniqueSemanticExpression pNewRootSemExp,
    const mystd::radix_map_str<std::string>* pLinkedInfosPtr) {
    auto expForMemory = std::make_shared<ExpressionWithLinks>(*this, std::move(pNewRootSemExp), pLinkedInfosPtr);
...
void SemanticMemoryBlock::addTrackerSemExp(UniqueSemanticExpression pNewRootSemExp,
                                           std::shared_ptr<SemanticTracker>& pSemTracker,
                                           const linguistics::LinguisticDatabase& pLingDb) {
    auto memKnow = std::make_unique<ExpressionWithLinks>(*this, std::move(pNewRootSemExp));
    memKnow->addAxiomListToMemory(
        *memKnow->semExp, &pSemTracker, InformationType::INFORMATION, true, nullptr, nullptr, nullptr, pLingDb, false);
...
void SemanticMemoryBlock::addRootSemExpWithHisContent(InformationType pInformationType,
                                                      UniqueSemanticExpression pNewRootSemExp,
                                                      const linguistics::LinguisticDatabase& pLingDb) {
    auto expForMem = addRootSemExp(std::move(pNewRootSemExp), pLingDb);
    expForMem->addAxiomListToMemory(
        *expForMem->semExp, nullptr, pInformationType, false, nullptr, nullptr, nullptr, pLingDb, false);
}
...
void SemanticMemoryBlock::clearSemExpBySemExpOnlyForTests(
    const linguistics::LinguisticDatabase& pLingDb,
    std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr) {
...
void SemanticMemoryBlock::removeExpression(
    ExpressionWithLinks& pExpressionToRemove,
    const linguistics::LinguisticDatabase& pLingDb,
    std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr) {
...
void SemanticMemoryBlock::print(std::list<SemLineToPrint>& pLines,
    ...
    std::stringstream ss;
    ss << "nb of expressions in memory: " << _impl->expressionsMemories.size()
...
std::unique_ptr<GroundedExpressionContainer> SemanticMemoryBlock::getEquivalentGrdExpPtr(
...
void SemanticMemoryBlock::getAllEquivalentUserIds(std::list<std::string>& pRes, const std::string& pUser) const {
...
bool SemanticMemoryBlock::_userIdToHardCodedUserId(std::string& pHardCodedUserId, const std::string& pUserId) const {
...
void SemanticMemoryBlock::_nameToUserIds(std::set<std::string>& pUserIds, const std::string& pName) const {
...
bool SemanticMemoryBlock::areSameUserConst(const std::string& pUserId1,
    ...
    if (_impl->userCenteredLinks.areSameUser(pUserId1, pUserId2, linksPtr)
        || (_impl->subBinaryBlockPtr && _impl->subBinaryBlockPtr->areSameUser(pUserId1, pUserId2))) {
        if (linksPtr != nullptr && pRelContextAxiomPtr != nullptr) {
            for (auto& currSemMemGrdExp : linksPtr->assertions)
                for (auto& currSemId : currSemMemGrdExp.second)
...
bool SemanticMemoryBlock::_doesUserIdExist(const std::string& pUserId) const {
    bool res = _impl->userCenteredLinks.doesUserIdExist(pUserId)
            || (_impl->subBinaryBlockPtr && _impl->subBinaryBlockPtr->doesUserIdExist(pUserId));
    if (res)
        return true;
...
std::unique_ptr<SemanticAgentGrounding> SemanticMemoryBlock::generateNewAgentGrd(
...
SemanticBehaviorDefinition SemanticMemoryBlock::extractActionFromMemorySentence(
    ...
    if (pMemorySentence.getContextAxiom().infCommandToDo != nullptr) {
        bool isAComposedAction =
            !SemExpGetter::hasGroundingType(*pMemorySentence.getContextAxiom().infCommandToDo,
        ...
        if (isAComposedAction)
            return SemanticBehaviorDefinition(pMemorySentence.grdExp.clone(),
                                              pMemorySentence.getContextAxiom().infCommandToDo->clone());
        return SemanticBehaviorDefinition(pMemorySentence.grdExp.clone());
    } else {
        assert(false);
    }
    ...
    for (const GroundedExpWithLinks* currConditionToActionsMemSent : pConditionToActionsMemSent) {
        ...
        if (contextAxiom.semExpToDo != nullptr) {
            pConditionToActionsSemExp.emplace_back(
                SemExpCreator::formulateConditionToAction(currConditionToActionsMemSent->grdExp,
                                                          *contextAxiom.semExpToDo,
                                                          contextAxiom.semExpToDoElse,
...
void SemanticMemoryBlock::writeInBinaryFile(const linguistics::LinguisticDatabase& pLingDb,
...
void SemanticMemoryBlock::_writeInBinary(binarymasks::Ptr& pPtr, const linguistics::LinguisticDatabase& pLingDb) const {
    ...
    for (const auto& expMem : _impl->expressionsMemories) {
        pPtr = radixmap::write<std::string>(expMem->linkedInfos, pPtr, &_writeStringPtr);
...

    {
        ...
        writeUserCenteredLinksInBinary(pPtr, _impl->userCenteredLinks, memGrdExpPtrOffsets, semExpPtrOffsets);
        pPtr = binarysaver::alignMemory(pPtr);
...

    {
        ...
        pPtr = radixmap::write<std::string>(
            _hardCodedUserIdResolution, pPtr, [](binarymasks::Ptr& pSubPtr, const std::string* pStrPtr) {
...
const SemanticExpression* SemanticMemoryBlock::getBeforeLastSemExpOfAnAuthor(
    const SemanticExpression& pAuthor,
    const linguistics::LinguisticDatabase& pLingDb) const {
        ...
        if (SemExpGetter::doesContainADialogSource(currSemExp)) {
            if (lastPtr == nullptr)
                ...
                if (contextAuthorPtr != nullptr
                    && SemExpComparator::semExpsAreEqualFromMemBlock(
                        pAuthor, *contextAuthorPtr, *this, pLingDb, nullptr))
                    return &currSemExp;
            }
        }
    }
...
const SemanticExpression* SemanticMemoryBlock::getBeforeLastSemExpOfNotAnAuthor(
    const SemanticExpression& pAuthor,
    const linguistics::LinguisticDatabase& pLingDb) const {
