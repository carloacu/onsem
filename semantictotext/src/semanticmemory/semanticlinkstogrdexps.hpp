...
enum class LinksAccessType { MAIN_VALUE, MAIN_VALUE_AND_ALL_KEYS, ALL };
...
void _semanticValueLinks_addValue(
    ...
    const std::function<bool(const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&,
                             const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&)>& pIsMoreRevelant) {
    if (!_mainValueIsSet) {
...
#define SemanticValueLinks_LinksAccessType_MAIN_VALUE                                                                 \
    ...
    const TVALUE& getValue() const {                                                                                  \
        assert(_mainValueIsSet);                                                                                      \
        return _mainValuesToSemExp.first;                                                                             \
    }                                                                                                                 \
    const SemanticSplitAssertAndInformLinks<LINKS_TYPE>& getLinks() const {                                           \
        assert(_mainValueIsSet);                                                                                      \
        return _mainValuesToSemExp.second;                                                                            \
    }                                                                                                                 \
    void addValue(const TVALUE& pValue,                                                                               \
                  const std::function<void(SemanticSplitAssertAndInformLinks<LINKS_TYPE>&)>& pUpdateTheLinks,         \
                  const std::function<bool(const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&,                      \
                                           const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&)>& pIsMoreRevelant) { \
        return _semanticValueLinks_addValue(                                                                          \
            _mainValueIsSet, _mainValuesToSemExp, _valuesToSemExps, pValue, pUpdateTheLinks, pIsMoreRevelant);        \
    }                                                                                                                 \
    void removeValue(                                                                                                 \
        const TVALUE& pValue,                                                                                         \
        const std::function<void(SemanticSplitAssertAndInformLinks<LINKS_TYPE>&)>& pUpdateTheLinks,                   \
        const std::function<bool(const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&,                                \
                                 const SemanticSplitAssertAndInformLinks<LINKS_TYPE>&)>& pIsMoreRevelant) {           \
        return _semanticValueLinks_removeValue(                                                                       \
            _mainValueIsSet, _mainValuesToSemExp, _valuesToSemExps, pValue, pUpdateTheLinks, pIsMoreRevelant);        \
    }                                                                                                                 \
                                                                                                                      \
private:                                                                                                              \
...
#define SemanticValueLinks_LinksAccessType_MAIN_VALUE_AND_ALL_KEYS                          \
    ...
    void iterateOnKeysStoppable(const std::function<bool(const TVALUE&)>& pOnValue) const { \
        if (!_mainValueIsSet || !pOnValue(_mainValuesToSemExp.first))                       \
            return;                                                                         \
        for (const auto& currElt : _valuesToSemExps)                                        \
...
struct SemanticValueLinks<TVALUE, LINKS_TYPE, MAP_TYPE, LinksAccessType::MAIN_VALUE_AND_ALL_KEYS> {
    SemanticValueLinks_LinksAccessType_MAIN_VALUE_AND_ALL_KEYS SemanticValueLinks_LinksAccessType_MAIN_VALUE
};

template<typename TVALUE, typename LINKS_TYPE, typename MAP_TYPE>
struct SemanticValueLinks<TVALUE, LINKS_TYPE, MAP_TYPE, LinksAccessType::ALL> {
    void iterateOnKeysValues(
...
using SemanticGenderLinks =
    SemanticValueLinks<SemanticGenderType,
                       LINKS_TYPE,
                       std::map<SemanticGenderType, SemanticSplitAssertAndInformLinks<LINKS_TYPE>>,
                       access>;
template<typename LINKS_TYPE, LinksAccessType access>
using SemanticUserNamesLinks = SemanticValueLinks<UserNames,
                                                  LINKS_TYPE,
                                                  std::map<UserNames, SemanticSplitAssertAndInformLinks<LINKS_TYPE>>,
                                                  access>;
template<typename LINKS_TYPE, LinksAccessType access>
using SemanticStringLinks = SemanticValueLinks<std::string,
                                               LINKS_TYPE,
                                               mystd::radix_map_str<SemanticSplitAssertAndInformLinks<LINKS_TYPE>>,
                                               access>;
template<typename LINKS_TYPE, LinksAccessType access>
using SemanticGrdExpPtrLinks =
    SemanticValueLinks<const GroundedExpression*,
                       LINKS_TYPE,
                       std::map<const GroundedExpression*, SemanticSplitAssertAndInformLinks<LINKS_TYPE>>,
                       access>;
...
struct UserCharacteristics {
    void clear() {
        genderLinks.clear();
        nameLinks.clear();
        equivalentUserIdLinks.clear();
        semExpLinks.clear();
    }
    bool empty() const {
        return genderLinks.empty() && nameLinks.empty() && equivalentUserIdLinks.empty() && semExpLinks.empty();
    }
...
struct SemanticUserCenteredMemoryLinks {
    void clear() {
        userIdToUserCharacteristics.clear();
        nameToUserIds.clear();
        semExpToUserIds.clear();
    }
    bool empty() const {
        return userIdToUserCharacteristics.empty() && nameToUserIds.empty() && semExpToUserIds.empty();
    }
    ...
    std::unique_ptr<SemanticNameGrounding> getNameGrd(
    ...
    void getAllEquivalentUserIds(std::list<std::string>& pRes, const std::string& pUserId) const;
    ...
    //! User identification to a grounded expression that describes the user and it's not his name, it's to avoid
    //! sentences like "Eva is Eva".
...
using SemanticUserCenteredMemoryLinksForMem = SemanticUserCenteredMemoryLinks<MemoryGrdExpLinks,
                                                                              LinksAccessType::MAIN_VALUE_AND_ALL_KEYS,
                                                                              LinksAccessType::MAIN_VALUE>;
using SemanticUserCenteredMemoryLinksForAMemSentence =
    SemanticUserCenteredMemoryLinks<MemoryGrdExpLinksForAMemSentence, LinksAccessType::ALL, LinksAccessType::ALL>;
...
public:
    ...
    void writeInBinary(binarymasks::Ptr& pPtr, const MemGrdExpPtrOffsets& pMemGrdExpPtrOffsets) const;

    void clear() {
        links.clear();
        pastLinks.clear();
        futureLinks.clear();
        infintiveLinks.clear();
    }
...
template<typename TVALUE, typename LINKS_TYPE, typename MAP_TYPE>
const SemanticSplitAssertAndInformLinks<LINKS_TYPE>*
    SemanticValueLinks<TVALUE, LINKS_TYPE, MAP_TYPE, LinksAccessType::ALL>::keyToLinks(const TVALUE& pKey) const {
    if (!_mainValueIsSet)
        return nullptr;
...
template<typename TVALUE, typename LINKS_TYPE, typename MAP_TYPE>
SemanticSplitAssertAndInformLinks<LINKS_TYPE>*
    SemanticValueLinks<TVALUE, LINKS_TYPE, MAP_TYPE, LinksAccessType::ALL>::keyToLinks(const TVALUE& pKey) {
    if (!_mainValueIsSet)
        return nullptr;
    if (_mainValuesToSemExp.first == pKey)
        return &_mainValuesToSemExp.second;
