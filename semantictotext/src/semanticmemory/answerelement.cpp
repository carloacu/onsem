...
std::unique_ptr<SemanticExpressionContainer> AnswerElementDynamic::getSemExpForGrammaticalType(
    ...
    if (rawRes != nullptr) {
        if (pHasSamePolarityPtr != nullptr && pFromGrdExpQuestion != nullptr && pLingDb != nullptr)
            *pHasSamePolarityPtr = SemExpComparator::haveSamePolarity(
                _memSentPtr->grdExp, *pFromGrdExpQuestion, pLingDb->conceptSet, true);
...
void AnswerElementStatic::_fillAnnotations() const {
    ...
    readAllEnumMapValues(annotationsPtr, grammaticalType_size, [&](unsigned char pKey, const unsigned char* pValue) {
...
std::unique_ptr<SemanticExpressionContainer> AnswerElementStatic::getSemExpForGrammaticalType(
    ...
    if (semExpPtr != nullptr) {
        if (pHasSamePolarityPtr != nullptr && pFromGrdExpQuestion != nullptr && pLingDb != nullptr)
            *pHasSamePolarityPtr =
                SemExpComparator::haveSamePolarity(grdExp, *pFromGrdExpQuestion, pLingDb->conceptSet, true);
