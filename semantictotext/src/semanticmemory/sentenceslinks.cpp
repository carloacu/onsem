...
void RelationsThatMatch<IS_MODIFIABLE>::addMemSentStatic(const unsigned char* pStaticMemorySentencePtr,
    ...
    if (filterPtr == nullptr || filterPtr->filterConditionStatic(*elt))
        res.staticLinks[SemanticMemoryBlockBinaryReader::memorySentenceToId(pStaticMemorySentencePtr)] = std::move(elt);
}
...
template<bool IS_MODIFIABLE>
MemoryLinksAccessor<IS_MODIFIABLE> RequestToMemoryLinks<IS_MODIFIABLE>::getMemoryLinksAccessors(
