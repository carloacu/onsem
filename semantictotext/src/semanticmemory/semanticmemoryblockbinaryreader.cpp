...
const unsigned char* _getTenseLinks(const unsigned char* pPtr, SemanticVerbTense pTense) {
    std::size_t nbOfMemoryLinksToSkip = 6;
    switch (pTense) {
        case SemanticVerbTense::PUNCTUALPAST:
        case SemanticVerbTense::PAST: nbOfMemoryLinksToSkip = 2; break;
        case SemanticVerbTense::FUTURE: nbOfMemoryLinksToSkip = 4; break;
        case SemanticVerbTense::PRESENT:
        case SemanticVerbTense::PUNCTUALPRESENT: return pPtr;
        case SemanticVerbTense::UNKNOWN: break;
    }
...
const unsigned char* _getVerbGoalLinks(const unsigned char* pPtr, VerbGoalEnum pVerbGoal) {
    if (pVerbGoal == VerbGoalEnum::ABILITY)
        return jumpAfterEnumMap(pPtr);
    return pPtr;
}

bool _getNameWithoutConsideringEquivalences(std::string& pRes, const unsigned char*& pNameLinksPtr) {
...
const unsigned char* SemanticMemoryBlockBinaryReader::getLinks(SemanticTypeOfLinks pTypeOfLinks,
                                                               SemanticVerbTense pTense,
                                                               VerbGoalEnum pVerbGoal) const {
    std::size_t linksId = 0;
    switch (pTypeOfLinks) {
        case SemanticTypeOfLinks::ANSWER: break;
        case SemanticTypeOfLinks::CONDITION_INFORMATION: linksId = 1; break;
        case SemanticTypeOfLinks::SENT_WITH_ACTION: linksId = 2; break;
    }
...
const unsigned char* SemanticMemoryBlockBinaryReader::getLinksForARequest(const unsigned char* pPtr,
...
const unsigned char* SemanticMemoryBlockBinaryReader::memorySentenceToAnnotationsPtr(const unsigned char* pPtr) {
    ...
    return pPtr + 1 /* nb of memory grd exp offset */ + ((*pPtr) /* nb of memory grd exp */ * 4);
}
...
const unsigned char* SemanticMemoryBlockBinaryReader::userIdToUserCharacteristicsPtr(const std::string& pUserId) const {
    ...
    return radixmap::read(
        ptr, pUserId, [](const unsigned char* pEndOfNodePtr) { return binaryloader::loadChar_0(pEndOfNodePtr); });
}
...
SemanticGenderType SemanticMemoryBlockBinaryReader::getGender(const std::string& pUserId) const {
    const unsigned char* ptr = userIdToUserCharacteristicsPtr(pUserId);
    if (ptr != nullptr && binaryloader::loadChar_1(ptr++))
...
std::string SemanticMemoryBlockBinaryReader::getName(const std::string& pUserId) const {
    ...
    if (ptr != nullptr) {
        ...
        for (uint32_t i = 0; i < nbOfOtherEquivalentUserIds; ++i) {
            ...
            if (subPtr != nullptr && _getNameWithoutConsideringEquivalences(res, subPtr))
                return res;
...
std::unique_ptr<SemanticNameGrounding> SemanticMemoryBlockBinaryReader::getNameGrd(const std::string& pUserId) const {
...
void SemanticMemoryBlockBinaryReader::getAllEquivalentUserIds(std::list<std::string>& pRes,
...
bool SemanticMemoryBlockBinaryReader::areSameUser(const std::string& pUserId1, const std::string& pUserId2) const {
...
std::string SemanticMemoryBlockBinaryReader::getUserIdFromGrdExp(const GroundedExpression& pGrdExp,
                                                                 const SemanticMemoryBlock& pMemBloc,
                                                                 const linguistics::LinguisticDatabase& pLingDb) {
    std::string res;
    readAllIntMapValues(_userLinksPtr, [&](int pGrdExpOffset, const unsigned char* pStrPtr) {
        if (res.empty()) {
...
std::string SemanticMemoryBlockBinaryReader::userIdToHardCodedUserIdsPtr(const std::string& pUserId) const {
    const unsigned char* strPtr = radixmap::read(
        _hardCodedUserIdsPtr, pUserId, [](const unsigned char* pEndOfNodePtr) { return *pEndOfNodePtr == true; });
    if (strPtr != nullptr) {
        ++strPtr;
