...
void _writeLinksGrdExps(binarymasks::Ptr& pPtr,
                        ...
                        const MemGrdExpPtrOffsets& pMemGrdExpPtrOffsets) {
    auto _writeGrdExpLinks = [&](binarymasks::Ptr& pSubPtr, const MemoryGrdExpLinks* pMemSentMapPtr) {
        _writeLinksOpt(pSubPtr, pMemSentMapPtr, pMemGrdExpPtrOffsets);
    };
    auto _writeGrdExpLinksRef = [&](binarymasks::Ptr& pSubPtr, const MemoryGrdExpLinks& pMemSentMap) {
    ...
    pPtr = radixmap::write<MemoryGrdExpLinks>(pLinksToGrdExpsMapPtr->conceptsToSemExps, pPtr, _writeGrdExpLinks);

    offsets.pint[0] = pPtr.val - offsets.val;
    writeEnumMap<SemanticLanguageEnum, std::map<int, MemoryGrdExpLinks>>(
        pPtr,
        pLinksToGrdExpsMapPtr->meaningsToSemExps,
        semanticLanguageEnum_allValues,
    ...
    writeEnumMap<SemanticEntityType, MemoryGrdExpLinks>(pPtr,
                                                        pLinksToGrdExpsMapPtr->everythingOrNoEntityTypeToSemExps,
                                                        semanticEntityType_allValues,
                                                        _writeGrdExpLinks);

    offsets.pint[2] = pPtr.val - offsets.val;
    writeEnumMap<SemanticEntityType, MemoryGrdExpLinks>(
        pPtr, pLinksToGrdExpsMapPtr->genGroundingTypeToSemExps, semanticEntityType_allValues, _writeGrdExpLinks);

    offsets.pint[3] = pPtr.val - offsets.val;
    pPtr = radixmap::write<MemoryGrdExpLinks>(pLinksToGrdExpsMapPtr->timeToSemExps, pPtr, _writeGrdExpLinks);

    offsets.pint[4] = pPtr.val - offsets.val;
    writeEnumMap<SemanticRelativeLocationType, MemoryGrdExpLinks>(
        pPtr, pLinksToGrdExpsMapPtr->relLocationToSemExps, semanticRelativeLocationType_allValues, _writeGrdExpLinks);

    offsets.pint[5] = pPtr.val - offsets.val;
    writeEnumMap<SemanticRelativeTimeType, MemoryGrdExpLinks>(
        pPtr, pLinksToGrdExpsMapPtr->relTimeToSemExps, semanticRelativeTimeType_allValues, _writeGrdExpLinks);

    offsets.pint[6] = pPtr.val - offsets.val;
    writeEnumMap<SemanticGroundingType, MemoryGrdExpLinks>(
        pPtr, pLinksToGrdExpsMapPtr->grdTypeToSemExps, semanticGroundingType_allValues, _writeGrdExpLinks);

    offsets.pint[7] = pPtr.val - offsets.val;
    pPtr = radixmap::write<MemoryGrdExpLinks>(pLinksToGrdExpsMapPtr->userIdToSemExps, pPtr, _writeGrdExpLinks);

    offsets.pint[8] = pPtr.val - offsets.val;
    writeEnumMap<SemanticLanguageEnum, mystd::radix_map_str<MemoryGrdExpLinks>>(
        pPtr,
        pLinksToGrdExpsMapPtr->textToSemExps,
        semanticLanguageEnum_allValues,
        [&](binarymasks::Ptr& pSubPtr, const mystd::radix_map_str<MemoryGrdExpLinks>* pTextToLinks) {
            pSubPtr = radixmap::write<MemoryGrdExpLinks>(*pTextToLinks, pSubPtr, _writeGrdExpLinks);
        });

    offsets.pint[9] = pPtr.val - offsets.val;
    writeEnumMap<SemanticLanguageEnum, MemoryGrdExpLinks>(
        pPtr, pLinksToGrdExpsMapPtr->languageToSemExps, semanticLanguageEnum_allValues, _writeGrdExpLinks);

    offsets.pint[10] = pPtr.val - offsets.val;
    pPtr = radixmap::write<MemoryGrdExpLinks>(pLinksToGrdExpsMapPtr->resourceToSemExps, pPtr, _writeGrdExpLinks);
}
...
void _writeMemoryLinksInBinary(binarymasks::Ptr& pPtr,
    ...
    writeEnumMap<SemanticRequestType, SemanticLinksToGrdExps>(
        pPtr,
        pLinks.reqToGrdExps,
        semanticRequestType_allValues,
        [&](binarymasks::Ptr& pSubPtr, const SemanticLinksToGrdExps* pLinksToGrdExpsMapPtr) {
...
bool SemanticLinksToGrdExpsTemplate<LINKS_TYPE>::empty() const {
    return conceptsToSemExps.empty() && meaningsToSemExps.empty() && everythingOrNoEntityTypeToSemExps.empty()
        && genGroundingTypeToSemExps.empty() && timeToSemExps.empty() && durationToSemExps.empty()
        && relLocationToSemExps.empty() && numberToSemExps.empty() && quantityTypeToSemExps.empty()
        && referenceWithoutConceptToSemExps.empty() && coreferenceWithoutConceptOrReferenceToSemExps.empty()
        && statementCoreferenceWithoutConceptToSemExps.empty() && relTimeToSemExps.empty() && grdTypeToSemExps.empty()
        && userIdToSemExps.empty() && userIdWithoutContextToSemExps.empty() && textToSemExps.empty()
        && languageToSemExps.empty() && resourceToSemExps.empty();
}

template<typename LINKS_TYPE, LinksAccessType access1, LinksAccessType access2>
bool SemanticUserCenteredMemoryLinks<LINKS_TYPE, access1, access2>::doesUserIdExist(const std::string& pUserId) const {
...
template<typename LINKS_TYPE, LinksAccessType access1, LinksAccessType access2>
std::string SemanticUserCenteredMemoryLinks<LINKS_TYPE, access1, access2>::getName(const std::string& pUserId) const {
...
void writeUserCenteredLinksInBinary(binarymasks::Ptr& pPtr,
    ...
    pPtr = radixmap::write<UserCharacteristics<MemoryGrdExpLinks, LinksAccessType::MAIN_VALUE>>(
        pUserCenteredMemoryLinks.userIdToUserCharacteristics,
        pPtr,
        [&](binarymasks::Ptr& pLeafPtr,
            ...
            if (pUserCharacteristicsPtr != nullptr) {
                ...
                if (!userCharacteristics.genderLinks.empty()) {
                    ...
                    binarysaver::writeChar_2To7(pLeafPtr.pchar++,
                                                semanticGenderType_toChar(userCharacteristics.genderLinks.getValue()));
                    _writeLinksFromAssertAndInform(
                        pLeafPtr, userCharacteristics.genderLinks.getLinks(), pMemGrdExpPtrOffsets);
                } else {
                ...
                if (!userCharacteristics.nameLinks.empty()) {
                    ...
                    if (nbOfNames > 0)
                        _writeLinksFromAssertAndInform(
                            pLeafPtr, userCharacteristics.nameLinks.getLinks(), pMemGrdExpPtrOffsets);
                } else {
                ...
                if (!userCharacteristics.semExpLinks.empty()) {
                    ...
                    if (mainSemExpLinksPtr != nullptr) {
                        ...
                        _writeLinksFromAssertAndInform(
                            pLeafPtr, userCharacteristics.semExpLinks.getLinks(), pMemGrdExpPtrOffsets);
                    } else {
    ...
    pPtr = radixmap::write<SemanticStringLinks<MemoryGrdExpLinks, LinksAccessType::MAIN_VALUE_AND_ALL_KEYS>>(
        pUserCenteredMemoryLinks.nameToUserIds,
        pPtr,
        [&](binarymasks::Ptr& pLeafPtr,
...
template<typename LINKS_TYPE, LinksAccessType access1, LinksAccessType access2>
bool SemanticUserCenteredMemoryLinks<LINKS_TYPE, access1, access2>::hasEquivalentUserIds(
    const std::string& pUserId) const {
    auto* userIdsPtr = userIdToUserCharacteristics.find_ptr(pUserId);
...
void SemanticUserCenteredMemoryLinks<LINKS_TYPE, access1, access2>::getAllEquivalentUserIds(
    ...
    auto* userIdsPtr = userIdToUserCharacteristics.find_ptr(pUserId);
    if (userIdsPtr != nullptr) {
        userIdsPtr->equivalentUserIdLinks.iterateOnKeys([&](const std::string& pName) { pRes.emplace_back(pName); });
    }
}
...
template<typename LINKS_TYPE, LinksAccessType access1, LinksAccessType access2>
std::unique_ptr<GroundedExpressionContainer>
    SemanticUserCenteredMemoryLinks<LINKS_TYPE, access1, access2>::getEquivalentGrdExpPtr(
        const std::string& pUserId) const {
    auto* userPtr = userIdToUserCharacteristics.find_ptr(pUserId);
...
const SemanticMemoryLinksForAnyVerbGoal& SemanticMemoryLinksForAnyVerbTense::getLinksForATense(
    ...
    switch (pTense) {
        ...
        case SemanticVerbTense::PAST: return pastLinks;
        case SemanticVerbTense::FUTURE: return futureLinks;
        case SemanticVerbTense::PRESENT:
        case SemanticVerbTense::PUNCTUALPRESENT: return links;
        case SemanticVerbTense::UNKNOWN: return infintiveLinks;
    }
    return infintiveLinks;
}

SemanticMemoryLinksForAnyVerbGoal& SemanticMemoryLinksForAnyVerbTense::getLinksForATense(SemanticVerbTense pTense) {
    switch (pTense) {
        case SemanticVerbTense::PUNCTUALPAST:
        case SemanticVerbTense::PAST: return pastLinks;
        case SemanticVerbTense::FUTURE: return futureLinks;
...
const SemanticMemoryLinks& SemanticMemoryLinksForAnyVerbTense::getLinks(SemanticVerbTense pTense,
...
void SemanticMemoryLinksForAnyVerbTense::writeInBinary(binarymasks::Ptr& pPtr,
...
const SemanticMemoryLinks& SemanticMemoryLinksForAnyVerbGoal::getLinksForAGoal(VerbGoalEnum pVerbGoal) const {
    switch (pVerbGoal) {
        case VerbGoalEnum::ABILITY: return ability;
...
template struct SemanticUserCenteredMemoryLinks<MemoryGrdExpLinks,
                                                LinksAccessType::MAIN_VALUE_AND_ALL_KEYS,
                                                LinksAccessType::MAIN_VALUE>;
template struct SemanticUserCenteredMemoryLinks<MemoryGrdExpLinksForAMemSentence,
                                                LinksAccessType::ALL,
                                                LinksAccessType::ALL>;
