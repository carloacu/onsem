...
std::list<ExecutionData>& ExecutionData::linkToChildList(VirtualOutputter::Link pLink) {
    switch (pLink) {
        case VirtualOutputter::Link::AND: return toRunInParallel;
        case VirtualOutputter::Link::IN_BACKGROUND: return toRunInBackground;
        default: return toRunSequencially;
    }
}
...
std::string ExecutionData::run(SemanticMemory& pSemanticMemory,
    ...
    if (punctualAssertion) {
        memoryOperation::notifyPunctually(**punctualAssertion, InformationType::ASSERTION, pSemanticMemory, pLingDb);
    }
    ...
    auto printList = [&](const std::list<ExecutionData>& pListToPrint, const std::string& pSeparator) {
    ...
    if (permanentAssertion) {
        memoryOperation::informAxiom(std::move(*permanentAssertion), pSemanticMemory, pLingDb);
    }
    if (informationToTeach) {
        mystd::unique_propagate_const<UniqueSemanticExpression> reaction;
        memoryOperation::teach(reaction,
                               pSemanticMemory,
                               std::move(*informationToTeach),
                               pLingDb,
...
void ExecutionDataOutputter::_exposeResource(const SemanticResource& pResource,
...
void ExecutionDataOutputter::_exposeText(const std::string& pText, SemanticLanguageEnum pLanguage) {
    auto& newElt = _getOrCreateNewElt();
    newElt.text = pText;
...
void ExecutionDataOutputter::_assertPunctually(const SemanticExpression& pSemExp) {
    _getLasExectInStack().punctualAssertion = std::make_unique<UniqueSemanticExpression>(pSemExp.clone());
}

void ExecutionDataOutputter::_teachInformation(UniqueSemanticExpression pUSemExp) {
    _getLasExectInStack().informationToTeach = std::make_unique<UniqueSemanticExpression>(std::move(pUSemExp));
}

void ExecutionDataOutputter::_assertPermanently(UniqueSemanticExpression pUSemExp) {
    _getLasExectInStack().permanentAssertion = std::make_unique<UniqueSemanticExpression>(std::move(pUSemExp));
}
