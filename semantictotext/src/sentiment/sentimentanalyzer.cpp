...
void SentimentAnalyzer::extract(std::list<std::unique_ptr<SentimentContext>>& pSentContexts,
    ...
    if (semExpAut1 != nullptr && semExpAut2 != nullptr) {
        ...
        sameAuthor =
            SemExpComparator::semExpsAreEqualFromMemBlock(*semExpAut1, *semExpAut2, _semMem.memBloc, _lingDb, nullptr);
    }

    completeWithContext(
        pSemExp, GrammaticalType::UNKNOWN, *_prevSemExp, sameAuthor, semExpAut1, _semMem.memBloc, _lingDb);
