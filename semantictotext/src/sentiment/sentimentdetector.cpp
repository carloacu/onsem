...
bool _invertConcept(std::string& pCptName, const ConceptSet& pConceptSet) {
...
void _sentimentFromAuthorToSubject(std::unique_ptr<SemanticExpression>& pAuthor,
                                   std::unique_ptr<SemanticExpression>& pReceiver,
                                   const GroundedExpression& pGrdExp,
                                   const SemanticAgentGrounding& pAuthorGrd) {
    pAuthor = std::make_unique<GroundedExpression>(std::make_unique<SemanticAgentGrounding>(pAuthorGrd));
...
void _sentimentFromAuthorToContext(std::unique_ptr<SemanticExpression>& pAuthor,
                                   std::unique_ptr<SemanticExpression>& pReceiver,
                                   const SemanticAgentGrounding& pAuthorGrd) {
    pAuthor = std::make_unique<GroundedExpression>(std::make_unique<SemanticAgentGrounding>(pAuthorGrd));
...
void _sentimentFromSubjectToObject(std::unique_ptr<SemanticExpression>& pAuthor,
                                   std::unique_ptr<SemanticExpression>& pReceiver,
                                   const GroundedExpression& pGrdExp) {
    auto itSubject = pGrdExp.children.find(GrammaticalType::SUBJECT);
    if (itSubject != pGrdExp.children.end()) {
...
void _addSent(std::map<std::string, std::unique_ptr<SentimentContext>>& pSentTypeToSpec,
              std::unique_ptr<SemanticExpression> pAuthor,
              std::unique_ptr<SemanticExpression> pReceiver,
              const std::string& pCptName,
              int pConfidence) {
    if (!pAuthor)
        pAuthor = std::make_unique<GroundedExpression>(std::make_unique<SemanticGenericGrounding>());
    if (!pReceiver)
        pReceiver = pAuthor->clone();

    std::unique_ptr<SentimentContext>& sentContextPtr = pSentTypeToSpec[pCptName];
    if (!sentContextPtr || sentContextPtr->sentimentStrengh <= pConfidence)
        sentContextPtr =
            std::make_unique<SentimentContext>(std::move(pAuthor), pCptName, pConfidence, std::move(pReceiver));
}

void _extractSentFromStatementGrounding(std::map<std::string, std::unique_ptr<SentimentContext>>& pSentTypeToSpec,
    ...
    for (const auto& currConcept : pSentimentConcepts) {
        ...
        _addSent(pSentTypeToSpec, std::move(author), std::move(receiver), cptName, confidence);
    }
}

void _sentimentFromSubjectToSubject(std::unique_ptr<SemanticExpression>& pAuthor,
                                    std::unique_ptr<SemanticExpression>& pReceiver,
                                    const GroundedExpression& pGrdExp) {
    auto itSubject = pGrdExp.children.find(GrammaticalType::SUBJECT);
    if (itSubject != pGrdExp.children.end()) {
...
void _extractSentFromGenGrounding(std::map<std::string, std::unique_ptr<SentimentContext>>& pSentTypeToSpec,
                                  const GroundedExpression& pGrdExp,
...
void _extractSentFromSemExp(std::map<std::string, std::unique_ptr<SentimentContext>>& pSentTypeToSpec,
                            const GroundedExpression& pGrdExp,
    ...
    if (grdGroundingType == SemanticGroundingType::STATEMENT) {
        ...
        if (!askAction && isAPresentTense(statementGrd.verbTense) && statementGrd.concepts.count("verb_go") != 0
            && (statementGrd.requests.empty() || statementGrd.requests.has(SemanticRequestType::YESORNO)
        ...
        if (pStatementGrdExpPtr == nullptr || pVerbIsEquality)
            pRootVerbAtInfinitive = statementGrd.isAtInfinitive()
                                 && !SemExpGetter::isACoreferenceFromStatementGrounding(
                                        statementGrd, CoreferenceDirectionEnum::UNKNOWN, true);
        ...
        _extractSentFromStatementGrounding(
            pSentTypeToSpec, sentimentConcepts, pGrdExp, authorIsTheSubject, verbConfidence, pAuthor, pConceptSet);
    } else if (grdGroundingType == SemanticGroundingType::GENERIC || grdGroundingType == SemanticGroundingType::NAME
        ...
        _extractSentFromGenGrounding(pSentTypeToSpec,
                                     pGrdExp,
                                     pRootGrdExp,
                                     pStatementGrdExpPtr,
                                     pVerbIsEquality,
                                     pRootVerbAtInfinitive,
                                     wordConfidence,
                                     pAuthor,
                                     pConceptSet);
    }
    ...
    if (!askSomething) {
        ...
        for (const auto& currChild : pGrdExp.children) {
            ...
            if (childGrdExp != nullptr) {
                ...
                _extractSentFromSemExp(pSentTypeToSpec,
                                       *childGrdExp,
                                       &pGrdExp,
                                       pStatementGrdExpPtr,
                                       pVerbIsEquality,
