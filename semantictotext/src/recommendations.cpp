...
void _getLinksFromMemBlock(std::map<const ExpressionWithLinks*, int>& pLinks,
    ...
    switch (pSemExp.type) {
        case SemanticExpressionType::METADATA: {
            ...
            auto language =
                metadaExp.fromLanguage != SemanticLanguageEnum::UNKNOWN ? metadaExp.fromLanguage : pLanguage;
        ...
        case SemanticExpressionType::GROUNDED: {
            ...
            semanticMemoryGetter::findGrdExpInRecommendationLinks(
                expPtr, grdExp, pContainer.goundingsToCoef.memBlock, pLingDb, pLanguage);
            ...
            if (!groundingCoef || *groundingCoef != 0)
                semanticMemoryGetter::findGrdExpWithCoefInRecommendationLinks(
                    pLinks, grdExp, groundingCoef, pContainer.memBlock, pLingDb, pLanguage);
        ...
        case SemanticExpressionType::CONDITION: break;
    }
}
...
void addARecommendation(SemanticRecommendationsContainer& pContainer,
    ...
    pContainer.recommendationsToNumberOfLinks.emplace(&*expWrappedForMemory,
                                                      std::pair<std::string, int>(pRecommendationId, sizeOfSemExp));
    _linkToMemBlock(*expWrappedForMemory, *expWrappedForMemory->semExp, pLingDb);
}
...
void getRecommendations(std::map<int, std::set<std::string>>& pRecommendations,
    ...
    for (const auto& currExpToSimilarity : expToSimilarities) {
        ...
        if (itToName != pContainer.recommendationsToNumberOfLinks.end()) {
            ...
            if (coef > minCoefToAllowANewRecommendation
                && pForbiddenRecommendations.count(itToName->second.first) == 0) {
