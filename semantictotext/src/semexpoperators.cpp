...
mystd::unique_propagate_const<UniqueSemanticExpression> answer(UniqueSemanticExpression pSemExp,
    ...
    controller::applyOperatorOnSemExpConstMem(compositeSemAnswers,
                                              semExp,
                                              SemanticOperatorEnum::ANSWER,
...
SemanticEngagementValue extractEngagement(const SemanticExpression& pSemExp) {
    ...
    if (fdkExpPtr != nullptr) {
        ...
        if (res != SemanticEngagementValue::UNKNWON) {
            const GroundedExpression* concernedExpPtr = fdkExpPtr->concernedExp->getGrdExpPtr_SkipWrapperPtrs();
    ...
    if (grdExpPtr != nullptr) {
        ...
        const SemanticStatementGrounding* statementGrdPtr = grdExpPtr->grounding().getStatementGroundingPtr();
        if (statementGrdPtr != nullptr && statementGrdPtr->verbTense == SemanticVerbTense::PRESENT) {
                ...
                if (subjectAgentGrdPtr != nullptr
                    ...
                    if (actionGrdExpPtr != nullptr) {
                        ...
                        if (actionStatementGrdPtr != nullptr
                            && ConceptSet::haveAnyOfConcepts(actionStatementGrdPtr->concepts,
                                                             {"verb_action_go", "verb_action_leave"})
                            && (!_hasObjectExecptCoreference(actionGrdExp) || SemExpGetter::isNow(actionGrdExp)))
...
bool isASubpart(const SemanticExpression& pInputSemExp,
    ...
    if (listExp != nullptr
        && (listExp->listType == ListExpressionType::UNRELATED || listExp->listType == ListExpressionType::AND))
    ...
    ImbricationType semExpsImbrications = SemExpComparator::getSemExpsImbrications(
        pInputSemExp, pSemExpToFind, pSemanticMemory.memBloc, pLingDb, nullptr, &comparisonErrorReporting);
    if (semExpsImbrications == ImbricationType::EQUALS || semExpsImbrications == ImbricationType::MORE_DETAILED
    ...
    if (grdExptrToMatch != nullptr) {
        ...
        if (genGrdPtrToMatch != nullptr && genGrdPtrToMatch->word.partOfSpeech == PartOfSpeech::INTERJECTION) {
            const FeedbackExpression* inputFdkExpPtr = pInputSemExp.getFdkExpPtr_SkipWrapperPtrs();
            if (inputFdkExpPtr != nullptr) {
                ImbricationType feedbackImbrications = SemExpComparator::getSemExpsImbrications(
                    *inputFdkExpPtr->feedbackExp, pSemExpToFind, pSemanticMemory.memBloc, pLingDb, nullptr);
...
mystd::unique_propagate_const<UniqueSemanticExpression> generateDefintionFromAnOldOrder(
    const SemanticExpression& pOrderSemExp,
    const GroundedExpression& pNewLabel,
    const SemanticMemory& pSemanticMemory,
    const linguistics::LinguisticDatabase& pLingDb) {
    auto copyGrdExpAtImperative = [](const GroundedExpression& pGrdExp, const SemanticStatementGrounding& pStatGrd) {
        auto res = std::make_unique<GroundedExpression>([&pStatGrd] {
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (statGrdPtr != nullptr) {
            ...
            if (statGrd.requests.types.size() == 1 && statGrd.requests.first() == SemanticRequestType::ACTION) {
                ...
                if (SemExpComparator::grdExpsAreEqual(
                        *subActionAtInfinitive, pNewLabel, pSemanticMemory.memBloc, pLingDb)) {
...
void mergeWithContext(UniqueSemanticExpression& pSemExp,
    ...
    if (lastText != nullptr) {
        ...
        if (currAuthorPtr != nullptr && contextAuthorPtr != nullptr)
            sameAuthor = SemExpComparator::semExpsAreEqualFromMemBlock(
                *currAuthorPtr, *contextAuthorPtr, pSemanticMemory.memBloc, pLingDb, nullptr);
        ...
        if (!sameAuthor && currAuthorPtr != nullptr) {
            ...
            if (SemExpGetter::isItAnActionLabeling(actionLabelPtr, equalityStatementPtr, *pSemExp)) {
                ...
                if (beforeLastTextPtr != nullptr) {
                    ...
                    auto defSemExp =
                        generateDefintionFromAnOldOrder(beforeLastText, actionLabel, pSemanticMemory, pLingDb);
                    if (defSemExp)
                        pSemExp = std::make_unique<InterpretationExpression>(
                            InterpretationSource::RECENTCONTEXT,
                            SemExpCreator::formulateActionDefinition(
                                actionLabel, *equalityStatementPtr, std::move(*defSemExp)),
                            std::move(pSemExp));
                }
            }
        }
        completeWithContext(
            pSemExp, GrammaticalType::UNKNOWN, *lastText, sameAuthor, currAuthorPtr, pSemanticMemory.memBloc, pLingDb);
    }
...
std::shared_ptr<ExpressionWithLinks> _informMetaMemory(
    ...
    std::unique_ptr<CompositeSemAnswer> compositeSemAnswers;
    controller::applyOperatorOnExpHandleInMemory(compositeSemAnswers,
                                                 newExpForMemRef,
    ...
    if (compositeSemAnswers) {
        controller::linkConditionalReactions(
            compositeSemAnswers->semAnswers, newExpForMemRef, pSemanticMemory, pLingDb, pInformationType);
...
void notifyPunctually(const SemanticExpression& pSemExp,
    ...
    controller::applyOperatorOnSemExp(
        compositeSemAnswers, pSemExp, SemanticOperatorEnum::INFORM, pInformationType, pSemanticMemory, pLingDb);
    if (compositeSemAnswers) {
...
std::shared_ptr<ExpressionWithLinks> inform(
    ...
    const mystd::radix_map_str<std::string>* pLinkedInfosPtr,
    std::map<const SentenceWithLinks*, TruenessValue>* pAxiomToConditionCurrentStatePtr) {
    return _informMetaMemory(std::move(pSemExp),
                             pSemanticMemory.memBloc,
                             pSemanticMemory,
                             pAxiomToConditionCurrentStatePtr,
                             pLingDb,
...
void defaultKnowledge(SemanticMemory& pSemanticMemory, const linguistics::LinguisticDatabase& pLingDb) {
    ...
    _informAxioms({"je vais bien"}, SemanticLanguageEnum::FRENCH, pSemanticMemory, pLingDb);
...
std::shared_ptr<ExpressionWithLinks> addFallback(UniqueSemanticExpression pSemExp,
    ...
    return _informMetaMemory(std::move(pSemExp),
                             *pSemanticMemory.memBloc.getFallbacksBlockPtr(),
                             pSemanticMemory,
                             nullptr,
                             pLingDb,
                             InformationType::FALLBACK,
                             pLinkedInfosPtr);
}
...
mystd::unique_propagate_const<UniqueSemanticExpression> resolveCommandFromMemBlock(
    ...
    controller::applyOperatorOnSemExpConstMem(compSemAnswers,
                                              *clonedSemExp,
                                              SemanticOperatorEnum::RESOLVECOMMAND,
                                              InformationType::INFORMATION,
                                              pMemblock,
                                              pCurrentUserId,
                                              nullptr,
...
mystd::unique_propagate_const<UniqueSemanticExpression> resolveCommand(const SemanticExpression& pSemExp,
                                                                       const SemanticMemory& pSemanticMemory,
                                                                       const linguistics::LinguisticDatabase& pLingDb) {
    return resolveCommandFromMemBlock(pSemExp, pSemanticMemory.memBloc, pSemanticMemory.getCurrUserId(), pLingDb);
}
...
mystd::unique_propagate_const<UniqueSemanticExpression> executeBehaviorFromMemBlock(
    ...
    controller::applyOperatorOnSemExpConstMem(compSemAnswers,
                                              *clonedSemExp,
                                              SemanticOperatorEnum::EXECUTEBEHAVIOR,
                                              InformationType::INFORMATION,
                                              pMemblock,
                                              pCurrentUserId,
                                              nullptr,
...
mystd::unique_propagate_const<UniqueSemanticExpression> executeBehavior(
    ...
    const SemanticMemory& pSemanticMemory,
    const linguistics::LinguisticDatabase& pLingDb) {
    return executeBehaviorFromMemBlock(pSemExp, pSemanticMemory.memBloc, pSemanticMemory.getCurrUserId(), pLingDb);
}
...
mystd::unique_propagate_const<UniqueSemanticExpression> executeFromCondition(
    ...
    controller::applyOperatorOnSemExpConstMem(compSemAnswers,
                                              *clonedSemExp,
                                              SemanticOperatorEnum::EXECUTEFROMCONDITION,
                                              InformationType::INFORMATION,
                                              pSemanticMemory.memBloc,
                                              pSemanticMemory.getCurrUserId(),
                                              nullptr,
...
mystd::unique_propagate_const<UniqueSemanticExpression> externalRequester(
    ...
    return privateImplem::externalTeachingRequester(pSemExp, memBloc, pLingDb, pSemanticMemory.getCurrUserId());
}
...
void pingTime(mystd::unique_propagate_const<UniqueSemanticExpression>& pReaction,
              ...
              const linguistics::LinguisticDatabase& pLingDb) {
    std::unique_ptr<CompositeSemAnswer> compSemAnswers;
    controller::notifyCurrentTime(compSemAnswers, pSemanticMemory, nullptr, pNowTimeDuration, pLingDb);
...
std::shared_ptr<ExpressionWithLinks> teach(mystd::unique_propagate_const<UniqueSemanticExpression>& pReaction,
    ...
    controller::applyOperatorOnExpHandleInMemory(
        compSemAnswers, expForMemRef, opEnum, informationType, pSemanticMemory, nullptr, pLingDb, nullptr);
...
std::shared_ptr<ExpressionWithLinks> teachSplitted(mystd::unique_propagate_const<UniqueSemanticExpression>& pReaction,
    ...
    return teach(pReaction, pSemanticMemory, std::move(teachSemExp), pLingDb, pActionOperator);
}
