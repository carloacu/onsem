...
void _tryAddAgentNames(UserInfosContainer& pUserInfosContainer,
...
void _tryToAddUserInfosOnSemExp(UserInfosContainer& pUserInfosContainer,
    ...
    switch (pGrdExp->type) {
        case SemanticGroundingType::STATEMENT: {
            ...
            if (pRootVerbTense == SemanticVerbTense::PRESENT) {
                ...
                if (SemExpGetter::getSubjectAndObjectUserIdsOfNameAssignement(
        ...
        case SemanticGroundingType::AGENT: {
            ...
            if (agentGrd.nameInfos)
                _tryAddAgentNames(pUserInfosContainer,
                                  agentGrd.userId,
                                  agentGrd.nameInfos->names,
                                  agentGrd.nameInfos->possibleGenders);
                ...
                for (const auto& currSpecGrdExp : specGrdExps) {
                    ...
                    if (statGrdPtr != nullptr
                        && ConceptSet::haveAnyOfConcepts(
                            statGrdPtr->concepts, {"verb_action_say", "predicate_hasName", "predicate_hasName_call"})) {
                        auto itCalledSubject = currSpecGrdExp->children.find(GrammaticalType::SUBJECT);
                        if (itCalledSubject != currSpecGrdExp->children.end()
                            && SemExpGetter::isACoreference(*itCalledSubject->second,
                            ...
                            if (itCalledObject != currSpecGrdExp->children.end()) {
                                ...
                                if (calledObjectAgentPtr != nullptr)
                                    pUserInfosContainer.addEquivalentUserIds(agentGrd.userId,
                                                                             calledObjectAgentPtr->userId);
                            }
                        }
                    }
                }
...
void tryToAddUserInfos(UserInfosContainer& pUserInfosContainer, const GroundedExpression& pGrdExp) {
