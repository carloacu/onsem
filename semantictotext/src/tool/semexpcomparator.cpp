...
{
  return pGenGrd.entityType == SemanticEntityType::HUMAN &&
      pGenGrd.referenceType == SemanticReferenceType::INDEFINITE &&
                                            ...
                                            const ComparisonExceptions* pExceptionsPtr)
{
  ...
  if (!doesMeaningDiffers)
  {
    ...
    doesMeaningDiffers = (!areCptsEqual && !pGenGrd1.word.isEmpty() && !pGenGrd2.word.isEmpty() && !_wordsAreEqual(pGenGrd1.word, pGenGrd2.word, pLingDb)) ||
  ...
  case SemanticGroundingType::TIME:
  {
    ...
    if (timeGrd2Ptr != nullptr)
    {
      ...
      if (!_modifyImbricationFromOptInt(res, timeGrd1.date.year, timeGrd2.date.year) ||
          ...
          timeGrd1.length != timeGrd2.length ||
          timeGrd1.timeOfDay != timeGrd2.timeOfDay)
        return ImbricationType::DIFFERS;
      return res;
    }
    else
...
ImbricationType _mergeChildImbrications(ImbricationType pChildImbrication1,
                                        ImbricationType pChildImbrication2)
{
  ...
  if (pChildImbrication2 == ImbricationType::EQUALS)
    return pChildImbrication1;
  if (pChildImbrication1 == ImbricationType::OPPOSES ||
      pChildImbrication2 == ImbricationType::OPPOSES)
    return ImbricationType::DIFFERS;
  if (pChildImbrication1 == ImbricationType::CONTAINS &&
      pChildImbrication2 != ImbricationType::ISCONTAINED)
    return pChildImbrication2;
  if (pChildImbrication2 == ImbricationType::CONTAINS &&
      pChildImbrication1 != ImbricationType::ISCONTAINED)
    return pChildImbrication1;
  if (pChildImbrication1 == ImbricationType::ISCONTAINED &&
      pChildImbrication2 != ImbricationType::CONTAINS)
    return pChildImbrication2;
  if (pChildImbrication2 == ImbricationType::ISCONTAINED &&
...
ImbricationType switchOrderOfEltsImbrication(ImbricationType pImbrication)
{
  ...
  case ImbricationType::OPPOSES:
    return pImbrication;
  case ImbricationType::CONTAINS:
    return ImbricationType::ISCONTAINED;
  case ImbricationType::ISCONTAINED:
    return ImbricationType::CONTAINS;
  case ImbricationType::MORE_DETAILED:
    return ImbricationType::LESS_DETAILED;
  case ImbricationType::LESS_DETAILED:
    return ImbricationType::MORE_DETAILED;
  case ImbricationType::HYPONYM:
    return ImbricationType::HYPERNYM;
  case ImbricationType::HYPERNYM:
    return ImbricationType::HYPONYM;
  }
  return pImbrication;
}
...
bool grdHaveNbSetToZero(const SemanticGrounding& pGrd)
{
  ...
  return genGrd != nullptr && genGrd->quantity.type == SemanticQuantityType::NUMBER &&
      genGrd->quantity.nb == 0;
}
...
bool doesGrdExpContainEverything(const GroundedExpression& pGrdExp)
{
  ...
  return genGrdPtr != nullptr &&
      ...
      (genGrdPtr->entityType == SemanticEntityType::THING ||
       genGrdPtr->entityType == SemanticEntityType::UNKNOWN);
}
                                         ...
                                         const ConceptSet& pConceptSet)
{
  ...
  auto grd1EquTogr2ExeptForQuantity = [](const SemanticGrounding& pGrd1,
      const SemanticGrounding& pGrd2)
  {
    ...
    if (genGrd1Ptr != nullptr)
    {
      ...
      if (genGrd1Ptr->entityType == SemanticEntityType::HUMAN)
        return pGrd2.getAgentGroundingPtr() != nullptr ||
            pGrd2.getNameGroundingPtr() != nullptr;
    }
    return false;
  };
  return grd1EquTogr2ExeptForQuantity(grd1, grd2) ||
      grd1EquTogr2ExeptForQuantity(grd2, grd1);
}
                                       ...
                                       GrammaticalType pParentGrammaticalType)
{
  ...
  if (_isExcluded(pGrdExp1, pGrdExp2, pMemBlock, pLingDb) ||
  ...
  auto tryToAddSemExp = [&](std::map<GrammaticalType, ListExpPtr>& pChildrenForAGramType,
                            ...
                            bool pFirstOrSecondArg)
  {
    if (pItChild->first == GrammaticalType::INTRODUCTING_WORD ||
        pItChild->first == GrammaticalType::SUB_CONCEPT ||
