...
ConditionForAUser::ConditionForAUser(const std::string& pUser, std::unique_ptr<SemanticExpression> pThenSemExp)
    : user(pUser)
...
LeafSemAnswer::LeafSemAnswer(ContextualAnnotation pType, UniqueSemanticExpression pReaction)
    : SemAnswer()
    , type(pType)
...
void CompositeSemAnswer::getGrdExps(std::list<const GroundedExpression*>& pGrdExps,
...
TruenessValue CompositeSemAnswer::getAgreementValue() const {
    ...
    for (const auto& currAnswer : leafAnsw.answerElts) {
        ...
        if (!answElts.answersGenerated.empty())
            return semExpAgreementDetector::semExpToAgreementValue(*answElts.answersGenerated.front().genSemExp);
    }
