...
void LingdbWordForms::xInit(LingdbMeaning* pMeaning) {
    fMeaning = pMeaning;
    fFlexions = nullptr;
    fFrequency = 4;
...
void LingdbWordForms::xAddFlexions(CompositePoolAllocator& pFPAlloc,
                                   PartOfSpeech pGram,
    ...
    if (fFlexions == nullptr) {
        ...
        fFlexions->xAddNewFlexions(pFPAlloc, pGram, pFlexions, this);
    }
}

void LingdbWordForms::copyFlexions(LinguisticIntermediaryDatabase& pLingDatabase,
...
void LingdbWordForms::xDeallocate(CompositePoolAllocator& pFPAlloc) {
    if (pFPAlloc.isAllocated(fMeaning) && fMeaning->xRemoveAPtrToThisMeaning() == 0) {
