...
void LingdbSaverTraductionWriter::writeTranslations(const std::filesystem::path& pFilnename,
    ...
    while (meaning != nullptr) {
        ...
        if (outLk1 != nullptr || outLk2 != nullptr) {
            ...
            if (outLk1 != nullptr) {
                xGetTradsWithConfidence(tradWithConfidences, meaning->getPartOfSpeech(), *outLk1);
                if (outLk2 != nullptr) {
                    std::set<MeaningAndConfidence> tradWithConfidences2;
                    xGetTradsWithConfidence(tradWithConfidences2, meaning->getPartOfSpeech(), *outLk2);
                    xMerge2Confidences_withSecondConfLessImportant(tradWithConfidences, tradWithConfidences2);
                }
            } else {
                xGetTradsWithConfidence(tradWithConfidences, meaning->getPartOfSpeech(), *outLk2);
            }
    ...
    std::cout << "before add words: nbMeaningsWithTrad / nbMeanings: " << nbMeaningsWithTrad << " / " << nbMeanings
    ...
    while (currNode != nullptr) {
        ...
        while (wfs != nullptr) {
            std::map<const LingdbMeaning*, std::set<MeaningAndConfidence> >::const_iterator itTrad =
                allTrads.find(wfs->elt->getMeaning());
    ...
    std::cout << "after add words: nbMeaningsWithTrad / nbMeanings: " << nbMeaningsWithTrad << " / " << nbMeanings
              << " = " << static_cast<float>(nbMeaningsWithTrad) / static_cast<float>(nbMeanings) << std::endl;
...
void LingdbSaverTraductionWriter::writeTraductionsForOneWiki(
    ...
    for (std::map<LingdbMeaning*, LingdbSaverOutLinks>::const_iterator itTrad = pTrads.begin(); itTrad != pTrads.end();
         ++itTrad) {
        std::set<MeaningAndConfidence> tradWithConfidences;
        xGetTradsWithConfidence(tradWithConfidences, itTrad->first->getPartOfSpeech(), itTrad->second);
...
void LingdbSaverTraductionWriter::writeTraductionsForOneWiki_fromReverseTraductions(
    ...
    for (std::map<LingdbMeaning*, LingdbSaverOutLinks>::const_iterator itTrad = pTrads.begin(); itTrad != pTrads.end();
        ...
        for (std::set<LingdbMeaning*>::const_iterator itLWM = itTrad->second.linkedWithInMeaningGram.begin();
             itLWM != itTrad->second.linkedWithInMeaningGram.end();
             ++itLWM) {
        ...
        for (std::set<LingdbMeaning*>::const_iterator itNLWM = itTrad->second.notLinkedWithInMeaningGram.begin();
             itNLWM != itTrad->second.notLinkedWithInMeaningGram.end();
             ++itNLWM) {
...
void LingdbSaverTraductionWriter::writeSummaryTraductions(
    ...
    for (std::map<LingdbMeaning*, std::set<MeaningAndConfidence> >::const_iterator itTrad = pTrads.begin();
         itTrad != pTrads.end();
         ++itTrad) {
        xWriteMeaning(outfile, itTrad->first);
        outfile << ":";
...
void LingdbSaverTraductionWriter::xMerge2Confidences_withSecondConfLessImportant(
    ...
    for (std::set<MeaningAndConfidence>::const_iterator itTrad2 = pTradWithConfidences2.begin();
         itTrad2 != pTradWithConfidences2.end();
         ++itTrad2) {
        std::set<MeaningAndConfidence>::const_iterator itTrad1 =
            xGetMeaningOfTradWC(pTradWithConfidences, itTrad2->meaning);
...
void LingdbSaverTraductionWriter::xGetTradsWithConfidence(std::set<MeaningAndConfidence>& pTradWithConfidences,
    ...
    for (std::set<LingdbMeaning*>::const_iterator itLWM = pOutLinks.linkedWithInMeaningGram.begin();
         itLWM != pOutLinks.linkedWithInMeaningGram.end();
         ++itLWM) {
        pTradWithConfidences.insert(
            MeaningAndConfidence(*itLWM, (*itLWM)->getPartOfSpeech() == pInPartOfSpeech ? 5 : 4));
    }

    for (std::set<LingdbMeaning*>::const_iterator itNLWM = pOutLinks.notLinkedWithInMeaningGram.begin();
         itNLWM != pOutLinks.notLinkedWithInMeaningGram.end();
         ++itNLWM) {
        if (xGetMeaningOfTradWC(pTradWithConfidences, *itNLWM) == pTradWithConfidences.end()) {
            pTradWithConfidences.insert(
                MeaningAndConfidence(*itNLWM, (*itNLWM)->getPartOfSpeech() == pInPartOfSpeech ? 3 : 2));
        }
    }
}
...
void LingdbSaverTraductionWriter::xWriteTrads(std::ofstream& pOutfile,
    ...
    for (std::set<MeaningAndConfidence>::const_iterator itTrad = pTradWithConfidences.begin();
         itTrad != pTradWithConfidences.end();
         ++itTrad) {
        xWriteMeaningWithConfidence(pOutfile, itTrad->meaning, pSep, itTrad->confidence);
    }
}

void LingdbSaverTraductionWriter::xWriteMeaning(std::ofstream& pOutfile, const LingdbMeaning* pMeaning) const {
    pOutfile << "<";
    xWriteStr(pOutfile, pMeaning->getLemma()->getWord());
    pOutfile << "," << partOfSpeech_toStr(pMeaning->getPartOfSpeech()) << ">";
}

void LingdbSaverTraductionWriter::xWriteMeaningWithConfidence(std::ofstream& pOutfile,
                                                              const LingdbMeaning* pMeaning,
                                                              const std::string& pSep,
                                                              char pConfidence) const {
    pOutfile << "<";
    xWriteStr(pOutfile, pMeaning->getLemma()->getWord());
    pOutfile << "," << partOfSpeech_toStr(pMeaning->getPartOfSpeech()) << pSep << static_cast<int>(pConfidence) << ">";
}

void LingdbSaverTraductionWriter::xWriteStr(std::ofstream& pOutfile, const std::string& pStr) const {
