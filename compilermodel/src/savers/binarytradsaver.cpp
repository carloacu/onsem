...
void BinaryTradSaver::save(
    ...
    for (std::list<WlksDatabaseLoader::WlksDatabaseLoader_TradSpec>::const_iterator it = pTrads.tradSpecs.begin();
         it != pTrads.tradSpecs.end();
         ++it) {
        ...
        for (std::map<LingdbMeaning*, std::set<MeaningAndConfidence> >::const_iterator itWords =
                 it->traductions.begin();
             itWords != it->traductions.end();
             ++itWords) {
            std::string currWord = itWords->first->getLemma()->getWord();
            treeOfWords.addWord(currWord, currWord, itWords->first->getPartOfSpeech(), std::vector<std::string>(), 4);
        }

        TreeTradWorkState treeTradWS(it->traductions, *it->inLingDb.lingDatabase, inMeaningsPtr, outMeaningsPtr);
        binarymasks::Ptr endMemory =
            xCreateRootNode(treeTradWS, treeOfWords.getFPAlloc().first<LingdbDynamicTrieNode>(), mem, mem);
        std::size_t sizeMemory = endMemory.val - mem.val;

        std::string twoLangsOfTheTrad = it->inLingDb.lingDatabase->getLanguage()->toStr() + "_to_"
                                      + it->outLingDb.lingDatabase->getLanguage()->toStr();
...
const std::map<const LingdbMeaning*, int>& BinaryTradSaver::xGetMeaningsPtrOfALang(
    ...
    auto itMeaningPtr =
        pLangToMeaningsPtr.find(semanticLanguageTypeGroundingEnumFromStr(pLingDb.getLanguage()->toStr()));
...
binarymasks::Ptr BinaryTradSaver::TreeTradWorkState::printEndOfANode(LingdbDynamicTrieNode* pNode,
                                                                     int,
    ...
    while (wfl != nullptr) {
        LingdbMeaning* inMeaning =
            lingDb.getMeaning(wfl->elt->getMeaning()->getLemma()->getWord(), wfl->elt->getMeaning()->getPartOfSpeech());
        ...
        std::map<LingdbMeaning*, std::set<MeaningAndConfidence> >::const_iterator itTradMeaning =
            meaningsToTrads.find(inMeaning);
        if (itTradMeaning == meaningsToTrads.end() || itTradMeaning->second.empty()) {
