 ...
 const boost::filesystem::path& pFilename) const
{
  ...
  endMemory = xCreateRootNode(treeConceptsWS,
                              conceptWords.getFPAlloc().first<LingdbDynamicTrieNode>(),
                              endMemory, endMemory);
...
binarymasks::Ptr BinaryDatabaseConceptsSaver::TreeConceptsWorkState::printEndOfANode
 ...
 binarymasks::Ptr pEndMemory)
{
  ...
  const ForwardPtrList<LingdbConcept>* oppCpts =
  ...
  const ForwardPtrList<LingdbConcept>* equCpts =
