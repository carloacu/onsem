...
void BinaryDatabaseConceptsSaver::saveConceptsDb(std::map<std::string, ConceptsBinMem>& pConceptsOffsets,
    ...
    for (const auto& currCpt : pCptStrToCptStruct)
        conceptWords.addWord(currCpt.first, currCpt.first, PartOfSpeech::NOUN, std::vector<std::string>(), 4);
    ...
    endMemory =
        xCreateRootNode(treeConceptsWS, conceptWords.getFPAlloc().first<LingdbDynamicTrieNode>(), endMemory, endMemory);

    auto writeConceptsPtrs =
        [this, &treeConceptsWS](const std::map<std::string, std::list<binarymasks::Ptr>>& pCptStrToCpts) {
...
binarymasks::Ptr BinaryDatabaseConceptsSaver::TreeConceptsWorkState::printEndOfANode(LingdbDynamicTrieNode* pNode,
                                                                                     int pDecNode,
    ...
    const LingdbConcept* cptStruct = itToStruct->second;

    // write number of opposite concepts
    const ForwardPtrList<LingdbConcept>* oppCpts = cptStruct->getOppositeConcepts();
    if (oppCpts != nullptr)
    ...
    const ForwardPtrList<LingdbConcept>* equCpts = cptStruct->getNearlyEqualConcepts();
    if (equCpts != nullptr)
