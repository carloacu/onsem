 ...
 const LinguisticIntermediaryDatabase& pLingDatabase) const
{
  ...
  endMemory = xCreateRootNode(treeCrWorkState, alloc.first<LingdbDynamicTrieNode>(),
                              endMemory, endMemory);
 ...
 const std::map<LingdbDynamicTrieNode const*, int>& pNodesPtr) const
{
  for (const auto& currMeaning : pMeaningsPtr)
  {
    ...
    std::map<LingdbDynamicTrieNode const*, int>::const_iterator
...
LingdbDynamicTrieNode* BinaryDatabaseDicoSaver::xGetMeaningLemma
 ...
 SemanticLanguageEnum pLangType) const
{
  ...
  {
    LingdbMultiMeaningsNode* multiNodeInfos =
        currMeaningPtr.first->getLemma()->getDatasIfItsAMultiMeaningNode();
    const LingdbMeaning* rootMeaning = multiNodeInfos == nullptr ?
...
(binarymasks::Ptr& pBeginMeaningsFromConcepts,
 const std::map<const LingdbConcept*,
...
binarymasks::Ptr BinaryDatabaseDicoSaver::xWriteMeanings
...
binarymasks::Ptr BinaryDatabaseDicoSaver::xWriteVerbConj
...
binarymasks::Ptr BinaryDatabaseDicoSaver::xWriteNodeRef
(binarymasks::Ptr pEndMemory,
 const std::map<LingdbDynamicTrieNode const*, int>& pNodesPtr,
...
binarymasks::Ptr BinaryDatabaseDicoSaver::TreeCreationWorkState::printEndOfANode
 ...
 binarymasks::Ptr pEndMemory)
{
  ...
  for (const auto& wfOrMeaning : wordFromsOrMeanings)
  {
    ...
    LingdbMeaning* currMeaning = (wfOrMeaning.second != nullptr || wordForm == nullptr) ?
...
void BinaryDatabaseDicoSaver::xWriteNbWithSpaces
