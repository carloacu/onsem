...
public:
    const LingdbConcept* getConcept() const { return fConcept; }

    char getRelatedToConcept() const { return fRelatedToConcept; }

    void setConcept(const LingdbConcept* pConcept) { fConcept = pConcept; }

    void setRelatedToConcept(char pRelatedToConcept) { fRelatedToConcept = pRelatedToConcept; }

    bool operator<(const LingdbLinkToAConcept& pOther) const;
...
private:
    ...
    void xInit(const LingdbConcept* pConcept, char pRelatedToConcept);
};
