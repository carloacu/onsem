...
void LingdbLinkToAConcept::xInit(const LingdbConcept* pConcept, char pRelatedToConcept) {
    fConcept = pConcept;
    fRelatedToConcept = pRelatedToConcept;
}

bool LingdbLinkToAConcept::operator<(const LingdbLinkToAConcept& pOther) const {
...
void LingdbLinkToAConcept::xGetPointers(std::vector<const void*>& pRes, void* pVar) {
    pRes.emplace_back(&reinterpret_cast<LingdbLinkToAConcept*>(pVar)->fConcept);
}
