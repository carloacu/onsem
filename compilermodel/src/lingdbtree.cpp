...
void _printAllSubPaths(std::ostream& pFStream, const std::string& pRelativePath, const std::string& pFolderPath) {
...
bool LingdbTree::xExtractVersionFromFile(std::string& pFormalismVersion,
                                         std::string& pVersion,
    ...
    getline(versionFile, line);
    {
        ...
        pFormalismVersion = line.substr(formalismVersionLabel.size(), line.size() - formalismVersionLabel.size());
    }
    ...
    getline(versionFile, line);
    {
        ...
        pVersion = line.substr(versionLabel.size(), line.size() - versionLabel.size());
    }
    versionFile.close();
    return true;
}
...
void LingdbTree::update(const std::string& pSdkShareDir,
    ...
    if (!xExtractVersionFromFile(inputDbFormalismVersion, inputDbVersion, inputVersionFilePath)) {
    ...
    bool needToUpdateAllStatBinDico = false;
    {
        ...
        xExtractVersionFromFile(installedDbFormalismVersion, installedDbVersion, installedVersionFilePath);
    ...
    if (needToUpdateAllStatBinDico) {
        ...
        anyLoader.open(pLoadDatabasesDir + "/reloadall.rla", fInputResourcesDir, currLingdb, *this);
        ...
        for (std::size_t i = 0; i < fLanguages.size(); ++i) {
            ...
            std::string lang = semanticLanguageEnum_toLanguageFilenameStr(fLanguages[i].langGroundingsType);
            binDicoSaver.save(
                langToMeaningsPtr[fLanguages[i].langGroundingsType],
                conceptsOffsets,
                linguisticDatabasesPath + "/" + xGetStatDatabaseFilename(lang),
...

            {
                ...
                while (itTreeConvsFolders != endit) {
                    if (std::filesystem::is_directory(itTreeConvsFolders->path())) {
                        ...
                        SemanticLanguageEnum langEnum = semanticLanguageTypeGroundingEnumFromStr(filename);
...
bool LingdbTree::xTryToLoadDynamicDbs() {
    for (std::size_t i = 0; i < fLanguages.size(); ++i) {
        ...
        } catch (const std::exception&) { fLanguages[i].dynDbVersion = -1; }
...
void LingdbTree::xGenerateTranslations(
    ...
    for (std::size_t i = 0; i < fLanguages.size(); ++i) {
        ...
        if (langType == SemanticLanguageEnum::UNKNOWN || fLanguages[i].dynDbVersion == -1) {
            continue;
        }

        std::string langStr = semanticLanguageEnum_toLegacyStr(langType);
        WlksDatabaseLoader::WlksDatabaseLoader_LangSpec& langSpec = wlksWorkState.strToLangSpecs[langStr];
...
void LingdbTree::xClearDirectory(const std::string& pDirectory) const {
    std::filesystem::path pathToRemove(pDirectory);
    for (std::filesystem::directory_iterator end_dir_it, itFilesToRemove(pathToRemove); itFilesToRemove != end_dir_it;
         ++itFilesToRemove) {
        std::filesystem::remove_all(itFilesToRemove->path());
    }
}

std::string LingdbTree::xGetDynDatabaseFilename(const std::string& pLanguage) const {
    return pLanguage + "." + getExtDynDatabase();
}

std::string LingdbTree::xGetStatDatabaseFilename(const std::string& pLanguage) const {
    return pLanguage + "database." + getExtBinaryDatabase();
}

void LingdbTree::getHoldingFolder(std::string& pFolder, const std::filesystem::path& pFilename) const {
    std::error_code ec;
...
void LingdbTree::xCreateDirectory(const std::string& pPath) const {
    std::filesystem::create_directory(pPath);
}

void LingdbTree::xRemoveDirectory(const std::filesystem::path& pPath) const {
    std::filesystem::remove_all(pPath);
}

void LingdbTree::xCopyFile(const std::string& pFrom, const std::string& pTo) const {
    /*
