...
signed char* LingdbQuestionWords::exportToBin(signed char* pEndMemory,
    ...
    *(pEndMemory++) = fQWords->length();
    ...
    while (qws != nullptr) {
        LingdbMeaning* meaning =
            pLingDatabase.getMeaning(qws->elt->word->toStr(), static_cast<PartOfSpeech>(qws->elt->gram));
        auto itMeaning = pMeaningsPtr.find(meaning);
        if (itMeaning == pMeaningsPtr.end()) {
            throw std::runtime_error("word: " + qws->elt->word->toStr()
                                     + ", with gram: " + partOfSpeech_toStr(static_cast<PartOfSpeech>(qws->elt->gram))
...
void LingdbQuestionWords::xClear(CompositePoolAllocator& pAlloc) {
    if (fQWords != nullptr) {
...
void LingdbQuestionWords::xDeallocate(CompositePoolAllocator& pAlloc) {
    xClear(pAlloc);
