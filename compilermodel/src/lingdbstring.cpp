...
void LingdbString::xInit(CompositePoolAllocator& pFPAlloc, const std::string& pTagName) {
...
void LingdbString::xDeallocate(CompositePoolAllocator& pFPAlloc) {
    pFPAlloc.deallocate<char>(fCharacters, fNbCharacters);
    pFPAlloc.deallocate<LingdbString>(this);
}

std::string LingdbString::toStr() const {
    return std::string(fCharacters, fNbCharacters);
}

unsigned char LingdbString::length() const {
    return fNbCharacters;
}

char* LingdbString::toCStr() const {
    return fCharacters;
}

void LingdbString::xGetPointers(std::vector<const void*>& pRes, void* pVar) {
    pRes.emplace_back(&reinterpret_cast<LingdbString*>(pVar)->fCharacters);
}
