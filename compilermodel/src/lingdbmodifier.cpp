...
std::size_t LingdbModifier::findQueAddQu(LinguisticIntermediaryDatabase& pLingDatabase) const {
    ...
    while (currNode != nullptr) {
        ...
        if (xEndWithQue(word)) {
            ...
            while (wf != nullptr) {
                pLingDatabase.addWordWithSpecificMeaning(newWord,
                                                         *wf->elt->getMeaning(),
                                                         std::vector<std::string>(),
                                                         4);    // TODO: put same flexion infos that the original word
                wf = wf->next;
            }
...
void LingdbModifier::associateANewGramForAMeaning(LinguisticIntermediaryDatabase& pLingDatabase,
...
std::size_t LingdbModifier::addToAtBeginOfVerbsForEnglish(LinguisticIntermediaryDatabase& pLingDatabase) const {
    ...
    while (currNode != nullptr) {
        ...
        while (wf != nullptr) {
            if (wf->elt->getMeaning()->getPartOfSpeech() == PartOfSpeech::VERB) {
                ...
                if (flexions != nullptr && flexions->replaceInfinitiveByImperative()) {
    ...
    for (std::list<LingdbWordForms*>::iterator it = imperativeVerbs.begin(); it != imperativeVerbs.end(); ++it) {
...
std::size_t LingdbModifier::delExprs(LinguisticIntermediaryDatabase& pLingDatabase) const {
...
std::size_t LingdbModifier::delAllWords(LinguisticIntermediaryDatabase& pWords) const {
    std::size_t nbDeletedWords = 0;
...
bool LingdbModifier::xEndWithQue(const std::string& pWord) const {
    if (pWord.size() < 4) {
        return false;
    }
...
bool LingdbModifier::xEndWithQuApostrophe(const std::string& pWord) const {
    if (pWord.size() < 4) {
        return false;
    }
...
LingdbWordForms* LingdbModifier::xGetWordFormThatAsASpecificMeaning(const ForwardPtrList<LingdbWordForms>* pWf,
