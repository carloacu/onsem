...
void LingdbMeaning::getAllContextInfos(std::list<char>& pAllContextInfos) const {
...
void LingdbMeaning::getLinkToConceptsStr(std::list<std::string>& pConceptsStr) const {
...
ForwardPtrList<char>* LingdbMeaning::pushBackContextInfo(LinguisticIntermediaryDatabase& pLingDatabase,
...
void LingdbMeaning::removeContextInfo(LinguisticIntermediaryDatabase& pLingDatabase, WordContextualInfos pContextInfo) {
    if (fContextInfos != nullptr) {
        char contextInfoChar = static_cast<char>(pContextInfo);
...
ForwardPtrList<char>* LingdbMeaning::xPushBackContextInfo(LinguisticIntermediaryDatabase& pLingDatabase,
...
void LingdbMeaning::addLinkToConcept(LinguisticIntermediaryDatabase& pLingDatabase,
        ...
        while (existingLinkToConcepts != nullptr) {
            ...
            if (needToReplace) {
                ...
                if (prevConcept == nullptr)
                    ForwardPtrList<LingdbLinkToAConcept>::clearNextElt(fLinkToConcepts, typeAlloc);
                else
                    ForwardPtrList<LingdbLinkToAConcept>::clearNextElt(prevConcept->next, typeAlloc);
            }
...
bool LingdbMeaning::xIsSecondConceptMorePrecise(const std::string& pConceptName1,
...
void LingdbMeaning::xDeallocate(CompositePoolAllocator& pFPAlloc) {
