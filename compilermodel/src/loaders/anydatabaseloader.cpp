...
void AnyDatabaseLoader::open(const std::string& pFilename,
    ...
    if (suffixStr == pLingdbTree.getExtDynDatabase()) {
        ...
        pCurrLingdb.clear();
        RlaDatabaseLoader::merge(pFilename, pInputResourcesDir, pCurrLingdb, pLingdbTree, *this, pImbricationLevel);
...
void AnyDatabaseLoader::mergeWith(const std::string& pFilename,
                                  const std::string& pInputResourcesDir,
                                  LinguisticIntermediaryDatabase& pCurrLingdb,
                                  const LingdbTree& pLingdbTree,
                                  std::size_t pImbricationLevel) const {
...
void AnyDatabaseLoader::exec(const std::string& pFilename, const LingdbTree& pLingdbTree) const {
    std::string suffixStr = mystd::filenameToSuffix(pFilename);
