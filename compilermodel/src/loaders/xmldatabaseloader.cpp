...
int XmlDatabaseLoader::getVersion(const std::string& pFilename) {
...
void XmlDatabaseLoader::merge(const std::string& pFilename,
                              LinguisticIntermediaryDatabase& pLingDatabase,
                              const LingdbTree& pLingdbTree) {
    boost::property_tree::ptree tree;
    boost::property_tree::read_xml(pFilename, tree);
    ...
    childLoop(tree, currSubTree, "linguisticdatabase") {
        ...
        if (currSubTree.first == "word") {
            ...
            if (concept != nullptr) {
                for (const auto& currCptTree : subTree) {
                    ...
                    if (currCptTree.first == "linkedMeaning") {
                        char relatedToConcept =
                            mystd::lexical_cast<char>(cptAttrs.get<std::string>("relationToConcept", "0"));
                        if (inputGramStr == "*") {
                            LingdbDynamicTrieNode* pointerEndOfWord = pLingDatabase.getPointerToEndOfWord(lemma);
                            if (pointerEndOfWord == nullptr) {
                                std::cerr << "concept: add a new word with unknown grammatical type: " << lemma
                                          << std::endl;
                                pLingDatabase.addWord(
                                    lemma, lemma, PartOfSpeech::UNKNOWN, std::vector<std::string>(), 4);
                                pointerEndOfWord = pLingDatabase.getPointerToEndOfWord(lemma);
                            }
                            if (pointerEndOfWord != nullptr) {
                                const ForwardPtrList<LingdbMeaning>* itMeanings =
                ...
                throw std::runtime_error("concept: \"" + conceptName + "\" is not declared " + "(in file: \""
                                         + pFilename + "\")");
            }
            ...
            if (cptPtr1 == nullptr) {
                std::cerr << "the concept: \"" << conceptName1
                          << "\" doesn't exist (from opposite of concepts declaration)." << std::endl;
            } else {
                std::string conceptName2 = subTreeAttrs.get<std::string>("concept2", "");
                LingdbConcept* cptPtr2 = pLingDatabase.getConcept(conceptName2);
                if (cptPtr2 == nullptr) {
                    std::cerr << "the concept: \"" << conceptName2
                              << "\" doesn't exist (from opposite of concepts declaration)." << std::endl;
                } else {
            ...
            if (cptPtr1 == nullptr) {
                std::cerr << "the concept: \"" << conceptName1
                          << "\" doesn't exist (from nearly equal concepts declaration)." << std::endl;
            } else {
                std::string conceptName2 = subTreeAttrs.get<std::string>("concept2", "");
                LingdbConcept* cptPtr2 = pLingDatabase.getConcept(conceptName2);
                if (cptPtr2 == nullptr) {
            ...
            if (meaningPtr == nullptr) {
                std::cerr << "Meaning not found: lemma: \"" << lemme << "\" gram: \"" << partOfSpeechStr
                          << "\" (in file: \"" << pFilename << "\")." << std::endl;
            } else {
                for (const auto& currMeaningTree : subTree) {
                    ...
                    if (currMeaningTree.first == "contextInfo") {
                        ...
                        if (contInfo == WordContextualInfos::ENDOFENUM)
                            throw std::runtime_error(contInfoStr
                                                     + " is not a valid meaning contextual info (in file: \""
                                                     + pFilename + "\")");
                        ...
                        throw std::runtime_error("\"" + currMeaningTree.first
                                                 + "\" is an unknown child of \"existingMeaning\" (in file: \""
                                                 + pFilename + "\")");
                    }
                }
            }
            ...
            throw std::runtime_error("Unknown tag name: \"" + currSubTree.first + "\" (in file: \"" + pFilename
                                     + "\")");
        }
    }
}

LingdbMeaning* XmlDatabaseLoader::xExtractMeaning(const boost::property_tree::ptree& pTree,
    ...
    LingdbMeaning* meaningPtr = pLingDatabase.getMeaning(lemma, lemmaPartOfSpeech);
    if (meaningPtr == nullptr)
        std::cerr << "Submeaning not exist, lemma: \"" << lemma << "\", gram: \"" << partOfSpeechStr << "\""
                  << std::endl;
    return meaningPtr;
}
...
void XmlDatabaseLoader::xSplit(std::vector<std::string>& pItems, const std::string& pStr, char pDelim) {
