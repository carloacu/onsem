        ...
        if (enLangSpecPtr != nullptr)
        {
          ...
          while (meaningPtr != nullptr)
          {
                ...
                !partOfSpeech_isPronominal(meaningPartOfSpeech))
            {
              ...
              {
                if (!LingdbConcept::conceptNameFinishWithAStar(itConcepts->elt->getConcept()->getName()->toStr()))
                {
              ...
              if (!isLinkToASpecificConcept)
              {
                ...
                LingdbConcept* meaningConcept = enLingDatabase.addConcept(cptInserted, meaningConceptStr, true);
                meaningPtr->addLinkToConcept(enLingDatabase, meaningConcept, meaningConceptStr, 3, false);
                for (auto& currTrad : tradSpecsOfEnglish)
                {
                  LingdbConcept* outConcept = currTrad->outLingDb.lingDatabase->addConcept(cptInserted, meaningConceptStr, true);
...
void WlksDatabaseLoader::xLoadDynNewDb
(WlksDatabaseLoader_WorkState& pWorkState,
 const std::string& pLang) const
{
  std::map<std::string, WlksDatabaseLoader_LangSpec>::iterator
...
void WlksDatabaseLoader::xGetNextMeaningInLine
(LingdbMeaning** pMeaning,
 char& pConfidence,
 std::size_t& pCurrPos,
...
bool WlksDatabaseLoader::xHasSlash
...
void WlksDatabaseLoader::xRemoveSlashesBeforeSpecifcChars
...
std::list<WlksDatabaseLoader::WlksDatabaseLoader_TradSpec*> WlksDatabaseLoader::WlksDatabaseLoader_WorkState::getTraductionsOfALanguage(
    const std::string& pLanguage)
{
