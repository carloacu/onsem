...
void GFSDatabaseLoader::merge(const std::filesystem::path& pFilename, LinguisticIntermediaryDatabase& pLingdb) {
            ...
            while (endOfGram != std::string::npos && endOfLemma != std::string::npos) {
                ...
                if (node != nullptr) {
                    ...
                    if (gramType == PartOfSpeech::UNKNOWN) {
                        std::cerr << "Error: gram type " << line.substr(beginOfGram, endOfGram - beginOfGram)
