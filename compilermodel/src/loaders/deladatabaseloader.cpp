...
namespace {
std::vector<std::string> _splitInflectionsWithoutComma(const std::string& pInflectionalCodes) {
    auto sizeOfCode = pInflectionalCodes.size();
    if (sizeOfCode > 0) {
        if (sizeOfCode > 1 && pInflectionalCodes.compare(pInflectionalCodes.size() - 2, 2, "sp") == 0) {
...
void _fillWordsWithAConcept(LinguisticIntermediaryDatabase& pWords,
...
DelaDatabaseLoader::DelaDatabaseLoader(bool pSaveWords)
    : fSaveWords(pSaveWords)
    , fWords() {}

void DelaDatabaseLoader::simplifyDelaFile(const std::string& pInFilename,
    ...
    std::vector<std::string> beginsOfWordsToSkip = {"pour ", "cet "};
    ...
    while (getline(infile, line)) {
        ...
        if (line[0] == '#') {
            ...
            if (pLemmaToKeep.count(newWord.lemma) == 0) {
                ...
                if (removeLine || newWord.word == "à midi" || newWord.word == "la première fois"
                    || newWord.word == "première fois" || newWord.word == "la dernière fois") {
                    removeLine = true;
                } else {
                        ...
                        if (currOthInf == "Dnum") {
                            if (pRemoveDnum) {
                                ...
                                continue;
                                   ...
                                   || currOthInf == "Dind" || currOthInf == "RelQ" || currOthInf == "Pdem"
                                   || currOthInf == "Ddem" || currOthInf == "Conc" || currOthInf == "Accus"
                                   || currOthInf == "Refl" || currOthInf == "Ddéf" || currOthInf == "Dadj"
                                   || currOthInf == "aux" || currOthInf == "PCPN" || currOthInf == "Advconjs"
                                   || currOthInf == "Unit" || currOthInf == "Abst" || currOthInf == "Nomin"
                            ...
                            continue;
                                   ...
                                   && (currOthInf == "0" || currOthInf == "1" || currOthInf == "2" || currOthInf == "3"
                                       || currOthInf == "4" || currOthInf == "5" || currOthInf == "6"
                                       || currOthInf == "7" || currOthInf == "8" || currOthInf == "9"
                                       || currOthInf == "A" || currOthInf == "t" || currOthInf == "i")) {
                            continue;
                        } else {
                            removeLine = true;
                            break;
...
void DelaDatabaseLoader::toXml(const std::vector<std::string>& pInFilenames, const std::string& pOutDir) {
    ...
    auto removeInfl = [&](const std::string& pWordToRemove, PartOfSpeech pPosToRemove) {
        bool res = false;
    ...
    for (auto& currWordToDel : partOfSpeechesToDel)
        for (auto& currPosToDel : currWordToDel.second)
            outfile << " <r l=\"" << currWordToDel.first << "\" p=\"" << partOfSpeech_toStr(currPosToDel) << "\">"
                    << std::endl;
    ...
    auto _addCptFile = [&](const std::string& pFilename,
        ...
        for (const auto& currElt : pWords) {
            for (const auto& currPos : currElt.second)
                outfile << "  <linkedMeaning lemme=\"" << currElt.first << "\" gram=\"" << partOfSpeech_toStr(currPos)
                        << "\" relationToConcept=\"4\" />" << std::endl;
        }
...
void DelaDatabaseLoader::merge(const std::string& pFilename, LinguisticIntermediaryDatabase& pWords) {
    ...
    while (getline(infile, line)) {
        ...
        if (line[0] == '#') {
            ...
            if (newWord.gram == PartOfSpeech::UNKNOWN) {
                std::cerr << "In line: " << line << "\" the word has no known grammatical type" << std::endl;
                continue;
            }
            ...
            pWords.addWord(newWord.word, newWord.lemma, newWord.gram, newWord.flexions, frequency);
            if (fSaveWords)
        ...
        } else if (line[0] == '%')    // associate a new gram for the wordforms that have a specific meaning
        {
            ...
            modifier.associateANewGramForAMeaning(pWords, lemme, oldGram, newGram);
        } else if (line[0] == '>') {
            LingdbModifier modifier;
            if (line == ">delExprs") {
...
void DelaDatabaseLoader::xFillNewWordInfos(NewWordInfos& pNewWord, const std::string& pLine) const {
    // get word
    std::size_t endOfWord = xGetStringUntilAChar(pNewWord.word, 1, ',', pLine);

    // get lemme
    std::size_t endOfLemme = xGetStringUntilAChar(pNewWord.lemma, endOfWord + 1, '.', pLine);
...
std::size_t DelaDatabaseLoader::xGetStringUntilAChar(std::string& pStrResult,
...
void DelaDatabaseLoader::xReadWordInfo(NewWordInfos& pNewWord,
    ...
    if (pLine[pBeforeInfo] == '.') {
        ...
        if (pNewWord.gram != PartOfSpeech::DETERMINER && info.size() > 4 && info.compare(0, 4, "Poss") == 0) {
...
PartOfSpeech DelaDatabaseLoader::xReadGram(const std::string& pStr) const {
    if (pStr == "SPACE") {
    ...
    if (pStr == "NDET" ||    // noun + determiner
        pStr == "PCDN3" || pStr == "X" || pStr == "XI" || pStr == "GN" || pStr == "GNP" || pStr == "GNPX" ||
        // only english
