...
public:
    void save(std::map<const LingdbMeaning*, int>& pMeaningsPtr,
    ...
    void getLemme(int pMeaning) const;

private:
    struct TreeCreationWorkState : public BinaryDatabaseSaver {
        TreeCreationWorkState(std::map<LingdbDynamicTrieNode const*, int>& pNodesPtr,
        ...
        binarymasks::Ptr printEndOfANode(LingdbDynamicTrieNode* pNode, int pDecNode, binarymasks::Ptr pEndMemory);
    ...
    void xWriteNbWithSpaces(std::ofstream& pOutFile, std::size_t pNb) const;

    void xWriteLemmeOfMeanings(binarymasks::Ptr pBeginMeaning,
                               const std::map<const LingdbMeaning*, int>& pMeaningsPtr,
                               const std::map<LingdbDynamicTrieNode const*, int>& pNodesPtr) const;

    LingdbDynamicTrieNode* xGetMeaningLemma(const LingdbMeaning& pMeaning) const;

    void xWriteSynthesizerDb(std::size_t pMaxSize,
     ...
     */
    binarymasks::Ptr xAddMeanings(std::map<const LingdbMeaning*, int>& pMeaningsPtr,
    ...
    binarymasks::Ptr xAddSomeFlexions(std::map<std::string, unsigned char>& pFlexionsPtr,
                                      const CompositePoolAllocator& pFPAlloc,
                                      const binarymasks::Ptr pBeginMemory,
                                      binarymasks::Ptr pEndMemory) const;
    ...
    binarymasks::Ptr xWriteMeanings(const LingdbConcept& pConcept,
    ...
    signed char* xWriteRuleWithMeanings(signed char* pEndMemory,
                                        const std::map<const LingdbMeaning*, int>& pMeaningsPtr,
                                        const LinguisticIntermediaryDatabase& pLingDatabase,
                                        TRULE* pRule,
