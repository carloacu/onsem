...
public:
    void save(const WlksDatabaseLoader::WlksDatabaseLoader_WorkState& pTrads,
...
private:
    struct TreeTradWorkState : public BinaryDatabaseSaver {
        TreeTradWorkState(const std::map<LingdbMeaning*, std::set<MeaningAndConfidence> >& pMeaningsToTrads,
        ...
        binarymasks::Ptr printEndOfANode(LingdbDynamicTrieNode* pNode, int, binarymasks::Ptr pEndMemory);
