...
struct ConceptsBinMem {
    ...
    ConceptsBinMem(unsigned int pId, unsigned int pAlignedBegNode)
        : id(pId)
...
public:
    void saveConceptsDb(std::map<std::string, ConceptsBinMem>& pConceptsOffsets,
...
private:
    struct TreeConceptsWorkState : public BinaryDatabaseSaver {
        TreeConceptsWorkState(std::map<std::string, ConceptsBinMem>& pConceptsOffsets,
                              const std::map<std::string, LingdbConcept*>& pCptStrToCptStruct)
