...
public:
    ...
    void getHoldingFolder(std::string& pFolder, const std::filesystem::path& pFilename) const;
...
private:
    struct LanguageStruct {
        LanguageStruct(SemanticLanguageEnum pLangGroundingsType)
    ...
    bool xExtractVersionFromFile(std::string& pFormalismVersion, std::string& pVersion, const std::string& pFilePath);

    std::string xGetDynDatabaseFilename(const std::string& pLanguage) const;

    std::string xGetStatDatabaseFilename(const std::string& pLanguage) const;

    void xCreateDirectory(const std::string& pPath) const;

    void xRemoveDirectory(const std::filesystem::path& pPath) const;

    void xCopyFile(const std::string& pFrom, const std::string& pTo) const;

    void xClearDirectory(const std::string& pDirectory) const;
