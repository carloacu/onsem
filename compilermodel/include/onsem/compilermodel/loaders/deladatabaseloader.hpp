...
public:
    ...
    void simplifyDelaFile(const std::string& pInFilename,
    ...
    void toXml(const std::vector<std::string>& pInFilenames, const std::string& pOutFilename);
     ...
     */
    void merge(const std::string& pFilename, LinguisticIntermediaryDatabase& pWords);
...
private:
    ...
    void xFillNewWordInfos(NewWordInfos& pNewWord, const std::string& pLine) const;

    std::size_t xGetStringUntilAChar(std::string& pStrResult,
                                     std::size_t pBeginOfSearch,
                                     char pEndingChar,
                                     const std::string& pLine) const;

    void xReadWordInfo(NewWordInfos& pNewWord,
    ...
    PartOfSpeech xReadGram(const std::string& pStr) const;
};
