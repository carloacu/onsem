...
public:
    ...
    void load(CompositePoolAllocator& pAlloc, const boost::property_tree::ptree& pXml);

    signed char* exportToBin(signed char* pEndMemory,
...
private:
    ...
    struct AQuestionWord {
        ...
        void init(CompositePoolAllocator& pFPAlloc,
        ...
        static void xGetPointers(std::vector<const void*>& pRes, void* pVar) {
            pRes.emplace_back(&reinterpret_cast<AQuestionWord*>(pVar)->word);
        }
    ...
    void xClear(CompositePoolAllocator& pAlloc);
...
inline void LingdbQuestionWords::xGetPointers(std::vector<const void*>& pRes, void* pVar) {
    pRes.emplace_back(&reinterpret_cast<LingdbQuestionWords*>(pVar)->fQWords);
}
