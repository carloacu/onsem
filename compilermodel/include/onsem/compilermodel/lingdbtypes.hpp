#ifndef ONSEM_COMPILERMODEL_LINGDBTYPES_HPP
#define ONSEM_COMPILERMODEL_LINGDBTYPES_HPP
...
struct ForwardPtrList {
    ...
    ForwardPtrList<ELT>* find(const ELT& pElt) {
        ForwardPtrList<ELT>* it = this;
        while (it != nullptr) {
            if (*it->elt == pElt) {
                return it;
    ...
    ForwardPtrList<ELT>* remove(const ELT& pElt, CompositePoolAllocator& pAlloc) {
    ...
    static void clearNextElt(ForwardPtrList<ELT>*& pListToModify, CompositePoolAllocator& pAlloc) {
    ...
    static void clearNextComposedElt(ForwardPtrList<ELT>*& pListToModify, CompositePoolAllocator& pAlloc) {
        assert(pListToModify != nullptr);
        ForwardPtrList<ELT>* eltToDel = pListToModify;
        pListToModify = pListToModify->next;
    ...
    void clear(CompositePoolAllocator& pFPAlloc) {
        ForwardPtrList<ELT>* it = this;
        do {
    ...
    void clearComposedElts(CompositePoolAllocator& pFPAlloc) {
        ForwardPtrList<ELT>* it = this;
        do {
    ...
    void clearWithoutDesallocateElts(CompositePoolAllocator& pFPAlloc) {
        ForwardPtrList<ELT>* it = this;
        do {
            ForwardPtrList<ELT>* toDel = it;
            it = it->next;
    ...
    static void getPointers(std::vector<const void*>& pRes, void* pVar) {
        pRes.emplace_back(&reinterpret_cast<ForwardPtrList<ELT>*>(pVar)->elt);
        pRes.emplace_back(&reinterpret_cast<ForwardPtrList<ELT>*>(pVar)->next);
    }
};

template<typename ELT>
void forwardListPushFront(CompositePoolAllocator& pAlloc, ForwardPtrList<ELT>** pList, ELT* pNewElt) {
    assert(pList != 0);
...
#endif    // ONSEM_COMPILERMODEL_LINGDBTYPES_HPP
