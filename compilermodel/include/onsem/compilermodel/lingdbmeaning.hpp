...
public:
    ...
    void addLinkToConcept(LinguisticIntermediaryDatabase& pLingDatabase,
    ...
    void getLinkToConceptsWithoutNullRelation(std::vector<const LingdbLinkToAConcept*>& pLinkToConcepts) const;
    ...
    void getLinkToConceptsStr(std::list<std::string>& pConceptsStr) const;

    const ForwardPtrList<char>* getContextInfos() const;

    void getAllContextInfos(std::list<char>& pAllContextInfos) const;

    ForwardPtrList<char>* pushBackContextInfo(LinguisticIntermediaryDatabase& pLingDatabase,
                                              WordContextualInfos pContextInfo);
    void removeContextInfo(LinguisticIntermediaryDatabase& pLingDatabase, WordContextualInfos pContextInfo);
...
private:
     ...
     */
    void xInit(LingdbDynamicTrieNode* pLemme, char pGram);
    ...
    bool xIsSecondConceptMorePrecise(const std::string& pConceptName1, const std::string& pConceptName2) const;
    ...
    ForwardPtrList<char>* xPushBackContextInfo(LinguisticIntermediaryDatabase& pLingDatabase,
