...
public:
     ...
     */
    LingdbWordForms* addWordForm(LinguisticIntermediaryDatabase& pLingDatabase,
                                 const std::string& pLemma,
                                 PartOfSpeech pPartOfSpeech,
                                 bool pAtFront = true);

    LingdbWordForms* addWordFormFromMeaning(LinguisticIntermediaryDatabase& pLingDatabase,
     ...
     */
    void putAWordFormAtTheTopOfTheList(PartOfSpeech pGram, const std::string& pLemme);
     ...
     * hold by this node.
     */
    LingdbWordForms* getWordForm(const std::string& pLemme, PartOfSpeech pPartOfSpeech) const;

    LingdbWordForms* getWordFormFromMeaning(LingdbMeaning& pMeaning) const;

    /**
     * @brief Get all the WordForms hold by this node.
     * @return All the WordForms hold by this node.
     */
    const ForwardPtrList<LingdbWordForms>* getWordForms() const;

    const ForwardPtrList<LingdbMeaning>* getMeaningsAtThisLemme() const;

    ForwardPtrList<LingdbDynamicTrieNode>* getMultiMeaningsNodes() const;

    LingdbMultiMeaningsNode* getDatasIfItsAMultiMeaningNode() const;

    /**
     * @brief Get the number of meanings hold by this node.
     * @return The number of meanings hold by this node.
     */
    unsigned char nbMeaningsAtThisLemme() const;

    unsigned char nbWordForms() const;

    void getWordFormsAndMeanings(std::list<std::pair<LingdbWordForms*, LingdbMeaning*> >& pWordFromsOrMeanings) const;

    /**
     * @brief Get the first child of this node.
     * @return The first child of this node.
     */
    LingdbDynamicTrieNode* getFirstChild() const;

    /**
     * @brief Get the next brother of this node.
     * @return The next brother of this node.
     */
    LingdbDynamicTrieNode* getNextBrother() const;

    LingdbDynamicTrieNode* getFather() const;

    /**
     * @brief Get the next node that represent the end of a word.
     * @return The next node that represent the end of a word.
     */
    LingdbDynamicTrieNode* getNextWordNode() const;
     ...
     */
    LingdbDynamicTrieNode* advanceInTrie(const std::string& word, bool pUntilEndOfWord) const;
     ...
     * @return The resulting node of the trie.
     */
    LingdbDynamicTrieNode* advanceInTrieIfEndOfAWord(const std::string& word) const;
...
private:
     ...
     */
    void xRemoveAllWordForms(CompositePoolAllocator& pAlloc);
     ...
     */
    void xRemoveWordForm(CompositePoolAllocator& pAlloc, PartOfSpeech pPartOfSpeech);
     ...
     * letter.
     */
    LingdbDynamicTrieNode* xGetChild(char pLetter) const;
     ...
     * have this letter.
     */
    LingdbDynamicTrieNode* xGetBrother(char pLetter) const;
     ...
     */
    void xRemoveMeaning(CompositePoolAllocator& pFPAlloc, PartOfSpeech pPartOfSpeech);
     ...
     */
    LingdbDynamicTrieNode* xInsertWord(CompositePoolAllocator& pFPAlloc, const std::string& pWord, std::size_t pOffset);

    LingdbWordForms* xAddWordFormFromMeaning(CompositePoolAllocator& pAlloc, LingdbMeaning& pMeaning, bool pAtFront);
     ...
     */
    LingdbMeaning* xAddMeaning(CompositePoolAllocator& pFPAlloc, PartOfSpeech pPartOfSpeech);

    /**
     * @brief Deallocate, if possible, the word that ends at this node.
     * @param pFPAlloc The allocator.
     */
    void xRemoveWordIfNoLongerAWord(CompositePoolAllocator& pFPAlloc);

    /**
     * @brief Recursively deallocate, if possible, the node.
     * @param pFPAlloc The allocator.
     */
    void xTryToDeallocateNodes(CompositePoolAllocator& pFPAlloc);
