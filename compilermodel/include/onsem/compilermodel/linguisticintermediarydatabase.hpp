...
public:
   ...
   */
  LingdbDynamicTrieNode* getPointerToEndOfWord
   ...
   */
  LingdbDynamicTrieNode* getPointerInTrie
  (const std::string& word) const;
  ...
  const LingdbString* getLanguage
  () const;
   ...
   */
  LingdbMeaning* getMeaning
  (const std::string& pLemma,
   PartOfSpeech pPartOfSpeech) const;


  LingdbConcept* getConcept
  (const std::string& pConceptName) const;

  LingdbConcept* addConcept(bool& pNewCptHasBeenInserted,
...
private:
  struct LingdbInfos
  {
    ...
    {
      pRes.emplace_back(&reinterpret_cast<LingdbInfos*>
                     (pVar)->language);
      pRes.emplace_back(&reinterpret_cast<LingdbInfos*>
                     (pVar)->questionWords);
    }
