...
{
  LingdbNodeLinkedMeaning
  (char pDirection,
   LingdbMeaning* pdMeaning)
  ...
  {
    pRes.emplace_back(&reinterpret_cast<LingdbNodeLinkedMeaning*>
                   (pVar)->meaning);
  }
};
...
public:
  ...
  LingdbMeaning* getRootMeaning() const
  {
    return fRootMeaning;
  }

  const ForwardPtrList<LingdbNodeLinkedMeaning>* getLinkedMeanings() const
  {
