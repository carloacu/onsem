...
void run(const LingdbTree& pLingDbTree, const std::string& pTmpFolder, const std::string& pInputResourcePath) {
    ...
    for (std::list<WlksDatabaseLoader::WlksDatabaseLoader_TradSpec>::iterator itTradSpecs =
             tradSummary.tradSpecs.begin();
         itTradSpecs != tradSummary.tradSpecs.end();
         ++itTradSpecs) {
        LinguisticIntermediaryDatabase& inLingDb = *itTradSpecs->inLingDb.lingDatabase;
        SemanticLanguageEnum inLang = semanticLanguageTypeGroundingEnumFromStr(inLingDb.getLanguage()->toStr());
        SemanticLanguageEnum outLang =
            semanticLanguageTypeGroundingEnumFromStr(itTradSpecs->outLingDb.lingDatabase->getLanguage()->toStr());

        if (inLang == SemanticLanguageEnum::ENGLISH && outLang == SemanticLanguageEnum::FRENCH) {
            std::map<LingdbMeaning*, std::set<MeaningAndConfidence>>& trads = itTradSpecs->traductions;
            for (auto& currTrad : trads) {
                ...
                if (inMeaning.getPartOfSpeech() == PartOfSpeech::ADJECTIVE) {
                    ...
                    if (adjWordStr_size > ingEndingStr_size
                        && adjWordStr.compare(adjWordStr_size - ingEndingStr_size, ingEndingStr_size, ingEndingStr)
                               == 0) {
                        ...
                        if (adjWordNode != nullptr) {
                            ...
                            while (wordsFromsForThisWord != nullptr) {
                                ...
                                if (correspondingVerbMeaning->getPartOfSpeech() == PartOfSpeech::VERB) {
                                    ...
                                    if (itVerbTrad != trads.end() && !itVerbTrad->second.empty()) {
        ...
        } else if (inLang == SemanticLanguageEnum::FRENCH && outLang == SemanticLanguageEnum::ENGLISH) {
            const std::string outputFilename = pInputResourcePath + "/common/readonly/french_to_english.wlks";
            std::cout << "writing: " << outputFilename << std::endl;
            writer.writeSummaryTraductions(outputFilename, itTradSpecs->traductions);
        }
    }
}
