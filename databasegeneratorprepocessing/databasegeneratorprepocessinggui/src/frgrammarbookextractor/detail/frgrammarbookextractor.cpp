 ...
 const std::set<FrGrammarBookExtHelpers::PrepToObjectType>& pPrepsToObjectType)
{
  ...
  for (std::set<FrGrammarBookExtHelpers::PrepToObjectType>::const_iterator
       itPO = pPrepsToObjectType.begin();
       itPO != pPrepsToObjectType.end(); ++itPO)
  {
    pInfosFile << "___" << itPO->prep << ":"
               << FrGrammarBookObjectType_toStr[itPO->objType] << std::endl;
  }
}



void grammarBookToInfosFile
(const std::string& pGrammarFilename,
 ...
 const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  while (getline(grammarFile, line))
  {
    if (line.empty() || line == "\r" ||
        line.compare(0, pageSeparatorStr.size(), pageSeparatorStr) == 0 ||
  ...
  for (std::list<FrGrammarBookExtHelpers::VerbToPreps>::const_iterator
       it = vbToPrepsList.begin(); it != vbToPrepsList.end(); ++it)
  {
    for (std::list<std::string>::const_iterator itVb = it->currVerbs.begin();
         itVb != it->currVerbs.end(); ++itVb)
    {
         ...
         const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  const auto grammarFilename = pShareDbFolder + "/databasegeneratorpreprocessing/frgrammarbookextractor/in/verbtoprep.txt";
  const auto infosFilename = pShareDbFolder + "/databasegeneratorpreprocessing/frgrammarbookextractor/in/infosFile.txt";
