...
FrGrammarBookExtHelpers::FrGrammarBookExtHelpers(const linguistics::StaticLinguisticDictionary& pBinDico,
...
void FrGrammarBookExtHelpers::removeAllSpacesAndSplit(std::list<std::string>& pRes, const std::string& pStr) {
...
void FrGrammarBookExtHelpers::removeAfterDoublePoint(std::string& pStr) {
...
void FrGrammarBookExtHelpers::removeBackSlashRAtEnd(std::string& pStr) {
    if (pStr[pStr.size() - 1] == '\r') {
        pStr = pStr.substr(0, pStr.size() - 1);
    }
}

void FrGrammarBookExtHelpers::toLowerCase(std::string& pStr) {
    for (std::size_t i = 0; i < pStr.size(); ++i) {
...
void FrGrammarBookExtHelpers::replaceSomeCharsToBlanck(std::string& pStr) {
    for (std::size_t i = 0; i < pStr.size(); ++i) {
        if (pStr[i] == '|' || pStr[i] == '\\') {
            pStr[i] = ' ';
        }
...
void FrGrammarBookExtHelpers::removeArrowAtBeginning(std::string& pStr) {
    pStr = pStr.substr(arrowStr.size(), pStr.size() - arrowStr.size());
}

bool FrGrammarBookExtHelpers::removeBlanksAtBeginning(std::string& pStr, std::size_t pMaxNbToRemove) {
...
bool FrGrammarBookExtHelpers::findVerb(std::list<std::string>& pVerbs, const std::string& pLine) const {
    bool res = false;
    ...
    if (parenthesisPos != std::string::npos) {
        if (pLine.compare(parenthesisPos, 3, "(ou") == 0) {
            ...
            if (endParenth != std::string::npos) {
                res |= xTryToAddVerb(pVerbs, pLine.substr(parenthesisPos, endParenth - parenthesisPos), false);
            }
        } else {
...
void FrGrammarBookExtHelpers::extractPrepsToObjectType(std::set<PrepToObjectType>& pPrepsToObjectType,
                                                       const std::string& pStr) const {
    std::list<std::string> currPreps;
    std::size_t strPos = 0;

    while (strPos != std::string::npos) {
        if (xTryToDetectAnyObjectType(pPrepsToObjectType, currPreps, strPos, pStr)
            || xTryToDetectAPrep(currPreps, strPos, pStr)) {
            continue;
        }
        if (pStr[strPos] == '(' || pStr[strPos] == ')' || pStr[strPos] == '/') {
            ++strPos;
            continue;
        }
        break;
...
void FrGrammarBookExtHelpers::removeVerbsThatOccur2Times(std::list<VerbToPreps>& pVerbToPreps) {
    ...
    for (std::list<VerbToPreps>::iterator it = ++(++pVerbToPreps.begin()); it != pVerbToPreps.end(); ++it) {
        if (it->currVerbs.front() == itPrev->currVerbs.front()) {
            itPrevPrev->prepsToObjectType.insert(itPrev->prepsToObjectType.begin(), itPrev->prepsToObjectType.end());
...
void FrGrammarBookExtHelpers::writeMostFrequentAfterPrep(
    ...
    for (std::list<FrGrammarBookExtHelpers::VerbToPreps>::const_iterator it = pVbToPrepsList.begin();
         it != pVbToPrepsList.end();
         ++it) {
        for (std::set<FrGrammarBookExtHelpers::PrepToObjectType>::const_iterator itPrep = it->prepsToObjectType.begin();
             itPrep != it->prepsToObjectType.end();
             ++itPrep) {
            ++prepsToStats[itPrep->prep][itPrep->objType];
        }
    }
    for (std::map<std::string, std::map<FrGrammarBookObjectType, int> >::const_iterator it = prepsToStats.begin();
         it != prepsToStats.end();
         ++it) {
...
void FrGrammarBookExtHelpers::writeVerbsPrepsXmlFile(
    ...
    rootXml.setAttribute("language",
                         QString::fromUtf8(semanticLanguageEnum_toLegacyStr(SemanticLanguageEnum::FRENCH).c_str()));
    resultXml.appendChild(rootXml);
    for (std::list<FrGrammarBookExtHelpers::VerbToPreps>::const_iterator itVbToPrep = pVbToPrepsList.begin();
         itVbToPrep != pVbToPrepsList.end();
         ++itVbToPrep) {
        for (std::list<std::string>::const_iterator itVb = itVbToPrep->currVerbs.begin();
             itVb != itVbToPrep->currVerbs.end();
             ++itVb) {
            QDomElement newVerb = resultXml.createElement("word");
            newVerb.setAttribute("lemma", QString::fromUtf8(itVb->c_str()));
            newVerb.setAttribute("pos", QString::fromUtf8(partOfSpeech_toStr(PartOfSpeech::VERB).c_str()));
            for (std::set<FrGrammarBookExtHelpers::PrepToObjectType>::const_iterator itPrep =
                     itVbToPrep->prepsToObjectType.begin();
                 itPrep != itVbToPrep->prepsToObjectType.end();
                 ++itPrep) {
                if (!itPrep->prep.empty() && itPrep->objType != OBJECTTYPE_AGENT
                    && xHas(prepsToPrintAlways, itPrep->prep) && itPrep->objType == OBJECTTYPE_OBJECT) {
...
bool FrGrammarBookExtHelpers::xTryToAddVerb(std::list<std::string>& pVerbs,
...
void FrGrammarBookExtHelpers::xWritePrepStats(std::ofstream& pInfosFile,
    ...
    for (std::map<FrGrammarBookObjectType, int>::const_iterator itObjType = pStats.begin(); itObjType != pStats.end();
         ++itObjType) {
        if (itObjType->second > 0 && (itBest == pStats.end() || itObjType->second >= itBest->second)) {
            itBest = itObjType;
        }
    }
    if (itBest != pStats.end()) {
        ...
        pInfosFile << ">" << pPrep << ":" << FrGrammarBookObjectType_toStr[itBest->first] << "|\t";
        for (std::map<FrGrammarBookObjectType, int>::const_iterator itObjType = pStats.begin();
             itObjType != pStats.end();
             ++itObjType) {
            pInfosFile << " " << FrGrammarBookObjectType_toStr[itObjType->first] << "(" << itObjType->second << ")";
        }
        pInfosFile << std::endl;
    }
}

bool FrGrammarBookExtHelpers::xTryToDetectAnyObjectType(std::set<PrepToObjectType>& pPrepsToObjectType,
                                                        std::list<std::string>& pCurrPreps,
                                                        std::size_t& pStrPos,
                                                        const std::string& pStr) const {
    return xTryToDetectObjectType(pPrepsToObjectType, pCurrPreps, pStrPos, pStr, fQuelqueChoseStr, OBJECTTYPE_OBJECT)
        || xTryToDetectObjectType(pPrepsToObjectType, pCurrPreps, pStrPos, pStr, fQuelquePartStr, OBJECTTYPE_LOCATION)
        || xTryToDetectObjectType(pPrepsToObjectType, pCurrPreps, pStrPos, pStr, fQuelquUnStr, OBJECTTYPE_AGENT);
}

bool FrGrammarBookExtHelpers::xTryToDetectObjectType(std::set<PrepToObjectType>& pPrepsToObjectType,
                                                     std::list<std::string>& pCurrPreps,
                                                     std::size_t& pStrPos,
    ...
    if (pStr.compare(pStrPos, pObjectStr.size(), pObjectStr) == 0) {
        for (std::list<std::string>::const_iterator itPrep = pCurrPreps.begin(); itPrep != pCurrPreps.end(); ++itPrep) {
        ...
        if (pStrPos < pStr.size()) {
            if (pStr[pStrPos] == '/') {
                ...
                ++pStrPos;
                insertedSomething |= xTryToDetectPrepsToAnObjectType(pPrepsToObjectType, pStrPos, pStr, pObjectType);
            }
        }
...
bool FrGrammarBookExtHelpers::xTryToDetectPrepsToAnObjectType(std::set<PrepToObjectType>& pPrepsToObjectType,
                                                              std::size_t& pStrPos,
                                                              const std::string& pStr,
                                                              FrGrammarBookObjectType pObjectType) const {
    bool res = false;
    for (; pStrPos < pStr.size(); ++pStrPos) {
        ...
        for (std::map<StrSortedBySize, std::optional<FrGrammarBookObjectType> >::const_iterator itPrep = fPreps.begin();
             itPrep != fPreps.end();
             ++itPrep) {
...
bool FrGrammarBookExtHelpers::xTryToDetectAPrep(std::list<std::string>& pCurrPreps,
                                                std::size_t& pStrPos,
                                                const std::string& pStr) const {
    for (auto itPrep = fPreps.begin(); itPrep != fPreps.end(); ++itPrep) {
        if (pStr.compare(pStrPos, itPrep->first.str.size(), itPrep->first.str) == 0) {
            ...
            if (pStrPos < pStr.size() && pStr[pStrPos] == '/') {
                ++pStrPos;
...
bool FrGrammarBookExtHelpers::xHas(const std::list<std::string>& pList, const std::string& pStr) {
