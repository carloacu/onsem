...
inline bool FrPatternRecognizer::isSeparator(const std::string& pLine) const {
    return pLine.size() > 6 && pLine.compare(pLine.size() - 2, 2, "==") == 0;
}

inline bool FrPatternRecognizer::isLanguage(const std::string& pLine) const {
    return pLine[pLine.size() - 3] == ' ' && pLine.find("langue") != std::string::npos;
}

inline PatternRecognizerLanguageEnum FrPatternRecognizer::getLanguage(const std::string& pLine) const {
...
inline bool FrPatternRecognizer::isGramInLineType(const std::string& pLine) const {
    return pLine.compare(0, 8, "=== {{S|") == 0 && pLine.find("fr", 9) != std::string::npos;
}

inline bool FrPatternRecognizer::isBeginLineOfTrads(const std::string& pLine) const {
