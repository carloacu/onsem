...
void WikiDataExtractor::extractDatasFromFile
        ...
        case PATTERNRECO_LANG_INLANG:
        {
          ...
          else if (fPatternReco.isBeginLineOfTrads(line))
          {
            ...
            if (!outMeanings.empty())
            {
              ...
              {
                xFillTrads(pTradsInToOut, pTradsOutToIn,
                           inNode, inGramTypes, outMeanings);
              }
              else
              {
...
void WikiDataExtractor::xExtractMeaningsOfOutWords
(std::set<LingdbMeaning*>& pOutMeanings,
 ...
 const LinguisticIntermediaryDatabase& pOutLingDatabase) const
{
  ...
  for (std::list<std::string>::const_iterator itTrad = tradsStr.begin();
       itTrad != tradsStr.end(); ++itTrad)
  {
    LingdbDynamicTrieNode* outNode =
        pOutLingDatabase.getPointerToEndOfWord(*itTrad);
    if (outNode == nullptr)
    {
...
void WikiDataExtractor::xAddNodeToNodes
(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTrads,
...
void WikiDataExtractor::xAddNodesToNode
(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTrads,
 const std::set<LingdbMeaning*>& pInMeanings,
 const LingdbDynamicTrieNode* pOutNode)
{
  for (std::set<LingdbMeaning*>::const_iterator itIM = pInMeanings.begin();
       itIM != pInMeanings.end(); ++itIM)
  {
...
void WikiDataExtractor::xFillTrads
(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsInToOut,
 std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsOutToIn,
 const LingdbDynamicTrieNode* pInNode,
 ...
 const std::set<LingdbMeaning*>& pOutMeanings)
{
  ...
  while (wfs != nullptr)
  {
    ...
    std::set<PartOfSpeech>::const_iterator
        itInGram = pInGramTypes.find(inMeaning->getPartOfSpeech());
    if (itInGram != pInGramTypes.end())
    {
      ...
      for (std::set<LingdbMeaning*>::const_iterator itOutMeaning = pOutMeanings.begin();
           itOutMeaning != pOutMeanings.end(); ++itOutMeaning)
      {
