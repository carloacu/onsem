...
inline bool EnPatternRecognizer::isSeparator
(const std::string& pLine) const
{
  return pLine.size() > 6 &&
...
inline bool EnPatternRecognizer::isLanguage
(const std::string& pLine) const
{
  return pLine[pLine.size() - 3] != '=';
}

inline PatternRecognizerLanguageEnum EnPatternRecognizer::getLanguage
(const std::string& pLine) const
{
...
inline bool EnPatternRecognizer::isBeginLineOfTrads
(const std::string& pLine) const
{
...
inline bool EnPatternRecognizer::isGramInLineType
(const std::string& pLine) const
{
  return pLine.compare(0, 3, "===") == 0 &&
      pLine.size() > 6 && pLine[4] != '=';
}
