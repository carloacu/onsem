...
ConceptsExtractor::ConceptsExtractor(const WikiKeyWords& pWikikeyWords, const PatternRecognizer& pPatternRecognizer)
    : _wikikeyWords(pWikikeyWords)
    , _patternReco(pPatternRecognizer) {}

void ConceptsExtractor::extractDatasFromFile(std::map<std::string, std::set<LingdbMeaning*>>& pConceptToMeanings,
    ...
    while (getline(pWikionaryFile, line)) {
        if (!line.empty()) {
            if (_patternReco.isTitle(line)) {
                ...
                nounMeaning = nodePtr != nullptr ? nodePtr->getMeaning(PartOfSpeech::NOUN) : nullptr;
...
void ConceptsExtractor::writeConcepts(const std::string& pOutFilename,
    ...
    for (const auto& currCptToMeaning : pConceptToMeanings) {
        ...
        for (const auto& currMeaning : currCptToMeaning.second) {
            outfile << "  <linkedMeaning lemme=\"" << currMeaning->getLemma()->getWord() << "\" gram=\""
                    << partOfSpeech_toStr(currMeaning->getPartOfSpeech()) << "\" relationToConcept=\"4\"/>\n";
        }
        outfile << " </concept>\n";
    }
    outfile << "</linguisticdatabase>";
