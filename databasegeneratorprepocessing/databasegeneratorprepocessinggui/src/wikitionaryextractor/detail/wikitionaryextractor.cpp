...
void extractDataFromSpecifcWiki(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsInToOut,
    ...
    dataExt.extractDatasFromFile(pTradsInToOut, pTradsOutToIn, wikionaryFile, pInLingDatabase, pOutLingDatabase);
...
void extractDataFromFrWiki(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsInToOut,
                           std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsOutToIn,
                           const LinguisticIntermediaryDatabase& pInLingDatabase,
                           const LinguisticIntermediaryDatabase& pOutLingDatabase,
                           const std::string& pWikionaryFilename) {
    ...
    extractDataFromSpecifcWiki(pTradsInToOut,
                               pTradsOutToIn,
                               pInLingDatabase,
                               pOutLingDatabase,
                               frWikiKeyWords,
...
void extractDataFromEnWiki(std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsInToOut,
                           std::map<LingdbMeaning*, LingdbSaverOutLinks>& pTradsOutToIn,
                           const LinguisticIntermediaryDatabase& pInLingDatabase,
                           const LinguisticIntermediaryDatabase& pOutLingDatabase,
                           const std::string& pWikionaryFilename) {
...
void addComposedWords(const std::string& pWikionaryFilename,
    ...
    pCompWordsAdder.extractDatasFromFile(newComposedWords, wikionaryFile, pLingDatabase, pLangEnum);
    pCompWordsAdder.writeNewComposedWords(newComposedWords, pOutFilename);
}
...
void _loadLingDb(LinguisticIntermediaryDatabase& pLingDatabase,
    ...
    pLingDatabase.load(pLingDbTree.getDynamicDatabasesFolder() + "/" + pLingDatabase.getLanguage()->toStr() + "."
...
void runTraductions(const LingdbTree& pLingDbTree,
    ...
    extractDataFromFrWiki(tradsFrToEn_FrWiki, tradsEnToFr_FrWiki, frLingDatabase, enLingDatabase, frWikionaryFilename);
    ...
    extractDataFromEnWiki(tradsEnToFr_EnWiki, tradsFrToEn_EnWiki, enLingDatabase, frLingDatabase, enWikionaryFilename);
    ...
    tradWriter.writeTranslations(frToEnFilename, tradsFrToEn_FrWiki, tradsFrToEn_EnWiki, frLingDatabase);

    std::cout << "Write file: " << enToFrFilename << std::endl;
    tradWriter.writeTranslations(enToFrFilename, tradsEnToFr_EnWiki, tradsEnToFr_FrWiki, enLingDatabase);
}
...
void addComposedWords(const LingdbTree& pLingDbTree,
    ...
    FrPatternRecognizer frPatternReco;
    onsem::addComposedWords(frWikionaryFilename,
                            frLingDatabase,
                            onsem::AddComposedWords(frPatternReco),
                            onsem::SemanticLanguageEnum::FRENCH,
                            frOutputFilename);

    EnPatternRecognizer enPatternReco;
    onsem::addComposedWords(enWikionaryFilename,
                            enLingDatabase,
                            onsem::AddComposedWords(enPatternReco),
                            onsem::SemanticLanguageEnum::ENGLISH,
                            enOutputFilename);
}
...
void addTransitiveVerbs(const LingdbTree& pLingDbTree,
    ...
    const std::string enOutputFilename =
        pInputResourcePath + "/english/readonly/english_contextinfos_transitiveverb.xml";
