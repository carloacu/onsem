...
AddComposedWords::AddComposedWords
...
void AddComposedWords::extractDatasFromFile
    ...
    if (!line.empty())
    {
      if (fPatternReco.isTitle(line))
      {
        ...
        if (pLangEnum == SemanticLanguageEnum::FRENCH &&
            !currWord.empty() && currWord[0] == 's')
        {
          if (currWord.compare(0, 3, "se ") == 0)
            xTryToAddPronominalFrenchVerb(pNewComposedWords, pInLingDatabase,
                                          currWord.substr(3, currWord.size() - 3));
          else if (currWord.compare(0, 2, "s'") == 0)
            xTryToAddPronominalFrenchVerb(pNewComposedWords, pInLingDatabase,
                                          currWord.substr(2, currWord.size() - 2));
        }
        else if (pLangEnum == SemanticLanguageEnum::ENGLISH)
        {
          std::size_t endFirstWord = currWord.find_first_of(' ');
          if (endFirstWord != std::string::npos &&
              endFirstWord + 1 < currWord.size() &&
              currWord.find_first_of(' ', endFirstWord + 1) == std::string::npos)
          {
            LingdbDynamicTrieNode* endOfFirstWord = pInLingDatabase.getPointerToEndOfWord
                (currWord.substr(0, endFirstWord));
              ...
              if (firstWordVerbMeaning != nullptr)
              {
                ...
                if (endOfSecondWord != nullptr)
                {
                  ...
                  if (subMeaning != nullptr)
                  {
                    ...
                    newComposedWord.subMeanings.emplace_back(subMeaning, LinkedMeaningDirection::FORWARD);
...
void AddComposedWords::writeNewComposedWords
 ...
 const std::string& poutFile)
{
  ...
  for (std::set<Wikitionary_ComposedWord>::const_iterator it = pNewComposedWords.begin();
       it != pNewComposedWords.end(); ++it)
  {
    QDomElement newWord = document.createElement("words");
    xFillMeaningAttriabutes(newWord, it->rootSubMeaning);

    for (auto itSMeaning = it->subMeanings.begin();
         itSMeaning != it->subMeanings.end(); ++itSMeaning)
    {
      QDomElement newSubMeaning;
      switch (itSMeaning->second)
      {
      case LinkedMeaningDirection::FORWARD:
        newSubMeaning = document.createElement("nextMeaning");
        break;
      case LinkedMeaningDirection::BACKWARD:
        newSubMeaning = document.createElement("prevMeaning");
        break;
      case LinkedMeaningDirection::BOTH:
      default:
        newSubMeaning = document.createElement("nextOrPrevMeaning");
        break;
      }
...
void AddComposedWords::xFillMeaningAttriabutes
...
void AddComposedWords::xTryToAddPronominalFrenchVerb
(std::set<Wikitionary_ComposedWord>& pNewComposedWords,
 const LinguisticIntermediaryDatabase& pInLingDatabase,
