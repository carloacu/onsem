...
public:
    WikiTransitiveVerbExtractor(const WikiKeyWords& pWikikeyWords, const PatternRecognizer& pPatternRecognizer);

    void extract(std::set<LingdbMeaning*>& pTransitiveVerbs,
                 std::ifstream& pWikionaryFile,
                 const LinguisticIntermediaryDatabase& pLingDatabase);

    void writeNewTransitiveVerbs(const std::set<LingdbMeaning*>& pTransitiveVerbs, const std::string& poutFile);
