...
public:
  explicit DbGeneratorPreprocessingWindow(
      const onsem::LingdbTree& pLingDbTree,
...
private:
  ...
  struct WordsToTagsToDisplay
  {
    WordsToTagsToDisplay()
      : conjugaisons(), wordSuggestions(), meaningsStr()
    {}
...
private:
  struct WordsToTagsCurrentState
  {
    WordsToTagsCurrentState()
      : wordFroms(nullptr), meaning(nullptr)
    {}
  ...
  void _conjAMeaning(std::string& pConj,
                     onsem::LingdbMeaning* pMeaning) const;
