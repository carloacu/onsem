...
bool loadXmlWebTranslations(QDomDocument& pDatabaseXml, const std::string& pFilename) {
...
void process_oneWebresultsFile(const std::string& pFilename,
    ...
    while (!currDbChild.isNull()) {
        ...
        if (tagName == "div") {
            PartOfSpeech gram = partOfSpeech_fromStr(currDbChild.attribute("gram", "").toUtf8().constData());
...
void process_webresults(const LingdbTree& pLingDbTree,
    ...
    lingDbEn.load(pLingDbTree.getDynamicDatabasesFolder() + "/" + lingDbEn.getLanguage()->toStr() + "."
    ...
    lingDbFr.load(pLingDbTree.getDynamicDatabasesFolder() + "/" + lingDbFr.getLanguage()->toStr() + "."
                  + pLingDbTree.getExtDynDatabase());
