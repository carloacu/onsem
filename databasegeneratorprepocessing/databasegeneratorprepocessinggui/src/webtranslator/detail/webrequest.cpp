...
void flushrequestInFile(const std::string& pInLang,
    ...
    outfile << "    new google.translate.TranslateElement({pageLanguage: '" << pInLang << "', includedLanguages: '"
            << pOutLang
            << "', layout: google.translate.TranslateElement.InlineLayout.SIMPLE}, 'google_translate_element');"
            << std::endl;
    outfile << "  }" << std::endl;
    outfile << "  </script><script type=\"text/javascript\" "
...
void processOneTranslationRequest(SemanticLanguageEnum pInLangEnum,
    ...
    while (meaning != nullptr) {
        ...
        if (partOfSpeech != PartOfSpeech::INTERSPACE && partOfSpeech != PartOfSpeech::LINKBETWEENWORDS
            && partOfSpeech != PartOfSpeech::PUNCTUATION && partOfSpeech != PartOfSpeech::AUX) {
            ss << "  <div gram=\"" << partOfSpeech_toStr(partOfSpeech) << "\" lemma=\""
               << meaning->getLemma()->getWord() << "\">";
...
void process_webrequest(const std::string& pInputResourcesDir) {
    ...
    processOneTranslationRequest(SemanticLanguageEnum::FRENCH, "fr", "en", lingDbTree);
    processOneTranslationRequest(SemanticLanguageEnum::ENGLISH, "en", "fr", lingDbTree);
}
