...
AdjBeforeNounExtractor::AdjBeforeNounExtractor(const CompositePoolAllocator& pAlloc)
    : fAdjStats() {
...
void AdjBeforeNounExtractor::processText(const linguistics::TokensTree& pTokensTree) {
    for (linguistics::TokCstIt itTok = pTokensTree.tokens.begin(); itTok != pTokensTree.tokens.end(); ++itTok) {
        if (itTok->inflWords.size() == 1 && itTok->inflWords.front().word.partOfSpeech == PartOfSpeech::NOUN) {
            linguistics::TokCstIt prevTok = getPrevToken(itTok, pTokensTree.tokens.begin(), pTokensTree.tokens.end());
            if (prevTok != pTokensTree.tokens.end()) {
                xRefreshAdjPositionInfo(prevTok->inflWords, true);
            }
            linguistics::TokCstIt nextTok = getNextToken(itTok, pTokensTree.tokens.end());
...
void AdjBeforeNounExtractor::xRefreshAdjPositionInfo(const std::list<linguistics::InflectedWord>& pIGrams,
                                                     bool pBeforeAfterNoun) {
    for (const auto& currIGram : pIGrams) {
        if (currIGram.word.partOfSpeech == PartOfSpeech::ADJECTIVE) {
            std::map<std::string, AdjStats>::iterator itAdjStats = fAdjStats.find(currIGram.word.lemma);
...
void AdjBeforeNounExtractor::writeResults(const std::string& pResultFilename) const {
    std::cout << "Write results life: " << pResultFilename << std::endl;
    std::ofstream resultFile(pResultFilename.c_str());
    for (std::map<std::string, AdjStats>::const_iterator it = fAdjStats.begin(); it != fAdjStats.end(); ++it) {
        resultFile << "#" << it->first << "\t#" << it->second.nbBeforeNoun << "_" << it->second.nbAfterNoun << "_"
                   << std::endl;
    }
    resultFile.close();
}

void AdjBeforeNounExtractor::writeXml(const std::string& pResultFilename, const std::string& pResultFilenameXml) {
    ...
    while (getline(resultFile, line)) {
        ...
        if (nbBeforeNoun > 50 && nbBeforeNoun > nbAfterNoun) {
            ...
            newAdjBefNoun.setAttribute("gram", QString::fromUtf8(partOfSpeech_toStr(PartOfSpeech::ADJECTIVE).c_str()));
