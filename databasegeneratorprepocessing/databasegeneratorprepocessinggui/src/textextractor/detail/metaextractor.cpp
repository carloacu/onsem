...
void MetaExtractor::spitResultLine(std::string& pLemma, std::vector<int>& pNbs, const std::string& pLine) {
    const std::string sepStr = "\t#";
    std::size_t idSep = pLine.find(sepStr);
    if (idSep != std::string::npos && idSep > 1) {
        ...
        std::cerr << "Line: \"" << pLine << "\" was no separator" << std::endl;
    }
}
