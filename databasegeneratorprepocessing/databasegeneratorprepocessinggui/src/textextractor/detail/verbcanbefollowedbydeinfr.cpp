...
VerbCanBeFollowedByDeInFr::VerbCanBeFollowedByDeInFr
...
void VerbCanBeFollowedByDeInFr::processText
...
void VerbCanBeFollowedByDeInFr::writeResults
(const std::string& pResultFilename) const
{
  ...
  for (auto it = _verbStats.begin(); it != _verbStats.end(); ++it)
    resultFile << "#" << it->first << "\t#"
               << it->second.nbOccs << "_"
               << it->second.nbFollowedByDe << "_" << std::endl;
  resultFile.close();
}


void VerbCanBeFollowedByDeInFr::writeXml
...
void VerbCanBeFollowedByDeInFr::xProcessChunkLinkList
 ...
 const std::string& pText)
{
  for (const auto& currChkLk : pChunkLinks)
  {
    ...
    if (chunk.type == linguistics::ChunkType::VERB_CHUNK)
    {
      ...
      {
        std::cerr << "verb: " << lemma << " is not in database ("
                  << pText << ")" << std::endl;
      }
      else
      {
