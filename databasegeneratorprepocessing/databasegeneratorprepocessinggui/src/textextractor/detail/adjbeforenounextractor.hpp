...
public:
    ...
    void processText(const linguistics::TokensTree& pTokensTree);

    void writeResults(const std::string& pResultFilename) const;

    static void writeXml(const std::string& pResultFilename, const std::string& pResultFilenameXml);
...
private:
    ...
    void xRefreshAdjPositionInfo(const std::list<linguistics::InflectedWord>& pIGrams, bool pBeforeAfterNoun);
};
