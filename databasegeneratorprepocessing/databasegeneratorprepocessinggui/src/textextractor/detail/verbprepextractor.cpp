...
void VerbPrepExtractor::processText(const linguistics::SyntacticGraph& pSyntGraph, const std::string& pText) {
    xProcessChunkLinkList(pSyntGraph.firstChildren, pText);
}

void VerbPrepExtractor::writeResults(const std::string& pResultFilename) const {
    std::cout << "Write results life: " << pResultFilename << std::endl;
    std::ofstream resultFile(pResultFilename.c_str());
    for (std::map<std::string, int>::const_iterator it = fVerbsToFreq.begin(); it != fVerbsToFreq.end(); ++it) {
        resultFile << "#" << it->first << "\t#" << it->second << "_" << std::endl;
    }
    resultFile.close();
}
...
void VerbPrepExtractor::xProcessChunkLinkList(const std::list<linguistics::ChunkLink>& pChunkLinks,
    ...
    for (const auto& currChkLk : pChunkLinks) {
        ...
        if (chunk.type == linguistics::ChunkType::VERB_CHUNK) {
            const linguistics::Chunk* questChunk =
                _getChildrenOfAChunk(chunk, linguistics::ChunkLinkType::QUESTIONWORD);
            if (questChunk != nullptr) {
                if (questChunk->head->inflWords.front().word == fAQuiPronWord) {
                    const linguistics::Chunk* doChunkPtr =
                        _getChildrenOfAChunk(chunk, linguistics::ChunkLinkType::DIRECTOBJECT);
            ...
            const linguistics::ChunkLink* subjectOfChunkLk = _getSubjectOfChunkLink(chunk);
...
void VerbPrepExtractor::xAddSubVerbIfInterestingChunkLink(const linguistics::ChunkLink& pChunkLink) {
    for (auto it = getTheNextestToken(pChunkLink.tokRange.getItBegin(), pChunkLink.tokRange.getItEnd());
...
void VerbPrepExtractor::xAddChunkHead(const linguistics::Chunk& pChunk) {
