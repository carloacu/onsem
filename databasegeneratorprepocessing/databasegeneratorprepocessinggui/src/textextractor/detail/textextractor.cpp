                   ...
                   bool pPrintLineNumber)
{
  ...
  for (const auto& currText : pTexts)
  {
    ...
    linguistics::tokenizeText(syntGraph.tokensTree, syntGraph.langConfig, currText,
                              std::shared_ptr<ResourceGroundingExtractor>());
 ...
 const std::string& pShareSemanticPath)
{
  ...
  lingIntDb.load(pLingDbTree.getDynamicDatabasesFolder() + "/" +
                 lingIntDb.getLanguage()->toStr() + "." +
  ...
  for (std::size_t i = 0; i < textExtractorNbOfThreads; ++i)
    workers.push_back(std::thread(_processTexts, std::ref(adjBefNouns[i]), std::ref(verbCanBeFollowedByDeInFrs[i]),
                                  std::ref(verbprepExts[i]), std::ref(textsToProccess[i]), std::ref(pLingDb),
                                  langEnum, i == 0));
  for (auto& th : workers)
    th.join();
 ...
 const std::string& pInputResourcesFolder)
{
  ...
  VerbCanBeFollowedByDeInFr::writeXml(pResultVerbFollByDeFilename,
                                      resultVerbFollByDeFilenameXml);
  AdjBeforeNounExtractor::writeXml(pResultAdjFilename,
                                   resultAdjFilenameXml);
}
         ...
         const std::string& pInputResourcesFolder)
{
  ...
  onsem::textsToResults(pLingDbTree, resultVerbFollByDeFilename, resultAdjFilename, resultVerbPrepFilename, pLingDb, pShareSemanticPath);
