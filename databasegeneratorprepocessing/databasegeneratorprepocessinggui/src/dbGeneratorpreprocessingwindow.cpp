...
void DbGeneratorPreprocessingWindow::on_pushButton_cptsFromLLM_clicked()
{
  _loadConeptFromLLM("location");
  _loadConeptFromLLM("object");
}


void DbGeneratorPreprocessingWindow::_loadConeptFromLLM(const std::string& pConceptName) const
{
  const std::string myDataMiningPath =
      _ui->lineEdit_MyDataMiningPath->text().toUtf8().constData();
  const auto locationsInfosPath = myDataMiningPath + "/resources/french/llm/noun_" + pConceptName + "_infos.txt";

  std::ifstream inFile(locationsInfosPath.c_str(), std::ifstream::in);
  if (!inFile.is_open())
  {
    std::cerr << "Error: Can't open " << locationsInfosPath << " file !" << std::endl;
    exit(1);
  }

  std::ofstream outFile(_inputResourcesFolder + "/french/concepts/" + pConceptName + "/concepts_" + pConceptName + "_from_llm.xml");
  outFile << "<linguisticdatabase>\n";
  outFile << " <concept name=\"" << pConceptName << "_*\">\n";
  std::string line;
  while (getline(inFile, line))
  {
    ...
    if (separator != std::string::npos &&
        ...
        line[separator + 1] == 'O')
    {
      auto word = line.substr(0, separator);
      outFile << "  <linkedMeaning lemme=\"" << word << "\" gram=\"noun\" relationToConcept=\"3\"/>\n";
    }
  }
...
void _writeSmallAnswer(std::ofstream& pOutFile,
                    const std::string& pQuestion,
                       const std::string& pWord)
{
  auto inJson = nlohmann::json::parse("{ \"model\": \"gpt-3.5-turbo\", \"messages\":[{\"role\":\"user\", \"content\":\"" + pQuestion + "\"}], \"max_tokens\": 1, \"temperature\": 0 }");
  auto chat = openai::chat().create(inJson);
  auto res = chat.dump(2);
  std::string resToWrite;
  if (res.find("\"content\": \"O") != std::string::npos)
    resToWrite = pWord + "|O";
  else
    resToWrite = pWord + "|Non";
  pOutFile << resToWrite << std::endl;
}


std::set<std::string> _getWordsOfLlmOutFile(const std::string& pFilename)
{
  ...
  while (getline(inFile, line))
  {
    std::size_t separator = line.find('|');
    if (separator != std::string::npos)
      words.insert(line.substr(0, separator));
  }
  inFile.close();
  return words;
}
...
void DbGeneratorPreprocessingWindow::on_pushButton_12_clicked()
{
  ...
  openai::start(""); // <= Add openai key here
  std::ofstream outLocationFile(locationFilename, std::ios::app);
  std::ofstream outObjectFile(objectFilename, std::ios::app);
  ...
  list_words_by_pos_for_a_langauge(_lingDbTree, SemanticLanguageEnum::FRENCH, [&](const std::string& pWord) {
    ...
    {
      bool locWrite = false;
      if (locationWords.count(pWord) == 0)
      {
        _writeSmallAnswer(outLocationFile, "Est-ce que \\\"" + pWord + "\\\" peut indiquer un lieu ?", pWord);
        locWrite = true;
      }
      bool objWrite = false;
      if (objectWords.count(pWord) == 0)
      {
        _writeSmallAnswer(outObjectFile, "Est-ce que \\\"" + pWord + "\\\" est un objet physique ?", pWord);
        objWrite = true;
      }
      if (locWrite || objWrite)
        std::cout << i << " - " << pWord << std::endl;
    }
  });
  outLocationFile.close();
  outObjectFile.close();
}
