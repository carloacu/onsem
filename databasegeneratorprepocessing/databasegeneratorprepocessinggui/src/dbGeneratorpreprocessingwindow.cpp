...
#include "listwordsbypos/listwordsbypos.hpp"
...
DbGeneratorPreprocessingWindow::DbGeneratorPreprocessingWindow(const LingdbTree& pLingDbTree,
...
void _conjAVerbTense(std::stringstream& pSs, const std::vector<WordLinkForConj>& pTense) {
...
void _conjImperativeVerbTense(std::stringstream& pSs, const std::vector<WordLinkForConj>& pTense) {
    if (pTense[0].node != nullptr) {
...
void DbGeneratorPreprocessingWindow::_conjAMeaning(std::string& pConj, LingdbMeaning* pMeaning) const {
...
void DbGeneratorPreprocessingWindow::refreshAMeaning(std::list<std::string>& pConceptsStr, int pIdMeaning) {
    fWordsToTagsCurrentState.meaning = nullptr;
    if (pIdMeaning >= 0) {
        const ForwardPtrList<LingdbWordForms>* wf = fWordsToTagsCurrentState.wordFroms;
        while (wf != nullptr) {
...
void DbGeneratorPreprocessingWindow::_refreshSuggestions(LingdbDynamicTrieNode* node) {
...
void DbGeneratorPreprocessingWindow::on_lineEdit_WordsToTags_Word1_Word_textChanged(const QString& arg1) {
    ...
    for (std::size_t i = 0; i < wf.meaningsStr.size(); ++i)
        _ui->listWidget_WordsToTags_Word1_Meanings->addItem(QString::fromUtf8(wf.meaningsStr[i].c_str()));
    _ui->listWidget_WordsToTags_Word1_Meanings->setCurrentRow(0);
    _ui->label_WordsToTags_Word1_Word_Suggestions->setText(QString::fromUtf8(wf.wordSuggestions.c_str()));

    _ui->textBrowser_WordsToTags_Conj->setText(QString::fromUtf8(wf.conjugaisons.c_str()));
}
...
void DbGeneratorPreprocessingWindow::on_listWidget_WordsToTags_Word1_Meanings_currentRowChanged(int currentRow) {
    ...
    for (std::list<std::string>::const_iterator it = conceptsStr.begin(); it != conceptsStr.end(); ++it) {
        _ui->listWidget_WordsToTags_Word1_Concepts->addItem(QString::fromUtf8(it->c_str()));
    }
}
...
void DbGeneratorPreprocessingWindow::_getFilenameToOpen(std::string& pFilename) {
    std::stringstream ss;
    ss << _getFilterDynDatabase() << ";;" << _getFilterRlaDatabase() << ";;" << _getFilterXmlDatabase() << ";;"
       << _getFilterDelaDatabase() << ";;" << _getFilterCptsDatabase();
    pFilename = QFileDialog::getOpenFileName(this, "Open a database", QString(), QString::fromUtf8(ss.str().c_str()))
                    .toUtf8()
                    .constData();
}
...
void DbGeneratorPreprocessingWindow::on_pushButton_words_open_clicked() {
    ...
    anyloader.open(_fileLoaded, _lingDbTree.getInputResourcesDir(), fWords, _lingDbTree);
...
void DbGeneratorPreprocessingWindow::_getFilenameToMerge(std::string& pFilename) {
    std::stringstream ss;
    ss << _getFilterXmlDatabase() << ";;" << _getFilterRlaDatabase() << ";;" << _getFilterDelaDatabase() << ";;"
       << _getFilterCptsDatabase() << ";;" << _getFilterGfsDatabase();
    pFilename = QFileDialog::getOpenFileName(
                    this, "Merge with another database", QString(), QString::fromUtf8(ss.str().c_str()))
                    .toUtf8()
                    .constData();
}
...
void DbGeneratorPreprocessingWindow::_getFilenameToSave(std::string& pFilename) {
    std::stringstream ss;
    ss << _getFilterDynDatabase() << ";;" << _getFilterBinaryDatabase();
    QString selectedFilter;
    QString fichier = QFileDialog::getSaveFileName(
        this, "Save the database", QString(), QString::fromUtf8(ss.str().c_str()), &selectedFilter);
...
void DbGeneratorPreprocessingWindow::on_pushButton_MemoryClearer_Refresh_clicked() {
    ...
    _ui->textBrowser_MemoryClearer_Display->setText(QString::fromUtf8(ss.str().c_str()));
}

void DbGeneratorPreprocessingWindow::on_pushButton_process_auxiliariesextractor_clicked() {
    const std::string inPath = _ui->lineEdit_auxilitiesextractorInputPath->text().toUtf8().constData();
...
void DbGeneratorPreprocessingWindow::on_pushButton_2_clicked() {
    ...
    delaLoader.simplifyDelaFile(_inputResourcesFolder + "/french/readonly/french_dela.dela",
    ...
    std::set<std::string> enLemmaToKeep{
        "acorn squash",   "banana squash", "bell pepper",      "bitter melon",   "black bean",
        "black-eyed pea", "broad bean",    "Brussel's sprout", "cayenne pepper", "chili pepper",
        "green bean",     "green onion",   "kidney bean",      "lima bean",      "mangel-wurzel",
        "mung bean",      "navy bean",     "pinto bean",       "root vegetable", "runner bean",
        "split pea",      "spring onion",  "sweet corn",       "sweet potato",   "water chestnut"};
    delaLoader.simplifyDelaFile(_inputResourcesFolder + "/english/readonly/english_dela.dela",
...
void DbGeneratorPreprocessingWindow::on_pushButton_4_clicked() {
    const std::string myDataMiningPath = _ui->lineEdit_MyDataMiningPath->text().toUtf8().constData();
...
struct WordKindness {
    WordKindness(std::size_t pKindness, std::size_t pWickedness)
        : kindness(pKindness)
...
std::size_t _calculateRevelance(
...
void _loadXmlDbPediaFile(
    std::map<std::size_t, std::list<OutLine>>& pTextsSorted,
        ...
        if (resultsTreeOpt) {
            ...
            for (const auto& currResult : *resultsTreeOpt) {
                ...
                if (!referenceStr.empty() && (!valueFrStr.empty() || !valueEnStr.empty())) {
                    ...
                    std::size_t revelance = _calculateRevelance(kindness,
                                                                wickedness,
                                                                wikiPageLength,
                                                                outLine.valueStr,
                                                                pLanguageToLemmaWithKindness,
                                                                language,
                                                                pLingDb);
                    std::stringstream revelancePrettyPrintSs;
                    revelancePrettyPrintSs << revelance << "=" << wikiPageLength << "*" << kindness << "/"
                                           << wickedness;
...
void DbGeneratorPreprocessingWindow::on_pushButton_dbpedia_xml_to_txt_clicked() {
    const std::filesystem::path fromFile = _ui->lineEdit_dbpedia_from_file->text().toUtf8().constData();
    ...
    const std::string toPath = _ui->lineEdit_dbpedia_to_path->text().toUtf8().constData();
...
void DbGeneratorPreprocessingWindow::on_pushButton_11_clicked() {
    onsem::list_words_by_pos(_lingDbTree);
}
