...
void AuxiliariesExtractor::processFile
(const std::string& pInFilename)
{
  ...
  {
    workers.push_back(std::thread(&AuxiliariesExtractor::_processTexts, this,
                                  textsToProccess[i], language, i == 0));
  }
  for (auto& th : workers)
  {
    th.join();
...

  {
    ...
    for (const auto& currVerbRes : _vertoToAuxMeaningRes)
    {
      ...
      if (currRes.nbFoundWithBeAux > 10 &&
          currRes.nbFoundWithBeAux * 2 > currRes.nbFound)
      {
        _verbsWithBeAux.emplace(lemmaStr);
      }
      if (currRes.nbFoundWithHaveAux * 150 > currRes.nbFound ||
          currRes.nbFoundWithHaveAux > 50)
      {
...
void AuxiliariesExtractor::_processTexts
 ...
 bool pPrintLineNumber)
{
  ...
  for (const auto& currText : pTexts)
  {
    ...
    linguistics::tokenizationAndSyntacticalAnalysis(syntGraph, currText, spellingMistakeTypesPossible,
      ...
      if (chunk.type == linguistics::ChunkType::VERB_CHUNK)
      {
        ...
        if (!linguistics::InflectionsChecker::verbIsAtPresentParticiple(verbIGram))
        {
          if (verbIGram.word.language == pLanguage)
          {
            ...
            ++auxRes.nbFound;
            const linguistics::Chunk* auxChk =
                _getAuxiliaryChunk(chunk);
            if (auxChk != nullptr)
            {
