...
struct SubordonateSyntGraphWorkingContext {
    SubordonateSyntGraphWorkingContext(Chunk* pChunk, TokensTree& pTokenTree)
        : chunk(pChunk)
        , tokenTree(pTokenTree)
        , firstChildren() {}

    SubordonateSyntGraphWorkingContext(const SubordonateSyntGraphWorkingContext& pOther)
        : chunk(pOther.chunk)
...
private:
    SubordonateSyntGraphWorkingContext& operator=(const SubordonateSyntGraphWorkingContext& pOther);
};

void _iterateOnParenthesis(std::list<SubordonateSyntGraphWorkingContext>& pSubordonatesOfChunks,
    ...
    for (ChunkLink& currChunkLink : pChunkLinks) {
        ...
        for (auto itTok = currChunkLink.chunk->tokRange.getItBegin(); itTok != currChunkLink.chunk->tokRange.getItEnd();
             ++itTok) {
            if (itTok->subTokens != nullptr && !itTok->subTokens->tokens.empty()
        ...
        _iterateOnParenthesis(pSubordonatesOfChunks, pPrevChunks, pLangConfig, currChunkLink.chunk->children);
    }
}
...
void _fillPassiveInformation(std::list<ChunkLink>& pChunkLinks, const AlgorithmSetForALanguage& pLangConfig) {
...
void _constructSyntacticTree(TokensTree& pTokensTree,
    ...
    if (!subExtFinished
        || pEndingStep.doWeShouldStopForDebug(LinguisticAnalysisFinishDebugStepEnum::SUBORDONATEEXTRACTOR)) {
        return;
    }
    ...
    // Consider the texts in parenthesis
    {
        ...
        for (auto it = subordonatesOfChunks.begin(); it != subordonatesOfChunks.end(); ++it) {
            _launchGrammRules(it->tokenTree,
                              it->firstChildren,
                              pParsingConfidence,
                              pLangConfig,
                              pSpecLingDb,
            ...
            auto _chunckCanBeLinkedToASubject = [&](const Chunk& pChunk) {
                return pChunk.type == ChunkType::VERB_CHUNK
                    && canLinkVerbToASubject(whereToLinkTheSubject(pChunk), currChunk, inflChecker, true);
            };
...
void tokenizeText(TokensTree& pTokensTree,
                  ...
                  const std::string& pInputSentence,
                  const std::shared_ptr<ResourceGroundingExtractor>& pCmdGrdExtractorPtr) {
    tokenizer::tokenize(pTokensTree.tokens,
                        pInputSentence,
...
UniqueSemanticExpression convertToSemExp(const SyntacticGraph& pSyntGraph,
    ...
    return semTranslator->process(pSyntGraph, pLocutionContext, pTimeGrd, std::move(pAgentWeAreTalkingAbout));
}
...
void extractProperNounsThatDoesntHaveAnyOtherGrammaticalTypes(std::set<std::string>& pNewProperNouns,
    ...
    for (const auto& currToken : tokens) {
        ...
        if (currToken.inflWords.size() == 1 && inflWordFrom.word.partOfSpeech == PartOfSpeech::PROPER_NOUN
            && inflWordFrom.inflections().type == InflectionType::EMPTY && beginWithUppercase(currToken.str)) {
            ...
            if (!tokensOfTheProperNoun.empty() && tokensOfTheProperNoun.front().inflWords.size() == 1)
