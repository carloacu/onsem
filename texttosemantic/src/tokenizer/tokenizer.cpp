...
struct StrReplacement {
    StrReplacement(std::string pOldStr, std::string pNewStr)
        : oldStr(pOldStr)
        , newStr(pNewStr) {}
...
struct TextForms {
    ...
    std::string getSubInputText(std::size_t pBegin, std::size_t pLength) const {
...
private:
    void _copy(std::size_t pBegin, std::size_t pEnd) {
    ...
    void _replace(std::size_t pBegin, const std::string& pNewSubStr) {
...
struct TokSent {
     ...
     */
    TokSent(std::list<Token>& pTokens, const std::string& pInputSentence)
        : tokens(pTokens)
...
void _listOfTokensToVectorOfTokens(std::vector<Token>& pVectorOfkens,
    ...
    for (const Token& currTok : pListOfTokens) {
        ...
        if (currTok.subTokens) {
            ...
            _listOfTokensToVectorOfTokens(newTok.subTokens->tokens, currTok.subTokens->tokens, &newTok.tokenPos);
        }
        // is empty
...
bool _mergeIfItsAComposedWord(std::list<Token>& pTokens,
    ...
    if (pAreTokensCompatible(it->inflWords.front().word.partOfSpeech, pInfosGram.front().word.partOfSpeech)) {
        for (std::list<InflectedWord>::iterator itIGram = pInfosGram.begin(); itIGram != pInfosGram.end(); ++itIGram) {
            switch (itIGram->word.partOfSpeech) {
                ...
                case PartOfSpeech::ADJECTIVE: {
                    ...
                    if (pInfosGram.front().word.partOfSpeech != PartOfSpeech::PROPER_NOUN
                        ...
                        auto itInflWord =
                            getInflWordWithASpecificPartOfSpeech(it->inflWords, itIGram->word.partOfSpeech);
                ...
                case PartOfSpeech::PRONOUN: return false;
                default: break;
            }
        }
    }
...
void _insertTokenWithStr(TokSent& pTokSent,
                         std::size_t pLength,
                         std::list<InflectedWord>& pInfosGram,
                         const std::string& pTokStr,
    ...
    bool originalWordBeginWithUpperCase = pTokSent.textForms.isInputTextAtUpperCase(pTokSent.posAfterPreviousToken);
    if (originalWordBeginWithUpperCase && pInfosGram.size() == 1
    ...
    if (pTokSent.tokens.size() >= 2) {
        if (pTokSent.tokens.back().str == "-" || pTokSent.tokens.back().str == "/")
            hasBeenMergedWithPreviousToken = _mergeIfItsAComposedWord(
                pTokSent.tokens, pTokStr, pInfosGram, [](PartOfSpeech pOs1, PartOfSpeech pOs2) {
                    return partOfSpeech_isAWord(pOs1)
                        && (!partOfSpeech_isVerbal(pOs1) || pOs2 == PartOfSpeech::PROPER_NOUN);
                });
        else if (pTokSent.tokens.back().str == "'")
            hasBeenMergedWithPreviousToken =
                _mergeIfItsAComposedWord(pTokSent.tokens, pTokStr, pInfosGram, [](PartOfSpeech pOs, PartOfSpeech) {
                    return pOs == PartOfSpeech::PROPER_NOUN;
                });
    }
    ...
    pTokSent.tokens.back().inflWords.splice(pTokSent.tokens.back().inflWords.begin(), pInfosGram);
 ...
 */
void _insertToken(TokSent& pTokSent,
                  std::size_t pLength,
                  std::list<InflectedWord>& pInfosGram,
                  SemanticLanguageEnum pLanguage) {
...
bool _tryToMergeUrlTokens(TokSent& pTokSent, const std::string& pExtension) {
 ...
 * @param pLingDico Lingistic dictionary.
 */
void _fushPrevCharactersInAToken(TokSent& pTokSent, bool pSearchGramType, const LinguisticDictionary& pLingDico) {
    std::list<InflectedWord> infosGram;
    // if we have to search the grammatical possibilites of the word in the database
    if (pSearchGramType) {
        pLingDico.getGramPossibilitiesAndPutUnknownIfNothingFound(infosGram,
                                                                  pTokSent.textForms.formattedText,
                                                                  pTokSent.posAfterPreviousToken,
                                                                  pTokSent.currPos - pTokSent.posAfterPreviousToken);
        if (!pTokSent.tokens.empty() && pTokSent.tokens.back().str == "." && !infosGram.empty()) {
            const auto& extension = infosGram.front().word.lemma;
            if (canBeTheExtensionOfAnUrl(extension) && _tryToMergeUrlTokens(pTokSent, extension)) {
                _refreshReferencePositionsafterAnInsertionWithLength(pTokSent,
                                                                     pTokSent.currPos - pTokSent.posAfterPreviousToken);
                return;
            }
        }
...
void _inParenthesis(TokSent& pTokSent,
                    const LinguisticDictionary& pLingDico,
                    const LinguisticDictionary& pCommonLingDico) {
    const std::size_t endingFound = getEndOfParenthesis(
    ...
    if (endingFound != std::string::npos) {
        ...
        if (begOfParenthesisChar == '(') {
            ...
            textInflWord.back().word.lemma =
                pTokSent.textForms.formattedText.substr(beginInsideParenthesis, lengthInsideParenthesis);
...
std::size_t _getMaxLength(TokSent& pTokSent,
    ...
    if (pTokSent.canBeAWordAtUppercase) {
        std::size_t res = pLingDico.getLengthOfLongestWord(pTokSent.textForms.formattedText, pTokSent.currPos);
        if (pLingDico.statDb.getLanguageType() != SemanticLanguageEnum::UNKNOWN
            ...
            std::size_t commonBinDicoLen =
                pCommonLingDico.getLengthOfLongestWord(pTokSent.textForms.formattedText, pTokSent.currPos);
...
void _inComplexQuotationMarks(TokSent& pTokSent, const LinguisticDictionary& pLingDico) {
...
bool _addTokenFollowedByASeparator(TokSent& pTokSent,
    ...
    if (partOfSpeech_isAWord(wordPartOfSpeech)) {
        ...
        if (pTokSent.currPos == pTokSent.posAfterPreviousToken) {
            ...
            const std::size_t longestSep =
                pLingDico.getLengthOfLongestWord(pTokSent.textForms.formattedText, pTokSent.currPos + pLongestWord);
            std::list<InflectedWord> infosGramSep;
            pLingDico.getGramPossibilitiesAndPutUnknownIfNothingFound(
                infosGramSep, pTokSent.textForms.formattedText, pTokSent.currPos + pLongestWord, longestSep);
    ...
    } else    // if the current token is a separator
    {
        ...
        if (currChar != ' '
            ...
            return false;
        if (currChar == '.' && pTokSent.currPos + 1 < pTokSent.endPosToTokenize
            && pTokSent.textForms.formattedText[pTokSent.currPos + 1] == ',')
            return false;
...
void _fillInflWordOfHour(std::list<InflectedWord>& pInflWord, SemanticLanguageEnum pLanguage) {
    // a number can be a determiner or a noun
    pInflWord.emplace_back([&] {
        InflectedWord nounIGram(PartOfSpeech::NOUN,
                                pLanguage == SemanticLanguageEnum::ENGLISH ? NominalInflections::get_inflections_ns()
...
void _addNumberOrHourToken(TokSent& pTokSent,
                           std::size_t pPosBegin,
                           std::size_t pPosEnd,
                           SemanticLanguageEnum pLanguage,
                           bool pIsAnHour,
    ...
    if (pEndOfNumberOpt) {
        ...
        const std::string tokNumberStr =
            pTokSent.textForms.formattedText.substr(pTokSent.posAfterPreviousToken, numberLength);
        if (pIsAnHour)
 ...
 * @param pLingDico Lingistic dictionary.
 */
void _tryToTokenizeANumber(TokSent& pTokSent, const LinguisticDictionary& pLingDico) {
    ...
    while (pTokSent.currPos < pTokSent.endPosToTokenize) {
        ...
        if (pTokSent.currPos > pTokSent.posAfterPreviousToken) {
                ...
                if (pTokSent.currPos + 2 <= pTokSent.endPosToTokenize) {
                    if (pTokSent.textForms.formattedText.compare(pTokSent.currPos, 2, "th") == 0) {
                        ...
                        if (afterSuffixPos == pTokSent.endPosToTokenize) {
                            _addNumberOrHourToken(
                                pTokSent, pTokSent.posAfterPreviousToken, pTokSent.currPos, languageType, isAnHour);
                        } else {
                            const std::size_t longestWord =
                                pLingDico.getLengthOfLongestWord(pTokSent.textForms.formattedText, afterSuffixPos);
                            if (longestWord > 0) {
                                std::list<InflectedWord> infosGramSep;
                                pLingDico.getGramPossibilitiesAndPutUnknownIfNothingFound(
            ...
            if (pTokSent.currPos == pTokSent.endPosToTokenize || isAPercentage || isADegree) {
                _addNumberOrHourToken(
                    pTokSent, pTokSent.posAfterPreviousToken, pTokSent.currPos, languageType, isAnHour, endOfNumber);
            } else {
                const std::size_t longestWord =
                    pLingDico.getLengthOfLongestWord(pTokSent.textForms.formattedText, pTokSent.currPos);
                if (longestWord > 0) {
                    std::list<InflectedWord> infosGramSep;
                    pLingDico.getGramPossibilitiesAndPutUnknownIfNothingFound(
    ...
    while (pTokSent.currPos < pTokSent.endPosToTokenize) {
        ...
        if (longestWord > 0) {
            pLingDico.getGramPossibilities(
                infosGramWord, pTokSent.textForms.formattedText, pTokSent.currPos, longestWord);
            if (pLingDico.statDb.getLanguageType() != SemanticLanguageEnum::UNKNOWN
        ...
        if (pLingDico.statDb.haveSeparatorBetweenWords()) {
            ...
            if (longestWord > 0 && !_addTokenFollowedByASeparator(pTokSent, longestWord, infosGramWord, pLingDico)
                && pTokSent.canBeAWordAtUppercase
                && lowerCaseFirstLetter(pTokSent.textForms.formattedText, pTokSent.currPos)) {
            ...
            if (pTokSent.currPos == prevCurrPos && pTokSent.currPos == pTokSent.posAfterPreviousToken) {
        ...
        {
            _addTokenNotFollowedByASeparator(pTokSent, longestWord, infosGramWord, pLingDico);
        }
...
void _insertBookmark(std::list<Token>& pTokens,
    ...
    } else if (pCmdGrdExtractorPtr && pCmdGrdExtractorPtr->isBeginOfAResource(bookmark)) {
        newInflWord.word.partOfSpeech = PartOfSpeech::UNKNOWN;
        newInflWord.moveInflections(_getAllNounInflections(pLanguage));
    } else {
