...
void process(std::vector<Token>& pTokens,
    ...
    while (keepInLoop) {
        ...
        for (const auto& currRulePtr : contextFilters) {
            ...
            if (pNbOfDebugRounds <= 1 && currRule.getName() == pEndingStep) {
                keepInLoop = false;
                break;
            }
            currId++;
    ...
    switch (pSpecLingDb.language) {
        case SemanticLanguageEnum::ENGLISH: {
            PartOfSpeechCustomFilter genTagger("noun at bottom if two times in a row exept for verb",
                                               pSpecLingDb.inflectionsChecker(),
                                               pSpecLingDb,
