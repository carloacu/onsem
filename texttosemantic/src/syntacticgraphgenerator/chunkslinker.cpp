...
void ChunksLinker::_linkPronounToMorePreciseSubject(
    ...
    std::list<ChunkLink>& pFirstChildren) const
{
  if (pChunkLink.chunk->type == ChunkType::VERB_CHUNK ||
      pChunkLink.chunk->type == ChunkType::INFINITVE_VERB_CHUNK)
  ...
  for (ChunkLink& currChkLk : pChunkLink.chunk->children)
  {
    ...
    {
      if (headPartOfSpeech == PartOfSpeech::PRONOUN ||
          headPartOfSpeech == PartOfSpeech::PRONOUN_COMPLEMENT)
        _tryToLinkToAPrevWord(currChkLk, pHistory, pStack);
      else
...
void ChunksLinker::_tryToLinkToAPrevWord(
    ...
    std::list<Chunk*>& pStack) const
{
  ...
  if (pHistory.prevNominal[1].chLk &&
      _canLinkWith(*pHistory.prevNominal[1].chLk->it, true))
  {
  ...
  else if (pHistory.prevSubject[1] != nullptr &&
           _canLinkWith(*pHistory.prevSubject[1], false))
    _linkPronounToReferentChunk(pPronounChkLk, *pHistory.prevSubject[1]);
  else if (pHistory.prevDO[1] != nullptr &&
           _canLinkWith(*pHistory.prevDO[1], false))
    _linkPronounToReferentChunk(pPronounChkLk, *pHistory.prevDO[1]);
  else if (pHistory.prevDO[0] != nullptr &&
           _canLinkWith(*pHistory.prevDO[0], false))
    _linkPronounToReferentChunk(pPronounChkLk, *pHistory.prevDO[0]);
}



bool _pronCanSubstituteAInflWord(const InflectedWord& pPronToSubstitute,
...
bool ChunksLinker::canSubstituteAChunkByAnother(
    ...
    bool pRefChunkIsAtRoot) const
{
  ...
  if (!pRefChunkIsAtRoot &&
      pronToSubstitute.infos.hasContextualInfo(WordContextualInfos::REFTOASENTENCE))
    return false;

  if (chunkTypeIsAList(pRefChunk.type))
  {
    if (flsChecker.pronounSetAt3ePers(pronToSubstitute) &&
        flsChecker.pronounCanBePlural(pronToSubstitute))
    {
...
void ChunksLinker::_linkPronounToReferentChunk(
    ...
    ChunkLink& pReferentChunkLink)
{
  ...
  if (inflRefWord.word.partOfSpeech == PartOfSpeech::PROPER_NOUN)
  {
    ...
    if (properNounInflections == nullptr ||
        properNounInflections->inflections.empty())
    {
      ...
      if (inflsPronounPtr != nullptr)
        inflRefWord.moveInflections(pronInflWord.inflections().getOtherInflectionsType(InflectionType::NOMINAL));
    }
  }
...
void ChunksLinker::_saveTheLink(
    ChunkLink*& pSaveLink,
    ChunkLink& pSubject) const
{
  const InflectedWord& subjectHeadIGram = pSubject.chunk->head->inflWords.front();
  if (chunkTypeIsAList(pSubject.chunk->type) ||
      subjectHeadIGram.word.partOfSpeech == PartOfSpeech::NOUN ||
