...
bool _canBeATimeSpecification(const Chunk& pChunk) {
    return pChunk.type != ChunkType::VERB_CHUNK || pChunk.requests.empty();
}

bool _setItThenAndItThenContent(std::list<ChunkLink>::iterator& pIt,
...
SubordinateExtractor::SubordinateExtractor(const AlgorithmSetForALanguage& pConfiguration)
    : fConf(pConfiguration)
    , fSemFrameDict(fConf.getSpecifcLingDb().getSemFrameDict())
    , fLingDico(fConf.getLingDico()) {}

bool SubordinateExtractor::process(std::list<ChunkLink>& pFirstChildren,
...
bool _isAChunkLinkOkForVerbModification(ChunkLinkType pChunkLinkType) {
    return pChunkLinkType == ChunkLinkType::SIMPLE || pChunkLinkType == ChunkLinkType::DIRECTOBJECT
        || pChunkLinkType == ChunkLinkType::SUBORDINATE_CLAUSE || pChunkLinkType == ChunkLinkType::PURPOSE_OF;
}

void SubordinateExtractor::xModifyTheVerbsOfAChunk(std::shared_ptr<Chunk>& pChunk,
                                                   ChunkLinkType pFatherChunkLinkType,
                                                   ChunkType pPrevChunkType) const {
    Chunk& chunk = *pChunk;
    switch (chunk.type) {
        case ChunkType::VERB_CHUNK: {
            if (chunk.requests.empty() && !haveASubject(chunk) && pPrevChunkType != ChunkType::INFINITVE_VERB_CHUNK) {
        ...
        case ChunkType::THEN_REVERSED_CHUNK: {
            ...
            if (beginItChild->chunk->type == ChunkType::INFINITVE_VERB_CHUNK && chunk.children.size() > 1) {
                ...
                ++itChild;
                {
                    ...
                    if (currChunkChild.type == ChunkType::VERB_CHUNK && pFatherChunkLinkType == ChunkLinkType::SIMPLE
                        && chunk.children.size() > 2 && doesChunkIsOrCanBeAtImperative(currChunkChild)) {
...
void SubordinateExtractor::xModifyTheVerbsForStatementChildren(std::list<ChunkLink>& pChunkList) const {
...
void SubordinateExtractor::xModifyTheVerbsForList(std::list<ChunkLink>& pChunkList,
...
void SubordinateExtractor::xLinkComplementaryNominalChunks(std::list<ChunkLink>& pChunkLinks) const {
    SemanticLanguageEnum language = fConf.getLanguageType();
    ...
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        if (it->chunk->type == ChunkType::PREPOSITIONAL_CHUNK && chunkCanBeAnObject(*it->chunk)) {
            auto linkType = ChunkLinkType::COMPLEMENT;
            if (ConceptSet::haveAConceptThatBeginWith(
                    it->chunk->tokRange.getItBegin()->inflWords.front().infos.concepts, "location_")) {
...
void SubordinateExtractor::xResolveBadObjectRequests(std::list<ChunkLink>& pChunkLinks) const {
    ...
    for (const auto& currChkLk : pChunkLinks) {
        ...
        if (currChunk.type == ChunkType::VERB_CHUNK && currChunk.requests.has(SemanticRequestType::OBJECT)) {
            ...
            if (subjectChunkPtr != nullptr && checkOrder(*subjectChunkPtr, currChunk)) {
                if (objectChunkPtr != nullptr && checkOrder(*subjectChunkPtr, *objectChunkPtr)
                    && (language == SemanticLanguageEnum::ENGLISH
                        || subjectChunkPtr->head->inflWords.front().infos.hasContextualInfo(
                    ...
                    currChunk.requests.set(SemanticRequestType::SUBJECT);
                           ...
                           && ConceptSet::haveAConceptThatBeginWith(currChunk.head->inflWords.begin()->infos.concepts,
                                                                    "verb_equal_")) {
...
void SubordinateExtractor::xLinkComplementaryNominalChunksSeparatredByCommas(std::list<ChunkLink>& pChunkLinks) const {
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        ChunkLink& currChunkLink = *it;
        Chunk& currChunk = *currChunkLink.chunk;
        PartOfSpeech currHeadPartOfSpeech = currChunk.head->inflWords.front().word.partOfSpeech;
                            ...
                            if (recInListConst(
                                    [](const Chunk& pChunk) {
                                        if (pChunk.type == ChunkType::NOMINAL_CHUNK) {
                                            ...
                                            return (InflectionsChecker::verbIsAtPresentParticiple(
                                                        pChunk.head->inflWords.front())
                                                    && !haveASubject(pChunk));
                                        }
                                        return false;
                                    },
            ...
            if (linkSubNominalChunks) {
                ...
                if (currChunk.type == ChunkType::NOMINAL_CHUNK) {
                    if (subNominalChunks.size() == 1) {
                        ...
                        xAddASubChunk(pChunkLinks,
                                      currChunk,
                                      frontElt.sepChunk,
                                      frontElt.nominalChunk,
                                      ChunkLinkType::SPECIFICATION,
                                      language);
                    }
                    ...
                    if (doChunkPtr == nullptr) {
                        for (auto& currSubNominalChunk : subNominalChunks)
                            xAddASubChunk(pChunkLinks,
                                          currChunk,
                                          currSubNominalChunk.sepChunk,
                                          currSubNominalChunk.nominalChunk,
...
void SubordinateExtractor::xLinkSubordonateVerbs(std::list<ChunkLink>& pChunkLinks) const {
    ...
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end();) {
        ...
        afterASeparator =
            it->chunk->type == ChunkType::SEPARATOR_CHUNK
            && (headOfChkWord.partOfSpeech == PartOfSpeech::LINKBETWEENWORDS
                || (headOfChkWord.partOfSpeech == PartOfSpeech::SUBORDINATING_CONJONCTION
                    && fLingDico.statDb.inflWordsToQuestionWord(headOfChkInflWords, true, false) == nullptr));
        ++it;
    }
}

void SubordinateExtractor::xLinkSubordinatesThatBeginWithAPreposition(std::list<ChunkLink>& pChunkLinks) const {
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end();) {
        if (chunkTypeIsVerbal(it->chunk->type)) {
            ...
            if (itAfter == pChunkLinks.end())
                return;
            if (itAfter->chunk->type == ChunkType::PREPOSITIONAL_CHUNK && chunkCanBeAnObject(*itAfter->chunk)) {
...
void SubordinateExtractor::xLinkSubjectInfinitiveVerbs(std::list<ChunkLink>& pChunkLinks) const {
    ...
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end();) {
        if (!previousChunkWasPrepositinal) {
            ...
            if (isAtInfinitive) {
                ...
                } else if (language == SemanticLanguageEnum::ENGLISH && nextChunk.type == ChunkType::SEPARATOR_CHUNK
...
void SubordinateExtractor::xLinkSubordonateOfCondition(std::list<ChunkLink>& pChunkLinks) const {
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        ...
        if (currChunk.type == ChunkType::SEPARATOR_CHUNK) {
            ...
            if (headInflWorld.infos.hasContextualInfo(WordContextualInfos::CONDITION)) {
                ++it;
                if (it == pChunkLinks.end() || !chunkTypeIsVerbal(it->chunk->type))
                    return;

                auto itEnd = it;
                ++itEnd;
                if (itEnd == pChunkLinks.end() || !chunkTypeIsVerbal(itEnd->chunk->type))
                    return;
                ++itEnd;
                if (itEnd == pChunkLinks.end()
                    || !(chunkTypeIsVerbal(itEnd->chunk->type)
                         || (itEnd->chunk->type == ChunkType::SEPARATOR_CHUNK
                             && itEnd->chunk->head->inflWords.front().infos.hasContextualInfo(
                                 WordContextualInfos::THEN))))
                    return;
...
void SubordinateExtractor::xLinkSubordinatesAtTheBeginning(std::list<ChunkLink>& pChunkLinks) const {
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        ...
        if (currChunk.type == ChunkType::PREPOSITIONAL_CHUNK && chunkCanBeAnObject(currChunk)) {
            auto itNext = it;
            ++itNext;
            if (itNext == pChunkLinks.end())
                break;
...
void SubordinateExtractor::xLinkElseSubordonates(std::list<ChunkLink>& pChunkLinks) const {
...
void SubordinateExtractor::xLinkSubordonateThatAreBeforeVerbs(std::list<ChunkLink>& pChunkLinks) const {
    for (auto it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        Chunk& currChunk = *it->chunk;
        // if we are at a separator node
        switch (currChunk.type) {
            case ChunkType::AND_CHUNK: {
                ...
                if (_isBeginOfTimeSubordinateBeforeVerb(subWordInlf, fSemFrameDict)) {
                    ...
                    ChunkLinkType ifContentChunkLinkType =
                        _getChunkLinkTypeOfCondition(subWordInlf, fSemFrameDict, *itThenContent->chunk);
                    itThenContent->chunk->children.splice(
                        itThenContent->chunk->children.begin(), currChunk.children, itLastListEltChkLk);
                ...
                if (_isBeginOfTimeSubordinateBeforeVerb(subWordInlf, fSemFrameDict)) {
                    ...
                    // get "then content" node
                    {
                        ...
                        ChunkLinkType ifContentChunkLinkType =
                            _getChunkLinkTypeOfCondition(subWordInlf, fSemFrameDict, *itThenContent->chunk);
                        itThenContent->chunk->children.splice(
                            itThenContent->chunk->children.begin(), pChunkLinks, itIfContent);
                        ChunkLink& ifContent = *itThenContent->chunk->children.begin();
                        ifContent.type = ifContentChunkLinkType;
                ...
                if (ConceptSet::haveAConceptThatBeginWith(it->chunk->getHeadConcepts(), "time_")) {
                    ...
                    if (nextIt->chunk->type == ChunkType::SEPARATOR_CHUNK) {
                        ...
                        if (nextNextIt->chunk->type == ChunkType::VERB_CHUNK) {
                            ...
                            nextNextIt->chunk->children.splice(
                                nextNextIt->chunk->children.begin(), pChunkLinks, timeChunkIt);
                        ...
                        timeChunkIt->type =
                            fConf.getEntityRecognizer().findNatureOfAChunkLink(*timeChunkIt, &*nextIt->chunk, false);
                        // Move sub time children into the children of the verb
                        {
                            ...
                            if (itTimeChildren != timeChunkIt->chunk->children.end())
                                nextIt->chunk->children.splice(
                                    nextIt->chunk->children.begin(), timeChunkIt->chunk->children, itTimeChildren);
                        }
...
bool SubordinateExtractor::xTryToLinkASubordonateVerb(std::list<ChunkLink>::iterator pChunkSep,
    ...
    if (pChunkSep != pSyntTree.begin()) {
        ...
        if (pIt->chunk->type == ChunkType::VERB_CHUNK && !haveASubject(*pIt->chunk)) {
            const InflectedWord& verbIGram = pIt->chunk->head->inflWords.front();
            // if the verb is at the past participle
            if (fls.verbIsAtPastParticiple(verbIGram) || fls.verbIsAtPresentParticiple(verbIGram)) {
        ...
        // if we succeed to link it to the previous chunk
        else if (chunkTypeIsAList(pIt->chunk->type) && pIt->chunk->children.front().chunk->type == ChunkType::VERB_CHUNK
            ...
            if (parentVerbChunkPtr != nullptr) {
                ...
                if (doChkLk != nullptr) {
                    if (pChunkSep->chunk->head->inflWords.front().word.partOfSpeech
...
bool SubordinateExtractor::xAddOneOrAListofPastParticipleVerbs(std::list<ChunkLink>& pSyntTree,
                                                               std::list<ChunkLink>::iterator& pIt,
    ...
    if (pNewPotentialRootChunk.type == ChunkType::NOMINAL_CHUNK
        && canLinkVerbToASubject(*pIt->chunk, pNewPotentialRootChunk, fConf.getFlsChecker(), true)) {
        xAddASubChunk(pSyntTree, pNewPotentialRootChunk, pPrev, pIt, ChunkLinkType::SUBJECT_OF, language);
        pIt = nexIt;
        return true;
        ...
        if (dObjPtr != nullptr) {
            Chunk& dObj = *dObjPtr;
            if (chunkTypeIsAList(dObj.type) && dObj.children.size() >= 1) {
                return xAddOneOrAListofPastParticipleVerbs(pSyntTree, pIt, pPrev, *dObj.children.back().chunk);
            } else if (canLinkVerbToASubject(*pIt->chunk, dObj, fConf.getFlsChecker(), true)) {
            ...
            if (subj != nullptr && checkOrder(pNewPotentialRootChunk, *subj)
                && canLinkVerbToASubject(*pIt->chunk, *subj, fConf.getFlsChecker(), true)) {
...
void SubordinateExtractor::xAddASubChunk(std::list<ChunkLink>& pSyntTree,
                                         Chunk& pMotherCunk,
                                         std::list<ChunkLink>::iterator pPrev,
                                         std::list<ChunkLink>::iterator pIt,
                                         ChunkLinkType pType,
    ...
    pIt->type = pType;
    if (pIt->chunk->type == ChunkType::PREPOSITIONAL_CHUNK && pType != ChunkLinkType::SIMPLE) {
...
void SubordinateExtractor::xLinkSubjectAndAuxiliaryToVerbsInsideAList(std::list<ChunkLink>& pChunkLinks) const {
    for (std::list<ChunkLink>::iterator it = pChunkLinks.begin(); it != pChunkLinks.end(); ++it) {
        if (chunkTypeIsAList(it->chunk->type) && !it->chunk->children.empty()) {
            ...
            for (auto itList = ++itFirstElt; itList != it->chunk->children.end(); ++itList) {
                if (itList->chunk->type != ChunkType::VERB_CHUNK || !itList->chunk->requests.empty()
                ...
                if (auxChunkPtr == nullptr) {
                    ...
                    } else if (canLinkVerbToASubject(
                                   *itList->chunk, *subjectChunkLink->chunk, fConf.getFlsChecker(), true)) {
                        subjectChunkLink->chunk->hasOnlyOneReference = false;
                        itList->chunk->children.emplace_back(*subjectChunkLink);
                    }
                } else if (canLinkVerbToASubject(*auxChunkPtr, *subjectChunkLink->chunk, fConf.getFlsChecker(), true)) {
                    subjectChunkLink->chunk->hasOnlyOneReference = false;
...
void SubordinateExtractor::xComparisonsFinder(std::list<ChunkLink>& pChunkLinks) const {
        ...
        if (currChunk.type == ChunkType::VERB_CHUNK
            ...
            if (itDO != currChunk.children.end()) {
                ...
                if (itSub != currChunk.children.end()) {
                    ...
                    for (TokIt itTokAfterVerb = getNextToken(currChunk.head, itEnd); itTokAfterVerb != itEnd;
                         itTokAfterVerb = getNextToken(itTokAfterVerb, itEnd)) {
                    ...
                    if (!isAComparison) {
                        ...
                        if (doInfosGram.word.partOfSpeech == PartOfSpeech::ADJECTIVE) {
                            if (langType == SemanticLanguageEnum::ENGLISH && fls.adjCanBeComparative(doInfosGram)) {
                                isAComparison = true;
                                compPolarity = ComparisonOperator::MORE;
                            } else if (ConceptSet::haveAConcept(doInfosGram.infos.concepts, "comparison_equal")) {
                                isAComparison = true;
                                compPolarity =
                                    currChunk.positive ? ComparisonOperator::EQUAL : ComparisonOperator::DIFFERENT;
                            }
                        }
                    }
