    ...
    const SemanticFrameDictionary& pFrameDic)
{
  ...
  else if (pQuestionWord.request == SemanticRequestType::SUBJECT &&
           pQuestionWord.canBeAlone)
  {
  ...
  if (!pCurrChunkLink.tokRange.isEmpty())
  {
    ...
    auto chunkLinkTypeOpt =
        pFrameDic.getChunkLinkFromContext(&verbInflWord, willBeAbleToSynthesizeIt,
                                          &pCurrChunkLink.tokRange.getItBegin()->inflWords.front(), nullptr);
    if (chunkLinkTypeOpt)
    {
    ...
    const Chunk& pVerbChunk)
{
  ...
  while (itNext != pVerbChunk.head &&
         itNext->inflWords.front().word.partOfSpeech == PartOfSpeech::PRONOUN_COMPLEMENT)
...
{
  PotentialQWordChunkLink(ChunkLink& pChunkLink,
                          const StaticLinguisticDictionary::QuestionWords& pQuestionWord)
...
bool _addIfItIsAListOfQWords
(ChunkLinkWorkingZone& pWorkingZone,
 std::list<ChunkLink>::iterator& pListChunkLink, // iterator that point to the chunk that contains the potential question word
 ...
 const SpecificLinguisticDatabase& pSpecLingDb)
{
  ...
  for (auto& currChild : rootQWordsChunk.children)
  {
    ...
    const auto* qWordPtr = pSpecLingDb.lingDico.statDb.wordToQuestionWord(mainInflWord.word, pAfterVerb, isCloseToTheVerb);
    if (qWordPtr != nullptr)
    {
  ...
  for (auto& currQWordChunkLink : potentialQWOrdChunks)
  {
    ...
    SemanticRequestType requestType = _getAppropriateRequestAccordingToTheContext(
        currQWordChunkLink.questionWord, pVerbChunk, currQWordChunkLink.chunkLink.chunk->head,
        pAfterVerb, isLatestQuestionWord, currQWordChunkLink.chunkLink, frameDic);
...
bool _addIfIsAQWord
(ChunkLinkWorkingZone& pWorkingZone,
 std::list<ChunkLink>::iterator& pCurrChunkLink, // iterator that point to the chunk that contains the potential question word
 Chunk& pVerbChunk, // verb chunk maybe related to the question word
 ...
 const SpecificLinguisticDatabase& pSpecLingDb)
{
  ...
  if (qWordPtr != nullptr)
  {
    ...
    for (auto itPrepToken = holdingChunkTokRange.getItBegin();
         itPrepToken != qWordTokenRange.getItBegin();
    ...
    if (qWordTokenRange.getItBegin() != holdingChunkTokRange.getItBegin())
    {
      ...
      {
        putBeginOfAChunkInTheChunkLink(pWorkingZone.syntTree(), pCurrChunkLink,
                                       qWordTokenRange.getItBegin(), language);
      }
      else
      {
        separateEndOfAChunk(pWorkingZone.syntTree(), pCurrChunkLink, qWordTokenRange.getItBegin(),
                            ChunkType::NOMINAL_CHUNK, language);
        ++pCurrChunkLink;
      }
    }
    if (pVerbChunk.form != LingVerbForm::INTERROGATIVE ||
        qWord.request != SemanticRequestType::YESORNO)
    {
      pVerbChunk.form = LingVerbForm::INTERROGATIVE;
      const auto& frameDic = pSpecLingDb.getSemFrameDict();
      // if we have specific request for the couple "verb" - "question word"
      auto requestType = _getAppropriateRequestAccordingToTheContext(qWord, pVerbChunk, pItQWord, pAfterVerb, true,
                                                                     *pCurrChunkLink, frameDic);
      pVerbChunk.requests.set(requestType);
    }
    if (qWordTokenRange.getItEnd() != holdingChunkTokRangeItEnd)
    {
      separateBeginOfAChunk(pWorkingZone.syntTree(), pCurrChunkLink, qWordTokenRange.getItEnd(),
                            ChunkType::NOMINAL_CHUNK, pItQWord, pSpecLingDb.lingDico.statDb.getLanguageType());
      --pCurrChunkLink;
    }
    ...
    else
    {
      ...
      do
      {
        qWordHasBeenLinked = false;
          ...
          for (auto itWord = getTheNextestToken(pCurrChunkLink->chunk->tokRange.getItBegin(), pCurrChunkLink->chunk->tokRange.getItEnd(), SkipPartOfWord::YES);
 ...
 const SpecificLinguisticDatabase& pSpecLingDb)
{
  ...
  if (pSecondVerbChunk != nullptr)
  {
    ...
    _addQuestionWords(pWorkingZone, currChunkLink,
                      *pSecondVerbChunk, false, pSpecLingDb);
  }
...
void extractQuestionWordsInsideAVerbGroup
 ...
 const SpecificLinguisticDatabase& pSpecLingDb)
{
  ...
  if (itFirstToken != pVerbChunkLink->chunk->head)
  {
    ...
    _addIfIsAQWord(workingZone, chLkVerb, *pVerbChunkLink->chunk,
                   itFirstToken, false, pSpecLingDb);
  }
