        ...
        for (const VerbalInflection& currVerbalI : verbalI.inflections) {
            if (currVerbalI.tense == LinguisticVerbTense::PRESENT_INDICATIVE) {
                switch (currVerbalI.person) {
                    case RelativePerson::FIRST_SING: hasFirstPersonOfIndicative = true; break;
                    case RelativePerson::SECOND_SING: hasSecondPersonOfIndicative = true; break;
                    case RelativePerson::THIRD_SING: hasThirdPersonOfIndicative = true; break;
                    default: break;
                }
            }
        }

        if (hasFirstPersonOfIndicative || hasSecondPersonOfIndicative || hasThirdPersonOfIndicative) {
        ...
        switch (currChunk.type) {
            case ChunkType::VERB_CHUNK: {
                ...
                if (!currChunk.requests.has(SemanticRequestType::SUBJECT)
                    && !currChunk.requests.has(SemanticRequestType::ACTION) && !haveASubject(currChunk)) {
                    if (currChunk.requests.has(SemanticRequestType::OBJECT)) {
                        ChunkLink* doChkLkPtr = getDOChunkLink(currChunk);
                        if (doChkLkPtr == nullptr || checkOrder(currChunk, *doChkLkPtr->chunk))
                            break;
                    }
                    isNotUnderstood = true;
                    if (isFirstChunk && currChunk.requests.empty()) {
                        bool subjectCanBeInAPreviousText =
                            pLingDico.getLanguage() == SemanticLanguageEnum::FRENCH
                ...
                for (const auto& currChildChunkLink : currChunk.children) {
                    ...
                    for (TokIt itTok = getNextToken(itPrevTok, childChunk.tokRange.getItEnd());
                         itTok != childChunk.tokRange.getItEnd();
                        ...
                        if (!pInlfChecker.areCompatibles(prevTokInflWord, tokInflWord)
                            || !InflectionsChecker::areInflectionsCompatibles(prevTokInflWord.inflections(),
                                                                              tokInflWord.inflections())) {
            ...
            case ChunkType::AND_CHUNK: {
                ...
                needToRestart = addNotUnderstood(currChunk.children,
                                                 subNbOfNotUnderstood,
                                                 pNbOfSuspiciousChunks,
