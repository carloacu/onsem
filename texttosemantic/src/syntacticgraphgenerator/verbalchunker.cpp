...
namespace {
void _addAuxiliary(Chunk& pRootChildChk, std::list<ChunkLink>& pPrevAuxs, std::list<ChunkLink>::iterator pPrevAuxIt) {
...
VerbalChunker::VerbalChunker(const AlgorithmSetForALanguage& pConfiguration)
    : fConf(pConfiguration)
    , fSpecLingDb(fConf.getSpecifcLingDb()) {}

void VerbalChunker::process(TokensTree& pTokensTree, std::list<ChunkLink>& pFirstChildren) const {
    ...
    for (std::list<TokIt>::const_iterator it = verbsAndAuxs.begin(); it != verbsAndAuxs.end(); ++it) {
        ...
        xDelimitVerbChunk(itVerbBeginGroup, itVerbEndGroup, *it, verbGroupTkenRange);
        ...
        if (minLimitVerbGroup != itVerbBeginGroup) {
            ...
            if (!prevAux.empty()) {
                ...
                Chunk subjectChunk(TokenRange(tokens, minLimitVerbGroup, itVerbBeginGroup), ChunkType::NOMINAL_CHUNK);
            ...
            if (!wordsBeforeAreSaved) {
                addNominalGroup(pFirstChildren, TokenRange(tokens, minLimitVerbGroup, itVerbBeginGroup), language);
            }
        }
        ...
        if (headIGram.word.partOfSpeech == PartOfSpeech::VERB) {
            ...
            pFirstChildren.emplace_back(ChunkLinkType::SIMPLE,
                                        Chunk(TokenRange(tokens, itVerbBeginGroup, itVerbEndGroup), verbChunkType));
        ...
        {
            prevAux.push_front(
                ChunkLink(ChunkLinkType::AUXILIARY,
                          Chunk(TokenRange(tokens, itVerbBeginGroup, itVerbEndGroup), ChunkType::AUX_CHUNK)));
    ...
    if (minLimitVerbGroup != tokens.end()) {
        addNominalGroup(pFirstChildren, TokenRange(tokens, minLimitVerbGroup, tokens.end()), language);
    }

    {
        ChunkLinkWorkingZone workingZone(pFirstChildren, pFirstChildren.begin(), pFirstChildren.end());
        xSplitChunks(workingZone,
                     {PartOfSpeech::CONJUNCTIVE,
                      PartOfSpeech::SUBORDINATING_CONJONCTION,
                      PartOfSpeech::LINKBETWEENWORDS,
                      PartOfSpeech::PUNCTUATION},
                     ChunkType::SEPARATOR_CHUNK);
    }
}

void VerbalChunker::xFindVerbsAndAuxiliaries(std::list<TokIt>& pConjVerbs, std::vector<Token>& pTokens) const {
    for (TokIt it = pTokens.begin(); it != pTokens.end(); it = getNextToken(it, pTokens.end())) {
        const Token& currToken = *it;
        const InflectedWord& currIGram = *currToken.inflWords.begin();
        if ((currIGram.word.partOfSpeech == PartOfSpeech::AUX || currIGram.word.partOfSpeech == PartOfSpeech::VERB)
            && currToken.getTokenLinkage() != TokenLinkage::PART_OF_WORD_GROUP)
            pConjVerbs.push_back(it);
    }
}

void VerbalChunker::xDelimitVerbChunk(TokIt& pItVerbBeginGroup,
    ...
    for (TokIt it = getTheNextestToken(pItVerbEndGroup, endOfPhrase, SkipPartOfWord::YES); it != endOfPhrase;) {
        if (ConceptSet::haveAConceptThatBeginWith(it->inflWords.front().infos.concepts, "time_")
    ...
    std::vector<PartOfSpeech> gramBeforeVerb = {PartOfSpeech::AUX, PartOfSpeech::ADVERB};
...
void VerbalChunker::xSplitChunks(ChunkLinkWorkingZone& pWorkingZone,
            ...
            if (tokenIsMoreProbablyAType(*tokenIt, pChunckSeps)
                ...
                if (tokenIt != beginNextChunk && tokenIt != itEnd) {
                    separateBeginOfAChunk(
                        pWorkingZone.syntTree(), it, tokenIt, it->chunk->type, fConf.getLanguageType());
                }
                ...
                if (currPartOfSpeech == PartOfSpeech::LINKBETWEENWORDS) {
                    ...
                    if (nextWord != itEnd && tokenIsMoreProbablyAType(*nextWord, pChunckSeps))
                        head = nextWord;
                }
                ...
                while (beginNextChunk != itEnd && !beginNextChunk->linkedTokens.empty()
                ...
                if (beginNextChunk != itEnd) {
                    separateBeginOfAChunk(
                        pWorkingZone.syntTree(), it, beginNextChunk, pChunkType, head, fConf.getLanguageType());
                } else {
...
TokIt VerbalChunker::xUntilNextConjVerb(std::list<TokIt>::const_iterator pCurrVerb,
...
bool VerbalChunker::xIsPositif(const Chunk& pChunk) const {
    ...
    if (fConf.getLanguageType() == SemanticLanguageEnum::FRENCH) {
        ...
        for (TokIt it = itBegin; it != itEnd; ++it)
            if (it->inflWords.front().infos.hasContextualInfo(WordContextualInfos::NEGATION) ||
                ...
                (it->str.size() > 4 && it->str.compare(it->str.size() - 3, 3, "n't") == 0) || it->str == "cannot"
                || it->str == "Cannot")
                return false;
    }
