...
public:
    ...
    ChunkLinkType findNatureOfAChunkLink(ChunkLink& pChunkLink,
                                         Chunk* pFirstVerbChunk,
                                         bool pFromEnglishPossessive = false) const;

    bool addSubordonatesToAVerb(Chunk& pVerbRoot, ChunkLinkWorkingZone& pWorkingZone) const;
     ...
     */
    mystd::optional<ChunkLinkType> getAppropriateChunkLink(InflectedWord* pVerbInflectedWord,
...
private:
    ...
    ChunkLinkType xFindNatureOfANominalGroup(ChunkLink& pChunkLink,
                                             Chunk* pFirstVerbChunk,
                                             bool pFromEnglishPossessive) const;

    ChunkLinkType xGetChkLinkTypeFromChunkLink(const ChunkLink& pChunkLink, Chunk* pFirstVerbChunk) const;
    ...
    void xAddComplementsOfVerbFromBegin(const Chunk& pRootVerb,
    ...
    void xAddComplementsOfVerbFromEnd(Chunk& pRootVerb,
    ...
    void xLinkPronounComplementBeforeVerb(Chunk& pRootVerb,
                                          ChunkLink* pSubjectChunkLink,
                                          Chunk& pCurrentChunk,
                                          Chunk& pChunkToSplit,
    ...
    void xLinkPronounComplementAfterVerb(const Chunk& pRootVerb,
                                         const ChunkLink* pSubjectChunkLink,
                                         Chunk& pCurrentChunk,
                                         Chunk& pChunkToSplit,
