...
public:
    explicit SubordinateExtractor(const AlgorithmSetForALanguage& pConfiguration);

    bool process(std::list<ChunkLink>& pFirstChildren, const SynthAnalEndingStepForDebug& pEndingStep) const;

private:
    struct SubordonatingPossibility {
        SubordonatingPossibility(std::list<ChunkLink>::reverse_iterator pSubSep,
    ...
    void xModifyTheVerbsForList(std::list<ChunkLink>& pChunkList, ChunkLinkType pFatherChunkLinkType) const;

    void xLinkComplementaryNominalChunks(std::list<ChunkLink>& pChunkLinks) const;
    void xLinkComplementaryNominalChunksSeparatredByCommas(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubordonateVerbs(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubordinatesThatBeginWithAPreposition(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubjectInfinitiveVerbs(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubordonateOfCondition(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubordinatesAtTheBeginning(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkElseSubordonates(std::list<ChunkLink>& pChunkLinks) const;

    void xLinkSubordonateThatAreBeforeVerbs(std::list<ChunkLink>& pChunkLinks) const;

    bool xTryToLinkASubordonateVerb(std::list<ChunkLink>::iterator pChunkSep,
                                    std::list<ChunkLink>::iterator& pIt,
                                    std::list<ChunkLink>& pSyntTree) const;

    bool xAddOneOrAListofPastParticipleVerbs(std::list<ChunkLink>& pSyntTree,
                                             std::list<ChunkLink>::iterator& pIt,
                                             std::list<ChunkLink>::iterator pPrev,
                                             Chunk& pNewPotentialRootChunk) const;

    static void xAddASubChunk(std::list<ChunkLink>& pSyntTree,
                              Chunk& pMotherCunk,
                              std::list<ChunkLink>::iterator pPrev,
                              std::list<ChunkLink>::iterator pIt,
                              ChunkLinkType pType,
    ...
    void xLinkSubjectAndAuxiliaryToVerbsInsideAList(std::list<ChunkLink>& pChunkLinks) const;

    void xComparisonsFinder(std::list<ChunkLink>& pChunkLinks) const;
};
