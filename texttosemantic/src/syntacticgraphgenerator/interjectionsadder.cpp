...
void _addAnInterjectionToAChunk(std::list<ChunkLink>& pChunkList,
                                Chunk& pChunkVerb,
...
void addInterjections(std::list<ChunkLink>& pChunkList) {
    ...
    for (auto it = pChunkList.begin(); it != pChunkList.end(); ++it) {
        ...
        if (chunk.type == ChunkType::INTERJECTION_CHUNK) {
            ...
            if (sepHeadGram == PartOfSpeech::PUNCTUATION || sepHeadGram == PartOfSpeech::SUBORDINATING_CONJONCTION
                || sepHeadGram == PartOfSpeech::CONJUNCTIVE) {
                if (lastVerb != nullptr && !itInterjections.empty())
                    _addAnInterjectionToAChunk(pChunkList, *lastVerb, itInterjections, itSeparators);
                lastVerb = nullptr;
        ...
        } else if (chunk.type == ChunkType::NOMINAL_CHUNK && sepHeadGram == PartOfSpeech::ADVERB) {
            itSeparators.emplace_back(it);
        } else if (chunk.type == ChunkType::VERB_CHUNK || chunk.type == ChunkType::NOMINAL_CHUNK
                   || chunkTypeIsAList(chunk.type)) {
            if (!itInterjections.empty()) {
                if (chunk.type == ChunkType::VERB_CHUNK && chunk.requests.has(SemanticRequestType::YESORNO)) {
                    ...
                    if (intjWord.lemma == "que" && intjWord.language == SemanticLanguageEnum::FRENCH) {
                        ...
                        _addAnInterjectionToAChunk(
                            pChunkList, chunk, itInterjections, itSeparators, ChunkLinkType::QUESTIONWORD);
                    }
                }
                if (!itInterjections.empty())
                    _addAnInterjectionToAChunk(pChunkList, chunk, itInterjections, itSeparators);
            }
            lastVerb = &*it->chunk;
            itInterjections.clear();
            itSeparators.clear();
