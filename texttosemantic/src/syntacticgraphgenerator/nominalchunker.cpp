...
NominalChunker::NominalChunker(const AlgorithmSetForALanguage& pConfiguration)
    : fConf(pConfiguration)
    , fLingDico(fConf.getLingDico()) {}

void NominalChunker::process(std::list<ChunkLink>& pFirstChildren) const {
    ...
    xProcessBetween2VerbChunks(pFirstChildren, prevVerb, pFirstChildren.end());
}
...
void NominalChunker::xSplitPronounToken(ChunkLinkWorkingZone& pWorkingZone) const {
    ...
    TokIt firstToken =
        getTheNextestToken(firstChunkLk->chunk->tokRange.getItBegin(), endOfTokenList, SkipPartOfWord::YES);
    if (firstToken != endOfTokenList && partOfSpeech_isPronominal(firstToken->inflWords.front().word.partOfSpeech)) {
        ...
        if (secondToken != endOfTokenList) {
            ...
            if (secondTokenPartOfSpeech == PartOfSpeech::DETERMINER || secondTokenPartOfSpeech == PartOfSpeech::PRONOUN
                ...
                separateBeginOfAChunk(pWorkingZone.syntTree(),
                                      firstChunkLk,
                                      secondToken,
                                      firstChunkLk->chunk->type,
                                      fConf.getLanguageType());
        }
    }
}

void NominalChunker::xProcessBetween2VerbChunks(std::list<ChunkLink>& pRootList,
    ...
    if (secondVerbChunk != nullptr) {
        subSecondVerbChunk = linguistics::whereToLinkTheSubjectPtr(secondVerbChunk, fConf.getFlsChecker());
    ...
    for (auto itChunkLink = workingZone.beginListIter(); itChunkLink != workingZone.endListIter(); ++itChunkLink)
    ...
    for (auto itChunkLink = workingZone.beginListIter(); itChunkLink != workingZone.endListIter(); ++itChunkLink) {
        if ((itChunkLink->chunk->type == ChunkType::NOMINAL_CHUNK
            ...
            if (itNextChunkLink != workingZone.endListIter()
                ...
                ChunkLinkType chkLkType =
                    fConf.getEntityRecognizer().findNatureOfAChunkLink(*itNextChunkLink, nullptr, false);
...
void NominalChunker::xSplitAChunk(ChunkLinkIter& pItChunkLink) const {
    static const std::vector<PartOfSpeech> subChunkSeps = {
        PartOfSpeech::PARTITIVE, PartOfSpeech::PREPOSITION, PartOfSpeech::DETERMINER, PartOfSpeech::ADVERB};
    ...
    for (TokIt currIt =
             getTheNextestToken(currChunk.tokRange.getItBegin(), currChunk.tokRange.getItEnd(), SkipPartOfWord::YES);
        ...
        if (itBeginOfWordGroup == currChunk.tokRange.getItBegin())
            continue;
        TokIt itTokenBefore =
            getPrevToken(itBeginOfWordGroup, currChunk.tokRange.getItBegin(), currChunk.tokRange.getItEnd());
...
void NominalChunker::xSplitLastTokens(ChunkLinkWorkingZone& pWorkingZone,
    ...
    if (lastToken != lastChunk->chunk->tokRange.getItEnd() && lastToken != lastChunk->chunk->tokRange.getItBegin()
        && tokenIsMoreProbablyAType(*lastToken, pSplitLastToken)) {
        // don't split the last token if there is a preposition before
        TokIt beforeLastToken =
            getPrevToken(lastToken, lastChunk->chunk->tokRange.getItBegin(), lastChunk->chunk->tokRange.getItEnd());
        ...
        separateBeginOfAChunk(
            pWorkingZone.syntTree(), lastChunk, lastToken, lastChunk->chunk->type, fConf.getLanguageType());
    }
}

void NominalChunker::xExtractEnglishSPossessive(std::list<ChunkLink>& pFirstChildren) const {
    ...
    auto tryToAddAOwner = [&itOwner](TokIt pItTok, std::list<ChunkLink>::iterator pItChlds) {
    ...
    for (auto itChlds = pFirstChildren.begin(); itChlds != pFirstChildren.end(); ++itChlds) {
        ...
        for (TokIt itTok = itFirstTok; itTok != itChlds->chunk->tokRange.getItEnd(); ++itTok) {
            ...
            if (itPoss.isEmpty()) {
                if (tokPartOfSpeech == PartOfSpeech::INTERSPACE) {
                    if (!itTok->str.empty() && itTok->str[0] == '\'' && !itOwner.isEmpty()) {
                ...
                if (partOfSpeech_isAWord(tokPartOfSpeech)) {
                    if (itTok->str == "s") {
                        ...
                        xEnglishSPossessiveAddAOwner(pFirstChildren, itOwner, itPoss, itSPoss, itMain);
                        tryToAddAOwner(itTok, itChlds);
                    }
                } else if (tokPartOfSpeech == PartOfSpeech::PUNCTUATION) {
                    xEnglishSPossessiveClearContext(itOwner, itPoss, itSPoss, itMain);
                }
            }
        }
    }
...
void NominalChunker::xEnglishSPossessiveAddAOwner(std::list<ChunkLink>& pFirstChildren,
    ...
    if (pItSPoss.itChkLk == pItMain.itChkLk) {
        ...
        if (itBeginMain != pItMain.itChkLk->chunk->tokRange.getItEnd()) {
            separateBeginOfAChunk(pFirstChildren,
                                  pItMain.itChkLk,
                                  itBeginMain,
    ...
    if (pItOwner.itChkLk == pItPoss.itChkLk) {
        separateEndOfAChunk(pFirstChildren,
                            pItOwner.itChkLk,
                            pItPoss.itToken,
    ...
    xEnglishSPossessiveClearContext(pItOwner, pItPoss, pItSPoss, pItMain);
}

void NominalChunker::xEnglishSPossessiveClearContext(TokPosInAChunk& pItOwner,
                                                     TokPosInAChunk& pItPoss,
                                                     TokPosInAChunk& pItSPoss,
