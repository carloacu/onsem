...
bool _doesSubjectCanBeCompletedWithAPronounSubjectFr(const SemanticRequests& pRequests) {
    for (const auto& currRequest : pRequests.types)
        if (currRequest == SemanticRequestType::SUBJECT || currRequest == SemanticRequestType::OBJECT
            || currRequest == SemanticRequestType::QUANTITY || currRequest == SemanticRequestType::LOCATION
...
void _modifyRequestOfChunkThatWillHaveASubject(Chunk& pVerbChunk,
    ...
    } else if (pSubVerbChunk.requestCanBeObject && pSubVerbChunk.requests.has(SemanticRequestType::SUBJECT)) {
...
VerbalToNominalChunksLinker::VerbalToNominalChunksLinker(const AlgorithmSetForALanguage& pConfiguration)
    : fConf(pConfiguration)
    , fSpecLingDb(fConf.getSpecifcLingDb())
    , fLingDico(fConf.getLingDico()) {}

void VerbalToNominalChunksLinker::process(std::list<ChunkLink>& pFirstChildren) const {
...
void VerbalToNominalChunksLinker::extractEnglishSubjectOf(std::list<ChunkLink>& pFirstChildren) const {
    auto endChild = pFirstChildren.end();
    for (auto it = pFirstChildren.begin(); it != endChild; ++it) {
...
void VerbalToNominalChunksLinker::_splitAdverbBeforeNouns(ChunkLinkIter& pChunkLinkIter) const {
    ...
    for (TokIt currIt = currChunk.tokRange.getItBegin(); currIt != currChunk.tokRange.getItEnd();
        ...
        if (isAnAdverbBefore) {
            ...
            separateBeginOfAChunk(pChunkLinkIter, currIt, currChunk.type, language);
...
void VerbalToNominalChunksLinker::_constructASyntGraphBetween2VerbChunks(
    ...
    if (workingZone.empty())
        return;
    Chunk* firstVerbChunk = pItFirstVerbChunk == pRootList.end() ? nullptr : &*pItFirstVerbChunk->chunk;
    Chunk* secondVerbChunk = pItSecondVerbChunk == pRootList.end() ? nullptr : &*pItSecondVerbChunk->chunk;
    ...
    if (!workingZone.empty())
        _extractQuestionWords(workingZone, firstVerbChunk, secondVerbChunk, subSecondVerbChunk);
    ...
    if (!workingZone.empty() && workingZone.begin()->chunk->type != ChunkType::SEPARATOR_CHUNK
        ...
        if (newFirstChunkHead != itFirstChunkLk->chunk->tokRange.getItEnd()
            ...
            setSubjectPronounInflectionsAccordingToTheVerb(
                newFirstChunkHead->inflWords.front(), firstVerbChunk->head->inflWords.front(), inflsCheker);
            ...
            if (newEnd != itFirstChunkLk->chunk->tokRange.getItEnd()) {
                ...
                ++itSecondChunkLk;
                if (itSecondChunkLk == workingZone.end() || itSecondChunkLk->chunk->type != ChunkType::NOMINAL_CHUNK
                    || itSecondChunkLk->chunk->head->inflWords.front().word.partOfSpeech == PartOfSpeech::ADVERB) {
                    separateBeginOfAChunk(workingZone.syntTree(),
                                          itFirstChunkLk,
                                          newEnd,
            ...
            if (!isTheSubjectInserted) {
                ...
                firstVerbChunk->children.splice(firstVerbChunk->children.end(), workingZone.syntTree(), itChLkSubject);
            }
            if (!workingZone.empty()) {
    ...
    if (firstVerbChunk != nullptr && !workingZone.empty()) {
        ...
        if (!codAlreadyLinkToFirstVerb && !workingZone.empty() && !haveAnAuxiliary(*firstVerbChunk)
        ...
        if (language == SemanticLanguageEnum::FRENCH && !codAlreadyLinkToFirstVerb && subSecondVerbChunk != nullptr
    ...
    if (!workingZone.empty()) {
        if (subSecondVerbChunk != nullptr) {
            ...
            if (subjectChunkLkPtr == nullptr) {
                ...
                auto potentialSubject = workingZone.end();
                --potentialSubject;
                if ((fConf.getLinker().canSubstituteAChunkByAnother(
                        *subjectChunkLkPtr, *potentialSubject->chunk, true))) {
                    subjectChunkLkPtr->tokRange = subjectChunkLkPtr->chunk->tokRange;
                    subjectChunkLkPtr->chunk = potentialSubject->chunk;
                    workingZone.syntTree().erase(potentialSubject);
                }
            }
        } else if (secondVerbChunk != nullptr && secondVerbChunk->type == ChunkType::INFINITVE_VERB_CHUNK
    ...
    if (subSecondVerbChunk != nullptr && secondVerbChunk != nullptr && !workingZone.empty()
        && (secondVerbChunk->form == LingVerbForm::INTERROGATIVE
            || (workingZone.begin()->chunk->type == ChunkType::SEPARATOR_CHUNK
                && fLingDico.statDb.wordToSubordinateRequest(workingZone.begin()->chunk->head->inflWords.front().word)
                       != SemanticRequestType::NOTHING))) {
        _linkObjectBeforeAnInterrogativeVerb(workingZone, *secondVerbChunk, *subSecondVerbChunk);
    }
            ...
            if (!itSeparator || itSeparator->atEnd()) {
                ...
                if (currChunk.type == ChunkType::NOMINAL_CHUNK
                    ...
                    if (firstVerbLastChildChLk.type != ChunkLinkType::QUESTIONWORD
                        ...
                        firstVerbLastChildChLk.chunk->children.splice(
                            firstVerbLastChildChLk.chunk->children.end(), workingZone.syntTree(), itChunkLink.getIt());
                    }
                }
                break;
            }
            if (canBeTheHeadOfASubordinate(itChunkLink->chunk->head->inflWords.front())) {
                ChunkLinkType subordinateChunkLink =
                    entityRecognizer.findNatureOfAChunkLink(*itChunkLink, firstVerbChunk);
                if (subordinateChunkLink != ChunkLinkType::INDIRECTOBJECT
                    ...
                    putSeparatorBefore(itChunkLink);
                    firstVerbChunk->children.splice(
                        firstVerbChunk->children.end(), workingZone.syntTree(), itChunkLink.getIt());
                }
            }
            itChunkLink = itNext;
        }
            ...
            for (auto it = firstVerbChunk->children.begin(); it != firstVerbChunk->children.end(); ++it) {
                if (it->type == ChunkLinkType::DIRECTOBJECT || it->type == ChunkLinkType::SUBORDINATE) {
                    if (itInterjection == firstVerbChunk->children.end()) {
                        ...
                        it->chunk->children.splice(
                            it->chunk->children.begin(), firstVerbChunk->children, itInterjection);
                    }
                }
            }
        }
    ...
    switch (fConf.getLanguageType()) {
        case SemanticLanguageEnum::ENGLISH: {
            if (pWorkingZone.sizeUntilNextNotNominal() > 1
                && firstVerbChunk->head->inflWords.front().word
                       == SemanticWord(SemanticLanguageEnum::ENGLISH, "be", PartOfSpeech::VERB)
                && !haveASubject(*firstVerbChunk)
                && canLinkVerbToASubject(*firstVerbChunk, *pWorkingZone.begin()->chunk, inflsCheker, false))
                return true;
            bool canBeASubjectAfterVerb =
                headPartOfSpeech == PartOfSpeech::PRONOUN_SUBJECT
                || (headPartOfSpeech == PartOfSpeech::PRONOUN
                    || (partOfSpeech_isNominal(headPartOfSpeech)
                        && !headInflWord.infos.hasContextualInfo(WordContextualInfos::CANBEBEFORENOUN)));
            if (!canBeASubjectAfterVerb || firstVerbInflWord.word != fLingDico.getBeVerb().word)
                return false;
            break;
        }
        case SemanticLanguageEnum::FRENCH: {
            if (firstVerbChunk->requests.has(SemanticRequestType::OBJECT) && !haveAnAuxiliary(*firstVerbChunk)
                && !haveASubject(*firstVerbChunk))
                return true;
...
void VerbalToNominalChunksLinker::_extractQuestionWords(ChunkLinkWorkingZone& pWorkingZone,
    ...
    if (pFirstVerbChunk != nullptr || secondVerbChunkForQuestionWord != nullptr) {
        questionWords::addQuestionWords(pWorkingZone, pFirstVerbChunk, secondVerbChunkForQuestionWord, fSpecLingDb);
...
void VerbalToNominalChunksLinker::_tryAddDirectObjectToImperativeVerb(Chunk& pVerbRoot,
                                                                      ChunkLinkWorkingZone& pWorkingZone) const {
    if (pVerbRoot.requests.empty() && !haveASubject(pVerbRoot)) {
...
void VerbalToNominalChunksLinker::_linkObjectBeforeAnInterrogativeVerb(ChunkLinkWorkingZone& pWorkingZone,
                                                                       Chunk& pSecondVerbChunk,
                                                                       Chunk& pSubSecondVerbChunk) const {
    ...
    while (itSubordinate != pWorkingZone.end()) {
        ...
        if ((qWordChunkPtr == nullptr || checkOrder(*qWordChunkPtr, subordonateChunk))) {
            if (fConf.getLanguageType() == SemanticLanguageEnum::FRENCH && itNext == pWorkingZone.end()
            ...
            if (chunkCanBeAnObject(subordonateChunk)) {
                ...
                if (!isChunkLinkTypeSet) {
                    ...
                    if (itSubordinate->type == ChunkLinkType::DIRECTOBJECT) {
                        itSubordinate->type =
                            fConf.getEntityRecognizer().findNatureOfAChunkLink(*itSubordinate, &pSubSecondVerbChunk);
                ...
                pSubSecondVerbChunk.children.splice(
                    pSubSecondVerbChunk.children.end(), pWorkingZone.syntTree(), itSubordinate);
            }
        }
...
bool VerbalToNominalChunksLinker::_linkAVerbGroupToHisCOD(ChunkLinkWorkingZone& pWorkingZone, Chunk* pVerbChunk) const {
...

    {
        ...
        if (codPartOfSpeech == PartOfSpeech::PRONOUN_SUBJECT) {
            if (fConf.getLanguageType() == SemanticLanguageEnum::FRENCH && pVerbChunk != nullptr) {
        ...
        if (codPartOfSpeech == PartOfSpeech::SUBORDINATING_CONJONCTION || codPartOfSpeech == PartOfSpeech::PREPOSITION)
            return false;
    }
    ...
    while (!pWorkingZone.empty()) {
        ...
        if (subordonateChunk.type == ChunkType::INFINITVE_VERB_CHUNK) {
            ...
            pVerbChunk->children.splice(pVerbChunk->children.end(), pWorkingZone.syntTree(), pWorkingZone.begin());
            break;
        }
        if (subordonateChunk.type != ChunkType::PREPOSITIONAL_CHUNK
            && pVerbChunk->requests.isAQuestionNotAskingAboutTheObject() && haveAQuestionWordChildAfter(*pVerbChunk)) {
...
bool VerbalToNominalChunksLinker::_linkAVerbChunkToHisSubject(Chunk& pSubVerbChunk,
                                                              Chunk& pVerbChunk,
                                                              ChunkLink& pPotentialSubject) const {
    if (canLinkVerbToASubject(pSubVerbChunk, *pPotentialSubject.chunk, fConf.getFlsChecker(), false)) {
