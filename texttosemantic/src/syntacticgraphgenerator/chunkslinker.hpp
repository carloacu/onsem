...
public:
    ...
    bool canSubstituteAChunkByAnother(const ChunkLink& pPronounChkLk,
                                      const Chunk& pRefChunk,
                                      bool pRefChunkIsAtRoot) const;

private:
    struct IterToChkLink {
        IterToChkLink(std::list<ChunkLink>& pList, std::list<ChunkLink>::iterator pIt)
            : list(pList)
            , it(pIt) {}
    ...
    void _linkPronounToMorePreciseSubject(ChunkLink& pChunkLink,
                                          History& pHistory,
                                          std::list<Chunk*>& pStack,
                                          std::list<ChunkLink>& pFirstChildren) const;

    void _tryToLinkToAPrevWord(ChunkLink& pPronounChkLk, History& pHistory, std::list<Chunk*>& pStack) const;

    void _saveTheLink(ChunkLink*& pSaveLink, ChunkLink& pSubject) const;

    static void _linkPronounToReferentChunk(ChunkLink& pPronounToLink, ChunkLink& pReferentChunkLink);
