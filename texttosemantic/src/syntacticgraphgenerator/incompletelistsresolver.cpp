...
namespace
{
bool _hasOnlyInterjection(std::list<ChunkLink>::iterator pItBegin,
...
{
  static const std::set<ChunkType> chunkTypeToAddInList = {ChunkType::VERB_CHUNK,
  ...
  for (auto it = pChunkList.begin(); it != pChunkList.end(); ++it)
  {
    ...
    {
      mystd::optional<ChunkType> chunkTypeOpt =
          getListType(currChunk.getHeadConcepts());
      if (chunkTypeOpt)
      {
        auto nextIt = it;
        ++nextIt;
              ...
              nextIt->chunk->getHeadPartOfSpeech() == PartOfSpeech::ADVERB)))
        {
          ...
          if (newChunkTypeOpt)
          {
            ...
            nextIt = pChunkList.erase(nextIt); // TODO: here we can remove some words from the original text in the syntactic tree
          }
        ...
        else if (hasAListEltAfter && !hasAListEltBefore)
        {
          ...
          if (chunkTypeToAddInList.count(nextChunk.type))
          {
            ...
            currChunk.children.splice(currChunk.children.end(),
                                      pChunkList, nextIt);
          }
        }
        else if (hasAListEltBefore && !hasAListEltAfter)
        {
          ...
          while (true)
          {
            ...
            if (addToTheList)
            {
              ...
              auto itToDel = prevIt;
              ++prevIt;
              currChunk.children.splice(currChunk.children.begin(),
                                        pChunkList, itToDel);
                 ...
                 it != pChunkList.begin())
        {
          ...
          if (chunkTypeIsVerbal(nextChunk.type))
          {
            ...
            {
              auto* qWordPtr = pLingDico.statDb.wordToQuestionWord(prevChunk.head->inflWords.front().word, false, false);
              if (qWordPtr != nullptr)
              {
                ...
                if (verbalChunkWithQuestionWord != nullptr)
                {
                  ...
                  verbalChunkWithQuestionWord->children.splice(verbalChunkWithQuestionWord->children.begin(), pChunkList, prevIt);
