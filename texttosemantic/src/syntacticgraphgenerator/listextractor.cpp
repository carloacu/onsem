...
ListExtractor::ListExtractor(const AlgorithmSetForALanguage& pConfiguration)
    : fFlschecker(pConfiguration.getFlsChecker())
    , fConf(pConfiguration) {}

bool _isAListSeparator(const InflectedWord& pIGram, ChunkType pListType) {
...
bool ListExtractor::extractLists(ChunkLinkWorkingZone& pWorkingZone,
                                 bool pOneWordList,
    ...
    while (currIt.getIt() != pWorkingZone.begin()) {
        ...
        if (!currList.listType) {
            if (currIt->chunk->type == ChunkType::SEPARATOR_CHUNK) {
                if (listsToMove.empty()
                    && _tryToExtractANewListAroundASeparator(currIt,
                                                             currList,
                                                             listsToMove,
                                                             canHaveAnotherList,
                                                             pNbMaxOfListElt,
                                                             pWorkingZone,
                                                             pOneWordList,
                                                             pAllowToNotRepeatTheSubject,
                                                             pExceptListType))
                    continue;
            } else if (pCanCompletePreviousList && chunkTypeIsAList(currIt->chunk->type) && currIt->tokRange.isEmpty()
        ...
        {
            if (_tryToCompleteListInfoWithAChunk(currIt,
                                                 currList,
                                                 listsToMove,
                                                 pNbMaxOfListElt,
                                                 pWorkingZone,
...
bool ListExtractor::_createNewList(std::list<ListToMove>& pListsToMove,
    ...
    if (pChunkEnd.atEnd()) {
        ...
        if (pCurrList.chunkToMoveInNextEltChunkLink && !pCurrList.chunkToMoveInNextEltChunkLink->atEnd()) {
        ...
        newChunkEltTrunc.chunk->head = getHeadOfNominalGroup(newChunkEltTrunc.chunk->tokRange, fConf.getLanguageType());
        ...
        complChunk.chunk->head = getHeadOfNominalGroup(complChunk.chunk->tokRange, fConf.getLanguageType());
        ...
        if (newChunkTruncPartoSpeech == PartOfSpeech::ADVERB) {
            ...
            pIt.getList()->insert(nextIt, complChunk);
                   ...
                   && partOfSpeech_isNominal(
                       complChunk.chunk->tokRange.getItBegin()->inflWords.front().word.partOfSpeech)) {
            ...
            if (tokenIsMoreProbablyAType(*complChunk.chunk->tokRange.getItBegin(), PartOfSpeech::PARTITIVE)) {
...
bool ListExtractor::_itIsInListToMove(const ChunkLinkIter& pIt, const std::list<ListToMove>& pListsToMove) {
    for (const auto& currLstToMv : pListsToMove) {
        if (currLstToMv.newParent == pIt || currLstToMv.itRootList == pIt) {
            return true;
        }
    }
    return false;
...
void ListExtractor::_addAListToMove(std::list<ListToMove>& pListsToMove, const ListToMove& pNewListToMove) {
...
bool ListExtractor::_addListElt(Chunk* pListChunk,
    ...
    if (pNextIt.getIt()->chunk->tokRange.getItBegin() == pNextChunkBegin) {
        ...
        nextChunkTrunc.chunk->head = getHeadOfNominalGroup(nextChunkTrunc.chunk->tokRange, fConf.getLanguageType());
        ...
        nextChunk.head = getHeadOfNominalGroup(pNextIt.getIt()->chunk->tokRange, fConf.getLanguageType());
    ...
    if (newlyAddedChunk.type == ChunkType::INFINITVE_VERB_CHUNK && !haveADO(newlyAddedChunk)) {
        ...
        if (headInfosGram.word.partOfSpeech == PartOfSpeech::VERB
            ...
            if (nextChkDOChkLk != nullptr && checkOrder(nextChunkInTheList, *nextChkDOChkLk->chunk)) {
...
bool ListExtractor::_addANewMultiWordsList(ListInfo& pCurrList,
    ...
    if (_isNewEltCompatibleWithTheList(pCurrList, nextChunkFirstElt, pAllowToNotRepeatTheSubject)) {
        ...
        if (fFlschecker.canBeAssociatedInAList(nextChunkFirstElt.head->inflWords.front(),
            ...
            if (prevChunk.type == ChunkType::PREPOSITIONAL_CHUNK) {
                if (prevBeginIt->inflWords.front().word
                    == nextChunk->chunk->tokRange.getItBegin()->inflWords.front().word)
                    createList = true;
        ...
        if (createList) {
            ...
            bool chunkHasBeenRemoved =
                _createNewList(pListsToMove, chunkEnd, chunkBegin, pNbMaxOfListElt, pCurrList, pPrevIt, pIt, pNextIt);
...
bool ListExtractor::_addANewOneWordList(ListInfo& pCurrList,
                                        std::list<ListToMove>& pListsToMove,
                                        int& pNbMaxOfListElt,
                                        ChunkLinkIter& pPrevIt,
                                        ChunkLinkIter& pIt,
    ...
    if (_getBeginNewEltAndEndOldElt(chunkBegin, chunkEnd, pCurrList.partOfSpeechAtBegining)) {
        ...
        while (!chunkHasBeenRemoved && pNbMaxOfListElt != 0) {
            ChunkAndTokIt chunkBegin(*pNextIt.getIt()->chunk);
        ...
        // if there is a chunk before the list
        if (!chunkHasBeenRemoved && canBeParentOfANominalGroup(pNextIt->chunk->head->inflWords.front())) {
...
bool ListExtractor::_isNewEltCompatibleWithTheList(ListInfo& pCurrList,
                                                   const Chunk& pChunk,
                                                   bool pAllowToNotRepeatTheSubject) const {
    ChunkType newChunkType = pChunk.type;
    bool doesBecomeAListOfInfinitiveVerbs = _doesBecomeAListOfInfinitiveVerbs(pCurrList, newChunkType);
    if ((newChunkType == pCurrList.eltType
         && (pAllowToNotRepeatTheSubject
             || !(pCurrList.eltType == ChunkType::VERB_CHUNK && !pCurrList.hasASubject && haveASubject(pChunk))))
        ...
        if ((headPartOfSpeech == PartOfSpeech::PRONOUN || headPartOfSpeech == PartOfSpeech::PRONOUN_SUBJECT)
            ...
            return false;
        return _updateExistingListInfoAccordingToTheNewElt(pCurrList, pChunk, doesBecomeAListOfInfinitiveVerbs);
    } else if (newChunkType == ChunkType::NOMINAL_CHUNK && pCurrList.eltType == ChunkType::VERB_CHUNK
...
void ListExtractor::_getVerbAndAuxInflections(VerbalInflections& pVerbInflections, const Chunk& pVerbChunk) const {
...
bool ListExtractor::_getBeginNewEltAndEndOldElt(ChunkAndTokIt& pChunkBegin,
                                                ChunkAndTokIt& pChunkEnd,
    ...
    if (lastTokNewChunk != newChunk.tokRange.getItEnd()) {
        ...
        if (pPartOfSpeechAtBegining == PartOfSpeech::DETERMINER) {
            ...
            if (firstIGramOfFirstTokenOfTheOldChunk.word.partOfSpeech == pPartOfSpeechAtBegining) {
                ...
                if (!pChunkEnd.atEnd()) {
                    ...
                    bool haveListEltCompatibles =
                        fFlschecker.canBeAssociatedInAList(chunkBeginFirstInflWord, chunkEndFirstInflWord);
                    if (!haveListEltCompatibles && chunkEndFirstInflWord.word.partOfSpeech == PartOfSpeech::NOUN
                    ...
                    if (haveListEltCompatibles) {
                        // checks the GRAMXs
                        TokIt detTokNewChunk =
                            getPrevToken(lastTokNewChunk, newChunk.tokRange.getItBegin(), newChunk.tokRange.getItEnd());
        ...
        // check that 2 chunks contains the same grammatical type
                 ...
                 || _tryAssociateNewChunkByTruncateTheEnd(
                     lastTokNewChunk, newChunk, firstIGramOfFirstTokenOfTheOldChunk)) {
            ...
            while (!pChunkEnd.atEnd() && pChunkEnd.tokIt->linkedTokens.size() == 1
...
bool ListExtractor::_tryAssociateNewChunkByTruncateTheEnd(TokIt& pLastTokNewChunk,
                                                          Chunk& pNewChunk,
    ...
    if (pIGramLastElt.word.partOfSpeech == PartOfSpeech::NOUN) {
        ...
        while (pLastTokNewChunk != pNewChunk.tokRange.getItEnd()
            ...
            pLastTokNewChunk =
                getPrevToken(pLastTokNewChunk, pNewChunk.tokRange.getItBegin(), pNewChunk.tokRange.getItEnd());
        }
        if (beginSubChunk != pLastTokNewChunk && pLastTokNewChunk != pNewChunk.tokRange.getItEnd()
            && pLastTokNewChunk->inflWords.front().word.partOfSpeech == PartOfSpeech::NOUN) {
            putEndOfAChunkToHisChildren(pNewChunk, beginSubChunk, fConf.getLanguageType());
            return true;
        }
    }
    return false;
...
void ListExtractor::extractSubordonates(ChunkLinkWorkingZone& pWorkingZone) const {
    ...
    while (it != pWorkingZone.begin()) {
        ...
        if ((currChunk.type == ChunkType::SEPARATOR_CHUNK
            ...
            || (currChunk.type == ChunkType::NOMINAL_CHUNK && currChunkHeadPartOfSpeech == PartOfSpeech::PARTITIVE)) {
            // if there is no phrase before, we skip this subordonate
            if (prevPhrase == pWorkingZone.end()
                || (language == SemanticLanguageEnum::FRENCH
                    && !recInListConst(chunkIsVerbalAndNotAYesOrNo, *prevPhrase->chunk))
            ...
            if (!currListType && listOfSubs.subWithSeps.size() == 1) {
            ...
            if (!listOfSubs.subWithSeps.empty() && currListType
                ...
                if (prevPhrase == pWorkingZone.end()) {
                    ...
                    if (language == SemanticLanguageEnum::ENGLISH) {
                        if (!recInListConst(chunkIsVerbal, *prevPhrase->chunk)) {
                            needToSkip = true;
                        } else if (!hasAChunkTypeOrAListOfChunkTypes(*prevPhrase->chunk,
                                                                     ChunkType::INFINITVE_VERB_CHUNK)
                                   && nextPhrase != pWorkingZone.end()
                        ...
                        needToSkip = recInListConst(
                            [&](const Chunk& pChunk) {
                                return pChunk.type != ChunkType::INFINITVE_VERB_CHUNK
                                    && (pChunk.type != ChunkType::VERB_CHUNK
                                        || (!pChunk.requests.isAQuestion()
                                            && !ConceptSet::haveAConcept(currChunk.getHeadConcepts(),
                                                                         "verb_action_say")));
                            },
                            *prevPhrase->chunk);
                    }
...
bool ListExtractor::_addSubordonates(std::list<ChunkLink>::iterator& pParentChunkLk,
    ...
    if (pSubOfChunk.size() == 1)    // TODO: have the same algo for one or many usbordinate!!!
    {
        ...
        if (chunkTypeIsAList(parentChunkPtr->type) && !parentChunkPtr->children.empty())
        ...
        if (parentChunkPtr->type == ChunkType::INFINITVE_VERB_CHUNK) {
            if (fConf.getLanguageType() == SemanticLanguageEnum::ENGLISH && tokRange.isEmpty()
        ...
        ChunkLink andChunkLink(ChunkLinkType::SUBORDINATE_CLAUSE, Chunk(pEndEltOfList->chunk->tokRange, *pListType));
        ...
        for (auto it = pSubOfChunk.begin(); it != pSubOfChunk.end(); ++it) {
            ...
            if (verbDOIsTheSubject) {
                if (chunkTypeIsVerbal(parentChunk.type)) {
                    ...
                    verbDOIsTheSubject =
                        canLinkVerbToASubject(whereToLinkTheSubject(*it->sub->chunk), parentChunk, fFlschecker, false);
                }
            }
        }
        if (verbDOIsTheSubject)
        ...
        if (pParentChunkLk->chunk->type == ChunkType::VERB_CHUNK) {
            ...
            if (dObj != nullptr && !chunkTypeIsAList(dObj->type)) {
...
mystd::optional<ChunkLinkType> ListExtractor::_getAppropriateChunkLinkFromTokens(
    ...
    if (itFirstWord != itEnd) {
        ...
        return fConf.getEntityRecognizer().getAppropriateChunkLink(
            pVerbInflectedWord, pIntroductingWord, &firstWordIGram, pTokensChunkType, &nextToken);
    }
    return mystd::optional<ChunkLinkType>();
}

bool ListExtractor::_linkVerbChunk_VerbChunk(Chunk& pFirstVerb,
    ...
    if (language == SemanticLanguageEnum::ENGLISH && pTokRange.isEmpty()) {
        ...
        for (TokIt it = pTokRange.getItBegin(); it != endIt; it = getNextToken(it, endIt)) {
            ...
            if (tokIGram.infos.hasContextualInfo(WordContextualInfos::CONDITION)
                ...
                && !ConceptSet::haveAConcept(pFirstVerb.head->inflWords.front().infos.concepts,
                                             "verb_action_say_ask")) {
                isACondition = true;
            }
        }
    ...
    if (!isACondition || pTokRange.isEmpty()) {
        ...
        if (!haveAnAuxiliary(pFirstVerb)) {
            ...
            if (verbalInflPtr != nullptr && verbalInflPtr->inflections.size() == 1
        ...
        newLkType = _getAppropriateChunkLinkFromTokens(
            &iGramVerb, pSecondVerb.chunk->introductingWordToSaveForSynthesis, pTokRange, pSecondVerb.chunk->type);
        ...
        if (tryToPutSubordonateOnChildOfTheDO) {
            if (subChunkLkToHoldTheSubordinatePtr == nullptr && iGramVerb.infos.concepts.count("verb_equal_be") > 0) {
                ...
                if (subChunkLkToHoldTheSubordinatePtr != nullptr
                    && partOfSpeech_isPronominal(
                        subChunkLkToHoldTheSubordinatePtr->chunk->head->inflWords.front().word.partOfSpeech))
            ...
            if (newLkType && (*newLkType == ChunkLinkType::OBJECT_OF || *newLkType == ChunkLinkType::DIRECTOBJECT))
                ...
                if (pFirstVerb.type == ChunkType::INFINITVE_VERB_CHUNK && pTokRange.isEmpty()
                    ...
                    if (subChunkLkToHoldTheSubordinatePtr != nullptr
                        && InflectionsChecker::verbIsAtInfinitive(
                            subChunkLkToHoldTheSubordinatePtr->chunk->head->inflWords.front())) {
            ...
            if (subChunkLkToHoldTheSubordinatePtr != nullptr) {
                if (chunkTypeIsVerbal(subChunkLkToHoldTheSubordinatePtr->chunk->type)) {
                    if (hasAChunkTypeOrAListOfChunkTypes(*subChunkLkToHoldTheSubordinatePtr->chunk,
                                                         ChunkType::INFINITVE_VERB_CHUNK)
                        || hasAChunkTypeOrAListOfChunkTypes(*pSecondVerb.chunk, ChunkType::INFINITVE_VERB_CHUNK)) {
                        subChunkLkToHoldTheSubordinatePtr->chunk->children.emplace_back(
                            pTokRange, newChunkLiknType, pSecondVerb.chunk);
                        return true;
                    }
                } else {
                    if (hasAChunkTypeOrAListOfChunkTypes(*pSecondVerb.chunk, ChunkType::INFINITVE_VERB_CHUNK)) {
                        PartOfSpeech subChunkPartOfSpeech =
                            subChunkLkToHoldTheSubordinatePtr->chunk->getHeadPartOfSpeech();
                        if (subChunkPartOfSpeech == PartOfSpeech::PREPOSITION
                            && subChunkLkToHoldTheSubordinatePtr->chunk->children.empty()) {
                            subChunkLkToHoldTheSubordinatePtr->tokRange =
                                subChunkLkToHoldTheSubordinatePtr->chunk->tokRange;
                            subChunkLkToHoldTheSubordinatePtr->chunk = pSecondVerb.chunk;
                        } else if (fConf.getLanguageType() == SemanticLanguageEnum::FRENCH && pTokRange.isEmpty()) {
                            if (getDOChunkLink(*pSecondVerb.chunk) != nullptr)
                                pFirstVerb.children.emplace_front(ChunkLinkType::INDIRECTOBJECT,
                                                                  *subChunkLkToHoldTheSubordinatePtr->chunk);
                            else
                                pSecondVerb.chunk->children.emplace_front(ChunkLinkType::DIRECTOBJECT,
                                                                          *subChunkLkToHoldTheSubordinatePtr->chunk);
                            subChunkLkToHoldTheSubordinatePtr->chunk = pSecondVerb.chunk;
                        } else if (partOfSpeech_isPronominal(subChunkPartOfSpeech))    // replace by any human
                        {
                            subChunkLkToHoldTheSubordinatePtr->chunk->children.emplace_back(
                                pTokRange, ChunkLinkType::SUBJECT_OF, pSecondVerb.chunk);
                        } else {
                            auto subChLkOpt = _getAppropriateChunkLinkFromTokens(
                                nullptr,
                            ...
                            if (subChLkOpt)
                                subChunkLkToHoldTheSubordinatePtr->chunk->children.emplace_back(
                                    pTokRange, *subChLkOpt, pSecondVerb.chunk);
                            else
                                subChunkLkToHoldTheSubordinatePtr->chunk->children.emplace_back(
                                    pTokRange, ChunkLinkType::DIRECTOBJECT, pSecondVerb.chunk);
                        }
                        return true;
                    }
                    if (!secondChunkHasASubject && !pTokRange.isEmpty()
                        && canLinkVerbToASubject(whereToLinkTheSubject(*pSecondVerb.chunk),
                                                 *subChunkLkToHoldTheSubordinatePtr->chunk,
                                                 fFlschecker,
                                                 false)) {
                        subChunkLkToHoldTheSubordinatePtr->chunk->children.emplace_back(
    ...
    if (isACondition)
        ...
        if (!secondChunkHasASubject && fConf.getLanguageType() == SemanticLanguageEnum::FRENCH) {
            recInList(
                [&](Chunk& pSecondVerbChunkElt) {
...
void ListExtractor::_refactorANewSubordinate(Chunk& pFirstVerb, Chunk& pSecondVerb, const TokenRange& pTokRange) const {
...
bool ListExtractor::_linkChunk_VerbChunk(Chunk& pFirstChunk,
                                         ChunkLink& pSecondVerb,
                                         const TokenRange& pTokRange) const {
    mystd::optional<ChunkLinkType> subType = _getAppropriateChunkLinkFromTokens(
        nullptr, pSecondVerb.chunk->introductingWordToSaveForSynthesis, pTokRange, pSecondVerb.chunk->type);
    if (!subType) {
...
bool ListExtractor::_canLinkDO_Subject(Chunk& pFirstVerb, Chunk& pSecondVerb) const {
    ...
    if (dObj != nullptr && !haveASubject(pSecondVerb)) {
        return canLinkVerbToASubject(whereToLinkTheSubject(pSecondVerb), *dObj, fFlschecker, false);
    }
    return false;
}
...
std::list<ChunkLink>::iterator ListExtractor::_tryToCompleteAnExistingList(
    ...
    ++itExistingList;
    while (itExistingList != pWorkingZone.end() && itExistingList->chunk->type == ChunkType::SEPARATOR_CHUNK
    ...
    if (chunkTypeIsAList(currExistingChunk.type)) {
        ...
        for (auto itListElt = currList.listChunk->children.rbegin(); itListElt != currList.listChunk->children.rend();
             ++itListElt) {
            ...
            if (firstListElt) {
                ...
                bool doesBecomeAListOfInfinitiveVerbs = _doesBecomeAListOfInfinitiveVerbs(currList, currChunkElt.type);
                _updateExistingListInfoAccordingToTheNewElt(currList, currChunkElt, doesBecomeAListOfInfinitiveVerbs);
            }
        }
        ...
        while (currIt.getIt() != pWorkingZone.begin()) {
            --currIt;
            if (!_tryToCompleteListInfoWithAChunk(
                    currIt, currList, pListsToMove, nbMaxOfListElt, pWorkingZone, false, false))
                break;
        }
        ...
        _tryToExtractANewListAroundASeparator(currIt,
                                              ...
                                              false,
                                              noExceptionListType);
        while (currList.listType && currIt.getIt() != pWorkingZone.begin()) {
            --currIt;
            if (_tryToCompleteListInfoWithAChunk(
                    currIt, currList, pListsToMove, nbMaxOfListElt, pWorkingZone, false, false))
                continue;
...
void ListExtractor::_initNewListInfo(ListInfo& pCurrList, const Chunk& pFirstListEltChunk) const {
...
bool ListExtractor::_updateExistingListInfoAccordingToTheNewElt(ListInfo& pCurrList,
    ...
    if (pCurrList.eltType == ChunkType::VERB_CHUNK) {
        if (pCurrList.requestsOfElts != pNewListEltChunk.requests
            && !(pCurrList.requestsOfElts.empty() && pNewListEltChunk.requests.has(SemanticRequestType::ACTION)))
            return false;

        const auto& headInlfWord = pNewListEltChunk.head->inflWords.front();
        for (auto itCpt = pCurrList.concepts.begin(); itCpt != pCurrList.concepts.end();) {
        ...
        if (pDoesBecomeAListOfInfinitiveVerbs)
            ...
            fFlschecker.intersectionOfSameVerbTenses(pCurrList.commonVerbInflections, verbFlexions);
        return !pCurrList.commonVerbInflections.inflections.empty();
    }
    return true;
}

bool ListExtractor::_doesBecomeAListOfInfinitiveVerbs(const ListInfo& pCurrList, const ChunkType& pNewChunkType) const {
    return fConf.getLanguageType() == SemanticLanguageEnum::ENGLISH && pNewChunkType == ChunkType::INFINITVE_VERB_CHUNK
        && pCurrList.eltType == ChunkType::VERB_CHUNK;
}

bool ListExtractor::_tryToCompleteListInfoWithAChunk(ChunkLinkIter& pCurrIt,
                                                     ListInfo& pCurrList,
                                                     std::list<ListToMove>& pListsToMove,
                                                     int& pNbMaxOfListElt,
    ...
    if (newEltListChunk.getIt() != pWorkingZone.begin()) {
        ...
        haveASubConjBefore = itNextNextChunk->chunk->type == ChunkType::SEPARATOR_CHUNK
                          && itNextNextChunk->chunk->head->inflWords.front().word.partOfSpeech
                                 == PartOfSpeech::SUBORDINATING_CONJONCTION
                          && itNextNextChunk->chunk->head->inflWords.front().infos.hasContextualInfo(
                              WordContextualInfos::SENTENCECANBEGINWITH);
    }
    ...
    if (!haveASubConjBefore
        ...
        if (!pOneWordList) {
            if (fFlschecker.canBeAssociatedInAList(
                    newEltListChunkFirstElt.head->inflWords.front(),
        ...
        // if we consider a list of words
            ...
            if (_getBeginNewEltAndEndOldElt(chunkBegin, chunkEnd, pCurrList.partOfSpeechAtBegining)) {
                if (!chunkBegin.atBegin()
                    ...
                    for (auto itTok = chunkBegin.getItBegin(); itTok != chunkBegin.tokIt;
    ...
    for (auto& currElt : pListsToMove) {
        ...
        if (headPartOfSpeech == PartOfSpeech::PREPOSITION || headPartOfSpeech == PartOfSpeech::PARTITIVE) {
            ...
            currElt.newParent.getIt()->chunk->children.splice(currElt.newParent.getIt()->chunk->children.end(),
                                                              *currElt.itRootList.getList(),
                                                              currElt.itRootList.getIt());
        }
    }
}

bool ListExtractor::_tryToExtractANewListAroundASeparator(ChunkLinkIter& pCurrIt,
                                                          ListInfo& pCurrList,
                                                          std::list<ListToMove>& pListsToMove,
                                                          ...
                                                          bool pAllowToNotRepeatTheSubject,
                                                          const mystd::optional<ChunkType>& pExceptListType) const {
    const auto& currItHeadConcepts = pCurrIt.getIt()->chunk->head->inflWords.front().infos.concepts;
    ...
    if (!pOneWordList && !prevIt.atEnd() && prevIt.getIt()->chunk->type == ChunkType::SEPARATOR_CHUNK) {
    ...
    if (pCurrList.listType && !prevIt.atEnd() && prevIt->type == ChunkLinkType::SIMPLE
        ...
        if (pCurrIt.getIt() != pWorkingZone.begin()) {
            ...
            // if we consider a list with several words in each element
            if (!pOneWordList) {
                if (_addANewMultiWordsList(pCurrList,
                                           pListsToMove,
                                           pNbMaxOfListElt,
                                           prevIt,
                                           pCurrIt,
            ...
            // if we consider a list of words
                ...
                if (_isNewEltCompatibleWithTheList(pCurrList, nextChunkFirstElt, pAllowToNotRepeatTheSubject)) {
                    ...
                    if (_addANewOneWordList(pCurrList, pListsToMove, pNbMaxOfListElt, prevIt, pCurrIt, nextChunk))
                        return true;
                }
            }
