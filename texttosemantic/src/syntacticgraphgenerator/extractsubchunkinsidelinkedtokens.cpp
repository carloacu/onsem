  ...
  for (auto& currChunkLk : pFirstChildren)
  {
    ...
    if (chunkTypeIsVerbal(chunk.type))
    {
      ...
      if (tokenIt->linkedTokens.size() > 1)
      {
        ...
        if (tokenIt != itBeginOfNominalGroup &&
            tokenIt != itEnd)
        {
          ...
          moveEndOfAChunk(chunk, itBeginOfNominalGroup, ChunkLinkType::DIRECTOBJECT,
                          ChunkType::NOMINAL_CHUNK, chunk.children, pLanguage);
