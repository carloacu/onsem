...
void _updateCarryOnValue(CarryOnFrom& pCurrentValue, std::size_t& pNbOfRetries, CarryOnFrom pNewValue) {
    switch (pNewValue) {
        ...
        case CarryOnFrom::SYNTACTIC_TREE:
            ...
            if (pCurrentValue != CarryOnFrom::PARTOFSPEECH_FILTERS)
                pCurrentValue = pNewValue;
            return;
        case CarryOnFrom::HERE: return;
    }
}

ErrorDetector::ErrorDetector(const AlgorithmSetForALanguage& pConfiguration)
...
CarryOnFrom _adjNounLists(std::list<ChunkLink>& pSyntTree) {
    ...
    for (auto itChkLk = pSyntTree.rbegin(); itChkLk != pSyntTree.rend(); ++itChkLk) {
        Chunk& currChk = *itChkLk->chunk;
        if (prevChunk != nullptr && currChk.type == ChunkType::SEPARATOR_CHUNK
            && ConceptSet::haveAConceptThatBeginWith(currChk.head->inflWords.front().infos.concepts, "list_")) {
            auto nextIt = itChkLk;
            ++nextIt;
...
CarryOnFrom ErrorDetector::falseGramPossibilitiesRemoved(std::list<ChunkLink>& pSyntTree,
    ...
    for (ChunkLinkIter chkLkIter(pSyntTree, pSyntTree.begin()); !chkLkIter.atEnd(); ++chkLkIter) {
        ...
        _updateCarryOnValue(
            res, pParsingConfidence.nbOfProblematicRetries, xSolveConjunctionUnlinked(chkLkIter.getIt()));

        if (nextIt.atEnd()) {
            _updateCarryOnValue(
                res, nbOfNotProblematicRetries, xRemoveSubordinatingConjonctionUnliked(*chkLkIter.getIt(), nullptr));
        } else {
            _updateCarryOnValue(res,
                                nbOfNotProblematicRetries,
        ...
        if (currChunkType == ChunkType::PREPOSITIONAL_CHUNK)
            _updateCarryOnValue(
                res, pParsingConfidence.nbOfProblematicRetries, xCorrectFalsePrepositionalChunk(chkLkIter.getIt()));
        if (res == CarryOnFrom::HERE) {
            _updateCarryOnValue(
                res, pParsingConfidence.nbOfProblematicRetries, xRemoveInvalidPronouns(*chkLkIter.getIt()->chunk));
            _updateCarryOnValue(
                res,
                pParsingConfidence.nbOfProblematicRetries,
                xSolveBadVerbChunks(chkLkIter, pParsingConfidence, chkLkIter->type, previousChunkType, firstChunk));
            if (chkLkIter.atEnd())
                return res;
        }
...
bool ErrorDetector::tryToConvertNounToImperativeVerbs(std::list<ChunkLink>& pSyntTree) const {
    bool rem = false;
    for (auto itChunkLink = pSyntTree.begin(); itChunkLink != pSyntTree.end(); ++itChunkLink) {
        ...
        if (chunk.type == ChunkType::SEPARATOR_CHUNK) {
            ...
            if (headInflWorld.infos.hasContextualInfo(WordContextualInfos::CONDITION)) {
                ...
                ++itNextChunk;
                if (itNextChunk != pSyntTree.end() && itNextChunk->chunk->type == ChunkType::VERB_CHUNK
...
bool ErrorDetector::xTryToConvertNounToImperativeVerbsForAChunk(Chunk& pChunk) const {
    ...
    if (pChunk.type == ChunkType::NOMINAL_CHUNK) {
        ...
        if ((headPartOfSpeech == PartOfSpeech::NOUN || headPartOfSpeech == PartOfSpeech::UNKNOWN)
            ...
            for (auto it = pChunk.tokRange.getItBegin(); it != pChunk.tokRange.getItEnd();
                 it = getNextToken(it, pChunk.tokRange.getItEnd())) {
...
void ErrorDetector::frFixOfVerbalChunks(std::list<ChunkLink>& pSyntTree) const {
    ...
    for (auto& currChunkLink : pSyntTree) {
        ...
        if (chunkTypeIsVerbal(currChunk.type)) {
            ...
            if (currChunk.positive) {
                if (currChunk.tokRange.getItBegin()->inflWords.front().infos.hasContextualInfo(
                        WordContextualInfos::NEGATION)) {
                    ...
                    if (nextTokenIt != currChunk.tokRange.getItEnd()
                        ...
                        if (doChunkLinkPtr != nullptr) {
                            if (!doChunkLinkPtr->tokRange.isEmpty()
                                ...
                                currChunk.positive = true;
                            else if (ConceptSet::haveAnyOfConcepts(
                                         doChunkLinkPtr->chunk->tokRange.getItBegin()->inflWords.front().infos.concepts,
                                         {"quantity_nothing", "number_0"}))
                                currChunk.positive = true;
                        }
                    }
                    ...
                    if (!currChunk.positive) {
                        ...
                        if (subjectChunkLinkPtr != nullptr
                            && ConceptSet::haveAnyOfConcepts(
                                subjectChunkLinkPtr->tokRange.getItBegin()->inflWords.front().infos.concepts,
                                {"quantity_nothing", "number_0"}))
                            currChunk.positive = true;
                    }
                    ...
                    for (auto& currChildLink : currChunk.children) {
                        if (currChildLink.chunk->type != ChunkType::VERB_CHUNK
                            && currChildLink.chunk->tokRange.getItBegin()->inflWords.front().infos.hasContextualInfo(
                                WordContextualInfos::NEGATION))
                            currChunk.positive = false;
                    }
                }
...
void ErrorDetector::addYesOrNoRequestForVerbsBeforeInterrogationPunctuation(std::list<ChunkLink>& pSyntTree) {
    std::list<ChunkLink>::iterator itLastChunk = pSyntTree.end();
    for (std::list<ChunkLink>::iterator it = pSyntTree.begin(); it != pSyntTree.end(); ++it) {
        ...
        if (currChunk.type == ChunkType::VERB_CHUNK) {
            ...
            if (currChunk.head->str.find_first_of('?') != std::string::npos && itLastChunk != pSyntTree.end()
                && itLastChunk->chunk->requests.empty() && !haveAQuestionInDirectObject(*itLastChunk->chunk)) {
...
CarryOnFrom ErrorDetector::xSolveConjunctionUnlinked(std::list<ChunkLink>::iterator pItChLk) const {
    SemanticLanguageEnum language = fConfiguration.getLanguageType();
    if (language == SemanticLanguageEnum::ENGLISH && pItChLk->chunk->type == ChunkType::SEPARATOR_CHUNK) {
        ...
        if (headIGram.word.partOfSpeech == PartOfSpeech::CONJUNCTIVE
            ...
            if (headIGram.word.lemma == "then") {
                if (headIGram.infos.concepts.count("list_then") == 0 && !headTok.thisTokenAlreadyCausedARestart) {
            ...
            else if (headIGram.word.lemma == "for" && headTok.inflWords.size() > 1
 ...
 *                   comme moi
 */
CarryOnFrom ErrorDetector::xRemoveSubordinatingConjonctionUnliked(ChunkLink& pItChLk, ChunkLink* pNextItChLk) const {
    if (pItChLk.chunk->type == ChunkType::SEPARATOR_CHUNK && pItChLk.chunk->head->inflWords.size() > 1) {
        ...
        if (headGram == PartOfSpeech::CONJUNCTIVE && pNextItChLk != nullptr
            && (pNextItChLk->chunk->type == ChunkType::NOMINAL_CHUNK || chunkTypeIsAList(pNextItChLk->chunk->type))
 ...
 *             (head: mal)
 */
CarryOnFrom ErrorDetector::xCheckThatNominalGroupHaveAValidPos(std::list<ChunkLink>::iterator pItChLk,
    ...
    if (chunk.type == ChunkType::NOMINAL_CHUNK || chunk.type == ChunkType::PREPOSITIONAL_CHUNK) {
        ...
        if (headPartOfSpeech == PartOfSpeech::DETERMINER || headPartOfSpeech == PartOfSpeech::PREPOSITION) {
            for (TokIt itTok = chunk.tokRange.getItBegin(); itTok != chunk.tokRange.getItEnd();
                ...
                if (itTok->getPartOfSpeech() == PartOfSpeech::INTERJECTION) {
                    if (itNextTok != chunk.tokRange.getItEnd()) {
                        ...
                        moveEndOfAChunk(chunk,
                                        itTok,
                                        ChunkLinkType::INTERJECTION,
                                        ChunkType::INTERJECTION_CHUNK,
                                        pParentVerbChunkPtr->children,
                                        fConfiguration.getLanguageType());
                        break;
                    }
                }
                itTok = itNextTok;
    ...
    for (auto itChild = chunk.children.begin(); itChild != chunk.children.end(); ++itChild) {
        _updateCarryOnValue(res, nbOfRetries, xCheckThatNominalGroupHaveAValidPos(itChild, pParentVerbChunkPtr));
    }
    return res;
}
...
CarryOnFrom ErrorDetector::xRemoveInvalidPronouns(Chunk& pChunk) const {
    ...
    if (pChunk.type == ChunkType::NOMINAL_CHUNK || pChunk.type == ChunkType::PREPOSITIONAL_CHUNK) {
        for (TokIt currIt =
                 getTheNextestToken(pChunk.tokRange.getItBegin(), pChunk.tokRange.getItEnd(), SkipPartOfWord::YES);
            ...
            PartOfSpeech headPartOfSpeech = itFrontHeadInflWords->word.partOfSpeech;
            if (headPartOfSpeech == PartOfSpeech::PRONOUN && inflWords.size() > 1
...
CarryOnFrom ErrorDetector::xCorrectFalsePrepositionalChunk(std::list<ChunkLink>::iterator pItChLk) const {
    Chunk& chunk = *pItChLk->chunk;
    ...
    if (firstTokenPartOfSpeech == PartOfSpeech::PREPOSITION) {
        if (hasAPartOfSpeech(firstToken.inflWords, PartOfSpeech::PARTITIVE)) {
            ...
            if (itSubClause != chunk.children.end()) {
                for (auto it = itSubClause->tokRange.getItBegin(); it != itSubClause->tokRange.getItEnd(); ++it) {
...
void ErrorDetector::xPutRepetitionChildToTheFatherNode(Chunk& pChunk) const {
    for (auto& currChild : pChunk.children) {
        ...
        if (currChild.type == ChunkLinkType::SUBORDINATE_CLAUSE) {
            auto repetitionChild = getChunkLink(subChildChunk, ChunkLinkType::REPETITION);
            if (repetitionChild != subChildChunk.children.end())
                pChunk.children.splice(pChunk.children.end(), subChildChunk.children, repetitionChild);
        }
    }
}

CarryOnFrom ErrorDetector::xSolveBadVerbChunks(ChunkLinkIter& pChkLkIter,
    ...
    if (isAList || fConfiguration.getLanguageType() == SemanticLanguageEnum::FRENCH)    // TODO: remove french filter
    {
        for (ChunkLinkIter it(pChkLkIter->chunk->children, pChkLkIter->chunk->children.begin()); !it.atEnd(); ++it) {
            if (isAList || it->tokRange.isEmpty()
                || fLingDico.statDb.wordToSubordinateRequest(it->tokRange.getItBegin()->inflWords.front().word)
                       != SemanticRequestType::OBJECT) {
                ChunkLinkType subParentChkLk = it->type == ChunkLinkType::SIMPLE ? pParentChkLk : it->type;
    ...
    if (pChkLkIter->chunk->type == ChunkType::VERB_CHUNK) {
        ...
        if (pParentChkLk != ChunkLinkType::SUBJECT_OF && xSolveVerbThatShouldHaveAnAuxiliary(verbChunk)) {
            res = CarryOnFrom::PARTOFSPEECH_FILTERS;
        } else if (haveASubject(verbChunk) || pParentChkLk == ChunkLinkType::SUBJECT_OF
        ...
        static const std::set<ChunkLinkType> ckLkTypes{
            ChunkLinkType::DIRECTOBJECT, ChunkLinkType::SUBORDINATE, ChunkLinkType::SUBORDINATE_CLAUSE};
        if (verbChunk.head->inflWords.front().infos.isOnlyTransitive() && !haveChildWithAuxSkip(verbChunk, ckLkTypes)
 ...
 *                    a
 */
bool ErrorDetector::xSolveVerbThatShouldHaveAnAuxiliary(Chunk& pVerbChunk) const {
    return pVerbChunk.head->inflWords.size() > 1
        ...
        && !haveAnAuxiliary(pVerbChunk) && delAPartOfSpeech(pVerbChunk.head->inflWords, PartOfSpeech::VERB);
}

bool ErrorDetector::xSolveVerbThatHaveASubjectThatBeginsWithAPrep(Chunk& pVerbChunk) const {
...
bool ErrorDetector::xTryToCorrectVerbsWithoutSubject(ChunkLinkIter& pChkLkIter, bool pFirstChunk) const {
    Chunk& verbChunk = *pChkLkIter->chunk;
    ...
    if (verbChunk.requests.empty() && language == SemanticLanguageEnum::FRENCH
    ...
    if ((verbChunk.requests.empty()
        ...
        if (!InflectionsChecker::verbCanHaveNoSubject(verbChunk.head->inflWords.front())) {
            ...
            assert(it
                   != verbChunk.head->inflWords
                          .end());    // because "verbChunk.head->inflWords.size() > 1" on previous condition
            ++it;
            while (it != verbChunk.head->inflWords.end()) {
                if (partOfSpeech_isVerbal(it->word.partOfSpeech) && InflectionsChecker::verbCanHaveNoSubject(*it))
...
void ErrorDetector::xBreakLinkedTokens(Token& pToken) const {
    for (std::list<TokIt>::iterator itLinkedTok = pToken.linkedTokens.begin(); itLinkedTok != pToken.linkedTokens.end();
         ++itLinkedTok) {
