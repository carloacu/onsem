...
void InterjectionalChunker::process(std::list<ChunkLink>& pFirstChildren) const {
            ...
            if (currPartOfSpeech == PartOfSpeech::INTERJECTION) {
                ...
                if (tokenIt != beginNextChunk && tokenIt != itEnd)
                    separateBeginOfAChunk(
                        workingZone.syntTree(), it, tokenIt, it->chunk->type, fConf.getLanguageType());
                ...
                while (beginNextChunk != itEnd) {
                    bool pursue = false;
                    if (!beginNextChunk->linkedTokens.empty() && beginNextChunk->linkedTokens.front() == prevIt)
                        pursue = true;
