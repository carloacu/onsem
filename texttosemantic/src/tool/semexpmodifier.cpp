...
void _setRequest(GroundedExpression& pGrdExp,
...
void clearRequestListOfSemExp(SemanticExpression& pSemExp)
{
  ...
  case SemanticExpressionType::GROUNDED:
    clearRequestList(pSemExp.getGrdExp());
    return;
  case SemanticExpressionType::INTERPRETATION:
    clearRequestListOfSemExp(*pSemExp.getIntExp().interpretedExp);
    return;
  case SemanticExpressionType::FEEDBACK:
    clearRequestListOfSemExp(*pSemExp.getFdkExp().concernedExp);
    return;
  case SemanticExpressionType::ANNOTATED:
    clearRequestListOfSemExp(*pSemExp.getAnnExp().semExp);
    return;
  case SemanticExpressionType::METADATA:
    clearRequestListOfSemExp(*pSemExp.getMetadataExp().semExp);
    return;
                              ...
                              SemanticReferenceType pRefType)
{
  ...
  {
  case SemanticExpressionType::GROUNDED:
    setReference(pSemExp.getGrdExp(), pRefType);
    return;
  case SemanticExpressionType::INTERPRETATION:
  ...
  case SemanticExpressionType::FEEDBACK:
    ...
    return;
  case SemanticExpressionType::ANNOTATED:
    setReferenceTypeOfSemExp(*pSemExp.getAnnExp().semExp, pRefType);
    return;
  case SemanticExpressionType::METADATA:
  ...
  if (grdExpPtr != nullptr)
  {
        ...
        !genGrdPtr->word.lemma.empty())
    {
      ...
      pLingDb.langToSpec[genGrdPtr->word.language].lingDico.getGramPossibilities(
            inflectedWords, genGrdPtr->word.lemma, 0,
            genGrdPtr->word.lemma.size());
                                 ...
                                 std::function<void(SemanticVerbTense&)> pVerbTenseModif)
{
  ...
  {
  case SemanticExpressionType::GROUNDED:
    applyVerbTenseModif(pSemExp.getGrdExp(), pVerbTenseModif);
    return;
  case SemanticExpressionType::INTERPRETATION:
...
{
  applyVerbTenseModif(pGrdExp, [pVerbTense](SemanticVerbTense& pVerbTenseToModify)
  {
    pVerbTenseToModify = pVerbTense;
  });
}
...
void modifyVerbTenseOfSemExp(SemanticExpression& pSemExp,
                             SemanticVerbTense pVerbTense)
{
  applyVerbTenseModifOfSemExp(pSemExp, [pVerbTense](SemanticVerbTense& pVerbTenseToModify)
  {
    pVerbTenseToModify = pVerbTense;
  });
}
                                 ...
                                 std::unique_ptr<SemanticTimeGrounding> pTimeGrd)
{
  ...
  if (grdExpPtr != nullptr)
  {
    ...
    SemanticStatementGrounding* statGrdExpPtr =
        grdExp->getStatementGroundingPtr();
...
void addNewChildWithConcept
(std::map<GrammaticalType, UniqueSemanticExpression>& pChildren,
                                ...
                                ListExpressionType pListType)
{
  ...
  if (grdExpPtr != nullptr)
    for (const auto& currChild : grdExpPtr->children)
      addChildFromSemExp(pSemExpToFill, currChild.first,
                         currChild.second->clone(), pListType);
}
                             ...
                             const std::map<std::string, char>& pLingConcepts)
{
  for (const auto& currLingCpt : pLingConcepts)
  {
    if (ConceptSet::doesConceptBeginWith(currLingCpt.first, "tolink_") ||
        ...
        currLingCpt.first == "agent" ||
        currLingCpt.first == "agent_human" ||
...
bool removeYearInformation(SemanticExpression& pSemExp)
{
  ...
  if (grdExpPtr != nullptr)
  {
    ...
    if (timeGrdPtr != nullptr &&
        timeGrdPtr->date.year && timeGrdPtr->date.month)
    {
...
bool removeDayInformation(SemanticExpression& pSemExp)
{
  ...
  if (grdExpPtr != nullptr)
  {
    auto* timeGrdPtr = grdExpPtr->grounding().getTimeGroundingPtr();
    if (timeGrdPtr != nullptr &&
        timeGrdPtr->date.day && timeGrdPtr->date.month)
    {
