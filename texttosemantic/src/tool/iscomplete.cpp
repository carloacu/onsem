...
{
  for (auto itChild = pSyntGraph.firstChildren.rbegin();
       itChild != pSyntGraph.firstChildren.rend(); ++itChild)
  {
    ...
    case ChunkType::VERB_CHUNK:
    {
      if (currChunk.requests.has(SemanticRequestType::TIME))
      {
        ...
        if (itSubject != currChunk.children.end() &&
            checkOrder(*itSubject->chunk, currChunk))
          return false;
      }
                ...
                SemanticLanguageEnum pLanguageType)
{
  ...
  tokenizationAndSyntacticalAnalysis(syntGraph, pText, spellingMistakeTypesPossible,
                                     cmdGrdExtractorPtr);
