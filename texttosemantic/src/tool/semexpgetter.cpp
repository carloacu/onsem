...
void _getAgentOrNameGrd(const SemanticAgentGrounding** pAgentGrd,
...
USEMEXP* _getCoreferenceFromGrdExp(GRDEXP& pGrdExp, CoreferenceDirectionEnum pCoreferenceDirection) {
        ...
        if (grdExpPtr != nullptr
            && (grdExpPtr->children.empty()
                || (grdExpPtr->children.size() == 1
                    && grdExpPtr->children.begin()->first == GrammaticalType::INTRODUCTING_WORD))) {
...
bool _isMandatoryGrdExp(const GroundedExpression& pGrdExp) {
    ...
    return statGrdPtr != nullptr
        && (statGrdPtr->isMandatoryInPresentTense() || ConceptSet::haveAConcept(statGrdPtr->concepts, "verb_haveto"));
}
...
std::unique_ptr<SemanticGrounding> _extractQuantityFromGrdExp(const GroundedExpression& pGrdExp,
    ...
    if (grd.type == SemanticGroundingType::GENERIC) {
        if (pUnityGrdPtr == nullptr) {
            ...
            if ((!pSimpleNumber || genGrd.word.isEmpty()) && genGrd.quantity.type == SemanticQuantityType::NUMBER) {
        ...
        if (pUnityGrdPtr->typeOfUnity == TypeOfUnity::TIME) {
            ...
            if (itSpec != pGrdExp.children.end()) {
                ...
                if (specGrdExpPtr != nullptr && specGrdExpPtr->grounding().type == SemanticGroundingType::DURATION) {
...
void _extractAskedChildrenFromGrdExp(std::set<GrammaticalType>& pAskedChildren,
...
void _extractAskedChildrenByAResourceFromGrdExp(std::set<GrammaticalType>& pAskedChildren,
                                                const GroundedExpression& pGrdExp) {
...
bool isNominal(const SemanticGrounding& pGrd) {
    if (pGrd.getNameGroundingPtr() != nullptr || pGrd.getAgentGroundingPtr() != nullptr)
        return true;
    const auto* genGrdPtr = pGrd.getGenericGroundingPtr();
    return genGrdPtr != nullptr && partOfSpeech_isNominal(genGrdPtr->word.partOfSpeech);
}
...
SemanticEntityType getEntity(const SemanticExpression& pSemExp) {
    ...
    if (listExpPtr != nullptr && listExpPtr->elts.size() >= 1)
...
bool isPassive(const GroundedExpression& pGrdExp) {
    const auto* statGrdPtr = pGrdExp->getStatementGroundingPtr();
    return statGrdPtr != nullptr && !statGrdPtr->requests.has(SemanticRequestType::SUBJECT)
...
SemanticNumberType _getNumberFromNominalOrPronominalInflections(const INFLECTIONS& pInflections) {
    ...
    if (numbers.size() == 1 && *numbers.begin() == SemanticNumberType::PLURAL)
        return SemanticNumberType::PLURAL;
...
SemanticNumberType getNumberFromInflections(const linguistics::InflectedWord& pInflections) {
    ...
    switch (infls.type) {
        case InflectionType::NOMINAL: return _getNumberFromNominalOrPronominalInflections(infls.getNominalI());
        case InflectionType::PRONOMINAL: return _getNumberFromNominalOrPronominalInflections(infls.getPronominalI());
        default: return SemanticNumberType::UNKNOWN;
    }
}
...
SemanticLanguageEnum getLanguage(const std::map<GrammaticalType, UniqueSemanticExpression>& pAnnotations) {
    ...
    if (languageChild != pAnnotations.end()) {
        ...
        if (langGrdExpPtr != nullptr && langGrdExpPtr->grounding().type == SemanticGroundingType::LANGUAGE)
...
std::unique_ptr<SemanticGrounding> mergeQuantities(const SemanticGrounding& pPreviousQuantity,
    ...
    if (pNewQuantity) {
        if (pPreviousQuantity.type == SemanticGroundingType::GENERIC
            ...
            if (prevQuantity.type == SemanticQuantityType::NUMBER && newQuantity.type == SemanticQuantityType::NUMBER)
...
bool doesSemExpContainsOnlyARequest(const SemanticExpression& pSemExp) {
    ...
    if (grdExpPtr != nullptr) {
        ...
        return reqListPtr != nullptr && !reqListPtr->empty();
    }
    return false;
}
...
bool hasChildOtherThanParameterToFill(const GroundedExpression& pGrdExp, GrammaticalType pChildType) {
    ...
    if (itChild != pGrdExp.children.end()) {
        ...
        if (childGrdExpPtr != nullptr) {
            ...
            return childCptGrdPtr == nullptr
                || !ConceptSet::haveAConcept(childCptGrdPtr->concepts, "stuff_informationToFill");
        }
        return true;
    }
    return false;
...
const GroundedExpression* getUnnamedGrdExpPtr(const SemanticExpression& pSemExp) {
    switch (pSemExp.type) {
        ...
        case SemanticExpressionType::LIST: return nullptr;
    }
    assert(false);
    return nullptr;
}
...
bool isNow(const GroundedExpression& pGrdExp) {
    ...
    if (itTimeChild != pGrdExp.children.end()) {
        ...
        if (timeGrdExpPtr != nullptr) {
            ...
            return timeGrdPtr != nullptr && timeGrdPtr->isEqualMoreOrLess10Seconds(SemanticTimeGrounding::now());
        }
    }
    return false;
}
...
bool doesSemExpCanBeCompletedWithContext(const SemanticExpression& pSemExp) {
    ...
    return grdExpPtr == nullptr || grdExpPtr->grounding().type != SemanticGroundingType::AGENT;
}

const SemanticExpression* getTimeInfo(const SemanticExpression& pSemExp) {
    const SemanticExpression* res = SemExpGetter::getGrammTypeInfo(pSemExp, GrammaticalType::TIME);
    if (res != nullptr)
        return res;

    const SemanticExpression* objectChildPtr = SemExpGetter::getGrammTypeInfo(pSemExp, GrammaticalType::OBJECT);
...
bool hasSpecifications(const SemanticExpression& pSemExp) {
    ...
    if (grdExpPtr != nullptr) {
        ...
        for (const auto& currChild : grdExp.children) {
            if (currChild.first == GrammaticalType::BETWEEN || currChild.first == GrammaticalType::OWNER
                || currChild.first == GrammaticalType::TIME || currChild.first == GrammaticalType::TODO)
                return true;
...
bool isAModifierFromGrdExp(const GroundedExpression& pGrdExp) {
    ...
    return genGrdPtr != nullptr && genGrdPtr->entityType == SemanticEntityType::MODIFIER;
}
...
SemanticRequestType convertSemGramToRequestType(GrammaticalType pGramType) {
    switch (pGramType) {
        case GrammaticalType::SUBJECT: return SemanticRequestType::SUBJECT;
        case GrammaticalType::OBJECT: return SemanticRequestType::OBJECT;
        case GrammaticalType::LOCATION: return SemanticRequestType::LOCATION;
        case GrammaticalType::MANNER: return SemanticRequestType::MANNER;
        case GrammaticalType::TIME: return SemanticRequestType::TIME;
...
bool isSemExpEqualToAnAgent(const SemanticExpression& pSemExp, const SemanticAgentGrounding& pAgentGrd) {
    ...
    if (grdExp != nullptr) {
        ...
        return agentGrdPtr != nullptr && agentGrdPtr->userId == pAgentGrd.userId;
    }
    return false;
}
...
bool doesSemExpHaveAnAgent(const SemanticExpression& pSemExp, const SemanticAgentGrounding& pAgentGrd) {
    switch (pSemExp.type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            if (agentGrdPtr != nullptr && agentGrdPtr->userId == pAgentGrd.userId)
                return true;
            for (const auto& currChild : grdExp.children)
        ...
        case SemanticExpressionType::COMMAND: return false;
    };
    assert(false);
    return false;
}
...
bool semExpIsAnEmptyStatementGrd(const SemanticExpression& pSemExp) {
    auto* grdExpPtr = pSemExp.getGrdExpPtr_SkipWrapperPtrs();
    return grdExpPtr != nullptr && grdExpIsAnEmptyStatementGrd(*grdExpPtr);
}
...
bool doesContainADialogSource(const SemanticExpression& pSemExp) {
    ...
    return from == SemanticSourceEnum::WRITTENTEXT || from == SemanticSourceEnum::TTS
        || from == SemanticSourceEnum::SEMREACTION || from == SemanticSourceEnum::ASR;
}
...
bool isASpecificHuman(const SemanticGenericGrounding& pGenGrd) {
    return pGenGrd.quantity.isEqualToOne() && pGenGrd.referenceType == SemanticReferenceType::DEFINITE
...
bool isAResourceOrATextFromGrdExp(const GroundedExpression& pGrdExp) {
    return pGrdExp->type == SemanticGroundingType::RESOURCE || pGrdExp->type == SemanticGroundingType::TEXT;
}
...
bool isSemExpAQuestion(const SemanticExpression& pSemExp) {
    switch (pSemExp.type) {
        case SemanticExpressionType::ANNOTATED: return isSemExpAQuestion(*pSemExp.getAnnExp().semExp);
        case SemanticExpressionType::COMMAND: return isSemExpAQuestion(*pSemExp.getCmdExp().semExp);
        case SemanticExpressionType::COMPARISON: {
            const auto& compExp = pSemExp.getCompExp();
            if (isSemExpAQuestion(*compExp.leftOperandExp))
                return true;
            if (compExp.rightOperandExp && isSemExpAQuestion(**compExp.rightOperandExp))
                return true;
            return false;
        }
        case SemanticExpressionType::CONDITION: return false;
        case SemanticExpressionType::FEEDBACK: return isSemExpAQuestion(*pSemExp.getFdkExp().concernedExp);
        case SemanticExpressionType::GROUNDED: return isGrdExpAQuestion(pSemExp.getGrdExp());
        case SemanticExpressionType::INTERPRETATION: return isSemExpAQuestion(*pSemExp.getIntExp().interpretedExp);
        case SemanticExpressionType::LIST: {
            const auto& listExp = pSemExp.getListExp();
            if (!listExp.elts.empty()) {
        ...
        case SemanticExpressionType::SETOFFORMS: return true;
        case SemanticExpressionType::FIXEDSYNTHESIS: return isSemExpAQuestion(pSemExp.getFSynthExp().getSemExp());
    }
    return false;
}
...
void extractSubjectAndObjectOfAVerbDefinition(std::list<const GroundedExpression*>& pSubjectGrdPtrs,
    ...
    if (statGrdPtr == nullptr || !ConceptSet::haveAConcept(statGrdPtr->concepts, "verb_equal_mean"))
        return;
    ...
    itSubject->second->getGrdExpPtrs_SkipWrapperLists(
        subjectGrdPtrs, true, false, false, [](const GroundedExpression& pGrdExp) {
    ...
    for (const auto& currObjPtr : objectGrdExpPtrs)
        if (!isAnInfinitiveGrdExp(*currObjPtr) && currObjPtr->grounding().getMetaGroundingPtr() == nullptr
...
void extractTeachElements(const GroundedExpression*& pPurposeGrdPtr,
    ...
    if (purposeGrdExpPtr == nullptr || !isAnInfinitiveGrdExp(*purposeGrdExpPtr))
        return;
...
void _recIterateOnList(std::list<UniqueSemanticExpression*>& pRes, UniqueSemanticExpression& pUSemExp) {
...
void _recIterateOnList(std::list<const SemanticExpression*>& pRes, const SemanticExpression& pSemExp) {
    auto* grdExpPtr = pSemExp.getGrdExpPtr_SkipWrapperPtrs();
    if (grdExpPtr != nullptr) {
...
void _recIterateOnListOfGrdExps(std::list<TGRDEXP*>& pRes, TSEMEXP& pSemExp) {
...
bool isAnInfinitiveGrdExp(const GroundedExpression& pGrdExp) {
    const SemanticStatementGrounding* statGrdPtr = pGrdExp->getStatementGroundingPtr();
    if (statGrdPtr != nullptr && statGrdPtr->isAtInfinitive())
        return true;
    return false;
}

bool isATeachingElement(const GroundedExpression& pGrdExp) {
    return _isMandatoryGrdExp(pGrdExp) || pGrdExp->getMetaGroundingPtr() != nullptr
        || pGrdExp->getResourceGroundingPtr() != nullptr;
}

bool isNothing(const GroundedExpression& pGrdExp) {
    const auto* genGrdPtr = pGrdExp->getGenericGroundingPtr();
    return genGrdPtr != nullptr && genGrdPtr->quantity.isEqualToZero() && genGrdPtr->concepts.empty()
...
int getRank(const SemanticExpression& pSemExp) {
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (!rankConcepts.empty()) {
            ...
            std::size_t posOfRankNumber =
                isAnInverseRank ? beginOfrankInverseCptName.size() : beginOfRankCptName.size();
...
std::unique_ptr<GroundedExpression> getCopyExceptChild(const GroundedExpression& pGrdExp,
    ...
    for (const auto& currChild : pGrdExp.children)
        if (currChild.first != pGrammaticalTypeToSkip)
            res->children.emplace(currChild.first, currChild.second->clone());
    return res;
}
...
bool agentIsTheSubject(const GroundedExpression& pGrdExp, const std::string& pSubjectUserId) {
    ...
    if (itSubject != pGrdExp.children.end()) {
        ...
        if (subjectGrdExpPtr != nullptr) {
            ...
            return agentGrdPtr != nullptr && agentGrdPtr->userId == pSubjectUserId;
        }
    }
    return false;
}

bool isACoreference(const SemanticExpression& pSemExp, CoreferenceDirectionEnum pDirection, bool pShouldNotHaveALemma) {
    const GroundedExpression* grdExpPtr = pSemExp.getGrdExpPtr_SkipWrapperPtrs();
    return grdExpPtr != nullptr && _isACoreferenceFromGrdExp(*grdExpPtr, pDirection, pShouldNotHaveALemma);
}
...
bool hasACoreference(const SemanticExpression& pSemExp, CoreferenceDirectionEnum pCorefenceDirection) {
    const GroundedExpression* grdExpPtr = pSemExp.getGrdExpPtr_SkipWrapperPtrs();
    if (grdExpPtr != nullptr) {
...
const UniqueSemanticExpression* getCoreferenceAfterFromGrdExp(const GroundedExpression& pGrdExp) {
    return _getCoreferenceFromGrdExp<const UniqueSemanticExpression, const GroundedExpression>(
        pGrdExp, CoreferenceDirectionEnum::AFTER);
}

UniqueSemanticExpression* getCoreferenceAfterFromGrdExp(GroundedExpression& pGrdExp) {
    return _getCoreferenceFromGrdExp<UniqueSemanticExpression, GroundedExpression>(pGrdExp,
                                                                                   CoreferenceDirectionEnum::AFTER);
}
...
bool getSubjectAndObjectUserIdsOfNameAssignement(std::string& pSubjectUserId,
    ...
    if (!equCpt && statGrd.concepts.count("predicate_hasName") == 0)
        return false;

    const GroundedExpression* objectGrdExpPtr = SemExpGetter::getGrdExpChild(pGrdExp, GrammaticalType::OBJECT);
    if (objectGrdExpPtr == nullptr)
        return false;

    const GroundedExpression* subjectGrdExpPtr = SemExpGetter::getGrdExpChild(pGrdExp, GrammaticalType::SUBJECT);
    if (subjectGrdExpPtr == nullptr)
        return false;
...
bool isANameAssignement(const GroundedExpression& pGrdExp) {
    ...
    return getSubjectAndObjectUserIdsOfNameAssignement(
        subjectUserId, objectUserId, subjectGrdExpPtr, objectGrdExpPtr, pGrdExp);
}
...
UniqueSemanticExpression returnAPositiveSemExpBasedOnAnInput(const GroundedExpression& pInputGrdExp) {
    ...
    return std::make_unique<GroundedExpression>(
        std::make_unique<SemanticConceptualGrounding>("sentiment_positive_joy"));
}
...
bool extractExternalTeachingLabel(const GroundedExpression& pGrdExp,
                                  ...
                                  const std::string& pAuthorUserId) {
    if (pStatGrd.verbTense != SemanticVerbTense::FUTURE || pAuthorUserId.empty()
    ...
    if (subStatGrdPtr == nullptr || subStatGrdPtr->verbTense != SemanticVerbTense::UNKNOWN)
        return false;
...
bool isAnExtractExternalTeaching(const GroundedExpression& pGrdExp, const std::string& pAuthorUserId) {
    ...
    return extractExternalTeachingLabel(
        pGrdExp, *statGrdPtr, externalTeachingLabelSemExpPtr, teachingLanguage, pAuthorUserId);
}
...
bool isWhoSemExp(const SemanticExpression& pSemExp) {
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (genGrdPtr != nullptr) {
            ...
            return genGrd.word.isEmpty() && !genGrd.coreference && genGrd.entityType == SemanticEntityType::HUMAN
...
bool isAnything(const GroundedExpression& pGrdExpToLookFor) {
    ...
    if (conceptOfTheGrdExp.empty()) {
        ...
        if (genGrdPtr != nullptr) {
            const SemanticGenericGrounding& genGrd = *genGrdPtr;
            return genGrd.word.isEmpty() && !genGrd.coreference
...
void getStatementSubordinates(std::set<const SemanticExpression*>& pStatementSubordinates,
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (pSearchOnRootSemExp && grdExp->type == SemanticGroundingType::STATEMENT)
...
std::unique_ptr<GroundedExpression> getASimplifiedVersionFromGrdExp(const GroundedExpression& pGrdExp) {
    auto res = std::make_unique<GroundedExpression>(pGrdExp.cloneGrounding());
    if (pGrdExp->type != SemanticGroundingType::AGENT && pGrdExp->type != SemanticGroundingType::NAME)
        for (const auto& currChild : pGrdExp.children)
...
UniqueSemanticExpression getASimplifiedVersion(const SemanticExpression& pSemExp) {
    switch (pSemExp.type) {
        ...
        case SemanticExpressionType::CONDITION: {
            ...
            auto res = std::make_unique<ConditionExpression>(condExp.isAlwaysActive,
        ...
        case SemanticExpressionType::COMPARISON: {
            const auto& compExp = pSemExp.getCompExp();
            auto res =
                std::make_unique<ComparisonExpression>(compExp.op, getASimplifiedVersion(*compExp.leftOperandExp));
...
bool hasGenericConcept(const UniqueSemanticExpression* pUSemExpPtr) {
    ...
    return grdExpPtr != nullptr && ConceptSet::haveAConcept(grdExpPtr->grounding().concepts, "generic");
}
...
std::vector<GrammaticalType> requestToGrammaticalTypes(SemanticRequestType pRequestType,
    ...
    switch (pRequestType) {
        case SemanticRequestType::CAUSE: return {GrammaticalType::CAUSE, GrammaticalType::PURPOSE};
        case SemanticRequestType::DURATION: return {GrammaticalType::DURATION, GrammaticalType::OBJECT};
        case SemanticRequestType::LOCATION: return {GrammaticalType::LOCATION};
        case SemanticRequestType::MANNER: return {GrammaticalType::MANNER, GrammaticalType::OBJECT};
        case SemanticRequestType::PURPOSE: return {GrammaticalType::PURPOSE};
        case SemanticRequestType::SUBJECT: return {GrammaticalType::SUBJECT};
        case SemanticRequestType::TIME: return {GrammaticalType::TIME};
        case SemanticRequestType::TOPIC: return {GrammaticalType::TOPIC};
        case SemanticRequestType::WAY: return {GrammaticalType::WAY};

        case SemanticRequestType::OBJECT:
        case SemanticRequestType::CHOICE: return {GrammaticalType::OBJECT};
        ...
        case SemanticRequestType::YESORNO: return {};
    }
