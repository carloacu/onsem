...
SemanticVerbTense _verbTimeTense(VerbGoalEnum& pVerbGoal, bool pRootIsAVerb, const VerbalInflection& pVerbInfl) {
    switch (pVerbInfl.tense) {
        ...
        case LinguisticVerbTense::INFINITIVE: return SemanticVerbTense::UNKNOWN;
    }
...
bool _isAdjAdjCompatibles(const Inflections& pAdjInfls1, const Inflections& pAdjInfls2) {
    if (pAdjInfls1.type != InflectionType::ADJECTIVAL || pAdjInfls2.type != InflectionType::ADJECTIVAL)
        return true;
...
bool _conjVerbCanBeAt3ePers(const VerbalInflection& pVerbInfl) {
    return pVerbInfl.tense == LinguisticVerbTense::PAST_PARTICIPLE
        || pVerbInfl.tense == LinguisticVerbTense::PRESENT_PARTICIPLE || _verbSetAt3ePers(pVerbInfl);
}

bool _isNounVerbCompatibles(const Inflections& pNomInflections, const Inflections& pVerbInflections) {
    if (pVerbInflections.type == InflectionType::VERBAL) {
...
bool _isPronounNounCompatibles(const InflectedWord& pIGramPron, const LinguisticDictionary& pLingDic) {
    return pLingDic.semWordToRequest(pIGramPron.word) == SemanticRequestType::OBJECT;
}

bool _isPronounAdjCompatibles(const InflectedWord& pIGramPron, const LinguisticDictionary& pLingDic) {
    return pLingDic.semWordToRequest(pIGramPron.word) == SemanticRequestType::OBJECT;
}

bool _isPronounSubjectNounCompatibles(const Inflections& pPronInfls, const Inflections& pNomInfls) {
...
bool _isPronounSubjectAdjCompatibles(const Inflections& pPronInfls, const Inflections& pAdjInfls) {
    const PronominalInflections* pronInfls = pPronInfls.getPronominalIPtr();
    if (pronInfls == nullptr || pronInfls->inflections.empty())
        return true;
    else {
...
bool _verbCanHaveASubject(const VerbalInflection& pVerbInfl) {
    return pVerbInfl.tense != LinguisticVerbTense::INFINITIVE && pVerbInfl.tense != LinguisticVerbTense::PAST_PARTICIPLE
        && pVerbInfl.tense != LinguisticVerbTense::PRESENT_PARTICIPLE
        && pVerbInfl.tense != LinguisticVerbTense::PRESENT_IMPERATIVE;
}

bool _isPronounVerbCompatibles(const InflectedWord& pIGramPron, const InflectedWord& pIGramVerb) {
    ...
    if (verbInfls.type == InflectionType::VERBAL) {
        for (const auto& currVerbInfl : verbInfls.getVerbalI().inflections) {
            ...
            if ((pronInflsPtr == nullptr || pronInflsPtr->inflections.empty()) && _conjVerbCanBeAt3ePers(currVerbInfl))
                return true;
...
bool _isPronounSubjectVerbCompatibles(const InflectedWord& pIGramPron, const InflectedWord& pIGramVerb) {
    const Inflections& verbInfls = pIGramVerb.inflections();
    if (verbInfls.type == InflectionType::VERBAL) {
        for (const auto& currVerbInfl : verbInfls.getVerbalI().inflections) {
            if (!_verbCanHaveASubject(currVerbInfl))
...
InflectionsChecker::InflectionsChecker(const SpecificLinguisticDatabase& pSpecLingDb)
...
bool InflectionsChecker::areCompatibles(const InflectedWord& pIGram1, const InflectedWord& pIGram2) const {
...
void InflectionsChecker::unionOfSameVerbTenses(VerbalInflections& pResVerInfls,
                                               const Inflections& pInflections1,
                                               const Inflections& pInflections2) const {
    if (pInflections1.type == InflectionType::VERBAL && pInflections2.type == InflectionType::VERBAL) {
...
void InflectionsChecker::intersectionOfSameVerbTenses(VerbalInflections& pExistingVerbInfls,
                                                      const VerbalInflections& pNewVerbInfls) const {
    for (auto it = pExistingVerbInfls.inflections.begin(); it != pExistingVerbInfls.inflections.end();) {
        bool flexionExists = false;
...
bool InflectionsChecker::verbIsOnlyAtPastParticiple(const InflectedWord& pIGram) const {
...
bool InflectionsChecker::verbIsOnlyAtPresentOrPastParticiple(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL) {
        const VerbalInflections& verbInfls = infls.getVerbalI();
        for (const auto& currInfl : verbInfls.inflections)
...
bool InflectionsChecker::verbIsAtPastParticiple(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
...
bool InflectionsChecker::verbCanBeAtImperative(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbCanBeAtThridOfSingularExceptImperative(const InflectedWord& pInflWord) {
...
bool InflectionsChecker::verbIsAtPresentIndicative(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbIsAtPresentParticiple(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
...
bool InflectionsChecker::verbalInflAreAtPresentParticiple(const VerbalInflections& pVerbalInfls) {
...
bool InflectionsChecker::verbRemoveAllInflectionsThatAreNotAtPastParticiple(InflectedWord& pInflWord) {
    bool res = false;
...
bool InflectionsChecker::verbIsAtInfinitive(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbIsConjugated(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
            if (currInfl.tense != LinguisticVerbTense::INFINITIVE)
...
bool InflectionsChecker::verbCanBeSingular(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbCanBePlural(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbIsConjAtPerson(const InflectedWord& pInflWord, RelativePerson pPerson) const {
    const Inflections& infls = pInflWord.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbCanHaveAnAuxiliary(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
bool InflectionsChecker::verbHasToHaveAnAuxiliary(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL) {
        const VerbalInflections& verbInfls = infls.getVerbalI();
        for (const auto& currInfl : verbInfls.inflections)
...
void InflectionsChecker::initGenderSetFromIGram(std::set<SemanticGenderType>& pPossibleGenders,
...
bool InflectionsChecker::verbCantHaveSubject(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
...
RelativePerson InflectionsChecker::imperativeVerbToRelativePerson(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::VERBAL)
        for (const auto& currInfl : infls.getVerbalI().inflections)
            if (currInfl.tense == LinguisticVerbTense::PRESENT_IMPERATIVE) {
...
bool InflectionsChecker::pronounSetAtSamePers(const InflectedWord& pIGram1, const InflectedWord& pIGram2) const {
    ...
    if (infls1.type == InflectionType::PRONOMINAL && infls2.type == InflectionType::PRONOMINAL) {
        ...
        return pronInfls1.inflections.size() == 1 && pronInfls2.inflections.size() == 1
            && pronInfls1.inflections.front().person() == pronInfls2.inflections.front().person();
    }
    return false;
}

bool InflectionsChecker::pronounCanBePlural(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
...
void InflectionsChecker::pronounRemovePluralPossibility(InflectedWord& pIGram) const {
    Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::PRONOMINAL) {
        auto& pronInfls = infls.getPronominalI();
        for (auto itInfl = pronInfls.inflections.begin(); itInfl != pronInfls.inflections.end();) {
...
void InflectionsChecker::pronounRemoveSingularPossibility(InflectedWord& pIGram) const {
    Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::PRONOMINAL) {
        auto& pronInfls = infls.getPronominalI();
        for (auto itInfl = pronInfls.inflections.begin(); itInfl != pronInfls.inflections.end();) {
...
bool InflectionsChecker::nounCanBePlural(const InflectedWord& pIGram) {
    const Inflections& infls = pIGram.inflections();
...
bool InflectionsChecker::adjCanBeComparative(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
...
bool InflectionsChecker::pronounSetAt3ePers(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::PRONOMINAL)
        for (const auto& currInfl : infls.getPronominalI().inflections)
...
bool InflectionsChecker::pronounCanReferToA3ePers(const InflectedWord& pIGram) const {
    const Inflections& infls = pIGram.inflections();
    if (infls.type == InflectionType::PRONOMINAL) {
...
RelativePerson InflectionsChecker::pronounGetPerson(const InflectedWord& pIGram) const {
...
bool InflectionsChecker::areVerbAndPronComplCanBeLinked(const Inflections& pPronComplInflections,
...
bool InflectionsChecker::canBeAssociatedInAList(const InflectedWord& pIGram1, const InflectedWord& pIGram2) const {
    if (pIGram1.word.partOfSpeech == pIGram2.word.partOfSpeech || pIGram1.word.partOfSpeech == PartOfSpeech::UNKNOWN
    ...
    switch (pIGram1.word.partOfSpeech) {
        case PartOfSpeech::PRONOUN: {
            return pIGram2.word.partOfSpeech == PartOfSpeech::PRONOUN_COMPLEMENT
                ...
                || (pIGram2.word.partOfSpeech == PartOfSpeech::NOUN
                    && !fBinDico.hasContextualInfo(WordContextualInfos::POSSESSIVE, pIGram1.word));
        }
        case PartOfSpeech::PRONOUN_COMPLEMENT: {
        ...
        case PartOfSpeech::NOUN: {
            return (pIGram2.word.partOfSpeech == PartOfSpeech::PRONOUN
                    && !fBinDico.hasContextualInfo(WordContextualInfos::POSSESSIVE, pIGram2.word))
