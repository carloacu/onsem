...
PartOfSpeechPatternMatcher::PartOfSpeechPatternMatcher
...
bool PartOfSpeechPatternMatcher::process
(std::vector<Token>& pTokens) const
{
       ...
       itCurrToken = getNextToken(itCurrToken, pTokens.end()))
  {
    ...
    auto itRootGram = [this, &itCurrToken]
    {
      ...
      {
        for (auto itIGram = itCurrToken->inflWords.begin(); itIGram != itCurrToken->inflWords.end(); ++itIGram)
 ...
 std::vector<Token>& pTokens) const
{
  WorkStructForADirection workStructBefore
      (pRes, pContextPoss.before, true, pRootIGram,
       pItCurrToken, pTokens.begin(), pTokens.end());
  ...
  if (workStructBefore.isLinked != LinkedValue::NOT_LINKED)
  {
    WorkStructForADirection workStructAfter
        (pRes, pContextPoss.after, false, pRootIGram,
         pItCurrToken, pTokens.begin(), pTokens.end());
    ...
    if (workStructBefore.isLinked == LinkedValue::UNKNOWN ||
        workStructAfter.isLinked == LinkedValue::UNKNOWN)
...
void PartOfSpeechPatternMatcher::_processCurrRule
(WorkStructForADirection& pWorkStruct,
 bool pIsTheFirstLoopInThisRule) const
{
  ...
  if (pWorkStruct.itCurrToken == pWorkStruct.tokEndIt)
  {
    ...
    {
      if (pWorkStruct.itCurrRule->canBeEmpty == CanBeEmpty::NO &&
          !pWorkStruct.itCurrRule->canBeAPunctuation())
      {
...
void PartOfSpeechPatternMatcher::_processRuleWithoutSkip
(WorkStructForADirection& pWorkStruct) const
{
...
LinkedValue PartOfSpeechPatternMatcher::_doesRuleMatchWithCurrToken
(WorkStructForADirection& pWorkStruct,
 const TaggerListOfTokenChecks& pRule) const
{
  auto tryToHadAMatchedToken = [this, &pWorkStruct]
      (const TaggerTokenCheck& pCurrIGramPoss,
      const std::list<InflectedWord>::iterator& pItSubIGram)
  {
    if (pCurrIGramPoss.iGramMatcher(*pItSubIGram))
    {
      ...
      {
      case CompatibilityCheck::IS_COMPATIBLE:
        compatibilityIsOk = isCompatibleFn();
        break;
      case CompatibilityCheck::ISNT_COMPATIBLE:
        compatibilityIsOk = !isCompatibleFn();
        break;
      case CompatibilityCheck::DONT_CARE:
        compatibilityIsOk = true;
        break;
      }
  ...
  for (const auto& currIGramPoss : pRule.elts)
  {
    ...
    case FinderConstraint::HAS:
    {
      ...
      for (auto itSubIGram = currToken.inflWords.begin();
           itSubIGram != currToken.inflWords.end(); ++itSubIGram)
      {
    ...
    case FinderConstraint::FIRST_ELT:
    {
      ...
      if (tryToHadAMatchedToken(currIGramPoss,
    ...
    case FinderConstraint::ONLY_ONE_ELT:
    {
      if (currToken.inflWords.size() == 1 &&
          tryToHadAMatchedToken(currIGramPoss,
                                currToken.inflWords.begin()))
      {
        res = currIGramPoss.linkedValue;
        stopForLoop = true;
...
{
  if (isLinked == LinkedValue::NOT_LINKED ||
      (isLinked == LinkedValue::UNKNOWN &&
       pIsLinked == LinkedValue::LINKED))
    return;
  isLinked = pIsLinked;
}
