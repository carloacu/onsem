...
bool PartOfSpeechDelBigramImpossibilities::process(std::vector<Token>& pTokens) const {
    ...
    for (std::size_t i = 0; i < fCheckCompatibilityAtTheBeginning.size(); ++i) {
        if (prevIt->inflWords.front().word.partOfSpeech == fCheckCompatibilityAtTheBeginning[i]
            && !fFls.areCompatibles(InflectedWord::getPuntuationIGram(), prevIt->inflWords.front())) {
...
bool PartOfSpeechDelBigramImpossibilities::xDelBigramImpossibilitiesForAToken(
    ...
    for (auto itGram = pTokenItToClean->inflWords.begin(); itGram != pTokenItToClean->inflWords.end();) {
