...
PartOfSpeechCustomFilter::PartOfSpeechCustomFilter
...
bool PartOfSpeechCustomFilter::process
(std::vector<Token>& pTokens) const
{
  ...
  if (fLabel == "gatherLinkedMeanings")
  {
    bool ifDel = false;
    // iterate over all the tokens of the text
    for (auto itTok = getPrevToken(pTokens.end(), pTokens.begin(), pTokens.end(), SkipPartOfWord::YES); itTok != pTokens.end();
        ...
        for (const LingWordsGroup& meanGroup : itIGram->infos.metaMeanings)
        {
          ...
          if (xTryToLinkAToken(linkedTokens, pTokens, itTok, itIGram, meanGroup))
          {
            ...
            SemExpGetter::wordToAMeaning(rootMeaning, *meanGroup.rootWord,
                                         fSpecLingDb.language, fSpecLingDb.lingDb);
 ...
 const LingWordsGroup& pMeanGroup) const
{
  ...
  for (const auto& currLkMean : pMeanGroup.linkedMeanings)
  {
    ...
    {
      upTok = xSearchWordAfter(pTokens, onlyItIGramToKeep, upTok, currSemWord,
                               *pRootItIGram);
    ...
    {
      downTok = xSearchWordBefore(pTokens, onlyItIGramToKeep, downTok, currSemWord,
                                  *pRootItIGram);
    ...
    {
      TokIt resTok = xSearchWordBefore(pTokens, onlyItIGramToKeep, downTok, currSemWord,
                                       *pRootItIGram);
...
(std::vector<Token>& pTokens,
 ...
 const InflectedWord& pMainIGram) const
{
  for (TokIt itTok = getPrevToken(pCurrTok, pTokens.begin(), pTokens.end(), SkipPartOfWord::YES); itTok != pTokens.end();
       itTok = getPrevToken(itTok, pTokens.begin(), pTokens.end(), SkipPartOfWord::YES))
  {
...
bool PartOfSpeechCustomFilter::xCanBeLinked
 ...
 const InflectedWord& pSubIGram) const
{
  ...
  {
    return fFls.areVerbAndPronComplCanBeLinked(pSubIGram.inflections(),
                                               pMainIGram.inflections());
  }
  return true;
}
...
void PartOfSpeechCustomFilter::xTestOfPuttingNounAtBottomOfListIfOccurTwoTimesInARow
(std::vector<Token>& pTokens,
...
void PartOfSpeechCustomFilter::xPutNounAtBottomIfNecessary
