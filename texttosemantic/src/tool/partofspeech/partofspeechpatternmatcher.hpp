...
{
public:
  PartOfSpeechPatternMatcher(const std::string& pName,
...
private:
  ...
  {
    InfoGramIts
    (const std::vector<Token>::iterator& pItToken,
     const std::list<InflectedWord>::iterator& pItInfoGram)
      : itToken(pItToken),
  ...
  struct WorkStructForADirection
  {
    ...
    void goToNextToken();
    void mergeOtherRes(std::map<const TaggerTokenCheck*, std::list<InfoGramIts>>& pOtherRes,
  ...
  void _processCurrRule(WorkStructForADirection& pWorkStruct,
                        bool pIsTheFirstLoopInThisRule = true) const;
  ...
  static bool _applyPatternIfLinked
