...
public:
    ...
    PartOfSpeechDelBigramImpossibilities(const InflectionsChecker& pFls, unsigned char const** pChar);

    bool process(std::vector<Token>& pTokens) const override;

private:
    struct GramCompatibility {
        GramCompatibility(PartOfSpeech pPartOfSpeech, bool pCheckCompabiblity)
    ...
    void xInit(const std::vector<std::pair<PartOfSpeech, PartOfSpeech> >& pImpSuccessions,
               const std::vector<std::pair<PartOfSpeech, PartOfSpeech> >& pCheckCompatibility);

    bool xDelBigramImpossibilitiesForAToken(TokIt& pTokenItToClean,
