...
TaggerTokenCheck::TaggerTokenCheck(PartOfSpeech pPartOfSpeech,
    ...
    : iGramMatcher(
        [pPartOfSpeech](const InflectedWord& pIGram) -> bool { return pIGram.word.partOfSpeech == pPartOfSpeech; })
    , actionIfLinked(pActionIfLinked)
    , actionIfNotLinked(pActionIfNotLinked)
...
TaggerTokenCheck::TaggerTokenCheck(std::function<bool(const InflectedWord&)> pIGramMatcher,
                                   FinderConstraint pDealOnlyWithTheMostProbableGram,
                                   CompatibilityCheck pCheckCompatibilityWithNeighborhood,
                                   ActionIfLinked pActionIfLinked,
                                   ActionIfNotLinked pActionIfNotLinked,
...
TaggerListOfTokenChecks::TaggerListOfTokenChecks(CanBeEmpty pCanBeEmpty,
...
bool TaggerListOfTokenChecks::canBeAPunctuation() const {
    static const InflectedWord punctuation(PartOfSpeech::PUNCTUATION, std::make_unique<EmptyInflections>());
