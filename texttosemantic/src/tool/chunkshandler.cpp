...
TokIt goAtBeginOfWordGroup(TokIt pTokenIt, const TokIt& pTokenBeginIt) {
    if (pTokenIt->linkedTokens.size() > 1) {
        ...
        auto itHeadToken = *it;
        ++it;
        if (it != pTokenIt->linkedTokens.end() && (*it)->tokenPos < itHeadToken->tokenPos
...
TokIt goAtEndOfWordGroup(TokIt pTokenIt, const TokenRange& pTokRangeContext) {
    if (pTokenIt->linkedTokens.size() > 1) {
        ...
        if (it != pTokenIt->linkedTokens.begin() && res->tokenPos < (*it)->tokenPos
...
TokenRange tokenToTokenRange(TokIt pTokenIt, const TokenRange& pTokRangeContext) {
    ...
    if (pTokenIt->linkedTokens.size() > 1) {
        ...
        if (it != pTokenIt->linkedTokens.begin() && itHeadToken->tokenPos < (*it)->tokenPos
            && (itEnd == pTokRangeContext.getTokList().end() || (*it)->tokenPos <= itEnd->tokenPos))
            itResEnd = *it;
    }
...
void addNominalGroup(std::list<ChunkLink>& pSyntTree, const TokenRange& pTokRange, SemanticLanguageEnum pLangType) {
    ...
    for (auto itTok = pTokRange.getItBegin(); itTok != pTokRange.getItEnd();
        ...
        if (pLangType == SemanticLanguageEnum::ENGLISH && prevGram == PartOfSpeech::PROPER_NOUN
    ...
    if (beginSubNomGroup == pTokRange.getItEnd()) {
        pSyntTree.emplace_back(ChunkLinkType::SIMPLE, Chunk(pTokRange, ChunkType::NOMINAL_CHUNK));
    ...
    } else {
        {
            ...
            pSyntTree.emplace_back(ChunkLinkType::SIMPLE, Chunk(firstToksRange, ChunkType::NOMINAL_CHUNK));
...

        {
            ...
            pSyntTree.emplace_back(ChunkLinkType::SIMPLE, Chunk(secondToksRange, ChunkType::NOMINAL_CHUNK));
...
void separateBeginOfAChunk(std::list<ChunkLink>& pSyntTree,
    ...
    pSyntTree.insert(pChunkIt,
                     ChunkLink(ChunkLinkType::SIMPLE,
                               Chunk(TokenRange(pChunkIt->chunk->tokRange.getTokList(),
    ...
    for (auto itChild = pChunkIt->chunk->children.begin(); itChild != pChunkIt->chunk->children.end();) {
        auto nextIt = itChild;
        ++nextIt;
        if (!checkOrder(*pChunkIt->chunk, *itChild->chunk))
            firstNewChunk->chunk->children.splice(
                firstNewChunk->chunk->children.end(), pChunkIt->chunk->children, itChild);
        itChild = nextIt;
    }
...
void separateEndOfAChunk(ChunkLinkIter& pChunkIt,
                         ...
                         ChunkType pChunkType,
                         SemanticLanguageEnum pLanguage) {
    separateEndOfAChunk(*pChunkIt.getList(), pChunkIt.getIt(), pBeginNewChunk, pChunkType, pLanguage);
}
...
void separateEndOfAChunk(std::list<ChunkLink>& pSyntTree,
    ...
    pSyntTree.insert(++insertPos,
                     ChunkLink(ChunkLinkType::SIMPLE,
                               Chunk(TokenRange(pChunkIt->chunk->tokRange.getTokList(),
                                                pBeginNewChunk,
                                                pChunkIt->chunk->tokRange.getItEnd()),
                                     pChunkType)));
    ...
    for (auto itChild = pChunkIt->chunk->children.begin(); itChild != pChunkIt->chunk->children.end();) {
        ...
        if (checkOrder(*lastNewChunk->chunk, *itChild->chunk))
            lastNewChunk->chunk->children.splice(
                lastNewChunk->chunk->children.end(), pChunkIt->chunk->children, itChild);
        itChild = nextIt;
    }
...
void moveEndOfAChunk(Chunk& pChunk,
                     ...
                     SemanticLanguageEnum pLanguage) {
    pDestinationList.emplace_back(ChunkLink(
        pChunkLinkType,
        Chunk(TokenRange(pChunk.tokRange.getTokList(), pBeginNewChunk, pChunk.tokRange.getItEnd()), pChunkType)));
...
void putEndOfAChunkToHisChildren(Chunk& pChunk, TokIt pBeginNewChunk, SemanticLanguageEnum pLanguage) {
    ...
    pChunk.children.emplace_back(
        ChunkLinkType::SIMPLE,
        Chunk(TokenRange(pChunk.tokRange.getTokList(), pBeginNewChunk, pChunk.tokRange.getItEnd()),
...
void putPrepositionInCunkLkTokenRange(ChunkLink& pChunkLink, SemanticLanguageEnum pLanguage) {
    ...
    if (itFirstWord != pChunkLink.chunk->head) {
        for (TokIt itNewBeginOfChunk = itFirstWord; itNewBeginOfChunk != chunkTokRange.getItEnd();
            ...
            if (itNewBeginOfChunk->getTokenLinkage() != TokenLinkage::PART_OF_WORD_GROUP
                ...
                if (itNewBeginOfChunk->inflWords.front().word.partOfSpeech == PartOfSpeech::PREPOSITION) {
                    continue;
                       ...
                       && itFirstWord->linkedTokens.front()->inflWords.front().word.partOfSpeech
                              == PartOfSpeech::PREPOSITION) {
                continue;
            }

            if (itNewBeginOfChunk != itFirstWord && itNewBeginOfChunk != chunkTokRange.getItEnd()) {
    ...
    switch (pLanguage) {
        ...
        case SemanticLanguageEnum::ENGLISH: {
            ...
            for (TokIt it = getPrevToken(enIt, begIt, enIt, SkipPartOfWord::YES); it != enIt;
                ...
                if (std::find(english_possiblesHead1.begin(), english_possiblesHead1.end(), partOfSpeech)
                        != english_possiblesHead1.end()
                    && !(partOfSpeech == PartOfSpeech::ADJECTIVE
                         && ConceptSet::haveAConceptThatBeginWithAnyOf(inflWord.infos.concepts, {"number_", "rank_"})))
                    return it;
            }

            static const std::vector<PartOfSpeech> english_possiblesHead2 = {PartOfSpeech::PROPER_NOUN,
...
TokIt getEndOfNominalGroup(const TokenRange& pTokRange, TokIt pBeginOfNominalGroup) {
...
bool ifContainToken(TokIt pTokenToSearch, const TokenRange& pTokRange) {
    for (TokIt it = pTokRange.getItBegin(); it != pTokRange.getItEnd(); it = getNextToken(it, pTokRange.getItEnd())) {
...
bool hasAChild(const Chunk& pChunk, const Chunk& pPossibleChild) {
    for (const auto& currChild : pChunk.children)
        if (&*currChild.chunk == &pPossibleChild || hasAChild(*currChild.chunk, pPossibleChild))
            return true;
    return false;
}
...
bool haveAQuestionWordChildAfter(const Chunk& pVerbChunk) {
    for (const auto& currChild : pVerbChunk.children) {
        if (currChild.type == ChunkLinkType::QUESTIONWORD && checkOrder(pVerbChunk, *currChild.chunk))
            return true;
    }
    return false;
}
...
bool haveChildWithAuxSkip(const Chunk& pVerbChunk, const std::set<ChunkLinkType>& pChildChunkLinks) {
    for (auto& currChild : pVerbChunk.children) {
        if (pChildChunkLinks.count(currChild.type) > 0)
            return true;
        if (currChild.type == ChunkLinkType::AUXILIARY && haveChildWithAuxSkip(*currChild.chunk, pChildChunkLinks))
            return true;
    }
    return false;
}
...
const Chunk& getFirstListEltChunk(const Chunk& pChunk) {
    if (chunkTypeIsAList(pChunk.type) && !pChunk.children.empty())
        return getFirstListEltChunk(*pChunk.children.front().chunk);
    return pChunk;
}

namespace {
bool _recHaveASubject(const Chunk& pVerbChunk, const SemanticRequests& pVerbRequests) {
    for (const auto& currChild : pVerbChunk.children) {
        switch (currChild.type) {
            ...
            case ChunkLinkType::QUESTIONWORD: {
                if (pVerbRequests.has(SemanticRequestType::OBJECT)
                    && !currChild.chunk->head->inflWords.front().infos.hasContextualInfo(
                        WordContextualInfos::CANNOTBEASUBJECT))
                    return true;
                break;
            }
            default: break;
...
bool doesChunkHaveDeterminerBeforeHead(const Chunk& pChunk) {
    ...
    for (TokIt it = pChunk.tokRange.getItBegin(); it != itEnd; it = getNextToken(it, itEnd)) {
...
Chunk& whereToLinkSubject(Chunk& pVerbChunk) {
    for (std::list<ChunkLink>::iterator it = pVerbChunk.children.begin(); it != pVerbChunk.children.end(); ++it) {
...
bool haveOtherEltsBetterToLinkInAList(ChunkLinkIter& pBeforeLastElt, ChunkLinkIter& pLastElt) {
    ...
    while (!beforeElt.atEnd()) {
        ...
        if (chunkTypeIsAList(beforeEltChunkType)
            || (beforeEltChunkType == ChunkType::SEPARATOR_CHUNK
                && beforeElt->chunk->getHeadPartOfSpeech() != PartOfSpeech::SUBORDINATING_CONJONCTION))
            return false;
...
ChunkLink* getChunkLinkWhereWeCanLinkASubodinate(Chunk& pVerbChunk) {
    for (auto it = pVerbChunk.children.rbegin(); it != pVerbChunk.children.rend(); ++it)
        if (it->type == ChunkLinkType::DIRECTOBJECT || it->type == ChunkLinkType::SUBORDINATE
            || it->type == ChunkLinkType::SUBORDINATE_CLAUSE || it->type == ChunkLinkType::WAY)
...
Chunk* getDOComplOrSubjClauseChunk(const Chunk& pVerbChunk) {
    for (const auto& currChild : pVerbChunk.children) {
        if (currChild.type == ChunkLinkType::DIRECTOBJECT || currChild.type == ChunkLinkType::SUBORDINATE_CLAUSE) {
            return &*currChild.chunk;
...
Chunk* getReflexiveChunk(const Chunk& pVerbChunk) {
    for (std::list<ChunkLink>::const_iterator it = pVerbChunk.children.begin(); it != pVerbChunk.children.end(); ++it) {
...
bool canLinkVerbToASubject(const Chunk& pVerbChunk,
    ...
    std::list<InflectedWord>::const_iterator infGramVerb = pVerbChunk.head->inflWords.begin();
    ...
    if (pSubjectChunk.type == ChunkType::AND_CHUNK || pSubjectChunk.type == ChunkType::THEN_CHUNK
...
bool chunkIsVerbalAndNotAYesOrNo(const Chunk& pChunk) {
    return chunkTypeIsVerbal(pChunk.type) && !pChunk.requests.has(SemanticRequestType::YESORNO);
}
...
bool chunkDontHaveASubjectAndCannotHaveNoSubject(const Chunk& pChunk) {
    return !haveASubject(pChunk) && !InflectionsChecker::verbCanHaveNoSubject(pChunk.head->inflWords.front());
}
...
void recInList(std::function<void(Chunk&)> pPredicate, Chunk& pChunk) {
    if (chunkTypeIsAList(pChunk.type)) {
        for (const auto& currChild : pChunk.children)
            if (currChild.type == ChunkLinkType::SIMPLE)
...
bool hasAChunkTypeOrAListOfChunkTypes(const Chunk& pChunk, ChunkType pChunkType) {
    return recInListConst([pChunkType](const Chunk& pChunk) { return pChunk.type == pChunkType; }, pChunk);
}

bool isACommandOrAListOfCommands(const Chunk& pChunk) {
    return recInListConst([](const Chunk& pChunk) { return pChunk.requests.has(SemanticRequestType::ACTION); }, pChunk);
}

bool doesChunkIsOrCanBeAtImperative(const Chunk& pChunk) {
    return pChunk.requests.has(SemanticRequestType::ACTION)
        || (pChunk.requests.empty() && !haveASubject(pChunk)
            && InflectionsChecker::verbCanBeAtImperative(pChunk.head->inflWords.front()));
}

bool chunkCanHaveSubordinates(const Chunk& pChunk) {
    return !pChunk.isPassive && chunkTypeIsVerbal(pChunk.type);
}
...
bool chunkCanBeAnObject(const Chunk& pChunk) {
    switch (pChunk.type) {
        case ChunkType::PREPOSITIONAL_CHUNK: {
            ...
            return headPartOfSpeech != PartOfSpeech::PREPOSITION && headPartOfSpeech != PartOfSpeech::PRONOUN_SUBJECT;
        }
        ...
        case ChunkType::VERB_CHUNK: return false;
        default: return true;
    }
}
...
bool doesTokRangeBeginsWithARequestSubordinate(const TokenRange& pTokRange,
                                               const StaticLinguisticDictionary& pBinDico) {
    return !pTokRange.isEmpty()
        && pBinDico.wordToSubordinateRequest(pTokRange.getItBegin()->inflWords.front().word)
...
bool isChunkAtPassiveForm(const Chunk& pVerbChunk,
    ...
    if (pLingDico.getLanguage() == SemanticLanguageEnum::FRENCH) {
        ...
        if (!vbIGram.infos.linkedMeanings) {
            ...
            } else if (verbTense == SemanticVerbTense::PUNCTUALPAST && pInflChecker.verbIsAtPastParticiple(vbIGram)) {
                return true;
            }
        }
        return false;
    ...
    if (auxChunk != nullptr) {
        const SemanticWord& auxWord = auxChunk->head->inflWords.front().word;
        return auxWord == pLingDico.getBeAux().word && !pInflChecker.verbIsAtPresentParticiple(vbIGram);
    }
    return false;
}
...
void setChunkAtInterrogativeForm(Chunk& pChunk) {
    ...
    if (doChkLinkPtr != nullptr && doChkLinkPtr->chunk->type == ChunkType::OR_CHUNK)
...
bool isTokenAtEndOfASentence(TokIt pTokenIt, const TokenRange& pTokRangeContext) {
    ...
    return nextItToken == itEnd || nextItToken->inflWords.front().word.partOfSpeech == PartOfSpeech::PUNCTUATION;
}
...
void linkPartitives(Chunk& rootChunk,
    ...
    if ((rootChunk.type == ChunkType::NOMINAL_CHUNK || rootChunk.type == ChunkType::PREPOSITIONAL_CHUNK)
        ...
        pItToPotentialPartitiveChunk->type =
            pEntityRecognizer.findNatureOfAChunkLink(*pItToPotentialPartitiveChunk, nullptr);
        rootChunk.children.splice(
            rootChunk.children.end(), *pItToPotentialPartitiveChunk.getList(), pItToPotentialPartitiveChunk.getIt());
    }
}
...
bool filterIncompatibleInflectionsInTokenList(std::vector<Token>& pTokens, const InflectionsChecker& pFls) {
    ...
    for (auto it = getNextToken(prevIt, pTokens.end()); it != pTokens.end(); it = getNextToken(it, pTokens.end())) {
...
void moveChildren(std::list<ChunkLink>& pRootChildren, ChunkLinkIter& pIteratorToMove, ChunkLinkType pNewChkLkType) {
    ...
    if (itToMove->chunk->children.size() == 1 && itToMove->chunk->children.front().type == ChunkLinkType::SIMPLE
...
bool getNumberBeforeHead(SemanticFloat& pNumber, const Chunk& pChunk) {
    for (TokIt itToken = getPrevToken(pChunk.head, pChunk.tokRange.getItBegin(), pChunk.head); itToken != pChunk.head;
