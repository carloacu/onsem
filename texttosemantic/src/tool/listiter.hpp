...
struct ChunkLinkIter
{
  ...
  bool operator==
  (const ChunkLinkIter& pOther) const
  {
    return workingZone == pOther.workingZone && it == pOther.it;
  }
  ...
  bool atEnd() const
  {
    return it == workingZone.end();
  }

  std::list<ChunkLink>* getList()
  {
    return workingZone.syntTreePtr();
  }

  std::list<ChunkLink>::iterator getIt() const
  {
    return it;
  }
  ...
  void eraseIt()
  {
    it = workingZone.syntTree().erase(it);
  }
