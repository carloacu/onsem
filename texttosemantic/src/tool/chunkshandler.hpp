...
public:
  ...
  bool operator==
  (const ChunkLinkWorkingZone& pOther) const
  {
    return _syntTreePtr == pOther._syntTreePtr &&
        _end == pOther._end &&
        _beginAtBeginOfList == pOther._beginAtBeginOfList &&
        _beforeBegin == pOther._beforeBegin;
  }
  ...
  std::list<ChunkLink>::iterator end() const
  {
    return _end;
  }
...
ChunkLink* getDOChunkLink
(Chunk& pVerbChunk);

const ChunkLink* getDOChunkLink
(const Chunk& pVerbChunk);
...
bool recInListConst(std::function<bool(const Chunk& pChunk)> pPredicate,
                    const Chunk& pChunk);
void recInList(std::function<void(Chunk& pChunk)> pPredicate,
               Chunk& pChunk);
