...
namespace {
bool _delAllExcept(std::list<InflectedWord>& pGrams,
...
bool tokenIsMoreProbablyAType(const linguistics::Token& pToken, PartOfSpeech pPartOfSpeech) {
    return pToken.inflWords.begin()->word.partOfSpeech == pPartOfSpeech;
}

bool tokenIsMoreProbablyAType(const linguistics::Token& pToken, const std::vector<PartOfSpeech>& pPartOfSpeechs) {
    const auto& word = pToken.inflWords.front().word;
    return hasAPartOfSpeech(pPartOfSpeechs, word.partOfSpeech);
}

bool hasAPartOfSpeech(const std::vector<PartOfSpeech>& pPartOfSpeechs, PartOfSpeech pPartOfSpeech) {
...
bool hasAPartOfSpeech(const std::list<InflectedWord>& pInflWords, PartOfSpeech pPartOfSpeech) {
...
std::list<InflectedWord>::iterator getInflWordWithASpecificPartOfSpeech(std::list<InflectedWord>& pIGrams,
...
std::list<InflectedWord>::iterator getInflWordWithAnyPartOfSeechOf(linguistics::Token& pToken,
...
void delAPartOfSpeech(std::list<InflectedWord>& pIGrams, std::list<InflectedWord>::iterator& pItToDel) {
...
bool delAPartOfSpeech(std::list<InflectedWord>& pIGrams, PartOfSpeech pPartOfSpeechToDel) {
    if (pIGrams.size() <= 1) {
        return false;
    }
    for (std::list<InflectedWord>::iterator it = pIGrams.begin(); it != pIGrams.end(); ++it) {
...
bool delPartOfSpeechs(std::list<InflectedWord>& pIGrams, const std::vector<PartOfSpeech>& pPartOfSpeechToDel) {
    bool res = false;
...
TokIt getNextWord(TokIt it, const TokIt& endIt) {
    if (it == endIt) {
        return endIt;
    }
...
TokIt getNextToken(TokIt it, const TokIt& endIt, PartOfSpeech pUnwantedGramType) {
    if (it == endIt) {
        return endIt;
    }
    for (TokIt res = ++it; res != endIt; ++res) {
        PartOfSpeech currPartOfSpeech = res->inflWords.begin()->word.partOfSpeech;
        if (currPartOfSpeech >= PartOfSpeech::LINKBETWEENWORDS && currPartOfSpeech != pUnwantedGramType) {
            return res;
        }
    }
    return endIt;
...
TokIt getTheNextestToken(TokIt it, const TokIt& endIt, SkipPartOfWord pSkipPartOfWord) {
    if (it == endIt) {
        return endIt;
    }
    for (TokIt res = it; res != endIt; ++res) {
...
bool delAllBefore(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt) {
...
bool delAllAfter(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt) {
    bool hasRemovedAGram = false;
...
bool delAllExept(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt, PartOfSpeech pPartOfSpeech) {
...
bool delAllExept(std::list<InflectedWord>& pGrams, PartOfSpeech pPartOfSpeech) {
    return _delAllExcept(pGrams, [&](std::list<InflectedWord>::iterator& pItCurrInfl) {
        return pItCurrInfl->word.partOfSpeech != pPartOfSpeech;
    });
}

void putOnTop(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt) {
    if (pGrams.begin() != pIt)
        pGrams.splice(pGrams.begin(), pGrams, pIt);
}

bool putOnTop(std::list<InflectedWord>& pGrams, PartOfSpeech pPartOfSpeech) {
...
void putOnBottom(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt) {
    if (pGrams.empty())
        return;
...
bool isAtBeginOfASentence(std::vector<Token>& pTokens, TokIt pItTok) {
...
void fillRelativeCharEncodedFromInflWord(LinguisticSubordinateId& pLinguisticSubordinateId,
    ...
    switch (pLinguisticSubordinateId.chunkLinkType) {
        case ChunkLinkType::LOCATION: {
            ...
            ConceptSet::extractConceptsThatBeginWith(
                relLocationConcepts, pInflWord.infos.concepts, locRelBeginOfCptStr);
            if (!relLocationConcepts.empty()) {
                ...
                if (conceptName.size() > locRelBeginOfCptStr.size())
                    pLinguisticSubordinateId.relativeSubodinate =
                        semanticRelativeLocationType_toChar(semanticRelativeLocationType_fromStr(conceptName.substr(
                            locRelBeginOfCptStr.size(), conceptName.size() - locRelBeginOfCptStr.size())));
            }
            break;
        }
        case ChunkLinkType::TIME: {
            ...
            ConceptSet::extractConceptsThatBeginWith(relTimeConcepts, pInflWord.infos.concepts, timeRelBeginOfCptStr);
            ...
            for (const auto& currRelTimeCpt : relTimeConcepts) {
                ...
                if (semanticRelativeTimeType_fromStr_ifFound(
                        relTimeType,
        ...
        case ChunkLinkType::DURATION: {
            ...
            ConceptSet::extractConceptsThatBeginWith(
                relDurationConcepts, pInflWord.infos.concepts, durationRelBeginOfCptStr);
            ...
            for (const auto& currRelDurationCpt : relDurationConcepts) {
                ...
                if (semanticRelativeDurationType_fromStr_ifFound(
                        relDurationType,
        ...
        for (const auto& currCpt : inflWord.infos.concepts) {
            ...
            if (ConceptSet::doesConceptBeginWith(currCptName, pBeginOfNumberCpt)
                ...
                std::string numberStr =
                    currCptName.substr(beginOfNumberCpt_size, currCptName.size() - beginOfNumberCpt_size);
...
mystd::optional<SemanticDate> extractDate(const TOKITTEMP& pTokenIt, const TOKENRANGE& pTokRange) {
    ...
    if (!monthConcepts.empty()) {
        ...
        if (iGram.word.language == SemanticLanguageEnum::ENGLISH) {
            ...
            if (dayToken != pTokenIt
                ...
                && dayNumber.isPositive() && dayNumber.isAnInteger() && dayNumber.value >= 1 && dayNumber.value <= 31) {
        ...
        if (yearToken != pTokRange.getItEnd()
            && getNumberHoldByTheInflWord(yearNumber, yearToken, itTokenEndChunk, "number_") && yearNumber.isPositive()
            && yearNumber.isAnInteger() && hasNotMoreThanANumberOfDigits(yearNumber.value, 4))
            res.year.emplace(yearNumber.value);
        resOpt.emplace(res);
        return resOpt;
    }
