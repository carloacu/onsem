...
bool _areInflectionsCompatibles(const INFL1& pInfls1, const INFL2& pInfls2) {
    bool res = true;
...
template<typename INFL1, typename INFL2>
bool _isInflectionsFilterWithOneConst(INFL1& pInfls1, const INFL2& pInfls2) {
...
template<typename INFL1, typename INFL2>
bool _isInflectionsFilterWithSpecTypes(INFL1& pInfls1, INFL2& pInfls2) {
...
bool _inflectionsFilter(Inflections& pInflections1, Inflections& pInflections2) {
    if (pInflections1.type == InflectionType::NOMINAL && pInflections2.type == InflectionType::NOMINAL) {
        return _isInflectionsFilterWithSpecTypes(pInflections1.getNominalI(), pInflections2.getNominalI());
    }
    if (pInflections1.type == InflectionType::NOMINAL && pInflections2.type == InflectionType::ADJECTIVAL) {
        return _isInflectionsFilterWithSpecTypes(pInflections1.getNominalI(), pInflections2.getAdjectivalI());
    }
    if (pInflections1.type == InflectionType::ADJECTIVAL && pInflections2.type == InflectionType::NOMINAL) {
        return _isInflectionsFilterWithSpecTypes(pInflections1.getAdjectivalI(), pInflections2.getNominalI());
    }
    if (pInflections1.type == InflectionType::ADJECTIVAL && pInflections2.type == InflectionType::ADJECTIVAL) {
        return _isInflectionsFilterWithSpecTypes(pInflections1.getAdjectivalI(), pInflections2.getAdjectivalI());
    }
    return false;
}

bool InflectionsChecker::areInflectionsCompatibles(const Inflections& pInfls1, const Inflections& pInfls2) {
    switch (pInfls1.type) {
...
bool InflectionsChecker::isAdjCompatibleWithNumberType(const Inflections& pAdjInfls, SemanticNumberType pNumberType) {
...
bool InflectionsChecker::filterIncompatibleInflections(const Token* pPrevPrevToken,
    ...
    switch (inflWord1.word.partOfSpeech) {
        ...
        case PartOfSpeech::PRONOUN_COMPLEMENT: {
            ...
            if (pPrevPrevToken != nullptr && pPrevPrevToken->inflWords.front().word.lemma == "ne")
                return false;
            ...
            if (infls2.type == InflectionType::VERBAL) {
                ...
                for (auto itInfl = verbInlfs.inflections.begin(); itInfl != verbInlfs.inflections.end();) {
        ...
        case PartOfSpeech::VERB: {
            ...
            return false;    // false because it's not sure that we decrease the number of inflections, if no it can
