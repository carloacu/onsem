...
void detNounPriorities(std::vector<Token>& pTokens, const InflectionsChecker& pInflsCheker) {
...
void verbPriorities(std::vector<Token>& pTokens, const InflectionsChecker& pInflsCheker) {
    ...
    for (TokIt itTok = pTokens.begin(); itTok != pTokens.end(); itTok = getNextToken(itTok, pTokens.end())) {
        ...
        if (firstVerbInflWord.word.partOfSpeech == PartOfSpeech::VERB && inflWords.size() > 1) {
            ...
            } else if (prevInflWordPtr != nullptr && prevInflWordPtr->word.partOfSpeech == PartOfSpeech::VERB
...
void NounPrioritiesFr(std::vector<Token>& pTokens, const InflectionsChecker& pInflsCheker, bool pIsRootLevel) {
    ...
    for (TokIt itTok = getPrevToken(pTokens.end(), pTokens.begin(), pTokens.end()); itTok != pTokens.end();
        ...
        if (partOfSpeech_isNominal(itFirstInflWord->word.partOfSpeech)) {
            ...
            if (tokenAfterPtr != nullptr) {
                ...
                if (currIGram.word.partOfSpeech == PartOfSpeech::PROPER_NOUN) {
                    if (tokenAfterAfterPtr != nullptr
                        ...
                        auto itProperNoun =
                            getInflWordWithASpecificPartOfSpeech(tokenAfter.inflWords, PartOfSpeech::PROPER_NOUN);
                    ...
                    if (itTok != pTokens.begin()) {
                        ...
                        if (itTokenBefore != pTokens.end()
                            ...
                            auto itNounOfTokenBefore =
                                getInflWordWithASpecificPartOfSpeech(itTokenBefore->inflWords, PartOfSpeech::NOUN);
                ...
                if (partOfSpeechAfter == PartOfSpeech::PRONOUN) {
                    ...
                    if (itDet != inflWords.end()) {
                        ...
                        for (auto itInfl = tokenAfter.inflWords.begin(); itInfl != tokenAfter.inflWords.end();
                             ++itInfl) {
            ...
            if (tryToMoveProperNounHasHigherPriority && currIGram.word.partOfSpeech != PartOfSpeech::PROPER_NOUN) {
                auto itInflWord = itFirstInflWord;
                ++itInflWord;
                while (itInflWord != inflWords.end()) {
        ...
        // If the current word is an adjective, sometimes we will prefer to modify the main part of speech of the
        // current word
        if (currIGram.word.partOfSpeech == PartOfSpeech::ADJECTIVE && pIsRootLevel) {
            // If the next word is a noun for exemple
            if (tokenAfterPtr != nullptr && partOfSpeech_isNominal(tokenAfterPtr->getPartOfSpeech())) {
                updateTokenAfterPtrs(itTok);
                continue;
            }
    ...
    for (TokIt itTok = getPrevToken(pTokens.end(), pTokens.begin(), pTokens.end()); itTok != pTokens.end();
        ...
        if (partOfSpeech_isNominal(currIGram.word.partOfSpeech)) {
            if (prevNounTokenPtr != nullptr) {
                ...
                if (itDet != inflWords.end()) {
                    putOnTop(inflWords, itDet);
                    for (auto itInfl = nounAfterToken.inflWords.begin(); itInfl != nounAfterToken.inflWords.end();
                         ++itInfl) {
                        if (pInflsCheker.areCompatibles(*itDet, *itInfl)) {
...
void pronounPriorities(std::vector<Token>& pTokens, const InflectionsChecker& pInflsCheker) {
    ...
    for (TokIt itTok = pTokens.begin(); itTok != pTokens.end(); itTok = getNextToken(itTok, pTokens.end())) {
        ...
        if (currInflWord.word.partOfSpeech == PartOfSpeech::PRONOUN_SUBJECT) {
                ...
                if (partOfSpeech_isNominal(itPrev->getPartOfSpeech())) {
                    ...
                    while (itPrev != pTokens.end()) {
                        ...
                        if (prevPartOfSpeech == PartOfSpeech::PUNCTUATION)
                            break;
                        if (prevPartOfSpeech != PartOfSpeech::VERB && prevPartOfSpeech != PartOfSpeech::ADVERB) {
        ...
        } else if (currInflWord.word.partOfSpeech == PartOfSpeech::PRONOUN_COMPLEMENT && inflWords.size() > 1) {
            if (!hasBefore(pTokens, itTok, verbAuxpPartOfSpeechsToFind, pInflsCheker, PartOfSpeech::ADVERB)
                ...
                putOnBottom(inflWords, inflWords.begin());
        } else if (currInflWord.word.partOfSpeech == PartOfSpeech::PRONOUN && inflWords.size() > 1) {
            auto itPrev = getPrevToken(itTok, pTokens.begin(), pTokens.end());
            if (itPrev == pTokens.end())
                continue;
...
void adjPriorities(std::vector<Token>& pTokens) {
    for (TokIt itTok = pTokens.begin(); itTok != pTokens.end(); itTok = getNextToken(itTok, pTokens.end())) {
        ...
        if (currInflWord.word.partOfSpeech == PartOfSpeech::ADJECTIVE && inflWords.size() > 1) {
            auto itNext = getNextToken(itTok, pTokens.end());
            if (itNext != pTokens.end() && itNext->getPartOfSpeech() == PartOfSpeech::PROPER_NOUN)
                putOnTop(inflWords, PartOfSpeech::PROPER_NOUN);
        }
    }
}
...
void advPriorities(std::vector<Token>& pTokens) {
    for (TokIt itTok = pTokens.begin(); itTok != pTokens.end();) {
        ...
        if (currInflWord.word.partOfSpeech == PartOfSpeech::ADVERB
            ...
            if (itNext != pTokens.end() && itNext->getPartOfSpeech() == PartOfSpeech::ADVERB)
        ...
        if (currInflWord.word.partOfSpeech == PartOfSpeech::PARTITIVE) {
            ...
            if (itNext != pTokens.end()) {
                ...
                if (itNextFirstInfl->word.partOfSpeech == PartOfSpeech::VERB
                    && pInflsCheker.verbIsConjugated(*itNextFirstInfl) && itNext->inflWords.size() > 1) {
                    putOnBottom(itNext->inflWords, itNextFirstInfl);
                }
            }

            auto itPrev = getPrevToken(itTok, pTokens.begin(), pTokens.end());
            if (itPrev != pTokens.end() && itPrev->inflWords.size() > 1) {
                auto itPrevFirstInfl = itPrev->inflWords.begin();
                if (itPrevFirstInfl->word.partOfSpeech == PartOfSpeech::VERB) {
                    if (!pInflsCheker.nounCanBePlural(currInflWord) && itNext != pTokens.end()) {
            ...
            for (auto itInflWord = ++inflWords.begin(); itInflWord != inflWords.end(); ++itInflWord) {
                ...
                if (currInflWord.word.partOfSpeech == PartOfSpeech::ADVERB
                    ...
                    if (nextNextTok != pTokens.end()
                        && ConceptSet::haveAConceptThatBeginWith(nextNextTok->inflWords.front().infos.concepts,
                                                                 "number_")) {
                        putOnTop(inflWords, itInflWord);
                        break;
                    }
