...
auto _langToContextFilter = [](const InflectionsChecker& pInfls, const SpecificLinguisticDatabase& pSpecLingDb) {
    switch (pSpecLingDb.language) {
        ...
        default: return std::list<std::unique_ptr<linguistics::PartOfSpeechContextFilter>>();
    }
};
...
void _addWord(const boost::property_tree::ptree& pTree,
    ...
    for (const auto& currSubWordTree : pTree) {
        ...
        if (beaconName == "inflectedWord") {
            ...
            pSpecLingDb.addInflectedWord(
                inflectWordStr, word, Inflections::create(inflType, inflectWordInflections), frequency);
        } else if (beaconName == "contextInfo") {
            linguistics::WordAssociatedInfos wordAssocInfos;
            wordAssocInfos.contextualInfos.insert(
                wordContextualInfos_fromStr(childTree.get<std::string>("<xmlattr>.val")));
...
void _addNoVerbInfos(const boost::property_tree::ptree& pTree, SpecificLinguisticDatabase& pSpecLingDb) {
    for (const auto& currSubWordTree : pTree) {
        if (currSubWordTree.first == "child") {
            const boost::property_tree::ptree& childTree = currSubWordTree.second;
            ChildSpecification childSpec;
...
void SpecificLinguisticDatabase::addInflectedWord(const std::string& pInflectedFrom,
    ...
    lingDico.addInflectedWord(pInflectedFrom, itWordToInfos->first, itWordToInfos->second.infos, inflections);
...
void SpecificLinguisticDatabase::addInfosToAWord(const SemanticWord& pWord, const WordAssociatedInfos& pWordInfos) {
    auto itWordToInfos = _saveWordInfos(pWord);
...
std::map<SemanticWord, SpecificLinguisticDatabase::InflectionsInfosForAWord>::iterator
    SpecificLinguisticDatabase::_saveWordInfos(const SemanticWord& pWord) {
...
LinguisticDatabase::LinguisticDatabase(LinguisticDatabaseStreams& pIStreams)
    ...
    , langToSpec(semanticLanguageEnum_size,
                 [&](std::size_t i) {
                     auto lang = semanticLanguageEnum_fromChar(static_cast<char>(i));
                     return std::make_unique<SpecificLinguisticDatabase>(
                         pIStreams.languageToStreams[lang], *this, lang);
                 })
    , transDict(pIStreams)
