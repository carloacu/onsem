...
void _add(SemanticFloat& pRes, const SemanticFloat& pNb1, const SemanticFloat& pNb2, Sign pNb2Sign) {
    ...
    if (newNbOfSignificantDigit > pNb1.nbOfSignificantDigit)
        nb1 *= tenPow(newNbOfSignificantDigit - pNb1.nbOfSignificantDigit);
    ...
    if (newNbOfSignificantDigit > pNb2.nbOfSignificantDigit)
        nb2 *= tenPow(newNbOfSignificantDigit - pNb2.nbOfSignificantDigit);

    int signedValueWithoutDecimal =
        pNb1.signedValueWithoutDecimal() + (pNb2Sign == Sign::POSITIVE ? pNb2.value : -pNb2.value);
    ...
    if (nbSum >= 0) {
        ...
        pRes.valueAfterTheDecimalPoint = tenPow(newNbOfSignificantDigit) + nbSum;
        --pRes.value;
    }
...
SemanticFloat::SemanticFloat(int pValue, std::size_t pValueAfterTheDecimalPoint, unsigned char pNbOfSignificantDigit)
...
bool SemanticFloat::operator==(const SemanticFloat& pOther) const {
    return value == pOther.value && valueAfterTheDecimalPoint == pOther.valueAfterTheDecimalPoint
        && nbOfSignificantDigit == pOther.nbOfSignificantDigit;
}

bool SemanticFloat::operator==(int pNb) const {
    return sign == (pNb >= 0 ? Sign::POSITIVE : Sign::NEGATIVE) && value == static_cast<std::size_t>(std::abs(pNb))
        && valueAfterTheDecimalPoint == 0 && nbOfSignificantDigit == 0;
}
...
bool SemanticFloat::operator<(const SemanticFloat& pOther) const {
    ...
    auto nb1 = valueAfterTheDecimalPoint * tenPow(nbOfSignificantDigit - minNbOfDigits);
    auto nb2 = pOther.valueAfterTheDecimalPoint * tenPow(pOther.nbOfSignificantDigit - minNbOfDigits);
...
bool SemanticFloat::isAnInteger() const {
    return valueAfterTheDecimalPoint == 0 && nbOfSignificantDigit == 0;
}
...
double SemanticFloat::toDouble() const {
    ...
    res += valueAfterTheDecimalPoint / tenPow(nbOfSignificantDigit);
    return res;
}
...
bool SemanticFloat::fromStrWithSeparator(const std::string& pStr, char pSeparator) {
    ...
    if (sepPos != std::string::npos) {
        try {
            ...
            valueAfterTheDecimalPoint =
                mystd::lexical_cast_unigned<int>(pStr.substr(sepPos + 1, newNbOfSignificantDigit));
            sign = newSign;
            value = newValue;
...
bool SemanticQuantity::operator==(const SemanticQuantity& pOther) const {
    return type == pOther.type && nb == pOther.nb && paramSpec == pOther.paramSpec
...
void SemanticQuantity::setNumberToFill(int pParamId, std::string pAttributeName) {
...
bool SemanticQuantity::getNumberToFill(int& pParamId, std::string& pAttributeName) const {
...
bool SemanticQuantity::isPlural() const {
    return (type == SemanticQuantityType::MOREOREQUALTHANNUMBER && nb >= 2)
        || (type == SemanticQuantityType::NUMBER && nb >= 2) || type == SemanticQuantityType::MAXNUMBER;
}
...
bool SemanticQuantity::isEqualToInit() const {
    return type == SemanticQuantityType::UNKNOWN && nb == 0 && paramSpec.empty()
        && subjectiveValue == SemanticSubjectiveQuantity::UNKNOWN;
}
