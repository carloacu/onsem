...
std::unique_ptr<SemanticGrounding> _loadMotherClass(const unsigned char*& pPtr, const StaticConceptSet& pCptSet) {
    SemanticGroundingType grdType = semanticGroundingsType_fromChar(binaryloader::loadChar_0To4(pPtr));
...
void _loadCoreference(mystd::optional<Coreference>& pCoreference, const unsigned char* pUChar) {
...
void _loadWord(SemanticWord& pWord, const unsigned char*& pPtr, const linguistics::LinguisticDatabase& pLingDb) {
...
void _loadGenders(std::set<SemanticGenderType>& pGenders, const unsigned char*& pPtr) {
...
void _loadNameInfos(NameInfos& pNameInfos, const unsigned char*& pPtr) {
...
int _loadCharOrInt(const unsigned char*& pPtr, bool pCharOrInt) {
    if (pCharOrInt)
        return static_cast<int>(*((const char*)(pPtr++)));
    return _loadInt(pPtr);
}

SemanticFloat _loadSemanticFloat(SemanticFloat& pRes, const unsigned char*& pPtr, bool pNbWrittenInACharOrInAInt) {
    ...
    pRes.sign =
        pNbWrittenInACharOrInAInt ? Sign::POSITIVE : (binaryloader::loadChar_0(pPtr) ? Sign::POSITIVE : Sign::NEGATIVE);
...
void _loadAngle(SemanticAngle& pAngle, const unsigned char*& pPtr) {
...
void _loadLength(SemanticLength& pLength, const unsigned char*& pPtr) {
...
void _loadDuration(SemanticDuration& pDuration, const unsigned char*& pPtr) {
...
std::unique_ptr<SemanticGrounding> _loadGrd(const unsigned char*& pPtr,
    ...
    switch (res->type) {
        case SemanticGroundingType::GENERIC: {
            ...
            if (binaryloader::loadChar_6(pPtr))    // if quantity is written
            {
                ...
                genGrd.quantity.subjectiveValue =
                    semanticSubjectiveQuantity_fromChar(binaryloader::loadChar_4To7(pPtr));
                ++pPtr;
        ...
        case SemanticGroundingType::AGENT: {
            ...
            auto agentGrd = std::make_unique<SemanticAgentGrounding>(userId, binaryloader::loadString(pPtr));
        ...
        case SemanticGroundingType::CONCEPTUAL: return res;
...
void _loadSemExpOpt(mystd::unique_propagate_const<UniqueSemanticExpression>& pSemExpOpt,
                    const unsigned char*& pPtr,
                    const linguistics::LinguisticDatabase& pLingDb) {
...
UniqueSemanticExpression loadSemExp(const unsigned char*& pPtr, const linguistics::LinguisticDatabase& pLingDb) {
    ...
    switch (semExpType) {
        ...
        case SemanticExpressionType::COMPARISON: {
            ...
            auto res = std::make_unique<ComparisonExpression>(compOperator, loadSemExp(pPtr, pLingDb));
            res->tense = tense;
            res->request = request;
        ...
        case SemanticExpressionType::SETOFFORMS: {
            ...
            for (unsigned char i = 0; i < nbOfPrioToForms; ++i) {
                ...
                for (unsigned char j = 0; j < nbOfQuestionExpForm; ++j) {
                    ...
                    qefList.emplace_back(std::make_unique<QuestExpressionFrom>(std::move(semExp), *(pPtr++)));
                }
