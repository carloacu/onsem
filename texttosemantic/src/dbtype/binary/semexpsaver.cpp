...
void _writeWord(binarymasks::Ptr& pPtr,
...
void _writeCoreference(signed char* pPchar,
                       const mystd::optional<Coreference>& pCorefOpt)
{
...
void _writeGenders(binarymasks::Ptr& pPtr,
                   const std::set<SemanticGenderType>& pGenders)
{
...
void _writeNameInfos(binarymasks::Ptr& pPtr,
...
void _writeCharOrInt(binarymasks::Ptr& pPtr,
                     int pVal,
                     bool pCharOrInt)
{
  assert(pCharOrInt == binarysaver::intCanBeStoredInAChar(pVal));
  if (pCharOrInt)
                     ...
                     const linguistics::LinguisticDatabase& pLingDb)
{
  ...
  case SemanticGroundingType::STATEMENT:
  {
    ...
    const bool writeVerbGoalAndCoreferenceAndIsPassive = statGrd.verbGoal != VerbGoalEnum::NOTIFICATION ||
        statGrd.coreference.has_value() || statGrd.isPassive;
...
void SemExpPtrOffsets::addSemExp(const SemanticExpression& pSemExp,
                                 unsigned char* pPtr)
{
...
uint32_t SemExpPtrOffsets::grdExpToOffset(const GroundedExpression& pGrdExp,
...
uint32_t SemExpPtrOffsets::semExpToOffset(const SemanticExpression& pSemExp,
                                          unsigned char* pPtr) const
{
