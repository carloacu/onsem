...
std::map<SemanticLanguageEnum, std::map<SemanticLanguageEnum, std::unique_ptr<StaticTranslationDictionary>>>
    TranslationDictionary::_pathToStatDbs{};

std::map<SemanticLanguageEnum, std::map<SemanticLanguageEnum, const StaticTranslationDictionary*>>
    TranslationDictionary::_getStatDbInstance(LinguisticDatabaseStreams& pIStreams) {
        ...
        for (const auto& currInLangEnum : semanticLanguageEnum_allValues) {
            if (currInLangEnum != SemanticLanguageEnum::UNKNOWN) {
                for (const auto& currOutLangEnum : semanticLanguageEnum_allValues) {
                    if (currOutLangEnum != SemanticLanguageEnum::UNKNOWN && currInLangEnum != currOutLangEnum)
                        _pathToStatDbs[currInLangEnum].emplace(
                            currOutLangEnum,
                            std::make_unique<StaticTranslationDictionary>(pIStreams, currInLangEnum, currOutLangEnum));
                }
            }
        }
    }
...
void TranslationDictionary::addTranslation(const SemanticWord& pFromWord, const SemanticWord& pToWord) {
...
void TranslationDictionary::getTranslation(int32_t& pTranslatedMeaningId,
    ...
    StaticLinguisticMeaning lingMeaning = pStatLingDico.getLingMeaning(pWord.lemma, pWord.partOfSpeech, true);
    if (pWord.language == pOutLanguage) {
        pTranslatedMeaningId = lingMeaning.meaningId;
        return;
    }
    if (lingMeaning.isEmpty() || pWord.language == SemanticLanguageEnum::UNKNOWN
...
bool TranslationDictionary::getTranslation(LinguisticMeaning& pTranslatedMeaningId,
    ...
    StaticLinguisticMeaning lingMeaning = pLingDico.statDb.getLingMeaning(pWord.lemma, pWord.partOfSpeech, true);
    if (lingMeaning.isEmpty() || pWord.language == SemanticLanguageEnum::UNKNOWN
        || pOutLanguage == SemanticLanguageEnum::UNKNOWN) {
        return false;
    }
