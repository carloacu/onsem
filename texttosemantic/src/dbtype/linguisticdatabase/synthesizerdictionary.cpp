                                             ...
                                             char pFrequency)
{
  ...
  if (nominalInflectionsPtr != nullptr &&
      !nominalInflectionsPtr->inflections.empty())
  {
    for (const auto& currNominal : nominalInflectionsPtr->inflections)
      _addInflectionsIfMoreFrequent(inflectionElt, currNominal,
                                    InflFormAndFrequency(pInflectedFrom, pFrequency));
  }
  else
  {
    _addInflectionsIfMoreFrequent(inflectionElt, NominalInflection(),
                                  InflFormAndFrequency(pInflectedFrom, pFrequency));
  }
}
...
std::string SynthesizerDictionary::getLemma(const LinguisticMeaning& pMeaning,
                                                   ...
                                                   SemanticNumberType pNumber) const
{
  ...
  StaticLinguisticMeaning statLingMeaning = _lingDict.getLingMeaning(pWord.lemma,
                                        ...
                                        SemanticNumberType& pNumber) const
{
  ...
  case LinguisticMeaningType::WORD:
  {
    ...
    if (itWordToInfl != _wordToInflections.end())
    {
      ...
      if (_tryGetInflectedFrom(pRes, SemanticGenderType::NEUTRAL, SemanticNumberType::SINGULAR, inflections))
      {
        pNumber = SemanticNumberType::SINGULAR;
        pGender = SemanticGenderType::NEUTRAL;
        return;
      }
      if (_tryGetInflectedFrom(pRes, SemanticGenderType::MASCULINE, SemanticNumberType::SINGULAR, inflections))
      {
        pNumber = SemanticNumberType::SINGULAR;
        pGender = SemanticGenderType::MASCULINE;
        return;
      }
      if (_tryGetInflectedFrom(pRes, SemanticGenderType::FEMININE, SemanticNumberType::SINGULAR, inflections))
      {
        pNumber = SemanticNumberType::SINGULAR;
        pGender = SemanticGenderType::FEMININE;
        return;
      ...
      if (_tryGetInflectedFrom(pRes, SemanticGenderType::MASCULINE, SemanticNumberType::PLURAL, inflections))
      {
        pNumber = SemanticNumberType::PLURAL;
        pGender = SemanticGenderType::MASCULINE;
        return;
    ...
    StaticLinguisticMeaning statLingMeaning = _lingDict.getLingMeaning(word.lemma,
...
bool SynthesizerDictionary::_tryGetInflectedFrom(std::string& pRes,
                                                 SemanticGenderType pGender,
