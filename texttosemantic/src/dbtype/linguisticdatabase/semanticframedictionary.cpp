...
void _skipDeterminer(TOKENIT& pItToken, const TOKENIT& pItEnd) {
    while (pItToken != pItEnd && pItToken->inflWords.front().word.partOfSpeech == PartOfSpeech::DETERMINER) {
...
bool _conceptConditionChecking(const LinguisticConditionTreeValue& pConditionValue,
    ...
    if (itToken != itEnd) {
        ...
        if (!pConditionValue.parameters.empty()
            && ((pBeginOfConceptOrHyponym
                 && ConceptSet::haveAConceptThatBeginWith(inflWord.infos.concepts, pConditionValue.parameters[0]))
                || (!pBeginOfConceptOrHyponym
                    && ConceptSet::haveAConceptOrAHyponym(inflWord.infos.concepts, pConditionValue.parameters[0])))) {
...
bool _checkDate(const SemanticDate& pDate, const std::vector<std::string>& pParameters) {
    return pParameters.empty() || pDate.day.has_value();
}

bool _isConditionIsVerified(const LinguisticConditionTreeValue& pConditionValue, const ConstTokenIterator* pNextToken) {
    switch (pConditionValue.condition) {
            ...
            if (itToken != itEnd) {
                ...
                if (itToken != itEnd
                    ...
                    if (itToken != itEnd) {
                        ...
                        const std::string timeCpt =
                            currInflWord.word.language == SemanticLanguageEnum::ENGLISH ? "time" : "times";
        ...
        case LinguisticCondition::FOLLOWEDBYPARTOFSPEECH: {
            if (!pConditionValue.parameters.empty()) {
                return pNextToken != nullptr && !pNextToken->atEnd()
                    && pNextToken->getToken().inflWords.front().word.partOfSpeech
                           == partOfSpeech_fromStr(pConditionValue.parameters[0]);
            }
            return false;
        }
        ...
        case LinguisticCondition::FOLLOWEDBYQUANTITYEVERYTHING: {
            return pNextToken != nullptr && !pNextToken->atEnd()
                && ConceptSet::haveAConcept(pNextToken->getToken().inflWords.front().infos.concepts,
                                            "quantity_everything");
        }
        ...
        case LinguisticCondition::LOCATIONEN_FR: {
            if (pNextToken != nullptr && !pNextToken->atEnd()) {
                ...
                if (ConceptSet::haveAConceptThatBeginWith(inflWord.infos.concepts, "country_")
                    ...
                    && !(gender == SemanticGenderType::MASCULINE && !inflWord.word.lemma.empty()
...
bool _doesConceptConditionMatchWithGrounding(const LinguisticConditionTreeValue& pConditionValue,
    ...
    if (!pConditionValue.parameters.empty()) {
        if ((pConditionValue.parameters.size() < 2 || pConditionValue.parameters[1] != "skip_determiners")
...
bool _doesConditionMatchWithGrounding(const LinguisticConditionTreeValue& pConditionValue,
    ...
    switch (pConditionValue.condition) {
        case LinguisticCondition::FOLLOWEDBYBEGINOFCONCEPT:
            return _doesConceptConditionMatchWithGrounding(
                pConditionValue, true, grounding, pObjectGrdExp, pOut, pNumber, pGender);
        case LinguisticCondition::FOLLOWEDBYCONCEPTORHYPONYM:
            return _doesConceptConditionMatchWithGrounding(
                pConditionValue, false, grounding, pObjectGrdExp, pOut, pNumber, pGender);
        case LinguisticCondition::FOLLOWEDBYDEFINITENOUN: {
        ...
        case LinguisticCondition::FOLLOWEDBYHOUR: {
            if (pOut != nullptr && !pOut->empty()
                && ConceptSet::haveAConcept(pOut->back().concepts, "time_relative_before"))
                return false;
            const auto* timeGrdPtr = grounding.getTimeGroundingPtr();
        ...
        case LinguisticCondition::LOCATIONEN_FR: {
            if (ConceptSet::haveAConceptThatBeginWith(grounding.concepts, "country_")
                ...
                && !(pGender == SemanticGenderType::MASCULINE && !pInflWord.word.lemma.empty()
...
bool _doesPrepIsChildSpecIntroWord(const ChildSpecification& pChildSpecification,
    ...
    return pChildSpecification.introWord && pChildSpecification.introWord->lemma == pPrepInflWordPtr->word.lemma
...
const ChildSpecification* _getChildSpecThatMatchFromTwoLists(
...
bool _ifWordWillBeChosenForSynthesisIsItTheSameLemma(bool& pRes,
                                                     const ChildSpecification* pChildSpecPtr,
                                                     const ChildSpecification* pSpecChoosenForLingAnalysisPtr,
                                                     const ConstTokenIterator* pNextToken) {
    if (pChildSpecPtr
        == pSpecChoosenForLingAnalysisPtr)    // This is only for optimisation because if we are on the same child spec
                                              // we already checked that the condition is true
    {
        pRes = true;
        return true;
    }
...
bool _willBeAbleToSynthesizeIt(const std::list<ChildSpecification*>& pChildSpecifications,
    ...
    bool res = false;
    for (const auto& currChildSpec : pChildSpecifications)
        if (_ifWordWillBeChosenForSynthesisIsItTheSameLemma(
                res, currChildSpec, pSpecChoosenForLingAnalysisPtr, pNextToken))
            return res;
    return false;
}
    ...
    if (itLemmaToSpecs != pDefaultChilds.lemmaToChildSpecs.end()) {
        if (pVerbChildsPtr != nullptr) {
            ...
            if (itVerbLemmaToSpecs != pVerbChildsPtr->lemmaToChildSpecs.end())
                return _getChildSpecThatMatchFromTwoLists(
                    itLemmaToSpecs->second, itVerbLemmaToSpecs->second, pPrepInflWordPtr, pNextToken);
        }
...
mystd::optional<ChunkLinkType> _getChunkLinkFromSchildsSpecs(bool& pWillBeAbleToSynthesizeIt,
                                                             ...
                                                             const ConstTokenIterator* pNextToken) {
    const ChildSpecification* childSpecThatMatchPtr =
        _getChildSpecFromContainers(pDefaultChilds, pVerbChildsPtr, pPrepInflWordPtr, pNextToken);
    if (childSpecThatMatchPtr != nullptr) {
        ...
        if (itLemmaToSpecs != pDefaultChilds.chkLinkToChildSpecs.end()) {
            if (pVerbChildsPtr != nullptr) {
                auto itVerbLemmaToSpecs =
                    pVerbChildsPtr->chkLinkToChildSpecs.find(childSpecThatMatchPtr->chunkLinkType);
                if (itVerbLemmaToSpecs != pVerbChildsPtr->chkLinkToChildSpecs.end()) {
                    pWillBeAbleToSynthesizeIt = _willBeAbleToSynthesizeItFromTwoLists(
                        itLemmaToSpecs->second, itVerbLemmaToSpecs->second, childSpecThatMatchPtr, pNextToken);
                    return childSpecThatMatchPtr->chunkLinkType;
                }
            }
            pWillBeAbleToSynthesizeIt =
                _willBeAbleToSynthesizeIt(itLemmaToSpecs->second, childSpecThatMatchPtr, pNextToken);
            return childSpecThatMatchPtr->chunkLinkType;
        } else if (pVerbChildsPtr != nullptr) {
            auto itVerbLemmaToSpecs = pVerbChildsPtr->chkLinkToChildSpecs.find(childSpecThatMatchPtr->chunkLinkType);
            if (itVerbLemmaToSpecs != pVerbChildsPtr->chkLinkToChildSpecs.end()) {
                pWillBeAbleToSynthesizeIt =
                    _willBeAbleToSynthesizeIt(itVerbLemmaToSpecs->second, childSpecThatMatchPtr, pNextToken);
                return childSpecThatMatchPtr->chunkLinkType;
            }
        }
    ...
    switch (pConditionTree->type) {
        case LinguisticConditionTreeType::VALUE: {
            if (pObjectGrdExpPtr != nullptr)
                return _doesConditionMatchWithGrounding(
                    pConditionTree->getValue(), *pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord);
            return false;
        }
        case LinguisticConditionTreeType::OPERAND: {
            const auto& operandStruct = pConditionTree->getOperand();
            switch (operandStruct.operand) {
                case LinguisticConditionTreeOperandEnum::AND: {
                    for (const auto& currChild : operandStruct.children)
                        if (!_getIntroWordCheckConditions(
                                currChild, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord))
                            return false;
                    return true;
                }
                case LinguisticConditionTreeOperandEnum::NOT: {
                    if (!operandStruct.children.empty())
                        return !_getIntroWordCheckConditions(
                            operandStruct.children.front(), pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord);
                    break;
                }
                case LinguisticConditionTreeOperandEnum::OR: {
                    for (const auto& currChild : operandStruct.children)
...
bool _getIntroWordFromSpec(mystd::optional<SemanticWord>& pIntroWord,
                           ...
                           const linguistics::InflectedWord& pInflWord) {
    bool conditionAreOk =
        _getIntroWordCheckConditions(pChildSpec.conditionTree, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord);
    if (conditionAreOk) {
...
bool _getIntroWordFromTwoLists(mystd::optional<SemanticWord>& pIntroWord,
    ...
    while (it1 != pChildSpecifications1.end() && it2 != pChildSpecifications2.end()) {
        if ((*it1)->templatePos < (*it2)->templatePos) {
            if (_getIntroWordFromSpec(
                    pIntroWord, **it1, pVerbConcepts, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord))
                return true;
            ++it1;
            continue;
        }
        if (_getIntroWordFromSpec(
                pIntroWord, **it2, pVerbConcepts, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord))
            return true;
        ++it2;
    }
...
bool _getIntroWordFromOneList(mystd::optional<SemanticWord>& pIntroWord,
    ...
    for (const auto& currChildSpec : pChildSpecifications)
        if (_getIntroWordFromSpec(
                pIntroWord, *currChildSpec, pVerbConcepts, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord))
            return true;
    return false;
}
...
bool _getIntroWord(mystd::optional<SemanticWord>& pIntroWord,
    ...
    if (itLemmaToSpecs != pDefaultChilds.chkLinkToChildSpecs.end()) {
        if (pVerbChildsPtr != nullptr) {
            ...
            if (itVerbLemmaToSpecs != pVerbChildsPtr->chkLinkToChildSpecs.end())
                return _getIntroWordFromTwoLists(pIntroWord,
                                                 itLemmaToSpecs->second,
                                                 itVerbLemmaToSpecs->second,
                                                 pVerbConcepts,
                                                 pObjectGrdExpPtr,
                                                 pOut,
                                                 pNumber,
                                                 pGender,
                                                 pInflWord);
            return _getIntroWordFromOneList(
                pIntroWord, itLemmaToSpecs->second, pVerbConcepts, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord);
        }
        return _getIntroWordFromOneList(
            pIntroWord, itLemmaToSpecs->second, pVerbConcepts, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord);
    } else if (pVerbChildsPtr != nullptr) {
        auto itVerbLemmaToSpecs = pVerbChildsPtr->chkLinkToChildSpecs.find(pChunkLinkType);
        if (itVerbLemmaToSpecs != pVerbChildsPtr->chkLinkToChildSpecs.end())
            return _getIntroWordFromOneList(pIntroWord,
                                            itVerbLemmaToSpecs->second,
                                            pVerbConcepts,
                                            pObjectGrdExpPtr,
                                            pOut,
...
void SemanticFrameDictionary::addWordToChildSpecifications(const SemanticWord& pWord, ChildSpecification&& pChildSpec) {
...
void SemanticFrameDictionary::addWordToTemplate(const SemanticWord& pWord, const std::string& pTemplateName) {
...
mystd::optional<ChunkLinkType> SemanticFrameDictionary::getChunkLinkFromContext(
    ...
    if (pVerbInflectedWordPtr != nullptr) {
        ...
        return _getChunkLinkFromSchildsSpecs(pWillBeAbleToSynthesizeIt,
                                             pVerbInflectedWordPtr->infos.concepts,
                                             *_childSpecificationsByDefault,
                                             verbChildsPtr,
                                             pPrepInflWordPtr,
                                             pNextToken);
    }

    const auto* childSpecThatMatchPtr =
        _getChildSpecFromContainers(*_childSpecificationsWithoutVerbByDefault, nullptr, pPrepInflWordPtr, pNextToken);
    if (childSpecThatMatchPtr != nullptr)
        return childSpecThatMatchPtr->chunkLinkType;
    return mystd::optional<ChunkLinkType>();
}
...
bool SemanticFrameDictionary::getIntroWord(mystd::optional<SemanticWord>& pIntroWord,
    ...
    if (itWordChildSpec != _wordToChildSpecifications.end())
        verbChildsPtr = &*itWordChildSpec->second;
    return _getIntroWord(pIntroWord,
                         pChunkLinkType,
                         pVerbConcepts,
                         *_childSpecificationsByDefault,
                         verbChildsPtr,
...
bool SemanticFrameDictionary::getIntroWordWithoutVerb(mystd::optional<SemanticWord>& pIntroWord,
    ...
    if (itLemmaToSpecs != _childSpecificationsWithoutVerbByDefault->chkLinkToChildSpecs.end()) {
        for (const auto& currChildSpec : itLemmaToSpecs->second) {
            if (_getIntroWordCheckConditions(
                    currChildSpec->conditionTree, pObjectGrdExpPtr, pOut, pNumber, pGender, pInflWord)) {
...
bool SemanticFrameDictionary::doesIntroductionWordHasChunkLinkType(const SemanticWord& pIntroductionWord,
    ...
    if (it != _childSpecificationsWithoutVerbByDefault->lemmaToChildSpecs.end())
        for (const auto& currChildSpec : it->second)
            if (currChildSpec->introWord && *currChildSpec->introWord == pIntroductionWord
                && currChildSpec->chunkLinkType == pChunkLinkType)
                return true;
    return false;
}

mystd::optional<ChunkLinkType> SemanticFrameDictionary::introductionWordToChunkLinkType(
    const SemanticWord& pIntroductionWord) const {
    auto it = _childSpecificationsWithoutVerbByDefault->lemmaToChildSpecs.find(pIntroductionWord.lemma);
    if (it != _childSpecificationsWithoutVerbByDefault->lemmaToChildSpecs.end())
        for (const auto& currChildSpec : it->second)
            if (currChildSpec->introWord && *currChildSpec->introWord == pIntroductionWord)
