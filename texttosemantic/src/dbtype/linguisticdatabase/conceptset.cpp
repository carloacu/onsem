...
void ConceptSet::notifyOppositeConcepts(const std::string& pConceptName1,
...
void ConceptSet::getOppositeConcepts
...
bool ConceptSet::areOppositeConcepts(const std::string& pConcept1,
...
bool ConceptSet::haveOppositeConcepts
...
void ConceptSet::conceptToChildConcepts(std::vector<std::string>& pResConcepts,
...
void ConceptSet::conceptToParentConcepts(std::vector<std::string>& pResConcepts,
...
std::size_t ConceptSet::getStaticConceptsSize(std::string& pErrorStr,
                                              bool& pIsLoaded) const
{
  return statDb.getSize(pErrorStr, pIsLoaded);
}

mystd::optional<bool> ConceptSet::areConceptsCompatibles
(const std::map<std::string, char>& pConcepts1,
 const std::map<std::string, char>& pConcepts2) const
{
...
bool ConceptSet::haveAnyOfConcepts
(const std::map<std::string, char>& pInputConcepts,
...
bool ConceptSet::haveAConceptOrAHyponym
(const std::map<std::string, char>& pInputConcepts,
 const std::string& pConcept)
{
  for (const auto& currConcept : pInputConcepts)
    if (doesConceptBeginWith(currConcept.first, pConcept) &&
        (currConcept.first.size() <= pConcept.size() ||
         currConcept.first[pConcept.size()] == '_'))
      return true;
  return false;
}
...
bool ConceptSet::doesConceptBeginWithWithAnyOf
(const std::string& pConceptName,
 const std::vector<std::string>& pBeginOfConcepts)
{
...
void ConceptSet::extractConceptsThatBeginWith
 ...
 const std::string& pBeginOfConceptName)
{
  ...
  for (const auto& currConcept : pInputConcepts)
    if (doesConceptBeginWith(currConcept.first, pBeginOfConceptName))
      pExtractedConcepts.emplace(currConcept.first,
                                 currConcept.second);
}


bool ConceptSet::extractUserId(std::string& pUserId,
                               const std::map<std::string, char>& pInputConcepts)
{
...
bool ConceptSet::haveAConceptIncompatibleWithSomethingCountable
(const std::map<std::string, char>& pInputConcepts)
{
...
char ConceptSet::getConfidenceOfConcept
(const std::map<std::string, char>& pInputConcepts,
 const std::string& pConceptName)
{
...
bool ConceptSet::haveAConceptNotAny
(const std::map<std::string, char>& pInputConcepts)
{
  for (const auto& currConcept : pInputConcepts)
...
{
  if (pConceptName.size() > 2 &&
      pConceptName.compare(pConceptName.size() - 2, 2, "_*") == 0)
    pConceptName = pConceptName.substr(0, pConceptName.size() - 2);
}


void ConceptSet::sortConceptsSortedByConfidence
...
bool ConceptSet::haveAConceptParentOf
(const std::map<std::string, char>& pConcepts1,
 const std::map<std::string, char>& pConcepts2)
{
...
void ConceptSet::sortAndPrintConcepts
...
bool ConceptSet::verbSyntesisIsFollowedByAnInfinitiveThatMeansAnImperative(const std::map<std::string, char>& pConcepts)
{
...
bool ConceptSet::rankConceptToNumberStr(
    std::string& pNumberStr,
    const std::map<std::string, char>& pConcepts)
{
  ...
  for (const auto& currCpt : pConcepts)
  {
    ...
    {
      pNumberStr = currCpt.first.substr(begOfRankCpt_size,
                                        currCpt.first.size() - begOfRankCpt_size);
      try
      {
