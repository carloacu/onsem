...
private:
    ...
    void _extractNewForms(std::list<std::pair<int, UniqueSemanticExpression>>& pNewFormsOfTheExp,
    ...
    void _searchRootOfSplitPossibilities(UniqueSemanticExpression& pSemExp,
                                         SemanticLanguageEnum pLanguage,
                                         bool pAllOrJustTheOneThatAreInBothDirections,
                                         const SemanticExpression& pRootSemExpForDebug,
                                         std::list<std::list<SemLineToPrint>>* pDebugOutput) const;

    void _addDifferentFormsOfExpressionForALanguage(UniqueSemanticExpression& pSemExp,
                                                    bool pAllOrJustTheOneThatAreInBothDirections,
                                                    const SemanticExpression& pRootSemExpForDebug,
                                                    std::list<std::list<SemLineToPrint>>* pDebugOutput) const;

    void _printAConversionInfos(std::list<std::list<SemLineToPrint>>& pDebugOutput,
                                const ConversionRule& pConvInfos) const;

    void _printAPatternNode(std::list<SemLineToPrint>& pOutLines,
                            std::stringstream& pSs,
                            std::size_t pOffsetNewLine,
                            const SemExpTreePatternNode& pRootPattern) const;

    void _recurssivelyAddForms(std::list<std::pair<int, UniqueSemanticExpression>>& pNewFormsOfTheExp,
                               UniqueSemanticExpression& pSemExp,
                               std::set<std::string>& pAlreadyDoneConvIds,
                               const ConceptTreeOfRules<ConversionRule>& pTreeOfConvs,
                               int pCurrPrio,
    ...
    static void _addExpsToDebugOutput(std::list<std::list<SemLineToPrint>>& pDebugOutput,
                                      const SemanticExpression& pSemExp);

    void _refactorSemExpForALanguage(UniqueSemanticExpression& pSemExp,
                                     TreePatternConventionEnum pFromConvention,
                                     TreePatternConventionEnum pToConvention,
                                     SemanticLanguageEnum pLanguage,
                                     std::list<std::list<SemLineToPrint>>* pDebugOutput) const;

    void _applyModifsOnSemExp(SemanticExpression& pRootInSemExp,
                              const std::map<std::string, std::list<std::shared_ptr<SemanticExpression>>>& pLinks,
                              const SemExpTreePatternNode& pRootPattern) const;

    void _applyModifsOnGrdExp(GroundedExpression& pRootInGrExp,
    ...
    bool _aSubSemExpIsInTheList(const ListExpression& pListExp,
                                const std::list<std::shared_ptr<SemanticExpression>>& pSubSemExp) const;

    void _applyModifsForAChild(GroundedExpression& pRootInGrdExp,
                               const std::map<std::string, std::list<std::shared_ptr<SemanticExpression>>>& pRepLinks,
    ...
    void _refactSemExpRec(UniqueSemanticExpression& pSemExp,
                          std::map<const ConversionRule*, std::set<const SemanticExpression*>>& pAlreadyDoneConv,
                          const ConceptTreeOfRules<ConversionRule>& pTreeOfConvs,
                          const SemanticExpression& pRootSemExpForDebug,
                          std::list<std::list<SemLineToPrint>>* pDebugOutput) const;

    void _refactSemExp(UniqueSemanticExpression& pSemExp,
                       std::map<const ConversionRule*, std::set<const SemanticExpression*>>& pAlreadyDoneConv,
                       const ConceptTreeOfRules<ConversionRule>& pTreeOfConvs,
                       const SemanticExpression& pRootSemExpForDebug,
                       std::list<std::list<SemLineToPrint>>* pDebugOutput) const;

    UniqueSemanticExpression _patternNodeToSemExp(
        const SemExpTreePatternNode& pTreePattern,
        const std::map<std::string, std::list<std::shared_ptr<SemanticExpression>>>& pRepLinks,
        bool pSetMotherConceptOfConceptsAny = false) const;

    template<typename RULE>
    void _getRulesThatHaveTheseConcepts(std::list<const RULE*>& pPossiblesConv,
    ...
    bool _doesASemExpMatchAPatternTree(SEMEXPCONTAINER*& pRootInSemExp,
