...
int StaticLinguisticDictionary::getVersion(const std::string& pFilename) {
...
void StaticLinguisticDictionary::xLoad(std::istream& pDictIStream) {
    ...
    */
    ...
    pDictIStream.read(reinterpret_cast<char*>(&fIfSeparatorBetweenWords), sizeof(bool));
...
SemanticRequestType StaticLinguisticDictionary::wordToSubordinateRequest(const SemanticWord& pWord) const {
...
SemanticRequestType StaticLinguisticDictionary::aloneWordToRequest(const SemanticWord& pWord) const {
...
SemanticRequestType StaticLinguisticDictionary::semWordToRequest(const SemanticWord& pWord) const {
...
void StaticLinguisticDictionary::StaticWord::setContent(const StaticLinguisticDictionary& pStatBinDico,
        ...
        for (unsigned int i = 0; i < nbOFElts; ++i) {
            ...
            *pChar += sizeof(int);
            ...
            if (meaning != LinguisticMeaning_noMeaningId) {
                ...
                if (canBeAlone)
                    fQuestWordsThatCanBeAlone[questionSemWord].emplace_back(
                        canBeBeforeVerb, canBeAfterVerb, canBeAlone, request, followedByRequestedWord);
                if (canBeBeforeVerb == WordToVerbRelation::NO && canBeAfterVerb == WordToVerbRelation::NO) {
                    fQuestWordsSubordinate[std::move(questionSemWord)].emplace_back(
                        canBeBeforeVerb, canBeAfterVerb, canBeAlone, request, followedByRequestedWord);
                } else {
                    if (canBeBeforeVerb != WordToVerbRelation::NO)
                        fQuestWordsBefore[questionSemWord].emplace_back(
                            canBeBeforeVerb, canBeAfterVerb, canBeAlone, request, followedByRequestedWord);
                    if (canBeAfterVerb != WordToVerbRelation::NO)
                        fQuestWordsAfter[std::move(questionSemWord)].emplace_back(
                            canBeBeforeVerb, canBeAfterVerb, canBeAlone, request, followedByRequestedWord);
                }
            }
        }
...
void StaticLinguisticDictionary::getGramPossibilitiesAndPutUnknownIfNothingFound(std::list<InflectedWord>& pInfosGram,
...
void StaticLinguisticDictionary::getGramPossibilities(std::list<InflectedWord>& pInfosGram,
                                                      const std::string& pWord,
                                                      std::size_t pBeginPos,
                                                      std::size_t pSizeOfWord) const {
    assert(xIsLoaded());
...
std::size_t StaticLinguisticDictionary::getConfidenceOfWordInThatLanguage(PartOfSpeech& pMainPartOfSpeech,
                                                                          const std::string& pWord,
                                                                          std::size_t pBeginPos,
                                                                          std::size_t pSizeOfWord) const {
    assert(xIsLoaded());
...
std::size_t StaticLinguisticDictionary::xGetConfOfNode(PartOfSpeech& pMainPartOfSpeech,
...
template<typename T>
void _initOneNomInflection(T& pNomInfl, unsigned char pInflection) {
...
void _initNomInflections(std::list<NominalInflection>& pNomInfls, const signed char* pFls, unsigned char pNbFlexions) {
...
void _initRelPerson(RelativePerson& pRelPerson, char pPerson, char pNumber) {
    switch (pPerson) {
        case '1': {
            pRelPerson = pNumber == 'p' ? RelativePerson::FIRST_PLUR : RelativePerson::FIRST_SING;
            break;
        }
        case '2': {
            pRelPerson = pNumber == 'p' ? RelativePerson::SECOND_PLUR : RelativePerson::SECOND_SING;
            break;
        }
        case '3': {
            pRelPerson = pNumber == 'p' ? RelativePerson::THIRD_PLUR : RelativePerson::THIRD_SING;
            break;
        }
        case '0': {
            if (pNumber != 'u')
...
void _initVerbInflections(std::list<VerbalInflection>& pVerbInfls, const signed char* pFls, unsigned char pNbFlexions) {
    for (unsigned char i = 0; i < pNbFlexions; ++i, ++pFls) {
...
void StaticLinguisticDictionary::xGetGramPossFromNode(std::list<InflectedWord>& pInfosGram,
    ...
    for (unsigned char i = 0; i < nbMeanings; ++i) {
        ...
        infosGramBack.moveInflections(_getInflections(infosGramBack.word.partOfSpeech, fls, nbFlexions));
        mns = xGetNextMeaningFromNode(mns);

        // Add _again concepts for french verbs that begin with "re"
        if (fLangEnum == SemanticLanguageEnum::FRENCH && infosGramBack.word.partOfSpeech == PartOfSpeech::VERB
            && infosGramBack.word.lemma.compare(0, 2, "re") == 0) {
            const auto* rootWordNode = xGetNode(infosGramBack.word.lemma, 2, infosGramBack.word.lemma.size() - 2, true);
...
void StaticLinguisticDictionary::xFillIGram(InflectedWord& pIGram, int32_t pMeaningId) const {
    xGetSemanticWord(pIGram.word, pMeaningId);
    xFillWordAssInfos(pIGram.infos, pMeaningId);
}

void StaticLinguisticDictionary::xFillWordAssInfos(WordAssociatedInfos& pInfos, int pMeaningId) const {
...
std::size_t StaticLinguisticDictionary::getLengthOfLongestWord(const std::string& pStr, std::size_t pBeginStr) const {
    assert(xIsLoaded());
    std::size_t pos = 0;
    xSearchInPatriciaTrie(pos,
                          pStr,
                          pBeginStr,
                          pStr.size() - pBeginStr,
                          true,
                          fIfSeparatorBetweenWords ? SearchForLongestWordMode::ENBABLED_BUTWITHSEPARATORAFTER
                                                   : SearchForLongestWordMode::ENABLED);
    return pos;
}

StaticLinguisticMeaning StaticLinguisticDictionary::getLingMeaning(const std::string& pWord,
                                                                   PartOfSpeech pGram,
...
void StaticLinguisticDictionary::getLemma(std::string& pLemmaStr, const std::string& pWord) const {
    assert(xIsLoaded());
    // search a simple meaning
    auto* node = xGetNode(pWord, 0, pWord.size(), false);
    if (node != nullptr && xNbMeanings(node) > 0) {
        xGetWord(pLemmaStr, xGetLemmeNodeId(fPtrMeaning + xMeaningFromNodeToMeaningId(xGetMeaningList(node))));
    }
}

std::string StaticLinguisticDictionary::getLemma(const StaticLinguisticMeaning& pMeaning,
...
void StaticLinguisticDictionary::getWord(SemanticWord& pWord, const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    assert(pMeaning.language == fLangEnum);
    xGetSemanticWord(pWord, pMeaning.meaningId);
}

void StaticLinguisticDictionary::getSemanticWord(SemanticWord& pWord, int pMeaningId) const {
    assert(xIsLoaded());
    xGetSemanticWord(pWord, pMeaningId);
}

void StaticLinguisticDictionary::xGetSemanticWord(SemanticWord& pWord, int32_t pMeaningId) const {
...
void StaticLinguisticDictionary::xGetMetaMeanings(std::list<LingWordsGroup>& pMetaMeanings,
...
void StaticLinguisticDictionary::xFillMeaningGroup(LingWordsGroup& pMeaningGroup, int pRootMeaningId) const {
    ...
    for (unsigned char i = 0; i < nbLinkedMeanings; ++i) {
        ...
        pMeaningGroup.linkedMeanings.emplace_back(std::move(rootWord),
...
void StaticLinguisticDictionary::xTryToFillMeaningGroup(mystd::optional<LingWordsGroup>& pLinkedMeanings,
...
StaticLinguisticMeaning StaticLinguisticDictionary::getRootMeaning(const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    assert(pMeaning.language == fLangEnum);
    if (_ptrPatriciaTrie == nullptr)
        return StaticLinguisticMeaning();
...
void StaticLinguisticDictionary::getInfoGram(InflectedWord& pIGram, const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    if (pMeaning.meaningId == LinguisticMeaning_noMeaningId) {
        pIGram.clear();
        return;
...
void StaticLinguisticDictionary::getContextualInfos(std::set<WordContextualInfos>& pContextualInfos,
...
void StaticLinguisticDictionary::getConcepts(std::map<std::string, char>& pConcepts,
                                             const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    if (pMeaning.meaningId == LinguisticMeaning_noMeaningId)
        return;
...
bool StaticLinguisticDictionary::isAGatheringMeaning(const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    assert(pMeaning.language == fLangEnum);
    if (pMeaning.meaningId == LinguisticMeaning_noMeaningId)
        return false;
...
bool StaticLinguisticDictionary::hasContextualInfo(WordContextualInfos pContextualInfo,
                                                   const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    if (pMeaning.meaningId == LinguisticMeaning_noMeaningId)
        return false;
...
void StaticLinguisticDictionary::xGetWordContextualInfos(std::set<WordContextualInfos>& pContextualInfos,
                                                         const signed char* pMeaningPtr) const {
...
bool StaticLinguisticDictionary::xWordHasContextualInfos(WordContextualInfos pContextualInfos,
                                                         const signed char* pMeaningPtr) const {
    char nbContextInfos = xNbContextInfos(pMeaningPtr);
    auto* contInfoPtr = xGetFirstContextInfo(pMeaningPtr);
    for (char j = 0; j < nbContextInfos; ++j) {
...
void StaticLinguisticDictionary::xGetConcepts(std::map<std::string, char>& pConcepts,
...
void StaticLinguisticDictionary::putRootMeaning(InflectedWord& pInfosGram) const {
    StaticLinguisticMeaning lingMeaning = getLingMeaning(pInfosGram.word.lemma, pInfosGram.word.partOfSpeech, true);
...
bool StaticLinguisticDictionary::getConjugaisionsId(int& pConjId, const StaticLinguisticMeaning& pMeaning) const {
    assert(xIsLoaded());
    assert(pMeaning.language == fLangEnum);
    if (pMeaning.meaningId == LinguisticMeaning_noMeaningId)
        return false;
...
void StaticLinguisticDictionary::getWord(std::string& pWord, int pWordNode) const {
    assert(xIsLoaded());
    xGetWord(pWord, pWordNode);
}

std::string StaticLinguisticDictionary::xGetLemma(int32_t pMeaningId, bool pWithLinkMeanings) const {
    if (pMeaningId == LinguisticMeaning_noMeaningId)
        return "";
