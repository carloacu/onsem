...
void StaticSynthesizerDictionary::xLoad(std::istream& pIStream)
{
  ...
  fTotalSize = conjugaisonsSize + conceptsToMeaningsSize +
      meaningsFromConceptsSize;

  if (!binaryloader::allocMemZone(&fPtrConjugaisons, pIStream, conjugaisonsSize) ||
      !binaryloader::allocMemZone(&fConceptsToMeanings, pIStream,
                                  conceptsToMeaningsSize) ||
      !binaryloader::allocMemZone(&fMeaningsFromConcepts, pIStream,
                                  meaningsFromConceptsSize))
  {
    xUnload();
...
std::string StaticSynthesizerDictionary::getLemma
...
StaticLinguisticMeaning StaticSynthesizerDictionary::conceptToMeaning
...
signed char* StaticSynthesizerDictionary::xGetPtrToMeaningLinkedAndMeaningLinkedBefore
(int pConceptId) const
{
  ...
  int meaningFromConceptId = binaryloader::alignedDecToInt
      (*reinterpret_cast<const int*>(fConceptsToMeanings + cptToMeaningId));
...
void StaticSynthesizerDictionary::getImperativeVerbForm
 ...
 bool pIsPositiveForm) const
{
  ...
  if (fLangEnum == SemanticLanguageEnum::ENGLISH &&
      !pIsPositiveForm)
  {
  ...
  case RelativePerson::SECOND_PLUR:
    ...
    break;
  default:
    wordEndingNode = *(reinterpret_cast<const int*>(fPtrConjugaisons + conjId) + decImperative);
    break;
  }
...
void StaticSynthesizerDictionary::getVerbForm
(std::list<WordToSynthesize>& pVerbForm,
 ...
 bool pQuestionForm) const
{
  ...
  if (fLangEnum == SemanticLanguageEnum::ENGLISH &&
      verbIGram.word.partOfSpeech != PartOfSpeech::AUX)
  {
    switch (pVerbTense)
    {
  ...
  if (verbIGram.word.partOfSpeech != PartOfSpeech::AUX &&
      (pIsPassive ||
       (fLangEnum == SemanticLanguageEnum::FRENCH && pVerbTense == LinguisticVerbTense::SIMPLE_PAST_INDICATIVE)))
  {
    ...
    if (!pIsASubordinateWithoutPreposition ||
        (fLangEnum == SemanticLanguageEnum::FRENCH &&
         pVerbTense == LinguisticVerbTense::PRESENT_INDICATIVE && verbIGram.word.lemma != "naître" && !ConceptSet::haveAConcept(verbIGram.infos.concepts, "verb_die")))
    {
      if (fLangEnum == SemanticLanguageEnum::FRENCH)
      {
        if (pPartOfAComposedVerb ||
            auxTense == LinguisticVerbTense::INFINITIVE ||
...
void StaticSynthesizerDictionary::getNounGenders
...
void StaticSynthesizerDictionary::getNounForm
(std::string& pRes,
 const StaticLinguisticMeaning& pMeaning,
 SemanticGenderType& pGender,
 SemanticNumberType& pNumber) const
{
  int wordEndingNode = xGetnounOrAdjForm(true, pMeaning, pGender, pNumber,
...
void StaticSynthesizerDictionary::getAdjForm
(std::string& pRes,
 const StaticLinguisticMeaning& pMeaning,
 SemanticGenderType pGender,
 ...
 ComparisonOperator pComparisonOperator) const
{
  ...
  int wordEndingNode = xGetnounOrAdjForm(false, pMeaning, pGender, pNumber,
                                         pComparisonOperator);
  if (wordEndingNode != 0)
    fStatLingDic.getWord(pRes, binaryloader::alignedDecToInt(wordEndingNode));
  if (!beforeAdj.empty())
...
int StaticSynthesizerDictionary::xGetnounOrAdjForm
(bool nounOrAdj,
 const StaticLinguisticMeaning& pMeaning,
 SemanticGenderType& pGender,
 SemanticNumberType& pNumber,
 ComparisonOperator pComparisonOperator) const
{
  ...
  if (!nounOrAdj &&
      fLangEnum == SemanticLanguageEnum::ENGLISH &&
      pComparisonOperator == ComparisonOperator::MORE)
  {
