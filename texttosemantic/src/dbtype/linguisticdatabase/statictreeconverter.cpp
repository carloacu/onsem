...
void StaticTreeConverter::refactorSemExp(UniqueSemanticExpression& pUSemExp,
    ...
    _refactorSemExpForALanguage(pUSemExp, pFromConvention, pToConvention, pLanguage, pDebugOutput);
    if (pLanguage != SemanticLanguageEnum::UNKNOWN) {
        _refactorSemExpForALanguage(
            pUSemExp, pFromConvention, pToConvention, SemanticLanguageEnum::UNKNOWN, pDebugOutput);
    }
}

void StaticTreeConverter::addDifferentForms(UniqueSemanticExpression& pSemExp,
                                            SemanticLanguageEnum pLanguage,
    ...
    _searchRootOfSplitPossibilities(
        pSemExp, pLanguage, pAllOrJustTheOneThatAreInBothDirections, *pSemExp, pDebugOutput);
}
...
GrammaticalType StaticTreeConverter::getChildThatShouldBeUnique(
    ...
    for (const auto& currInfoRulePtr : uniqueInfosRules) {
        ...
        if (_doesASemExpMatchAPatternTree<const SemanticExpressionContainer>(
                rootInSemExp, links, semExpWrapped, alreadyTreatedRoots, rootNode, rootNode, false)) {
            ...
            if (itOfUniqueChildInPattern != rootNode.children.end()) {
                ...
                pChildThatShouldBeUnique.emplace(
                    _patternNodeToSemExp(itOfUniqueChildInPattern->second, repLinks, true));
            }
...
void StaticTreeConverter::_searchRootOfSplitPossibilities(UniqueSemanticExpression& pSemExp,
                                                          SemanticLanguageEnum pLanguage,
                                                          bool pAllOrJustTheOneThatAreInBothDirections,
    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            if (grdExp->type == SemanticGroundingType::STATEMENT) {
                _addDifferentFormsOfExpressionForALanguage(
                    pSemExp, pAllOrJustTheOneThatAreInBothDirections, pRootSemExpForDebug, pDebugOutput);
            }

            auto* grdExpPtr = pSemExp->getGrdExpPtr();
            if (grdExpPtr != nullptr) {
                for (auto& currChild : grdExpPtr->children) {
                    _searchRootOfSplitPossibilities(currChild.second,
                                                    pLanguage,
                                                    pAllOrJustTheOneThatAreInBothDirections,
                                                    pRootSemExpForDebug,
                                                    pDebugOutput);
                }
            }
        ...
        case SemanticExpressionType::LIST: {
            ...
            for (auto& currElt : listExp.elts) {
                _searchRootOfSplitPossibilities(
                    currElt, pLanguage, pAllOrJustTheOneThatAreInBothDirections, pRootSemExpForDebug, pDebugOutput);
            }
            break;
        }
        case SemanticExpressionType::CONDITION: {
            ConditionExpression& condExp = pSemExp->getCondExp();
            _searchRootOfSplitPossibilities(condExp.conditionExp,
                                            pLanguage,
                                            pAllOrJustTheOneThatAreInBothDirections,
                                            pRootSemExpForDebug,
                                            pDebugOutput);
            _searchRootOfSplitPossibilities(
                condExp.thenExp, pLanguage, pAllOrJustTheOneThatAreInBothDirections, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::INTERPRETATION: {
            InterpretationExpression& intExp = pSemExp->getIntExp();
            _searchRootOfSplitPossibilities(intExp.interpretedExp,
                                            pLanguage,
                                            pAllOrJustTheOneThatAreInBothDirections,
                                            pRootSemExpForDebug,
                                            pDebugOutput);
        ...
        case SemanticExpressionType::FEEDBACK: {
            ...
            _searchRootOfSplitPossibilities(fdkExp.concernedExp,
                                            pLanguage,
                                            pAllOrJustTheOneThatAreInBothDirections,
                                            pRootSemExpForDebug,
                                            pDebugOutput);
        ...
        case SemanticExpressionType::ANNOTATED: {
            ...
            _searchRootOfSplitPossibilities(
                annExp.semExp, pLanguage, pAllOrJustTheOneThatAreInBothDirections, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::METADATA: {
            MetadataExpression& metadataExp = pSemExp->getMetadataExp();
            _searchRootOfSplitPossibilities(metadataExp.semExp,
                                            pLanguage,
                                            pAllOrJustTheOneThatAreInBothDirections,
                                            pRootSemExpForDebug,
                                            pDebugOutput);
...
void StaticTreeConverter::_addDifferentFormsOfExpressionForALanguage(
    ...
    const auto& semanticForms = pAllOrJustTheOneThatAreInBothDirections ? _semanticForms : _semanticFormsBothDirections;
    _recurssivelyAddForms(
        newFormsOfTheExp, pSemExp, alreadyDoneConvIds, semanticForms, 10, pRootSemExpForDebug, pDebugOutput);

    if (!newFormsOfTheExp.empty()) {
        auto setOfFormsExp = std::make_unique<SetOfFormsExpression>();
        setOfFormsExp->prioToForms[-10].emplace_back(std::make_unique<QuestExpressionFrom>(std::move(pSemExp), true));
...
void StaticTreeConverter::_recurssivelyAddForms(std::list<std::pair<int, UniqueSemanticExpression>>& pNewFormsOfTheExp,
                                                UniqueSemanticExpression& pSemExp,
    ...
    _extractNewForms(
        provNewFormsOfTheExp, pSemExp, pAlreadyDoneConvIds, pTreeOfConvs, pCurrPrio, pRootSemExpForDebug, pDebugOutput);
    ...
    for (auto& currElt : provNewFormsOfTheExp) {
        ...
        _recurssivelyAddForms(pNewFormsOfTheExp,
                              currElt.second,
                              alreadyDoneConvIds,
                              pTreeOfConvs,
                              currElt.first,
...
void StaticTreeConverter::_addExpsToDebugOutput(std::list<std::list<SemLineToPrint>>& pDebugOutput,
                                                const SemanticExpression& pSemExp) {
    pDebugOutput.emplace_back();
    printer::prettyPrintSemExp(pDebugOutput.back(), pSemExp);
}

void StaticTreeConverter::_refactorSemExpForALanguage(UniqueSemanticExpression& pSemExp,
                                                      TreePatternConventionEnum pFromConvention,
                                                      TreePatternConventionEnum pToConvention,
                                                      SemanticLanguageEnum pLanguage,
                                                      std::list<std::list<SemLineToPrint>>* pDebugOutput) const {
    ...
    if (itConvs != itConvForLang->second.end()) {
        ...
        _refactSemExpRec(pSemExp, alreadyDoneConv, itConvs->second, *pSemExp, pDebugOutput);
    }
}
...
void StaticTreeConverter::_refactSemExpRec(
    ...
    switch (pSemExp->type) {
        case SemanticExpressionType::ANNOTATED: {
            ...
            _refactSemExpRec(annExp.semExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::CONDITION: {
            auto& condExp = pSemExp->getCondExp();
            _refactSemExpRec(condExp.thenExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            if (condExp.elseExp)
                _refactSemExpRec(*condExp.elseExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::FEEDBACK: {
            auto& fdkdExp = pSemExp->getFdkExp();
            _refactSemExpRec(fdkdExp.concernedExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::GROUNDED: {
            _refactSemExp(pSemExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::INTERPRETATION: {
            auto& intExp = pSemExp->getIntExp();
            _refactSemExpRec(intExp.interpretedExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::LIST: {
            ...
            if (listExp.listType == ListExpressionType::UNRELATED) {
                for (auto& currElt : listExp.elts) {
                    std::map<const ConversionRule*, std::set<const SemanticExpression*>> alreadyDoneConv;
                    _refactSemExpRec(currElt, alreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
                }
            }
            _refactSemExp(pSemExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        ...
        case SemanticExpressionType::METADATA: {
            ...
            _refactSemExpRec(metadataExp.semExp, pAlreadyDoneConv, pTreeOfConvs, pRootSemExpForDebug, pDebugOutput);
            break;
        }
        case SemanticExpressionType::FIXEDSYNTHESIS:
        case SemanticExpressionType::COMPARISON:
...
void StaticTreeConverter::_refactSemExp(
    ...
    do {
        ...
        for (const auto& currPossConv : possiblesConv) {
            ...
            while (_doesASemExpMatchAPatternTree(
                rootInSemExp, links, pSemExp, alreadyTreatedRoots, rootNode, rootNode, true)) {
                alreadyTreatedRoots.insert(&**rootInSemExp);

                std::map<std::string, std::list<std::shared_ptr<SemanticExpression>>> repLinks;
                _convertFindLinksToReplacementLinks(repLinks, links);
                _applyModifsOnSemExp(**rootInSemExp, repLinks, *currPossConv->treePatternOut);
...
void StaticTreeConverter::_extractNewForms(std::list<std::pair<int, UniqueSemanticExpression>>& pNewFormsOfTheExp,
                                           UniqueSemanticExpression& pSemExp,
                                           std::set<std::string>& pAlreadyDoneConvIds,
                                           const ConceptTreeOfRules<ConversionRule>& pTreeOfConvs,
                                           int pCurrPrio,
    ...
    for (const ConversionRule* currPossConv : possiblesConv) {
        ...
        std::map<std::string, std::list<UniqueSemanticExpression*>> links;
        const SemExpTreePatternNode& rootNode = *currPossConv->treePatternIn;
        if (_doesASemExpMatchAPatternTree(
                rootInSemExp, links, pSemExp, std::set<const SemanticExpression*>(), rootNode, rootNode, false)) {
            ...
            pNewFormsOfTheExp.emplace_back(pCurrPrio + currPossConv->priorityOfOutPattern, std::move(pSemExp));
            pSemExp = std::move(semExpCopied);
        }
    }
}

void StaticTreeConverter::_printAConversionInfos(std::list<std::list<SemLineToPrint>>& pDebugOutput,
...
void StaticTreeConverter::_printAPatternNode(std::list<SemLineToPrint>& pOutLines,
    ...
    if (pRootPattern.word) {
        ...
        if (word.isEmpty())
            ...
            pSs << " word(" << word.lemma << "|" << partOfSpeech_toStr(word.partOfSpeech)
...
void StaticTreeConverter::_getRulesThatHaveTheseConcepts(std::list<const RULE*>& pPossiblesConv,
...
void StaticTreeConverter::_applyModifsOnGrdExp(
    ...
    // fill grounding infos
    {
        ...
        for (const auto& currCpt : pRootPattern.concepts) {
            ...
            if (semExpGrd.concepts.find(conceptName) == semExpGrd.concepts.end())
                semExpGrd.concepts[conceptName] =
                    currCpt.second.empty() ? static_cast<char>(4) : currCpt.second.front();
        }
    ...
    for (const auto& currPattChild : pRootPattern.children) {
        _applyModifsForAChild(pRootInGrExp, pRepLinks, currPattChild.first, currPattChild.second);
    }
}
    ...
    if (!pChildPattern.id.empty()) {
        ...
        if (itLk != pRepLinks.end()) {
            ...
            if (itChild != pRootInGrdExp.children.end()) {
                ...
                if (listSubExp != nullptr && _aSubSemExpIsInTheList(*listSubExp, itLk->second)) {
                ...
                if (itLk->second.size() == 1 && &*itChild->second == &*itLk->second.front()) {
                    _applyModifsOnSemExp(*itLk->second.front(), pRepLinks, pChildPattern);
                } else {
...
UniqueSemanticExpression StaticTreeConverter::_patternNodeToSemExp(
    ...
    const std::map<std::string, std::list<std::shared_ptr<SemanticExpression>>>& pRepLinks,
    bool pSetMotherConceptOfConceptsAny) const {
    auto newChild = std::make_unique<GroundedExpression>(pTreePattern.groundingType
...
bool StaticTreeConverter::_doesASemExpMatchAPatternTree(SEMEXPCONTAINER*& pRootInSemExp,
    ...
    if (grdExpPtr != nullptr) {
        if (pAlreadyTreatedRoots.find(&*pSemExp) == pAlreadyTreatedRoots.end()) {
            ...
            if ((!pCurrNode.groundingType || grd.type == *pCurrNode.groundingType)
                && xIsASubSetOnMapLabel(pCurrNode.concepts, grd.concepts)) {
                bool res = true;

                for (auto itNotCpt = pCurrNode.notConcepts.begin(); itNotCpt != pCurrNode.notConcepts.end();
                     ++itNotCpt) {
                ...
                if (res && pCurrNode.groundingType) {
                    switch (*pCurrNode.groundingType) {
                        case SemanticGroundingType::STATEMENT: {
                            ...
                            if (statGrd != nullptr && pCurrNode.requests.isASubSetOf(statGrd->requests)) {
                                ...
                                if (res && pCurrNode.time.has_value() && statGrd->verbTense != *pCurrNode.time)
                                    res = false;
                        ...
                        case SemanticGroundingType::GENERIC: {
                            ...
                            if (genGrd != nullptr) {
                                if (pCurrNode.type.has_value() && genGrd->entityType != *pCurrNode.type) {
                                    res = false;
                                }

                                if (pCurrNode.nb.has_value() && !genGrd->quantity.isEqualTo(*pCurrNode.nb)) {
                                    res = false;
                                }

                                if (pCurrNode.reference.has_value() && genGrd->referenceType != *pCurrNode.reference) {
                                    res = false;
                                }

                                for (auto itNotType = pCurrNode.notTypes.begin(); itNotType != pCurrNode.notTypes.end();
                                     ++itNotType) {
                        ...
                        case SemanticGroundingType::AGENT: {
                            ...
                            if (agentGrd == nullptr || pCurrNode.reference.has_value()) {
                                res = false;
                            }
                            break;
                        }
                        case SemanticGroundingType::RELATIVETIME: {
                            ...
                            if (relTimeGrdPtr != nullptr) {
                                if (pCurrNode.timeType.has_value() && relTimeGrdPtr->timeType != *pCurrNode.timeType)
                                    res = false;
                            } else {
                                res = false;
                            }
                ...
                if (res) {
                    for (auto itPatternNotChild = pCurrNode.notChildren.begin();
                         itPatternNotChild != pCurrNode.notChildren.end();
                         ++itPatternNotChild) {
                ...
                if (res) {
                    for (auto itPatternChild = pCurrNode.children.begin(); itPatternChild != pCurrNode.children.end();
                         ++itPatternChild) {
                        auto itSemExpChild = grdExpPtr->children.find(itPatternChild->first);
                        if (itSemExpChild == grdExpPtr->children.end()
                            || !_doesASemExpMatchAPatternTree<SEMEXPCONTAINER>(pRootInSemExp,
                                                                               pLinks,
                                                                               itSemExpChild->second,
                                                                               std::set<const SemanticExpression*>(),
                                                                               pRootNode,
                                                                               itPatternChild->second,
        ...
        if (pCanMatchWithAChildNode) {
            for (auto& currChild : grdExpPtr->children) {
                if (_doesASemExpMatchAPatternTree<SEMEXPCONTAINER>(pRootInSemExp,
                                                                   pLinks,
                                                                   currChild.second,
                                                                   pAlreadyTreatedRoots,
                                                                   pRootNode,
                                                                   pRootNode,
                                                                   pCanMatchWithAChildNode)) {
                    return true;
