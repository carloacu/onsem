...
bool MetaWordTreeDb::isASeparator(const std::string& pString, const std::size_t pPos) {
    static const std::string apos = "’";
    static const std::size_t aposSize = apos.size();
    if (pPos < pString.size()) {
        char follChar = pString[pPos];
        return follChar == ' ' || follChar == '\'' || follChar == '"' || follChar == '-' || follChar == ','
            || follChar == '.' || follChar == '?' || follChar == '!' || follChar == ';' || follChar == ':'
            || follChar == '=' || follChar == '(' || follChar == ')' || follChar == '|' || follChar == '{'
            || follChar == '}' || follChar == '\n' || follChar == '\t' || pString.compare(pPos, aposSize, apos) == 0;
    }
    return true;
}

const signed char* MetaWordTreeDb::xSearchInPatriciaTrie(std::size_t& pLongestWord,
                                                         const std::string& pString,
                                                         const std::size_t pBeginOfString,
                                                         const std::size_t pSizeOfString,
            ...
            for (unsigned char i = 0; i < nbChildren; ++i) {
                ...
                if (xIfEndOfAWord(currNode, pOnlyWordWithWordFroms)) {
                    ...
                    switch (pLongWordMode) {
                        case SearchForLongestWordMode::ENABLED: pLongestWord = offWord; break;
                        case SearchForLongestWordMode::ENBABLED_BUTWITHSEPARATORAFTER:
                            if (begWithASeparator || isASeparator(pString, pBeginOfString + offWord))
                                pLongestWord = offWord;
                            break;
                        case SearchForLongestWordMode::DISABLED: break;
                    }
...
void MetaWordTreeDb::xGetWordsThatBeginWith(std::list<const signed char*>& pResWords,
...
void MetaWordTreeDb::xGetWord(std::string& pWord, int pWordNode) const {
    if (_ptrPatriciaTrie == nullptr)
        return;
    ...
    do {
        ...
        currNode = xAlignedDecToPtr(_ptrPatriciaTrie, *xGetFather(currNode));
...
void MetaWordTreeDb::xGetWordsFromANodeOfTheTree(std::list<const signed char*>& pResWords,
                                                 const signed char* pCurrNode,
                                                 bool pCanAddCurrNode) const {
    if (_ptrPatriciaTrie == nullptr)
        return;
    if (pCanAddCurrNode && xIfEndOfAWord(pCurrNode, false)) {
