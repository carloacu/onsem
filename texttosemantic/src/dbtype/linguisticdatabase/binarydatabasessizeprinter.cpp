...
void _printSize(std::list<std::pair<std::size_t, std::string> >& dbsSizePrints,
    ...
    while (itWhereToAdd != dbsSizePrints.end() && itWhereToAdd->first > pSize) {
        ++itWhereToAdd;
    }
    dbsSizePrints.insert(itWhereToAdd, std::pair<std::size_t, std::string>(pSize, ss.str()));
}
...
void _printBinDbSize(std::list<std::pair<std::size_t, std::string> >& dbsSizePrints,
    ...
    _printSize(dbsSizePrints, pTotalSize, pDbName, isLoaded, errorMsg, dbSize);
}

void printSize(std::string& pText, const linguistics::LinguisticDatabase& pLingDb) {
...

    {
        ...
        _printSize(dbsSizePrints, totalSize, "conceptsdatabase\t\t", isLoaded, errorMsg, dbSize);
    }

    {
        ...
        _printSize(dbsSizePrints, totalSize, "conversionsdatabase\t\t", isLoaded, errorMsg, dbSize);
    }
    ...
    for (SemanticLanguageEnum currLang : dbLanguages) {
        ...
        if (currLang != SemanticLanguageEnum::UNKNOWN) {
            for (SemanticLanguageEnum currLang2 : dbLanguages) {
                if (currLang2 != currLang && currLang2 != SemanticLanguageEnum::UNKNOWN) {
    ...
    std::stringstream ss;
    for (std::list<std::pair<std::size_t, std::string> >::iterator itDbsSize = dbsSizePrints.begin();
         itDbsSize != dbsSizePrints.end();
         ++itDbsSize) {
        ss << itDbsSize->second << "\n";
    }
    ss << "\n=================================================\n\n";
