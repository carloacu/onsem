...
bool StaticConceptSet::isUpToDate(const std::string& pFilename, int pCurrentVersion) {
    ...
    return header.intValues[0] == fFormalism && header.intValues[1] == pCurrentVersion;
}
...
void StaticConceptSet::conceptToChildConcepts(std::vector<std::string>& pResConcepts,
...
void StaticConceptSet::getConceptName(std::string& pConceptName, int pConceptId) const {
    assert(xIsLoaded());
...
std::string StaticConceptSet::conceptName(int pConceptId) const {
    assert(xIsLoaded());
...
int StaticConceptSet::getConceptToMeaningId(int pConceptId) const {
    assert(xIsLoaded());
    if (_ptrPatriciaTrie == nullptr || pConceptId == noConcept) {
        return LinguisticMeaning_noMeaningId;
    }
    return binaryloader::alignedDecToInt(*xGetPtrOfConceptToMeaningId(_ptrPatriciaTrie + pConceptId));
}

void StaticConceptSet::getOppositeConcepts(std::set<std::string>& pOppositeConcepts,
...
void StaticConceptSet::conceptsToNearlyEqualConcepts(std::vector<std::string>& pResConcepts,
                                                     const std::string& pConceptName) const {
    assert(xIsLoaded());
    if (_ptrPatriciaTrie == nullptr)
        return;
...
bool StaticConceptSet::areConceptsNearlyEqual(const std::string& pConcept1, const std::string& pConcept2) const {
    assert(xIsLoaded());
    if (_ptrPatriciaTrie == nullptr)
        return false;
...
int StaticConceptSet::getConceptId(const std::string& pConceptName) const {
    assert(xIsLoaded());
...
int StaticConceptSet::xNodePtrToNodeId(const signed char* pNode) const {
...
const int* StaticConceptSet::xGetPtrOfConceptToMeaningId(const signed char* pNode) const {
    return xGetBeginOfEndingStruct(pNode);
}

unsigned char StaticConceptSet::xGetNbOfOppositeConcepts(const signed char* pNode) const {
    return *(reinterpret_cast<const signed char*>(xGetPtrOfConceptToMeaningId(pNode) + 1));
}

unsigned char StaticConceptSet::xGetNbOfNearlyEqualConcepts(const signed char* pNode) const {
    return *(reinterpret_cast<const signed char*>(xGetPtrOfConceptToMeaningId(pNode) + 1) + 1);
}

const int* StaticConceptSet::xGetFirstOppositeConcept(const signed char* pNode) const {
    return xGetPtrOfConceptToMeaningId(pNode) + 2;
}

const int* StaticConceptSet::xGetFirstNearlyEqualConcept(const signed char* pNode) const {
    return xGetPtrOfConceptToMeaningId(pNode) + 2 + xGetNbOfOppositeConcepts(pNode);
}
