...
void _extractConfidenceAtEndOfStr(std::string& pCptWithConfidence,
                                  std::list<char>& pConfidences)
{
  std::size_t lastComma = pCptWithConfidence.find_last_of(',');
  if (lastComma != std::string::npos &&
      lastComma + 1 < pCptWithConfidence.size())
  {
    std::size_t beginNbConfidence = lastComma + 1;
    try
    {
      char newConfidence = static_cast<char>
          (mystd::lexical_cast<int>(pCptWithConfidence.substr
                                    (beginNbConfidence,
                                     pCptWithConfidence.size() - beginNbConfidence)));
...
void SemExpTreeConversionDatabase::xLoadConvFile
 ...
 const std::string& pLocalPath)
{
  ...
  std::map<TreePatternEnumPair,
           std::list<std::pair<std::list<std::string>, ConversionRule> > > cptsToConvs;
  ...
  childLoop(tree, currSubTree, "semantictreeconversions")
  {
          ...
          if (currElt.first != "tree")
            throw std::runtime_error
                ("Only a \"tree\" beacon can be directly inside a \"conversion\" beacon. "
                 "Here we found a \"" + currElt.first + "\" beacon.");
    ...
    {
      ++fCurrConversionNb;
      xLoadAddPossibleTreesBeacon(childTree, questionsPossTreesBothDirectionList,
                                  questionsPossList);
    }
 ...
 std::list<std::pair<std::list<std::string>, UniqueInformationRule>>& pSemanticUniquePatterns) const
{
  ...
  pSemanticUniquePatterns.emplace_back(treePattern.concepts,
                                       std::move(semanticUniquePattern));
}
 ...
 std::list<std::pair<std::list<std::string>, RULE>>& pCptToCvs) const
{
  ...
  if (itMoreFrequent != cptToFreqs.end())
  {
    ...
    {
      auto itMoreFreqCpt =
          std::find(itCptToCvs->first.begin(), itCptToCvs->first.end(), moreFrequentCpt);
      if (itMoreFreqCpt != itCptToCvs->first.end())
      {
        itCptToCvs->first.erase(itMoreFreqCpt);
        moreFreqCptToCvs.splice(moreFreqCptToCvs.begin(),
                                pCptToCvs, itCptToCvs++);
        continue;
      }
      ++itCptToCvs;
    }
    ...
    xFillTreeOfConvs(pTreeOfConvs.children[moreFrequentCpt],
                     moreFreqCptToCvs);
 ...
 ALPatternWorkStruct& pPatternWkStruct) const
{
  ...
  for (std::map<TreePatternConventionEnum, std::list<RootNodeWithRelatedCpts> >::iterator
       itFrom = pPatternWkStruct.begin(); itFrom != pPatternWkStruct.end(); ++itFrom)
  {
    if (itFrom->first != TREEPATTERN_OUTTEXT)
    {
      // iterate through "to step" conversions
      for (std::map<TreePatternConventionEnum, std::list<RootNodeWithRelatedCpts> >::iterator
           itTo = pPatternWkStruct.begin(); itTo != pPatternWkStruct.end(); ++itTo)
      {
        // from and to should be different AND
        // from cannot be "inText"
        if (itFrom->first != itTo->first &&
            itTo->first != TREEPATTERN_INTEXT)
        {
          // iterate through "from" conversion trees
          for (std::list<RootNodeWithRelatedCpts>::iterator
               itPatternFrom = itFrom->second.begin(); itPatternFrom != itFrom->second.end(); ++itPatternFrom)
          {
            // iterate through "to" conversion trees
            for (std::list<RootNodeWithRelatedCpts>::iterator
                 itPatternTo = itTo->second.begin(); itPatternTo != itTo->second.end(); ++itPatternTo)
            {
              if (&*itPatternFrom->rootNode != &*itPatternTo->rootNode)
              {
                pRes[TreePatternEnumPair(itFrom->first, itTo->first)].emplace_back
                    (itPatternFrom->concepts,
                     ConversionRule(fCurrFilename, fCurrConversionNb, 10,
                                       itPatternFrom->rootNode, itPatternTo->rootNode));
              }
            }
          }
        }
 ...
 const boost::property_tree::ptree& pTree) const
{
  ...
  childLoop(pTree, currAttribute, "<xmlattr>")
  {
    TreePatternConventionEnum convEnumVal =
        treePatternConventionEnum_fromStr(currAttribute.first);
...
void SemExpTreeConversionDatabase::xAddTreePattern
...
void SemExpTreeConversionDatabase::xAddSemExp
 ...
 SemExpTreePatternNode* pFatherNode) const
{
  ...
  else
  {
    ...
    {
      throw std::runtime_error
          ("Semantic Expressions tree converter: root beacon badly placed");
    }
    ...
    else if (pBeaconName.size() > 3 &&
             pBeaconName.compare(0, 3, "not") == 0)
    {
  ...
  for (const auto& currElt : pTree)
    if (currElt.first != "<xmlattr>")
      xAddSemExp(pRootNodeWithRelatedCpts, pCurrNode,
                 currElt.first, currElt.second, fatherNode);
}



void SemExpTreeConversionDatabase::xFillNode
(RootNodeWithRelatedCpts& pRootNodeWithRelatedCpts,
 SemExpTreePatternNode& pCurrNode,
 const boost::property_tree::ptree& pTree) const
{
  childLoop(pTree, currAttribute, "<xmlattr>")
  {
    ...
    else if (attrName == "word")
    {
      ...
      try
      {
        ...
        {
          pCurrNode.word = SemanticWord(semanticLanguageEnum_fromStr(strs[2]), strs[0],
              partOfSpeech_fromStr(strs[1]));
        }
        else
        {
...
void SemExpTreeConversionDatabase::xWriteErrorMsg
(const std::string& pMsg) const
{
  std::cerr << "Error in TreeConvertor:\nIn file: " << fCurrFilename << " ("
