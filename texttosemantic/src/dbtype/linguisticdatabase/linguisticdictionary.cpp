...
StaticLinguisticDictionary& LinguisticDictionary::_getStatDbInstance(std::istream* pDictIStreamPtr,
    ...
    if (itElt == _pathToStatDbs.end()) {
        ...
        std::tie(itElt, inserted) = _pathToStatDbs.emplace(
            pLangEnum, std::make_unique<StaticLinguisticDictionary>(pDictIStreamPtr, pStaticConceptSet, pLangEnum));
    }
    return *itElt->second;
}

LinguisticDictionary::InflectedInfos::InflectedInfos(const SemanticWord& pWord,
...
LinguisticDictionary::InflectedInfos::InflectedInfos(const WordAndInfos& pWordAndInfos, const Inflections* pInflections)
...
LinguisticDictionary::InflectedInfos& LinguisticDictionary::InflectedInfos::operator=(InflectedInfos&& pOther) {
    wordAndInfos = std::move(pOther.wordAndInfos);
    inflections = std::move(pOther.inflections);
    return *this;
}
...
LinguisticDictionary::InflectedInfos::InflectedInfos(const InflectedInfos& pOther)
    : wordAndInfos(std::make_unique<WordAndInfos>(&pOther.wordAndInfos->word(), &pOther.wordAndInfos->infos()))
    , inflections(pOther.inflections) {}

LinguisticDictionary::InflectedInfos& LinguisticDictionary::InflectedInfos::operator=(const InflectedInfos& pOther) {
...
LinguisticDictionary::LinguisticDictionary(const LinguisticDictionary& pOther)
    ...
    , _lemmaToPosOfWordToRemoveFromStaticDico(std::make_unique<mystd::radix_map_str<std::list<PartOfSpeech>>>(
          *pOther._lemmaToPosOfWordToRemoveFromStaticDico))
    , _inflectedCharaters(
          std::make_unique<mystd::radix_map_str<std::list<InflectedInfos>>>(*pOther._inflectedCharaters)) {}
...
void LinguisticDictionary::addInflectedWord(const std::string& pInflectedFrom,
...
void LinguisticDictionary::addInfosToAWord(const SemanticWord& pWord, const WordAssociatedInfos* pWordAssociatedInfos) {
...
void LinguisticDictionary::getInfoGram(InflectedWord& pIGram, const LinguisticMeaning& pMeaning) const {
    switch (pMeaning.getLinguisticMeaningType()) {
        ...
        case LinguisticMeaningType::WORD: {
            ...
            {
                auto statLingMeaning = statDb.getLingMeaning(word.lemma, word.partOfSpeech, true);
        ...
        case LinguisticMeaningType::EMPTY: break;
    }
}

void LinguisticDictionary::getContextualInfos(std::set<WordContextualInfos>& pContextualInfos,
...
void LinguisticDictionary::getConceptsFromWord(std::map<std::string, char>& pConcepts,
    ...
    {
        auto statLingMeaning = statDb.getLingMeaning(pWord.lemma, pWord.partOfSpeech, true);
        if (!statLingMeaning.isEmpty())
            statDb.getConcepts(pConcepts, statLingMeaning);
    }
    ...
    const std::list<InflectedInfos>* dynInfosGram = _inflectedCharaters->find_ptr(pWord.lemma, 0, pWord.lemma.size());
    if (dynInfosGram != nullptr) {
        // merge with static infosGram
        for (const auto& currInfleForms : *dynInfosGram) {
...
bool LinguisticDictionary::hasContextualInfo(WordContextualInfos pContextualInfo, const SemanticWord& pWord) const {
    if (_isARemovedWord(pWord))
        return false;

    StaticLinguisticMeaning statLingMeaning = statDb.getLingMeaning(pWord.lemma, pWord.partOfSpeech, true);
    if (!statLingMeaning.isEmpty() && statDb.hasContextualInfo(pContextualInfo, statLingMeaning))
        return true;
...
bool LinguisticDictionary::hasContextualInfo(WordContextualInfos pContextualInfo,
                                             const LinguisticMeaning& pMeaning) const {
    switch (pMeaning.getLinguisticMeaningType()) {
        case LinguisticMeaningType::ID: return statDb.hasContextualInfo(pContextualInfo, pMeaning.getStaticMeaning());
        case LinguisticMeaningType::WORD: {
            const auto& word = pMeaning.getWord();
...
std::size_t LinguisticDictionary::getLengthOfLongestWord(const std::string& pStr, std::size_t pBeginStr) const {
...
void LinguisticDictionary::getGramPossibilitiesAndPutUnknownIfNothingFound(std::list<InflectedWord>& pInfosGram,
    ...
    if (pInfosGram.empty()) {
        ...
        if (newInflWord.word.lemma.size() > 1 && newInflWord.word.lemma[0] == '@') {
...
void LinguisticDictionary::getGramPossibilities(std::list<InflectedWord>& pInfWords,
                                                const std::string& pWord,
                                                std::size_t pBeginPos,
                                                std::size_t pSizeOfWord) const {
    ...
    const std::list<InflectedInfos>* dynInfosGram = _inflectedCharaters->find_ptr(pWord, pBeginPos, pSizeOfWord);
    if (dynInfosGram != nullptr) {
        // merge with static infosGram
        for (const auto& currInfleForms : *dynInfosGram) {
