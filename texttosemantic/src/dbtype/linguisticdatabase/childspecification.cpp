  ...
  for (std::size_t i = pBegin; i < pEnd; i++)
  {
    if (pConditionStr[i] == '(')
    {
      if (linguisticCondition_fromStr(condition, pConditionStr.substr(pBegin, i - pBegin)))
      {
        ...
        {
          std::string allParametersStr = pConditionStr.substr(begOfParametersPos, itEndOfParenthesis - begOfParametersPos);
                                   ...
                                   const std::size_t pEnd)
{
  ...
  {
  case '&':
    operatorEnumOpt = LinguisticConditionTreeOperandEnum::AND;
    break;
  case '|':
    operatorEnumOpt = LinguisticConditionTreeOperandEnum::OR;
    break;
  }
  if (operatorEnumOpt)
  {
