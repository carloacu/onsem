...
std::unique_ptr<GroundedExpression> GroundedExpression::clone(
    ...
    for (const auto& currChild : children) {
        if (pChildrenToSkip != nullptr && pChildrenToSkip->count(currChild.first) > 0)
            continue;
...
std::unique_ptr<SemanticGrounding> GroundedExpression::cloneGrounding(
    const IndexToSubNameToParameterValue* pParams) const {
                    ...
                    for (const auto& currParam : *pParams)
                        if (paramId == currParam.first)
                            for (const auto& currAttrName : currParam.second)
                                if (attributeName == currAttrName.first && currAttrName.second) {
