...
ListExpressionType _getGrdExpPtrs_skipWrapperLists(SEMEXPTYPE& self,
    ...
    switch (self.type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            if (pRecurssiveCallsOnEmptyGrounding && grdExp->isEmpty()) {
                for (auto& currChild : grdExp.children)
                    _getGrdExpPtrs_skipWrapperLists(*currChild.second,
                                                    pRes,
                                                    pFollowInterpretations,
                                                    pRecurssiveCallsOnEmptyGrounding,
                                                    pOnlyMainForm,
                                                    pGrdExpFilderPtr);
        ...
        case SemanticExpressionType::INTERPRETATION: {
            ...
            if (pFollowInterpretations)
                return _getGrdExpPtrs_skipWrapperLists(*intExp.interpretedExp,
                                                       pRes,
                                                       pFollowInterpretations,
                                                       pRecurssiveCallsOnEmptyGrounding,
                                                       pOnlyMainForm,
                                                       pGrdExpFilderPtr);
            return _getGrdExpPtrs_skipWrapperLists(*intExp.originalExp,
                                                   pRes,
                                                   pFollowInterpretations,
                                                   pRecurssiveCallsOnEmptyGrounding,
                                                   pOnlyMainForm,
        ...
        case SemanticExpressionType::FEEDBACK: {
            return _getGrdExpPtrs_skipWrapperLists(*self.getFdkExp().concernedExp,
                                                   pRes,
                                                   pFollowInterpretations,
                                                   pRecurssiveCallsOnEmptyGrounding,
                                                   pOnlyMainForm,
        ...
        case SemanticExpressionType::ANNOTATED: {
            return _getGrdExpPtrs_skipWrapperLists(*self.getAnnExp().semExp,
                                                   pRes,
                                                   pFollowInterpretations,
                                                   pRecurssiveCallsOnEmptyGrounding,
                                                   pOnlyMainForm,
        ...
        case SemanticExpressionType::METADATA: {
            return _getGrdExpPtrs_skipWrapperLists(*self.getMetadataExp().semExp,
                                                   pRes,
                                                   pFollowInterpretations,
                                                   pRecurssiveCallsOnEmptyGrounding,
                                                   pOnlyMainForm,
        ...
        case SemanticExpressionType::COMMAND: {
            return _getGrdExpPtrs_skipWrapperLists(*self.getCmdExp().semExp,
                                                   pRes,
                                                   pFollowInterpretations,
                                                   pRecurssiveCallsOnEmptyGrounding,
                                                   pOnlyMainForm,
        ...
        case SemanticExpressionType::SETOFFORMS: {
            ...
            if (pOnlyMainForm) {
                ...
                if (originalFrom != nullptr)
                    return _getGrdExpPtrs_skipWrapperLists(**originalFrom,
                                                           pRes,
                                                           pFollowInterpretations,
                                                           pRecurssiveCallsOnEmptyGrounding,
                                                           pOnlyMainForm,
                ...
                for (auto& currPrioToFrom : setOfFormsExp.prioToForms)
                    for (auto& currFrom : currPrioToFrom.second)
                        _getGrdExpPtrs_skipWrapperLists(*currFrom->exp,
                                                        pRes,
                                                        pFollowInterpretations,
                                                        pRecurssiveCallsOnEmptyGrounding,
                                                        pOnlyMainForm,
        ...
        case SemanticExpressionType::FIXEDSYNTHESIS: {
            ...
            if (semExp != nullptr)
                return _getGrdExpPtrs_skipWrapperLists(*semExp,
                                                       pRes,
                                                       pFollowInterpretations,
                                                       pRecurssiveCallsOnEmptyGrounding,
                                                       pOnlyMainForm,
        ...
        case SemanticExpressionType::LIST: {
            ...
            for (auto& currElt : listExp.elts)
                _getGrdExpPtrs_skipWrapperLists(*currElt,
                                                pRes,
                                                pFollowInterpretations,
                                                pRecurssiveCallsOnEmptyGrounding,
                                                pOnlyMainForm,
        ...
        case SemanticExpressionType::CONDITION: return ListExpressionType::UNRELATED;
    }
    assert(false);
    return ListExpressionType::UNRELATED;
}

template ListExpressionType _getGrdExpPtrs_skipWrapperLists<SemanticExpression, GroundedExpression>(
    SemanticExpression&,
    std::list<GroundedExpression*>&,
    bool,
    bool,
    bool,
    bool (*pGrdExpFilderPtr)(const GroundedExpression&));
template ListExpressionType _getGrdExpPtrs_skipWrapperLists<const SemanticExpression, const GroundedExpression>(
...
ReturnType getIntExpPtrSkipWrapperPtrs(SemExpType& self) {
    switch (self.type) {
        ...
        case SemanticExpressionType::LIST:
        case SemanticExpressionType::COMPARISON:
        case SemanticExpressionType::GROUNDED: return nullptr;
    }
    assert(false);
    return nullptr;
}
...
template<typename ReturnType, typename SemExpType>
ReturnType getGrdExpPtrSkipWrapperPtrs(SemExpType& self, bool pFollowInterpretations) {
    switch (self.type) {
        case SemanticExpressionType::GROUNDED: {
...
ListExpressionType SemanticExpression::getGrdExpPtrs_SkipWrapperLists(
    std::list<GroundedExpression*>& pRes,
    bool pFollowInterpretations,
    bool pRecurssiveCallsOnEmptyGrounding,
    bool pOnlyMainForm,
    bool (*pGrdExpFilderPtr)(const GroundedExpression&)) {
    return _getGrdExpPtrs_skipWrapperLists(
        *this, pRes, pFollowInterpretations, pRecurssiveCallsOnEmptyGrounding, pOnlyMainForm, pGrdExpFilderPtr);
}

ListExpressionType SemanticExpression::getGrdExpPtrs_SkipWrapperLists(
    std::list<const GroundedExpression*>& pRes,
    bool pFollowInterpretations,
    bool pRecurssiveCallsOnEmptyGrounding,
    bool pOnlyMainForm,
...
std::unique_ptr<SemanticExpression> SemanticExpression::clone(
    ...
    switch (type) {
        case SemanticExpressionType::GROUNDED: {
            ...
            // Fill the parameters
            if (pParams != nullptr && grdExp.grounding().type == SemanticGroundingType::META) {
                const SemanticMetaGrounding* metaGr = dynamic_cast<const SemanticMetaGrounding*>(&grdExp.grounding());
                for (const auto& currParam : *pParams)
                    if (metaGr->paramId == currParam.first)
                        for (const auto& currAttrName : currParam.second)
                            if (metaGr->attibuteName == currAttrName.first && currAttrName.second)
                                return currAttrName.second->getSemExp().clone(
                                    nullptr, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
            }
            return grdExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::LIST: {
            const auto& listExp = getListExp();
            return listExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::CONDITION: {
            const auto& condExp = getCondExp();
            return condExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::COMPARISON: {
        ...
        case SemanticExpressionType::ANNOTATED: {
            ...
            return annExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::METADATA: {
            ...
            return metadataExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::SETOFFORMS: {
            const auto& setOfFromsExp = getSetOfFormsExp();
            return setOfFromsExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::COMMAND: {
            const auto& cmdExp = getCmdExp();
            return cmdExp.clone(pParams, pRemoveRecentContextInterpretations, pExpressionTypesToSkip);
        }
        case SemanticExpressionType::FIXEDSYNTHESIS: {
            const auto& fSynthExp = getFSynthExp();
            if (pExpressionTypesToSkip != nullptr
                && pExpressionTypesToSkip->count(SemanticExpressionType::FIXEDSYNTHESIS) > 0)
                return fSynthExp.getSemExp().clone(
...
bool SemanticExpression::operator==(const SemanticExpression& pOther) const {
    ...
    switch (type) {
        case SemanticExpressionType::GROUNDED:
            return getGrdExp().isEqual(pOther.getGrdExp());    // use isEqual() instead of == to be sure that the child
                                                               // class has implemented the equality comparison
        case SemanticExpressionType::LIST: return getListExp().isEqual(pOther.getListExp());
        case SemanticExpressionType::CONDITION: return getCondExp().isEqual(pOther.getCondExp());
        case SemanticExpressionType::COMPARISON: return getCompExp().isEqual(pOther.getCompExp());
        case SemanticExpressionType::INTERPRETATION: return getIntExp().isEqual(pOther.getIntExp());
        case SemanticExpressionType::FEEDBACK: return getFdkExp().isEqual(pOther.getFdkExp());
        case SemanticExpressionType::ANNOTATED: return getAnnExp().isEqual(pOther.getAnnExp());
        case SemanticExpressionType::METADATA: return getMetadataExp().isEqual(pOther.getMetadataExp());
        case SemanticExpressionType::SETOFFORMS: return getSetOfFormsExp().isEqual(pOther.getSetOfFormsExp());
        case SemanticExpressionType::COMMAND: return getCmdExp().isEqual(pOther.getCmdExp());
        case SemanticExpressionType::FIXEDSYNTHESIS: return getFSynthExp().isEqual(pOther.getFSynthExp());
    }
...
void SemanticExpression::assertEqual(const SemanticExpression& pOther) const {
    ...
    switch (type) {
        ...
        case SemanticExpressionType::CONDITION: return getCondExp().assertEltsEqual(pOther.getCondExp());
        case SemanticExpressionType::COMPARISON: return getCompExp().assertEltsEqual(pOther.getCompExp());
        case SemanticExpressionType::INTERPRETATION: return getIntExp().assertEltsEqual(pOther.getIntExp());
        case SemanticExpressionType::FEEDBACK: return getFdkExp().assertEltsEqual(pOther.getFdkExp());
        case SemanticExpressionType::ANNOTATED: return getAnnExp().assertEltsEqual(pOther.getAnnExp());
        case SemanticExpressionType::METADATA: return getMetadataExp().assertEltsEqual(pOther.getMetadataExp());
        case SemanticExpressionType::SETOFFORMS: return getSetOfFormsExp().assertEltsEqual(pOther.getSetOfFormsExp());
        case SemanticExpressionType::COMMAND: return getCmdExp().assertEltsEqual(pOther.getCmdExp());
        case SemanticExpressionType::FIXEDSYNTHESIS: return getFSynthExp().assertEltsEqual(pOther.getFSynthExp());
    }
    assert(false);
}
...
void SemanticExpression::_assertSemExpOptsEqual(
    const mystd::unique_propagate_const<UniqueSemanticExpression>& pSemExpOpt1,
...
std::unique_ptr<GroundedExpressionContainer> makeGrdExpContainer(UniqueSemanticExpression& pUSemExp) {
