...
static const std::vector<int> number_of_days_per_month_for_not_leap_years = {0,31,28,31,30,31,30,31,31,30,31,30,31};
...
int _weekDayCalculation_GetMonthId(
    SmanticTimeMonth pMonth,
    int pYear)
{
  switch (pMonth)
  {
  case SmanticTimeMonth::JANUARY:
    return _isALeapYear(pYear) ? 6 : 0;
  case SmanticTimeMonth::FEBRUARY:
    return _isALeapYear(pYear) ? 2 : 3;
  case SmanticTimeMonth::MARCH:
    return 3;
  case SmanticTimeMonth::APRIL:
    return 6;
  case SmanticTimeMonth::MAY:
    return 1;
  case SmanticTimeMonth::JUNE:
    return 4;
  case SmanticTimeMonth::JULY:
    return 6;
  case SmanticTimeMonth::AUGUST:
    return 2;
  case SmanticTimeMonth::SEPTEMBER:
    return 5;
  case SmanticTimeMonth::OCTOBER:
    return 0;
  case SmanticTimeMonth::NOVEMBER:
    return 3;
  case SmanticTimeMonth::DECEMBER:
    return 5;
  }
  assert(false);
  return -1;
...
TimeWeekdayEnum SemanticDate::getWeekDay() const
{
  ...
  return TimeWeekdayEnum(((*day) + monthId + yearLastTwoDigits +
                          yearLastTwoDigitsDivByFour + centuryNumber) % 7);
}
...
bool SemanticDate::isTheSameDay(const SemanticDate& pOther) const
{
  return year && month && day &&
      year == pOther.year && month == pOther.month && day == pOther.day;
}
...
void _printNbWithSpecificNbOfDigits(
...
bool SemanticTimeGrounding::operator==
(const SemanticTimeGrounding& pOther) const
{
...
bool SemanticTimeGrounding::isEqual
(const SemanticTimeGrounding &pOther) const
{
...
bool SemanticTimeGrounding::isAfter
(const SemanticTimeGrounding& pOther) const
{
  if (pOther.date < date)
    return true;
...
bool SemanticTimeGrounding::modifyTimeGrdAccordingToADayPart
...
{
  static const std::vector<std::string> dayConcepts =
  {"time_day_morning", "time_day_noon", "time_day_afternoon",
   "time_day_evening", "time_day_midnight", "time_day_night"};
  ...
  for (const auto& currDayConcept : dayConcepts)
  {
    ...
    if (localTimeGrd.modifyTimeGrdAccordingToADayPart(currDayConcept) &&
        localTimeGrd.timeOfDay.isNearlyEqual(timeOfDay) &&
        localTimeGrd.length == length)
    {
                                                       ...
                                                       SemanticTimeGrounding& pRefTime) const
{
  ...
  {
    if (pRefTime.length.isEmpty() &&
        isEqualMoreOrLess10Seconds(pRefTime))
    {
...
void SemanticTimeGrounding::setAnHardCodedTimeElts
(bool pDateAndTimeOfTheDay,
...
void SemanticTimeGrounding::_setToNow(
    SemanticDate& pDate,
    SemanticDuration& pTimeDuration)
{
  ...
  if (hardCodedLessThanASecondNextValue)
  {
    if (*hardCodedLessThanASecondNextValue != 0)
      pTimeDuration.timeInfos[SemanticTimeUnity::LESS_THAN_A_MILLISECOND] =
          *hardCodedLessThanASecondNextValue;
  }
  else
  {
    std::unique_lock<std::mutex> lock(_lastGivenTimeMutex);
    if (_lastGivenTime &&
        pTimeDuration == *_lastGivenTime)
    {
      pTimeDuration.timeInfos[SemanticTimeUnity::LESS_THAN_A_MILLISECOND] =
          ++lessThanASecondNextValue;
    }
    else
    {
