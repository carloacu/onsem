...
bool SemanticLength::operator==(const SemanticLength& pOther) const {
    return lengthInfos == pOther.lengthInfos;
}

void SemanticLength::printLength(std::list<std::string>& pListElts, const std::string& pLabelName) const {
...
void SemanticLength::convertToUnity(SemanticLengthUnity pUnity) {
    ...
    for (auto it = lengthInfos.begin(); it != lengthInfos.end();) {
        if (it->first != pUnity) {
            nbInGoodUnity +=
                it->second * tenPow(semanticLengthUnity_untityConvertionTenPowerCoefficient(it->first, pUnity));
