...
bool SemanticDuration::operator<(const SemanticDuration& pOther) const {
...
bool SemanticDuration::operator==(const SemanticDuration& pOther) const {
    return sign == pOther.sign && timeInfos == pOther.timeInfos;
}
...
SemanticDuration SemanticDuration::fromRadixMapStr(const std::string& pRadixMapStr) {
    ...
    while (i < pRadixMapStr_size) {
        SemanticTimeUnity timeUnity = isDirPositive ? semanticTimeUnity_fromUnorderredChar(pRadixMapStr[i++])
...
bool SemanticDuration::isNearlyEqual(const SemanticDuration& pOther) const {
    return _isNearlyEqualOneSideCheck(pOther) && pOther._isNearlyEqualOneSideCheck(*this);
}
...
bool SemanticDuration::_isNearlyEqualOneSideCheck(const SemanticDuration& pOther) const {
    ...
    for (const auto& currTimeInfo : timeInfos) {
        if (currTimeInfo.first == SemanticTimeUnity::LESS_THAN_A_MILLISECOND)
            break;
        if (itOtherTimeInfo == pOther.timeInfos.end() || currTimeInfo.second != itOtherTimeInfo->second)
            return false;
        ++itOtherTimeInfo;
    }
    return true;
...
void SemanticDuration::add(SemanticTimeUnity pTimeUnity, const SemanticFloat& pNbToAdd) {
    timeInfos[pTimeUnity] += pNbToAdd;
    _normalize();
}

SemanticDuration SemanticDuration::_addition(const SemanticDuration& pOther, int pCoefOfOther) const {
...
void SemanticDuration::printDuration(std::list<std::string>& pListElts, const std::string& pDurationLabelName) const {
    if (timeInfos.empty())
        return;
    std::stringstream ss;
...
bool SemanticDuration::_invertDirectionIfNecessary() {
    ...
    if (itTimeInfo != timeInfos.end() && itTimeInfo->second < 0)
