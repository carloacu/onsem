...
SemanticGenericGrounding::SemanticGenericGrounding(SemanticReferenceType pReferenceType, SemanticEntityType pAgentType)
...
bool SemanticGenericGrounding::isEqual(const SemanticGenericGrounding& pOther) const {
    return _isMotherClassEqual(pOther) && referenceType == pOther.referenceType && coreference == pOther.coreference
        && entityType == pOther.entityType && quantity == pOther.quantity && word == pOther.word
...
std::unique_ptr<SemanticGenericGrounding> SemanticGenericGrounding::makeThingThatHasToBeCompletedFromContext() {
    auto res = std::make_unique<SemanticGenericGrounding>(SemanticReferenceType::DEFINITE, SemanticEntityType::THING);
