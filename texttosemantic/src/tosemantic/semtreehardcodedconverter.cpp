...
void _putConditionAtRoot(std::map<GrammaticalType, std::unique_ptr<ConditionExpression>>& pChildTypeToCondition,
...
bool _applySemExpModifier(std::unique_ptr<SemanticExpression>& pSemExp,
    ...
    if (itObjectChild != pGrdExpRoot.children.end()) {
        ...
        if (grdExpObject != nullptr) {
            ...
            if (statementGrdObject != nullptr && !statementGrdObject->coreference
        ...
        if (listExpObject != nullptr) {
            ...
            for (auto& currElt : listExpObject->elts) {
                ...
                for (const auto& currChild : pGrdExpRoot.children)
                    SemExpModifier::addChild(*eltGrdExp, currChild.first, currChild.second->clone());
            }
        ...
        if (subjectGrdExpPtr != nullptr) {
            if (pGrdExp.children.count(GrammaticalType::MANNER) == 0) {
                if (pGrdExp.children.size() > 1)
                    return;
                if (pStatementGrd.requests.empty() || pStatementGrd.requests.has(SemanticRequestType::YESORNO)) {
                    pGrdExp.children.emplace(GrammaticalType::MANNER, std::make_unique<GroundedExpression>([] {
                                                 auto res = std::make_unique<SemanticGenericGrounding>();
                                                 res->word = SemanticWord(
                                                     SemanticLanguageEnum::FRENCH, "bien", PartOfSpeech::ADVERB);
            ...
            if (subjectGrdExpPtr->grounding().type == SemanticGroundingType::GENERIC) {
                if (pStatementGrd.requests.empty())
                    ...
                    subjectGrdExpPtr->moveGrounding(
                        std::make_unique<SemanticAgentGrounding>(pTextProcContext.receiver));
            }
        }
    }
}
            ...
            if (ConceptSet::haveAConceptThatBeginWith(statGrd.concepts, "comparison_")) {
                ...
                if (itSubjectChild != pGrdExp->children.end()) {
                    ...
                    if (itObjectChild != pGrdExp->children.end()) {
                        auto compExp =
                            std::make_unique<ComparisonExpression>(compOperator, std::move(itSubjectChild->second));
                ...
                if (itObjectChild != pGrdExp->children.end()) {
                    ...
                    if (objectGrdExpPtr != nullptr) {
                        SemanticGenericGrounding* genGrdObjectPtr =
                            objectGrdExpPtr->grounding().getGenericGroundingPtr();
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (statementGrdPtr != nullptr) {
            ...
            if (statementGrd.verbTense == SemanticVerbTense::PUNCTUALPRESENT
                 ...
                 *      <subj>
                 */
                _applySemExpModifier(pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addFutureVerbTense, false);
                return;
            }

            if (statementGrd.concepts.count("verb_have") != 0) {
                _applySemExpModifier(
                    pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addMandatoryVerbGoal, false);
                return;
            }
        }
    }
    ...
    if (grdExpPtr != nullptr) {
        ...
        if (statementGrdPtr != nullptr) {
            ...
            if (statementGrd.verbTense == SemanticVerbTense::PUNCTUALPRESENT
                 ...
                 *      <subj>
                 */
                if (!_applySemExpModifier(
                        pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addFutureVerbTense, false)) {
            ...
            if (statementGrd.concepts.count("verb_haveto") != 0) {
                const bool isFalloirVerb =
                    statementGrd.word == SemanticWord(SemanticLanguageEnum::FRENCH, "falloir", PartOfSpeech::VERB);
                bool modifDone = false;
                if (statementGrd.verbGoal == VerbGoalEnum::CONDITIONAL)
                    modifDone = _applySemExpModifier(
                        pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addAdviceVerbGoal, false);
                else
                    modifDone = _applySemExpModifier(
                        pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addMandatoryVerbGoal, false);
                if (isFalloirVerb && modifDone) {
                    ...
                    for (auto* newGrdExpPtr : gGrdExps) {
                        ...
                        if (itSubject != newGrdExpPtr->children.end()
                            ...
                            itSubject->second = std::make_unique<GroundedExpression>(
                                std::make_unique<SemanticConceptualGrounding>("generic"));
                    }
                }
                return;
            }
            ...
            if (ConceptSet::haveAnyOfConcepts(statementGrd.concepts, {"verb_can", "verb_able"})) {
                _applySemExpModifier(
                    pSemExp, pChildTypeToCondition, grdExp, statementGrd, &_addAbilityVerbGoal, pIsPassive);
                return;
            }
        }
    }
