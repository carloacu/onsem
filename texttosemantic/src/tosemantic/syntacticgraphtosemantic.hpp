...
protected:
    struct ToGenRepContext {
        ...
        ToGenRepContext(const ToGenRepContext& pOther, const ChunkLink& pChunkLink, Chunk& pChunk)
            : chLink(pChunkLink)
            , chunk(pChunk)
    ...
    struct ToGenRepGeneral {
        ToGenRepGeneral(mystd::unique_propagate_const<UniqueSemanticExpression>& pUSemExp,
                        const TextProcessingContext& pTextProcContext,
...
private:
    ...
    UniqueSemanticExpression xAddSemExpWithMainSentimentalWord(const TokensTree& pTokensTree) const;

    void xAddModifiers(std::unique_ptr<GroundedExpression>& pGrdExpPtr,
                       ToGenRepContext& pContext,
                       const Chunk& pChunk,
                       bool pLookAtTokenAfterTheHead) const;

    void xAddModifiersOfATokenAfterVerb(GroundedExpression& pGrdExp,
                                        ToGenRepContext& pContext,
                                        TokIt pItToken,
                                        const TokIt& pItEndToken) const;

    void xFillPossibleGenders(std::set<SemanticGenderType>& pPossibleGenders,
                              const InflectedWord& pInflWord,
                              SemanticNumberType pNumberOfInflWord) const;

    void xInitGenGroundingsFromToken(SemanticGenericGrounding& pGenGroundings,
                                     TokIt pToken,
                                     const TokIt& pItEndToken) const;

    void xConvertVerbChunkToRootGrounding(SemanticStatementGrounding& pRootGrounding, const Chunk& pChunk) const;
    ...
    mystd::unique_propagate_const<UniqueSemanticExpression> xConvertToTimeGrounding(
        ToGenRepContext& pContext,
        ToGenRepGeneral& pGeneral,
        const Chunk& pChunk,
    ...
    SemanticVerbTense xChunkToVerbTimeTense(VerbGoalEnum& pVerbGoal,
                                            const Chunk& pVerbChunk,
                                            ChunkLinkType pChunkLinkType) const;

    SemanticVerbTense xChunkToVerbTimeTenseForFrench(VerbGoalEnum& pVerbGoal,
                                                     const Chunk& pVerbChunk,
                                                     ChunkLinkType pChunkLinkType) const;

    SemanticVerbTense xChunkToVerbTimeTenseForEnglish(VerbGoalEnum& pVerbGoal,
                                                      const Chunk& pVerbChunk,
                                                      ChunkLinkType pChunkLinkType) const;

    static bool xVerbAuxiliaryIndicateFutureForEnglish(const Chunk& pVerbChunk);

    std::unique_ptr<GroundedExpression> xInitNewSentence(ToGenRepGeneral& pGeneral,
                                                         ToGenRepContext& pContext,
                                                         Chunk& pVerbChunk) const;

    void xFillVerbChunk(std::unique_ptr<GroundedExpression>& pGrdExp,
                        ToGenRepGeneral& pGeneral,
                        ToGenRepContext& pContext,
                        Chunk& pVerbChunk) const;

    SemanticRequestType xSubBordinateChkLkToRequest(const ChunkLink& pSubBordinateChkLk) const;

    const TextProcessingContext& xGetTextProcContext(const ToGenRepContext& pContext,
                                                     const ToGenRepGeneral& pGeneral) const;

    std::unique_ptr<SemanticExpression> xTranslateRelativePersToPerson(RelativePerson pRelPers,
                                                                       const InflectedWord& pIGram,
                                                                       const ToGenRepContext& pContext,
                                                                       const ToGenRepGeneral& pGeneral) const;

    UniqueSemanticExpression xConvertInterjectionChunk(ToGenRepGeneral& pGeneral, ToGenRepContext& pContext) const;

    UniqueSemanticExpression xConvertNominalChunk(ToGenRepGeneral& pGeneral, ToGenRepContext& pContext) const;
    ...
    mystd::unique_propagate_const<UniqueSemanticExpression> xConvertSeparatorChunkToSemExp(
        ToGenRepGeneral& pGeneral,
        ToGenRepContext& pContext) const;
    ...
    void xAfterCreationModificationsOnSentence(GroundedExpression& pGrdExpSentence,
                                               const ToGenRepContext& pContext,
                                               const ToGenRepGeneral& pGeneral) const;
    ...
    void xFillCondition(std::map<GrammaticalType, std::unique_ptr<ConditionExpression>>& pChildTypeToCondition,
                        GrammaticalType pCurrGramChild,
                        ToGenRepGeneral& pGeneral,
                        const ChunkLink& pSubClauseChunkLink,
                        const ToGenRepContext& pContext) const;

    void xFillSubordonateClause(UniqueSemanticExpression& pSemExpSentence,
    ...
    void xAddNewGrammInfo(GroundedExpression& pGrdExpSentence,
                          ToGenRepGeneral& pGeneral,
                          ToGenRepContext& pContext,
