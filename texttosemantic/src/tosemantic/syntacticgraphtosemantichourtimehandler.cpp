...
enum class BeforeOrAfter { BEFORE, AFTER, UNKNOWN };
enum class AmOrPm { AM, PM, UNKNOWN };
...
std::unique_ptr<SemanticTimeGrounding> SyntacticGraphToSemantic::_fillHourTimeStructFr(const Chunk& pChunk) const {
    auto itToken = pChunk.tokRange.getItBegin();
    auto endIt = pChunk.tokRange.getItEnd();
    ...
    if (separatorOfHourMinute != std::string::npos) {
        try {
            auto time = std::make_unique<SemanticTimeGrounding>();
            time->timeOfDay.sign = Sign::POSITIVE;
            time->timeOfDay.timeInfos[SemanticTimeUnity::HOUR] =
                mystd::lexical_cast<int>(itToken->str.substr(0, separatorOfHourMinute));
            ...
            if (itToken->str.size() > beginOfMinutePos) {
                ...
                time->timeOfDay.timeInfos[SemanticTimeUnity::MINUTE] =
                    mystd::lexical_cast<int>(itToken->str.substr(beginOfMinutePos, minuteStrSize));
            }
            return time;
        } catch (...) {}
    }
...
std::unique_ptr<SemanticTimeGrounding> SyntacticGraphToSemantic::xFillHourTimeStruct(const Chunk& pChunk) const {
