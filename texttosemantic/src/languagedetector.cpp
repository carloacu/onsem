...
std::size_t _getMaxLength(std::string& pLowerCaseText,
                          ...
                          bool pCanLowerCase) {
    std::size_t res = pBinDico.getLengthOfLongestWord(pLowerCaseText, pCurrPos);
    ...
    return pBinDico.getLengthOfLongestWord(pLowerCaseText, pCurrPos);
}

bool _moveUntilNextSeparator(std::size_t& pCurrPos,
                             const std::string& pLowerCaseText,
                             const StaticLinguisticDictionary& pBinDico) {
    bool directlyASeparator = true;
    while (pCurrPos < pLowerCaseText.size()) {
        const std::size_t longestSep = pBinDico.getLengthOfLongestWord(pLowerCaseText, pCurrPos);
        if (longestSep == 0) {
            ++pCurrPos;
        }
        PartOfSpeech mainPartOfSpeech = PartOfSpeech::UNKNOWN;
        pBinDico.getConfidenceOfWordInThatLanguage(mainPartOfSpeech, pLowerCaseText, pCurrPos, longestSep);
        pCurrPos += longestSep;
...
SemanticLanguageEnum getLanguage(const std::string& pText, const LinguisticDatabase& pLingDb) {
    ...
    for (std::size_t i = 0; i < langToSpec_size; ++i) {
        ...
        while (currPos < lowerCaseText.size()) {
            ...
            std::size_t length = _getMaxLength(lowerCaseText, beginWithUpperCase, currPos, binDico, canLowerCase);
            canLowerCase = false;

            if (length > 0) {
                PartOfSpeech mainPartOfSpeech = PartOfSpeech::UNKNOWN;
                std::size_t nbOfMeanings =
                    binDico.getConfidenceOfWordInThatLanguage(mainPartOfSpeech, lowerCaseText, currPos, length);
                currPos += length;
                ...
                if (beginWithUpperCase) {
                    ...
                    confidenceOfCurrentLanguageForTheText -=
                        static_cast<int>(lengthOfUnknownCharacters
