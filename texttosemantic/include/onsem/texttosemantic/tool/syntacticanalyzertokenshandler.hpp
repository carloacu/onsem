...
ONSEM_TEXTTOSEMANTIC_API bool tokenIsMoreProbablyAType(const Token& pToken, PartOfSpeech pPartOfSpeech);

ONSEM_TEXTTOSEMANTIC_API bool tokenIsMoreProbablyAType(const linguistics::Token& pToken,
                                                       const std::vector<PartOfSpeech>& pPartOfSpeechs);

ONSEM_TEXTTOSEMANTIC_API bool hasAPartOfSpeech(const std::vector<PartOfSpeech>& pPartOfSpeechs,
                                               PartOfSpeech pPartOfSpeech);

ONSEM_TEXTTOSEMANTIC_API bool hasAPartOfSpeech(const std::list<InflectedWord>& pInflWords, PartOfSpeech pPartOfSpeech);
...
ONSEM_TEXTTOSEMANTIC_API bool delPartOfSpeechs(std::list<InflectedWord>& pIGrams,
                                               const std::vector<PartOfSpeech>& pPartOfSpeechToDel);

ONSEM_TEXTTOSEMANTIC_API TokIt getNextWord(TokIt it, const TokIt& endIt);

ONSEM_TEXTTOSEMANTIC_API TokIt getNextToken(TokIt it, const TokIt& endIt, PartOfSpeech pUnwantedGramType);

ONSEM_TEXTTOSEMANTIC_API TokIt getTheNextestToken(TokIt it,
                                                  const TokIt& endIt,
...
ONSEM_TEXTTOSEMANTIC_API bool delAllExept(std::list<InflectedWord>& pGrams,
                                          std::list<InflectedWord>::iterator pIt,
                                          PartOfSpeech pPartOfSpeech);

ONSEM_TEXTTOSEMANTIC_API bool delAllExept(std::list<InflectedWord>& pGrams, PartOfSpeech pPartOfSpeech);

ONSEM_TEXTTOSEMANTIC_API void putOnTop(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt);

ONSEM_TEXTTOSEMANTIC_API bool putOnTop(std::list<InflectedWord>& pGrams, PartOfSpeech pPartOfSpeech);

ONSEM_TEXTTOSEMANTIC_API void putOnBottom(std::list<InflectedWord>& pGrams, std::list<InflectedWord>::iterator pIt);
...
ONSEM_TEXTTOSEMANTIC_API TOKITTEMP eatNumber(mystd::optional<SemanticFloat>& pNumber,
                                             TOKITTEMP pToken,
                                             const TOKITTEMP& pItEndToken,
                                             const std::string& pBeginOfNumberCpt);

template<typename TOKITTEMP>
ONSEM_TEXTTOSEMANTIC_API bool getNumberHoldByTheInflWord(SemanticFloat& number,
                                                         TOKITTEMP pToken,
                                                         const TOKITTEMP& pItEndToken,
                                                         const std::string& pNumberCpt);

template<typename TOKITTEMP, typename TOKENRANGE>
ONSEM_TEXTTOSEMANTIC_API mystd::optional<SemanticDate> extractDate(const TOKITTEMP& pTokenIt,
...
T getPrevToken(T pIt, const T& pBeginIt, const T& pEndIt, SkipPartOfWord pSkipPartOfWord = SkipPartOfWord::NO) {
    ...
    while (true) {
        ...
        if (currToken.inflWords.front().word.partOfSpeech >= PartOfSpeech::LINKBETWEENWORDS
            && (pSkipPartOfWord == SkipPartOfWord::NO
