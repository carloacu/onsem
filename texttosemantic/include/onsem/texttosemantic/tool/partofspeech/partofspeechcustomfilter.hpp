...
private:
    ...
    bool xTryToLinkAToken(std::list<std::pair<TokIt, std::list<InflectedWord>::iterator> >& pLinkedTokens,
    ...
    std::vector<Token>::iterator xSearchWordAfter(std::vector<Token>& pTokens,
    ...
    std::vector<Token>::iterator xSearchWordBefore(std::vector<Token>& pTokens,
                                                   std::list<InflectedWord>::iterator& pOnlyItIGramToKeep,
                                                   std::vector<Token>::iterator pCurrTok,
                                                   const SemanticWord& pWord,
                                                   const InflectedWord& pMainIGram) const;

    bool xCanBeLinked(const InflectedWord& pMainIGram, const InflectedWord& pSubIGram) const;

    void xTestOfPuttingNounAtBottomOfListIfOccurTwoTimesInARow(std::vector<Token>& pTokens, bool pExceptForVerb) const;

    void xPutNounAtBottomIfNecessary(std::list<InflectedWord>& pInflWord, bool pExceptForVerb) const;
};
