...
enum class SemanticExpressionType : char { SEMANTIC_EXPRESSTION_TYPE_TABLE };
#undef SEMANTIC_EXPRESSION_TYPE

#define SEMANTIC_EXPRESSION_TYPE(a, b) b,
static const std::vector<std::string> _semanticExpressionType_toStr = {SEMANTIC_EXPRESSTION_TYPE_TABLE};
#undef SEMANTIC_EXPRESSION_TYPE
...
static inline std::string semanticExpressionType_toStr(SemanticExpressionType pSemExpType) {
    return _semanticExpressionType_toStr[semanticExpressionType_toChar(pSemExpType)];
}

static inline SemanticExpressionType semanticExpressionType_fromStr(const std::string& pSemExpTypeStr) {
...
struct ONSEM_TEXTTOSEMANTIC_API SemanticExpression {
    ...
    ListExpressionType getGrdExpPtrs_SkipWrapperLists(
    ...
    std::unique_ptr<SemanticExpression> clone(
...
struct UniqueSemanticExpression : public SemanticExpressionContainer {
    ...
    ONSEM_TEXTTOSEMANTIC_API static UniqueSemanticExpression& wrapInContainer(UniqueSemanticExpression& pUSemExp) {
        return pUSemExp;
    }
    ONSEM_TEXTTOSEMANTIC_API static const UniqueSemanticExpression& wrapInContainer(
        const UniqueSemanticExpression& pUSemExp) {
        return pUSemExp;
    }

    ONSEM_TEXTTOSEMANTIC_API std::shared_ptr<SemanticExpression> getSharedPtr() { return _semanticExpression; }
