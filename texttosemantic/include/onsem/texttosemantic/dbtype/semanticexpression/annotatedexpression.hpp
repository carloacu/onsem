...
{
  return synthesizeAnnotations == pOther.synthesizeAnnotations &&
      annotations == pOther.annotations &&
 ...
 const std::set<SemanticExpressionType>* pExpressionTypesToSkip) const
{
  ...
  for (const auto& currAnn : annotations)
    res->annotations.emplace(currAnn.first, currAnn.second->clone(pParams,
                                                                  pRemoveRecentContextInterpretations,
