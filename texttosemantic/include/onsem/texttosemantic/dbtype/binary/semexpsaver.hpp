...
public:
    ...
    void addSemExp(const SemanticExpression& pSemExp, unsigned char* pPtr);
    void clearSemExps();
    uint32_t grdExpToOffset(const GroundedExpression& pGrdExp, unsigned char* pPtr) const;
    uint32_t semExpToOffset(const SemanticExpression& pSemExp, unsigned char* pPtr) const;
