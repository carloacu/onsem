...
struct ONSEM_TEXTTOSEMANTIC_API SpecificLinguisticDatabase {
    SpecificLinguisticDatabase(KeyToStreams& pIStreams, LinguisticDatabase& pLingDb, SemanticLanguageEnum pLanguage);
    ...
    void addInfosToAWord(const SemanticWord& pWord, const WordAssociatedInfos& pWordInfos);
    ...
    const std::list<std::unique_ptr<PartOfSpeechContextFilter>>& getContextFilters() const { return _contextFilters; }
...
private:
    ...
    void _addDynamicConceptInfo(const boost::property_tree::ptree& pTree, const std::string& pConceptName);
    ...
    void _addConcept(const boost::property_tree::ptree& pTree, SpecificLinguisticDatabase& pSpecLingDb);
};
