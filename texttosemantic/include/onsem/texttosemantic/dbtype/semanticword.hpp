...
struct ONSEM_TEXTTOSEMANTIC_API SemanticWord {
    ...
    SemanticWord(SemanticLanguageEnum pLanguage, const std::string& pLemma, PartOfSpeech pPartOfSpeech)
        : language(pLanguage)
        , lemma(pLemma)
        , partOfSpeech(pPartOfSpeech) {}

    void setContent(SemanticLanguageEnum pLanguage, const std::string& pLemma, PartOfSpeech pPartOfSpeech) {
        language = pLanguage;
        lemma = pLemma;
        partOfSpeech = pPartOfSpeech;
    }
    ...
    bool isEmpty() const {
        return language == SemanticLanguageEnum::UNKNOWN && lemma.empty() && partOfSpeech == PartOfSpeech::UNKNOWN;
    }

    bool isReflexive() const {
        return language == SemanticLanguageEnum::FRENCH && partOfSpeech == PartOfSpeech::VERB && lemma.size() > 3
...
inline bool SemanticWord::operator<(const SemanticWord& pOther) const {
...
inline bool SemanticWord::operator==(const SemanticWord& pOther) const {
    return lemma == pOther.lemma && partOfSpeech == pOther.partOfSpeech && language == pOther.language;
}
