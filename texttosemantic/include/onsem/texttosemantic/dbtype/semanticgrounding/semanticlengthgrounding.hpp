...
enum class SemanticLengthUnity : char { SEMANTIC_LENGTH_UNITY_TABLE };
#undef SEMANTIC_LENGTH_UNITY

#define SEMANTIC_LENGTH_UNITY(a, b, c, d) b,
static const std::vector<std::string> _semanticLengthUnity_toAbreviation = {SEMANTIC_LENGTH_UNITY_TABLE};
#undef SEMANTIC_LENGTH_UNITY

#define SEMANTIC_LENGTH_UNITY(a, b, c, d) c,
static const std::vector<std::string> _semanticLengthUnity_toStr = {SEMANTIC_LENGTH_UNITY_TABLE};
#undef SEMANTIC_LENGTH_UNITY
...
static const std::map<std::string, SemanticLengthUnity> _semanticLengthUnity_fromStr = {SEMANTIC_LENGTH_UNITY_TABLE};
#undef SEMANTIC_LENGTH_UNITY

#define SEMANTIC_LENGTH_UNITY(a, b, c, d) d,
static const std::vector<int64_t> _semanticLengthUnity_toNbOfMillimetersPowerTen = {SEMANTIC_LENGTH_UNITY_TABLE};
#undef SEMANTIC_LENGTH_UNITY
...
static const std::vector<SemanticLengthUnity> semanticLengthUnities = {SEMANTIC_LENGTH_UNITY_TABLE};
#undef SEMANTIC_LENGTH_UNITY
...
static inline SemanticLengthUnity semanticLengthUnity_fromStr(const std::string& pLengthUnityStr) {
...
static inline SemanticLengthUnity semanticLengthUnity_fromAbreviation(const std::string& pLengthUnityStr) {
...
static inline signed char semanticLengthUnity_untityConvertionTenPowerCoefficient(SemanticLengthUnity pLengthUnity1,
...
struct ONSEM_TEXTTOSEMANTIC_API SemanticLength {
    ...
    void printLength(std::list<std::string>& pListElts, const std::string& pLabelName) const;
...
inline bool SemanticLengthGrounding::isEqual(const SemanticLengthGrounding& pOther) const {
    return _isMotherClassEqual(pOther) && length == pOther.length;
}
