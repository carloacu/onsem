...
struct ONSEM_TEXTTOSEMANTIC_API NameInfos {
    ...
    bool operator==(const NameInfos& pOther) const {
        return names == pOther.names && possibleGenders == pOther.possibleGenders;
    }
...
struct ONSEM_TEXTTOSEMANTIC_API SemanticNameGrounding : public SemanticGrounding {
    ...
    SemanticNameGrounding(const std::string& pName, const std::string& pUserId)
        : SemanticGrounding(SemanticGroundingType::NAME)
        , nameInfos(pName) {
    ...
    static std::string namesToStrFromBinary(unsigned char pNbOfStrings, const unsigned char*& pUChars);
...
inline bool SemanticNameGrounding::isEqual(const SemanticNameGrounding& pOther) const {
    return _isMotherClassEqual(pOther) && nameInfos == pOther.nameInfos;
}
