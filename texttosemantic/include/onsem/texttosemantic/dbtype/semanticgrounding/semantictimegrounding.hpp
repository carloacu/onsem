...
struct ONSEM_TEXTTOSEMANTIC_API SemanticTimeGrounding : public SemanticGrounding {
    ...
    bool tryToConvertToADayConcept(std::string& pRelativedayConcept, SemanticTimeGrounding& pRefToday) const;
    bool tryToConvertToATimeConcept(std::string& pRelativedayConcept, SemanticTimeGrounding& pRefTime) const;
    ...
    static void setAnHardCodedTimeElts(bool pDateAndTimeOfTheDay, bool pLessThanASecond);

private:
    static std::mutex _lastGivenTimeMutex;
    static std::unique_ptr<SemanticDuration> _lastGivenTime;

    static void _setToNow(SemanticDate& pDate, SemanticDuration& pTimeDuration);
};
