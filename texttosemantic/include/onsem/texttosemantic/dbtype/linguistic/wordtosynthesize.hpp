...
struct ONSEM_TEXTTOSEMANTIC_API InflectionToSynthesize {
    ...
    bool operator==(const InflectionToSynthesize& pOther) const {
        return str == pOther.str && ifCanHaveSpaceBefore == pOther.ifCanHaveSpaceBefore
            && ifCanHaveSpaceAfter == pOther.ifCanHaveSpaceAfter && canHavePunctionAfter == pOther.canHavePunctionAfter
            && fromResourcePtr == pOther.fromResourcePtr;
    }
    bool operator!=(const InflectionToSynthesize& pOther) const { return !operator==(pOther); }
...
enum class WordToSynthesizeTag { ANY, DATE };
...
struct ONSEM_TEXTTOSEMANTIC_API WordToSynthesize {
    ...
    bool operator==(const WordToSynthesize& pOther) const {
        return word == pOther.word && inflections == pOther.inflections;
    }
    bool operator!=(const WordToSynthesize& pOther) const { return !operator==(pOther); }
