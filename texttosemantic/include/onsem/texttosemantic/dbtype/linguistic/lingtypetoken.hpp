...
enum class TokenLinkage { STANDALONE, HEAD_OF_WORD_GROUP, PART_OF_WORD_GROUP };

enum class SkipPartOfWord { YES, NO };

struct ONSEM_TEXTTOSEMANTIC_API TokenPos {
    void set(const TokenPos& pOther, std::size_t pPos) {
        tokenIndexes.reserve(pOther.tokenIndexes.size() + 1);
        tokenIndexes.insert(tokenIndexes.begin(), pOther.tokenIndexes.begin(), pOther.tokenIndexes.end());
    ...
    void fromStr(const std::string& pStr) {
        ...
        for (const auto& currSep : posOfSeparators) {
            tokenIndexes.emplace_back(
                mystd::lexical_cast_unigned<std::size_t>(pStr.substr(currPos, currSep - currPos)));
            currPos = currSep + 1;
        }
    }
    ...
    bool isAtBegin() const { return tokenIndexes.size() == 1 && tokenIndexes[0] == 0; }
...
struct ONSEM_TEXTTOSEMANTIC_API Token {
     ...
     */
    Token(const std::string& pStr, const std::list<InflectedWord>& pInfosGram = std::list<InflectedWord>());
    ~Token();
...
struct TokenRangeTemplate {
    ...
    TokenRangeTemplate(TOKENSVECTOR& pTokList, const TOKENIT pItBegin, const TOKENIT pItEnd);
...
private:
    ...
    void xReplaceNewLineBySpace(std::string& pResult, const std::string& pInputString) const;
};
...
struct TokenIteratorTemplate {
    TokenIteratorTemplate(TOKENSVECTOR& pTokenList, std::size_t pOffset)
        : _tokenRange(pTokenList)
        , _itToken(pTokenList.begin())
        , _subIterator()
        , _offset(pOffset) {}

    TokenIteratorTemplate(TokenRangeTemplate<TOKENSVECTOR, TOKENIT>& pTokenList, std::size_t pOffset)
        : _tokenRange(pTokenList)
        ...
        for (auto res = ++currToken; res != endIt; ++res) {
            if (res->inflWords.begin()->word.partOfSpeech >= PartOfSpeech::LINKBETWEENWORDS
                && (pSkipPartOfWord == SkipPartOfWord::NO
                    || res->getTokenLinkage() != TokenLinkage::PART_OF_WORD_GROUP)) {
                _itToken = res;
                return;
            }
        }
...
bool tokListHaveSameInflectedFormThan(const std::vector<Token>& pTokVect1, const std::vector<Token>& pTokVect2);
