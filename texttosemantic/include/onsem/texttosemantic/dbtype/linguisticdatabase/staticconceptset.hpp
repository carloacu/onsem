...
public:
     ...
     */
    static bool isUpToDate(const std::string& pFilename, int pCurrentVersion);

    void getConceptName(std::string& pConceptName, int pConceptId) const;
    ...
    void conceptsToNearlyEqualConcepts(std::vector<std::string>& pResConcepts, const std::string& pConceptName) const;

    bool areConceptsNearlyEqual(const std::string& pConcept1, const std::string& pConcept2) const;
...
private:
     ...
     */
    StaticConceptSet& operator=(const StaticConceptSet&) { return *this; }
