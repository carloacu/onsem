...
enum class SearchForLongestWordMode { DISABLED, ENBABLED_BUTWITHSEPARATORAFTER, ENABLED };
...
protected:
    ...
    static bool isASeparator(const std::string& pString, std::size_t pPos);
     ...
     */
    const signed char* xSearchInPatriciaTrie(std::size_t& pLongestWord,
    ...
    void xGetWordsThatBeginWith(std::list<const signed char*>& pResWords, const std::string& pBeginOfWords) const;

    void xGetWord(std::string& pWord, int pWordNode) const;
     ...
     */
    const signed char* xGetNode(const std::string& pWord,
    ...
    const int* xGetFather(const signed char* pNode) const;

    const signed char* xAlignedDecToPtr(signed char* pBeginMem, int pAlignedDec) const;
...
private:
    MetaWordTreeDb(const MetaWordTreeDb& pOther);

    MetaWordTreeDb& operator=(const MetaWordTreeDb& pOther);

    void xGetWordsFromANodeOfTheTree(std::list<const signed char*>& pResWords,
