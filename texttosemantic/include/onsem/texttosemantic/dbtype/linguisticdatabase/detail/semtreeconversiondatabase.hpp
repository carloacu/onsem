...
protected:
    ...
    struct ConversionRule {
        ConversionRule(const std::string& pFilename,
                       int pConvesionNb,
    ...
    struct TreePatternEnumPair {
        TreePatternEnumPair(TreePatternConventionEnum pEnumIn, TreePatternConventionEnum pEnumOut)
            : enumIn(pEnumIn)
            , enumOut(pEnumOut) {}

        bool operator<(const TreePatternEnumPair& pEnumPair) const {
    ...
    typedef std::map<TreePatternConventionEnum, std::list<RootNodeWithRelatedCpts>> PatternWorkStruct;
    ...
    virtual bool xIsLoaded() const { return isLoaded; }

    void xWriteErrorMsg(const std::string& pMsg) const;
    ...
    void xAddSemExp(RootNodeWithRelatedCpts& pRootNodeWithRelatedCpts,
                    SemExpTreePatternNode*& pCurrNode,
                    const std::string& pBeaconName,
                    const boost::property_tree::ptree& pTree,
                    SemExpTreePatternNode* pFatherNode) const;

    void xFillNode(RootNodeWithRelatedCpts& pRootNodeWithRelatedCpts,
                   SemExpTreePatternNode& pCurrNode,
                   const boost::property_tree::ptree& pTree) const;

    template<typename RULE>
    void xFillTreeOfConvs(ConceptTreeOfRules<RULE>& pTreeOfConvs,
                          std::list<std::pair<std::list<std::string>, RULE>>& pCptToCvs) const;

    void xAddConversionTreePattern(PatternWorkStruct& pPatternWkStruct, const boost::property_tree::ptree& pTree) const;

    void xAddTreePattern(RootNodeWithRelatedCpts& pPatternToFill, const boost::property_tree::ptree& pTree) const;
    ...
    void xLoadConvFile(SemanticLanguageEnum pLangEnum, std::istream& pIStream, const std::string& pLocalPath);

    template<typename T1, typename T2>
    bool xIsASubSetOnMapLabel(const T1& pConceptsSubSet, const T2& pAllConcepts) const;
...
template<typename T1, typename T2>
bool SemExpTreeConversionDatabase::xIsASubSetOnMapLabel(const T1& pConceptsSubSet, const T2& pAllConcepts) const {
    for (typename T1::const_iterator itConvCpt = pConceptsSubSet.begin(); itConvCpt != pConceptsSubSet.end();
         ++itConvCpt) {
