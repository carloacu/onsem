...
public:
    ...
    void getLemma(std::string& pLemmaStr, const std::string& pWord) const;

    std::string getLemma(const StaticLinguisticMeaning& pMeaning, bool pWithLinkMeanings) const;

    void getWord(SemanticWord& pWord, const StaticLinguisticMeaning& pMeaning) const;

    void getSemanticWord(SemanticWord& pWord, int pMeaningId) const;

    StaticLinguisticMeaning getRootMeaning(const StaticLinguisticMeaning& pMeaning) const;

    void getInfoGram(InflectedWord& pIGram, const StaticLinguisticMeaning& pMeaning) const;

    void getContextualInfos(std::set<WordContextualInfos>& pContextualInfos,
                            const StaticLinguisticMeaning& pMeaning) const;

    void getConcepts(std::map<std::string, char>& pConcepts, const StaticLinguisticMeaning& pMeaning) const;
    ...
    StaticLinguisticMeaning getLingMeaning(const std::string& pWord, PartOfSpeech pGram, bool pWordIsALemma) const;

    bool hasContextualInfo(WordContextualInfos pContextualInfo, const StaticLinguisticMeaning& pMeaning) const;

    bool isAGatheringMeaning(const StaticLinguisticMeaning& pMeaning) const;

    void putRootMeaning(InflectedWord& pInfosGram) const;

    bool getConjugaisionsId(int& pConjId, const StaticLinguisticMeaning& pMeaning) const;

    void getWord(std::string& pWord, int pWordNode) const;

    struct QuestionWords {
        QuestionWords(WordToVerbRelation pCanBeBeforeVerb,
    ...
    const QuestionWords* wordToQuestionWord(const SemanticWord& pWord, bool pAfterVerb, bool pIsCloseToTheVerb) const;
...
protected:
    virtual bool xIfEndOfAWord(const signed char* pNode, bool pOnlyWordWithWordFroms) const;
...
private:
    ...
    void xGetSemanticWord(SemanticWord& pWord, int32_t pMeaningId) const;

    void xFillIGram(InflectedWord& pIGram, int32_t pMeaningId) const;

    void xFillWordAssInfos(WordAssociatedInfos& pInfos, int pMeaningId) const;

    void xTryToFillMeaningGroup(mystd::optional<LingWordsGroup>& pLinkedMeanings, int pMeaningId) const;

    void xGetMetaMeanings(std::list<LingWordsGroup>& pMetaMeanings, const signed char* pMeaningPtr) const;
     ...
     */
    StaticLinguisticDictionary& operator=(const StaticLinguisticDictionary& pDB);

    std::size_t xGetConfOfNode(PartOfSpeech& pMainPartOfSpeech, const signed char* pNode) const;

    void xGetGramPossFromNode(std::list<InflectedWord>& pInfosGram,
                              const signed char* pNode,
                              bool pBeginsWithUpperCase) const;

    std::string xGetLemma(int32_t pMeaningId, bool pWithLinkMeanings) const;

    void xGetWordContextualInfos(std::set<WordContextualInfos>& pContextualInfos, const signed char* pMeaningPtr) const;

    bool xWordHasContextualInfos(WordContextualInfos pContextualInfos, const signed char* pMeaningPtr) const;
    ...
    void xFillMeaningGroup(LingWordsGroup& pMeaningGroup, int pRootMeaningId) const;
    ...
    bool xIsAWordFrom(const int* pMeaningFromNode) const;

    int32_t xMeaningFromNodeToMeaningId(const int32_t* pMeaningFromNode) const;

    const signed char* xGetRawCharPtrNbFlexions(const int* pMeaningFromNode) const;

    bool xAreTheFlexionsWritenInTheNode(const int* pMeaningFromNode) const;

    const signed char* xGetNbFlexionsPtr(const int* pMeaningFromNode) const;

    const int* xGetNextMeaningFromNode(const int* pMeaningFromNode) const;
    ...
    PartOfSpeech xGetPartOfSpeech(const signed char* pMeaning) const;
