...
struct ONSEM_TEXTTOSEMANTIC_API SemLineToPrint {
    SemLineToPrint(const std::string& pLine)
        : offset(0)
        , elts(1, pLine)
        , inGrey(false)
        , inBold(false)
    ...
    SemLineToPrint(std::size_t pOffset, const std::string& pLine)
        : offset(pOffset)
        , elts(1, pLine)
        , inGrey(false)
        , inBold(false)
    ...
    SemLineToPrint(std::size_t pOffset, std::list<std::string>& pElts, const SemanticExpression* pSemExp = nullptr)
        : offset(pOffset)
        , elts()
        , inGrey(false)
        , inBold(false)
