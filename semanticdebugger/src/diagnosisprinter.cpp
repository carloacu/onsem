                                 ...
                                 const std::vector<std::string>& pModes)
{
  if (pItMode != pModes.end())
  {
    ...
    SemanticDebug::debugTextAnalyze(autoAnnotToDisplay, *pItMode, spellingMistakeTypesPossible,
                                    debugOptions, pLanguageType, pLingDb);
                     ...
                     const linguistics::LinguisticDatabase& pLingDb)
{
  if (pItMode != pModes.end())
  {
    ...
    if (*pItMode == "memory")
    {
      ++pItMode;
      if (pItMode == pModes.end() ||
          !_prettyPrintMemory(pLines, *pItMode, pSemanticMemory))
      {
                                   ...
                                   const std::vector<std::string>& pModes)
{
  ...
  if (pModes.size() == 1)
  {
    if (pModes[0] == "text")
    {
      ...
      for (std::list<SemanticLanguageEnum>::const_iterator
           itLang = languageTypes.begin(); itLang != languageTypes.end(); ++itLang)
      {
  ...
  if (pModes.size() == 2)
  {
    if (pModes[0] == "text")
    {
      ...
      for (std::list<ConvertionOutputEnum>::const_iterator
           itConv = convValues.begin(); itConv != convValues.end(); ++itConv)
      {
