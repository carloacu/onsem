...
void SemExpLinesToStr::printAlternativelySemExpAndDiffOfSemExps(
    ...
    for (std::list<std::list<SemLineToPrint>>::const_iterator it = pConvOutputs.begin(); it != pConvOutputs.end();
         ++it) {
        comparedPrint(pSemExpPrettyPrint, *it, lastSemExp, weArePrintingASemExp);
    ...
    if (pConvOutputs.size() > 1) {
        comparedPrint(pSemExpPrettyPrint, pConvOutputs.back(), nullptr, true);
    }
}

void SemExpLinesToStr::printLines(std::string& pSemExpPrettyPrint,
                                  const std::list<SemLineToPrint>& pSemExpToPrint) const {
    for (std::list<SemLineToPrint>::const_iterator itLine = pSemExpToPrint.begin(); itLine != pSemExpToPrint.end();
         ++itLine) {
        if (itLine->inBold) {
...
void SemExpLinesToStr::comparedPrint(std::string& pSemExpPrettyPrint,
    ...
    if (pWeArePrintingASemExp && pLastSemExpToPrint != nullptr) {
        _printDiffComparedToLastSemExp(pSemExpPrettyPrint, pSemExpToPrint, *pLastSemExpToPrint);
    } else {
        if (!pWeArePrintingASemExp) {
            pSemExpPrettyPrint += fBeginGrey;
        }
...
void SemExpLinesToStr::_printDiffComparedToLastSemExp(std::string& pSemExpPrettyPrint,
                                                      const std::list<SemLineToPrint>& pSemExpToPrint,
    ...
    for (std::list<SemLineToPrint>::const_iterator itLine = pSemExpToPrint.begin(); itLine != pSemExpToPrint.end();) {
        ...
        if (_areLinesNearlyTheSame(itLine->elts, itLastLine->elts)) {
            ...
            _printDiffComparedToLastLine(pSemExpPrettyPrint, itLine->elts, itLastLine->elts);
            pSemExpPrettyPrint += fNewLine;
            ++itLine;
            ++itLastLine;
            continue;
...
void SemExpLinesToStr::_printDiffComparedToLastLine(std::string& pSemExpPrettyPrint,
...
bool SemExpLinesToStr::_areLinesNearlyTheSame(const std::list<std::string>& pElts1,
...
void SemExpLinesToStr::_printElts(std::string& pSemExpPrettyPrint, const std::list<std::string>& pElts) const {
...
int SemExpLinesToStr::_nbEltsWeShouldSkip(const std::string& pElt,
...
int SemExpLinesToStr::_nbLinesWeShouldSkip(const SemLineToPrint& pRefLine,
