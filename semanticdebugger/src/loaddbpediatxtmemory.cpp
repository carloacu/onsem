                       ...
                       bool pAddReferences)
{
  ...
  if (pTextReplacedFilePtr)
  {
    grdExpDisabledByGrdExpConnection =
        pSemanticMemory.memBloc.grdExpReplacedGrdExps.connectUnsafe([&](const GroundedExpression& pNewGrdExp,
                                                                        const std::list<const GroundedExpression*>& pGrdExpsDisabled) {
      ...
      converter::semExpToText(newText, pNewGrdExp.clone(), textProcContext, false,
      ...
      *pTextReplacedFilePtr << "Expression \"" << newText << "\" replaced";
      ...
      for (const auto& currGrdExp : pGrdExpsDisabled)
      {
        ...
        converter::semExpToText(textDisabled, currGrdExp->clone(), textProcContext, false,
                                pSemanticMemory.memBloc, pSemanticMemory.getCurrUserId(),
                                pLingDb, nullptr);
          ...
          if (posEndOfLabel != std::string::npos)
          {
            ...
            if (line.size() > posBeginOfLanguage)
            {
              ...
              if (posEndOfLanguage != std::string::npos)
              {
                ...
                std::string langaugeStr = line.substr(posBeginOfLanguage, posEndOfLanguage - posBeginOfLanguage);
                SemanticLanguageEnum language = langaugeStr == "fr" ? SemanticLanguageEnum::FRENCH : SemanticLanguageEnum::ENGLISH;
                ...
                if (posEndORevelance != std::string::npos)
                {
                  ...
                  TextProcessingContext textProcContext(SemanticAgentGrounding::userNotIdentified,
                                                        SemanticAgentGrounding::userNotIdentified,
                                                        language);
                  linguistics::extractProperNounsThatDoesntHaveAnyOtherGrammaticalTypes(pProperNouns, reference, language, pLingDb);
