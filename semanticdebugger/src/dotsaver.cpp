...
std::string _replaceString(std::string subject, const std::string& search, const std::string& replace) {
    size_t pos = 0;
...
std::string _stringToPrintByDot(const std::string& subject) {
    std::string res = _replaceString(subject, "\\", "\\\\");
    return _replaceString(res, "\"", "\\\"");
}

bool _writeAChunkNode(std::ostream& pSs,
                      NodeToId& pNodesWriten,
                      std::size_t& pNodeId,
                      std::size_t& pNewId,
    ...
    if (pNewNode.chunk->type == ChunkType::VERB_CHUNK || pNewNode.chunk->type == ChunkType::AUX_CHUNK) {
        pSs << ",color=orange";
...
void _writeAChunk(std::ostream& pSs,
                  NodeToId& pNodesWriten,
                  std::size_t pMotherId,
                  std::size_t& pNewId,
                  const ChunkLink& pNewNode) {
    std::size_t nodeId;
    bool newNode = _writeAChunkNode(pSs, pNodesWriten, nodeId, pNewId, pNewNode);
    pSs << pMotherId << "->" << nodeId << " [label=\"" << strToAvoidWarnings << chunkLinkType_toStr(pNewNode.type);
...
int _generateDotFile(const std::string& pInFilename, const std::string& pOutFilename) {
    std::stringstream ss;
