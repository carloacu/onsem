...
void TimeChecker::beginOfTimeSlot() {
    _currTimeSlotStart = std::chrono::system_clock::time_point(std::chrono::system_clock::now().time_since_epoch());
}

void TimeChecker::endOfTimeSlot(const std::string& pName) {
    std::chrono::system_clock::time_point currTimeSlotEnd(std::chrono::system_clock::now().time_since_epoch());
...
void _microToStr(std::stringstream& pSs, std::int_least64_t pTimeInMicro) {
...
TimeChecker::TimeSlot::TimeSlot(std::string pName, std::int_least64_t pTimeInMs)
    : name(pName)
