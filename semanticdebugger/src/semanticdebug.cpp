...
void SyntacticAnalysisResultToDisplay::saveConcepts
(const linguistics::TokensTree& pTokensTree)
{
  for (linguistics::ConstTokenIterator itTokens = pTokensTree.beginToken();
       !itTokens.atEnd(); ++itTokens)
  {
...
void SyntacticAnalysisResultToDisplay::saveContextInfos
(const linguistics::TokensTree& pTokensTree)
{
  for (linguistics::ConstTokenIterator itTokens = pTokensTree.beginToken();
       !itTokens.atEnd(); ++itTokens)
...
void debugTextAnalyze
(SyntacticAnalysisResultToDisplay& pAutoAnnotToDisplay,
 ...
 const std::map<std::string, std::string>* pEquivalencesPtr)
{
  ...
  pAutoAnnotToDisplay.isReformulationOk = areTextEquivalent(pSentence,
                                                            pAutoAnnotToDisplay.highLevelResults.reformulationInputLanguage,
                                                            pEquivalencesPtr);
...
void semAnalResultToStructToDisplay
(SyntacticAnalysisResultToDisplay& pAutoAnnotToDisplay,
 const SyntacticGraphResult& pSemAnalResult)
{
  // Print the list of tokens
  for (linguistics::ConstTokenIterator itTokens = pSemAnalResult.syntGraph.tokensTree.beginToken();
       !itTokens.atEnd(); ++itTokens)
