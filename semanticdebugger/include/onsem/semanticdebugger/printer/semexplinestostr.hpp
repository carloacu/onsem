...
namespace onsem {
enum class PrintSemExpDiffsOutPutFormat { HTML, CONSOLE };
...
public:
    ...
    void printAlternativelySemExpAndDiffOfSemExps(std::string& pSemExpPrettyPrint,
                                                  const std::list<std::list<SemLineToPrint>>& pConvOutputs) const;

    void printLines(std::string& pSemExpPrettyPrint, const std::list<SemLineToPrint>& pSemExpToPrint) const;

    void comparedPrint(std::string& pSemExpPrettyPrint,
...
private:
    ...
    void _setOutputFormat(PrintSemExpDiffsOutPutFormat pOutputFormat);

    void _printDiffComparedToLastSemExp(std::string& pSemExpPrettyPrint,
                                        const std::list<SemLineToPrint>& pSemExpToPrint,
                                        const std::list<SemLineToPrint>& pLastSemExpToPrint) const;

    void _printDiffComparedToLastLine(std::string& pSemExpPrettyPrint,
                                      const std::list<std::string>& pEltsToPrint,
                                      const std::list<std::string>& pLastEltsToPrint) const;

    bool _areLinesNearlyTheSame(const std::list<std::string>& pElts1, const std::list<std::string>& pElts2) const;

    void _printElts(std::string& pSemExpPrettyPrint, const std::list<std::string>& pElts) const;

    int _nbEltsWeShouldSkip(const std::string& pElt,
                            std::list<std::string>::const_iterator pListIt,
                            const std::list<std::string>& pList) const;

    int _nbLinesWeShouldSkip(const SemLineToPrint& pRefLine,
