...
void _loadConvForALanguage(KeyToFStreams& pIStreams,
    ...
    while (itTreeConvsFolders != endit) {
        ...
        if (std::filesystem::is_directory(itTreeConvsFolders->path())) {
            _loadConvForALanguage(
                pIStreams, pLangEnum, absolutePath, pLocalPath + "/" + itTreeConvsFolders->path().filename().string());
        } else if (!absolutePath.empty() && absolutePath[absolutePath.size() - 1] != '~') {
            pIStreams.addConversionFile(
                pLangEnum, pLocalPath + "/" + itTreeConvsFolders->path().filename().string(), absolutePath);
        }
        ++itTreeConvsFolders;
    }
}
    ...
    for (std::size_t i = 0; i < semanticLanguageEnum_size; ++i) {
        ...
        if (language != SemanticLanguageEnum::UNKNOWN) {
            for (std::size_t j = 0; j < semanticLanguageEnum_size; ++j) {
                ...
                if (language != secondLanguage && secondLanguage != SemanticLanguageEnum::OTHER
                    && secondLanguage != SemanticLanguageEnum::UNKNOWN) {
                    auto filename = pLingDbPath + "/" + semanticLanguageEnum_toLegacyStr(language) + "_to_"
...

    {
        ...
        while (itTreeConvsFolders != endit) {
            if (std::filesystem::is_directory(itTreeConvsFolders->path())) {
                ...
                _loadConvForALanguage(iStreams, langEnum, itTreeConvsFolders->path().string(), filename);
            }
            ++itTreeConvsFolders;
        }
    }
