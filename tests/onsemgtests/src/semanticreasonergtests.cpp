...
const TextProcessingContext SemanticReasonerGTests::fromRobot =
    TextProcessingContext::getTextProcessingContextFromRobot(SemanticLanguageEnum::UNKNOWN);
const TextProcessingContext SemanticReasonerGTests::fromUser =
    TextProcessingContext::getTextProcessingContextToRobot(SemanticLanguageEnum::UNKNOWN);
...
TEST_F(SemanticReasonerGTests, diagnosisMethod) {
    ...
    std::string res = diagnosisPrinter::diagnosis({"loadedDatabases"}, SemanticMemory(), lingDb);
...
TEST_F(SemanticReasonerGTests, languageDetection) {
    const linguistics::LinguisticDatabase& lingDb = *lingDbPtr;
    ASSERT_EQ(SemanticLanguageEnum::FRENCH, linguistics::getLanguage("je suis Paul", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::FRENCH, linguistics::getLanguage("tu es cool", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::FRENCH, linguistics::getLanguage("Dis salut tout le monde", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::FRENCH, linguistics::getLanguage("Comment lire la Bible ?", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::ENGLISH, linguistics::getLanguage("I am Paul", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::ENGLISH, linguistics::getLanguage("bye-bye", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::ENGLISH, linguistics::getLanguage("February 11th", lingDb));
    ASSERT_EQ(SemanticLanguageEnum::UNKNOWN, linguistics::getLanguage("adeadeade", lingDb));
}
...
TEST_F(SemanticReasonerGTests, synthesizeManyCommands) {
    UniqueSemanticExpression semExp = [] {
        ...
        auto textOfCommand = std::make_unique<GroundedExpression>(std::make_unique<SemanticResourceGrounding>(
            "aMethodCall", SemanticLanguageEnum::UNKNOWN, "Service.method()"));
...
TEST_F(SemanticReasonerGTests, CheckThatTheTokenListEqualsInitialText) {
    ...
    ASSERT_TRUE(_doesTokenListEqualsInitalText(errorMessage, frenchStr, lingDb, _corpusInputFolder)) << errorMessage;
    ASSERT_TRUE(_doesTokenListEqualsInitalText(errorMessage, englishStr, lingDb, _corpusInputFolder)) << errorMessage;
    ASSERT_TRUE(_doesTokenListEqualsInitalText(errorMessage, japaneseStr, lingDb, _corpusInputFolder)) << errorMessage;
}
...
TEST_F(SemanticReasonerGTests, CheckFillParameters) {
...

    {
        ...
        UniqueSemanticExpression resExp = expToFill->clone(&params);
        EXPECT_EQ("You don't see any human.",
                  semExpToText(std::move(resExp), SemanticLanguageEnum::ENGLISH, semMem, lingDb));
    }

    {
        ...
        params[0].emplace("", std::make_unique<UniqueSemanticExpression>(textToSemExp("0", lingDb)));
        UniqueSemanticExpression resExp = expToFill->clone(&params);
        EXPECT_EQ("You don't see any other human.",
                  semExpToText(resExp->clone(), SemanticLanguageEnum::ENGLISH, semMem, lingDb));
        EXPECT_EQ("Tu ne vois aucun autre humain.",
                  semExpToText(std::move(resExp), SemanticLanguageEnum::FRENCH, semMem, lingDb));
    }

    {
        ...
        UniqueSemanticExpression resExp = expToFill->clone(&params);
        EXPECT_EQ("You see another human.",
                  semExpToText(resExp->clone(), SemanticLanguageEnum::ENGLISH, semMem, lingDb));
        EXPECT_EQ("Tu vois un autre humain.",
                  semExpToText(std::move(resExp), SemanticLanguageEnum::FRENCH, semMem, lingDb));
    }

    {
        ...
        UniqueSemanticExpression resExp = expToFill->clone(&params);
        EXPECT_EQ("You see 2 other humans.",
                  semExpToText(resExp->clone(), SemanticLanguageEnum::ENGLISH, semMem, lingDb));
        EXPECT_EQ("Tu vois 2 autres humains.",
                  semExpToText(std::move(resExp), SemanticLanguageEnum::FRENCH, semMem, lingDb));
    }
