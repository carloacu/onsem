...
void _triggers_addFromSemExp(UniqueSemanticExpression& pTriggerSemExp,
    ...
    TextProcessingContext answerProcContext(SemanticAgentGrounding::getMe(), SemanticAgentGrounding::getCurrentUser(), pLanguage);
    answerProcContext.isTimeDependent = false;
    answerProcContext.linguisticAnalysisConfig.cmdGrdExtractorPtr = std::make_shared<ResourceGroundingExtractor>(
        std::vector<std::string>{resourceLabelForTests_cmd, resourceLabelForTests_url});
    auto answerSemExp = converter::textToContextualSemExp(
        pAnswerText, answerProcContext, SemanticSourceEnum::WRITTENTEXT, pLingDb, &pReferences);
...
void triggers_addFromSemExps(UniqueSemanticExpression& pTriggerSemExp,
    ...
    if (pAddInfinitveForms) {
        ...
        if (infinitiveActionSemExp)
            triggers::add(std::move(*infinitiveActionSemExp), pAnswerSemExp->clone(), pSemanticMemory, pLingDb);
    }
...
void triggers_addAnswerWithOneParameter(const std::string& pTriggerText,
    ...
    triggers_addAnswerWithManyParameters(
        pTriggerText, parameterLabelToQuestionsStrs, pSemanticMemory, pLingDb, pLanguage);
}
...
TEST_F(SemanticReasonerGTests, operator_addATrigger_basic) {
    ...
    // simple scenario
    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("I don't know how you can be happy.", operator_answer(howCanIBeHappy, semMem, lingDb));
    ...
    // simple scenario in french
    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas ce que c'est un champignon.",
                                operator_react(whatIsAMushroom, semMem, lingDb));
    ...
    // many trigger questions
    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("I don't know what I like.", operator_answer(question1Str, semMem, lingDb));
        ONSEM_ANSWERNOTFOUND_EQ("I don't know if I like soccer.", operator_answer(question2Str, semMem, lingDb));
        ...
        ONSEM_ANSWER_EQ("It's more precise.", operator_answer("Does Paul like banana ?", semMem, lingDb));
    ...
    // a where question
    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas où est la clé abîmée.", operator_react(questionsStr, semMem, lingDb));
    ...
    // must question
    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas ce qu'il faut faire.", operator_answer(questionsStr, semMem, lingDb));
    ...
    // affirmation trigger
    {
        ...
        ONSEM_QUESTION_EQ("Why is Paul a nice guy?", operator_react(affirmationStr, semMem, lingDb));
    ...
    // sentence not completed
    {
        ...
        ONSEM_FEEDBACK_EQ("Ok, tu dis.", operator_react(affirmationStr, semMem, lingDb));
    ...
    // order trigger
    {
        ...
        ONSEM_BEHAVIORNOTFOUND_EQ("Je ne sais pas te donner ma main.", operator_react(order1Str, semMem, lingDb));
    ...
    // nominal group trigger
    {
        ...
        ONSEM_QUESTION_EQ("Quel est ton espace Carrefour dédié ?",
                          operator_react(inStr, semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ...
    // condition trigger
    {
        ...
        ONSEM_BEHAVIORNOTFOUND_EQ("Je ne sais pas regarder à droite.", operator_react(triggerStr, semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_addATrigger_checkReferences) {
    ...
    triggers_add(
        howCanITeachYou, "for example, you can say \"to salute is to say hello\"", semMem, lingDb, debugReferences);
    compareWithRef("For example, you can say \"to salute is to say hello\".",
                   debugReferences,
...
TEST_F(SemanticReasonerGTests, operator_addATrigger_subMemory_affirmation) {
    ...
    semMem.memBloc.subBlockPtr = &subSemMem.memBloc;
    ONSEM_ANSWER_EQ("Effectivement. C'est son anniversaire demain.", operator_react(affirmationStr, semMem, lingDb));
    // 2 levels of depth
    {
        SemanticMemory semMemTopLevel;
        semMemTopLevel.memBloc.subBlockPtr = &semMem.memBloc;
...
TEST_F(SemanticReasonerGTests, operator_addATrigger_subMemory_nominalGroup) {
    ...
    ONSEM_QUESTION_EQ("Qui est Paul ?",
                      operator_react(propernounStr, semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ONSEM_NOANSWER(operator_react("Je ne sais pas", semMem, lingDb));
    semMem.memBloc.subBlockPtr = &subSemMem.memBloc;
    ONSEM_ANSWER_EQ("C'est son anniversaire demain.",
                    operator_react(propernounStr, semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ONSEM_ANSWER_EQ("C'est des nems.",
                    operator_react(nounStr, semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ...
    // 2 levels of depth
    {
        ...
        ONSEM_ANSWER_EQ(
            ...
            operator_react(propernounStr, semMemTopLevel, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    }
}
