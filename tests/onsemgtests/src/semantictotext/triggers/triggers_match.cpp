...
DetailedReactionAnswer triggers_match(const std::string& pText,
    ...
    return _operator_reactFromTrigger_fromSemExp(
        std::move(semExp), pSemanticMemory, pLingDb, textLanguage, pReactionOptions);
}
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_basic_fr) {
    ...
    const std::string trigger22 = "Dis la date";
    ...
    const std::string trigger25 = "cuisine";
    const std::string reaction25 = "Il faut aller à table.";
    ...
    triggers_add(trigger25, reaction25, semMem, lingDb, {}, SemanticLanguageEnum::UNKNOWN, true);
    ...
    ONSEM_ANSWER_EQ(reaction8,
                    triggers_match("Descends le volume tout de suite", semMem, lingDb, SemanticLanguageEnum::FRENCH));
    ...
    ONSEM_ANSWER_EQ(reaction25, triggers_match("la cuisine", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_basic_en) {
    ...
    ONSEM_NOANSWER(
        triggers_match("Designing robots with emotional understanding requires advanced algorithms.", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_withParameters_fr) {
    ...
    triggers_addAnswerWithOneParameter(
        "Fais une rotation à gauche", howManyDegreesParameterQuestion, semMem, lingDb, language);
    ...
    triggers_addAnswerWithOneParameter(
        "Fais une rotation à droite", howManyDegreesParameterQuestion, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Tourne la tête à gauche", howManyDegreesParameterQuestion, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Tourne la tête à droite", howManyDegreesParameterQuestion, semMem, lingDb, language);
    ...
    triggers_addAnswerWithOneParameter(
        "Descends le volume", howMuchInPercentageParameterQuestions, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Diminue le volume", howMuchInPercentageParameterQuestions, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Mets le volume moins fort", howMuchInPercentageParameterQuestions, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Parle moins fort", howMuchInPercentageParameterQuestions, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Monte le volume", howMuchInPercentageParameterQuestions, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Augmente le volume", howMuchInPercentageParameterQuestions, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Mets le volume plus fort", howMuchInPercentageParameterQuestions, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Parle plus fort", howMuchInPercentageParameterQuestions, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Mets le volume", howMuchInPercentageParameterQuestions, semMem, lingDb, language);
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=2 mètres)\\",
                      triggers_match("Avance de deux mètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=0,7 mètre)\\", triggers_match("Avance de 0,7 m", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=0,1 mètre)\\",
                      triggers_match("Je veux que tu avances de 10 cm", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=0,1 mètre)\\",
                      triggers_match("J'aimerais que tu avances de 10 cm", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=0,3 mètre)\\",
                      triggers_match("Avance de 30 centimètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=0,4 mètre)\\", triggers_match("Avance de 40 cm", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va vers l'avant(param1=5 mètres)\\",
                      triggers_match("Va vers l'avant 5 mètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va vers l'avant(param1=6 mètres)\\",
                      triggers_match("Va vers l'avant de 6 mètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va tout droit\\", triggers_match("Va tout droit", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va tout droit(param1=7 mètres)\\",
                      triggers_match("Va tout droit 7 mètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va tout droit(param1=8 mètres)\\",
                      triggers_match("Va tout droit de 8 mètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Avance(param1=7 mètres)\\",
                      triggers_match("Je veux que tu avances de 7 mètres", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va en arrière(param1=0,7 mètre)\\",
                      triggers_match("Va en arrière 0,7 mètre", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va en arrière(param1=0,35 mètre)\\",
                      triggers_match("Va en arrière trente cinq centimètres", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va en arrière(param1=0,5 mètre)\\",
                      triggers_match("Va 50 centimètres en arrière", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va un peu en arrière\\", triggers_match("Va un peu en arrière", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va un peu en arrière\\",
                      triggers_match("Va un petit peu en arrière", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Recule beaucoup\\", triggers_match("Recule beaucoup", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va beaucoup en arrière\\",
                      triggers_match("Va beaucoup en arrière", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va beaucoup en arrière\\", triggers_match("Va très en arrière", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne à gauche(param1=34 degrés)\\",
                      triggers_match("Tourne à gauche de 34 degrés", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne à droite(param1=37 degrés)\\",
                      triggers_match("Tourne à droite de 37 degrés", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va à droite(param1=42 degrés)\\",
                      triggers_match("Va 42 degrés à droite", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Va à droite(param1=45 degrés)\\",
                      triggers_match("Va à droite de 45 degrés", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais une rotation à droite(param1=23 degrés)\\",
                      triggers_match("Fais une rotation à droite de 23 degrés", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne à gauche(param1=25 degrés)\\",
                      triggers_match("Tourne à gauche de vingt cinq degrés", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne la tête à gauche\\",
                      triggers_match("Tourne la tête à gauche", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne la tête à gauche(param1=38 degrés)\\",
                      triggers_match("Tourne la tête à gauche de trente huit degrés", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne la tête à droite\\",
                      triggers_match("Tourne la tête à droite", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne la tête à droite(param1=25 degrés)\\",
                      triggers_match("Tourne la tête à droite de vingt cinq degrés", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Lève la tête(param1=11 degrés)\\",
                      triggers_match("Monte la tête de onze degrés", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Baisse la tête(param1=29 degrés)\\",
                      triggers_match("descends la tête de vingt neuf degrés", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Baisse la tête(param1=31 degrés)\\",
                      triggers_match("descends la tête de 31°", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Descends le volume\\",
                      triggers_match("Descends ton volume", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Descends le volume(param1=30 pour cent)\\",
                      triggers_match("Descends le volume de 30 %", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Descends le volume(param1=31 pour cent)\\",
                      triggers_match("Descends ton volume de 31 %", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Diminue le volume(param1=32 pour cent)\\",
                      triggers_match("Je veux que tu diminues le son de 32 %", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume moins fort\\",
                      triggers_match("Mets le volume moins fort", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume moins fort\\",
                      triggers_match(("Mets le volume encore moins fort"), semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Parle plus fort\\",
                      triggers_match(("parle encore plus fort"), semMem, lingDb, language));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Monte le volume(param1=12 pour cent)\\",
                      triggers_match("Monte le volume de 12 %", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume(param1=90 pour cent)\\",
                      triggers_match("Mets le volume à 90 %", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume(param1=80)\\",
                      triggers_match("Mets le volume à 80", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume(param1=91 pour cent)\\",
                      triggers_match("Mets le volume à quatre vingt onze pour cent", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume plus fort\\",
                      triggers_match("Mets le volume plus fort", semMem, lingDb, language));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Mets le volume plus fort\\",
                      triggers_match("Mets le volume encore plus fort", semMem, lingDb, language));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Lance(param1=Akinator)\\", triggers_match("Lance akinator", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne sans t'arrêter\\",
                      triggers_match("Tourne sans t'arrêter", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne sans t'arrêter(param1=Gauche)\\",
                      triggers_match("Tourne sans t'arrêter à gauche", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne en continu\\", triggers_match("Tourne en continu", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne en continu(param1=Droite)\\",
                      triggers_match("tourne en continu à droite", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne sans arrêt\\", triggers_match("tourne sans arrêt", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne sans arrêt(param1=Gauche)\\",
                      triggers_match("Tourne sans arrêt à gauche", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#fais la toupie\\", triggers_match("fais la toupie", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#fais la toupie(param1=Droite)\\",
                      triggers_match("Fais la toupie à droite", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour complet(nbOfTimes=1)\\",
                      triggers_match("Fais un tour complet", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour complet(nbOfTimes=2)\\",
                      triggers_match("Fais un tour complet 2 fois", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour complet(nbOfTimes=1, speed=Vite)\\",
                      triggers_match("Fais un tour complet vite", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour complet(nbOfTimes=1, speed=Rapidement)\\",
                      triggers_match("Fais un tour complet rapidement", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour complet(location=Droite, nbOfTimes=1)\\",
                      triggers_match("Fais un tour complet à droite", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Tourne sur toi même(location=Toi, nbOfTimes=1)\\",
                      triggers_match("Tourne sur toi même", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais une pirouette(nbOfTimes=1)\\",
                      triggers_match("Fais une pirouette", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un tour sur toi même(location=Toi, nbOfTimes=1)\\",
                      triggers_match("Fais un tour sur toi même", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un 360(nbOfTimes=1)\\", triggers_match("Fais un 360", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un 360(nbOfTimes=1, speed=Lentement)\\",
                      triggers_match("Fais un 360 lentement", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un 360(nbOfTimes=3, speed=Lentement)\\",
                      triggers_match("Fais un 360 lentement 3 fois", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Fais un 360(location=Droite, nbOfTimes=4, speed=Vite)\\",
                      triggers_match("Fais un 360 à droite vite 4 fois", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Éteins-toi\\", triggers_match("éteins toi", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Éteins-toi\\", triggers_match("Éteins-toi", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Éteins-toi(param1=3 minutes)\\",
                      triggers_match("Éteins-toi pendant 3 minutes", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Éteins-toi(param1=120 minutes)\\",
                      triggers_match("Éteins-toi pendant 2 heures", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Éteins-toi(param1=60 minutes)\\",
                      triggers_match("éteins toi une heure", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Reste ici(param1=120 minutes)\\",
                      triggers_match("reste là pendant 2 heures", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Reste immobile(param1=5 minutes)\\",
                      triggers_match("reste immobile 5 minutes", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#Raconte une blague\\",
                      triggers_match("raconte    une autre histoire drôle", semMem, lingDb));
    ONSEM_NOANSWER(triggers_match("Dis moi quel robot es-tu ?", semMem, lingDb));
    ONSEM_ANSWER_EQ("\\label=#fr_FR#Qui es tu ?\\",
                    triggers_match("Pourriez-vous me dire qui vous êtes ?", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_withParameters_fr2) {
    ...
    std::map<std::string, std::vector<std::string>> whatAndWhereParameters{{"subject", {"qu'est-ce qui"}},
                                                                           {"location", {"où"}}};
    ...
    std::map<std::string, std::vector<std::string>> putParameters{{"object", {"quoi"}}, {"location", {"où"}}};
    ...
    ONSEM_ANSWER_EQ("\\label=#fr_FR#tu es(param1=L'ascenseur)\\",
                    triggers_match("Tu es à l'ascenseur", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#va(param1=La cuisine)\\", triggers_match("Va à la cuisine", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#fr_FR#mets(location=Dans la cuisine, object=Les bonbons)\\",
                      triggers_match("Mets les bonbons dans la cuisine", semMem, lingDb));
    ONSEM_ANSWER_EQ("\\label=#fr_FR#\\p_meta=0\\ sont(location=Dans la cuisine, subject=Les bonbons)\\",
                    triggers_match("les bonbons sont dans la cuisine", semMem, lingDb));
    ONSEM_ANSWER_EQ("\\label=#fr_FR#\\p_meta=0\\ sont déplacés(object=Les bonbons)\\",
                    triggers_match("les bonbons sont déplacés", semMem, lingDb));
    ONSEM_ANSWER_EQ("\\label=#fr_FR#\\p_meta=0\\ sont déplacés(object=La bouteille)\\",
                    triggers_match("la bouteille est déplacée", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_withParameters_en) {
    ...
    triggers_addAnswerWithOneParameter("Advance", howFarInMetersParameterQuestion, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter("Move forward a little", {""}, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter("Move forward a lot", {""}, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter("Move backward", howFarInMetersParameterQuestion, semMem, lingDb, language);
    ...
    triggers_addAnswerWithOneParameter(
        "Turn on your left", {howFarInDegreesParameterQuestion}, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter("Turn right", {howFarInDegreesParameterQuestion}, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Turn on your right", {howFarInDegreesParameterQuestion}, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Turn the head to the right", {howFarInDegreesParameterQuestion}, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Turn the head to the left", {howFarInDegreesParameterQuestion}, semMem, lingDb, language);
    ...
    triggers_addAnswerWithOneParameter(
        "Lower the volume", howMuchInPercentageParameterQuestion, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Lower the voice", howMuchInPercentageParameterQuestion, semMem, lingDb, language);

    triggers_addAnswerWithOneParameter(
        "Turn up the volume", howMuchInPercentageParameterQuestion, semMem, lingDb, language);
    triggers_addAnswerWithOneParameter(
        "Set the volume", howMuchInPercentageParameterQuestion, semMem, lingDb, language);
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Advance(param1=0.2 meter)\\",
                      triggers_match("move forward 20 cm", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Advance(param1=0.2 meter)\\",
                      triggers_match("move forward 20 centimeters", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Advance(param1=2 meters)\\",
                      triggers_match("Go forward two meters", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Advance(param1=3 meters)\\", triggers_match("Advance 3 meters", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move forward a little\\",
                      triggers_match("can you go forward a little", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move forward a little\\",
                      triggers_match("could you go forward a little", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move backward(param1=3 meters)\\",
                      triggers_match("Move backward 3 meters", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move backward(param1=0.4 meter)\\",
                      triggers_match("move backward 40 centimeters", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move backward(param1=1 meter)\\",
                      triggers_match("Go backward one meter", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Move backward a little\\",
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn right(param1=33 degrees)\\",
                      triggers_match("turn right 33 degrees", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn the head to the right(param1=34 degrees)\\",
                      triggers_match("Turn the head to the right 34 degrees", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn the head to the left(param1=41 degrees)\\",
                      triggers_match("Turn the head to the left 41 degrees", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head up(param1=24 degrees)\\",
                      triggers_match("Head up 24 degrees", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head up(param1=43 degrees)\\",
                      triggers_match("Raise the head 43 degrees", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head up(param1=43 degrees)\\",
                      triggers_match("Raise your head 43 degrees", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head down(param1=25 degrees)\\",
                      triggers_match("Head down 25 degrees", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head down(param1=25 degrees)\\", triggers_match("Head down 25°", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Head down(param1=23 degrees)\\",
                      triggers_match("Lower your head 23 degrees", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the volume(param1=29 percents)\\",
                      triggers_match("Turn down the volume by 29 %", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the volume(param1=28)\\",
                      triggers_match("Turn down the volume by 28", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the volume(param1=30 percents)\\",
                      triggers_match("Lower the volume by 30%", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the volume(param1=31 percents)\\",
                      triggers_match("Lower your volume by 31%", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the voice(param1=28 percents)\\",
                      triggers_match("Lower the voice by 28%", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Lower the voice(param1=29 percents)\\",
                      triggers_match("Lower your voice by 29%", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn up the volume\\", triggers_match("Turn up the volume", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn up the volume(param1=31 percents)\\",
                      triggers_match("Turn up the volume by 31%", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Set the volume\\", triggers_match("Set the volume", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Set the volume(param1=54 percents)\\",
                      triggers_match("Set the volume to 54%", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn without stopping\\",
                      triggers_match("Turn without stopping", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn without stopping(param1=To the left)\\",
                      triggers_match("Turn without stopping to the left", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn around(param1=Quickly)\\",
                      triggers_match("Turn around quickly", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Make a full turn(nbOfTimes=1)\\",
                      triggers_match("Make a full turn", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Make a full turn(nbOfTimes=1)\\",
                      triggers_match("Make a complete turn", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Make a full turn(nbOfTimes=1, speed=Quickly)\\",
                      triggers_match("Make a full turn quickly", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Make a full turn(nbOfTimes=2, speed=Quickly)\\",
                      triggers_match("Make a full turn quickly 2 times", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Make a full turn(location=The left, nbOfTimes=3, speed=Slowly)\\",
                      triggers_match("Make a full turn to the left slowly 3 times", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn off(param1=4 minutes)\\",
                      triggers_match("Shut down for 4 minutes", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn off(param1=5 minutes)\\",
                      triggers_match("Shut down during 5 minutes", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn off(param1=3 minutes)\\",
                      triggers_match("Shut down 3 minutes", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Turn off(param1=0.5 minute)\\",
                      triggers_match("Turn off 30 seconds", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Do not move\\", triggers_match("Do not move", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Do not move(param1=5 minutes)\\",
                      triggers_match("Do not move 5 minutes", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Stop moving\\", triggers_match("Stop moving", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("\\label=#en_US#Stop moving(param1=240 minutes)\\",
...
TEST_F(SemanticReasonerGTests, operator_reactFromTrigger_mergeWithContext_fr) {
    ...
    triggers_add("Je ne veux pas savoir comment je peux t'apprendre un comportement",
                 "D'accord je ne t'embêterai plus avec ça",
                 semMem,
                 lingDb);

    operator_inform_fromRobot(
        "Est-ce que tu veux savoir comment tu peux m'apprendre un comportement ?", semMem, lingDb);
