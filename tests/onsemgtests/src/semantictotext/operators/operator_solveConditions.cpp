...
TEST_F(SemanticReasonerGTests, operator_solveConditions_basic) {
    ...
    ONSEM_ANSWERNOTFOUND_EQ("", operator_solveConditions("If 1 is greater than 2 then it's a bug.", semMem, lingDb));
    ...
    ONSEM_ANSWER_EQ("A banana",
                    operator_solveConditions("If Paul is happy then a chocolate else a banana.", semMem, lingDb));
    ONSEM_ANSWERNOTFOUND_EQ("", operator_solveConditions("If Paul is happy then a chocolate.", semMem, lingDb));
    ONSEM_ANSWER_EQ(
        "You will drink water.",
        operator_solveConditions(
            "If Paul is happy then a chocolate. If 1 is greater than 0 then I will drink water.", semMem, lingDb));
    ONSEM_ANSWERNOTFOUND_EQ(
        "",
        operator_solveConditions(
            "If Paul is happy then a chocolate. If 1 is greater than 2 then I will drink water.", semMem, lingDb));
}
