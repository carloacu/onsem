...
namespace {
bool _hasACoreferenceFromStr(const std::string& pText, const linguistics::LinguisticDatabase& pLingDb) {
...
TEST_F(SemanticReasonerGTests, operator_mergeWithContext_basic) {
...

    {
        ...
        EXPECT_EQ("/path/to/thriller_dance est Thriller.",
                  semExpToText(std::move(itIsThriller), frLanguage, semMem, lingDb));
    }
...

    {
        ...
        EXPECT_EQ("Salut", semExpToText(std::move(helloSemExp), frLanguage, semMem, lingDb));
    }

    {
        ...
        memoryOperation::mergeWithContext(helloSemExp, semMem, lingDb);
        EXPECT_EQ("Tu vas bien.", semExpToText(std::move(helloSemExp), frLanguage, semMem, lingDb));
    }

    {
        ...
        EXPECT_EQ("C'est tout.", semExpToText(std::move(userSemExp), frLanguage, semMem, lingDb));
    }
