...
std::string operator_askForMoreInformation(const std::string& pText,
                                           SemanticMemory& pSemanticMemory,
                                           const linguistics::LinguisticDatabase& pLingDb) {
    return operator_sayFeedback(
        pText, SemanticTypeOfFeedback::ASK_FOR_ADDITIONAL_INFORMATION, pSemanticMemory, pLingDb);
}
...
TEST_F(SemanticReasonerGTests, operator_sayFeedback_askForMoreInformation) {
    ...
    EXPECT_EQ("What do you like?", operator_askForMoreInformation("what do I like ?", semMem, lingDb));
    EXPECT_EQ("Why are you sad?", operator_askForMoreInformation("I am sad", semMem, lingDb));
    EXPECT_EQ("Why are you happy?", operator_askForMoreInformation("I am happy", semMem, lingDb));
    EXPECT_EQ("Why do you like me?", operator_askForMoreInformation("I like you", semMem, lingDb));
    EXPECT_EQ("When did you play football?", operator_askForMoreInformation("I played football", semMem, lingDb));
    EXPECT_EQ("What is Mickey Mouse?", operator_askForMoreInformation("do you know Mickey Mouse", semMem, lingDb));
    EXPECT_EQ("Pourquoi est-ce cool ?", operator_askForMoreInformation("C'est cool", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_sayFeedback_reactOnSimilarities) {
    ...
    EXPECT_EQ("Yes, I know Paul eats chocolate.", operator_reactOnSimilarities("Paul eats chocolate", semMem, lingDb));
    EXPECT_EQ("Paul also eats chocolate.", operator_reactOnSimilarities("André eats chocolate", semMem, lingDb));
    EXPECT_EQ("I thought the opposite.", operator_reactOnSimilarities("Paul doesn't eat chocolate", semMem, lingDb));
    ...
    EXPECT_EQ("Michel a aussi travaillé aux États-Unis.",
              operator_reactOnSimilarities("Je travaille aux Etats Unis", semMem, lingDb));
