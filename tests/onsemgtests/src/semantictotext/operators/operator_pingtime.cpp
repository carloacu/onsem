...
DetailedReactionAnswer operator_pingtime(const SemanticDuration& pSemDuration,
...
TEST_F(SemanticReasonerGTests, operator_pingtime_basic) {
    ...
    auto timeConditionConnection = semMem.memBloc.semanticTimesWithConditionsLinked.connectUnsafe(
        [&](const SemanticDuration& pSemanticBeginOfTime) { semanticBeginOfTimes.emplace_back(pSemanticBeginOfTime); });

    ONSEM_BEHAVIOR_EQ("Hi", operator_react("say hi", semMem, lingDb));
    EXPECT_EQ(0u, semanticBeginOfTimes.size());
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, I will say hi tomorrow.",
                                       operator_react("say hi tomorrow", semMem, lingDb));
    ASSERT_EQ(1u, semanticBeginOfTimes.size());
    ONSEM_BEHAVIOR_EQ("Hi",
                      operator_pingtime(semanticBeginOfTimes.front(), semMem, lingDb, SemanticLanguageEnum::ENGLISH));
    ...
    {
        ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, I will say hi in 2 minutes.",
                                           operator_react("say hi in 2 minutes", semMem, lingDb));
        ASSERT_EQ(1u, semanticBeginOfTimes.size());
        const SemanticDuration& absoluteDuration = semanticBeginOfTimes.front();
        SemanticDuration relativeDuration = SemanticTimeGrounding::absoluteToRelative(absoluteDuration);
    ...
    {
        ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, I will say toto in 500 milliseconds.",
                                           operator_react("say toto in 500 milliseconds", semMem, lingDb));
        ASSERT_EQ(1u, semanticBeginOfTimes.size());
        const SemanticDuration& absoluteDuration = semanticBeginOfTimes.front();
        SemanticDuration relativeDuration = SemanticTimeGrounding::absoluteToRelative(absoluteDuration);
        EXPECT_EQ(Sign::POSITIVE, relativeDuration.sign);
