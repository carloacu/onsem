...
std::vector<std::string> operator_get(const std::string& pText,
    ...
    for (auto& currAnswer : answers) {
        std::string subRes;
        converter::semExpToText(subRes, std::move(currAnswer), outContext, true, pSemanticMemory, pLingDb, nullptr);
        res[i++] = subRes;
    }
    return res;
}
...
TEST_F(SemanticReasonerGTests, operator_get_length) {
    const linguistics::LinguisticDatabase& lingDb = *lingDbPtr;
    SemanticMemory semMem;

    operator_inform("Je dois avancer de 3 mètres", semMem, lingDb);
    {
    ...
    operator_inform("Je dois tourner à droite de 47 degrés", semMem, lingDb);
    {
        auto res = operator_get("De combien dois-je tourner en degrés ?", semMem, lingDb);
        ASSERT_EQ(1u, res.size());
        EXPECT_EQ("47 degrés", res[0]);
    }
}
