...
std::string operator_answerIDontKnow(const std::string& pText, const linguistics::LinguisticDatabase& lingDb) {
    return _operator_answerIDontKnow(pText, true, lingDb);
}

std::string operator_answerICannotDo(const std::string& pText, const linguistics::LinguisticDatabase& lingDb) {
...
TEST_F(SemanticReasonerGTests, operator_answerIDontKnow_basic) {
    ...
    EXPECT_EQ("I don't know if I like chocolate.",
              operator_answerIDontKnow("I like chocolate. Do you like chocolate?", lingDb));
    EXPECT_EQ("I don't know what I like.", operator_answerIDontKnow("What do you like? Me, I like chocolate.", lingDb));
    EXPECT_EQ("I don't know if I am happy. I don't know why I am shy.",
              operator_answerIDontKnow("Are you happy? Why are you shy?", lingDb));
    EXPECT_EQ("I don't know if I want to see a video that describes Carrefour.",
              operator_answerIDontKnow("Do you want to see a video that describes Carrefour", lingDb));
    ...
    EXPECT_EQ("Je ne sais pas ce que j'en pense.",
              operator_answerIDontKnow("Je ne sais pas. Qu'est-ce que tu en penses ?", lingDb));
    EXPECT_EQ("Je ne sais pas comment je vais.", operator_answerIDontKnow("Comment ça va ? Moi ça va bien.", lingDb));
    EXPECT_EQ("Je ne sais pas comment Paul s'est senti.",
              operator_answerIDontKnow("Comment Paul s’est-il senti ?", lingDb));
    EXPECT_EQ("Je ne sais pas si je vais au Japon.", operator_answerIDontKnow("Est-ce que tu vas au Japon ?", lingDb));
    EXPECT_EQ("Je ne sais pas si je veux voir une vidéo qui décrit Carrefour.",
              operator_answerIDontKnow("Veux tu voir une vidéo qui décrit Carrefour", lingDb));
    ...
    EXPECT_EQ("Je ne sais pas quelle est la différence entre un bébé et un crocodile.",
              operator_answerIDontKnow("Quelle est la différence entre un bébé et un crocodile?", lingDb));
    ...
    EXPECT_EQ("Je ne sais pas ce qu'il en est du bien-être des animaux.",
              operator_answerIDontKnow("Qu'en est-il du bien-être des animaux ?", lingDb));
    EXPECT_EQ("Je ne sais pas à qui demander de l'aide.",
              operator_answerIDontKnow("A qui demander de l'aide ?", lingDb));
    EXPECT_EQ("Je ne sais pas quel restaurant je te conseille.",
              operator_answerIDontKnow("quel restaurant me conseilles-tu", lingDb));
    EXPECT_EQ("Je ne sais pas qui est plus fort que toi.",
              operator_answerIDontKnow("qui est plus fort que moi", lingDb));
...
TEST_F(SemanticReasonerGTests, operator_answerICannotDo_basic) {
    ...
    EXPECT_EQ("I can't show you a video that describes N5.",
              operator_answerICannotDo("Show me a video that describes N5", lingDb));
    ...
    EXPECT_EQ("Je ne sais pas te montrer une vidéo qui décrit N5.",
              operator_answerICannotDo("Montre moi une vidéo qui décrit N5", lingDb));
    EXPECT_EQ("Je ne sais pas arrêter de t'écouter maintenant.",
              operator_answerICannotDo("arrête de m'écouter maintenant", lingDb));
}
