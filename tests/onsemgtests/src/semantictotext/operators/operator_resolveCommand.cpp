...
std::string operator_resolveCommand(const std::string& pText,
    ...
    auto resSemExp =
        memoryOperation::resolveCommandFromMemBlock(*semExp, pSemanticMemory.memBloc, textProc.author.userId, pLingDb);
    if (resSemExp)
        return semExpToOutputStr(**resSemExp, pLanguage, pSemanticMemory, pLingDb, &*semExp);
    return "";
}
...
TEST_F(SemanticReasonerGTests, operator_resolveCommand_basic) {
    ...
    static const std::string resource_goToPostureStr =
    ...
    static const std::string resource_movebackStr =
        "\\" + resourceLabelForTests_cmd + "=semantic/animations/move_back.pmt\\";
    operator_inform("to move back means " + resource_movebackStr, semMem, lingDb);
    static const std::string resource_raiseLeftHandStr =
        "\\" + resourceLabelForTests_cmd + "=semantic/animations/raise_left_hand.qianim\\";
    operator_inform("to raise your left hand means " + resource_raiseLeftHandStr, semMem, lingDb);
    static const std::string resource_whiteEyesStr =
        "\\" + resourceLabelForTests_cmd + "=ALLeds.fadeRGB(\"FaceLeds\", \"white\", 1)\\";
    operator_inform("to have white eyes means " + resource_whiteEyesStr, semMem, lingDb);
    static const std::string resource_yellowEyesStr =
        "\\" + resourceLabelForTests_cmd + "=ALLeds.fadeRGB(\"FaceLeds\", \"yellow\", 1)\\";
    operator_inform("to have yellow eyes means " + resource_yellowEyesStr, semMem, lingDb);

    EXPECT_EQ("Yes", operator_resolveCommand("say yes", semMem, lingDb));
    EXPECT_EQ("Yes", operator_resolveCommand("please say yes", semMem, lingDb));
    EXPECT_EQ("No", operator_resolveCommand("say no", semMem, lingDb));
    static const std::string iAmVeryHappyToSeeYouInFrench = "\"Je suis très content dete voir!!\"";
    EXPECT_EQ(
        iAmVeryHappyToSeeYouInFrench,
        operator_resolveCommand("say " + iAmVeryHappyToSeeYouInFrench, semMem, lingDb, SemanticLanguageEnum::ENGLISH));
    EXPECT_EQ("Quelque chose", operator_resolveCommand("s'il te plait dis quelque chose", semMem, lingDb));
    EXPECT_EQ("Quelque chose", operator_resolveCommand("s'il te plaît dis quelque chose", semMem, lingDb));
    EXPECT_EQ("Salut tout le monde", operator_resolveCommand("dis salut tout le monde", semMem, lingDb));
    EXPECT_EQ("Bonjour", operator_resolveCommand("dis-moi bonjour", semMem, lingDb));
    EXPECT_EQ(resource_whiteEyesStr, operator_resolveCommand("have white eyes", semMem, lingDb));
    EXPECT_EQ(resource_goToPostureStr, operator_resolveCommand("stand up", semMem, lingDb));
    EXPECT_EQ(resource_movebackStr, operator_resolveCommand("move backward", semMem, lingDb));
    EXPECT_EQ(resource_movebackStr, operator_resolveCommand("move backwards", semMem, lingDb));
    EXPECT_EQ(resource_raiseLeftHandStr, operator_resolveCommand("lève la main gauche", semMem, lingDb));
    EXPECT_EQ(resource_raiseLeftHandStr, operator_resolveCommand("raise your left hand", semMem, lingDb));
    ...
    EXPECT_EQ("Paul", operator_resolveCommand("say my name", semMem, lingDb));
    operator_inform("I want to fly.", semMem, lingDb);
    EXPECT_EQ("You want to fly.", operator_resolveCommand("say what I want to do", semMem, lingDb));
    ...
    EXPECT_EQ(resource_doGoodbyeStr, operator_resolveCommand("do goodbye", semMem, lingDb));
    ...
    EXPECT_EQ("You are a nice man.", operator_resolveCommand("do goodbye", semMem, lingDb));
    operator_inform("to compliment means to do goodbye in french", semMem, lingDb);
    EXPECT_EQ("Tu es un homme agréable.", operator_resolveCommand("compliment", semMem, lingDb));
    ...
    // translation
    EXPECT_EQ("Bonjour", operator_resolveCommand("say hello in French", semMem, lingDb));
    // repetition
    EXPECT_EQ("Hello\tNUMBER_OF_TIMES: 4", operator_resolveCommand("say hello four times", semMem, lingDb));
    // repetition + translation
    EXPECT_EQ("Bonjour\tNUMBER_OF_TIMES: 4", operator_resolveCommand("say hello four times in French", semMem, lingDb));
    ...
    EXPECT_EQ(
        "(\t\"a1\"\tAND\t\"a2\"\t)\tTHEN\t(\t\"a3\"\tAND\t\"a4\"\t)\tTHEN\t(\t\"a5\"\tAND\t\"a6\"\t)",
        operator_resolveCommand(
            "Say \"a1\" and say \"a2\" then say \"a3\" and say \"a4\" then say \"a5\" and say \"a6\"", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_resolveCommand_nonImperativeSentences) {
    ...
    EXPECT_EQ("Hello", operator_resolveCommand("you have to say hello", semMem, lingDb));
    EXPECT_EQ("Hello", operator_resolveCommand("I want you to say hello", semMem, lingDb));
    EXPECT_EQ("", operator_resolveCommand("I want chocolate", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_resolveCommand_say) {
    ...
    EXPECT_EQ("I am happy.", operator_resolveCommand("say I am happy", semMem, lingDb));
    EXPECT_EQ("You are happy.", operator_resolveCommand("say that I am happy", semMem, lingDb));
    EXPECT_EQ("You are cool.", operator_resolveCommand("say that I am cool", semMem, lingDb));
    EXPECT_EQ("I love you.", operator_resolveCommand("say I love you", semMem, lingDb));
    EXPECT_EQ("You love me.", operator_resolveCommand("say that I love you", semMem, lingDb));

    EXPECT_EQ("Bonjour", operator_resolveCommand("dis bonjour", semMem, lingDb));
    EXPECT_EQ("Bonjour", operator_resolveCommand("Vas-y dis bonjour", semMem, lingDb));
    EXPECT_EQ("Je suis content.", operator_resolveCommand("dis je suis content", semMem, lingDb));
    EXPECT_EQ("Je suis sympa.", operator_resolveCommand("dis je suis sympa", semMem, lingDb));
    EXPECT_EQ("Tu es content.", operator_resolveCommand("dis que je suis content", semMem, lingDb));
    EXPECT_EQ("Je t'aime.", operator_resolveCommand("dis que tu m'aimes", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_resolveCommand_ask) {
    ...
    EXPECT_EQ("Qu'aimes-tu ?", operator_resolveCommand("ask me in French what I like", semMem, lingDb));
}
