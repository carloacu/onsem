...
TEST_F(SemanticReasonerGTests, operator_react_basic) {
    ...
    // Bad question detection
    {
        ...
        ONSEM_NOANSWER(
            operator_react("Parlant de politique", semMem, lingDb));    // don't react a sentence without subject
...

    {
        ...
        ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas qui est Mehdi.", operator_react("Qui est Mehdi ?", semMem, lingDb));
    }
    ...
    ONSEM_QUESTION_EQ("Je ne sais pas.\tTHEN\tQui es-tu ?", operator_react("qui suis je", semMem, lingDb));
    ...
    EXPECT_EQ(
        ...
        trackSemMemoryNotifications.getInfActions());
    ...
    EXPECT_EQ("", trackSemMemoryNotifications.getConditionToActions());
    const std::string standUpCommandStr =
        "\\" + resourceLabelForTests_cmd + "=ALRobotPosture.goToPosture(\"Stand\", 0.8)\\";
    ...
    EXPECT_EQ("Arise if I like chocolate!", trackSemMemoryNotifications.getConditionToActions());
    ONSEM_REMOVEALLCONDITIONS_EQ("Ok, je supprime toutes les conditions.",
                                 operator_react("Supprime toutes les conditions", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Oui, je sais que j'aime le chocolat.", operator_react("tu aimes le chocolat", semMem, lingDb));

    // permanent condition (introduced by "à chaque fois que") to an action
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
        "Ok, je me lèverai à chaque fois que Paul regarde à droite.",
    ...
    ONSEM_NOANSWER(
        operator_react("Paul regarde", semMem, lingDb));    // don't stand up, because "à droite" info is missing

    // condition when the trigger is contained in the condition
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
        "Ok, je dirai j'entends quelqu'un à chaque fois que j'entends quelqu'un.",
    ...
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
        "Ok, I will say my left hand is touched whenever it's touched.",
    ...
    const std::string raiseRightHandCommandStr =
        "\\" + resourceLabelForTests_cmd + "=semantic/animations/raise_right_hand.qianim\\";
    ...
    ONSEM_TEACHINGFEEDBACK_EQ(
        "Ok, to scratch my head means to say I am scratching my head.",
    ...
    ONSEM_BEHAVIOR_EQ("I am scratching my head.", operator_react("rub your head", semMem, lingDb));
    ONSEM_REMOVEALLCONDITIONS_EQ("Ok, I am removing all conditions.",
                                 operator_react("Remove all conditions", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("I am rubbing my head.", operator_react("rub your head", semMem, lingDb));

    // condition to a permanent actions
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
        "Ok, I will stand up and I will raise my right hand whenever I see you.",
    ...
    // say action
    {
        ONSEM_BEHAVIOR_EQ("Hello", operator_react("say hello", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Bonjour", operator_react("dis bonjour", semMem, lingDb));
        ...
        ONSEM_ANSWER_EQ("La blague d'aujourd'hui est " + jokeStr + ".",
                        operator_react("quel est la blague du jour", semMem, lingDb));
    ...
    {
        ONSEM_BEHAVIOR_EQ("Thanks", operator_react("say thanks", semMem, lingDb));
        ONSEM_TEACHINGFEEDBACK_EQ("Ok, to be nice is to say thanks.",
                                  operator_react("that is to be nice", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Thanks", operator_react("be nice", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Thanks", operator_react("I want you to be nice", semMem, lingDb));
    }
    ...
    // action labeling after execution of a sequence of actions
    {
        ...
        ONSEM_BEHAVIOR_EQ("I like you.\tTHEN\tWhat is your name?", operator_react("welcome", semMem, lingDb));
    }
    ...
    // action labeling after execution of a sequence of actions even more complex
    {
        ONSEM_BEHAVIOR_EQ("\"a1\"\tTHEN\t(\t\"a2\"\tAND\t\"a3\"\t)\tTHEN\t\"a4\"",
                          operator_react("Say \"a1\" then say \"a2\" and say \"a3\" then say \"a4\"", semMem, lingDb));
        ONSEM_TEACHINGFEEDBACK_EQ(
            "Ok, to become crazy is to say \"a1\", then to say \"a2\" and to say \"a3\" and then to say \"a4\".",
            operator_react("that's to become crazy", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ(
            "(\t\"a1\"\tTHEN\t(\t\"a2\"\tAND\t\"a3\"\t)\tTHEN\t\"a4\"\t)\tTHEN\tI finished to become crazy.",
    ...
    ONSEM_ANSWERNOTFOUND_EQ("I don't know what I eat.", operator_react("what do you eat ?", semMem, lingDb));
    ...
    ONSEM_ANSWER_EQ("Bonjour", operator_react("bonjour", semMem, lingDb));
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, je dirai salut à chaque fois que tu dis bonjour.",
                                       operator_react("quand je dis bonjour, dis salut", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Salut", operator_react("bonjour", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Salut", operator_react("bonjour", semMem, lingDb));
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, je ne ferai rien à chaque fois que tu dis bonjour.",
                                       operator_react("quand je dis bonjour, ne fais rien", semMem, lingDb));
    ONSEM_ANSWER_EQ("Bonjour", operator_react("bonjour", semMem, lingDb));
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ("Ok, I will ask what you can do for me whenever you look at me.",
                                       operator_react("when I look at you ask what can I do for you", semMem, lingDb));
    ...
    {
        ONSEM_ANSWERNOTFOUND_EQ("I don't know where Paul is.", operator_react("Where is Paul ?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Paul is in the bedroom.",
                        operator_react("Paul is in the bedroom. Where is Paul ?", semMem, lingDb));
    }
    ...
    // filter answers acording to a concept
    {
        ...
        ONSEM_ANSWER_EQ("Tes cheveux sont roux.", operator_react("de quelle couleur sont mes cheveux", semMem, lingDb));
        ONSEM_ANSWER_EQ("Your hairs are auburn.", operator_react("what color are my hairs", semMem, lingDb));
    }

    {
        const std::string lookRightCommandStr =
            "\\" + resourceLabelForTests_cmd + "=semantic/animations/look_right.qianim\\";
        operator_inform("to look right means " + lookRightCommandStr, semMem, lingDb);
        ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
            "Ok, je regarderai à droite à chaque fois que Paul est content.",
    ...
    // condition of knowing something
    {
        ...
        ONSEM_BEHAVIOR_EQ("Christophe", operator_react("hello", semMem, lingDb));
        ONSEM_REMOVEALLCONDITIONS_EQ("Ok, I am removing all conditions.",
                                     operator_react("Remove all conditions", semMem, lingDb));
    }
    ...
    // time answer
    {
        ...
        ONSEM_ANSWER_EQ("Oui, tu es content.", operator_react("est-ce que je suis content ?", semMem, lingDb));
    }
    ...
    // learn a command
    {
        ONSEM_BEHAVIORNOTFOUND_EQ("I can't salute.", operator_react("salute", semMem, lingDb));
        ONSEM_TEACHINGFEEDBACK_EQ("Ok, to salute means to say hello.",
                                  operator_react("to salute means to say hello", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Hello", operator_react("salute", semMem, lingDb));
        ...
        ONSEM_TEACHINGFEEDBACK_EQ(
            "Ok, to salute means to say hello and to raise the right arm.",
            operator_react("to salute means to say hello and raise the right arm", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Hello\tAND\t" + raiseRightHandCommandStr, operator_react("salute", semMem, lingDb));
        ...
        ONSEM_ANSWER_EQ("Pour saluer il faut dire salut homme", operator_react("Comment saluer ?", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Pour saluer il faut dire salut homme", operator_react("Dis comment saluer", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Hi man", operator_react("salute me", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("Paul smiled.", operator_react("Say Paul smiled", semMem, lingDb));
        ...
        ONSEM_BEHAVIOR_EQ("Paul smiled", operator_react("Do a test", semMem, lingDb));
    }
    ...
    // recall past experiences
    {
        ...
        ONSEM_ANSWER_EQ("Yes, you said Remy is happy.", operator_react("did I say that Remy is happy", semMem, lingDb));
        ...
        // ask about the time
        ONSEM_ANSWER_EQ("It was 11 seconds ago.", operator_react("when was it", semMem, lingDb));
        ...
        ONSEM_ANSWER_EQ("Paul found a pen.", operator_react("what did Paul find", semMem, lingDb));
        ...
        // in french
        ONSEM_ANSWER_EQ("J'ai répondu que Paul a trouvé un stylo.", operator_react("et puis", semMem, lingDb));

        // "and then", "and after that", ...
        ONSEM_ANSWER_EQ("You asked me if you said Paul found a pen.", operator_react("and then", semMem, lingDb));
        ONSEM_ANSWER_EQ("I answered yes you said Paul found a pen.", operator_react("and after that", semMem, lingDb));
    ...
    // merge with context
    {
        ONSEM_QUESTION_EQ("I don't know.\tTHEN\tWhat do you want?", operator_react("What do I want ?", semMem, lingDb));
        operator_inform_fromRobot("What do you want ?", semMem, lingDb);
        operator_mergeAndInform("to play soccer", semMem, lingDb);
        ONSEM_ANSWER_EQ("You want to play soccer.", operator_react("What do I want ?", semMem, lingDb));
    }
    ...
    // merge with context - incomplete sentences
    {
        ...
        ONSEM_QUESTION_EQ("When did you go to the cinema?", operator_react("cinema", semMem, lingDb));

        // not ok for sentences
        ONSEM_NOANSWER(operator_react("I went to the", semMem, lingDb));
        ONSEM_NOANSWER(operator_react("Jean is an old man", semMem, lingDb));
        static const std::string spinRightCommandStr =
            "\\" + resourceLabelForTests_cmd + "=semantic/animations/spin_right.qianim\\";
        ONSEM_NOANSWER(operator_react("I went to the", semMem, lingDb));
        ...
        ONSEM_NOANSWER(operator_react("I went to the", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ(spinRightCommandStr, operator_react("spin right", semMem, lingDb));
    }
    ...
    // positive sentiment feedbacks
    {
        ...
        ONSEM_FEEDBACK_EQ("I am happy to hear that.", operator_react("I am happy", semMem, lingDb));
    }

    // purpose answer
    {
        ONSEM_QUESTION_EQ(
            "When did Andrew climb in order to see what is on the top of the mountain?",
    ...
    {
        ONSEM_QUESTION_EQ("When did you work at Paris?", operator_react("I worked at Paris.", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("I thought the opposite.", operator_react("I didn't worked at Paris.", semMem, lingDb));
        ...
        ONSEM_FEEDBACK_EQ("Ok, you work at Issy.", operator_react("I work at Issy.", semMem, lingDb));
        ONSEM_ANSWER_EQ("Claire and your brother", operator_react("Who worked at Paris?", semMem, lingDb));
    ...
    {
        ONSEM_QUESTION_EQ("Où as-tu marché hier ?", operator_react("J’ai marché hier.", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Ok, tu as marché à Paris hier.", operator_react("à Paris", semMem, lingDb));
        ONSEM_ANSWER_EQ("Tu as marché à Paris.", operator_react("Où j'ai marché hier ?", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Oui, je sais que tu as marché hier.", operator_react("J’ai marché hier.", semMem, lingDb));
    }
    ...
    // time proactive question
    {
        ...
        ONSEM_FEEDBACK_EQ("Ok, tu as fait les courses dans le 10ème hier.", operator_react("hier", semMem, lingDb));
    ...
    {
        ONSEM_QUESTION_EQ("Quand as-tu couru ?", operator_react("J’ai couru.", semMem, lingDb));
        ONSEM_QUESTION_EQ("Où as-tu couru avant-hier ?", operator_react("avant-hier", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Ok, tu as couru à Paris avant-hier.", operator_react("à Paris", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Oui, je sais que tu as couru.", operator_react("J’ai couru.", semMem, lingDb));
    }
    ...
    // quantity question when the number is explicitly said
    {
        ...
        ONSEM_ANSWER_EQ("You have 2 children.", operator_react("How many children do I have?", semMem, lingDb));
        ONSEM_ANSWER_EQ("No, you have 2 children.", operator_react("Do I have 3 children?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Yes, you have 2 children.", operator_react("Do I have 2 children?", semMem, lingDb));
    }
    ...
    // quantity question when the number is not explicitly said
    {
        ...
        ONSEM_ANSWER_EQ("Donna bought 8 fruits.", operator_react("How many fruits did Donna buy?", semMem, lingDb));
        ONSEM_ANSWER_EQ("No, Donna bought 8 fruits.", operator_react("Did Donna buy 7 fruits?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Yes, Donna bought 8 fruits.", operator_react("Did Donna buy 8 fruits?", semMem, lingDb));
    }

    // "ça va" question
    {
        ONSEM_ANSWER_EQ("Oui, je vais bien. Merci", operator_react("ça va ?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Oui, je vais bien. Merci", operator_react("ca va ?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Je vais bien. Merci", operator_react("comment ça va ?", semMem, lingDb));
        ONSEM_ANSWER_EQ("Je vais bien. Merci", operator_react("comment ça va", semMem, lingDb));
    ...
    // condition -> information -> action
    {
        ...
        ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
            "Ok, Innocent est content si il sourit.",
            operator_react("si Innocent sourit, Innocent est content", semMem, lingDb));    // fix with "quand"
        ONSEM_BEHAVIOR_EQ("Youpis", operator_react("Innocent sourit", semMem, lingDb));
    }
    ...
    // do an action from the answer of a question
    {
        ...
        ONSEM_BEHAVIOR_EQ("I do a ballet spin.", operator_react("Do a ballet spin", semMem, lingDb));
        ONSEM_QUESTION_EQ("What must I do?", operator_react("Do", semMem, lingDb));
        ONSEM_BEHAVIOR_EQ("I do a ballet spin.", operator_react("A ballet spin", semMem, lingDb));
    }
    ...
    // "Qui c'est" question management
    {
        ...
        ONSEM_ANSWER_EQ("C'est ton ami.", operator_react("Qui c'est", semMem, lingDb));
        ONSEM_NOANSWER(operator_react("C'est", semMem, lingDb));
    }

    // reaction on yes/no answers
    {
        ONSEM_BEHAVIOR_EQ("Veux-tu manger ?", operator_react("Demande si je veux manger", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Ok, tu veux manger.", operator_react("Oui", semMem, lingDb));
    }
    ...
    {
        ONSEM_QUESTION_EQ("Why do you like everybody?", operator_react("I like everybody", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("You also look at me.", operator_react("Alexis looks at you", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Ok, you look at chocolate.", operator_react("I look at chocolate", semMem, lingDb));
    ...
    {
        ONSEM_NOANSWER(operator_react("Adrien-Marie Legendre, né le 18 septembre 1752 à Paris et mort le 10 janvier "
                                      "1833 à Auteuil, est un mathématicien français.",
                                      semMem,
                                      lingDb));
        ...
        ONSEM_FEEDBACK_EQ("Oui, je sais que tu deviens.", operator_react("je t'aime et devient", semMem, lingDb));
        ...
        ONSEM_ANSWER_EQ("Adrien-Marie Legendre", operator_react("qui est un mathématicien", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Adrien-Marie Legendre est aussi un mathématicien.",
                          operator_react("je suis un mathématicien", semMem, lingDb));
        ONSEM_FEEDBACK_EQ(
            "Oui, je sais qu'Adrien-Marie Legendre né à Paris est un mathématicien français.",
            operator_react("Adrien-Marie Legendre, né à Paris, est un mathématicien français.", semMem, lingDb));
        ONSEM_ANSWER_EQ("Toi et Adrien-Marie Legendre", operator_react("qui est un mathématicien", semMem, lingDb));
        ...
        ONSEM_NOANSWER(
            operator_react("Deee est une actrice de cinéma et de théâtre, chanteuse et compositrice. "
                           "Elle est la seule comédienne allemande à avoir tenu un rôle en vedette à Broadway.",
                           semMem,
                           lingDb));
        ONSEM_NOANSWER(operator_react("Initialement cuisinier, il est devenu chanteur", semMem, lingDb));
        ONSEM_ANSWER_EQ("Deee", operator_react("Who is a singer", semMem, lingDb));
        ...
        ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas qui est Paul Io.", operator_react("Qui est Paul Io ?", semMem, lingDb));
        ONSEM_FEEDBACK_EQ("Deee is also a singer.", operator_react("Malena Ernman is a singer", semMem, lingDb));
        ...
        ONSEM_NOANSWER(operator_react(
            "Barack Obama, né le 4 août 1961 à Honolulu (Hawaï), est un homme d'État américain.", semMem, lingDb));
        ONSEM_NOANSWER(operator_react(
            "Francky Vandendriessche est devenu un entraîneur des gardiens à KV Courtrai.", semMem, lingDb));
        ONSEM_NOANSWER(operator_react("Reese Witherspoon, Michelle Obama, Hillary Clinton et Andrea Jung Andrea Jung "
                                      "est une femme d'affaires américaine",
                                      semMem,
                                      lingDb));
        ...
        ONSEM_FEEDBACK_EQ("Oui, je connais Lady Gaga.", operator_react("tu connais Lady Gaga", semMem, lingDb));
        ONSEM_ANSWER_EQ("C'est une chanteuse.", operator_react("qui c'est", semMem, lingDb));
        ONSEM_ANSWER_EQ("Barack Obama", operator_react("Qui est né le 4 août 1961 ?", semMem, lingDb));
    }

    // merge similar answers
    {
        ONSEM_QUESTION_EQ("Quand feras-tu des courses ?", operator_react("je vais faire des courses", semMem, lingDb));
        ONSEM_QUESTION_EQ("Où feras-tu des courses demain ?", operator_react("demain", semMem, lingDb));
        ONSEM_ANSWER_EQ("Tu feras des courses demain.", operator_react("qu'est-ce que je vais faire", semMem, lingDb));
    }
    ...
    ONSEM_ANSWER_EQ("Barack Obama", operator_react("Qui est né le 4 août 1961 ?", semMemBinary, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_useActionAsVariables) {
    ...
    ONSEM_ANSWER_EQ("To welcome is to say nice to meet you.", operator_react("what is to welcome", semMem, lingDb));

    EXPECT_EQ(std::vector<std::string>{"To say nice to meet you."}, operator_get("to welcome", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("Nice to meet you", operator_react("welcome", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("Nice to meet you\tTHEN\tHello", operator_react("welcome then say hello", semMem, lingDb));

    ONSEM_TEACHINGFEEDBACK_EQ("Ok, to welcome is to say nice to meet you and then to say hello.",
                              operator_react("that is to welcome", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("Nice to meet you\tTHEN\tHello", operator_react("welcome", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_fromNominalGroups) {
    ...
    ONSEM_QUESTION_EQ(
        "Qui est la femme dont je parle ?",
        operator_react("La femme dont tu parles.", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ...
    ONSEM_ANSWER_EQ(
        "La femme dont je parle est Eva.",
        operator_react("La femme dont tu parles.", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));

    ONSEM_QUESTION_EQ("Qu'est-ce que pizza ?",
                      operator_react("pizza", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ...
    ONSEM_ANSWER_EQ("Une pizza est un plat.",
                    operator_react("une pizza", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ONSEM_ANSWER_EQ("La pizza est un plat.",
                    operator_react("la pizza", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
    ONSEM_ANSWER_EQ("Une pizza est un plat.", operator_react("qu'est-ce qu'une pizza", semMem, lingDb));

    ONSEM_QUESTION_EQ("Qu'est-ce qu'une table ?",
                      operator_react("une table", semMem, lingDb, SemanticLanguageEnum::UNKNOWN, &canReactToANoun));
...
TEST_F(SemanticReasonerGTests, operator_react_saveAnswerToObjectQuestions) {
    ...
    ONSEM_ANSWER_EQ("Tu aimes le chocolat.", operator_react("qu'est-ce que j'aime ?", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_checkReactionType) {
    ...
    ONSEM_BEHAVIORNOTFOUND_EQ("Je ne sais pas ouvrir la bouche.",
                              operator_react("si on touche ta tête, ouvre la bouche", semMem, lingDb));
    ONSEM_ANSWERNOTFOUND_EQ("Je ne sais pas quel est le sens de la vie.",
                            operator_react("quel est le sens de la vie ?", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_react_checkReferences) {
    ...
    operator_mergeAndInform(
        "Cesar has a big empire because he wins a lot of battles.", semMem, lingDb, {"History book of Cesar"});

    compareWithRef("Yes, Cesar likes Egypt.",
                   {"History book", "Asterix"},
                   operator_react("Does Cesar like Egypt?", semMem, lingDb));
    compareWithRef(
        "Cesar likes Egypt.", {"History book", "Asterix"}, operator_react("What does Cesar like ?", semMem, lingDb));
    compareWithRef("Cesar has a big empire because he wins a battles lot.",
                   {"History book of Cesar"},
                   operator_react("Why Cesar has a big empire ?", semMem, lingDb));
    compareWithRef("Cesar", {"History book", "Asterix"}, operator_react("Who likes Egypt ?", semMem, lingDb));
    compareWithRef("Yes, I know I am a robot.", {"Own body"}, operator_react("You are a robot", semMem, lingDb));
    compareWithRef("No, I am a robot.", {"Own body"}, operator_react("You are not a robot", semMem, lingDb));
    compareWithRef("Cesar also has a big empire.",
                   {"History book of Cesar"},
                   operator_react("Dede has a big empire", semMem, lingDb));
    compareWithRef("Spiderman is Paul.", {"Spiderman movie"}, operator_react("Who is Spiderman?", semMem, lingDb));
    compareWithRef("Oui, Spiderman est Paul.",
                   {"Spiderman movie"},
    ...
    compareWithRef(
        "Cesar likes Egypt.", {"History book", "Asterix"}, operator_react("What does Cesar like ?", semMem2, lingDb));
    // 2 levels of depth
    {
        SemanticMemory semMemTopLevel;
        semMemTopLevel.memBloc.subBlockPtr = &semMem2.memBloc;
        compareWithRef("Cesar likes Egypt.",
                       {"History book", "Asterix"},
...
TEST_F(SemanticReasonerGTests, operator_react_occurrenceOrderOfPastMemories) {
    ...
    ONSEM_ANSWER_EQ("You said you like me.", operator_react("what did I say for the first time", semMem, lingDb));

    // repeat learning
    ONSEM_BEHAVIOR_EQ("You said you like me.", operator_react("say the last thing that you said", semMem, lingDb));
    operator_inform("to repeat means to say the last thing that you said", semMem, lingDb);
    ONSEM_BEHAVIOR_EQ("You said you like me.", operator_react("repeat", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("say hi", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("repeat", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("repeat", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("repeat please", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("please repeat", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("alright repeat", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Hi", operator_react("Alright repeat", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_cause) {
    ...
    const std::string urlStr1 = "\\" + resourceLabelForTests_url + "=https://www.youtube.com/watch?v=4yQil_1ZA98\\";
    ...
    const std::string urlStr2 = "\\" + resourceLabelForTests_url + "=https://www.youtube.com/watch?v=4GZNoxrKZUQ\\";
    ...
    const std::string urlStr3 = "\\" + resourceLabelForTests_url + "=https://www.youtube.com/watch?v=ZYCV4DmWbkw\\";
    ...
    const std::string urlStr4 = "\\" + resourceLabelForTests_url + "=https://www.youtube.com/watch?v=zMyaKqtu0TE\\";
    ...
    ONSEM_ANSWER_EQ("Oui, tu es content.", operator_react("je suis content ?", semMem, lingDb));
    ONSEM_ANSWER_EQ("Parce que \tTHEN\t" + urlStr4, operator_react("pourquoi ?", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_sayThanks) {
    ...
    ONSEM_FEEDBACK_EQ("Thanks, you are kind.", operator_react("I like you", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Merci, tu es gentil.", operator_react("Je t'aime", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Merci, tu es gentil.", operator_react("Je t'aime", semMem, lingDb));

    operator_inform("You like everybody", semMem, lingDb);
    ONSEM_FEEDBACK_EQ("Thanks, I also like you.", operator_react("I like you", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Merci, je t'aime aussi.", operator_react("Je t'aime", semMem, lingDb));

    ONSEM_FEEDBACK_EQ("Thanks, you are kind.", operator_react("You are cool", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Merci, tu es gentil.", operator_react("tu es cool", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_sayYouAreWelcome) {
    ...
    ONSEM_FEEDBACK_EQ("You're welcome", operator_react("Thanks", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("You're welcome", operator_react("thanks.", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("You're welcome", operator_react("Thank you", semMem, lingDb));

    ONSEM_FEEDBACK_EQ("De rien", operator_react("Merci", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("De rien", operator_react("merci.", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_onInterjections) {
    ...
    ONSEM_FEEDBACK_EQ("Nice, you like that.", operator_react("It's nice", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Cool, you like that.", operator_react("it's cool", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Awesome, you like that.", operator_react("It's awesome", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("It's not nice", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("it's not cool", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("It's not awesome", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Super, tu l'aimes.", operator_react("C'est super", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Désolé, j'essaierai de m'améliorer.", operator_react("C'est pas super", semMem, lingDb));

    ONSEM_FEEDBACK_EQ("Nice, you like that.", operator_react("Nice", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Cool, you like that.", operator_react("cool", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Awesome, you like that.", operator_react("Awesome", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("Not nice", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("not cool", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("Not awesome", semMem, lingDb));
    ONSEM_FEEDBACK_EQ(sorryIWillTryToImproveMyslefEnglish, operator_react("F*cking jackass", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("You are the kind with me.", operator_react("Awesome", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_objectsMergeAndRemoval) {
    ...
    ONSEM_ANSWER_EQ("You see a moving object.", operator_react("what do I see ?", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_react_evenIfTheIsASpellingMistake) {
    ...
    ONSEM_ANSWER_EQ("Alexis looks at a chair.", operator_react("what Alexis look at?", semMem, lingDb));
    ONSEM_ANSWER_EQ("Alexis looks at a chair.", operator_react("what Alexis looks at?", semMem, lingDb));
    ONSEM_ANSWER_EQ(
        "Alexis looks at a chair.",
...
TEST_F(SemanticReasonerGTests, operator_react_choice) {
    ...
    ONSEM_QUESTION_EQ("I don't know.\tTHEN\tDo you like chocolate or banana?",
...
TEST_F(SemanticReasonerGTests, operator_react_abilityQuestion) {
    ...
    memoryOperation::learnSayCommand(semMem, lingDb);
    ONSEM_QUESTION_EQ("Yes, I can.\tTHEN\tDo you want me to say something now?",
                      operator_react("can you say something?", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Something", operator_react("yes", semMem, lingDb));
    ONSEM_QUESTION_EQ("Oui, je peux.\tTHEN\tVeux-tu que je dise bonjour maintenant ?",
                      operator_react("sais-tu dire bonjour ?", semMem, lingDb));
    ...
    ONSEM_QUESTION_EQ("No, I can't but you can teach me.\tTHEN\tDo you want to know how?",
                      operator_react("can you sing?", semMem, lingDb));
    ONSEM_ANSWER_EQ("For example, you can tell me to sing is to say I am singing.",
                    operator_react("yes", semMem, lingDb));
    ONSEM_TEACHINGFEEDBACK_EQ("Ok, to sing is to say I am singing.",
                              operator_react("to sing is to say I am singing", semMem, lingDb));
    ...
    ONSEM_QUESTION_EQ("Non, je ne peux pas mais tu peux m'apprendre.\tTHEN\tVeux-tu savoir comment ?",
                      operator_react("peux-tu courir ?", semMem, lingDb));
    ONSEM_ANSWER_EQ("Par exemple, tu peux me dire que courir c'est dire je cours.",
                    operator_react("oui", semMem, lingDb));
    ONSEM_TEACHINGFEEDBACK_EQ("Ok, courir c'est dire je cours.",
                              operator_react("courir c'est dire je cours.", semMem, lingDb));
    ONSEM_QUESTION_EQ("Oui, je peux.\tTHEN\tVeux-tu que je coure maintenant ?",
                      operator_react("peux-tu courir ?", semMem, lingDb));
    ...
    ONSEM_ANSWER_EQ("I can say something, ask something, repeat, sing, run and crouch.",
                    operator_react("what can you do?", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_linkUserToAnAlreadyKnowAgent) {
    ...
    ONSEM_QUESTION_EQ("I don't know.\tTHEN\tDo you like to swim?",
                      operator_react("do I like to swim?", semMem, lingDb));
    ...
    ONSEM_QUESTION_EQ("I don't know.\tTHEN\tAre you a goalkeeper?",
                      operator_react("am I a goalkeeper?", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_react_learnAnAction) {
    ...
    ONSEM_EXTERNALTEACHINGREQUEST_EQ("\\learn=#en_US#To salute.\\",
                                     operator_react("I will teach you how to salute", semMem, lingDb));
    ONSEM_EXTERNALTEACHINGREQUEST_EQ("\\learn=#en_US#To swap my ass.\\",
                                     operator_react("I'll teach you how to swap your ass", semMem, lingDb));
    ONSEM_EXTERNALTEACHINGREQUEST_EQ("\\learn=#en_US#To jump.\\",
                                     operator_react("I am going to teach you how to jump", semMem, lingDb));
    ONSEM_EXTERNALTEACHINGREQUEST_EQ("\\learn=#en_US#To go forward.\\",
                                     operator_react("I am going to teach you how to go forward", semMem, lingDb));
    ONSEM_EXTERNALTEACHINGREQUEST_EQ("\\learn=#fr_FR#Saluer.\\",
                                     operator_react("je vais t'apprendre à saluer", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_conditionalActions) {
    ...
    ONSEM_BEHAVIORNOTFOUND_EQ("Je ne sais pas chanter.", operator_react("chante", semMem, lingDb));
    ONSEM_BEHAVIORNOTFOUND_EQ("Je ne sais pas chanter.", operator_react("chante si tu me vois", semMem, lingDb));
    ...
    ONSEM_BEHAVIOR_EQ("Bonjour", operator_react("tu me vois", semMem, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_react_answerOfGeneratedQuestions) {
    ...
    ONSEM_BEHAVIOR_EQ("Es-tu sympa ?", operator_react("demande-moi si je suis sympa", semMem, lingDb));
    ONSEM_QUESTION_EQ("Je suis content de l'entendre.\tTHEN\tPourquoi es-tu sympa ?",
                      operator_react("oui toujours", semMem, lingDb));

    // + translation
    ONSEM_BEHAVIOR_EQ("Es-tu content ?", operator_react("ask me in French if I am happy", semMem, lingDb));
    ONSEM_QUESTION_EQ("Je suis content de l'entendre.\tTHEN\tPourquoi es-tu content ?",
                      operator_react("oui", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_similarities) {
    ...
    ONSEM_FEEDBACK_EQ("Damien est aussi né le 3 avril.", operator_react("je suis né le 3 avril 1995.", semMem, lingDb));
    operator_inform_fromRobot("Quand es-tu né ?", semMem, lingDb);
    ONSEM_FEEDBACK_EQ("Damien est aussi né en avril 1978.", operator_react("5 avril 1978.", semMem, lingDb));
    operator_inform_fromRobot("Quand es-tu né ?", semMem, lingDb);
    ONSEM_FEEDBACK_EQ("Damien est aussi né en avril.", operator_react("le 5 avril 1995.", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_changeTheUser_namesSaving) {
    ...
    ONSEM_FEEDBACK_EQ("Enchanté Paul", operator_react("je m'appelle Paul", semMem, lingDb));
    ONSEM_ANSWER_EQ("Tu t'appelles Paul.", operator_react("comment je m'appelle ?", semMem, lingDb));
    ONSEM_FEEDBACK_EQ("Enchanté Pierre", operator_react("je m'appelle Pierre", semMem, lingDb));
    ONSEM_ANSWER_EQ("Tu t'appelles Pierre.", operator_react("comment je m'appelle ?", semMem, lingDb));
    ...
    ONSEM_FEEDBACK_EQ("Nice to meet you André", operator_react("André", semMem, lingDb));
    ONSEM_ANSWER_EQ("Tu t'appelles André.", operator_react("comment je m'appelle ?", semMem, lingDb));
    semMem.setCurrUserId("1");
    ONSEM_ANSWER_EQ("Tu t'appelles Pierre.", operator_react("comment je m'appelle ?", semMem, lingDb));
    ...
    ONSEM_ANSWER_EQ("Ton père s'appelle Philippe.", operator_react("Comment s'appelle mon père ?", semMem, lingDb));
    // check that it still work after a serialization
    SemanticMemory semMem2;
    copyMemory(semMem2, semMem, lingDb);
    ONSEM_ANSWER_EQ("Ton père s'appelle Philippe.", operator_react("Comment s'appelle mon père ?", semMem2, lingDb));
...
TEST_F(SemanticReasonerGTests, operator_react_changeTheUser_genderHandling) {
    ...
    semMem.setCurrUserId("2");
    ONSEM_BEHAVIOR_EQ("Aaa est un chanteur.", operator_react("say in French that Aaa is a singer", semMem, lingDb));
    operator_mergeAndInform("Aaa is a girl", semMem, lingDb);
    ONSEM_BEHAVIOR_EQ("Aaa est une chanteuse.", operator_react("say in French that Aaa is a singer", semMem, lingDb));
    ...
    ONSEM_FEEDBACK_EQ("Aaa is also a girl.", operator_react("Bb is a girl", semMem, lingDb));
    ONSEM_BEHAVIOR_EQ("Aaa et Bb sont des chanteuses.",
                      operator_react("say in French that Aaa and Bb are singers", semMem, lingDb));

    ONSEM_BEHAVIOR_EQ("Aaa est une artiste.", operator_react("say in French that Aaa is an artist", semMem, lingDb));
    operator_mergeAndInform("Aaa is a boy", semMem, lingDb);
    ONSEM_BEHAVIOR_EQ("Aaa est un artiste.", operator_react("say in French that Aaa is an artist", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_changeTheUser_auestionQboutUser) {
    ...
    ONSEM_ANSWER_EQ("Aaa likes chocolate.", operator_react("what does Aaa like?", semMem, lingDb));
    ONSEM_ANSWER_EQ(
        "Aaa likes chocolate.",
...
TEST_F(SemanticReasonerGTests, operator_react_changeTheUser_askAboutWhatAnotherUserWillDo) {
    ...
    ONSEM_NOTIFYSOMETHINGWILLBEDONE_EQ(
        "Ok, je demanderai à Erwan en anglais ce qu'Erwan fera ce soir.",
        operator_react("demande à Erwan en anglais ce qu'il fera ce soir", semMem, lingDb));
    {
        ...
        ONSEM_BEHAVIOR_EQ("What will you do this evening?",
                          operator_react_fromSemExp(
                              std::move(semExpsForThisSpecificUser.front()), semMem, lingDb, semMem.defaultLanguage));
    }
...
TEST_F(SemanticReasonerGTests, operator_react_handleEverythingConcept) {
    ...
    ONSEM_ANSWER_EQ("You", operator_react("who likes Paul?", semMem, lingDb));
}
...
TEST_F(SemanticReasonerGTests, operator_react_subBlockPtr) {
    ...
    ONSEM_ANSWERNOTFOUND_EQ("I don't know what Paul likes.", operator_react("what does Paul like?", semMem, lingDb));
    ...
    // 2 levels of depth
    {
        ...
        ONSEM_ANSWER_EQ("Yes, Paul likes banana.", operator_react("Does Paul like banana?", semMemTopLevel, lingDb));
    }

    ONSEM_ANSWER_EQ("Paul likes banana.", operator_react("what does Paul like?", semMem, lingDb));
    operator_mergeAndInform("Paul likes bear", semMem, lingDb);
    ONSEM_ANSWER_EQ("Paul likes banana and the bear.", operator_react("what does Paul like?", semMem, lingDb));
