...
struct TrackStateForTests {
    ...
    ~TrackStateForTests() { connection.disconnect(); }
...
void operator_track(std::shared_ptr<TrackStateForTests>& pTrackState, const TextProcessingContext& pContext) {
    auto semExp = converter::textToSemExp(pTrackState->text, pContext, pTrackState->lingDb, true);
    memoryOperation::track(
        pTrackState->semanticMemory, std::move(semExp), pTrackState->semTracker, pTrackState->lingDb);
    ...
    pTrackState->connection = pTrackState->semTracker->val.connect([pTrackState](
        ...
        converter::semExpToText(
            newResStr, pSemExp->clone(), outContext, false, pTrackState->semanticMemory, pTrackState->lingDb, nullptr);
    });
}

void operator_untrack(std::shared_ptr<TrackStateForTests>& pTrackState,
                      const linguistics::LinguisticDatabase& pLingDb) {
    memoryOperation::untrack(pTrackState->semanticMemory.memBloc, pTrackState->semTracker, pLingDb, nullptr);
}
...
TEST_F(SemanticReasonerGTests, operator_track_basic) {
    ...
    auto objectTracking = std::make_shared<TrackStateForTests>("what do you eat ?", semanticMemory, lingDb);
    operator_track(objectTracking, fromUser);
    auto timeTracking = std::make_shared<TrackStateForTests>("when do you eat ?", semanticMemory, lingDb);
    operator_track(timeTracking, fromUser);
    auto yesOrNoTracking = std::make_shared<TrackStateForTests>("Do you eat chocolate ?", semanticMemory, lingDb);
    operator_track(yesOrNoTracking, fromUser);
    auto affirmationTracking = std::make_shared<TrackStateForTests>("You eat chocolate", semanticMemory, lingDb);
...
TEST_F(SemanticReasonerGTests, operator_track_list) {
    ...
    auto objectTracking = std::make_shared<TrackStateForTests>("what do you eat ?", semanticMemory, lingDb);
    operator_track(objectTracking, fromUser);
    auto yesOrNoTracking =
        std::make_shared<TrackStateForTests>("you eat chocolate and lettuce?", semanticMemory, lingDb);
    operator_track(yesOrNoTracking, fromUser);
    auto affirmationTracking =
        std::make_shared<TrackStateForTests>("you eat chocolate and lettuce", semanticMemory, lingDb);
    operator_track(affirmationTracking, fromUser);
    auto clearTrackersResults = [=] {
        objectTracking->resultsStr.clear();
...
TEST_F(SemanticReasonerGTests, operator_track_advanced) {
    ...
    auto affirmationTracking = std::make_shared<TrackStateForTests>("I don't like chocolate", semanticMemory, lingDb);
    operator_track(affirmationTracking, fromUser);
    auto yesOrNoTracking = std::make_shared<TrackStateForTests>("do I like chocolate", semanticMemory, lingDb);
    operator_track(yesOrNoTracking, fromUser);
    auto leftHandTracking =
        std::make_shared<TrackStateForTests>("your left hand is not touched", semanticMemory, lingDb);
    ...
    try {
        ...
        auto idExp2 = operator_inform(
            "somebody touches your left hand", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        EXPECT_EQ(constant::noResult, affirmationTracking->getResult());
        EXPECT_EQ(constant::noResult, yesOrNoTracking->getResult());
        EXPECT_EQ(constant::noResult, leftHandTracking->getResult());
...
TEST_F(SemanticReasonerGTests, operator_track_seePeople) {
    ...
    auto nobodyTracking = std::make_shared<TrackStateForTests>("I don't see anybody", semanticMemory, lingDb);
    auto onePersonTracking = std::make_shared<TrackStateForTests>("I see one person", semanticMemory, lingDb);
    auto twoPeopleTracking = std::make_shared<TrackStateForTests>("I see 2 people", semanticMemory, lingDb);
    auto threePeopleTracking = std::make_shared<TrackStateForTests>("I see 3 people", semanticMemory, lingDb);
    ...
    try {
        ...
        auto idExp1 =
            operator_inform_fromRobot("I see no person", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        ...
        clearTrackersResults();
        auto idExp2 =
            operator_inform_fromRobot("I see one person", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        ...
        EXPECT_EQ(constant::noResult, threePeopleTracking->getResult());
        clearTrackersResults();
        idExp1 = operator_inform_fromRobot(
            "I don't see you and I see 2 people", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp2, lingDb, &axiomToConditionCurrentState);
        ONSEM_ANSWER_EQ("I see 2 people.", operator_answer("How many people do you see?", semanticMemory, lingDb));
        EXPECT_EQ(constant::noResult, nobodyTracking->getResult());
        EXPECT_EQ(constant::noResult, onePersonTracking->getResult());
        EXPECT_EQ("", twoPeopleTracking->getResult());
        EXPECT_EQ(constant::noResult, threePeopleTracking->getResult());
        clearTrackersResults();
        idExp2 = operator_inform_fromRobot(
            "I see you and I see 0 other people", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp1, lingDb, &axiomToConditionCurrentState);
        ONSEM_ANSWER_EQ("I see one person.", operator_answer("How many people do you see?", semanticMemory, lingDb));
        ...
        clearTrackersResults();
        idExp1 = operator_informAxiom_fromRobot(
            "I see you and I see another person", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp2, lingDb, &axiomToConditionCurrentState);
        ONSEM_ANSWER_EQ("I see 2 people.", operator_answer("How many people do you see?", semanticMemory, lingDb));
        ONSEM_FALSE(operator_check(nobodyTracking->text, semanticMemory, lingDb, fromRobot));
        ...
        EXPECT_EQ(constant::noResult, threePeopleTracking->getResult());
        clearTrackersResults();
        idExp2 = operator_informAxiom_fromRobot(
            "I see you and I see 2 other people", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp1, lingDb, &axiomToConditionCurrentState);
        EXPECT_EQ(constant::noResult, nobodyTracking->getResult());
        EXPECT_EQ(constant::noResult, onePersonTracking->getResult());
        EXPECT_EQ(constant::noResult, twoPeopleTracking->getResult());
        EXPECT_EQ("", threePeopleTracking->getResult());
        clearTrackersResults();
        operator_inform_fromRobot("I don't see you and I see 0 people",
                                  semanticMemory,
                                  lingDb,
                                  {},
                                  &axiomToConditionCurrentState);    // contrary to an assert so nothing is raised
        EXPECT_EQ(constant::noResult, nobodyTracking->getResult());
        EXPECT_EQ(constant::noResult, onePersonTracking->getResult());
        EXPECT_EQ(constant::noResult, twoPeopleTracking->getResult());
        EXPECT_EQ(constant::noResult, threePeopleTracking->getResult());
        idExp1 = operator_informAxiom_fromRobot(
            "I don't see you and I see 0 people", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp2, lingDb, &axiomToConditionCurrentState);
        EXPECT_EQ("", nobodyTracking->getResult());
        EXPECT_EQ(constant::noResult, onePersonTracking->getResult());
        EXPECT_EQ("", twoPeopleTracking->getResult());
        ...
        clearTrackersResults();
        idExp2 = operator_informAxiom_fromRobot(
            "I don't see you and I see one person", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp1, lingDb, &axiomToConditionCurrentState);
        EXPECT_EQ(constant::noResult, nobodyTracking->getResult());
        EXPECT_EQ("", onePersonTracking->getResult());
        EXPECT_EQ(constant::noResult, twoPeopleTracking->getResult());
        ...
        clearTrackersResults();
        idExp1 = operator_informAxiom_fromRobot(
            "I see you and I see 1 other people", semanticMemory, lingDb, {}, &axiomToConditionCurrentState);
        semanticMemory.memBloc.removeExpression(*idExp2, lingDb, &axiomToConditionCurrentState);
        EXPECT_EQ(constant::noResult, nobodyTracking->getResult());
        EXPECT_EQ(constant::noResult, onePersonTracking->getResult());
        EXPECT_EQ("", twoPeopleTracking->getResult());
