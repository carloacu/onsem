...
SemanticExpressionCategory operator_categorize(const std::string& pText,
...
TEST_F(SemanticReasonerGTests, operator_categorize_basic) {
    ...
    EXPECT_EQ(SemanticExpressionCategory::EXTERNALTEACHING,
              operator_categorize("I will teach you how to salute", lingDb));
    EXPECT_EQ(SemanticExpressionCategory::EXTERNALTEACHING,
              operator_categorize("Je vais t'apprendre à saluer", lingDb));
    ...
    EXPECT_EQ(SemanticExpressionCategory::CONDITION,
              operator_categorize("if it's raining then it's a bad weather", lingDb));
    EXPECT_EQ(SemanticExpressionCategory::CONDITION,
              operator_categorize("Si il pleut alors on ne va pas sortir.", lingDb));

    EXPECT_EQ(SemanticExpressionCategory::CONDITIONTOCOMMAND,
              operator_categorize("if it's raining then say that it's raining", lingDb));
    EXPECT_EQ(SemanticExpressionCategory::CONDITIONTOCOMMAND,
              operator_categorize("whenever your left hand is touched sleep", lingDb));
    EXPECT_EQ(SemanticExpressionCategory::CONDITIONTOCOMMAND,
              operator_categorize("Si tu vois quelqu'un dis bonjour", lingDb));
...
TEST_F(SemanticReasonerGTests, operator_categorize_orderNotAtImperativeForm) {
    ...
    EXPECT_EQ(SemanticExpressionCategory::AFFIRMATION,
              operator_categorize("I want to know how I can teach you to smile", lingDb));
