...
std::shared_ptr<ExpressionWithLinks> operator_inform_withTextProc(
    ...
    auto semExp = converter::textToContextualSemExp(
        pText, pTextProcContext, SemanticSourceEnum::ASR, pLingDb, &pReferences, std::move(pAgentWeAreTalkingAbout));
    memoryOperation::resolveAgentAccordingToTheContext(semExp, pSemanticMemory, pLingDb);
    return memoryOperation::inform(
        std::move(semExp), pSemanticMemory, pLingDb, nullptr, pAxiomToConditionCurrentStatePtr);
}
...
std::shared_ptr<ExpressionWithLinks> operator_inform(
    ...
    return operator_inform_withTextProc(
        pText, pSemanticMemory, pLingDb, pReferences, pAxiomToConditionCurrentStatePtr, inContext);
}

std::shared_ptr<ExpressionWithLinks> operator_inform_fromRobot(
    const std::string& pText,
    SemanticMemory& pSemanticMemory,
    const linguistics::LinguisticDatabase& pLingDb,
    const std::list<std::string>& pReferences,
...
std::shared_ptr<ExpressionWithLinks> operator_informAxiom_fromRobot(
    ...
    memoryOperation::resolveAgentAccordingToTheContext(semExp, pSemanticMemory, pLingDb);
    return memoryOperation::informAxiom(
        std::move(semExp), pSemanticMemory, pLingDb, nullptr, pAxiomToConditionCurrentStatePtr);
}

std::shared_ptr<ExpressionWithLinks> operator_addFallback(const std::string& pText,
                                                          SemanticMemory& pSemanticMemory,
                                                          const linguistics::LinguisticDatabase& pLingDb) {
    auto semExp = converter::textToContextualSemExp(
        pText,
...
std::shared_ptr<ExpressionWithLinks> operator_inform_withAgentNameFilter(
    ...
    auto semExp = converter::textToContextualSemExp(pText,
                                                    inContext,
                                                    SemanticSourceEnum::ASR,
                                                    pLingDb,
                                                    &references,
...
TEST_F(SemanticReasonerGTests, operator_informWithAnnotations) {
...

    {
        ...
        annExp->annotations.emplace(GrammaticalType::TIME, converter::textToSemExp("Paul leaves", inContext, lingDb));
