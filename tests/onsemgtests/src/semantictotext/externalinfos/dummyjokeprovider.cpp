    ...
    if (itParam != pParams.end()) {
        ...
        if (itAttrName != itParam->second.end()) {
            ...
            if (grdExp != nullptr) {
                ...
                if (timeGrd != nullptr && timeGrd->date.day == 1 && timeGrd->date.month == 1
                    && timeGrd->date.year == 2000) {
                    if (pLanguageType == SemanticLanguageEnum::ENGLISH) {
                        return mystd::unique_propagate_const<UniqueSemanticExpression>(
                            std::make_unique<GroundedExpression>(
                                std::make_unique<SemanticTextGrounding>(englishJoke(), SemanticLanguageEnum::ENGLISH)));
                    } else if (pLanguageType == SemanticLanguageEnum::FRENCH) {
                        return mystd::unique_propagate_const<UniqueSemanticExpression>(
                            std::make_unique<GroundedExpression>(
                                std::make_unique<SemanticTextGrounding>(frenchJoke(), SemanticLanguageEnum::FRENCH)));
                    }
                }
            }
        }
