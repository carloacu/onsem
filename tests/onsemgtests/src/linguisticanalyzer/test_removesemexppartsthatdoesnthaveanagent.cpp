...
namespace {
std::string _removesemexppartsthatdoesnthaveanagent(const std::string& pText,
...
TEST_F(SemanticReasonerGTests, removesemexppartsthatdoesnthaveanagent) {
    ...
    EXPECT_EQ("",
              _removesemexppartsthatdoesnthaveanagent("Toto est content.", "Paul Ftrer", semMem, frLanguage, lingDb));
    EXPECT_EQ("Paul Ftrer est dans la cuisine.",
              _removesemexppartsthatdoesnthaveanagent(
                  "Paul Ftrer est dans la cuisine. Toto est content.", "Paul Ftrer", semMem, frLanguage, lingDb));
    EXPECT_EQ(
        "Paul Ftrer est dans la cuisine.",
        _removesemexppartsthatdoesnthaveanagent(
            "Paul Ftrer est dans la cuisine. Toto est content.", "Paul Ftrer (cuisinier)", semMem, frLanguage, lingDb));
}
