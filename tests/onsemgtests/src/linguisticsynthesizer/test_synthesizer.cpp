...
TEST_F(SemanticReasonerGTests, infinitiveVerb) {
    ...
    converter::semExpToText(synthesized, std::move(expression), textContext, false, memory, lingDb, nullptr);
...
TEST_F(SemanticReasonerGTests, simpleSentences) {
    ...
    EXPECT_EQ("Robert walks on the street.",
              reformulate("Robert walks on the street.", semMem, lingDb, SemanticLanguageEnum::ENGLISH));
    EXPECT_EQ("Robert looks at you.",
              reformulate("Robert looks at me.", semMem, lingDb, SemanticLanguageEnum::ENGLISH));
    ...
    EXPECT_EQ("Eminem de son vrai nom Marshall Bruce Mathers III né le 17 octobre 1972 à Saint Joseph dans l'État du "
              "Missouri stylisé souvent EMINƎM est un auteur-compositeur-interprète de rap américain, il est également "
              "producteur et il est acteur. Il est aussi membre du groupe D12 dont il est le cofondateur.",
              reformulate(
                  "Eminem (souvent stylisé EMINƎM), de son vrai nom Marshall Bruce Mathers III, né le 17 octobre 1972 "
                  "à Saint Joseph dans l'État du Missouri, est un auteur-compositeur-interprète de rap américain, "
                  "également producteur et acteur. Il est aussi membre du groupe D12 dont il est le cofondateur",
                  semMem,
                  lingDb,
