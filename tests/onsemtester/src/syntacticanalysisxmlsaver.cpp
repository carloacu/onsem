...
void _saveTheConcepts(boost::property_tree::ptree& pTree, const linguistics::Token& pCurrToken) {
...
void _saveText(boost::property_tree::ptree& pTree,
    ...
    // tokens
    {
        ...
        for (linguistics::ConstTokenIterator itToken = pTextAnalRes.semAnal.syntGraph.tokensTree.beginToken();
            ...
            _saveIGramPossibilities(currTokenTree, "contextualInfosGramList", currToken);
...
void compareResults(std::shared_ptr<syntacticAnalysisXmlLoader::DeserializedTextResults>& pDiffResults,
    ...
    childLoop(tree, currSubTree, "syntacticAnalysisResults") {
        ...
        _getTextToResult(newResult,
                         ...
                         debugOptions,
                         langType);
        syntacticAnalysisXmlLoader::compareOneTextResults(*pDiffResults, oldResult, *newResult, pEquivalencesPtr);
        if (pPerformancesPtr != nullptr) {
            if (timeChecker)
...
void save(const std::string& pFilename,
    ...
    boost::property_tree::write_xml(
        pFilename, tree, std::locale(), boost::property_tree::xml_writer_make_settings<std::string>(' ', 2));
}
