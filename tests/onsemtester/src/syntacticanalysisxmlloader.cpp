...
void _updateDiffFromTokensTree(WhatPartAreDifferent& pCurrDiffs,
                               ...
                               bool pCheckConcepts) {
    for (linguistics::ConstTokenIterator itOldTok = pOldTokensTree.beginToken(), itNewTok = pNewTokensTree.beginToken();
         !itOldTok.atEnd() || !itNewTok.atEnd();
         ++itOldTok, ++itNewTok) {
        const linguistics::Token& oldTok = itOldTok.getToken();
        const linguistics::Token& newTok = itNewTok.getToken();
        if (oldTok.str != newTok.str || oldTok.inflWords.size() != newTok.inflWords.size()) {
            if (pWhatNeedToChecked.tokens) {
                pCurrDiffs.tokens = true;
            }
        } else {
            for (auto itOldIgram = oldTok.inflWords.begin(), itNewIgram = newTok.inflWords.begin();
                 itOldIgram != oldTok.inflWords.end();
            ...
            if (pCheckConcepts && oldTok.inflWords.front().infos.concepts != newTok.inflWords.front().infos.concepts)
...
void compareOneTextResults(DeserializedTextResults& pDiffResults,
    ...
    if (oldHighLevelResult.initialGramPossibilities.size() != newHighLevelResult.initialGramPossibilities.size()
        || oldResult.syntGraph.tokensTree.size() != newResult.syntGraph.tokensTree.size()) {
        if (pDiffResults.whatNeedToChecked.tokens)
            currDiffs.tokens = true;
    } else {
        _updateDiffFromTokensTree(currDiffs,
                                  pDiffResults.whatNeedToChecked,
    ...
    // check semExps
    if (pDiffResults.whatNeedToChecked.semExps && oldHighLevelResult.semExpStr != newHighLevelResult.semExpStr) {
        currDiffs.semExps = true;
    }

    // check allForms
    if (pDiffResults.whatNeedToChecked.allForms && oldHighLevelResult.allFormsStr != newHighLevelResult.allFormsStr) {
...
void writeBilan(DeserializedTextResults& pDiffResults) {
    ...
    for (std::size_t i = 0; i < pDiffResults.whatsDiff.size(); ++i) {
        bool aDiff = false;
        if (pDiffResults.whatsDiff[i].tokens) {
            ++tokens;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].tagsGram) {
            ++tagsGram;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].tokConcepts) {
            ++conceptsByTokens;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].syntaticGraph) {
            ++syntaticGraph;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].confidence) {
            ++confidence;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].semExps) {
            ++semExps;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].allForms) {
            ++allForms;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].sentimentsInfos) {
            ++sentInfos;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].completeness) {
            ++completeness;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].reformulations) {
            ++reformulations;
            aDiff = true;
        }
        if (pDiffResults.whatsDiff[i].input_reformulation) {
            ++input_reformulation;
            aDiff = true;
        }
        if (aDiff) {
    ...
    std::stringstream ss;
    if (pDiffResults.whatNeedToChecked.tokens) {
        ss << "Nb of different token lists:\t\t" << tokens << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different token lists:\t\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.tagsGram) {
        ss << "Nb of different grammatical tags:\t" << tagsGram << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different grammatical tags:\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.tokConcepts) {
        ss << "Nb of different concepts:\t\t" << conceptsByTokens << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different concepts:\t\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.syntaticGraph) {
        ss << "Nb of different syntactic graphs:\t" << syntaticGraph << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different syntactic graphs:\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.confidence) {
        ss << "Nb of different confidences:\t" << confidence << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different confidences:\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.semExps) {
        ss << "Nb of different semantic expressions:\t" << semExps << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different semantic expressions:\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.allForms) {
        ss << "Nb of different all forms:\t\t" << allForms << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different all forms:\t\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.sentimentsInfos) {
        ss << "Nb of different sentiments:\t" << sentInfos << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different sentiments:\t\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.completeness) {
        ss << "Nb of different completeness:\t" << completeness << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
        ss << "Nb of different completeness:\t\tdisabled\n";
    }

    if (pDiffResults.whatNeedToChecked.reformulations) {
        ss << "Nb of different reformulations:\t" << reformulations << " / " << pDiffResults.nbOfTexts << "\n";
    } else {
    ...
    if (pDiffResults.whatNeedToChecked.input_reformulation) {
        ...
        ss << "Reformulation as input:\t\t" << input_reformulation << " / " << pDiffResults.nbOfTexts << " ("
    ...
    ss << "===================================================================\n\n";
    ss << "Nb of different texts:\t\t" << nbOfDiffs << " / " << pDiffResults.nbOfTexts;
    pDiffResults.bilan = ss.str();
}

void xLoadOneIGram(DeserializedCurrWorkingStruct& pWkStruct, const boost::property_tree::ptree& pTree) {
...
void xLoadOneConcept(DeserializedCurrWorkingStruct& pWkStruct, const boost::property_tree::ptree& pTree) {
    const boost::property_tree::ptree& attributes = pTree.get_child("<xmlattr>");
    pWkStruct.currToken->inflWords.front().infos.concepts[attributes.get<std::string>("str")] =
        attributes.get<char>("coef");
}

void xLoadOneToken(DeserializedCurrWorkingStruct& pWkStruct, const boost::property_tree::ptree& pTree) {
    const boost::property_tree::ptree& attributes = pTree.get_child("<xmlattr>");
    ...
    pWkStruct.currDesText->idsToTokList[tokenPosStr] = &pWkStruct.currFinalTokensTree->tokens;
    ...
    { childLoop(pTree, currSubTree, "contextualInfosGramList") xLoadOneIGram(pWkStruct, currSubTree.second); }

    // concepts list
    { childLoop(pTree, currSubTree, "concepts") xLoadOneConcept(pWkStruct, currSubTree.second); }

    // sub tokens
    auto optChildTokens = pTree.get_child_optional("tokens");
    if (optChildTokens) {
        pWkStruct.currFinalTokensTree->tokens.back().subTokens = std::make_unique<linguistics::TokensTree>();

        linguistics::TokensTree* finalTokensTreeSAVE = pWkStruct.currFinalTokensTree;

        pWkStruct.currFinalTokensTree = &*pWkStruct.currFinalTokensTree->tokens.back().subTokens;
...
void loadOneText(SemanticAnalysisResult& pTextOutput, const boost::property_tree::ptree& pTree) {
    ...
    // tokens
    {
        childLoop(pTree, currSubTree, "tokens") xLoadOneToken(wkStruct, currSubTree.second);
