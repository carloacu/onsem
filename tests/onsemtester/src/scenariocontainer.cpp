...
void ScenarioContainer::getAllScenarios(std::set<std::string>& pScenarios, const std::string& pScenariosSpecFolder) {
...
bool ScenarioContainer::compareScenariosToReferenceResults(
    ...
    for (const auto& currScenarioName : scenarios) {
        ...
        pGetResultOfAScenario(newResult, pInputScenariosFolder + "/" + currScenarioName, semanticMemory, pLingDb);
        ...
        if (!refResultFile.is_open()) {
            ...
            while (getline(refResultFile, line)) {
                ...
                if (lastCharOfLine != '"' && lastCharOfLine != '>' && lastCharOfLine != ')' && lastCharOfLine != ']'
            ...
            if (errorStr.empty() && itScenarioNewResult != newResult.end()) {
...
void ScenarioContainer::updateScenariosResults(
    ...
    for (const auto& currScenario : scenarios) {
        ...
        pGetResultOfAScenario(scenarioResult, pInputScenariosFolder + "/" + currScenario, semMemory, pLingDb);
...
void ScenarioContainer::getFileContent(std::list<std::string>& pFileContent, const std::string& pFilename) {
...
void ScenarioContainer::load(const std::string& pScenariosSpecFolder) {
    clear();
...
void ScenarioContainer::_addNewChangedScenario(const std::string& pFilename, const std::list<std::string>& pOldCntent) {
    _scenarios.emplace_back(pFilename, pOldCntent);
    _itCurrScenario = --_scenarios.end();
}

void ScenarioContainer::getCurrScenarioFilename(std::string& pScenarioFilename) const {
    if (!_scenarios.empty())
        pScenarioFilename = _itCurrScenario->filename;
}

void ScenarioContainer::getOldContent(std::list<std::string>& pOldContent) const {
    if (!_scenarios.empty())
...
void ScenarioContainer::writeScenarioToFile(const std::string& pFilename,
                                            const std::string& pText,
                                            const std::list<std::string>& pInputLabels) {
    auto _doesLineAsToBeSaveInAScenario = [&pText, &pInputLabels](std::size_t pBeginLine) {
    ...
    while (currPos != std::string::npos) {
        if (_doesLineAsToBeSaveInAScenario(beginPos) && currPos > beginPos) {
    ...
    if (_doesLineAsToBeSaveInAScenario(beginPos) && pText.size() > beginPos) {
