...
DetailedReactionAnswer operator_react_fromSemExp(UniqueSemanticExpression pSemExp,
    ...
    auto inputSemExpInMemory =
        memoryOperation::react(reaction, pSemanticMemory, std::move(pSemExp), pLingDb, pReactionOptions);
...
bool test_knowTheNameOf(const std::string& pName,
                        ...
                        const linguistics::LinguisticDatabase& pLingDb) {
    bool res = true;
    res = _test_oneAnswerValue("quel est mon nom ?",
                               "Ton nom est " + pName + ".",
                               ...
                               __LINE__)
       && res;
    res = _test_oneAnswerValue("mon nom est " + pName + " ?",
                               "Oui, ton nom est " + pName + ".",
                               ContextualAnnotation::ANSWER,
                               pSemanticMemory,
                               pLingDb,
                               __LINE__)
       && res;
    res = _test_oneAnswerValue(
              "qui suis je", "Tu es " + pName + ".", ContextualAnnotation::ANSWER, pSemanticMemory, pLingDb, __LINE__)
       && res;
    res = _test_oneAnswerValue(pName + " c'est mon nom ?",
                               "Oui, " + pName + " est ton nom.",
                               ContextualAnnotation::ANSWER,
                               pSemanticMemory,
                               pLingDb,
                               __LINE__)
       && res;
    res = _test_oneAnswerValue("comment je m'appelle",
                               "Tu t'appelles " + pName + ".",
                               ContextualAnnotation::ANSWER,
                               pSemanticMemory,
                               pLingDb,
    ...
    res = _test_oneAnswerValue(
              "qui suis je", "Tu es " + pName + ".", ContextualAnnotation::ANSWER, pSemanticMemory, pLingDb, __LINE__)
       && res;
    res = _test_oneAnswerValue("dis mon nom", pName, ContextualAnnotation::BEHAVIOR, pSemanticMemory, pLingDb, __LINE__)
       && res;
    res = _test_oneAnswerValue("mon nom est Unautrenom ?",
                               "Non, ton nom est " + pName + ".",
                               ContextualAnnotation::ANSWER,
                               pSemanticMemory,
                               pLingDb,
                               __LINE__)
    ...
    res = _test_oneAnswerValue("what is my name",
                               "Your name is " + pName + ".",
                               ContextualAnnotation::ANSWER,
                               pSemanticMemory,
                               pLingDb,
                               __LINE__)
       && res;
    res = _test_oneAnswerValue("say my name", pName, ContextualAnnotation::BEHAVIOR, pSemanticMemory, pLingDb, __LINE__)
       && res;
    return res;
}
...
void getResultOfAScenario(std::list<std::string>& pResult,
    ...
    while (getline(scenarioSpecFile, line)) {
        ...
        if (lastCharOfLine != '"' && lastCharOfLine != '>' && lastCharOfLine != ']') {
            beforeLine += line + "\n";
            continue;
        }
        if (!beforeLine.empty()) {
        ...
        if (line.compare(0, inLabel.size(), inLabel) != 0 || line.size() == inLabel.size()
            || line[line.size() - 1] != '\"') {
            scenarioSpecFile.close();
            throw std::runtime_error("Wrong line: \"" + line + "\" for file: \"" + pFilename + "\"!");
        }

        const std::string currText = line.substr(inLabel.size(), line.size() - inLabel.size() - 1);
...
void loadOneFileInSemanticMemory(std::size_t& pNbOfInforms,
    ...
    loadDbPediaMemory(
        pNbOfInforms, properNouns, pSemanticMemory, pLingDb, pFilename, textReplacedFilePtr, pAddReferences);
