...
struct ONSEMTESTER_API WhatPartAreDifferent {
    ...
    bool areDifferent() const {
        return tokens || tagsGram || tokConcepts || syntaticGraph || confidence || semExps || allForms
            || sentimentsInfos || completeness || reformulations || input_reformulation;
    }
...
struct ONSEMTESTER_API SemanticAnalysisResult {
    SemanticAnalysisResult(const linguistics::LinguisticDatabase& pLingDb, SemanticLanguageEnum pLanguage)
    ...
    std::string getTokenId(const std::vector<linguistics::Token>& pTokenVect, linguistics::TokCstIt pItToken) const {
        for (auto endingTokIt = endingTokensToId.begin(); endingTokIt != endingTokensToId.end(); ++endingTokIt)
...
private:
    void xRecSaveEndOfLists(int& pNextEndingId, linguistics::TokensTree& pTokensTree) {
...
struct ONSEMTESTER_API DeserializedTextResults {
    DeserializedTextResults() { whatNeedToChecked.setAllTrue(); }
