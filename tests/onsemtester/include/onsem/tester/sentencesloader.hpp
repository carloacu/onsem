...
public:
    ...
    void loadFolder(const std::string& pFoldername);
    ...
    const std::vector<std::string>& getSentences() const { return fResults->diffsInputSentences; }

    const syntacticAnalysisXmlLoader::DeserializedTextResults& getOldResults() { return *fResults; }

    std::size_t getCurrIndex() { return fCurrIndex; }

private:
    std::shared_ptr<syntacticAnalysisXmlLoader::DeserializedTextResults> fResults;
    std::size_t fCurrIndex;

    void xLoadFile(const std::string& pFilename);

    void xLoadXml(const std::string& pFilename);

    void xLoadTxt(const std::string& pFilename);
};
