...
ComponentPoolAllocator::ComponentPoolAllocator(const std::filesystem::path& name)
    : fName(name.string()) {}

void ComponentPoolAllocator::serialize(const std::string& pFilename) {
...
void ComponentPoolAllocator::serializeAndClear(const std::string& pFilename) {
    std::set<FPAShift> shifts;
    xSerialize(shifts, pFilename);
    clear();
}

void ComponentPoolAllocator::deserialize(std::string& pErrorMessage, const std::string& pFilename, float pCoef) {
    clear();
...
void ComponentPoolAllocator::xSerialize(std::set<FPAShift>& pShifts, const std::string& pFilename) {
...
void ComponentPoolAllocator::xInvertShifts(std::set<FPAShift>& pNewShifts,
                                           const std::set<FPAShift>& pPrevShifts) const {
    for (std::set<FPAShift>::iterator it = pPrevShifts.begin(); it != pPrevShifts.end(); ++it) {
        FPAShift newDec;
...
void ComponentPoolAllocator::xDefragmentMemory(const std::set<FPAShift>& pShifts) {
    for (std::set<FPAShift>::const_reverse_iterator it = pShifts.rbegin(); it != pShifts.rend(); ++it) {
...
void ComponentPoolAllocator::xFragmentMemory(const std::set<FPAShift>& pShifts) {
    for (std::set<FPAShift>::const_iterator it = pShifts.begin(); it != pShifts.end(); ++it) {
