...
public:
     ...
     */
    void deserialize(std::string& pErrorMessage, const std::string& pFilename, float pCoef = 1.2f);
...
protected:
    ...
    union FPAPtr {
         ...
         */
        bool operator!=(const FPAPtr& b) const { return ptr != b.ptr; }
        /**
    ...
    struct FPAShift {
         ...
         */
        bool operator<(const FPAShift& b) const { return begin > b.begin; }
     ...
     * store the serialization.
     */
    void xSerialize(std::set<FPAShift>& pShifts, const std::string& pFilename);

    /**
     * @brief Write the memory in a stream.
     * @param pOstr The stream.
     */
    virtual void xWriteMemoryInAStream(std::ostream& pOstr) const = 0;
     ...
     * we reserve relatively to the size of the stream.
     */
    virtual void xReadMemoryFromAStream(std::set<FPAShift>& pShifts, std::istream& pIstr, float pCoef) = 0;

    /**
     * @brief Find shifts in order defragment the memory.
     * @param[out] pShifts Shifts in order defragment the memory.
     */
    virtual void xFindShiftsForDefragmentation(std::set<FPAShift>& pShifts) const = 0;

    /**
     * @brief Change the pointers according to the given shifts.
     * @param pShifts Memory pool shifts.
     */
    virtual void xChangePointers(const std::set<FPAShift>& pShifts) = 0;

    /**
     * @brief Defragment the memory according to the given shifts.
     * @param pShifts Memory pool shifts.
     */
    void xDefragmentMemory(const std::set<FPAShift>& pShifts);

    /**
     * @brief Fragment the memory according to the given shifts.
     * @param pShifts Memory pool shifts.
     */
    void xFragmentMemory(const std::set<FPAShift>& pShifts);
...
private:
     ...
     */
    void xInvertShifts(std::set<FPAShift>& pNewShifts, const std::set<FPAShift>& pPrevShifts) const;
};
