...
public:
     ...
     */
    LeafPoolAllocator(const std::string& pName, unsigned char pMemoryAlignment, get_pointers pFunc = nullptr);
     ...
     */
    virtual void accept(TreeMemoryPrettyPrinter& pV) const;
...
protected:
     ...
     */
    virtual void xWriteMemoryInAStream(std::ostream& pOstr) const;
     ...
     * of the stream.
     */
    virtual void xReadMemoryFromAStream(std::set<FPAShift>& pShifts, std::istream& pIstr, float pCoef);

    /**
     * @brief Find shifts in order defragment the memory.
     * @param[out] pShifts Shifts in order defragment the memory.
     */
    virtual void xFindShiftsForDefragmentation(std::set<FPAShift>& pShifts) const;

    /**
     * @brief Change the pointers according to the given shifts.
     * @param pShifts Memory pool shifts.
     */
    virtual void xChangePointers(const std::set<FPAShift>& pShifts);
...
private:
     ...
     */
    static std::size_t xCalculateSizeStructAligned(unsigned char pMemoryAlignment);
     ...
     */
    std::size_t xModifyAPtr(std::set<FPAShift>::const_iterator& pRefPool,
                            FPAPtr pPtr,
