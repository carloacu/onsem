...
LineEditHistoricalWrapper::LineEditHistoricalWrapper(QLineEdit* pLineEdit, QWidget* pHoldingWidget)
    : _lineEdit(pLineEdit)
    , _isActivated(false)
    , _historical()
    , _currPos(_historical.end())
...
LineEditHistoricalWrapper::LineEditHistoricalWrapper(const LineEditHistoricalWrapper& pOther)
...
LineEditHistoricalWrapper& LineEditHistoricalWrapper::operator=(const LineEditHistoricalWrapper& pOther) {
    _lineEdit = pOther._lineEdit;
    _isActivated = pOther._isActivated;
    _historical = pOther._historical;
    _currPos = _historical.end();
...
void LineEditHistoricalWrapper::getHistorical(std::vector<std::string>& pHistorical) {
    std::copy(_historical.begin(), _historical.end(), std::back_inserter(pHistorical));
    _currPos = _historical.end();
}

void LineEditHistoricalWrapper::setHistorical(const std::vector<std::string>& pHistorical) {
...
void LineEditHistoricalWrapper::addNewText(const std::string& pText, bool pCanClearOldTexts) {
    ...
    if (pCanClearOldTexts && _historical.size() > 100) {
...
void LineEditHistoricalWrapper::displayNextText() {
    if (_isActivated && !_historical.empty()) {
        ...
        if (_currPos == _historical.end() && beginText.empty()) {
            _lineEdit->clear();
        }
    }
}
...
void LineEditHistoricalWrapper::concat() {
    ...
    while (_currPos != _historical.begin()) {
        if (_currPos != _historical.end() && _currPos->compare(0, beginText_size, beginText) == 0) {
    ...
    if (!possibilities.empty()) {
        ...
        std::list<std::list<std::string>::iterator>::iterator itPossib = ++possibilities.begin();
        while (currSize > minSize && itPossib != possibilities.end()) {
...
std::size_t LineEditHistoricalWrapper::_updateMinAndMaxSize(std::size_t pCurrSize,
                                                            std::size_t pMinSize,
