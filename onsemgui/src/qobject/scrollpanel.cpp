...
void ScrollPanel::setParentWidget(QWidget* pParent) {
...
void ScrollPanel::setGeometry(int pX, int pY, int pW, int pH) {
    fScrollBar->setGeometry(pX + pW - 16, pY, 16, pH);
    fViewWidget->setGeometry(pX, pY, pW - 26, pH);
    xInitScrollBar();
}

void ScrollPanel::setContentHeight(int pContentHeight) {
...
QWidget* ScrollPanel::getContentWidget() const {
    return fContentWidget;
}

void ScrollPanel::onVerticalScrollBarValueChanged(int pValue) {
    fContentWidget->setGeometry(fContentWidget->x(), -pValue, fContentWidget->width(), fContentWidget->height());
}
