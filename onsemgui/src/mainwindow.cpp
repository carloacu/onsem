...
MainWindow::MainWindow(const std::filesystem::path& pCorpusEquivalencesFolder,
    ...
    for (std::list<SemanticLanguageEnum>::const_iterator it = languageTypes.begin(); it != languageTypes.end(); ++it)
        _ui->comboBox_AATester_languageSelection->addItem(
            QString::fromUtf8(semanticLanguageEnum_toLegacyStr(*it).c_str()));
    ...
    for (std::size_t i = 0; i < CONV_OUTPUT_TABLE_ENDFORNOCOMPILWARNING; ++i) {
        _ui->comboBox_AATester_convertionToShow->addItem(QString::fromUtf8(ConvertionOutputEnum_toStr[i].c_str()));
    }

    // fill tokenizer steps
    auto fillDebugSteps = [this](std::list<std::string>& pSteps, SemanticLanguageEnum pLanguageType) {
...
void MainWindow::onRescaleTokens() {
    ...
    fTokensPanel.setGeometry(offsetXY,
                             40 + offsetXY,
                             _ui->tab_AATester_Logs_Words->width() - 20,
...
void MainWindow::_updateCurrentLanguage(SemanticLanguageEnum pNewLanguage) {
    if (pNewLanguage != _currentLanguage) {
        ...
        auto itStepsForLanguage = fLangToTokenizerSteps.find(pNewLanguage);
        if (itStepsForLanguage != fLangToTokenizerSteps.end()) {
            for (const auto& currStep : itStepsForLanguage->second) {
                _ui->comboBox_tokenizer_endingStep->addItem(QString::fromUtf8(currStep.c_str()));
            }
        }
        if (_ui->comboBox_tokenizer_endingStep->count() > 0) {
            _ui->comboBox_tokenizer_endingStep->addItem("finish");
            _ui->comboBox_tokenizer_endingStep->setCurrentIndex(_ui->comboBox_tokenizer_endingStep->count() - 1);
...
void MainWindow::on_lineEdit_AATester_InputSentence_textChanged(const QString& arg1) {
    ...
    debugOptions.convOutput = ConvertionOutputEnum(_ui->comboBox_AATester_convertionToShow->currentIndex());
    {
        ...
        } catch (...) { debugOptions.endingStep.nbOfDebugRoundsForTokens = 1; }
...
void MainWindow::xDisplayResult(const SyntacticAnalysisResultToDisplay& pAutoAnnotToDisplay) {
    ...
    for (auto tokensIt = pAutoAnnotToDisplay.tokens.begin(); tokensIt != pAutoAnnotToDisplay.tokens.end();
         ++tokensIt, ++initGramsIt, ++finalGramsIt, ++finalConceptsIt, ++contextualInfosIt) {
        ...
        fTokens[idToken]->setGeometry(
            static_cast<int>(_leftTokens + tokensIt->first * 20), static_cast<int>(10 + idToken * 30), 150, 25);
        fGramPossiblities[idToken]->setGeometry(
            static_cast<int>(_leftGramPoss + tokensIt->first * 20), static_cast<int>(10 + idToken * 30), 200, 25);
        fFinalGramPossiblities[idToken]->setGeometry(
            static_cast<int>(_leftFinalGramPoss + tokensIt->first * 20), static_cast<int>(10 + idToken * 30), 200, 25);
        fFinalConcepts[idToken]->setGeometry(
            static_cast<int>(_leftFinalConcepts + tokensIt->first * 20), static_cast<int>(10 + idToken * 30), 200, 25);
        fContextualInfos[idToken]->setGeometry(static_cast<int>(_leftContextualInfos + tokensIt->first * 20),
                                               static_cast<int>(10 + idToken * 30),
                                               200,
                                               25);
    ...
    idToken = 0;
    std::list<std::list<std::string> >::const_iterator tagsIt =
        pAutoAnnotToDisplay.taggedTokensTagsPossibilities.begin();
    ...
    _ui->label_AATester_Logs_Performances->setText(QString::fromUtf8(pAutoAnnotToDisplay.performances.c_str()));

    _ui->textBrowser_genRep->setHtml(QString::fromUtf8(pAutoAnnotToDisplay.highLevelResults.semExpStr.c_str()));

    _ui->textBrowser_allForms->setHtml(QString::fromUtf8(pAutoAnnotToDisplay.highLevelResults.allFormsStr.c_str()));
    ...
    _ui->pushButton_addEquivalence_AATester_Logs_Reformulation->setVisible(!pAutoAnnotToDisplay.isReformulationOk);
    ...
    _showImageInACanvas(syntPngFile, *_ui->tab_AATester_Logs_SyntacticGraph, *_ui->comboBox_AATester_syntGraph);
}
...
void MainWindow::onRescaleSynthGraph() {
    ...
    if (fDispDotImage) {
        ...
        _showImageInACanvas(synthFilename, *_ui->tab_AATester_Logs_SyntacticGraph, *_ui->comboBox_AATester_syntGraph);
    }
}

void MainWindow::_showImageInACanvas(const std::string& pImagePath,
    ...
    float coefPanel = static_cast<float>(pHoldingWidget.width()) / static_cast<float>(pHoldingWidget.height());
    int x, y, w, h;
    y = 0;
...
void MainWindow::on_pushButton_AATester_PrevSentence_clicked() {
    ...
    _ui->lineEdit_AATester_InputSentence->setText(QString::fromUtf8(sentence.c_str()));
    if (!_newOrOldVersion)
        xDisplayOldResult();
}
...
void MainWindow::xDisplayOldResult() {
    SyntacticAnalysisResultToDisplay autoAnnotToDisplay;
    const auto& resToDisp = *fSentenceLoader.getOldResults().oldResultThatDiffers[fSentenceLoader.getCurrIndex()];
...
void MainWindow::on_pushButton_AATester_Logs_Compare_OldXml_clicked() {
    ...
    syntacticAnalysisXmlSaver::save(
        _corpusResultsFolder.string() + "/" + selLanguageStr + "_syntacticanalysis_results.xml",
        _getSelectedLanguageType(),
        sentencesXml.getSentences(),
        _lingDb);
}
...
void MainWindow::on_pushButton_AATester_Logs_Compare_NewXml_clicked() {
    ...
    diffResults->whatNeedToChecked.tokens = _ui->checkBox_regressiontests_tokens->isChecked();
    diffResults->whatNeedToChecked.tagsGram = _ui->checkBox_regressiontests_gramtags->isChecked();
    diffResults->whatNeedToChecked.tokConcepts = _ui->checkBox_regressiontests_concepts->isChecked();
    diffResults->whatNeedToChecked.syntaticGraph = _ui->checkBox_regressiontests_syntgraph->isChecked();
    diffResults->whatNeedToChecked.confidence = _ui->checkBox_regressiontests_confidence->isChecked();
    diffResults->whatNeedToChecked.sentimentsInfos = _ui->checkBox_regressiontests_sentimentsinfos->isChecked();
    diffResults->whatNeedToChecked.completeness = _ui->checkBox_regressiontests_completeness->isChecked();
    diffResults->whatNeedToChecked.semExps = _ui->checkBox_regressiontests_semexps->isChecked();
    diffResults->whatNeedToChecked.allForms = _ui->checkBox_regressiontests_allforms->isChecked();
    diffResults->whatNeedToChecked.reformulations = _ui->checkBox_regressiontests_reformulations->isChecked();
    diffResults->whatNeedToChecked.input_reformulation = _ui->checkBox_regressiontests_inputReformulation->isChecked();

    std::map<std::string, std::string> equivalences;
    if (diffResults->whatNeedToChecked.input_reformulation)
        _readEquivalences(equivalences, _getEquivalencesFilename());
    std::string performances;
    syntacticAnalysisXmlSaver::compareResults(
        diffResults, _getSelectedLanguageStr(), _lingDb, _corpusResultsFolder.string(), &equivalences, &performances);
...
void MainWindow::onRescaleLinguisticAnalyzerPanel() {
    _ui->comboBox_AATester_languageSelection->setGeometry(
        _ui->tab_AATester->width() - _ui->comboBox_AATester_languageSelection->width() - 10,
    ...
    _ui->lineEdit_AATester_InputSentence->setGeometry(textEditX,
                                                      _ui->lineEdit_AATester_InputSentence->y(),
                                                      textEditW,
                                                      _ui->lineEdit_AATester_InputSentence->height());
    _ui->pushButton_micro->setGeometry(textEditX + textEditW + 10,
                                       _ui->pushButton_micro->y(),
                                       _ui->pushButton_micro->width(),
                                       _ui->pushButton_micro->height());

    _ui->tabWidget_AATester_Logs->setGeometry(10,
                                              _ui->tabWidget_AATester_Logs->y(),
                                              _ui->tab_AATester->width() - 20,
                                              _ui->tab_AATester->height() - _ui->tabWidget_AATester_Logs->y() - 10);

    _ui->comboBox_AATester_convertionToShow->setGeometry(
        _ui->tabWidget_AATester_Logs->width() - _ui->comboBox_AATester_convertionToShow->width() - 40,
...
void MainWindow::onRescaleChatPanel() {
    ...
    _ui->label_chat_title->setGeometry(10, 10, _ui->tab_Chat->width(), _ui->label_chat_title->height());
    {
        ...
        _ui->textBrowser_chat_history->setGeometry(
            10, mainFrameY, _ui->tab_Chat->width() - 20, asrFrameY - mainFrameY - 20);
    }

    // speech frame
    _ui->frame_history_asr->setGeometry(10, asrFrameY, _ui->tab_Chat->width() - 20, _bottomBoxHeight);
    _ui->lineEdit_history_newText->setGeometry(
        20 + grouBoxSidesWidth, 15, _ui->frame_history_asr->width() - 40 - grouBoxSidesWidth * 2, lineEditHeight);
    _ui->widgetRight_history->setGeometry(
        _ui->frame_history_asr->width() - 10 - grouBoxSidesWidth, 0, grouBoxSidesWidth, _bottomBoxHeight);
}

void MainWindow::onRescaleChatDiagnosisPanel() {
    _ui->textBrowser_PrintMemory->setGeometry(
        10, 100, _ui->tab_chatdiagnosis->width() - 20, _ui->tab_chatdiagnosis->height() - 110);
}

void MainWindow::onRescaleGenRep() {
    _ui->textBrowser_genRep->setGeometry(
        10, 10, _ui->tabWidget_AATester_Logs->width() - 20, _ui->tabWidget_AATester_Logs->height() - 20);
}

void MainWindow::onRescaleAllForms() {
    _ui->textBrowser_allForms->setGeometry(
        10, 10, _ui->tab_AATester_Logs_AllForms->width() - 20, _ui->tab_AATester_Logs_AllForms->height() - 20);
}

void MainWindow::onRescaleSentiments() {
    _ui->textBrowser_AATester_Logs_Sentiments->setGeometry(
        10, 10, _ui->tab_AATester_Logs_Sentiments->width() - 20, _ui->tab_AATester_Logs_Sentiments->height() - 20);
}

void MainWindow::on_tabWidget_AATester_Logs_currentChanged(int index) {
    switch (index) {
        case 0: onRescaleTokens(); break;
        case 1: onRescaleSynthGraph(); break;
        case 2: onRescaleGenRep(); break;
        case 3: onRescaleAllForms(); break;
        case 5: onRescaleSentiments(); break;
    }
}
...
void MainWindow::on_pushButton_AATester_SwitchVersion_clicked() {
    ...
    if (_newOrOldVersion) {
        ...
        _ui->lineEdit_AATester_InputSentence->setText(QString::fromUtf8(currText.c_str()));
        xDisplayOldResult();
    }
}
...
void MainWindow::on_tabWidget_currentChanged(int index) {
    switch (index) {
        case 0: onRescaleLinguisticAnalyzerPanel(); break;
        case 1: onRescaleChatPanel(); break;
        case 2: onRescaleChatDiagnosisPanel(); break;
    }
}
...
void MainWindow::on_pushButton_MemoryClearer_RefreshPkg_clicked() {
    _ui->textBrowser_MemoryClearer_DisplayPkg->setText(
        QString::fromUtf8(diagnosisPrinter::diagnosis({"loadedDatabases"}, SemanticMemory(), _lingDb).c_str()));
}
...
void MainWindow::_loadSentences(bool pTxtFirstChoice, const std::string& pTextCorpusPath) {
    std::string filter =
        pTxtFirstChoice ? "Text file (*.txt);;Xml file (*.xml)" : "Xml file (*.xml);;Text file (*.txt)";
...
void MainWindow::on_actionExport_to_ldic_triggered() {
    ...
    QString fileToWrite =
        QFileDialog::getSaveFileName(this, "Export the dictionaries", QString(), firstStr, &selectedFilter);
...
void MainWindow::on_actionImport_from_ldic_triggered() {
    ...
    std::string filename =
        QFileDialog::getOpenFileName(this, "Import dictionaries", QString(), firstStr).toUtf8().constData();
    if (filename.empty())
        return;
    boost::property_tree::ptree propTree;
...
std::string MainWindow::_operator_react(ContextualAnnotation& pContextualAnnotation,
...
void MainWindow::_onNewTextSubmitted(const std::string& pText,
    ...
    _ui->textBrowser_chat_history->append(QString::fromUtf8(_inLabel.c_str()) + " \"" + QString::fromUtf8(pText.c_str())
                                          + "\"");
    ...
    auto text = _operator_react(contextualAnnotation, references, pText, textLanguage);
    if (!text.empty()) {
...
void MainWindow::_sayText(std::list<TextWithLanguage>& pTextsToSay) {
    ...
    for (auto& currTextToSay : pTextsToSay) {
        ...
        comandLine +=
            "gtts-cli \"" + currTextToSay.text + "\" --output " + outSoundFilename + " -l " + languageCode + " && ";
...
void MainWindow::on_actionSave_triggered() {
    ...
    const std::string scenarioContent = _ui->textBrowser_chat_history->document()->toPlainText().toUtf8().constData();
    if (scenarioContent.empty()) {
        QMessageBox::warning(
            this, this->tr("Nothing to save"), this->tr("The current scenario is empty."), QMessageBox::Ok);
        return;
    }
...
void MainWindow::_loadCurrScenario() {
    ...
    if (_ui->pushButton_Chat_SwitchToReference->text() == _referenceResultsStr) {
        ...
        getResultOfAScenario(
            linesToDisplay, (_inputScenariosFolder / scenarioFilename).string(), *_semMemoryPtr, _lingDb);
    }
...
void MainWindow::_appendLogs(const std::list<std::string>& pLogs) {
    for (const auto& currLog : pLogs) {
        _ui->textBrowser_chat_history->setTextColor(currLog.compare(0, _inLabel.size(), _inLabel) == 0 ? _inFontColor
                                                                                                       : _outFontColor);
...
void MainWindow::on_pushButton_History_compareResults_clicked() {
    ...
    if (_scenarioContainer.compareScenariosToReferenceResults(
            bilan, _inputScenariosFolder.string(), _outputScenariosFolder.string(), &getResultOfAScenario, _lingDb)) {
        _ui->pushButton_Chat_PrevScenario->setEnabled(true);
        _ui->pushButton_Chat_NextScenario->setEnabled(true);
        _loadCurrScenario();
    } else {
...
void MainWindow::on_pushButton_History_updateResults_clicked() {
    _scenarioContainer.updateScenariosResults(
        _inputScenariosFolder.string(), _outputScenariosFolder.string(), &getResultOfAScenario, _lingDb);
}

void MainWindow::on_pushButton_clicked() {
    _ui->textBrowser_PrintMemory->setText(QString::fromUtf8(
...
std::filesystem::path MainWindow::_getEquivalencesFilename() {
    ...
    return _corpusEquivalencesFolder / std::filesystem::path(languageStr + "_equivalences.xml");
}
...
void MainWindow::_readEquivalences(std::map<std::string, std::string>& pEquivalences,
    ...
    try {
        ...
        for (const auto& currSubTree : tree.get_child("equivalences")) {
            ...
            pEquivalences.emplace(attrs.get<std::string>("in"), attrs.get<std::string>("out"));
        }
    } catch (...) {}
}
...
void MainWindow::on_actionLoad_chat_content_triggered() {
    ...
    std::string filenameStr =
        QFileDialog::getOpenFileName(this, "Import text corpus", QString(), firstStr).toUtf8().constData();
...
void MainWindow::on_actionSave_memory_triggered() {
    ...
    QString selectedFilter;
    QString fileToWrite =
        QFileDialog::getSaveFileName(this, "Serialize the memory", QString(), firstStr, &selectedFilter);
    std::string fileToWriteStr = std::string(fileToWrite.toUtf8().constData());
    if (fileToWriteStr.empty())
        return;
...
void MainWindow::on_actionLoad_memory_triggered() {
    ...
    std::string filename =
        QFileDialog::getOpenFileName(this, "Deserialize a memory", QString(), firstStr).toUtf8().constData();
    if (filename.empty())
        return;
...
void MainWindow::on_actionload_a_smb_triggered() {
    ...
    std::string filenameStr =
        QFileDialog::getOpenFileName(this, "Load a binary memory", QString(), firstStr).toUtf8().constData();
    if (filenameStr.empty())
        return;
...
std::string MainWindow::_getAsrText(bool& pTextEnd) {
    ...
    while (getline(fin, line)) {
        ...
        } else if (line.compare(0, textBeginOfStr_size, textBeginOfStr) == 0 && line.size() > textBeginOfStr_size - 1) {
