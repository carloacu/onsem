...
void ALLingdbFlexions::xDeallocate
(ALCompositePoolAllocator& pFPAlloc)
{
  pFPAlloc.deallocate<unsigned char>(fFlexions, fNbFlexions);
  pFPAlloc.deallocate<ALLingdbFlexions>(this);
}


ALLingdbFlexions* ALLingdbFlexions::xClone
(ALCompositePoolAllocator& pFPAlloc) const
{
  ALLingdbFlexions* newFlexions = pFPAlloc.allocate<ALLingdbFlexions>(1);
...
ALLingdbFlexions::ALLingdbFlexions()
  : fNbFlexions(0),
    fFlexions(nullptr)
{
}
 ...
 ALLingdbWordForms* pWFForErrorLog)
{
  ...
  {
    fFlexions[i] = xFlexionStringToChar(pGram, pFlexions[i],
                                        pWFForErrorLog);
  }
}


void ALLingdbFlexions::xAddNewFlexions
(ALCompositePoolAllocator& pFPAlloc,
 PartOfSpeech pGram,
 const std::vector<std::string>& pFlexions,
 ALLingdbWordForms* pWFForErrorLog)
{
  ...
  for (std::size_t i = 0; i < pFlexions.size(); ++i)
  {
    char newChar = xFlexionStringToChar(pGram, pFlexions[i],
                                        pWFForErrorLog);
...
unsigned char ALLingdbFlexions::getNbFlexions
() const
{
  return fNbFlexions;
}
...
const unsigned char* ALLingdbFlexions::getMemory
() const
{
  return fFlexions;
}

void ALLingdbFlexions::writeInStream
(std::ostream& pOs,
...
void ALLingdbFlexions::xGetPointers
(std::vector<const void*>& pRes, void* pVar)
{
  pRes.emplace_back(&reinterpret_cast<ALLingdbFlexions*>
                 (pVar)->fFlexions);
}

char ALLingdbFlexions::xFlexionStringToChar
(PartOfSpeech pGram,
 ...
 ALLingdbWordForms* pWFForErrorLog) const
{
  ...
  std::cerr << partOfSpeech_toStr(pGram)
            << " doesn't allow any flexion, but found: "
            << pFlexion << " for the word: "
            << pWFForErrorLog->getMeaning()->getLemma()->getWord()
            << std::endl;
  return 0;
}
...
char ALLingdbFlexions::xFlexionStringToCharForNoun
...
char ALLingdbFlexions::xFlexionStringToCharForPronoun
(const std::string& pFlexion) const
{
  std::size_t i = 0;
  char res = 0;
...
void ALLingdbFlexions::fillVerbConjugaison
 ...
 char pFrequency) const
{
  for (unsigned char i = 0; i < fNbFlexions; ++i)
  {
    ...
    {
      xFillVerbTense(pVerbConjugaison.present,
                     fFlexions[i], pTriNode, pFrequency);
    }
    else if ((fFlexions[i] & 0x0F) == 0x01) // imperfect
    {
      xFillVerbTense(pVerbConjugaison.imperfect,
                     fFlexions[i], pTriNode, pFrequency);
    }
    else if ((fFlexions[i] & 0x0F) == 0x02) // present subjunctive
    {
      xFillVerbTense(pVerbConjugaison.presentSubjunctive,
                     fFlexions[i], pTriNode, pFrequency);
    }
    else if ((fFlexions[i] & 0x0F) == 0x04) // imperative
    {
      xFillImperativeVerbTense(pVerbConjugaison.imperative,
                               fFlexions[i], pTriNode, pFrequency);
    }
    else if ((fFlexions[i] & 0x0F) == 0x05) // conditional
    {
      xFillVerbTense(pVerbConjugaison.conditional,
                     fFlexions[i], pTriNode, pFrequency);
    }
    ...
    {
      xFillVerbNumberAndGenderConjugaison(pVerbConjugaison.pastParticiple,
                                          fFlexions[i], pTriNode, pFrequency);
    }
    else if ((fFlexions[i] & 0x0F) == 0x0A) // future
    {
      xFillVerbTense(pVerbConjugaison.future,
                     fFlexions[i], pTriNode, pFrequency);
    }
  }
}
...
void ALLingdbFlexions::xFillVerbNumberAndGenderConjugaison
...
void ALLingdbFlexions::fillNounConjugaison
...
void ALLingdbFlexions::xFillImperativeVerbTense
...
void ALLingdbFlexions::xFillVerbTense
(std::vector<WordLinkForConj>& pVerbTense,
 char pCurrFlexion,
 const ALLingdbDynamicTrieNode* pTriNode,
 char pFrequency) const
...
bool ALLingdbFlexions::replaceInfinitiveByImperative
() const
{
  for (unsigned char i = 0; i < fNbFlexions; ++i)
  {
...
char ALLingdbFlexions::xFlexionStringToCharForVerb
(const std::string& pFlexion) const
{
  unsigned char res = 0;
