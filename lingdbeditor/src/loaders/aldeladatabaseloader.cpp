...
{
  ALLingdbConcept* conceptPtr = pWords.getConcept(pConcept);
  ...
  for (const auto& eltForALemma : pLemmaAndGramThatPointsToTheConcept)
  {
    ...
    {
      ALLingdbMeaning* meaning = pWords.getMeaning(eltForALemma.first, eltForAGram);
      if (meaning != nullptr)
...
ALDelaDatabaseLoader::ALDelaDatabaseLoader
(bool pSaveWords)
...
void ALDelaDatabaseLoader::simplifyDelaFile
...
void ALDelaDatabaseLoader::merge
...
void ALDelaDatabaseLoader::xFillNewWordInfos
...
std::size_t ALDelaDatabaseLoader::xGetStringUntilAChar
...
void ALDelaDatabaseLoader::xReadWordInfo
(NewWordInfos& pNewWord,
...
PartOfSpeech ALDelaDatabaseLoader::xReadGram
