...
int ALXmlDatabaseLoader::getVersion
 ...
 const ALLingdbTree& pLingdbTree)
{
  ...
  childLoop(tree, currSubTree, "linguisticdatabase")
  {
    ...
    {
      ALLingdbMeaning* meaningPtr = xExtractMeaning(subTree, pLingDatabase);
      if (meaningPtr != nullptr)
      {
        std::list<std::pair<ALLingdbMeaning*, LinkedMeaningDirection> > linkedMeanings;
        for(const auto& currWordTree : subTree)
        {
          if (currWordTree.first == "<xmlattr>")
            continue;
          ...
          {
            ALLingdbMeaning* subMeaning = xExtractMeaning(currWordTree.second, pLingDatabase);
    ...
    else if (currSubTree.first == "animationTag")
    {
      ...
      for(const auto& currAnimTree : subTree)
      {
        ...
        else if (currAnimTree.first == "linkedMeaning")
        {
          ...
          ALLingdbMeaning* meaningPtr = pLingDatabase.getMeaning(lemma, lemmaPartOfSpeech);
          if (meaningPtr == nullptr)
          {
    ...
    else if (currSubTree.first == "concept")
    {
      ...
      ALLingdbConcept* concept = pLingDatabase.getConcept(conceptName);
      if (concept != nullptr)
      {
        for(const auto& currCptTree : subTree)
        {
          ...
          if (currCptTree.first == "linkedMeaning")
          {
            ...
            if (inputGramStr == "*")
            {
              ALLingdbDynamicTrieNode* pointerEndOfWord = pLingDatabase.getPointerToEndOfWord(lemma);
              ...
              if (pointerEndOfWord != nullptr)
              {
                const ForwardPtrList<ALLingdbMeaning>* itMeanings = pointerEndOfWord->getMeaningsAtThisLemme();
    ...
    else if (currSubTree.first == "oppositeconcepts")
    {
      ...
      ALLingdbConcept* cptPtr1 = pLingDatabase.getConcept(conceptName1);
      if (cptPtr1 == nullptr)
      {
      ...
      else
      {
        ...
        ALLingdbConcept* cptPtr2 = pLingDatabase.getConcept(conceptName2);
        if (cptPtr2 == nullptr)
        {
    ...
    else if (currSubTree.first == "genericRule")
    {
      ...
      {
        ALLingdbQuestionWords* qWords = pLingDatabase.xGetFPAlloc().allocate<ALLingdbQuestionWords>(1);
        qWords->init();
...
ALLingdbMeaning* ALXmlDatabaseLoader::xExtractMeaning
...
/*
bool ALXmlDatabaseLoader::xLoad
...
void ALXmlDatabaseLoader::xSplit
