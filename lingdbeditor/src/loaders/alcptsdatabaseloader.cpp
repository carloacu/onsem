 ...
 LinguisticIntermediaryDatabase& pLingdb)
{
  ...
  std::string line;
  ALLingdbConcept const* currConcept = nullptr;
  std::string currConceptStr;
  ALLingdbConcept const* contraryConcept = nullptr;
  ...
  while (getline(infile, line))
  {
    ...
    if (line[0] == '#')
    {
      ...
      if (gramSepPos != std::string::npos)
      {
        ...
        {
          meaningToHandle->addLinkToConcept(pLingdb, currConcept, currConceptStr,
                                            relToConcept, true);
        }
      }
      else
      {
        ...
        if (relToConcept == 0)
        {
          _fillConcept(pLingdb, word, currConcept, currConceptStr,
                       0, pFilename, line);
          _fillConcept(pLingdb, word, contraryConcept, contraryConceptStr,
                       0, pFilename, line);
        }
        else if (relToConcept > 0)
        {
          _fillConcept(pLingdb, word, currConcept, currConceptStr,
                       relToConcept, pFilename, line);
        }
        else if (relToConcept < 0)
        {
          _fillConcept(pLingdb, word, contraryConcept, contraryConceptStr,
                       static_cast<char>(-relToConcept), pFilename, line);
        }
      }
    }
    else if (line[0] == '[')
    {
      ...
      if (line.size() > conceptLabel.size() &&
          line.compare(0, conceptLabel.size(), conceptLabel) == 0)
      {
        currConceptStr = line.substr(conceptLabel.size(),
                                     line.size() - conceptLabel.size());
      ...
      {
        contraryConceptStr = line.substr(contraryConceptLabel.size(),
                                         line.size() - conceptLabel.size());
      ...
      {
        std::cerr << "the line \"" << line << "\" has to begin with \"[concept : \" and be followed by a concept name" << std::endl;
      }
    }
    else
    {
...
void ALCptsDatabaseLoader::_fillConcept
(LinguisticIntermediaryDatabase& pLingdb,
 ...
 const std::string& pLine)
{
  ...
  {
    std::cerr << "Error: in " << pFilename
              << " the line: " << pLine << " has no associated concept" << std::endl;
    return;
  }
  ALLingdbDynamicTrieNode* ptrEndOfWord = pLingdb.getPointerToEndOfWord(pWord);
  if (ptrEndOfWord != nullptr)
  {
    const ForwardPtrList<ALLingdbWordForms>* itWordForms = ptrEndOfWord->getWordForms();
    while (itWordForms != nullptr)
    {
      itWordForms->elt->getMeaning()->addLinkToConcept(pLingdb, pNewConcept, pNewConceptStr,
                                                       pRelatedToConcept, true);
