...
void ALBinaryDatabaseDicoSaver::save
 ...
 const LinguisticIntermediaryDatabase& pLingDatabase) const
{
  ...
  binarymasks::Ptr endMemory = xAddMeanings(pMeaningsPtr, conceptToMeanings,
                                          pConceptsOffsets, alloc, mem, mem);
  ...
  endMemory = xCreateRootNode(treeCrWorkState, alloc.first<ALLingdbDynamicTrieNode>(),
                              endMemory, endMemory);
  ...
  {
    xWriteSynthesizerDb(maxSize, beginMeanings, conceptToMeanings,
                        pConceptsOffsets, pMeaningsPtr, nodesPtr,
  ...
  // ------------------------
  endMemory.pchar = xWriteRuleWithMeanings(endMemory.pchar,
                                           pMeaningsPtr, pLingDatabase,
                                           pLingDatabase.getQuestionWords(), 1);
...
void ALBinaryDatabaseDicoSaver::xSaveAnimations
...
void ALBinaryDatabaseDicoSaver::xWriteLemmeOfMeanings
 ...
 const std::map<ALLingdbDynamicTrieNode const*, int>& pNodesPtr) const
{
  ...
  {
    ALLingdbDynamicTrieNode* lemmaNode = xGetMeaningLemma(*currMeaning.first);
    auto* meaningPtr = pBeginMeaning.pchar + currMeaning.second * 4;
    std::map<ALLingdbDynamicTrieNode const*, int>::const_iterator
        itNode = pNodesPtr.find(lemmaNode);
...
ALLingdbDynamicTrieNode* ALBinaryDatabaseDicoSaver::xGetMeaningLemma
(const ALLingdbMeaning& pMeaning) const
{
  ALLingdbDynamicTrieNode* lemmaNode = pMeaning.getLemma();
...
void ALBinaryDatabaseDicoSaver::xWriteSynthesizerDb
(std::size_t pMaxSize,
 binarymasks::Ptr pBeginMeaning,
 ...
 SemanticLanguageEnum pLangType) const
{
  ...
  ALLingdbMeaningToWords meaningToWords;
  std::map<const ALLingdbMeaning*, VerbConjugaison> verbConjugaison;
  std::map<const ALLingdbMeaning*, NounAdjConjugaison> nounConjugaison;
  ...
  std::map<const ALLingdbMeaning*, int> meaningToConj;
  for (const auto& currMeaning : pMeaningsPtr)
  {
    ...
    if (currMeaning.first->getLemma()->getDatasIfItsAMultiMeaningNode() == nullptr)
    {
      ...
      if (currPartOfSpeech == PartOfSpeech::VERB || currPartOfSpeech == PartOfSpeech::AUX)
      {
        ...
        {
          endMemory = xWriteNodeRef(endMemory, pNodesPtr, conj.pastParticiple.masculineSingular.node); // 22
      ...
      else if (partOfSpeech_isNominal(currPartOfSpeech) || currPartOfSpeech == PartOfSpeech::ADJECTIVE)
      {
        ...
        if (pLangType == SemanticLanguageEnum::ENGLISH &&
            currPartOfSpeech == PartOfSpeech::ADJECTIVE)
        {
  ...
  {
    ALLingdbMultiMeaningsNode* multiNodeInfos =
        currMeaningPtr.first->getLemma()->getDatasIfItsAMultiMeaningNode();
    const ALLingdbMeaning* rootMeaning = multiNodeInfos == nullptr ?
          currMeaningPtr.first : multiNodeInfos->getRootMeaning();
  ...
  endMemory = xAddConceptsToMeanings(beginMeaningsFromConcepts,
                                     pConceptToMeanings, pConceptsOffsets,
                                     pMeaningsPtr, endMemory);
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xAddConceptsToMeanings
(binarymasks::Ptr& pBeginMeaningsFromConcepts,
 const std::map<const ALLingdbConcept*,
 std::set<MeaningAndConfidence> >& pConceptToMeanings,
 const std::map<std::string, ConceptsBinMem>& pConceptsOffsets,
 const std::map<const ALLingdbMeaning*, int>& pMeaningsPtr,
 binarymasks::Ptr pEndMemory) const
{
  ...
  {
    const ALLingdbConcept* currCptPtr = currCptToMn.first;
    pEndMemory = xWriteMeanings(*currCptPtr, currCptToMn.second, pConceptsOffsets,
                                pMeaningsPtr, begCptToMns,
                                pBeginMeaningsFromConcepts, pEndMemory);
  }
  return pEndMemory;
}
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xWriteMeanings
 ...
 binarymasks::Ptr pEndMemory) const
{
  ...
  binarysaver::writeChar(pEndMemory.pchar++,
                         static_cast<char>(pMeaningsAndConf.size()));
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xWriteVerbConj
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xWriteNodeRef
(binarymasks::Ptr pEndMemory,
 const std::map<ALLingdbDynamicTrieNode const*, int>& pNodesPtr,
 ALLingdbDynamicTrieNode const* pNode) const
{
  ...
  {
    std::map<ALLingdbDynamicTrieNode const*, int>::const_iterator itNode = pNodesPtr.find(pNode);
    if (itNode == pNodesPtr.end())
    {
      binarysaver::writeInt(pEndMemory.pchar, 0);
    }
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xAddAnimationsTags
(int& pNbAnimTags,
 const std::map<std::string, ConceptsBinMem>& pConceptsOffsets,
 const std::map<const ALLingdbMeaning*, int>& pMeaningsPtr,
...
{
  pNbAnimTags = 0;
  ALLingdbAnimationsTag* currAnimTag = pFPAlloc.first<ALLingdbAnimationsTag>();
  while (currAnimTag != nullptr)
  {
    ...
    const ForwardPtrList<ALLingdbLinkToAConcept>* currLkConcept = currAnimTag->getLinksToConcept();
    ...
    currAnimTag = pFPAlloc.next<ALLingdbAnimationsTag>(currAnimTag);
  }
  return pEndMemory;
}
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xAddMeanings
(std::map<const ALLingdbMeaning*, int>& pMeaningsPtr,
...
{
  std::map<ALLingdbMeaning*, std::list<binarymasks::Ptr> > meaningsPointersToWrite;
  ...
  ALLingdbMeaning* meaning = pFPAlloc.first<ALLingdbMeaning>();
  while (meaning != nullptr)
  {
    ...
    ALLingdbDynamicTrieNode* lemmaNode = meaning->getLemma();

    // write number of gathering meanings
    std::list<ALLingdbDynamicTrieNode*> gatheringMeanings;
    const ForwardPtrList<ALLingdbDynamicTrieNode>* multiMeaningsNodes = lemmaNode->getMultiMeaningsNodes();
    while (multiMeaningsNodes != nullptr)
    {
      ALLingdbMeaning* rootMeaning =
          multiMeaningsNodes->elt->getDatasIfItsAMultiMeaningNode()->getRootMeaning();
    ...
    unsigned char gatheringMeanings_size =
        static_cast<unsigned char>(gatheringMeanings.size());
    ...
    const ForwardPtrList<ALLingdbNodeLinkedMeaning>* linkedMeanings = nullptr;
    {
      ALLingdbMultiMeaningsNode* gatherMeaningInfos = lemmaNode->getDatasIfItsAMultiMeaningNode();
    ...
    std::vector<const ALLingdbLinkToAConcept*> linkToConcepts;
    ...
    for (std::list<char>::const_iterator itContInfo = contextInfos.begin();
         itContInfo != contextInfos.end(); ++itContInfo)
    {
    ...
    {
      const ALLingdbConcept* concept = linkToConcepts[iLkToCpt]->getConcept();
      std::string conceptStr = concept->getName()->toStr();
      char relatedToConcept = linkToConcepts[iLkToCpt]->getRelatedToConcept();

      if (!ALLingdbConcept::conceptNameFinishWithAStar(conceptStr))
      {
        pConceptToMeanings[concept].insert
            (MeaningAndConfidence(meaning, relatedToConcept));
      }
      std::map<std::string, ConceptsBinMem>::const_iterator
          itLink = pConceptsOffsets.find(conceptStr);
    ...
    for (const ALLingdbDynamicTrieNode* multiMeaning : gatheringMeanings)
    {
    ...
    meaning = pFPAlloc.next<ALLingdbMeaning>(meaning);
  }
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::xAddSomeFlexions
(std::map<std::string, unsigned char>& pFlexionsPtr,
 const ALCompositePoolAllocator& pFPAlloc,
 const binarymasks::Ptr pBeginMemory, binarymasks::Ptr pEndMemory) const
{
  ALLingdbFlexions* flexion = pFPAlloc.first<ALLingdbFlexions>();
  while (flexion != nullptr)
  {
    assert(flexion->getNbFlexions() > 0);
    std::string strFlexion(reinterpret_cast<const char*>
                           (flexion->getMemory()), flexion->getNbFlexions());
    ...
    flexion = pFPAlloc.next<ALLingdbFlexions>(flexion);
  }
  return pEndMemory;
}
...
binarymasks::Ptr ALBinaryDatabaseDicoSaver::TreeCreationWorkState::printEndOfANode
 ...
 binarymasks::Ptr pEndMemory)
{
  ...
  std::list<std::pair<ALLingdbWordForms*, ALLingdbMeaning*> > wordFromsOrMeanings;
  ...
  {
    ALLingdbWordForms* wordForm = wfOrMeaning.first;
    ALLingdbMeaning* currMeaning = (wfOrMeaning.second != nullptr || wordForm == nullptr) ?
          wfOrMeaning.second : wordForm->getMeaning();
...
void ALBinaryDatabaseDicoSaver::xWriteNbWithSpaces
