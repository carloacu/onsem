 ...
 const boost::filesystem::path& pOutFolder) const
{
  ...
  for (std::list<ALWlksDatabaseLoader::ALWlksDatabaseLoader_TradSpec>::const_iterator
       ...
       it != pTrads.tradSpecs.end(); ++it)
  {
    ...
    binarymasks::Ptr endMemory = xCreateRootNode(treeTradWS,
                                               treeOfWords.getFPAlloc().first<ALLingdbDynamicTrieNode>(),
                                               mem, mem);
...
const std::map<const ALLingdbMeaning*, int>& ALBinaryTradSaver::xGetMeaningsPtrOfALang
(const std::map<SemanticLanguageEnum, std::map<const ALLingdbMeaning*, int> >& pLangToMeaningsPtr,
...
binarymasks::Ptr ALBinaryTradSaver::TreeTradWorkState::printEndOfANode
 ...
 binarymasks::Ptr pEndMemory)
{
  ...
  // for each word form
  const ForwardPtrList<ALLingdbWordForms>* wfl = pNode->getWordForms();
  while (wfl != nullptr)
  {
    ALLingdbMeaning* inMeaning = lingDb.getMeaning(wfl->elt->getMeaning()->getLemma()->getWord(),
    ...
    std::map<ALLingdbMeaning*, std::set<MeaningAndConfidence> >::const_iterator
